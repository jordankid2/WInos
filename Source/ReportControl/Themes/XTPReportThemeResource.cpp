// XTPReportThemeResource.cpp : implementation of the CXTPReportThemeResource class.
//
...
/////////////////////////////////////////////////////////////////////////////
// class: CXTPReportThemeResource
/////////////////////////////////////////////////////////////////////////////

CXTPReportThemeResource::CXTPReportThemeResource()
{
	m_columnStyle = xtpReportColumnResource;
}

void CXTPReportThemeResource::RefreshMetrics()
{
	...
	else
	{
		...
		if (pImages->IsValid())
		{
			m_grcGradientColumn.SetStandardValue(pImages->GetImageColor(_T("ReportControl"), _T("NormalColumnLight"), RGB(252, 253,255)),
				pImages->GetImageColor(_T("ReportControl"), _T("NormalColumnDark"), RGB(196, 221, 255)));
			m_grcGradientColumnPushed.SetStandardValue(pImages->GetImageColor(_T("ReportControl"), _T("PressedColumnLight"), RGB(196, 221, 255)),
				pImages->GetImageColor(_T("ReportControl"), _T("PressedColumnDark"), RGB(252, 253, 255)));

			m_clrGradientColumnShadow.SetStandardValue(pImages->GetImageColor(_T("ReportControl"), _T("ColumnShadow"), RGB(101, 147, 207)));
			m_clrGradientColumnSeparator.SetStandardValue(pImages->GetImageColor(_T("ReportControl"), _T("ColumnSeparator"), RGB(154, 198, 255)));
			m_clrGroupBoxBack.SetStandardValue(pImages->GetImageColor(_T("ReportControl"), _T("GroupBoxBackground"), RGB(227, 239, 255)));
			m_crlNoGroupByText.SetStandardValue(pImages->GetImageColor(_T("ReportControl"), _T("GroupBoxTextColor"), RGB(50, 75, 44)));
			m_clrGroupShadeBack.SetStandardValue(pImages->GetImageColor(_T("ReportControl"), _T("GroupShadeBackground"), RGB(190, 218, 251)));
			m_clrGroupShadeBorder.SetStandardValue(pImages->GetImageColor(_T("ReportControl"), _T("GroupShadeBorder"), RGB(111, 157, 217)));
			m_clrIndentControl.SetStandardValue(pImages->GetImageColor(_T("ReportControl"), _T("GroupShadeIndent"), RGB(253, 238, 201)));
...
CSize CXTPReportThemeResource::DrawCollapsedBitmap(CDC* pDC, const CXTPReportRow* pRow, CRect& rcBitmap)
{
	...
	{
		return CXTPReportPaintManager::DrawCollapsedBitmap(pDC, pRow, rcBitmap);
	}
}

void CXTPReportThemeResource::DrawNoGroupByText(CDC* pDC, CRect rcItem)
{
	ASSERT(m_pControl);
	if (!m_pControl)
		return;
	...
	if (m_pControl->GetMarkupContext())
	{
		...
		if (m_pMarkupUIElement_NoGroupBy)
		{
			...
			XTPMarkupSetDefaultFont(m_pControl->GetMarkupContext(), m_xtpFontCaption, m_crlNoGroupByText);
...
void CXTPReportThemeResource::FillFooter(CDC* pDC, CRect rcFooter)
{
	...
	DrawHorizontalLine(pDC, rcFooter.left, rcFooter.top + XTP_DPI_Y(1), rcFooter.Width(), m_clrGradientColumnShadow);
	DrawHorizontalLine(pDC, rcFooter.left, rcFooter.top + XTP_DPI_Y(2), rcFooter.Width(), MixColor(m_grcGradientColumn.clrDark, m_clrGradientColumnShadow, 0.25));
	DrawHorizontalLine(pDC, rcFooter.left, rcFooter.top + XTP_DPI_Y(3), rcFooter.Width(), MixColor(m_grcGradientColumn.clrDark, m_clrGradientColumnShadow, 0.1));

	DrawHorizontalLine(pDC, rcFooter.left, rcFooter.bottom - XTP_DPI_Y(1), rcFooter.Width(), m_clrGradientColumnShadow);
}

void CXTPReportThemeResource::DrawTriangle(CDC* pDC, CRect rcTriangle, BOOL bToDown, int nShadowWidth, BOOL /*bColumnPressed*/)
{
	CRect rcTRect;
	...
	if (rcTriangle.Width() > XTP_DPI_X(10))
	{
		...
		if (bToDown)
		{
			CXTPDrawHelpers::Triangle(pDC, CPoint(pt.x - XTP_DPI_X(4), pt.y - XTP_DPI_Y(2)),
				CPoint(pt.x, pt.y + XTP_DPI_Y(2)), CPoint(pt.x + XTP_DPI_X(4), pt.y - XTP_DPI_Y(2)), m_clrGradientColumnSeparator);
		}
		else
		{
			CXTPDrawHelpers::Triangle(pDC, CPoint(pt.x - XTP_DPI_X(4), pt.y + XTP_DPI_Y(2)),
...
void CXTPReportThemeResource::DrawPlainColumnBackground(CDC* pDC, CRect rcColumn)
{
...
void CXTPReportThemeResource::DrawColumnBackground(CDC* pDC, CXTPReportColumn* pColumn, CRect rcColumn, BOOL& bColumnPressed, BOOL& /*bDraggingFromHeader*/, CXTPReportHeader* /*pHeader*/)
{
	...
	DrawHorizontalLine(pDC, rcColumn.left, rcColumn.bottom - XTP_DPI_Y(1), rcColumn.Width(), m_clrGradientColumnShadow);

	if (pColumn->GetDrawHeaderDivider())
		DrawVerticalLine(pDC, rcColumn.right - XTP_DPI_X(1), rcColumn.top + XTP_DPI_Y(2), rcColumn.Height() - XTP_DPI_Y(2) - XTP_DPI_Y(4), m_clrGradientColumnSeparator);
...
void CXTPReportThemeResource::DrawColumnFooterDivider(CDC* pDC, CXTPReportColumn* pColumn, CRect rcColumn)
{
	if (pColumn->GetDrawFooterDivider())
	{
		...
		DrawVerticalLine(pDC, rcColumn.right - XTP_DPI_X(1), rcColumn.top + XTP_DPI_Y(5), rcColumn.Height() - XTP_DPI_Y(9), m_clrGradientColumnSeparator);
	}
}

void CXTPReportThemeResource::DrawColumnText(CDC* pDC, CRect rcText, CString strCaption, int nHeaderAlignment, BOOL bIsHeader, BOOL bColumnPressed)
{
...
void CXTPReportThemeResource::DrawSectionDivider(
	...
	CXTPPaintManagerColor           clrDivider)
{
	if (dividerStyle == xtpReportFixedRowsDividerOutlook)
	{
		...
		DrawHorizontalLine(pDC, rc.left, rc.bottom - XTP_DPI_Y(1), rc.Width(), m_clrGradientColumnSeparator);
	}
	else
	{
