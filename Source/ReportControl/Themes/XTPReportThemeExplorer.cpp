// XTPReportThemeExplorer.cpp : implementation of the CXTPReportPaintManager class.
//
...
/////////////////////////////////////////////////////////////////////////////
// class: CXTPReportThemeExplorer
/////////////////////////////////////////////////////////////////////////////

CXTPReportThemeExplorer::CXTPReportThemeExplorer()
{
	m_columnStyle = xtpReportColumnExplorer;
}

void CXTPReportThemeExplorer::DrawTriangle(CDC* pDC, CRect rcTriangle, BOOL bToDown, int nShadowWidth)
{
	CRect rcTRect;
...
CSize CXTPReportThemeExplorer::DrawGlyph(CDC *pDC, CRect rcColumn, int iGlyph)
{
	CSize size(0,0);

	if (XTPReportGlyphIsCheckbox(iGlyph) && m_themeButton->IsAppThemeActive())
	{
		m_themeButton->GetThemePartSize(NULL, BP_CHECKBOX, 0, NULL, TS_TRUE, &size);

		if (pDC)
		{
			CRect rectCheckBox(CPoint(rcColumn.left + XTP_DPI_X(1), (rcColumn.top + rcColumn.bottom - size.cy) /2), size);
...
BOOL CXTPReportThemeExplorer::IsColumHotTrackingEnabled() const
{
	return m_bHotTracking && m_themeHeader->IsAppThemeActive();
}

void CXTPReportThemeExplorer::DrawColumnBackground(CDC* pDC, CXTPReportColumn* pColumn, CRect rcColumn, BOOL& bColumnPressed, BOOL& /*bDraggingFromHeader*/, CXTPReportHeader* pHeader)
{
	...
	else
	{
		...
		else
		{
			...
			{
				DrawVerticalLine(pDC, rcColumn.right - XTP_DPI_X(1), rcColumn.top, rcColumn.Height() - XTP_DPI_Y(1), m_clrHeaderControl);
				DrawVerticalLine(pDC, rcColumn.right, rcColumn.top, rcColumn.Height() - XTP_DPI_Y(1), m_clrHeaderControl);
			...
			{
				DrawVerticalLine(pDC, rcColumn.left - XTP_DPI_X(1), rcColumn.top, rcColumn.Height() - XTP_DPI_Y(1), m_clrHeaderControl);
				pDC->SetPixel(rcColumn.left - XTP_DPI_X(1), rcColumn.bottom - XTP_DPI_Y(1), m_clrControlDark);
			}
		}
	}
...
void CXTPReportThemeExplorer::DrawSectionDivider(
