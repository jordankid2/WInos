...
public:
	...
	// Parameters:
	//     nIndex - index to insertion expanded rows. Used in the recursion.
	//     pRow   - A row to expand.
	//     bUpdateSelection - BOOL flag which allow to update selection of rows. 
	//             Prevents multiple invocation of CXTPReportSelectedRows::_OnExpanded
	//             in recursion through InsertRow.
	//-----------------------------------------------------------------------
	virtual int _DoExpand(int nIndex, CXTPReportRow *pRow, BOOL bUpdateSelection);
