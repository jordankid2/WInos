...
IMPLEMENT_SERIAL(CXTPReportRecord, CXTPCmdTarget, VERSIONABLE_SCHEMA | _XTP_SCHEMA_CURRENT)
...
void CXTPReportRecord::SetReportControl(CXTPReportControl* pReportControl)
{
...
BEGIN_DISPATCH_MAP(CXTPReportRecord, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPReportRecord, CXTPCmdTarget)
	INTERFACE_PART(CXTPReportRecord, XTPDIID_IReportRecord, Dispatch)
	//INTERFACE_PART(CXTPReportRecord, IID_IEnumVARIANT, EnumVARIANT)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPReportRecord, XTPDIID_IReportRecord)

CXTPReportRecord* AFX_CDECL CXTPReportRecord::FromDispatch(LPDISPATCH pDisp)
{
	CXTPReportRecord* pInst = NULL;
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPReportRecord, CXTPReportRecord::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
...
void CXTPReportRecord::OleAddEx(LPDISPATCH lpDisp)
{
	CXTPReportRecordItem* pItem = DYNAMIC_DOWNCAST(CXTPReportRecordItem, 
		CXTPReportRecordItem::FromIDispatchSafe(lpDisp));
	if (pItem)
	{
		AddItem(pItem);
