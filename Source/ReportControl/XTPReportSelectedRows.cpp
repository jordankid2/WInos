// XTPReportSelectedRows.cpp : implementation of the CXTPReportSelectedRows class.
//
...
void CXTPReportSelectedRows::DeselectBlock(int nIndexB, int nIndexE, BOOL bCollapse)
{
...
void CXTPReportSelectedRows::_OnCollapsed(int nIndexB, int nCount)
{
	ASSERT(nCount > 0);
...
void CXTPReportSelectedRows::AddBlock(int ib, int ie, BOOL bDeselectIfSelected)
{
	SwapIfNeed(ib, ie);
...
void CXTPReportSelectedRows::SelectBlock(int nBlockBegin, int nEnd, BOOL bControlKey)
{
	...
	BOOL bHandleGroups = bSkipGroupFocus && m_pControl->GetColumns()->GetGroupsOrder()->GetCount() != 0;
	...
	if (bHandleGroups)
	{
		int nRowBlockBegin = m_nRowBlockBegin;	// save m_nRowBlockBegin because it is (-1) after Add(pRow)
		for (int i = nBegin; i <= nEnd; i++)
		{
			CXTPReportRow* pRow = pRows->GetAt(i);
			if (pRow && (!bSkipGroupFocus || !pRow->IsGroupRow() || !pRow->IsExpanded() || (i == nBegin) || (i == nEnd)))
			{
				Add(pRow);
			}
		}
...
void CXTPReportSelectedRows::_NotifyStateChanged(int nBegin, int nEnd)
{
	if (m_pControl)
	{
		...
		m_pControl->SendNotifyMessage(XTP_NM_REPORT_STATECHANGED, &nm.hdr);
	}
}
...
BOOL CXTPReportSelectedRows::_NotifySelChanging(XTPReportSelectionChangeType nType, CXTPReportRow *pRow)
{
	...
	if (NULL != m_pControl)
	{
		...
		LRESULT lResult = m_pControl->SendNotifyMessage(XTP_NM_REPORT_SELCHANGING, &nm.hdr);
...
BEGIN_DISPATCH_MAP(CXTPReportSelectedRows, CXTPCmdTarget)
	DISP_FUNCTION_ID(CXTPReportSelectedRows, "Count", dispidCount, OleGetItemCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPReportSelectedRows, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
	DISP_FUNCTION_ID(CXTPReportSelectedRows, "Add", 2, OleAdd, VT_EMPTY, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPReportSelectedRows, "DeleteAll", 5, OleDeleteAll, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CXTPReportSelectedRows, "Row", DISPID_VALUE, OleGetItem, VT_DISPATCH, VTS_I4)
END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPReportSelectedRows, CXTPCmdTarget)
	INTERFACE_PART(CXTPReportSelectedRows, XTPDIID_IReportSelectedRows, Dispatch)
	//INTERFACE_PART(CXTPReportSelectedRows, IID_IEnumVARIANT, EnumVARIANT)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPReportSelectedRows, XTPDIID_IReportSelectedRows)

void CXTPReportSelectedRows::OleAdd(LPDISPATCH lpRow)
{
	CXTPReportRow* pRow = CXTPReportRow::FromDispatch(lpRow);

	Add(pRow);
}

void CXTPReportSelectedRows::OleDeleteAll()
{
	Clear(GetNotifyOnClear());
}

int CXTPReportSelectedRows::OleGetItemCount()
{
	return GetCount();
}

LPDISPATCH CXTPReportSelectedRows::OleGetItem(long nIndex)
{
	for (int i = 0; i < (int) m_arrSelectedBlocks.GetSize(); i++)
	{
		int nCount = m_arrSelectedBlocks[i].nIndexEnd - m_arrSelectedBlocks[i].nIndexBegin;
...
IMPLEMENT_ENUM_VARIANTLIST(CXTPReportSelectedRows)

BOOL CXTPReportSelectedRows::OleGetNextItem(/* [in, out] */ POSITION& rPos, /* [out] */ VARIANT *pVar)
{
	ASSERT(pVar);
	if (!rPos || !pVar)
	{
	...
	CXTPReportRow* pRow = GetNextSelectedRow(rPos);
	if (pRow)
	{
		pVar->vt = VT_DISPATCH;
...
POSITION CXTPReportSelectedRows::OleGetFirstItemPosition()
{
