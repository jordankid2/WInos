...
#include "ReportControl/XTPReportView.h"
...
#include "ReportControl/Themes/XTPReportThemeVisualStudio2015.h"
...
#define XTP_REPORT_HSCROLL_STEP XTP_DPI_X(7)
#define XTP_REPORT_VSCROLL_STEP XTP_DPI_Y(7)
...
//ICON_VIEW_MODE RELATED >>
	...
	SetTheme(xtpReportThemeOfficeXP);
...
BOOL CXTPReportControl::_ApplyFilter(CXTPReportRecord* pRecord, CString strFilterText, BOOL bIncludePreview)
{
	...
	szToken = STRTOK_S(strFilterText.GetBuffer(strFilterText.GetLength()), m_bStrictFiltering ? szSepsStrict : szSeps, &lpszContext);
...
int CXTPReportControl::DrawRows(CDC *pDC, CRect &rcClient, int y, CXTPReportRows *pRows, int nTopRow, int nColumnFrom, int nColumnTo, int *pnHeight)
{
	...
	if (NULL != pHeader->GetNextVisibleColumn(-1, xtpReportColumnDirectionRight))
	{
		...
		{
			CXTPReportColumn *pColumn = GetColumns()->GetAt(nLastFreezeColumnIndex);
		...
		while (true)
		{
			...
			CXTPReportRecordMergeItems mergeItemsF;
			...
			while (pos)
			{
				...
				while (mergeItem.FindRange(range, rcItem))
				{
					...
					{		// draw frozen border again to prevent showing grid under border
						CXTPReportColumn *pColumn = GetColumns()->GetAt(range.m_nColumnFrom);
					...
					if (bDrawFreezeColsDivider)
					{
						...
						CXTPReportRow* pRowFromMerge = NULL;
...
#ifdef _XTP_ACTIVEX
	void CXTPReportControl::SetCustomDraw(long dwFlags)
	{
...
int CXTPReportControl::GetReportAreaRows(int nStartRow, BOOL bMoveDown)
{
	...
	return bMoveDown ? max(0, m_pRows->GetCount() - nStartRow - 1): nStartRow; //-this is better way!
}
...
void CXTPReportControl::GetParametersOfHScrollingBlockCount(int &nTotalCount, int &nScrollPage) const
{
	nTotalCount			= 0;
	nScrollPage			= 0;
	...
	{
		CXTPReportColumn *pColumn = pColumns->GetAt(nLastFreezeColumnIndex);
		if (NULL != pColumn)
			nFreezeWidth = pColumn->GetRect().right;
	}
	int nTotalWidth		= GetReportHeader()->GetWidth() - nFreezeWidth;
	...
	{
		// calc nScrollPage from the end of columns to get correct  nLimit = si.nMax - si.nPage + 1 in OnHScroll()
...
void CXTPReportControl::GetParametersOfHScrollingBlockSize(int &nTotalWidth, int &nScrollPage) const
{
	int nFreezeRight = 0;
	int nLastFreezeColumnIndex = -1;
	GetFreezeColumnsCount(nLastFreezeColumnIndex);
	...
	nTotalWidth = GetReportHeader()->GetWidth() - nFreezeRight - 1;
...
void CXTPReportControl::UpdateScrollBarVisibility(int nBar)
{
	CScrollBar* pScrollBar = GetScrollBarCtrl(nBar);
	if (NULL != pScrollBar)
	{
		...
		{
		case SB_VERT:
			bVisible = m_bVScrollBarVisible;
			break;
		case SB_HORZ:
			bVisible = m_bHScrollBarVisible;
			break;
		default:
			ASSERT(!"Not supported scroll bar ID");
			break;
		}
		...
		if (bCurrentState != bVisible)
		{
			...
			CXTPReportView* pReportView = DYNAMIC_DOWNCAST(CXTPReportView, GetParent());
			if (NULL != pReportView)
			{
				pReportView->UpdateScrollBars();
			}
		}
	}
}
...
void CXTPReportControl::AdjustScrollBarH(BOOL bUpdate, UINT nMask)
{
	...
	if (xtpReportScrollModeBlockCount == m_scrollModeH)
	{
		if ((SIF_RANGE & nMask) || (SIF_PAGE & nMask))
		{
			...
			if (SIF_PAGE  & nMask) si.nPage = nScrollPage;
		}
	...
	if (xtpReportScrollModeBlockSize == m_scrollModeH)
	{
		if ((SIF_RANGE & nMask) || (SIF_PAGE & nMask))
		{
			...
			if (SIF_RANGE & nMask) si.nMin  = 0;
			if (SIF_RANGE & nMask) si.nMax  = nTotalWidth;
			if (SIF_PAGE  & nMask) si.nPage = nScrollPage;
		}
	...
	if (xtpReportScrollModeSmooth == m_scrollModeH)
	{
		...
		if (SIF_RANGE & nMask) si.nMin   = 0;
		if (SIF_RANGE & nMask) si.nMax   = GetReportHeader()->GetWidth() - XTP_DPI_X(1);
	...
	else if (xtpReportScrollModeBlockCount == m_scrollModeH)
	{
		si.nPage = min(si.nPage, UINT(si.nMax)+1);
		...
		if ((si.nPos > 0) && (si.nPos + int(si.nPage) - 1 > si.nMax))
		{
			si.nPos = si.nMax - si.nPage + 1;						// offset in number of columns
...
int CXTPReportControl::GetPageRowCount(CDC *pDC) const
{
	...
	{
		CXTPReportRow *pRow = m_pRows->GetAt(nLinesCount);
		ASSERT(NULL != pRow);
			 ...
			 xtpReportScrollModeBlockSize  == m_scrollModeV)
		{
			...
			switch (nSBCode)
			{
				...
				case SB_LINEDOWN:           // 1
					...
					break;
				case SB_PAGEUP:             // 2
					nTopRowPos = max(si.nPos - max(1, GetReportAreaRows(si.nPos, FALSE)), 0);
					break;
				case SB_PAGEDOWN:           // 3
					nTopRowPos = min(si.nPos + max(1, GetReportAreaRows(si.nPos, TRUE)), nLimit);
					break;
				case SB_THUMBPOSITION:      // 4
				case SB_THUMBTRACK:         // 5
...
void CXTPReportControl::OnNcPaint()
{
...
{
	CRect rc(m_pSectionBody->m_rcSection.left, y, m_pSectionBody->m_rcSection.right, y + XTP_DPI_Y(1));
			...
			if (IsSelectionEnabled() && !bShiftKey && bControlKeyOnly && IsMultipleSelection())
			{
				...
				else
				{
					if (m_pSelectedRows->m_nRowType == xtpRowTypeBody &&
						GetRecords()->GetCount() == m_pSelectedRows->GetCount() )	//all rows were selected before
					{
						break;
					}
...
{
	if (CWnd::GetCapture() != this && GetMouseMode() != xtpReportMouseNothing)
		GetReportHeader()->CancelMouseMode();
...
void CXTPReportControl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (GetMouseMode() == xtpReportMouseNothing)
	{
		...
		if (m_bPrepareDrag && (labs (point.x - m_ptDrag.x) > XTP_DPI_X(GetDragSensitivityX()) ||
...
//R-DD >>
			...
			{
				if ((m_nDropPos > m_pSectionBody->m_rcSection.top - XTP_DPI_Y(1) || m_nDropPos < m_pSectionBody->m_rcSection.bottom - XTP_DPI_Y(1)))
					m_nDropPos = point.y;
			...
			if (m_nDropPos == 0 && GetRows()->GetCount() == 0)
				m_nDropPos = m_pSectionBody->m_rcSection.top;
			else if ((m_nDropPos < m_pSectionBody->m_rcSection.top - XTP_DPI_Y(1) || m_nDropPos > m_pSectionBody->m_rcSection.bottom - XTP_DPI_Y(1)))
				dropEffect = DROPEFFECT_NONE;
		}
	}
	...
	if (m_nOLEDropMode)
	{
		CXTPOleDataObject::FireOleDragOverEvent((COleControl*)GetParent(), pDataObject, dropEffect, point);
...
BOOL CXTPReportControl::OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect, CPoint point)
{
...
#ifdef _XTP_ACTIVEX
	BOOL bSkipOLEDrop = (m_dwDragDropFlags & xtpReportAllowDrop) &&
						!IsVirtualMode() && pDataObject &&
	...
	BOOL bAbove = TRUE;
	CXTPReportRecord*  pTargetRecord	= NULL;
	CXTPReportRecords* pTargetRecords	= GetRecords();
...
void CXTPReportControl::ReleaseSorted()
{
	...
	if (nSortCnt > 0 && pRows)
	{
		...
		CXTPReportRecord* pDropRec = NULL;
...
void CXTPReportControl::DoAutoVertScrollIfNeed(CPoint ptClick, CPoint pt)
{
	...
	if (xtpReportControlDirectionNone == nDirection || m_bDragMode &&
	  ((xtpReportControlDirectionDown == nDirection && pt.y - ptClick.y < XTP_DPI_Y(3)) ||
		(xtpReportControlDirectionUp   == nDirection && ptClick.y - pt.y < XTP_DPI_Y(3))))
	{
		EnsureStopAutoVertScroll();
		return;
	}
...
BOOL CXTPReportControl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	if (nHitTest == HTCLIENT)
	{
		switch (GetMouseMode())
		{
			...
			case xtpReportMouseOverRowDivide:
				SetCursor(GetReportHeader()->m_hResizeVCursor);
				return TRUE;
		}
	}
...
void CXTPReportControl::EditItem(XTP_REPORTRECORDITEM_ARGS *pItemArgs)
{
	...
	if (pItem && pItemArgs && pItemArgs->pRow)
	{
		...
		if (m_bShowIconWhenEditing && pItemArgs->pColumn)
		{
			...
			drawArgs.nTextAlign = pItem->GetAlignment() == -1 ? pItemArgs->pColumn->GetAlignment() : pItem->GetAlignment();
			...
			if (nIcon != XTP_REPORT_NOICON && (drawArgs.nTextAlign & xtpColumnIconCenter) == 0)
			{
...
#ifdef _XTP_ACTIVEX
					pVirtualRecord->AddItem(new CXTPReportRecordItemText());
#else
...
void CXTPReportControl::SetLayoutRTL(BOOL bRightToLeft)
{
	...
	if (GetParent())
	{
		...
		GetParent()->SetWindowPos(NULL, 0, 0, rc.Width() + XTP_DPI_X(1), rc.Height(), SWP_NOMOVE | SWP_NOZORDER);
	}
}
													...
													CXTPReportColumn* pColumn, int nStartRow, int nRowsCount)
{
	...
	for (int i = max(0, nStartRow); i < nEndRow; i++)
	{
		...
		{
			nWidth = XTPMarkupMeasureElement(pItem->GetMarkupUIElement(), INT_MAX, 0).cx + XTP_DPI_X(4);
		}
		else
		{
...
CRect CXTPReportControl::GetElementRect(XTPReportElementRect nElement) const
{
	CRect rcElement;
	switch (nElement)
	{
	case xtpReportElementRectReportArea:
		rcElement = m_pSectionBody->m_rcSection;
		break;
	case xtpReportElementRectGroupByArea:
		rcElement = m_rcGroupByArea;
		break;
	case xtpReportElementRectHeaderArea:
		rcElement = m_rcHeaderArea;
		break;
	case xtpReportElementRectFooterArea:
		rcElement = m_rcFooterArea;
		break;
	case xtpReportElementRectHeaderRecordsArea:
		rcElement = m_pSectionHeader->m_rcSection;
		break;
	case xtpReportElementRectFooterRecordsArea:
		rcElement = m_pSectionFooter->m_rcSection;
		break;
	...
	case xtpReportElementRectFooterRecordsDividerArea:
		...
		break;
	default:
		ASSERT(!"Unexpected report element specified");
		break;
	}
...
{
	return GetElementRect(static_cast<XTPReportElementRect>(nElement));
}
...
// Reset Markup UI Elements
	...
	{
		CXTPReportSection* pSection = m_pSections->GetAt(nSection);
		if (pSection == NULL)
			continue;
		
		CXTPReportRows* pRows = pSection->GetRows();
		if (pRows)
		{			
			for (int nRow=0; nRow<pRows->GetCount(); nRow++)
			{
				...
				if (pRow && pRow->IsGroupRow())
				{
					((CXTPReportGroupRow*) pRow)->ResetMarkupUIElement();
				}
			}
		}
		
		CXTPReportRecords* pRecords = pSection->GetRecords();
		if (pRecords)
		{			
			for (int nRecord=0; nRecord<pRecords->GetCount(); nRecord++)
			{
				...
				{
					CXTPReportRecordItem* pRecordItem = pRecord->GetItem(nItem);
					if (pRecordItem)
...
BEGIN_INTERFACE_MAP(CXTPReportControl, CWnd)
	INTERFACE_PART(CXTPReportControl, IID_IAccessible, ExternalAccessible)
	XTP_DECLARE_CMDTARGETPROVIDER_INTERFACE_PART(CXTPReportControl)
END_INTERFACE_MAP()

XTP_IMPLEMENT_CMDTARGETPROVIDER_INTERFACE(CXTPReportControl)
...
{
	int nLastFreezeColumnIndex = -1;
	return GetFreezeColumnsCount(nLastFreezeColumnIndex);
}

int CXTPReportControl::GetFreezeColumnsCount(int& rnLastFreezeColumnIndex) const
{
	int nCount = 0;
	rnLastFreezeColumnIndex = -1;
	...
	for (int nColumn=0; nColumn<nColumnsCount; nColumn++)
	{
		...
		if (pColumn->IsVisible())
		{
			if (pColumn->IsFrozen())
			{
				rnLastFreezeColumnIndex = nColumn;
				++nCount;
			}
			else
			{
...
void CXTPReportControl::SetTheme(XTPReportPaintTheme paintTheme, BOOL bEnableMetrics/*=FALSE*/)
{
	...
	case xtpReportThemeOfficeXP:
		SetPaintManager(new CXTPReportThemeOfficeXP());
		break;
	case xtpReportThemeOffice2003:
		SetPaintManager(new CXTPReportThemeOffice2003());
		break;
	case xtpReportThemeOffice2003Luna:
		SetPaintManager(new CXTPReportThemeOffice2003Luna());
		break;
	case xtpReportThemeOffice2013:
		SetPaintManager(new CXTPReportThemeOffice2013());
		break;
	case xtpReportThemeVisualStudio2012Light:
		SetPaintManager(new CXTPReportThemeVisualStudio2012Light());
		break;
	case xtpReportThemeVisualStudio2012Dark:
		SetPaintManager(new CXTPReportThemeVisualStudio2012Dark());
		break;
	case xtpReportThemeVisualStudio2012Blue:
		SetPaintManager(new CXTPReportThemeVisualStudio2012Blue());
		break;
	case xtpReportThemeVisualStudio2015:
		SetPaintManager(new CXTPReportThemeVisualStudio2015());
		break;
	case xtpReportThemeExplorer:
		SetPaintManager(new CXTPReportThemeExplorer());
		break;
	case xtpReportThemeResource:
		SetPaintManager(new CXTPReportThemeResource());
		break;
	default:
		SetPaintManager(new CXTPReportThemeDefault());
		break;
	}
}

BOOL CXTPReportControl::IsCellMergingAllowed() const
{
	return GetPaintManager() && GetPaintManager()->m_bAllowMergeCells;
}

void CXTPReportControl::SetDragSensitivityX(int nDragSensitivityX)
{
...
void CXTPReportControl::SetDragSensitivityY(int nDragSensitivityY)
{
