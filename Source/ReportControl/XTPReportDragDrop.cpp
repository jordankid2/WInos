...
#include "ReportControl/XTPReportDragDrop.h"
...
void CXTPReportHeaderDragWnd::OnTimer(UINT_PTR nIDEvent)
{
	...
	else
	{
		...
		{
			CXTPReportColumn* pLastFCol = pControl->GetColumns()->GetAt(nLastFrozenColumnIndex);
			if (pLastFCol)
			{
...
BOOL CXTPReportHeaderDropWnd::Create(CWnd* /*pParentWnd*/, int nHeight)
{
	...
	BOOL bResult = CreateEx(dwExStyle, AfxRegisterWndClass(0, AfxGetApp()->LoadStandardCursor(IDC_ARROW)), NULL, dwStyle,
		0, 0, XTP_DPI_X(12), m_nHeight,
		NULL, NULL, NULL);

	CRgn rgn1, rgn2;
	POINT ptArrow[7];

	ptArrow[0].x = XTP_DPI_X(8);   ptArrow[0].y = 0;
	ptArrow[1].x = XTP_DPI_X(8);   ptArrow[1].y = XTP_DPI_Y(4);
	ptArrow[2].x = XTP_DPI_X(11);  ptArrow[2].y = XTP_DPI_Y(4);
	ptArrow[3].x = XTP_DPI_X(6);   ptArrow[3].y = XTP_DPI_Y(9);
	ptArrow[4].x = XTP_DPI_X(1);   ptArrow[4].y = XTP_DPI_Y(4);
	ptArrow[5].x = XTP_DPI_X(4);   ptArrow[5].y = XTP_DPI_Y(4);
	ptArrow[6].x = XTP_DPI_X(4);   ptArrow[6].y = 0;
	rgn1.CreatePolygonRgn(ptArrow, 7, ALTERNATE);


	ptArrow[0].x = XTP_DPI_X(4);   ptArrow[0].y = m_nHeight;
	ptArrow[1].x = XTP_DPI_X(4);   ptArrow[1].y = m_nHeight-XTP_DPI_Y(4);
	ptArrow[2].x = 0;   ptArrow[2].y = m_nHeight-XTP_DPI_Y(4);
	ptArrow[3].x = XTP_DPI_X(6);   ptArrow[3].y = m_nHeight-XTP_DPI_Y(10);
	ptArrow[4].x = XTP_DPI_X(12);  ptArrow[4].y = m_nHeight-XTP_DPI_Y(4);
	ptArrow[5].x = XTP_DPI_X(8);   ptArrow[5].y = m_nHeight-XTP_DPI_Y(4);
	ptArrow[6].x = XTP_DPI_X(8);   ptArrow[6].y = m_nHeight;
...
{
	CWnd::SetWindowPos(0, x - XTP_DPI_X(6), y - (m_nHeight / 2), 0, 0, SWP_NOSIZE | SWP_SHOWWINDOW | SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOOWNERZORDER);
}
