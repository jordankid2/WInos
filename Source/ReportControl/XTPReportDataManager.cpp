...
IMPLEMENT_DYNAMIC(CXTPReportDataManager, CXTPCmdTarget);
...
BEGIN_DISPATCH_MAP(CXTPReportDataManager, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPReportDataManager, "DataSource", 1, OleGetDataSource, OleSetDataSource, VT_DISPATCH)
	DISP_FUNCTION_ID(CXTPReportDataManager, "DataBind", 2, DataBind, VT_BOOL, VTS_NONE)
	DISP_FUNCTION_ID(CXTPReportDataManager, "UpdateField", 3, OleUpdateField, VT_BOOL, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPReportDataManager, "UpdateRecord", 4, OleUpdateRecord, VT_BOOL, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPReportDataManager, "AddRecord", 5, OleAddRecord, VT_BOOL, VTS_DISPATCH VTS_BOOL)
	DISP_FUNCTION_ID(CXTPReportDataManager, "DeleteRecord", 6, OleDeleteRecord, VT_BOOL, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPReportDataManager, "CreateEmptyRecord", 7, OleCreateEmptyRecord, VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPReportDataManager, "UpdateRecordItem", 8, OleUpdateRecordItem, VT_BOOL, VTS_DISPATCH)
END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPReportDataManager, CXTPCmdTarget)
	INTERFACE_PART(CXTPReportDataManager, XTPDIID_IReportDataManager, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPReportDataManager, XTPDIID_IReportDataManager)

CXTPReportDataManager* AFX_CDECL CXTPReportDataManager::FromDispatch(LPDISPATCH pDisp)
{
	CXTPReportDataManager* pInst = NULL;
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPReportDataManager, 
			CXTPReportDataManager::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
...
LPDISPATCH CXTPReportDataManager::OleGetDataSource()
{
...
void CXTPReportDataManager::OleSetDataSource(LPDISPATCH lpDispatch)
{
	if (!lpDispatch)
	{
		SetDataSource(NULL);
	...
	XTPREPORTADODB::_RecordsetPtr pDataSource = NULL;
	HRESULT hr = lpDispatch->QueryInterface(&pDataSource);
	if (pDataSource != NULL)
		SetDataSource(pDataSource);
}

BOOL CXTPReportDataManager::OleUpdateField(LPDISPATCH lpDispatch)
{
	if (!lpDispatch)
		return FALSE;
	CXTPReportRecordItemVariant* pItem = DYNAMIC_DOWNCAST(CXTPReportRecordItemVariant, 
		CXTPReportRecordItemVariant::FromIDispatchSafe(lpDispatch));
	if (!pItem)
	{
		return FALSE;
	}
...
BOOL CXTPReportDataManager::OleUpdateRecord(LPDISPATCH lpDispatch)
{
	if (!lpDispatch)
		return FALSE;
	CXTPReportRecord* pRecord = CXTPReportRecord::FromDispatch(lpDispatch);
	if (!pRecord)
		return FALSE;
	return SUCCEEDED(UpdateRecord(pRecord));
}

BOOL CXTPReportDataManager::OleUpdateRecordItem(LPDISPATCH lpDispatch)
{
	if (!lpDispatch)
		return FALSE;
	CXTPReportRecordItem* pRecordItem = DYNAMIC_DOWNCAST(CXTPReportRecordItem, 
...
BOOL CXTPReportDataManager::OleAddRecord(LPDISPATCH lpDispatch, BOOL bAddToReport)
{
	if (!lpDispatch)
		return FALSE;
	CXTPReportRecord* pRecord = CXTPReportRecord::FromDispatch(lpDispatch);
	...
	return SUCCEEDED(AddRecord(pRecord, bAddToReport));
}

BOOL CXTPReportDataManager::OleDeleteRecord(LPDISPATCH lpDispatch)
{
	if (!lpDispatch)
		return FALSE;
	CXTPReportRecord* pRecord = CXTPReportRecord::FromDispatch(lpDispatch);
...
LPDISPATCH CXTPReportDataManager::OleCreateEmptyRecord()
{
	CXTPReportRecord* pRecord = NULL;
