...
#include "ReportControl/XTPReportFilterEditControl.h"
...
void CXTPReportHeader::OnLButtonUp(UINT nFlags, CPoint ptClick)
{
	...
	if (mouseMode == xtpReportMousePrepareDragColumn)
	{
		...
		if (pColumn->IsSortable() && m_bAllowColumnSort)
		{
			...
			XTPReportRowType nSelectedRowType = xtpRowTypeInvalid;
	...
	if (m_rcGroupBy.PtInRect(ptPoint))
	{
		...
		if (bExactSearch)
		{
			...
			for (int nColumn = 0; nColumn < nColumnsCount; nColumn++)
			{
				...
				if (pColumn)
				{
					CRect rcItem(x, y, x + pColumn->m_rcGroupBy.Width(), y + nHeaderHeight - XTP_DPI_Y(3));
...
void CXTPReportHeader::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
	...
	if (pCol != NULL && pCol->IsPlusMinus())
	{
		...
		if (rcPlusMinus.right - point.x <= XTP_DPI_X(20) && rcPlusMinus.right - point.x > XTP_DPI_X(15))
		{
...
void CXTPReportHeader::DrawGroupByControl(CDC* pDC, CRect& rcGroupBy)
{
	...
	for (int nColumn = 0; nColumn < nColumnsCount; nColumn++)
	{
		...
		if (pColumn /*&& pColumn->IsVisible()*/)
		{
			CRect rcItem(x, y, x + pColumn->GetCaptionWidth(pDC) + XTP_DPI_X(50), y + nHeaderHeight - XTP_DPI_Y(3));
			...
			if (nColumn < nColumnsCount - 1)
				pPM->DrawConnector(pDC, CPoint(rcItem.right - XTP_DPI_X(5), rcItem.bottom), CPoint(x, rcItem.bottom + XTP_DPI_Y(4)));
		}
	}
...
BEGIN_INTERFACE_MAP(CXTPReportHeader, CXTPCmdTarget)
