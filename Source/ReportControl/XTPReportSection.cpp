...
void CXTPReportSection::Populate(BOOL bUpdate)
{
	...
	CXTPReportRow    *pFocusedRow    = m_pRows->GetFocusedRow();
		...
		if (IsAllowGroup() && pColumns->GetGroupsOrder()->GetCount() > 0)
		{
			...
			for (int nPlainRow = 0; nPlainRow < m_pTree->GetCount(); nPlainRow++)
			{
				...
				for (int nColumn = 0; nColumn < pColumns->GetGroupsOrder()->GetCount(); nColumn++)
				{
					...
					COleVariant vtGroupCaptionValue = pItem ? pItem->GetGroupCaptionValue(pColumn) : COleVariant();
...
int CXTPReportSection::_DoExpand(int nIndex, CXTPReportRow *pRow, BOOL bUpdateSelection)
{
	int nCount		= 0;
...
void CXTPReportSection::RefreshIndexes()
{
	RefreshIndexes(0);
}
...
BOOL CXTPReportSection::EnsureVisible(CDC *pDC, CXTPReportRow *pRow)
{
	...
	if (NULL == pDC || NULL == pRow || 
		!pRow->IsVisible() || this != pRow->GetSection())
	{
		return FALSE;
	}
...
int CXTPReportSection::EnsureVisibleCalc(CDC *pDC, CXTPReportRow *pRow)
{
	...
	if (nIndexRow > nIndexTop)
	{
		...
		{
			CXTPReportRow* pCurrentRow = GetRows()->GetAt(nIndex);
		...
		if (top >= m_rcSection.bottom)
		{
			...
			{
				CXTPReportRow* pCurrentRow = m_pRows->GetAt(nIndex);
				ASSERT(NULL != pCurrentRow);
				if (pCurrentRow == NULL)
					continue;
	...
	int nRecordChildIndex)
{
				...
				for (int nColumn = pInsertRowPos ? pInsertRowPos->GetGroupLevel() : 0; nColumn < pColumns->GetGroupsOrder()->GetCount(); nColumn++)
				{
					...
					if (pGroupToAdd)
					{
						if (!pPlainTreeRow)
						{
							...
							pGroupToAdd->GetChilds()->InsertAt(!pInsertRowPos ? 0 : (pInsertRowPos->m_nChildIndex + (bInsertAfter ? 1 : 0)), pGroup);
						}
						else
							pGroupToAdd->AddChild(pGroup);
	...
	//              pNewRow->m_pParentRow = pGroupToAdd;
			...
			else
			{
				...
				if (pGroupToAdd)
				{
					pGroupToAdd->GetChilds()->InsertAt(!pInsertRowPos ? 0 : (pInsertRowPos->m_nChildIndex + (bInsertAfter ? 1 : 0)), pNewRow);
					pNewRow->m_pParentRows = pGroupToAdd->GetChilds();
					pNewRow->m_pParentRow = pGroupToAdd;
					pNewRow->SetVisible(pGroupToAdd->IsExpanded());
				}
...
void CXTPReportSection::CollapseAll()
{
...
void CXTPReportSection::ExpandAll(BOOL bRecursive)
{
	BOOL bNotify = m_pControl->GetBehavior()->Notifications->Row->bRowExpanded;
...
IMPLEMENT_OLETYPELIB_EX(CXTPReportSection, XTPDIID_IReportSection)

BEGIN_DISPATCH_MAP(CXTPReportSection, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPReportSection, "Index",        1, GetIndex,      SetNotSupported, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPReportSection, "AllowAccess", 10, IsAllowAccess, AllowAccess,     VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPReportSection, "AllowEdit",   11, IsAllowEdit,   AllowEdit,       VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPReportSection, "AllowFilter", 12, IsAllowFilter, AllowFilter,     VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPReportSection, "AllowGroup",  13, IsAllowGroup,  AllowGroup,      VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPReportSection, "AllowSort",   14, IsAllowSort,   AllowSort,       VT_BOOL)
END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPReportSection, CXTPCmdTarget)
	INTERFACE_PART(CXTPReportSection, XTPDIID_IReportSection, Dispatch)
END_INTERFACE_MAP()
