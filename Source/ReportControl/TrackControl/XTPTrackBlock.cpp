// XTPTrackBlock.cpp : implementation of the CXTPTrackBlock class.
//
...
#include "ReportControl/TrackControl/XTPTrackUndoManager.h"
...
IMPLEMENT_SERIAL(CXTPTrackBlock, CXTPCmdTarget, VERSIONABLE_SCHEMA | _XTP_SCHEMA_CURRENT)
...
{
	if (m_pItem && m_pItem->GetTrackControl() && m_pItem->GetTrackControl()->GetPaintManager())
	{
		return m_pItem->GetTrackControl()->GetPaintManager()->DrawTrackBlock(pDC, rc, bSelected, this);
	}
...
int CXTPTrackKey::Draw(CDC* pDC, CRect rc, BOOL bSelected)
{
	...
	rc.left  = pTrackControl->PositionToTrack(m_nPosition) - XTP_DPI_X(4);
	rc.right = rc.left + XTP_DPI_X(8);

	int X = (rc.left + rc.right) / 2;
	int Y = rc.CenterPoint().y;

	if (m_nVerticalAlignment == DT_TOP)
		Y = rc.top + XTP_DPI_Y(4);
	if (m_nVerticalAlignment == DT_BOTTOM)
		Y = rc.bottom - XTP_DPI_Y(6);

	rc.top = Y - XTP_DPI_Y(4);
	rc.bottom = Y + XTP_DPI_Y(4);
	...
	CPoint pts[] = {CPoint(X - XTP_DPI_X(4), Y), CPoint(X, Y - XTP_DPI_Y(4)), CPoint(X + XTP_DPI_X(4), Y), CPoint(X + XTP_DPI_X(4), Y + XTP_DPI_Y(4)), CPoint(X - XTP_DPI_X(4), Y + XTP_DPI_Y(4))};
	pDC->Polygon(pts, 5);
...
IMPLEMENT_SERIAL(CXTPTrackMarker, CXTPCmdTarget, VERSIONABLE_SCHEMA | _XTP_SCHEMA_CURRENT)
...
BEGIN_DISPATCH_MAP(CXTPTrackBlock, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPTrackBlock, CXTPCmdTarget)
INTERFACE_PART(CXTPTrackBlock, XTPDIID_TrackBlock, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPTrackBlock, XTPDIID_TrackBlock)
...
BEGIN_DISPATCH_MAP(CXTPTrackMarker, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPTrackMarker, CXTPCmdTarget)
	INTERFACE_PART(CXTPTrackMarker, XTPDIID_TrackMarker, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPTrackMarker, XTPDIID_TrackMarker)
...
BEGIN_DISPATCH_MAP(CXTPTrackMarkers, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPTrackMarkers, CXTPCmdTarget)
	INTERFACE_PART(CXTPTrackMarkers, XTPDIID_TrackMarkers, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPTrackMarkers, XTPDIID_TrackMarkers)
...
BEGIN_DISPATCH_MAP(CXTPTrackSelectedBlocks, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPTrackSelectedBlocks, CXTPCmdTarget)
INTERFACE_PART(CXTPTrackSelectedBlocks, XTPDIID_SelectedTrackBlocks, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPTrackSelectedBlocks, XTPDIID_SelectedTrackBlocks)
