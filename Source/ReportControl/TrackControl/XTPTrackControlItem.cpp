...
void CXTPTrackControlItem::HighlightWorkArea(CDC* pDC, CRect rcItem)
{
	...
	for (int i = 0; i < pControl->GetMarkers()->GetCount(); i++)
	{
		...
		pDC->FillSolidRect(CRect(nPos, rcItem.top, nPos + XTP_DPI_Y(1), rcItem.bottom + XTP_DPI_Y(10)), pControl->GetTrackPaintManager()->GetGridColor());
	}
}
...
BOOL CXTPTrackControlItem::OnLButtonDown(XTP_REPORTRECORDITEM_CLICKARGS* pClickArgs)
{
	...
	if (pBlock && !pBlock->IsLocked())
	{
		...
		if (pControl->m_bAllowBlockScale && pBlock->IsResizable() && pBlock->GetRect().left >= ptFrom.x - XTP_DPI_X(4))
		{
			pControl->StartDragBlocks(1);
		}
		else if (pControl->m_bAllowBlockScale && pBlock->IsResizable() && pBlock->GetRect().right <= ptFrom.x + XTP_DPI_X(4))
		{
	...
	if (pRow)
	{
		...
		if (pBlock && !pBlock->IsLocked())
		{
			if (((CXTPTrackControl*)pControl)->m_bAllowBlockScale && pBlock->IsResizable()
				&& (pBlock->GetRect().left >= point.x - XTP_DPI_X(4) || pBlock->GetRect().right <= point.x + XTP_DPI_X(4)))
			{
