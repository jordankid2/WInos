...
#include "ReportControl/TrackControl/XTPTrackControlItem.h"
#include "ReportControl/TrackControl/XTPTrackControl.h"
#include "ReportControl/TrackControl/XTPTrackBlock.h"
...
void CXTPTrackPaintManager::SetColumnStyle(XTPReportColumnStyle columnStyle)
{
	...
	if (::IsWindow(pTrackControl->GetSafeHwnd()))
	{
		...
		case xtpReportColumnShaded:
			pTrackControl->SetTheme(xtpReportThemeOffice2003, FALSE);
			break;
		case xtpReportColumnFlat:
			pTrackControl->SetTheme(xtpReportThemeOfficeXP, FALSE);
			break;
		case xtpReportColumnExplorer:
			pTrackControl->SetTheme(xtpReportThemeExplorer, FALSE);
			break;
		case xtpReportColumnOffice2003:
			pTrackControl->SetTheme(xtpReportThemeOffice2003Luna, FALSE);
			break;
		case xtpReportColumnResource:
			pTrackControl->SetTheme(xtpReportThemeResource, FALSE);
			break;
		}
	}
}
...
void CXTPTrackPaintManager::DrawTimeLine(CDC* pDC)
{
	...
	CRect rcSliderArea(nLeftOffset, rcTimeLineArea.top, nRightOffset, rcTimeLineArea.top + XTP_DPI_Y(10));
	...
	for (int pos = nFirstPos; pos <= nTimeScaleMax; pos += dx)
	{
		...
		pDC->FillSolidRect(x, rcTimeArea.bottom - min(XTP_DPI_Y(4), nTickLength), XTP_DPI_X(1), nTickLength, m_clrTimeLineTick);

		int nCaptionWidth = pDC->GetTextExtent(strCaption).cx;
		pDC->DrawText(strCaption, CRect(x - nCaptionWidth / 2, rcTimeArea.top,
...
void CXTPTrackPaintManager::DrawMarkers(CDC* pDC)
{
	...
	CRect rcColumnArea(nLeftOffset, rcHeaderArea.top, nRightOffset, rcHeaderArea.bottom - XTP_DPI_Y(3));
	...
	if (pTrackControl->m_bShowTimeLinePosition)
	{
		...
		CPoint pts[] = {CPoint(X - XTP_DPI_X(5), Y), CPoint(X, Y + XTP_DPI_Y(5)), CPoint(X + XTP_DPI_X(5), Y), CPoint(X + XTP_DPI_X(5), Y - XTP_DPI_Y(10)), CPoint(X - XTP_DPI_X(5), Y - XTP_DPI_Y(10))};
	...
	for (int i = 0; i < pTrackControl->GetMarkers()->GetCount(); i++)
	{
		...
		if (!strCaption.IsEmpty())
		{
			...
			pDC->FillSolidRect(CRect (nTrackedPos, rcColumnArea.top + XTP_DPI_Y(2), nTrackedPos + XTP_DPI_X(12) + dx, rcColumnArea.bottom - XTP_DPI_Y(2)), m_clrTrackMarkerTextBack);
			pDC->Draw3dRect(CRect (nTrackedPos, rcColumnArea.top + XTP_DPI_Y(2), nTrackedPos + XTP_DPI_X(12) + dx, rcColumnArea.bottom - XTP_DPI_Y(2)), m_clrTrackMarkerTextBorder, m_clrTrackMarkerTextBorder);

			pDC->DrawText(strCaption, CRect (nTrackedPos + XTP_DPI_X(8), rcColumnArea.top, nTrackedPos + XTP_DPI_X(8) + dx, rcColumnArea.bottom), DT_VCENTER | DT_SINGLELINE);
		...
		CPoint pts[] = {CPoint(X - XTP_DPI_X(5), Y), CPoint(X, Y - XTP_DPI_Y(5)), CPoint(X + XTP_DPI_X(5), Y), CPoint(X + XTP_DPI_X(5), Y + XTP_DPI_Y(5)), CPoint(X - XTP_DPI_X(5), Y + XTP_DPI_Y(5))};
		pDC->Polygon(pts, 5);

		pMarker->m_rcMarker = CRect(nTrackedPos - XTP_DPI_X(5), rcHeaderArea.bottom - XTP_DPI_Y(20), nTrackedPos + XTP_DPI_X(5), rcHeaderArea.bottom - XTP_DPI_Y(5));
	}

	if (pTrackControl->m_bShowTimeLinePosition)
	{
		pDC->FillSolidRect(CRect(nPos, rcColumnArea.top, nPos + XTP_DPI_X(1), rcHeaderArea.bottom), m_clrTimeLinePosition);
	}

}

int CXTPTrackPaintManager::DrawTrackBlock(CDC* pDC, CRect rc, BOOL bSelected, CXTPTrackBlock* pTrackBlock)
{
	if (!pTrackBlock)
		return 0;
	...
	if (!pTrackBlock->m_strCaption.IsEmpty())
	{
		...
		XTPMarkupDrawText(pTrackBlock->m_pItem->GetTrackControl()->GetMarkupContext(), pDC->GetSafeHdc(),
			pTrackBlock->m_strCaption, rcText, DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
	}
...
void CXTPTrackPaintManager::DrawTrackHeader(CDC* pDC)
{
	...
	pDC->FillSolidRect(nLeftOffset - XTP_DPI_X(1), rcTimeLineArea.top, XTP_DPI_X(1), rcTimeLineArea.Height() + rcHeaderArea.Height(), m_clrTimeHeaderDivider);


	pDC->FillSolidRect(nLeftOffset + XTP_DPI_X(7), rcColumnArea.top, nRightOffset - nLeftOffset - XTP_DPI_X(14), rcColumnArea.Height(), m_clrTimeHeaderDark);
	pDC->FillSolidRect(nLeftOffset, rcColumnArea.bottom - XTP_DPI_Y(1), nRightOffset - nLeftOffset, XTP_DPI_Y(1), m_clrTimeHeaderBorder);


	pDC->FillSolidRect(nLeftOffset + XTP_DPI_X(7), rcSliderArea.top, nRightOffset - nLeftOffset - XTP_DPI_X(14), rcSliderArea.Height(), m_clrTimeSliderBackground);


	pDC->FillSolidRect(nLeftOffset, rcSliderArea.bottom, XTP_DPI_X(7), rcColumnArea.top - rcSliderArea.bottom, m_clrTimeHeaderDark);
	pDC->FillSolidRect(nRightOffset - XTP_DPI_X(7), rcSliderArea.bottom, XTP_DPI_X(7), rcColumnArea.top - rcSliderArea.bottom, m_clrTimeHeaderDark);
...
// Work ScrollBar
	...
	if (pTrackControl->m_bShowWorkArea)
	{
		...
		pDC->Draw3dRect(rcWorkAreaScrollBar.left - XTP_DPI_X(1), rcWorkAreaScrollBar.top, rcWorkAreaScrollBar.Width() + XTP_DPI_X(2), rcWorkAreaScrollBar.Height(), m_clrScrollBarBorderDark, m_clrScrollBarBorderLight);

		XTPDrawHelpers()->GradientFill(pDC, CRect(rcWorkAreaScrollBar.left - XTP_DPI_X(7), rcWorkAreaScrollBar.top, rcWorkAreaScrollBar.left + 0, rcWorkAreaScrollBar.bottom),
			m_clrWorkAreaScrollBarLight, m_clrWorkAreaScrollBarDark, FALSE);

		pDC->Draw3dRect(CRect(rcWorkAreaScrollBar.left - XTP_DPI_X(7), rcWorkAreaScrollBar.top, rcWorkAreaScrollBar.left + 0, rcWorkAreaScrollBar.bottom),
			m_clrWorkAreaScrollBarBorder, m_clrWorkAreaScrollBarBorder);

		XTPDrawHelpers()->GradientFill(pDC, CRect(rcWorkAreaScrollBar.right - 0, rcWorkAreaScrollBar.top, rcWorkAreaScrollBar.right + XTP_DPI_X(7), rcWorkAreaScrollBar.bottom),
			m_clrWorkAreaScrollBarLight, m_clrWorkAreaScrollBarDark, FALSE);

		pDC->Draw3dRect(CRect(rcWorkAreaScrollBar.right - 0, rcWorkAreaScrollBar.top, rcWorkAreaScrollBar.right + XTP_DPI_X(7), rcWorkAreaScrollBar.bottom),
			m_clrWorkAreaScrollBarBorder, m_clrWorkAreaScrollBarBorder);
	}
...
// Zoom Slider
	...
	pDC->Draw3dRect(rcZoomSlider.left - XTP_DPI_X(1), rcZoomSlider.top, rcZoomSlider.Width() + XTP_DPI_X(2), rcZoomSlider.Height(), m_clrScrollBarBorderDark, m_clrScrollBarBorderDark);

	XTPDrawHelpers()->GradientFill(pDC, CRect(rcZoomSlider.left - XTP_DPI_X(7), rcZoomSlider.top, rcZoomSlider.left + 0, rcZoomSlider.bottom),
		m_clrWorkAreaScrollBarLight, m_clrWorkAreaScrollBarDark, FALSE);

	pDC->Draw3dRect(CRect(rcZoomSlider.left - XTP_DPI_X(7), rcZoomSlider.top, rcZoomSlider.left + 0, rcZoomSlider.bottom),
		m_clrWorkAreaScrollBarBorder, m_clrWorkAreaScrollBarBorder);

	XTPDrawHelpers()->GradientFill(pDC, CRect(rcZoomSlider.right - 0, rcZoomSlider.top, rcZoomSlider.right + XTP_DPI_X(7), rcZoomSlider.bottom),
		m_clrWorkAreaScrollBarLight, m_clrWorkAreaScrollBarDark, FALSE);

	pDC->Draw3dRect(CRect(rcZoomSlider.right - 0, rcZoomSlider.top, rcZoomSlider.right + XTP_DPI_X(7), rcZoomSlider.bottom),
		m_clrWorkAreaScrollBarBorder, m_clrWorkAreaScrollBarBorder);
	...
	if (pTrackControl->m_bShowTimeLinePosition)
	{
		...
		pDC->FillSolidRect(CRect(nZoomAreaTimeLinePos, rcZoomSlider.top, nZoomAreaTimeLinePos + 1, rcZoomSlider.bottom), m_clrTimeLinePosition);
	}
...
BEGIN_DISPATCH_MAP(CXTPTrackPaintManager, CXTPReportPaintManager)
	...
	DISP_PROPERTY_NOTIFY_ID(CXTPTrackPaintManager, "ColumnStyle", 7, m_columnStyle, OleSetColumnStyle, VT_I4)
