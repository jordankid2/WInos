...
#include "ReportControl/TrackControl/XTPTrackHeader.h"
...
int CXTPTrackControl::DrawRows(CDC* pDC, CRect& rcClient, int y, CXTPReportRows *pRows, int nTopRow, int nColumnFrom, int nColumnTo, int *pnHeight)
{
	...
	if (m_bShowTimeLinePosition)
	{
		...
		{
			pDC->FillSolidRect(CRect(nPos, m_pSectionBody->GetRect().top, nPos + 1, m_pSectionBody->GetRect().bottom + XTP_DPI_Y(10)), RGB(205, 0, 0));
		}
	}
	...
	if (!m_rcSelectedArea.IsRectEmpty())
	{
		...
		LPDWORD lpBits;
		HBITMAP hBitmap = CXTPImageManager::Create32BPPDIBSection(pDC->GetSafeHdc(), XTP_DPI_X(1), XTP_DPI_Y(1), (LPBYTE*)&lpBits);
		if (hBitmap)
		{
			...
			XTPImageManager()->AlphaBlend2(pDC->GetSafeHdc(), rcSelectedArea, dc, CRect(0, 0, XTP_DPI_X(1), XTP_DPI_Y(1)), 100);
...
void CXTPTrackControl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
		...
		&& (nChar == VK_LEFT || nChar == VK_RIGHT || nChar == VK_UP || nChar == VK_DOWN))
	{
		...
		if (nChar == VK_LEFT) pt.x -= XTP_DPI_X(20);
		if (nChar == VK_RIGHT) pt.x += XTP_DPI_X(20);
		if (nChar == VK_UP) pt.y -= XTP_DPI_Y(20);
		if (nChar == VK_DOWN) pt.y += XTP_DPI_Y(20);
		...
		if (bResize) pt.x = rcBlock.right - XTP_DPI_X(2);
		ClientToScreen(&pt);
...
BOOL CXTPTrackControl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	if (nHitTest == HTCLIENT)
	{
		...
		{
			case xtpTrackCursorDrag:
				SetCursor(m_hMoveCursor);
				return TRUE;

			case xtpTrackCursorResize:
				SetCursor(m_hResizeCursor);
				return TRUE;
		}
	}

	return CXTPReportControl::OnSetCursor(pWnd, nHitTest, message);
}
