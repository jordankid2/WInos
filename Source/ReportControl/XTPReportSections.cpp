...
void CXTPReportSections::ResetContent()
{
	for (int nSection=0; nSection<GetSize(); nSection++)
	{
		CXTPReportSection *pSection = GetAt(nSection);
...
void CXTPReportSections::RefreshIndexes(BOOL bAdjustLayout)
{
	// Re-sort rows of all sections
	for (int nSection=0; nSection<GetSize(); nSection++)
	{
...
IMPLEMENT_OLETYPELIB_EX(CXTPReportSections, XTPDIID_IReportSections)

BEGIN_DISPATCH_MAP(CXTPReportSections, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPReportSections, "Count",   1, OleGetCount,   SetNotSupported, VT_I4)
	DISP_FUNCTION_ID   (CXTPReportSections, "Section", 2, OleGetSection, VT_DISPATCH, VTS_I4)
	//DISP_PROPERTY_EX_ID(CXTPReportSections, "Section", 2, OleGetSection, SetNotSupported, VT_DISPATCH)
END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPReportSections, CXTPCmdTarget)
	INTERFACE_PART(CXTPReportSections, XTPDIID_IReportSections, Dispatch)
END_INTERFACE_MAP()


LPDISPATCH CXTPReportSections::OleGetSection(int nIndex) const
{
	CXTPReportSection *pSection = GetAt(nIndex);
