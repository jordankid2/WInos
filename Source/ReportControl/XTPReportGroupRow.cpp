...
IMPLEMENT_DYNAMIC(CXTPReportGroupRow, CXTPReportRow);
...
void CXTPReportGroupRow::SetCaption(LPCTSTR lpszCaption, COleVariant vtValue)
{
	SetCaption(lpszCaption);
	m_vtCaptionValue = vtValue;
}

void CXTPReportGroupRow::ResetMarkupUIElement()
{
	XTPMarkupReleaseElement(m_pMarkupUIElement);
...
COleVariant CXTPReportGroupRow::GetCaptionValue() const
{
...
void CXTPReportGroupRow::OnClick(CPoint ptClicked)
{
		 ...
		 && !IsLockExpand())
	{
		...
		const BOOL bKeyControl = (::GetKeyState(VK_CONTROL) < 0) || m_pControl->IsMultiSelectionMode();
...
BEGIN_INTERFACE_MAP(CXTPReportGroupRow, CXTPReportRow)
	INTERFACE_PART(CXTPReportGroupRow, XTPDIID_IReportGroupRow, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPReportGroupRow, XTPDIID_IReportGroupRow)
