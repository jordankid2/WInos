...
public:
	...
	int GetRowHeightEx(CDC* pDC, CXTPReportRow* pRow, int nTotalWidth, const BOOL firstLevel = TRUE);
	...
	//          Height of rows depend through merge cells which can contain big text or other content.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	BOOL FindRowAdjacentToMergedEx(const CXTPReportColumns* pColumns, const CXTPReportRow* pRow, int& nAdjRowIndex, BOOL bUpward);

	//-----------------------------------------------------------------------
	// Summary: wrapper for method FindRowAdjacentToMergedEx. Search topmost or bottommost row witch depend on heigh of current row.
	// Parameters:
	//     pColumns  - Pointer to CXTPReportColumns object
	//     pRow      - Pointer to CXTPReportRow object.
	//     bUpward   - BOOL flag to set searching direction up or down from current row.
	...
	//     The index of the topmost or bottommost row witch depend on heigh of current row.
	//-----------------------------------------------------------------------
	int FindRowAdjacentToMerged(const CXTPReportColumns* pColumns, const CXTPReportRow* pRow, BOOL bUpward);
	...
	//     The height of the default rectangle where row's items will draw.
	//-----------------------------------------------------------------------
	int RecalcTotalHightOfMergedRows(CXTPReportColumns* pColumns, CDC* pDC, CXTPReportRow* pRow, int nRowHeight, int nTotalWidth);
...
protected:
	...
	//-----------------------------------------------------------------------
	virtual void DrawGridPattern(CDC* pDC, CBrush* pBrush, CRect rc, const void *pPattern, COLORREF clr, BOOL bUpdateBrush);
	virtual void DrawGridPattern(CDC* pDC, CXTPBrush* pBrush, CRect rc, const void *pPattern, COLORREF clr, BOOL bUpdateBrush); // <combine CXTPReportPaintManager::DrawGridPattern@CDC*@CBrush*@CRect@const void *@COLORREF@BOOL>
...
public:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Draws the divider between report sections (e.g. dividing header record rows and body rows).
	// Parameters:
	//     pDC   - Pointer to the used Device Context.
	//     rc - Rectangle coordinates.
...
public:
	...
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CFont, m_fontBoldText, m_xtpFontBoldText, GetBoldTextFontHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CFont, m_fontCaption, m_xtpFontCaption, GetCaptionFontHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CFont, m_fontPreview, m_xtpFontPreview, GetPreviewFontHandle);
	...
	CXTPPaintManagerColor m_clrDisableBitmapLightest;   // The lightest color of bitmap which covers disabled report.
	CXTPPaintManagerColor m_clrDisableBitmapDarkest;    // The darkest color of bitmap which covers disabled report.
	...
	//{{AFX_CODEJOCK_PRIVATE
	__declspec(property(get = GetDrawSortTriangleAlways, put = SetDrawSortTriangleAlways)) BOOL m_bDrawSortTriangleAlways;
...
protected:
	...
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushTreeStructure, m_xtpBrushTreeStructure, GetTreeStructureBrushHandle);

	CXTPBrush m_xtpBrushGroupGridLineStyle;       // Stores brush to draw group row grid lines
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushGroupGridLineStyle, m_xtpBrushGroupGridLineStyle, GetGroupGridLineStyleBrushHandle);
	...
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushDotsSmallH, m_xtpBrushDotsSmallH, GetDotsSmallHBrushHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushDotsLargeH, m_xtpBrushDotsLargeH, GetDotsLargeHBrushHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushDashH, m_xtpBrushDashH, GetDashHBrushHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushDotsSmallV, m_xtpBrushDotsSmallV, GetDotsSmallVBrushHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushDotsLargeV, m_xtpBrushDotsLargeV, GetDotsLargeVBrushHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushDashV, m_xtpBrushDashV, GetDashVBrushHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushVeriticalGrid, m_xtpBrushVeriticalGrid, GetVeriticalGridBrushHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CBrush, m_brushHorizontalGrid, m_xtpBrushHorizontalGrid, GetHorizontalGridBrushHandle);
	
	XTPReportGridStyle m_nGridLineStylePreviousV;	// Stores previous style for vertical   grid lines. Used for update brush.
	XTPReportGridStyle m_nGridLineStylePreviousH;	// Stores previous style for horizontal grid lines. Used for update brush.
