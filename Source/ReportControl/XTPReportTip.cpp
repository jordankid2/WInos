...
BOOL CXTPReportTip::Create(CWnd* pParentWnd)
{
	...
	return CWnd::CreateEx(WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE, AfxRegisterWndClass(0, AfxGetApp()->LoadStandardCursor(IDC_ARROW)), NULL, WS_POPUP,
...
void CXTPReportTip::OnPaint()
{
	...
	CWnd* pToolTip = m_pReportControl->GetToolTipContext()->CreateToolTip(m_pParentWnd);
	if (NULL != pToolTip)
	{
		pToolTip->SetWindowPos(&CWnd::wndTop, 0, 0, rc.Width(), rc.Height(),
			SWP_HIDEWINDOW | SWP_NOZORDER | SWP_NOACTIVATE
			| SWP_NOOWNERZORDER | SWP_NOREDRAW | SWP_NOMOVE | SWP_NOCOPYBITS);
		bToolipDrawn = (TRUE == pToolTip->SendMessage(WM_PRINTCLIENT,
...
void CXTPReportTip::SetFont(CFont* pFont)
{
	ASSERT_VALID(pFont);
