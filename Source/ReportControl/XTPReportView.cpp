...
void CXTPReportView::SetTheme(XTPReportPaintTheme paintTheme, BOOL bEnableMetrics)
{
	switch(paintTheme)
	{
	...
	GetReportCtrl().SetTheme(paintTheme, bEnableMetrics);
}
...
void CXTPReportView::UpdateScrollBars()
{
	...
	BOOL bV = (m_pScrollBarVer != NULL)
		&& (m_pScrollBarVer->GetSafeHwnd() != NULL)
		&& 0 != (m_pScrollBarVer->GetStyle() & WS_VISIBLE);
	BOOL bH = (m_pScrollBarHor != NULL)
		&& (m_pScrollBarHor->GetSafeHwnd() != NULL)
...
void CXTPReportView::OnShowWindow(BOOL bShow, UINT nStatus)
{
...
int CXTPReportView::PrintFixedRowsDivider(CDC* pDC, const CRect& rc, BOOL bHeaderRows)
{
	...
	CXTPPaintManagerColor dividerColor = bHeaderRows
		...
		: GetReportCtrl().m_pSectionFooter->GetDividerColor();
	...
	int nRowFrom, int nColumnFrom, int nColumnTo)
{
	...
	rcReport.top    += rcPageHeader.Height() + XTP_DPI_Y(2);
	rcReport.bottom -= rcPageFooter.Height() + XTP_DPI_Y(2);
