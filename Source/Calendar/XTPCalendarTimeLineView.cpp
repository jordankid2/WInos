...
BOOL CXTPCalendarTimeLineView::HitTestEx(CPoint pt, XTP_CALENDAR_HITTESTINFO_TIMELINE_VIEW* pHitTest) const
{
...
void CXTPCalendarTimeLineView::RefreshMetrics()
{
	...
	else
	{
		...
		m_clrEventBarLine        = ::GetSysColor(COLOR_INACTIVECAPTION);
		m_clrTimeScaleBackground = ::GetSysColor(COLOR_BTNFACE);
		m_clrTimeScaleBorder     = ::GetSysColor(COLOR_3DDKSHADOW);
		m_clrTimeScaleHighlight  = ::GetSysColor(COLOR_HIGHLIGHT);
		m_clrTimeScaleText       = ::GetSysColor(COLOR_WINDOWTEXT);
		m_clrText                = ::GetSysColor(COLOR_WINDOWTEXT);
		m_clrBackground          = ::GetSysColor(COLOR_WINDOW);
		m_clrSelectedText        = ::GetSysColor(COLOR_HIGHLIGHTTEXT);
		m_clrSelectedBackground  = ::GetSysColor(COLOR_3DSHADOW);
	}
}
