...
void CXTPCalendarEventCategoryComboBox::DrawCollapsed(CDC* pDC, CRect rcItem, BOOL bFocused, BOOL bDisabled)
{
	if (!m_pCategories)
		return;

	// fill the background.
	pDC->FillSolidRect(&rcItem, ::GetSysColor(bDisabled ? COLOR_3DFACE : bFocused ? COLOR_HIGHLIGHT : COLOR_WINDOW));

	// determine the size of the color rectangle.
	CRect rColor(CalcColorRect(rcItem));

	pDC->SetBkColor(::GetSysColor(bDisabled ? COLOR_3DFACE : bFocused ? COLOR_HIGHLIGHT : COLOR_WINDOW));
	pDC->SetTextColor(::GetSysColor(bDisabled ? COLOR_GRAYTEXT : bFocused ? COLOR_HIGHLIGHTTEXT : COLOR_WINDOWTEXT));
	...
	for (int i = 0; i < nSize; i++)
	{
		...
		if (m_nTheme == xtpCalendarThemeResource)
		{
			CXTPCalendarTheme::DrawRoundRect(pDC, rColor,
				pCategory->GetBorderColor(), FALSE, pCategory->GetBackgroundColor());
		}
		else
		{
...
void CXTPCalendarEventCategoryComboBox::DrawExpanded(CDC* pDC, CRect rcItem, BOOL bSelected, UINT uItemID)
{
	if (uItemID == (UINT)-1)
		return;
	...
	CXTPCalendarEventCategory* pCategory = (CXTPCalendarEventCategory*)GetItemDataPtr(XTPToIntChecked(uItemID));
	...
	if (bChecked)
	{
		XTPPrimitiveDrawer()->DrawSymbol(pDC,
			xtpPrimitiveSymbolCheckMark, rCheck, ::GetSysColor(bSelected ? COLOR_HIGHLIGHTTEXT : COLOR_WINDOWTEXT));
	}
	...
	if (m_nTheme == xtpCalendarThemeResource)
	{
		if (bSelected)
		{
			...
			CXTPCalendarTheme::DrawRoundRect(pDC, rBorder, ::GetSysColor(COLOR_HIGHLIGHTTEXT), FALSE,
				CBrush::FromHandle((HBRUSH)GetStockObject(LTGRAY_BRUSH)));
		}
...
void CXTPCalendarEventCategoryComboBox::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	{
		DrawCollapsed(CDC::FromHandle(lpDIS->hDC), lpDIS->rcItem,
			(GetFocus() == this), ((lpDIS->itemState & ODS_DISABLED) == ODS_DISABLED));
	}
	else
	{
...
void CXTPCalendarBusyStatusComboBox::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	switch (itemID)
	{
		...
		{
			CXTPDrawHelpers::DrawColorRect(pDC, rColor, RGB(255, 255, 255), bDarkEdge ? crDarkEdge : crLight);
			break;
		}
		...
		case xtpCalendarBusyStatusElsewhere:
		{
			...
			CXTPDrawHelpers::DrawBrushRect(pDC, rColor, RGB(255, 255, 255), bDarkEdge ? crDarkEdge : crLight, bitsBrush);
			break;
		}
...
BOOL CXTPCalendarEventLabelComboBox::Init(CXTPCalendarEvent* pEvent, XTPCalendarTheme nTheme)
{
	...
	CXTPCalendarEventLabels* pLabels = XTP_SAFE_GET2(pEvent, GetDataProvider(), GetLabelList(), NULL);
	if (!pLabels)
		return FALSE;
...
void CXTPCalendarEventLabelComboBox::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (pLabel)
	{
		...
		COLORREF crEdge = bDarkEdge ? ::GetSysColor(COLOR_3DDKSHADOW) : CXTPDrawHelpers::DarkenColorLum(pLabel->m_clrColor, 0.5);
		...
		// draw text.
		pDC->SetBkMode(TRANSPARENT);
		pDC->DrawText(pLabel->m_strName, rText, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
	}
}
		...
		COleDateTime dtInitial)
{
	...
	m_nTheme = XTP_SAFE_GET2(m_ptrControl, GetPaintManager(), GetPaintTheme(), xtpCalendarThemeUnknown);
...
{
	CXTPCalendarEventCategories* pCategories = XTP_SAFE_GET2(m_ptrEditingEvent, GetDataProvider(), GetEventCategories(), NULL);
	if (!pCategories)
	{
		ASSERT(0);
		return;
	...
	CXTPCalendarEventCategoryIDs* pChecked = XTP_SAFE_GET1(m_ptrEditingEvent, GetCategories(), NULL);
	if (!pChecked)
	{
		ASSERT(0);
		return;
...
{
	m_wndLabelCombo.EnableWindow(
		m_wndCategoryCombo.GetCategoryArray().GetSize() == 0);
}
...
void CXTPCalendarEventPropertiesDlg::InitReminderComboBox()
{
	...
	{
		m_wndReminderCombo.SetItemData(m_wndReminderCombo.AddString(LoadString(XTP_IDS_CALENDAR_MINUTES_0)), NULL);
		m_wndReminderCombo.SetItemData(m_wndReminderCombo.AddString(LoadString(XTP_IDS_CALENDAR_MINUTES_1)), NULL);
		m_wndReminderCombo.SetItemData(m_wndReminderCombo.AddString(LoadString(XTP_IDS_CALENDAR_MINUTES_5)), NULL);
		m_wndReminderCombo.SetItemData(m_wndReminderCombo.AddString(LoadString(XTP_IDS_CALENDAR_MINUTES_10)), NULL);
		m_wndReminderCombo.SetItemData(m_wndReminderCombo.AddString(LoadString(XTP_IDS_CALENDAR_MINUTES_15)), NULL);
	}
