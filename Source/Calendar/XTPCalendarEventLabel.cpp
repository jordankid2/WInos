...
CXTPCalendarEventCategory::CXTPCalendarEventCategory(UINT nID, LPCTSTR pcszName, COLORREF clrBorder, COLORREF clrBkBase)
{
	m_nID = nID;
	m_strName = pcszName;
...
CXTPCalendarEventCategories::CXTPCalendarEventCategories() : m_bDefaultValues(TRUE)
{
	InitDefaultValues();
...
void CXTPCalendarEventCategories::InitDefaultValues()
{
	...
	Add(pEvCat);
	//---------------------------------------
	pEvCat = new CXTPCalendarEventCategory(xtpCalendarEventCategoryPurple, LoadCategoryName(XTP_IDS_CLR_PURPLE),
...
void CXTPCalendarEventCategories::UpdateDefaultValues(COLORREF* pClrBkBase, COLORREF* pClrBorder, int nCount)
{
	if (m_bDefaultValues)
	{
		if (nCount != GetCount())
...
void CXTPCalendarEventCategories::OleSetItem(long nIndex, LPDISPATCH pdispItem)
{
	...
	CXTPCalendarEventCategory* pCat = DYNAMIC_DOWNCAST(CXTPCalendarEventCategory, 
		CXTPCalendarEventCategory::FromIDispatchSafe(pdispItem));
	if (!pCat)
	{
