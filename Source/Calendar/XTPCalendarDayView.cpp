...
void CXTPCalendarDayView::ScrollH(int nPos, int nPos_raw)
{
	...
	int nXmax = XTPToIntChecked(si.nMax - si.nPage + 1);
...
BOOL CXTPCalendarDayView::GetScrollBarInfoV(SCROLLINFO* pSI) const
{
	...
	if (m_bShowAllDayEventsOnly)
	{
		...
		pSI->nPage = XTPToUIntChecked(m_LayoutX.m_nVisibleAllDayCount);
	...
	else
	{
		...
		pSI->nPage = XTPToUIntChecked(m_LayoutX.m_nVisibleRowCount);
...
BOOL CXTPCalendarDayView::GetScrollBarInfoH(SCROLLINFO* pSI, int* pnScrollStep) const
{
	...
	pSI->nMin = 0;
	pSI->nMax = nFullWidth;
	pSI->nPage = XTPToUIntChecked(rcClient.Width());
...
int CXTPCalendarDayView::CalculateHeaderFormatAndHeight(CDC* pDC, int nCellWidth)
{
	...
	const int cnMinHeight = XTP_DPI_Y(19);
	...
	int nHeight = max(cnMinHeight, sizeH.cy + XTP_DPI_Y(6));
...
void CXTPCalendarDayView::Draw(CDC* pDC)
{
	...
	for (i = nCount - 1; i >= 0; i--)
	{
		XTP_SAFE_CALL1(GetViewDay(i), DrawEvents(pDC));
	}
			...
			for (int nDayIdx = 0; nDayIdx < nDaysCount; nDayIdx++)
			{
				...
				for (int g = 0; g < nGroupsCount; g++)
				{
					...
					if (iAllDayCount > nVisAllDayEvents)
					{
						...
						rcIconsMax1.top = rcIconsMax1.bottom - XTP_DPI_Y(13);
						rcIconsMax1.bottom = rcIconsMax1.top + XTP_DPI_Y(9);
						rcIconsMax1.left = pViewGroup->GetAllDayEventsRect().right - XTP_DPI_X(17);
						rcIconsMax1.right = rcIconsMax1.left + XTP_DPI_X(14);
							...
							if (pViewEvent)
							{
								if (pViewEvent->IsMultidayEvent())
								{
									...
									if (rcEvent.PtInRect(pt))
									{
										...
										pt.x = rcEvent.left + XTP_DPI_X(10);
										pHitTest->pt = pt;
...
{
	static const int nVScrollZone = XTP_DPI_Y(20);
	static const int nHScrollZone = XTP_DPI_X(80);
	...
	BOOL b2Up = (m_ptLBtnDownMousePos.y - pnt.y) > XTP_DPI_Y(2);
	BOOL b2Down = (pnt.y - m_ptLBtnDownMousePos.y) > XTP_DPI_Y(2);
	if (b2Up
		&& pnt.y >= nTop
	...
	if (b2Down
		&& (pnt.y + XTP_DPI_Y(3)) < nHeight
		&& (pnt.y + XTP_DPI_Y(3)) >= (nHeight - nVScrollZone)
		&& nCurPos < nLimit)
	...
	BOOL b2Left = (m_ptLBtnDownMousePos.x - pnt.x) > XTP_DPI_X(2);
	BOOL b2Right = (pnt.x - m_ptLBtnDownMousePos.x) > XTP_DPI_X(2);
	...
	if (b2Right
		&& (pnt.x + XTP_DPI_X(3)) < rcVisible.right
		&& (pnt.x + XTP_DPI_X(3)) >= (rcVisible.right - nHScrollZone)
		&& nCurPosH < nLimitH)
...
BOOL CXTPCalendarDayView::IsOutOfClientRect(CPoint pnt, XTP_CALENDAR_HITTESTINFO* pHitInfo)
{
	...
	//bottom
	if (pnt.y + XTP_DPI_Y(3) >= rcCtrl.Height())
		return TRUE;
	//top
	CRect rcAllDayEvents = GetAllDayEventsRectangle();
...
BOOL CXTPCalendarDayView::HorzEventScroll(BOOL bLeft)
{
	...
	int nCurPos = si.nPos;
	int nCurPos_raw = si.nPos;
	int nPage = XTPToIntChecked(si.nPage);
...
//TRACE(_T("OnEndDragging\n"));
	...
	CRect rcCtrl;
	m_pControl->GetWindowRect(rcCtrl);
	BOOL bReturnBack = point.y + XTP_DPI_Y(3) >= rcCtrl.Height();
...
BOOL CXTPCalendarDayView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (HitTestEx(point, &hitTestInfo))
	{
		...
		{
			BOOL bFixBegin = 0 != (nFlags & MK_SHIFT);
...
void CXTPCalendarDayView::SelectDay(CXTPCalendarViewDay* /*pDay*/, int /*nGroupIndex*/)
{
	// Do nothing for day view.
}

void CXTPCalendarDayView::SelectDay(CXTPCalendarViewDay* /*pDay*/)
{
	// Do nothing for day view.
}
...
//TRACE(_T("nGroupIndex=%d %s nCurPos=%d\n"), m_selectedBlock.nGroupIndex, dtNewSelEnd.Format(_T("%d %m %Y")), nCurPos);
		...
		else
		{
			if (nChar == VK_LEFT)
			{
				...
				if(GetLayout().m_nTopAllDayRow > 0)
				{
					CXTPCalendarControl::CUpdateContext updateContextAll(GetCalendarControl(), xtpCalendarUpdateAll);
					GetLayout().m_nTopAllDayRow = 0;
				}
			}
			else if (nChar == VK_RIGHT)
			{
...
BOOL CXTPCalendarDayView::_EnsureVisibleSelectionH()
{
	...
	{
		nNewScrollPos += rcGroup.right - rcVisible.right + XTP_DPI_X(1);
	}
...
BEGIN_INTERFACE_MAP(CXTPCalendarDayView, CXTPCalendarView)
	INTERFACE_PART(CXTPCalendarDayView, XTPDIID_CalendarDayView, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarDayView, XTPDIID_CalendarDayView)
...
BOOL CXTPCalendarDayView::OleEnsureVisible(LPDISPATCH pdispViewEvent)
{
	CXTPCalendarViewEvent* pViewEvent = DYNAMIC_DOWNCAST(CXTPCalendarViewEvent, 
		CXTPCalendarViewEvent::FromIDispatchSafe(pdispViewEvent));
	if (!pViewEvent)
	{
		AfxThrowOleException(E_INVALIDARG);
	}
...
{
	CXTPCalendarViewGroup* pViewGroup = DYNAMIC_DOWNCAST(CXTPCalendarViewGroup, 
		CXTPCalendarViewGroup::FromIDispatchSafe(pdispViewGroup));
	if (!pViewGroup)
	{
		AfxThrowOleException(E_INVALIDARG);
	}
