...
#include "Calendar/Themes/XTPCalendarVisualStudio2015Theme.h"
...
CXTPCalendarControl::CXTPCalendarControl()
{
	...
	// default theme.
	SetPaintTheme(xtpCalendarThemeOffice2000);
...
void CXTPCalendarControl::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS* lpncsp)
{
	UNREFERENCED_PARAMETER(bCalcValidRects);

	if (m_pPaintManager)
	{
		RECT* prcWindow = &lpncsp->rgrc[0];
		prcWindow->left += m_Borders.left;
		prcWindow->top += m_Borders.top;
		prcWindow->right = max(prcWindow->left, prcWindow->right - m_Borders.right);
		prcWindow->bottom = max(prcWindow->top, prcWindow->bottom - m_Borders.bottom);
...
void CXTPCalendarControl::OnNcPaint()
{
	if (m_pPaintManager)
	{
		...
		m_pPaintManager->DrawBorders(&dc, rc, m_Borders);
		CWnd::OnNcPaint();
	}
	else
	{
...
void CXTPCalendarControl::UpdateBorders()
{
	if (::IsWindow(m_hWnd))
	{
...
void CXTPCalendarControl::SetBorders(int nLeft, int nTop, int nRight, int nBottom)
{
...
void CXTPCalendarControl::AdjustScrollBarEx(int nSBType)
{
	...
	::EnableScrollBar(m_hWnd, XTPToUInt(nSBType), 
		XTPToUInt((bEnabled && IsWindowEnabled()) ? ESB_ENABLE_BOTH : ESB_DISABLE_BOTH));
}
...
void CXTPCalendarControl::AdjustLayout()
{
	...
	if (m_pActiveView && m_pActiveView->GetScrollBarInfoH(&si))
	{
		...
		{
			rcView.left -= si.nPos;
			rcView.right = rcView.left + si.nMax + XTP_DPI_X(1) + m_pDayView->_GetTimeScaleWith();
		}
	}
...
void CXTPCalendarControl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	DWORD dwPoint = XTPToDWORD(MAKELONG(point.x, point.y));
...
void CXTPCalendarControl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	case SB_THUMBPOSITION:
		nCurPos = nCurPos_raw = XTPToIntChecked(nPos);
		break;
	case SB_ENDSCROLL:
...
BOOL CXTPCalendarControl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	if (m_nRowsPerWheel == -1)
	{
		...
		uiMsg = XTPToUInt(nPagesScrolled > 0 ? SB_PAGEUP : SB_PAGEDOWN);
		nScrollsCount = nPagesScrolled > 0 ? nPagesScrolled : -nPagesScrolled;
	}
	else
	{
		int nRowsScrolled = m_nRowsPerWheel * zDelta / 120;
		uiMsg = XTPToUInt(nRowsScrolled > 0 ? SB_LINEUP : SB_LINEDOWN);
		nScrollsCount = nRowsScrolled > 0 ? nRowsScrolled : -nRowsScrolled;
	}

	// post scroll messages
	for (int i = 0; i < nScrollsCount; i++)
	{
		PostMessage(XTPToUInt(m_bTimelineMode ? WM_HSCROLL : WM_VSCROLL), uiMsg, 0);
	}
	...
	if (m_pActiveView)
	{
		...
		BOOL bScrollEnabled = m_bTimelineMode
			? m_pActiveView->GetScrollBarInfoH(&si)
			: m_pActiveView->GetScrollBarInfoV(&si);
		...
		if (bScrollEnabled)
		{
				...
				nViewType == xtpCalendarWorkWeekView)
			{
				...
				GetScrollRange(
					m_bTimelineMode ? SB_HORZ : SB_VERT, &nPosMin, &nPosMax);
				
				nPosMax = GetScrollLimit(
					m_bTimelineMode ? SB_HORZ : SB_VERT);
...
{
	SendNotification(XTP_NC_CALENDAROPTIONSWASCHANGED, XTPToWPARAM(nOptionRelation), 0);
	...
	XTPCalendarDataProvider eDataProvider, LPCTSTR lpszConnectionString)
{
	...
	XTP_SAFE_CALL1(GetPaintManager(), UpdateEventLabelsDefaultColors());
	XTP_SAFE_CALL1(GetPaintManager(), UpdateEventCategoriesDefaultColors());
}
...
void CXTPCalendarControl::ShowToolTip(const CString& strText, const CRect rcToolTip, CFont* pFont, BOOL bAdvanced)
{
	...
	if (!bAdvanced)
	{
		rcTip.OffsetRect(XTP_DPI_X(7), 0);
	...
	{
		rcTip.DeflateRect(0, 0, XTP_DPI_X(7), 0);
//<<>>
...
void CXTPCalendarControl::OnCalendarTimeline()
{
	...
	if (m_bTimelineMode)
	{
		...
		else if (viewtype == xtpCalendarWeekView || viewtype == xtpCalendarFullWeekView)
		{
...
//TRACE(pTLView->GetStartViewDate().Format(_T("OnScrollEnd %d %m %Y\n")));
	...
	// when timeline view is toggled, reposition scrollbars.	
...
//-- Fix for no selection case!
	...
	BOOL bHitTestOk = GetActiveView()->HitTest(pos, &hitInfo);
//-- Fix for no selection case!
	...
	// forward message to subscriber
	DWORD dwPoint = XTPToDWORD(MAKELONG(ptClient.x, ptClient.y));
...
BOOL CXTPCalendarControl::DoDeleteEvent(CXTPCalendarViewEvent* pViewEvent)
{
	...
	{
		int iShId = XTPToIntChecked(pViewEvent->GetEvent()->GetScheduleID());
			...
			if (pSch)
			{
				...
				if (!m_OwnerUser.IsEmpty())
				{
					if (m_OwnerUser == sName)
						iShID = XTPToIntChecked(pSch->GetID());
					else if (sName == _T("Public"))
						iShIDPublic = XTPToIntChecked(pSch->GetID());
...
void CXTPCalendarControl::SetTheme(CXTPCalendarTheme* pTheme)
{
	...
	SendNotification(XTP_NC_CALENDAR_THEME_CHANGED, (WPARAM)this, 0);
}
...
void CXTPCalendarControl::SetLayoutRTL(BOOL bRightToLeft)
{
	...
	ModifyStyleEx(XTPToDWORD(bRightToLeft ? 0 : WS_EX_LAYOUTRTL), 
		XTPToDWORD(!bRightToLeft ? 0 : WS_EX_LAYOUTRTL));
...
BOOL CXTPCalendarControl::IsLayoutRTL() const
{
	...
	return ((CWnd::GetExStyle() & WS_EX_LAYOUTRTL) != 0);
}
...
XTPCalendarThemeVersion CXTPCalendarControl::GetThemeVersion() const
{
	...
	case xtpCalendarThemeVisualStudio2015:
		themeVersion = xtpCalendarVersion2015;
		break;
...
void CXTPCalendarControl::SetPaintTheme(CXTPCalendarPaintManager* pPaintManager, CXTPCalendarTheme* pTheme)
{
	if (pPaintManager == NULL)
		pPaintManager = new CXTPCalendarOffice2000Theme();
...
void CXTPCalendarControl::SetPaintTheme(XTPCalendarTheme ePaintTheme)
{
	...
	{
	case xtpCalendarThemeOfficeXP:
		SetPaintTheme(new CXTPCalendarOfficeXPTheme, NULL);
		break;

	case xtpCalendarThemeOffice2003:
		SetPaintTheme(new CXTPCalendarOffice2003Theme, NULL);
		break;

	case xtpCalendarThemeOffice2013:
		SetPaintTheme(new CXTPCalendarOffice2013Theme, NULL);
		break;

	case xtpCalendarThemeVisualStudio2015:
		SetPaintTheme(new CXTPCalendarVisualStudio2015Theme, NULL);
		break;
	...
	case xtpCalendarThemeResource:
		{
			if (!m_pTheme)
			{
				SetPaintTheme(new CXTPCalendarResourceTheme, new CXTPCalendarThemeOffice2007);
			}
			else
			{
	...
	case xtpCalendarThemeOffice2000:
	default:
		SetPaintTheme(new CXTPCalendarOffice2000Theme, NULL);
		break;
	}
