...
{
	CXTPCalendarEventColorListBox* pOwnerWnd = DYNAMIC_DOWNCAST(CXTPCalendarEventColorListBox, GetOwner());
	if (pOwnerWnd)
	{
		switch (pMsg->wParam)
		{
...
void CXTPCalendarEventColorListBox::Init(CXTPCalendarData* pData, CXTPCalendarEvent* pEvent, BOOL bCategories)
{
	m_pData = pData;
	ASSERT(m_pData);
...
void CXTPCalendarEventColorListBox::DrawItem(LPDRAWITEMSTRUCT lpDIS) 
{
	...
	pDC->SetBkColor(::GetSysColor(bDisabled ? COLOR_3DFACE : bSelected ? COLOR_HIGHLIGHT : COLOR_WINDOW));
		...
		if (m_pEvent)
		{
			...
			if (bChecked)
			{
				XTPPrimitiveDrawer()->DrawSymbol(pDC,
					xtpPrimitiveSymbolCheckMark, rCheck, ::GetSysColor(bDisabled ? COLOR_GRAYTEXT : COLOR_WINDOWTEXT));
			}
	...
	CXTPDrawHelpers::DrawColorRect(pDC, rColor, bDisabled ? CXTPDrawHelpers::GreyScaleColor(clrBack) : clrBack,
...
void CXTPCalendarEventColorListBox::BeginEdit()
{
	...
	if (::IsWindow(m_hWnd))
	{
		...
		if (m_pWndEdit->Create(WS_VISIBLE|WS_CHILD|WS_BORDER|ES_LEFT|ES_AUTOHSCROLL, rEdit, this, 0xff))
		{
...
int CXTPCalendarEventColorListBox::AddItem()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		if (m_bCategories)
		{
			SetItemData(nItem, XTPToUIntPtr(new CXTPCalendarEventCategory(XTPToUIntChecked(nItem + 1),
				dlg.m_strName, CXTPDrawHelpers::DarkenColorLum(dlg.m_crColor, 0.5), dlg.m_crColor)));
		...
		{
			SetItemData(nItem, XTPToUIntPtr(new CXTPCalendarEventLabel(
				nItem, dlg.m_crColor, dlg.m_strName)));
...
void CXTPCalendarEventColorListBox::UpdateCategories(CXTPCalendarData* pData)
{
	...
	if (IsModified())
	{
		...
		for (int i = 0; i < GetCount(); i++)
		{
			...
			if (pCategory)
			{
				pData->AddEventCategory(pCategory->GetID(),
					pCategory->GetName(), pCategory->GetBorderColor(), pCategory->GetBkBaseColor());
			}
		}
	}
...
void CXTPCalendarEventColorListBox::UpdateLabels(CXTPCalendarData* pData)
{
	...
	for (int i = 0; i < GetCount(); i++)
	{
		...
		if (pLabel)
		{
			if (IsModified())
			{
				pData->AddEventLabel(pLabel->m_nLabelID,
					pLabel->m_strName, pLabel->m_clrColor);
			}
