...
void AFX_CDECL CXTPCalendarReminderForOccurrenceArray::ClearProperties(CXTPCalendarCustomProperties* pProps)
{
	ASSERT(pProps);
	if (!pProps)
	{
...
void AFX_CDECL CXTPCalendarReminderForOccurrenceArray::_PropName(LPCTSTR pcszProp, int nIndex, CString& rstrPropNameI)
{
	TCHAR szIndex[64];
...
#define DISP_OPTION(szExternalName, dispid, vtPropType) \
	DISP_PROPERTY_EX_ID(CXTPCalendarRecurrencePatternOptions, #szExternalName, dispid, OleGet##szExternalName, OleSet##szExternalName, vtPropType)
...
BEGIN_INTERFACE_MAP(CXTPCalendarRecurrencePatternOptions, CXTPCmdTarget)
	INTERFACE_PART(CXTPCalendarRecurrencePatternOptions, XTPDIID_CalendarRecurrencePatternOptions, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarRecurrencePatternOptions, XTPDIID_CalendarRecurrencePatternOptions)



BEGIN_DISPATCH_MAP(CXTPCalendarRecurrencePattern, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPCalendarRecurrencePattern, "StartTime", 1, OleGetStartTime, OleSetStartTime, VT_DATE)
	DISP_PROPERTY_ID(CXTPCalendarRecurrencePattern, "DurationMinutes", 2, m_nDurationMinutes, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPCalendarRecurrencePattern, "StartDate", 3, OleGetStartDate, OleSetStartDate, VT_DATE)
	DISP_PROPERTY_EX_ID(CXTPCalendarRecurrencePattern, "EndDate", 4, OleGetEndDate, OleSetEndDate, VT_DATE)
	DISP_PROPERTY_EX_ID(CXTPCalendarRecurrencePattern, "EndAfterOccurrences", 5, OleGetEndAfterOccurrences, OleSetEndAfterOccurrences, VT_I4)
	DISP_PROPERTY_ID(CXTPCalendarRecurrencePattern, "EndMethod", 6, m_PatternEnd.m_nUseEnd, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPCalendarRecurrencePattern, "MasterEvent", 8, OleGetMasterEvent, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarRecurrencePattern, "Options", 9, OleGetOptions, SetNotSupported, VT_DISPATCH)

	DISP_FUNCTION_ID(CXTPCalendarRecurrencePattern, "Exceptions", 10, OleGetExceptions, VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarRecurrencePattern, "SetException", 11, OleSetException, VT_EMPTY, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPCalendarRecurrencePattern, "RemoveException", 12, OleRemoveException, VT_EMPTY, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPCalendarRecurrencePattern, "RemoveAllExceptions", 13, OleRemoveAllExceptions, VT_EMPTY, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarRecurrencePattern, "CustomProperties", 14, OleCustomProperties, VT_DISPATCH, VTS_NONE)
...
{
	CXTPCalendarRecurrencePatternOptions* pOptions = new CXTPCalendarRecurrencePatternOptions(m_Options);
...
{
	CXTPCalendarEvent* pExc = DYNAMIC_DOWNCAST(CXTPCalendarEvent, 
		CXTPCalendarEvent::FromIDispatchSafe(pExcEventDisp));
	if (!pExc)
	{
