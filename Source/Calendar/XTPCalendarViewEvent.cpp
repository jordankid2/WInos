...
CXTPCalendarViewEventSubjectEditorColored::CXTPCalendarViewEventSubjectEditorColored(
	CXTPCalendarControl* pOwner,
	CXTPCalendarViewEvent* pViewEvent, COLORREF clrBackground)
...
BOOL CXTPCalendarViewEvent::MarkupHandleMouseEvent(UINT nMessage, UINT nFlags, const CPoint& point)
{
	...
	{
		LRESULT lUnused = 0;
		bHandled = XTPMarkupRelayMessage(
			m_pUIElement,
			nMessage,
			nFlags,
...
void CXTPCalendarViewEvent::OnPostAdjustLayout()
{
	if (IsMultidayEvent())
	{
		...
		m_rcText.left += min(m_rcText.Width() - XTP_DPI_X(1), m_rcFirstClock.Width() + XTP_DPI_X(4));
		m_rcText.right -= min(m_rcText.Width() - XTP_DPI_X(1), m_rcLastClock.Width() + XTP_DPI_X(4));

		int nGlyphs = CalcIconsRect(m_rcEvent);
		CRect rcGlyphs(max(m_rcText.CenterPoint().x - (m_szText.cx/2 + nGlyphs + XTP_DPI_X(3)), m_rcText.left),
...
{
	int nEventHeight = XTP_SAFE_GET3(GetViewGroup_(), GetViewDay_(), GetView_(), GetRowHeight(), XTP_DPI_Y(1));
...
int CXTPCalendarViewEvent::CalcIconsRect(CRect rc)
{
	...
	if (pEvent)
	{
		...
		{
			nMinSize = CalcIconRect(
				rc, szIcon, pt, m_rcReminderGlyph);
		}
		if (pEvent->IsPrivate())
		{
			nMinSize = CalcIconRect(
				rc, szIcon, pt, m_rcPrivateGlyph);
		}
		if (pEvent->GetRecurrenceState() > xtpCalendarRecurrenceNotRecurring)
		{
			nMinSize = CalcIconRect(
				rc, szIcon, pt, m_rcRecurrenceGlyph);
		}
		if (pEvent->IsMeeting())
		{
			nMinSize = CalcIconRect(
				rc, szIcon, pt, m_rcMeetingGlyph);
		}
	}
...
BEGIN_DISPATCH_MAP(CXTPCalendarViewEvent, CXTPCalendarWMHandler)
	DISP_PROPERTY_EX_ID(CXTPCalendarViewEvent, "Event", DISPID_VALUE, OleGetEvent, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarViewEvent, "Selected", 1, OleGetSelected, OleSetSelected, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPCalendarViewEvent, "ViewDay", 2, OleGetViewDay, SetNotSupported, VT_DISPATCH)
...
BEGIN_DISPATCH_MAP(CXTPCalendarViewEvents, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPCalendarViewEvents, "ViewEvent", DISPID_VALUE, OleGetItem, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPCalendarViewEvents, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
END_DISPATCH_MAP()
