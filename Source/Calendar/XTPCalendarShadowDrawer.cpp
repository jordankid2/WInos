...
{
	return RGB(
		CheckValue(XTPToInt(GetRValue(crPixel)-i)),
	...
	if ((bmi.bmiHeader.biWidth > 0) && (bmi.bmiHeader.biHeight > 0))
	{
		...
		if (hbitmap)
		{
				...
				m_pDC->GetSafeHdc(), rcShadow.left, rcShadow.top, SRCCOPY))
			{
				...
				if (pvBits)
				{
					m_pDC->BitBlt(rcShadow.left, rcShadow.top, bmi.bmiHeader.biWidth, bmi.bmiHeader.biHeight, CDC::FromHandle(hdc), 0, 0, SRCCOPY);
				}
			}
			// do cleanup
...
void CXTPCalendarShadowDrawer::ApplyShadow2007BR(UINT* pBitmap, const ULONG ulBitmapWidth, const ULONG ulBitmapHeight)
{
	...
	const int sz = 5;
	static const UINT uiShadowBL[sz][sz] =
	{
		{213,215,219,224,230},
		{217,219,223,224,233},
		{219,221,225,221,241},
...
void CXTPCalendarShadowDrawer::ApplyShadow2007Center(UINT* pBitmap, const ULONG ulBitmapWidth, const ULONG ulBitmapHeight)
{
	if (!pBitmap)
		return;
