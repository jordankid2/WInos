	...
	, m_bVisible(bVisible)
{
	...
	m_nWidth = XTP_DPI_X(48); // default value for default WinXP settings.
...
void CXTPCalendarDayViewTimeScale::DrawLTR(CDC* pDC)
{
	...
	else
	{
		...
		if(nDrawCellCount == 1 && rcStartCell.top < m_Layout.m_rcTimeZone.top)
			nDrawCellCount++;
...
CString CXTPCalendarDayViewTimeScale::Format(COleDateTime dtTime, BOOL bTopCell, CString* pHour, CString* pMin)
{
	...
	CString strMin = dtTime.Format(_T("%M"));
	...
	return strHour + m_timeFormat.strSeparator + strMin;
}
