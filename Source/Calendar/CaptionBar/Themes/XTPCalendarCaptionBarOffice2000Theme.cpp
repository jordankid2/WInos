// XTPCalendarCaptionBarOffice2000Theme.cpp : implementation of the CXTPCalendarCaptionBarOffice2000Theme class.
//
...
CXTPCalendarCaptionBarOffice2000Theme::CXTPCalendarCaptionBarOffice2000Theme(CXTPCalendarCaptionBarTheme* pTheme /*=NULL*/)
...
void CXTPCalendarCaptionBarOffice2000Theme::PartAdjustLayout(CXTPSwitchViewButtonPart* pBarPart, CDC* pDC, const CRect& rcRect)
{
	ASSERT(pBarPart);
	if (!pBarPart)
		return;
...
void CXTPCalendarCaptionBarOffice2000Theme::PartAdjustLayout(CXTPScrollDateButtonPart* pBarPart, CDC* pDC, const CRect& rcRect)
{
...
void CXTPCalendarCaptionBarOffice2000Theme::PartAdjustLayout(CXTPRadioButtonPart* pBarPart, CDC* pDC, const CRect& rcRect)
{
	ASSERT(pBarPart);
	if (!pBarPart)
		return;
	...
	if (pDC && pDC->GetSafeHdc())
	{
		...
		if (szRadio.cx + XTP_DPI_X(9) + szText.cx + XTP_DPI_X(8) > pBarPart->m_rcRect.Width())
			pBarPart->m_rcRect.right = pBarPart->m_rcRect.left + szRadio.cx + XTP_DPI_X(9) + szText.cx + XTP_DPI_X(8);

		szRadio.cy = max(szRadio.cy, szText.cy);
	}
}

void CXTPCalendarCaptionBarOffice2000Theme::PartAdjustLayout(CXTPCheckboxButtonPart* pBarPart, CDC* pDC, const CRect& rcRect)
{
	ASSERT(pBarPart);
...
void CXTPCalendarCaptionBarOffice2000Theme::PartAdjustLayout(CXTPExpandButtonPart* pBarPart, CDC* pDC, const CRect& rcRect)
{
	UNREFERENCED_PARAMETER(pDC);
	...
	pBarPart->m_rcRect.top = pBarPart->m_rcRect.top + pBarPart->m_rcRect.Height() / 2 - szButton.cy / 2;
	pBarPart->m_rcRect.bottom = pBarPart->m_rcRect.top + szButton.cy;
}
...
void CXTPCalendarCaptionBarOffice2000Theme::PartDraw(CXTPSwitchViewButtonPart* pBarPart, CDC* pDC)
{
	...
	UINT uFlags = XTPToUInt(DFCS_BUTTONPUSH 
...
void CXTPCalendarCaptionBarOffice2000Theme::PartDrawScrollTriangle(CXTPScrollDateButtonPart* pBarPart, CDC* pDC, CRect rcRect, BOOL bLeftDirection, COLORREF clrColor)
{
	ASSERT(pDC);
	if (!pDC || !pDC->GetSafeHdc())
		return;
	...
	XTPPrimitiveDrawer()->DrawSymbol(pDC, bLeftDirection ? xtpPrimitiveSymbolPlayLeft : xtpPrimitiveSymbolPlayRight, rcRect, clrColor);
}
...
void CXTPCalendarCaptionBarOffice2000Theme::PartDraw(CXTPRadioButtonPart* pBarPart, CDC* pDC)
{
	...
	UINT uFlags = XTPToUInt(DFCS_BUTTONRADIO
		| ((pBarPart->m_nState & xtpBtnStateChecked) ? DFCS_CHECKED : 0)
		| ((pBarPart->m_nState & xtpBtnStatePressed) ? DFCS_PUSHED : 0));
...
void CXTPCalendarCaptionBarOffice2000Theme::PartDraw(CXTPCheckboxButtonPart* pBarPart, CDC* pDC)
{
	...
	UINT uFlags = XTPToUInt(DFCS_BUTTONCHECK
		| ((pBarPart->m_nState & xtpBtnStateChecked) ? DFCS_CHECKED : 0)
		| ((pBarPart->m_nState & xtpBtnStatePressed) ? DFCS_PUSHED : 0));
...
void CXTPCalendarCaptionBarOffice2000Theme::PartDraw(CXTPDateLabelPart* pBarPart, CDC* pDC)
{
	...
	COLORREF clrText = m_pTheme->IsOneLine() ? GetSysColor(COLOR_BTNTEXT) : pBarPart->m_clrTextColor;
...
void CXTPCalendarCaptionBarOffice2000Theme::PartDrawExpandTriangle(CXTPExpandButtonPart* pBarPart, CDC* pDC, CRect rcRect, BOOL bDownDirection, COLORREF clrColor)
{
	ASSERT(pDC);
	if (!pDC || !pDC->GetSafeHdc())
		return;
