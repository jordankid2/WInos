// XTPCalendarCaptionBarOffice2003Theme.cpp : implementation of the CXTPCalendarCaptionBarOfficeXPTheme class.
//
...
CXTPCalendarCaptionBarOffice2003Theme::CXTPCalendarCaptionBarOffice2003Theme(CXTPCalendarCaptionBarTheme* pTheme /*=NULL*/)
...
void CXTPCalendarCaptionBarOffice2003Theme::PartDraw(CXTPSwitchViewButtonPart* pBarPart, CDC* pDC)
{
	...
	{
		int nState = (pBarPart->m_nState & xtpBtnStatePressed) ? PBS_PRESSED : (pBarPart->m_nState & xtpBtnStateMouseOver) ? PBS_HOT: PBS_NORMAL;
...
void CXTPCalendarCaptionBarOffice2003Theme::PartDraw(CXTPScrollDateButtonPart* pBarPart, CDC* pDC)
{
	...
	if (m_pThemeButton->IsAppThemeActive())
	{
		int nState = (pBarPart->m_nState & xtpBtnStatePressed) ? PBS_PRESSED :
		(pBarPart->m_nState & xtpBtnStateMouseOver) ? PBS_HOT: PBS_NORMAL;
		
		m_pThemeButton->DrawThemeBackground(
			pDC->GetSafeHdc(), BP_PUSHBUTTON, nState, rcRect, NULL);
	}
	else
	{
		UINT uFlags = XTPToUInt(DFCS_BUTTONPUSH
...
void CXTPCalendarCaptionBarOffice2003Theme::PartDraw(CXTPRadioButtonPart* pBarPart, CDC* pDC)
{
	...
	if (bThemed)
	{
		...
		int nState = (pBarPart->m_nState & xtpBtnStatePressed) ? RBS_UNCHECKEDPRESSED :
				(pBarPart->m_nState & xtpBtnStateMouseOver) ? RBS_UNCHECKEDHOT : RBS_UNCHECKEDNORMAL;

		if (pBarPart->m_nState & xtpBtnStateChecked)
			nState += 4;

		m_pThemeButton->DrawThemeBackground(pDC->GetSafeHdc(), BP_RADIOBUTTON, nState, rcRadio, NULL);
	}
	else
	{
		UINT uFlags = XTPToUInt(DFCS_BUTTONRADIO | DFCS_FLAT
			| ((pBarPart->m_nState & xtpBtnStateChecked) ? DFCS_CHECKED : 0)
			| ((pBarPart->m_nState & xtpBtnStatePressed) ? DFCS_PUSHED : 0));
...
void CXTPCalendarCaptionBarOffice2003Theme::PartDraw(CXTPCheckboxButtonPart* pBarPart, CDC* pDC)
{
	...
	{
		UINT uFlags = XTPToUInt(DFCS_BUTTONCHECK | DFCS_FLAT
			| ((pBarPart->m_nState & xtpBtnStateChecked) ? DFCS_CHECKED : 0)
			| ((pBarPart->m_nState & xtpBtnStatePressed) ? DFCS_PUSHED : 0));
