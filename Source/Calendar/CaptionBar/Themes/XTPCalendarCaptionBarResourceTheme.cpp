// XTPCalendarCaptionBarResourceTheme.cpp : implementation of the CXTPCalendarCaptionBarOffice2013Theme class.
//
...
CXTPCalendarCaptionBarResourceTheme::CXTPCalendarCaptionBarResourceTheme(CXTPCalendarCaptionBarTheme* pTheme /*=NULL*/)
...
void CXTPCalendarCaptionBarResourceTheme::PartAdjustLayout(CXTPExpandButtonPart* pBarPart, CDC* pDC, const CRect& rcRect)
{
	ASSERT(pDC);
	ASSERT(pBarPart);
	...
	if (pImage)
	{
		...
		pBarPart->m_rcRect.top = pBarPart->m_rcRect.top + pBarPart->m_rcRect.Height() / 2 - rcImage.Height() / 2;
...
void CXTPCalendarCaptionBarResourceTheme::PartDraw(CXTPScrollDateButtonPart* pBarPart, CDC* pDC)
{
	...
	if (pImage)
	{
		int nImagePart = (pBarPart->m_nState & xtpBtnStatePressed) ? 2 : (pBarPart->m_nState & xtpBtnStateMouseOver) ? 1 : 0;
		...
		{
			if (pImage->IsAlphaImage())
				pImage->DrawImage(pDC, pBarPart->m_rcRect, pImage->GetSource(3, 4), CRect(XTP_DPI_X(1), XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1)));
			else
				pImage->DrawImage(pDC, pBarPart->m_rcRect, pImage->GetSource(3, 4), CRect(XTP_DPI_X(1), XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1)), RGB(255, 0, 255));
		}
	}
	else
	{
