...
CRect CXTPCalendarMonthViewGroup::GetTooltipRect(const CPoint& ptHit, const XTP_CALENDAR_HITTESTINFO& hitInfo) const
{
...
void CXTPCalendarMonthViewDay::Draw(CDC* pDC)
{
	...
	XTP_SAFE_CALL3(GetView(), GetPaintManager(), GetMonthViewEventPart(),
		OnDrawDayDate(pDC, m_Layout.m_rcDayHeader, bToday, IsSelected(), m_dtDate.GetDay() == 1, strDate));
