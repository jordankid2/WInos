...
{
	return (m_xtpFontCustomValue.GetSafeHandle() == NULL) && (m_xtpFontStandardValue.GetSafeHandle() == NULL);
}
...
{
	return  (m_xtpFontCustomValue.GetSafeHandle() == 0) ? m_xtpFontStandardValue : m_xtpFontCustomValue;
}
...
{
	m_xtpFontCustomValue.DeleteObject();
	if (pLogFont) m_xtpFontCustomValue.CreateFontIndirect(pLogFont);
	return *this;
}
...
void CXTPCalendarViewPart::DrawText(CDC* pDC, const CString& str, LPRECT lpRect, UINT nFormat, COLORREF clrText /*= COLORREF_NULL*/, CFont* pFont /*= NULL*/)
{
	int nState = pDC->SaveDC();
	
	if (!pFont->GetSafeHandle()) { pFont = &GetTextFont(); }
	pDC->SelectObject(pFont);
	
	if (clrText == COLORREF_NULL) { clrText = GetTextColor(); }
...
void CXTPCalendarViewPart::DrawLine_CenterLR(CDC* pDC, const CString& str, LPRECT lpRect, UINT nFormat)
{
...
void CXTPCalendarViewPart::DrawLineEx(CDC* pDC, const CString& str, LPRECT lpRect, UINT nFormatNormal, UINT nFormatSmall)
{
	CFont* pOldFont = pDC->SelectObject(&GetTextFont());
	pDC->SetTextColor(GetTextColor());
	UINT nFormat = 0;
...
void CXTPCalendarThemeStringValue::DoPX_Value(CXTPPropExchange* pPX, LPCTSTR pcszPropName, CString& rXValue, BOOL bStandard)
{
	if (pPX->IsStoring())
	{
...
void CXTPCalendarThemeIntValue::DoPX_Value(CXTPPropExchange* pPX, LPCTSTR pcszPropName, int& rXValue, BOOL bStandard)
{
	if (pPX->IsStoring())
	{
		if (bStandard && !m_bIsStandardSet ||
...
void CXTPCalendarThemeBOOLValue::DoPX_Value(CXTPPropExchange* pPX, LPCTSTR pcszPropName, BOOL& rXValue, BOOL bStandard)
{
	if (pPX->IsStoring())
	{
		if (bStandard && !m_bIsStandardSet ||
...
void CXTPCalendarThemeRectValue::DoPX_Value(CXTPPropExchange* pPX, LPCTSTR pcszPropName, CRect& rXValue, BOOL bStandard)
{
...
void CXTPCalendarTimeLineViewTimeScalePart::DrawHeader(CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineView* pView, int nLabelInterval)
{
	...
	for (int nP = 0; nP < 100; nP++)
	{
		...
		XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayDown, rcTrangleRect, pView->m_clrTimeScaleText);
