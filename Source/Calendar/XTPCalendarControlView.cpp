	...
	: CPageSetupDialog(dwFlags, pParentWnd)
{
	...
	if (m_pOptions)
	{
		...
		DWORD dwMeasure = XTPToDWORD(bIsInches ? PSD_INTHOUSANDTHSOFINCHES : PSD_INHUNDREDTHSOFMILLIMETERS);
...
BOOL CXTPCalendarPageSetupDialog::OnInitDialog ()
{
	...
	UINT uStrID = XTPToUInt(bIsInches ? XTP_IDS_CALENDAR_MARGINS_INCH : XTP_IDS_CALENDAR_MARGINS_MM);
...
void CXTPCalendarControlView::SetScrollBarCtrl(CScrollBar* pScrollBar, int nBar /*=SB_VERT*/)
{
	if (nBar == SB_VERT)
...
BOOL CXTPCalendarControlView::OnEraseBkgnd(CDC* pDC)
{
	// fill small rectangle in bottom corner where horizontal / vertical scrollbars intersect.
	if (GetCalendarCtrl().m_bTimelineMode && ::IsWindow(m_pVScrollBar->GetSafeHwnd()))
	{
		...
		if (GetCalendarCtrl().IsLayoutRTL())
		{
			rcClient.right = rcClient.left + cxVScroll;
			rcClient.top = rcClient.bottom - cyHScroll;
		}
		else
		{
			rcClient.left = rcClient.right - cxVScroll;
			rcClient.top = rcClient.bottom - cyHScroll;
		}
		
		CXTPCalendarPaintManager* pPaintManager = GetCalendarCtrl().GetPaintManager();
		COLORREF crBack = pPaintManager ? pPaintManager->m_clrTLVBack : ::GetSysColor(COLOR_BTNFACE);
...
{
	if (nBar == SB_VERT && m_pVScrollBar)
		return m_pVScrollBar;

	if (nBar == SB_HORZ && m_pHScrollBar)
...
{
	if (m_pVScrollBar && pScrollBar == m_pVScrollBar)
		GetCalendarCtrl().OnVScroll(nSBCode, nPos, 0);

	CView::OnVScroll(nSBCode, nPos, pScrollBar);
}

void CXTPCalendarControlView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (m_pHScrollBar && pScrollBar == m_pHScrollBar)
		GetCalendarCtrl().OnHScroll(nSBCode, nPos, 0);
...
void CXTPCalendarControlView::_ResizeControls()
{
	...
	if (::IsWindow(m_wndCalendarCaptionBar.m_hWnd))
	{
		...
		m_wndCalendarCaptionBar.MoveWindow(0, 0, cx, cyCaptionBar);
	}

	if (::IsWindow(GetCalendarCtrl().m_hWnd))
	{
		int cxVScroll = ::GetSystemMetrics(SM_CXVSCROLL);
		int cyHScroll = 0;
		...
		CRect rcCalendar;
		rcCalendar.left = 0;
		rcCalendar.top = cyCaptionBar;
		rcCalendar.right = cx;
		rcCalendar.bottom = cy;

		if (::IsWindow(m_pHScrollBar->GetSafeHwnd()))
		{
			rcCalendar.bottom -= cyHScroll;

			if (GetCalendarCtrl().IsLayoutRTL())
			{
				m_pHScrollBar->MoveWindow(
					cxVScroll, cy - cyHScroll, cx - cxVScroll, cyHScroll);
			}
			else
			{
				m_pHScrollBar->MoveWindow(
					0, cy - cyHScroll, cx - cxVScroll, cyHScroll);
			}
		}

		if (::IsWindow(m_pVScrollBar->GetSafeHwnd()))
		{
			if (GetCalendarCtrl().IsLayoutRTL())
			{
				m_pVScrollBar->MoveWindow(
					0, cyCaptionBar, cxVScroll, max(0, cy - cyCaptionBar) - cyHScroll);

				rcCalendar.left = cxVScroll;
			}
			else
			{
				m_pVScrollBar->MoveWindow(
					cx - cxVScroll, cyCaptionBar, cxVScroll, max(0, cy - cyCaptionBar) - cyHScroll);

				rcCalendar.right = max(0, cx - cxVScroll);
			}
		}

		GetCalendarCtrl().MoveWindow(rcCalendar);
	}
}
...
void CXTPCalendarControlView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* pInfo)
{
	...
	if (m_bDayViewMode)
	{
		...
		pInfo->SetMaxPage(XTPToUIntChecked(nMaxPage)); //estimation only - can be adjusted later
...
void CXTPCalendarControlView::SetupCurrentDayViewPrintPage(CDC* pDC, CPrintInfo* pInfo)
{
	if (pInfo->m_nCurPage == 1)
	{
		...
		if (m_PageStart.GetSize() == 0)
			m_PageStart.Add(XTPToUIntChecked(m_nCurCell));
	}

	if (pInfo->m_bPreview) //to allow preview go back
	{
		if ((int) pInfo->m_nCurPage - 1 < m_PageStart.GetSize())
			m_nCurCell = XTPToIntChecked(m_PageStart.GetAt(XTPToIntPtrChecked(pInfo->m_nCurPage - 1)));
	}
...
{
	int nZeroBasedCurPage = XTPToIntChecked(pInfo->m_nCurPage - 1);
...
void CXTPCalendarControlView::SelectNextDayViewPrintPage(CPrintInfo* pInfo)
{
	...
	else //preview mode - user can switch form page to page in any direction
	{
		...
		{
			m_nCurCell = XTPToIntChecked(m_PageStart.GetAt(XTPToIntPtr(pInfo->m_nCurPage - 1)) + m_nCellSteps);
			m_PageStart.Add(XTPToUIntChecked(m_nCurCell));
		}
...
void CXTPCalendarControlView::_OnPrint2(CDC* pDC, CPrintInfo* pInfo, CRect rcPrint)
{
		...
		&& (m_bPrintRepeatHeaderFooterMode || pInfo->m_nCurPage == 1))
	{
		...
		rcCalendar.top = m_pritAdjustContext.m_rcDateHeader.bottom + XTP_DPI_Y(2);
	}

	rcCalendar.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2));
	...
	rcBorder.InflateRect(XTP_DPI_X(1), XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1));
...
void CXTPCalendarControlView::DrawDateHeader(BOOL bCalculate, CDC* pDC, CRect& rrcHeader)
{
	...
	BOOL bRTL = GetCalendarCtrl().IsLayoutRTL();
	...
	rcHeader1 = rrcHeader;
	rcHeader1.DeflateRect(XTP_DPI_X(10), XTP_DPI_Y(5), XTP_DPI_X(10), XTP_DPI_Y(5));
	rcHeader1.right = nCalendar2XPos - XTP_DPI_X(20);
									...
									CRect& rrcHeaderCalendar)
{
	...
	for (i = 0; i < 7; i++)
	{
		CString sWD = CXTPCalendarUtils::GetLocaleString(XTPToUIntChecked(
			CXTPCalendarUtils::GetDayOfWeekLocaleStringId(((i - 1 + 7) % 7) + 1)), 100);
	...
	CString strMonthName = CXTPCalendarUtils::GetLocaleString(XTPToUIntChecked(LOCALE_SMONTHNAME1 + nMonth - 1), 100);
	...
	rcLine1.top += nMonthRowY + nWSaysRowY - XTP_DPI_Y(1);
	rcLine1.bottom = rcLine1.top + XTP_DPI_Y(1);

	CRect rcLine2 = rrcHeaderCalendar;
	rcLine2.left = rcLine1.left;
	rcLine2.right = rcLine2.left + XTP_DPI_X(1);
	...
	for (int w = 0; w < 6; w++)
	{
		...
		if (nDay <= nMaxMonthDay)
		{
			...
			rcWeek.right = rcWeek.left + sizeCell.cx - XTP_DPI_X(2);
		...
		for (i = 0; i < 7; i++)
		{
			if (nDay > 0 && nDay <= nMaxMonthDay)
			{
				...
				if (nDay == nTodayDay_toMark)
				{
					...
					rcToday.left += max(XTP_DPI_X(1), sizeCell.cx/10);
					rcToday.right += max(XTP_DPI_X(1), sizeCell.cx/10);
		...
		CString& strHeader1, CString& strHeader2, CString& strHeaderWeekDay)
{
		...
		|| (b && nViewType == xtpCalendarFullWeekView))
	{
		...
		if (nCount == 1)
		{
			...
			strHeaderWeekDay = CXTPCalendarUtils::GetLocaleString(XTPToUIntChecked(LOCALE_SDAYNAME1 + nWdShift), 100);
		}
...
CRect AFX_CDECL CXTPCalendarControlView::_HimetricToLP(CDC* pDC, const CRect rcHimetric)
{
...
LPFONTDISP AxCreateOleFont(const LOGFONT& logFont, CObject* pParent, LPFNFONTCHANGED lpFontChanged);

#define DISP_PROPERTY_2EX_ID(theClass, szExternalName, dispid, vtPropType)\
	DISP_PROPERTY_EX_ID(theClass, #szExternalName, dispid, OleGet##szExternalName, OleSet##szExternalName, vtPropType)

BEGIN_DISPATCH_MAP(CXTPCalendarControlViewPrintOptions, CXTPPrintOptions)

	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "PrintFrom",    11, m_dtPrintFrom, VT_DATE)
	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "PrintTo",      12, m_dtPrintTo, VT_DATE)
	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "PrintFromToExactly", 13, m_bPrintFromToExactly, VT_BOOL)
	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "ThreeSmallCal", 14, m_b3SmallCal, VT_BOOL)
	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "MonthViewPrintBeginDate",    24, m_dtMonthViewPrintBeginDate, VT_DATE)
	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "MonthViewPrintEndDate",      25, m_dtMonthViewPrintEndDate, VT_DATE)

	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "PrintDateHeader", 21, m_bPrintDateHeader, VT_BOOL)
	DISP_PROPERTY_2EX_ID(CXTPCalendarControlViewPrintOptions, DateHeaderFont, 22, VT_FONT)
	DISP_PROPERTY_2EX_ID(CXTPCalendarControlViewPrintOptions, DateHeaderWeekDayFont, 26, VT_FONT)
	DISP_PROPERTY_2EX_ID(CXTPCalendarControlViewPrintOptions, DateHeaderCalendarFont, 23, VT_FONT)

END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPCalendarControlViewPrintOptions, CXTPPrintOptions)
	INTERFACE_PART(CXTPCalendarControlViewPrintOptions, XTPDIID_CalendarPrintOptions, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarControlViewPrintOptions, XTPDIID_CalendarPrintOptions)


LPFONTDISP CXTPCalendarControlViewPrintOptions::OleGetDateHeaderFont()
{
	return AxCreateOleFont(m_lfDateHeaderFont, this, (LPFNFONTCHANGED)&CXTPCalendarControlViewPrintOptions::OleSetDateHeaderFont);
}

void CXTPCalendarControlViewPrintOptions::OleSetDateHeaderFont(LPFONTDISP pFontDisp)
{
	VERIFY(AxGetLogFont(m_lfDateHeaderFont, pFontDisp));
}

LPFONTDISP CXTPCalendarControlViewPrintOptions::OleGetDateHeaderWeekDayFont()
{
	return AxCreateOleFont(m_lfDateHeaderWeekDayFont, this, (LPFNFONTCHANGED)&CXTPCalendarControlViewPrintOptions::OleSetDateHeaderWeekDayFont);
}

void CXTPCalendarControlViewPrintOptions::OleSetDateHeaderWeekDayFont(LPFONTDISP pFontDisp)
{
	VERIFY(AxGetLogFont(m_lfDateHeaderWeekDayFont, pFontDisp));
}

LPFONTDISP CXTPCalendarControlViewPrintOptions::OleGetDateHeaderCalendarFont()
{
	return AxCreateOleFont(m_lfDateHeaderCalendarFont, this, (LPFNFONTCHANGED)&CXTPCalendarControlViewPrintOptions::OleSetDateHeaderCalendarFont);
}

void CXTPCalendarControlViewPrintOptions::OleSetDateHeaderCalendarFont(LPFONTDISP pFontDisp)
{
	VERIFY(AxGetLogFont(m_lfDateHeaderCalendarFont, pFontDisp));
}
