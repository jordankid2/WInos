...
//===========================================================================
IMPLEMENT_DYNAMIC(CXTPCalendarWMHandler, CXTPCmdTarget)
...
#define XTP_CALENDAR_VIEWSELPROP_DECLARE(oleType, propName) \
	oleType OleGet##propName(); \
	void OleSet##propName(oleType);

#define XTP_CALENDAR_VIEWSELPROP_IMPLEMENT(oleType, propName, sbMember) \
	oleType CXTPCalendarViewSelection::OleGet##propName()   {   \
		if (!m_pView)\
			return 0;\
		return m_pView->m_selectedBlock.##sbMember; \
	}                                                                   \
	void CXTPCalendarViewSelection::OleSet##propName(oleType oleVal)    \
	{                                                       \
		if (m_pView) \
			m_pView->m_selectedBlock.##sbMember = oleVal;   \
	}

class CXTPCalendarViewSelection : public CXTPCmdTarget
{
public:
	CXTPCalendarViewSelection(CXTPCalendarView* pOwnerView);
	virtual ~CXTPCalendarViewSelection();

	// clear internal reference to CXTPCalendarView owner object.
	virtual void Detach();
...
protected:
	...
	DECLARE_OLETYPELIB_EX(CXTPCalendarViewSelection);

	BOOL OleIsValid();

	XTP_CALENDAR_VIEWSELPROP_DECLARE(DATE, Begin);
	XTP_CALENDAR_VIEWSELPROP_DECLARE(DATE, End);
	XTP_CALENDAR_VIEWSELPROP_DECLARE(BOOL, AllDay);
	XTP_CALENDAR_VIEWSELPROP_DECLARE(long, GroupIndex);

protected:
	CXTPCalendarView* m_pView;

	LPDISPATCH OleGetBeginV();
	LPDISPATCH OleGetEndV();
};
//}}AFX_CODEJOCK_PRIVATE
#endif
		...
		m_pControl(pCalendarControl)
{
	...
	m_Layout.m_nRowHeight = XTP_DPI_Y(18);
...
#ifdef _XTP_ACTIVEX
	XTP_SAFE_CALL1(m_pSelOleWrapper, Detach()); // clear internal reference to CXTPCalendarView owner object.
	CMDTARGET_RELEASE(m_pSelOleWrapper);
#endif
...
CXTPCalendarViewDay* CXTPCalendarView::GetViewDay_(int nIndex) const
{
	ASSERT(this);
	if (!this)
		return NULL;
	...
	ASSERT(0 < nCount ? (nIndex >= 0 && nIndex < nCount) : nIndex == 0);

	return (nIndex >= 0 && nIndex < nCount) ? m_arDays.GetAt(nIndex) : NULL;
}

BOOL CXTPCalendarView::IsInDiscreteSelection(UINT uDay) const
{
	BOOL bFound = FALSE;
...
BOOL CXTPCalendarView::AddToDiscreteSelection(UINT uDay)
{
	BOOL bAdd(TRUE);
...
COleDateTime CXTPCalendarView::GetDiscreteSelectionValue(UINT id) const
{
	COleDateTime dt;

	if (id < (UINT)m_DiscreteSelection.GetSize())
		dt = COleDateTime((double)m_DiscreteSelection.GetAt(XTPToIntPtr(id)));
	return dt;
}
...
BOOL CXTPCalendarView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	const BOOL bHandled = TBase::OnLButtonUp(nFlags, point);
	...
	if (!OnEndDragging(point, &hitInfo) && !m_pControl->IsReadonlyMode())
	{
		if (!bHandled && bHitTest)
		{
				...
				hitInfo.pViewEvent && hitInfo.pViewEvent->IsSelected())
			{
				...
				//----------------
				// Behavior: 1st click selects the event, 2nd click enables editor.

				if (bEnabled && (m_pLastEvent == hitInfo.pViewEvent))
				{
				...
				m_pLastEvent = hitInfo.pViewEvent; 
			}
			else
			{
				m_pLastEvent = NULL; 
...
//Preventing continues selection if ALT key hold
		...
		else if (nFlags & MK_LBUTTON && m_bStartedClickInside)
		{
			...
			if (m_ptStartDragging.x == LONG_MAX || m_ptStartDragging.y == LONG_MAX ||
					abs(point.x - m_ptStartDragging.x) >= XTP_DPI_X(cEpsilon) ||
					abs(point.y - m_ptStartDragging.y) >= XTP_DPI_Y(cEpsilon))
			{
...
BOOL CXTPCalendarView::OnEndDragging(CPoint point, XTP_CALENDAR_HITTESTINFO* phitInfo)
{
	...
	UNREFERENCED_PARAMETER(phitInfo);
...
void CXTPCalendarView::SelectViewEvent(CXTPCalendarViewEvent* pViewEvent, BOOL bSelect)
{
	...
	if (bSelect)
	{
		int nIndex = m_pSelectedEvents->Find(pViewEvent->GetEvent());
		if (-1 == nIndex)
		{
...
CRect CXTPCalendarView::GetTooltipRect(const CPoint& ptHit, const XTP_CALENDAR_HITTESTINFO& hitInfo) const
{
	...
	if (hitInfo.pViewGroup)
	{
		...
		if (pThemeX)
		{
			...
			{
				rcTip.right = rcExclude.left - XTP_DPI_X(1);
			}
			...
			{
				rcTip.left = rcExclude.right + XTP_DPI_X(1);
			}
		}
	}
...
void CXTPCalendarView::CalculateHeaderFormat(CDC* pDC, int nWidth, CFont* pTextFont)
{
	...
	int arBorder[] = {XTP_DPI_X(40), XTP_DPI_X(35), XTP_DPI_X(20), 0};
...
CXTPCalendarViewSelection::CXTPCalendarViewSelection(CXTPCalendarView* pOwnerView)
{
	ASSERT(pOwnerView);
...
CXTPCalendarViewSelection::~CXTPCalendarViewSelection()
{
}

void CXTPCalendarViewSelection::Detach()
{
	m_pView = NULL;
}

BOOL CXTPCalendarViewSelection::OleIsValid()
{
	return XTP_SAFE_GET1(m_pView, m_selectedBlock.IsValid(), FALSE);
}

XTP_CALENDAR_VIEWSELPROP_IMPLEMENT(DATE, Begin,      dtBegin);
XTP_CALENDAR_VIEWSELPROP_IMPLEMENT(DATE, End,        dtEnd);
XTP_CALENDAR_VIEWSELPROP_IMPLEMENT(BOOL, AllDay,     bAllDayEvent);
XTP_CALENDAR_VIEWSELPROP_IMPLEMENT(long, GroupIndex, nGroupIndex);

BEGIN_DISPATCH_MAP(CXTPCalendarViewSelection, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPCalendarViewSelection, "Begin",      1, OleGetBegin,      OleSetBegin,      VT_DATE)
	DISP_PROPERTY_EX_ID(CXTPCalendarViewSelection, "End",        2, OleGetEnd,        OleSetEnd,        VT_DATE)
	DISP_PROPERTY_EX_ID(CXTPCalendarViewSelection, "AllDay",     3, OleGetAllDay,     OleSetAllDay,     VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPCalendarViewSelection, "GroupIndex", 4, OleGetGroupIndex, OleSetGroupIndex, VT_I4)
	DISP_FUNCTION_ID(CXTPCalendarViewSelection, "IsValid", 10, OleIsValid, VT_BOOL, VTS_NONE)


	DISP_PROPERTY_EX_ID(CXTPCalendarViewSelection, "BeginV", 36, OleGetBeginV, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarViewSelection, "EndV", 37, OleGetEndV, SetNotSupported, VT_DISPATCH)

END_DISPATCH_MAP()


LPDISPATCH CXTPCalendarViewSelection::OleGetBeginV()
{
	if (!m_pView)
		return 0;

	SYSTEMTIME st;
	m_pView->m_selectedBlock.dtBegin.GetAsSystemTime(st);

	CXTPCalendarSystemTime* lpDisp = new CXTPCalendarSystemTime(&st);
	return lpDisp->GetIDispatch(FALSE);
}

LPDISPATCH CXTPCalendarViewSelection::OleGetEndV()
{
	if (!m_pView)
		return 0;

	SYSTEMTIME st;
	m_pView->m_selectedBlock.dtEnd.GetAsSystemTime(st);

	CXTPCalendarSystemTime* lpDisp = new CXTPCalendarSystemTime(&st);
	return lpDisp->GetIDispatch(FALSE);
}

BEGIN_INTERFACE_MAP(CXTPCalendarViewSelection, CXTPCmdTarget)
	INTERFACE_PART(CXTPCalendarViewSelection, XTPDIID_CalendarViewSelection, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarViewSelection, XTPDIID_CalendarViewSelection)


//{{AFX_CODEJOCK_PRIVATE
class CXTPCalendarHitTestInfo : public CXTPCmdTarget, public XTP_CALENDAR_HITTESTINFO
{
public:
	CXTPCalendarHitTestInfo()
	{
		EnableAutomation();
		EnableTypeLib();
	}
	...
	LPDISPATCH OleGetViewEvent();
	LPDISPATCH OleGetViewDay();
	LPDISPATCH OleGetViewGroup();
	int OleGetHitCode();
	DATE OleGetHitDateTime();
	BOOL OleGetTimePartValid();

};
//}}AFX_CODEJOCK_PRIVATE

BEGIN_DISPATCH_MAP(CXTPCalendarView, CXTPCalendarWMHandler)
	DISP_PROPERTY_ID(CXTPCalendarView, "DayHeaderFormatLong",     50, m_strDayHeaderFormatLong,     VT_BSTR)
	DISP_PROPERTY_ID(CXTPCalendarView, "DayHeaderFormatMiddle",   55, m_strDayHeaderFormatMiddle,   VT_BSTR)
	DISP_PROPERTY_ID(CXTPCalendarView, "DayHeaderFormatShort",    60, m_strDayHeaderFormatShort,    VT_BSTR)
	DISP_PROPERTY_ID(CXTPCalendarView, "DayHeaderFormatShortest", 65, m_strDayHeaderFormatShortest, VT_BSTR)

	DISP_FUNCTION_ID(CXTPCalendarView, "ResetSelection", 98, OleResetSelection, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "SetSelection", 99, OleSetSelection, VT_EMPTY, VTS_DATE VTS_DATE VTS_BOOL)
	DISP_FUNCTION_ID(CXTPCalendarView, "GetSelection", 100, OleGetSelection, VT_BOOL, VTS_PDATE VTS_PDATE VTS_PBOOL)
	DISP_FUNCTION_ID(CXTPCalendarView, "GetSelectionV", 150, OleGetSelectionV, VT_BOOL, VTS_PVARIANT VTS_PVARIANT VTS_PVARIANT)
	DISP_FUNCTION_ID(CXTPCalendarView, "Selection", 132, OleSelection, VT_DISPATCH, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarView, "HitTest", 101, OleHitTest, VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "HitTestEx", 151, OleHitTestEx, VT_DISPATCH, VTS_I4 VTS_I4)

	DISP_FUNCTION_ID(CXTPCalendarView, "ShowDay", 102, OleShowDay, VT_EMPTY, VTS_DATE VTS_VARIANT)

	DISP_FUNCTION_ID(CXTPCalendarView, "DaysCount", 103, OleGetItemCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "Days", DISPID_VALUE, OleGetItem, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPCalendarView, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarView, "GetSelectedEvents", 110, OleGetSelectedEvents, VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "SelectViewEvent", 111, OleSelectViewEvent, VT_EMPTY, VTS_DISPATCH VTS_BOOL)
	DISP_FUNCTION_ID(CXTPCalendarView, "UnselectAllEvents", 112, OleUnselectAllEvents, VT_EMPTY, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarView, "EnableVScroll", 113, OleEnableVScroll, VT_EMPTY, VTS_BOOL)
	DISP_FUNCTION_ID(CXTPCalendarView, "EnableHScroll", 114, OleEnableHScroll, VT_EMPTY, VTS_BOOL)

	DISP_FUNCTION_ID(CXTPCalendarView, "CanCut", 120, CanCut, VT_BOOL, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "CanCopy", 121, CanCopy, VT_BOOL, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "CanPaste", 122, CanPaste, VT_BOOL, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarView, "CanUndo", 123, CanUndo, VT_BOOL, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "CanRedo", 124, CanRedo, VT_BOOL, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarView, "Cut", 125, Cut, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "Copy", 126, Copy, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "Paste", 127, Paste, VT_EMPTY, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarView, "Undo", 128, Undo, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "Redo", 129, Redo, VT_EMPTY, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarView, "MultipleResources", 130, OleGetMultipleResources, VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "SetMultipleResources", 131, OleSetMultipleResources, VT_EMPTY, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPCalendarView, "CanStartEdit", 133, OleCanStartEdit, VT_BOOL, VTS_NONE)

END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPCalendarView, CXTPCalendarWMHandler)
	INTERFACE_PART(CXTPCalendarView, XTPDIID_CalendarView, Dispatch)
	//INTERFACE_PART(CXTPCalendarView, IID_IEnumVARIANT, EnumVARIANT)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarView, XTPDIID_CalendarView)

IMPLEMENT_ENUM_VARIANT(CXTPCalendarView)

LPDISPATCH CXTPCalendarView::OleSelection()
{
	if (!m_pSelOleWrapper)
		m_pSelOleWrapper = new CXTPCalendarViewSelection(this);

	return m_pSelOleWrapper ? m_pSelOleWrapper->GetIDispatch(TRUE) : NULL;
}

BOOL CXTPCalendarView::OleCanStartEdit()
{
	return (!IsEditingSubject() && GetDraggingMode() == xtpCalendaDragModeUnknown);
}

BOOL CXTPCalendarView::OleGetSelection(DATE* pBegin, DATE* pEnd,
		VARIANT_BOOL* pvbAllDay)
{
	COleDateTime dtBegin, dtEnd;
	BOOL bAllDay = FALSE;
	BOOL bSelection = GetSelection(&dtBegin, &dtEnd, &bAllDay);

	if (pBegin && dtBegin.GetStatus() == COleDateTime::valid)
		*pBegin = dtBegin;

	if (pEnd && dtEnd.GetStatus() == COleDateTime::valid)
		*pEnd = dtEnd;

	if (pvbAllDay)
		*pvbAllDay = bAllDay ? VARIANT_TRUE : VARIANT_FALSE;

	return bSelection;
}

BOOL CXTPCalendarView::OleGetSelectionV(VARIANT* pBegin, VARIANT* pEnd,
										VARIANT* pvbAllDay)
{
	DATE dtBegin = 0, dtEnd = 0;
	VARIANT_BOOL vbAllDay = 0;

	BOOL bRes = OleGetSelection(&dtBegin, &dtEnd, &vbAllDay);
	if (bRes)
	{
		if (pBegin)
			*pBegin = COleVariant(COleDateTime(dtBegin));

		if (pEnd)
			*pEnd = COleVariant(COleDateTime(dtEnd));

		if (pvbAllDay)
			*pvbAllDay = COleVariant((short)vbAllDay, VT_BOOL);
	}
	return bRes;
}

void CXTPCalendarView::OleResetSelection()
{
	ResetSelection();
}

void CXTPCalendarView::OleSetSelection(DATE Begin, DATE End, BOOL bAllDay)
{
	SetSelection(Begin, End, bAllDay);
}

LPDISPATCH CXTPCalendarView::OleHitTest()
{
	CPoint pt;
	GetCursorPos(&pt);

	return OleHitTestEx(pt.x, pt.y);
}

LPDISPATCH CXTPCalendarView::OleHitTestEx(long X, long Y)
{
	if (!m_pControl)
	{
		ASSERT(FALSE);
	...
	CXTPCalendarHitTestInfo* pHitTestInfo = new CXTPCalendarHitTestInfo;
	if (!pHitTestInfo)
		return NULL;

	HitTest(pt, pHitTestInfo);

	return pHitTestInfo->GetIDispatch(FALSE);
}

void CXTPCalendarView::OleShowDay(DATE dt, const VARIANT& Select)
{
	_variant_t vtSelect(Select);
	BOOL bSelect = TRUE;
	if (vtSelect.vt == VT_ERROR && vtSelect.scode == DISP_E_PARAMNOTFOUND)
	{
		// default
	}
	else
	{
		bSelect = (long)vtSelect != 0;
	}

	ShowDay(dt, bSelect);
}

int CXTPCalendarView::OleGetItemCount()
{
	return GetViewDayCount();
}

LPDISPATCH CXTPCalendarView::OleGetItem(long nIndex)
{
	CXTPCalendarViewDay* pDay = GetViewDay_(nIndex);
	if (!pDay)
		AfxThrowOleException(DISP_E_BADINDEX);

	return pDay->GetIDispatch(TRUE);
}

LPDISPATCH CXTPCalendarView::OleGetSelectedEvents()
{
	CXTPCalendarViewEventsPtr ptrEvents = GetSelectedEvents();
	return ptrEvents ? ptrEvents->GetIDispatch(TRUE) : NULL;
}

void CXTPCalendarView::OleSelectViewEvent(LPDISPATCH pViewEventDisp, BOOL bSelect)
{
	CXTPCalendarViewEvent* pViewEvent = DYNAMIC_DOWNCAST(CXTPCalendarViewEvent, 
		CXTPCalendarViewEvent::FromIDispatchSafe(pViewEventDisp));
	if (!pViewEvent)
	{
...
void CXTPCalendarView::OleUnselectAllEvents()
{
	UnselectAllEvents();
}

void CXTPCalendarView::OleEnableVScroll(BOOL bEnable)
{
	EnableVScroll(bEnable);
}

void CXTPCalendarView::OleEnableHScroll(BOOL bEnable)
{
	EnableHScroll(bEnable);
}

LPDISPATCH CXTPCalendarView::OleGetMultipleResources()
{
	return XTP_SAFE_GET1(GetResources(), GetIDispatch(TRUE), NULL);
}

void CXTPCalendarView::OleSetMultipleResources(LPDISPATCH pDispResources)
{
	if (!pDispResources)
	{
		SetResources(NULL);
	...
	CXTPCalendarResources* pResources = DYNAMIC_DOWNCAST(CXTPCalendarResources, 
		CXTPCalendarResources::FromIDispatchSafe(pDispResources));
	if (!pResources)
	{
		AfxThrowOleException(E_INVALIDARG);
	}
...
BEGIN_DISPATCH_MAP(CXTPCalendarHitTestInfo, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPCalendarHitTestInfo, "ViewEvent", 1, OleGetViewEvent, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarHitTestInfo, "ViewDay", 2, OleGetViewDay, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarHitTestInfo, "HitCode", 3, OleGetHitCode, SetNotSupported, VT_I4)
	DISP_FUNCTION_ID(CXTPCalendarHitTestInfo, "HitDateTime", 4, OleGetHitDateTime, VT_DATE, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarHitTestInfo, "TimePartValid", 5, OleGetTimePartValid, VT_BOOL, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPCalendarHitTestInfo, "ViewGroup", 6, OleGetViewGroup, SetNotSupported, VT_DISPATCH)

END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPCalendarHitTestInfo, CXTPCmdTarget)
	INTERFACE_PART(CXTPCalendarHitTestInfo, XTPDIID_CalendarHitTestInfo, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarHitTestInfo, XTPDIID_CalendarHitTestInfo)

LPDISPATCH CXTPCalendarHitTestInfo::OleGetViewEvent()
{
	return pViewEvent ? pViewEvent->GetIDispatch(TRUE) : 0;
}

LPDISPATCH CXTPCalendarHitTestInfo::OleGetViewDay()
{
	return pViewDay ? pViewDay->GetIDispatch(TRUE) : 0;
}

LPDISPATCH CXTPCalendarHitTestInfo::OleGetViewGroup()
{
	return pViewGroup ? pViewGroup->GetIDispatch(TRUE) : 0;
}

int CXTPCalendarHitTestInfo::OleGetHitCode()
{
	return uHitCode;
}

DATE CXTPCalendarHitTestInfo::OleGetHitDateTime()
{
	return dt;
}

BOOL CXTPCalendarHitTestInfo::OleGetTimePartValid()
{
	return bTimePartValid;
}
...
BOOL CXTPCalendarWMHandler::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	ASSERT(NULL != pResult);

	BOOL bHandled = FALSE;

	const int nCount = GetChildHandlersCount();
	for (int i = 0; i < nCount; i++)
	{
		CXTPCalendarWMHandler* pChild = GetChildHandlerAt(i);
		if (NULL != pChild && pChild->OnWndMsg(message, wParam, lParam, pResult))
		{
			bHandled = TRUE;
			break;
		}
