		...
		TBase(pCalendarControl, xtpCalendarMonthView)
{
	...
	m_dtLightColorDateBase.SetStatus(COleDateTime::invalid);
...
void CXTPCalendarMonthView::ScrollV(int /*nPos*/, int nPos_raw)
{
	...
	if (GetSelection())
	{
		...
		if(GetCalendarControl()->GetThemeVersion() == xtpCalendarVersion2010)
			SetMDayViewLightColorBaseDate(m_selectedBlock.dtBegin);
...
void CXTPCalendarMonthView::_ReadDefaultDateFormats()
{
	...
	strLongDateFormat.TrimRight(strSpecials);
	strSmallDateFormat.TrimRight(strSpecials);
	...
	m_strDayHeaderFormatDefaultShort.Replace(_T("MMM"), _T("MM"));
...
{
	CString strDate;
	CString strLong = m_strLongDateFormat;
	CString strShort = m_strSmallDateFormat;

	if (GetPaintManager())
	{
		GetPaintManager()->GetDateFormat(strShort, strLong);
	}
	...
	CString strFormat = (bLong || nDay == 1) ? strLong : strShort;
	
	if (CXTPCalendarUtils::GetAsSystemTime(dtDay, st))
	{
...
BOOL CXTPCalendarMonthView::GetScrollBarInfoV(SCROLLINFO* pSI) const
{
	ASSERT(pSI);
	...
	pSI->nMax = 105;
	pSI->nPage = XTPToUIntChecked(m_pGrid->GetWeeksCount());

	return TRUE;
}

CXTPCalendarViewDay* CXTPCalendarMonthView::GetViewDay_(int nIndex) const
{
	if (!m_pGrid)
	{
		return NULL;
	...
	CXTPCalendarMonthViewDay* pDView = (CXTPCalendarMonthViewDay*)m_arDays.GetAt(nIndex);
...
COleDateTime CXTPCalendarMonthView::GetViewDayDate(int nIndex) const
{
	int nWeekIndex = nIndex / 7;
	int nWeekDayIndex = nIndex % 7;
...
void CXTPCalendarMonthView::_CalculateEventCaptionFormat(CDC* pDC)
{
	...
	if (!bShowAsClocks)
	{
		CString strTime = GetItemTextEventTimeMax();
		m_nEventTimeWidth = pDC->GetTextExtent(strTime).cx + szW.cx;
		m_nEventTimeWidth = min(m_nEventTimeWidth, max(0, nCellWidth - XTP_DPI_X(5)));
	}
	...
	DWORD dwOpt = XTP_SAFE_GET1(GetCalendarOptions(), dwAdditionalOptions, 0);

	if (dwOpt & xtpCalendarOptMonthViewShowStartTimeAlways)
		m_nEventCaptionFormat = xtpCalendarCaptionFormatStartSubject;

	if (bShowEnd && (dwOpt & xtpCalendarOptMonthViewShowEndTimeAlways))
		m_nEventCaptionFormat = xtpCalendarCaptionFormatStartEndSubject;
}
...
void CXTPCalendarMonthView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
			...
			nChar == VK_PRIOR || nChar == VK_NEXT)
	{
		...
		{
			CXTPCalendarControl::CUpdateContext updateContext1(m_pControl, xtpCalendarUpdateAll);
...
CXTPCalendarMonthView::CMonthViewGrid::CMonthViewGrid(CXTPCalendarMonthView* pView)
{
	...
	m_nColHeaderHeight = XTP_DPI_Y(18);
...
COleDateTime CXTPCalendarMonthView::CMonthViewGrid::GetMiddleScrollBeginDate() const
{
	COleDateTime dtNow = CXTPCalendarUtils::GetCurrentTime();
	COleDateTime dtFirst(dtNow.GetYear(), dtNow.GetMonth(), 1, 0, 0, 0);
...
int CXTPCalendarMonthView::CMonthViewGrid::GetScrollPos() const
{
...
COleDateTime CXTPCalendarMonthView::CMonthViewGrid::ShiftDateToCell_00(COleDateTime dtDate) const
{
...
CRect CXTPCalendarMonthView::CMonthViewGrid::GetDayRect(int nWeekIndex, int nWeekDayIndex, BOOL bIncludeBorders) const
{
	...
	CRect rcDay(0, 0, 0, 0);
	rcDay.left = XTPToLongChecked(m_arColsLeftX[nColIdx]);
	rcDay.right = XTPToLongChecked(m_arColsLeftX[nColIdx + 1]);

	UINT nHalfHeight = (m_arRowsTopY[nWeekIndex + 1] - m_arRowsTopY[nWeekIndex]) / 2;

	if (bCompressWeD && nWDay == 7)
	{
		rcDay.top = XTPToLongChecked(m_arRowsTopY[nWeekIndex]);
		rcDay.bottom = XTPToLongChecked(rcDay.top + nHalfHeight);
	}
	else
		if (bCompressWeD && nWDay == 1)
		{
			rcDay.top = XTPToLongChecked(m_arRowsTopY[nWeekIndex] + nHalfHeight);
			rcDay.bottom = XTPToLongChecked(m_arRowsTopY[nWeekIndex + 1]);
		}
		else
		{
			rcDay.top = XTPToLongChecked(m_arRowsTopY[nWeekIndex]);
	...
	if (!bIncludeBorders)
	{
		rcDay.DeflateRect(XTP_DPI_X(1), XTP_DPI_Y(1), 0, 0);
	}
...
CRect CXTPCalendarMonthView::CMonthViewGrid::GetCelRect(int nWeekIndex, int nColIndex, BOOL bIncludeBorders) const
{
	...
	rcCell.left = XTPToLongChecked(m_arColsLeftX[nColIndex]);
	rcCell.right = XTPToLongChecked(m_arColsLeftX[nColIndex + 1]);

	rcCell.top = XTPToLongChecked(m_arRowsTopY[nWeekIndex]);
	rcCell.bottom = XTPToLongChecked(m_arRowsTopY[nWeekIndex + 1]);

	if (!bIncludeBorders)
	{
		rcCell.DeflateRect(XTP_DPI_X(1), XTP_DPI_Y(1), 0, 0);
	}
...
CXTPCalendarMonthViewDay* CXTPCalendarMonthView::CMonthViewGrid::GetViewDay(int nWeekIndex, int nWeekDayIndex) const
{
...
void CXTPCalendarMonthView::CMonthViewGrid::AdjustHeader(CDC* pDC)
{
	...
	{
		LC_WeekDAYNAME1 = XTPToUIntChecked(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(1));
	}
	//---------------------------------------------------------------------------
	...
	for (i = 0; i < 7; i++)
	{
		...
		if (!sVal.IsEmpty())
		{
			...
			CString sSat = CXTPCalendarUtils::GetLocaleString(XTPToUIntChecked(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(6)), 100);
			CString sSun = CXTPCalendarUtils::GetLocaleString(XTPToUIntChecked(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(7)), 100);
	...
	m_nColHeaderHeight = max(XTP_DPI_Y(18), sizeH.cy + XTP_DPI_Y(6));
...
void CXTPCalendarMonthView::CMonthViewGrid::AdjustGrid(const CRect& rcRect)
{
	...
	{
		m_arRowsTopY[nWeek] = XTPToUIntChecked(nCurrPos);
	...
	m_arRowsTopY[m_nWeeksCount] = XTPToUIntChecked(nCurrPos);

	nCurrPos = rcRect.left; //0;
	for (int nDayColl = 0; nDayColl < nColsCount; nDayColl++)
	{
		m_arColsLeftX[nDayColl] = XTPToUIntChecked(nCurrPos);
	...
	m_arColsLeftX[nColsCount] = XTPToUIntChecked(nCurrPos);
}
...
void CXTPCalendarMonthView::CMonthViewGrid::Draw(CDC* pDC)
{
	...
	XTP_SAFE_CALL3(m_pView, GetPaintManager(), GetMonthViewGridPart(), OnDrawGrid(pDC, m_pView));
}


#ifdef _XTP_ACTIVEX

BEGIN_DISPATCH_MAP(CXTPCalendarMonthView, CXTPCalendarView)
	DISP_PROPERTY_EX_ID(CXTPCalendarMonthView, "WeeksCount", 1, OleGetWeeksCount, OleSetWeeksCount, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPCalendarMonthView, "DraftMode", 2, OleGetDraftMode, OleSetDraftMode, VT_BOOL)
END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPCalendarMonthView, CXTPCalendarView)
	INTERFACE_PART(CXTPCalendarMonthView, XTPDIID_CalendarMonthView, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarMonthView, XTPDIID_CalendarMonthView)


long CXTPCalendarMonthView::OleGetWeeksCount()
{
	return XTP_SAFE_GET1(GetGrid(), GetWeeksCount(), 2);
}
void CXTPCalendarMonthView::OleSetWeeksCount(long nWeeksCount)
{
	CXTPCalendarControl::CViewChangedContext viewChanged(this);

	XTP_SAFE_CALL1(GetGrid(), SetWeeksCount(nWeeksCount));

	XTP_SAFE_CALL1(GetCalendarControl(), Populate());
}

BOOL CXTPCalendarMonthView::OleGetDraftMode()
{
	return m_bDraftMode;
}

void CXTPCalendarMonthView::OleSetDraftMode(BOOL bSet)
{
	m_bDraftMode = bSet;
}
