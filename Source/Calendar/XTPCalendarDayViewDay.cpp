...
void CXTPCalendarDayViewDay::Populate(COleDateTime dtDayDate)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		{
			int iSchID = XTPToIntChecked(pRC->GetSchedules()->GetAt(I));
			arScheduleIDs.Add(XTPToUIntChecked(iSchID));
			arGroupIDs.Add(XTPToUIntChecked(i));
		}
			...
			if (pGroup)
			{
				...
				for (int j = 0; j < nGroupViewEventsCount; j++)
				{
					...
					if (pVe)
					{
						...
						if (pE)
						{
							int iOwnId = XTPToIntChecked(pE->GetScheduleID());
...
void CXTPCalendarDayViewDay::DrawEvents(CDC* pDC)
{
	ASSERT(pDC);
...
void CXTPCalendarDayViewGroup::AdjustLayout(CDC* pDC, const CRect& rcGroup)
{
	...
	for (int i = 0; i < nEventsCount; i++)
	{
		...
		if (pViewEvent && pViewEvent->IsMultidayEvent())
		{
			...
			rcAllDayEvents.top += XTP_DPI_Y(1);
...
CRect CXTPCalendarDayViewGroup::GetAllDayEventsScrollButtonRect(XTP_DAY_VIEW_GROUP_LAYOUT::XTPEnumHotItem nButton) const
{
	...
	rc.left = GetAllDayEventsRect().right - XTP_DPI_X(23);
	rc.right = rc.left + XTP_DPI_X(20);

	switch (nButton)
	{
	case XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotAllDayEventsScrollUp:
		//rc.top += 9;
		rc.bottom = rc.top + XTP_DPI_Y(13);
		break;
	case XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotAllDayEventsScrollDown:
		rc.top = rc.bottom - XTP_DPI_Y(13);
		rc.bottom = rc.top + XTP_DPI_Y(9);
		break;

	case XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotHeader:
...
void CXTPCalendarDayViewGroup::DrawEvents(CDC* pDC)
{
	ASSERT(pDC);
	...
	BOOL bHasSelectedEvents = XTP_SAFE_GET2(GetViewDay(), GetView(), HasSelectedViewEvent(), FALSE);
	int nGroupIndex = GetGroupIndex();
