...
{
	CXTPCalendarSystemTime* pStandardDate = DYNAMIC_DOWNCAST(CXTPCalendarSystemTime, 
		CXTPCalendarSystemTime::FromIDispatchSafe(pDispStandardDate));
	if (!pStandardDate)
	{
...
{
	CXTPCalendarSystemTime* pDaylightDate = DYNAMIC_DOWNCAST(CXTPCalendarSystemTime, 
		CXTPCalendarSystemTime::FromIDispatchSafe(pDispDaylightDate));
	if (!pDaylightDate)
	{
		AfxThrowOleException(E_INVALIDARG);
	}
...
{
	CXTPCalendarTimeZone* pTZI2;
	pTZI2 = DYNAMIC_DOWNCAST(CXTPCalendarTimeZone, 
...
BOOL AFX_CDECL CXTPCalendarTimeZones::GetRegBSTR(HKEY hKey, LPCWSTR pcszValueNameW, BSTR& rbstrValue)
{
	if (rbstrValue)
	{
...
BOOL AFX_CDECL CXTPCalendarTimeZones::GetRegStr(HKEY hKey, LPCTSTR pcszValueName, CString& rstrValue)
{
	rstrValue.Empty();
...
BOOL AFX_CDECL CXTPCalendarTimeZones::GetRegTZI(HKEY hKey, LPCTSTR pcszValueName,
