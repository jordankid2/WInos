...
CXTPDatePickerList::CXTPDatePickerList(CXTPDatePickerControl* pControl, COleDateTime dtStartMonth)
	: m_pControl(pControl)
	, m_dtStartMonth(dtStartMonth)
	, m_dtMiddleMonth(dtStartMonth)
	, m_dtSelMonth(dtStartMonth)
{
	m_nItemsAbove = 3;
	m_nItemsBelow = 3;
...
void CXTPDatePickerList::OnTimer(UINT_PTR /*nIDEvent*/)
{
	...
	if (ptPos.y < rcClient.top)
	{
		...
		int nScroll = nDiff / rcClient.Height() + XTP_DPI_Y(1);
		ScrollUp(nScroll);
...
CSize CXTPDatePickerList::GetListSize() const
{
	ASSERT(m_pControl);
	if (!m_pControl)
		return 0;
	...
	szMaxItem.cy = dc.GetTextExtent(_T(" "), 1).cy + XTP_DPI_Y(3);
	...
	// calculate rectangle
	CSize szCalendar(szMaxItem.cx + XTP_DPI_X(20), szMaxItem.cy * (m_nItemsAbove + 1 + m_nItemsBelow) + XTP_DPI_Y(2));

	return szCalendar;
}

void AFX_CDECL CXTPDatePickerList::ShiftMonthUp(int& nYear, int& nMonth, const int nCount)
{
...
void AFX_CDECL CXTPDatePickerList::ShiftMonthDown(int& nYear, int& nMonth, const int nCount)
{
	for (int nItem = 0; nItem < nCount; nItem++)
	{
		nMonth--;
...
int CXTPDatePickerList::GetMonthInterval() const
{
...
BOOL CXTPDatePickerList::Create(CRect rcList)
{
	...
	m_rcListControl.left = m_rcListControl.CenterPoint().x - szCalendar.cx / 2 - XTP_DPI_X(1);
	m_rcListControl.right = m_rcListControl.left + szCalendar.cx;
	m_rcListControl.top = m_rcListControl.CenterPoint().y - szCalendar.cy / 2 + XTP_DPI_Y(1);
