...
BEGIN_DISPATCH_MAP(XTP_DAYITEM_METRICS, CXTPCmdTarget)
	DISP_PROPERTY_ID(XTP_DAYITEM_METRICS, "BackColor", DISPID_BACKCOLOR, clrBackground, VT_COLOR)
	DISP_PROPERTY_ID(XTP_DAYITEM_METRICS, "ForeColor", DISPID_FORECOLOR, clrForeground, VT_COLOR)
	DISP_PROPERTY_EX_ID(XTP_DAYITEM_METRICS, "Font", DISPID_FONT, OleGetFont, OleSetFont, VT_FONT)
	DISP_PROPERTY_EX_ID(XTP_DAYITEM_METRICS, "Picture", DISPID_PICTURE, OleGetPicture, OleSetPicture, VT_PICTURE)
END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(XTP_DAYITEM_METRICS, CXTPCmdTarget)
	INTERFACE_PART(XTP_DAYITEM_METRICS, XTPDIID_IDatePickerDayMetrics, Dispatch)
END_INTERFACE_MAP()

LPFONTDISP XTP_DAYITEM_METRICS::OleGetFont()
{
	return AxCreateOleFont(&m_xtpFontText, this, (LPFNFONTCHANGED)&XTP_DAYITEM_METRICS::OleSetFont);
}

void XTP_DAYITEM_METRICS::OleSetFont(LPFONTDISP pFontDisp)
{
...
LPPICTUREDISP XTP_DAYITEM_METRICS::OleGetPicture()
{
	return m_olePicture.GetPictureDispatch();
}

void XTP_DAYITEM_METRICS::OleSetPicture(LPPICTUREDISP pPictureDisp)
{
	m_olePicture.SetPictureDispatch(pPictureDisp);
}
...
CString CXTPDatePickerButton::GetCaption() const
{
	if (m_strCaption.IsEmpty())
		return CXTPCalendarUtils::LoadString(XTPToUIntChecked(m_nID));
...
CXTPDatePickerControl::CXTPDatePickerControl()
{
	...
	//SetTheme(new CXTPDatePickerThemeResource());
...
CXTPDatePickerControl::~CXTPDatePickerControl()
{
	...
	if (m_nTimerID != 0 && m_hWnd)
		KillTimer(XTPToUIntPtrChecked(m_nTimerID));
...
void CXTPDatePickerControl::OnEventResourceImagesChanged(XTP_NOTIFY_CODE Event, WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	if (m_pPaintManager && m_pPaintManager->GetPaintTheme() == xtpCalendarThemeResource)
	{
...
void CXTPDatePickerControl::AddButton(UINT nID)
{
	...
	pButton->m_nID = XTPToIntChecked(nID);
...
void CXTPDatePickerControl::AdjustLayout(CRect rcClient)
{
	...
	for (int nRow = 0; nRow < m_nRows; nRow++)
	{
		for (int nCol = 0; nCol < m_nColumns; nCol++)
		{
			...
			rcMonth.DeflateRect(XTP_DPI_X(1), XTP_DPI_Y(1));
...
void CXTPDatePickerControl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	// fire mouse move
	DWORD dwPoint = XTPToUInt(MAKELONG(point.x, point.y));
...
{
	WNDCLASS wndcls;
	if (hInstance == NULL) hInstance = XTPGetInstanceHandle();
...
{
	WPARAM wprmDay = XTPToWPARAM((XTP_DATE_VALUE)(DATE)dtDay);
...
CSize CXTPDatePickerControl::CalcButtonSize() const
{
	...
	for (int i = 0; i < GetButtonCount(); i++)
	{
		...
		szButton.cx = max(szButton.cx, sz.cx + XTP_DPI_X(12));
		szButton.cy = max(szButton.cy, sz.cy + XTP_DPI_Y(6));
	}
...
void CXTPDatePickerControl::SetButtonRect()
{
	...
	for (int i = 0; i < GetButtonCount(); i++)
	{
		...
		pButton->m_rcButton = CRect(CPoint(nLeft, m_rcGrid.bottom + XTP_DPI_Y(1)), szButton);
		nLeft += nButtonLen;
	}
}
...
void CXTPDatePickerControl::ShowListHeader(CRect rcHeader, COleDateTime dtMonth)
{
	...
	m_nTimerID = XTPToIntChecked(SetTimer(XTP_DATEPICKER_TIMERID, 2 * XTP_DATEPICKER_TIMER_INTERVAL / 3, NULL));
...
void CXTPDatePickerControl::ProcessButtons(CPoint point)
{
	for (int i = 0; i < GetButtonCount(); i++)
	{
		...
		else if (pButton->m_bPressed)
		{
			...
			if (bHighlight)
				OnButtonClick(XTPToUIntChecked(pButton->m_nID));
			_RedrawControl(FALSE);
		}
...
LRESULT CXTPDatePickerControl::SendMessageToParent(int nMessage, NMHDR* pNMHDR)
{
	...
	pNMHDR->idFrom = XTPToUIntPtrChecked(GetDlgCtrlID());
	pNMHDR->code = XTPToUInt(nMessage);
...
void CXTPDatePickerControl::InitNames()
{
	...
	{
		CString strMonth = CXTPCalendarUtils::GetLocaleString(XTPToUIntChecked(LOCALE_SMONTHNAME1 + nMonth), 255);
	...
	for (nDay = 0; nDay < 7; nDay++)
	{
		CString strDayName = CXTPCalendarUtils::GetLocaleString(
			XTPToUIntChecked(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(nDay + 1)), 255);
...
//RTL case fix
	...
	EnableSysYearMonthFormat(TRUE);
}

void CXTPDatePickerControl::EnableSysYearMonthFormat(BOOL nEnable)
{
	m_strYearMonthFormat.Empty();

	if (XTPSystemVersion()->IsWin2KOrGreater() && nEnable)
	{
...
BOOL AFX_CDECL CXTPDatePickerControl::ShiftDate(COleDateTime &refDate, int nMonthCount)
{
	...
	if (pHitMonth)
	{
		...
		else
		{
			...
			if (pHitMonth->m_rcDaysOfWeek.PtInRect(ptHit))
			{
				...
				CPoint ptTopRow(ptHit.x, pHitMonth->m_rcDaysOfWeek.bottom + XTP_DPI_Y(1));
			...
			if (pHitMonth->m_rcWeekNumbers.PtInRect(ptHit))
			{
				...
				CPoint ptLeftRow(pHitMonth->m_rcWeekNumbers.right + XTP_DPI_X(1), ptHit.y);
...
void CXTPDatePickerControl::SetLayoutRTL(BOOL bRightToLeft)
{
	...
	ModifyStyleEx(XTPToUInt(bRightToLeft ? 0 : WS_EX_LAYOUTRTL),
		XTPToUInt(!bRightToLeft ? 0 : WS_EX_LAYOUTRTL));
	InitNames();
	RedrawControl();
}
...
void CXTPDatePickerControl::SetPaintTheme(XTPCalendarTheme ePaintTheme)
{
	...
	case xtpCalendarThemeOfficeXP:
		SetTheme(new CXTPDatePickerThemeOfficeXP());
		break;

	case xtpCalendarThemeOffice2003:
		SetTheme(new CXTPDatePickerThemeOffice2003());
		break;

	case xtpCalendarThemeOffice2013:
		SetTheme(new CXTPDatePickerThemeOffice2013());
		break;

	case xtpCalendarThemeVisualStudio2015:
		SetTheme(new CXTPDatePickerThemeVisualStudio2015());
		break;
	...
	case xtpCalendarThemeUnknown:
		ASSERT(!"xtpCalendarThemeUnknown cannot be set as a theme");
		break;

	case xtpCalendarThemeOffice2000:
	default:
		SetTheme(new CXTPDatePickerThemeOffice2000());
		break;
	}
}
