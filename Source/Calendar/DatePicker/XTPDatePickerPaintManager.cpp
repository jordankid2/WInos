...
void CXTPDatePickerPaintManager::DrawBorder(CDC* pDC, const CXTPDatePickerControl* pControl, CRect& rcClient, BOOL bDraw)
{
	...
	if (bDraw)
	{
		if (borderStyle == xtpDatePickerBorder3D)
		{
			...
			pDC->Draw3dRect(XTP_DPI_X(1), XTP_DPI_Y(1), rcClient.Width() - XTP_DPI_X(2), rcClient.Height() - XTP_DPI_Y(2), m_clrControlBack, m_clr3DShadow);
			pDC->Draw3dRect(XTP_DPI_X(2), XTP_DPI_Y(2), rcClient.Width() - XTP_DPI_X(4), rcClient.Height() - XTP_DPI_Y(4), m_clr3DFace, m_clr3DFace);
			pDC->Draw3dRect(XTP_DPI_X(3), XTP_DPI_Y(3), rcClient.Width() - XTP_DPI_X(6), rcClient.Height() - XTP_DPI_Y(6), m_clr3DShadow, m_clrControlBack);
		}
													...
													COLORREF clrColor, BOOL bYears)
{
	...
	if (bLeftDirection)
	{
		...
		{
			Triangle(pDC,
				CPoint(ptCenter.x - XTP_DPI_X(8), ptCenter.y),
				CPoint(ptCenter.x - XTP_DPI_X(4), ptCenter.y - XTP_DPI_Y(4)),
				CPoint(ptCenter.x - XTP_DPI_X(4), ptCenter.y + XTP_DPI_Y(4)),
				clrColor);

			Triangle(pDC,
				CPoint(ptCenter.x + XTP_DPI_X(4), ptCenter.y),
