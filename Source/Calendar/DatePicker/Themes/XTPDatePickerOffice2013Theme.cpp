...
void CXTPDatePickerThemeOffice2013::RefreshMetrics()
{
    ...
    m_clrNonMonthDayBack       = XTPIniColor(_T("Calendar.DatePicker"), _T("NonMonthDayBack"),       RGB(255, 255, 255));
    m_clrNonMonthDayText       = XTPIniColor(_T("Calendar.DatePicker"), _T("NonMonthDayText"),       RGB( 68,  68,  68));
    m_clrSelectedDayBack       = XTPIniColor(_T("Calendar.DatePicker"), _T("SelectedDayBack"),       RGB(205, 230, 247));
    m_clrSelectedDayText       = XTPIniColor(_T("Calendar.DatePicker"), _T("SelectedDayText"),       RGB( 68,  68,  68));
    m_clrMonthBorder           = XTPIniColor(_T("Calendar.DatePicker"), _T("MonthBorder"),           RGB(212, 212, 212));
    m_clrMonthHeaderBack       = XTPIniColor(_T("Calendar.DatePicker"), _T("MonthHeaderBack"),       RGB(240, 240, 240));
    m_clrMonthHeaderText       = XTPIniColor(_T("Calendar.DatePicker"), _T("MonthHeaderText"),       RGB( 68,  68,  68));
    m_clrMonthHeaderButton     = XTPIniColor(_T("Calendar.DatePicker"), _T("MonthHeaderButton"),     RGB( 68,  68,  68));
    m_clrDaysOfWeekBack        = XTPIniColor(_T("Calendar.DatePicker"), _T("DaysOfWeekBack"),        RGB(255, 255, 255));
    m_clrDaysOfWeekText        = XTPIniColor(_T("Calendar.DatePicker"), _T("DaysOfWeekText"),        RGB( 68,  68,  68));
    m_clrWeekNumbersBack       = XTPIniColor(_T("Calendar.DatePicker"), _T("WeekNumbersBack"),       RGB(255, 255, 255));
    m_clrWeekNumbersText       = XTPIniColor(_T("Calendar.DatePicker"), _T("WeekNumbersText"),       RGB( 68,  68,  68));
    m_clrHighlightToday        = XTPIniColor(_T("Calendar.DatePicker"), _T("HighlightToday"),        RGB(  0, 114, 198));
    m_clrHighlightTodayText    = XTPIniColor(_T("Calendar.DatePicker"), _T("HighlightTodayText"),    RGB(255, 255, 255));
    m_clrHighlightTodayBorder  = XTPIniColor(_T("Calendar.DatePicker"), _T("HighlightTodayBorder"),  RGB(  0, 114, 198));
    ...
    m_clrButtonBorder          = XTPIniColor(_T("Calendar.DatePicker"), _T("ButtonBorder"),          RGB(146, 192, 224));
    m_clrButtonPressed         = XTPIniColor(_T("Calendar.DatePicker"), _T("ButtonPressed"),         RGB(146, 192, 224));
    m_clrButtonPressedText     = XTPIniColor(_T("Calendar.DatePicker"), _T("ButtonPressedText"),     RGB( 68,  68,  68));
    m_clrButtonPressedBorder   = XTPIniColor(_T("Calendar.DatePicker"), _T("ButtonPressedBorder"),   RGB( 42, 141, 212));
    m_clrButtonHighlight       = XTPIniColor(_T("Calendar.DatePicker"), _T("ButtonHighlight"),       RGB(205, 230, 247));
    m_clrButtonHighlightText   = XTPIniColor(_T("Calendar.DatePicker"), _T("ButtonHighlightText"),   RGB( 68,  68,  68));
    m_clrButtonHighlightBorder = XTPIniColor(_T("Calendar.DatePicker"), _T("ButtonHighlightBorder"), RGB(146, 192, 224));
}
...
void CXTPDatePickerThemeOffice2013::DrawBackground(CDC* pDC, CRect rcClient)
{
	...
	{
		XTPDrawHelpers()->GradientFill(
			pDC, &rcClient, m_clrBackground, FALSE);
	}
	else
	{
		pDC->FillSolidRect(
			rcClient, m_clrBackground.clrDark);
	}
}
...
//	pDC->FillSolidRect(pMonth->m_rcWeekNumbers, GetWeekNumbersBackColor());
	...
	for (int nWeek = 0; nWeek < XTP_MAX_WEEKS; nWeek++)
	{
		...
		CXTPDatePickerItemDay* pDayFirst = pMonth->GetDay(XTPToIntChecked(nWeek * XTP_WEEK_DAYS));
		CXTPDatePickerItemDay* pDayLast = pMonth->GetDay(XTPToIntChecked((nWeek + 1) * XTP_WEEK_DAYS - 1));
	...
	pDC->FillSolidRect(rcLine.right - XTP_DPI_X(1), rcLine.top - XTP_DPI_Y(1), XTP_DPI_X(1), rcLine.Height(), m_clrMonthBorder);
}

void CXTPDatePickerThemeOffice2013::DrawButton(CDC* pDC, const CRect& rcButton, const CString& strButton, BOOL bIsDown, BOOL bIsHighLight)
{
	CRect rc(rcButton);
...
void CXTPDatePickerThemeOffice2013::DrawDaysOfWeek(CDC* pDC, CXTPDatePickerItemMonth* pMonth)
{
	...
	for (int nDayDelta = 0; nDayDelta < XTP_WEEK_DAYS; nDayDelta++)
	{
		...
		lfFont.lfHeight = CXTPDrawHelpers::ComputeFittingFontHeight(pDC, strText,
			strText.GetLength(), rcItem, *pFont);
