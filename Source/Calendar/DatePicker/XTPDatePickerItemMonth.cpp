...
void CXTPDatePickerItemMonth::OnLButtonDown(UINT nFlags, CPoint point)
{
		...
		&& m_rcLeftScroll.PtInRect(point))
	{
		...
		m_pControl->m_nTimerID = XTPToIntChecked(m_pControl->SetTimer(XTP_DATEPICKER_TIMERID, XTP_DATEPICKER_TIMER_INTERVAL, NULL));
	...
	if (!m_rcHeader.IsRectEmpty()
		&& CRect(m_rcHeader.left + XTP_DPI_X(20), m_rcHeader.top, m_rcHeader.right - XTP_DPI_X(20), m_rcHeader.bottom).PtInRect(point))
