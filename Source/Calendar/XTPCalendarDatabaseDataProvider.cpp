...
class CXTPCalendarDatabaseDataProvider::CADOCommand : public CXTPCmdTarget
{
public:
...
void CXTPCalendarDatabaseDataProvider::_UpdateSchedules(CXTPCalendarSchedules* pUpdateSet, CXTPCalendarSchedules* pOrigSet)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		long nID = XTPToLongChecked(pSch->GetID());
		...
		if (pOrigSet)
		{
			int nIdx = pOrigSet->FindIndex(XTPToUIntChecked(nID));
			ASSERT(nIdx >= 0);
...
void CXTPCalendarDatabaseDataProvider::_DeleteSchedules(CUIntArray* pDeleteIDs)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		ITOT_S(nID, szBuff, _countof(szBuff), 10);
		strIDs += szBuff;
	}
...
CXTPCalendarEventPtr CXTPCalendarDatabaseDataProvider::_ReadEvent_common(XTPADODB::_Recordset* pRS, BOOL bEcxception)
{
	...
	try
	{
		...
		ptrEvent->SetScheduleID(XTPToUIntChecked(_GetField<long>(pRS, L"ScheduleID", 0)));
								...
								COleDateTime dtFrom, COleDateTimeSpan spPeriod)
{
	...
	TCHAR szBuff[64];
	strSqlText.Replace(_T("$normal$"), ITOT_S((int)xtpCalendarRecurrenceNotRecurring, szBuff, _countof(szBuff), 10));
	strSqlText.Replace(_T("$master$"), ITOT_S((int)xtpCalendarRecurrenceMaster, szBuff, _countof(szBuff), 10));
	strSqlText.Replace(_T("$exception$"), ITOT_S((int)xtpCalendarRecurrenceException, szBuff, _countof(szBuff), 10));
