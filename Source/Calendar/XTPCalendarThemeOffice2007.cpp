...
public:
	...
	virtual ~CXTPCalendarTheme_DayViewSingleDayEventPlaceBuilder();
...

{
	...
	if ((sz1.cy + sz2.cy + XTP_DPI_Y(1)) > height)
	{
...
int AFX_CDECL CXTPCalendarThemeOffice2007::DrawClock(CDC* pDC, COleDateTime dtClockTime,
	CRect rcView, COLORREF clrBackground, XTPCalendarClockAlignFlags cafAlign)
{
	...
	if (cafAlign == xtpCalendarClockAlignLeft)
		ptClock.Offset(XTP_DPI_X(1), (rcView.Height() - m_szClkRc.cy)/2 + XTP_DPI_Y(1));
	else if (cafAlign == xtpCalendarClockAlignRight)
		ptClock.Offset(rcView.Width() - m_szClkRc.cx - XTP_DPI_X(1),
			(rcView.Height() - m_szClkRc.cy)/2 + XTP_DPI_Y(1));
...
void CXTPCalendarThemeOffice2007::RefreshMetrics()
{
	RefreshMetrics(TRUE);
}
...
int CXTPCalendarThemeOffice2007::CTODay::HitTestExpandDayButton(const CXTPCalendarViewDay* pViewDay, const CPoint* pPoint)
{
	...
	rc.right -= XTP_DPI_X(5);
	rc.bottom -= XTP_DPI_Y(5);
...
void CXTPCalendarThemeOffice2007::CTODayView::Draw(CDC* pDC)
{
	...
	if (pThemeX->GetPrevNextEventButtonsPartX())
		pThemeX->GetPrevNextEventButtonsPartX()->Draw((CXTPCalendarView*)NULL, pDC);
};
														...
														CRect& rrcRect, LONG lArrowOffset)
{
	...
	rcArrow.left = rcEventMax.left + XTP_DPI_X(5);
...
BOOL CXTPCalendarThemeOffice2007::CTOEvent::Draw_ArrowR(CXTPCalendarViewEvent* pViewEvent, CDC* pDC, CRect& rrcRect)
{
	...
	rcArrow.top = rcEvent.top + rcEvent.Height() / 2 - szArrow.cy / 2;
	rcArrow.bottom = rcArrow.top + szArrow.cy;
	rcArrow.left = rcEventMax.right - XTP_DPI_X(5) - szArrow.cx;
	rcArrow.right = rcArrow.left + szArrow.cx;
								...
								CDC* pDC, const CRect& rcIconsMax, BOOL bCalculate)
{
	...
	if (nGlyphCount > 0)
	{
		...
		for (int i = 0; i < nGlyphCount && pGlyphs; i++)
		{
				...
				ptNext_rb.y - szGlyph.cy >= rcIconsMax.top)
			{
				...
				ii.m_pImage = pGlyphs;
				ii.m_rcSrc = pGlyphs->GetSource(XTPToIntChecked(uIn), 5);
	...
	rcIcons.left = max(rcIcons.right - szIcons.cx - XTP_DPI_X(2), rcIconsMax.left);
	rcIcons.top = max(rcIcons.bottom - szIcons.cy - XTP_DPI_Y(2), rcIconsMax.top);
				...
				CDC* pDC, CThemeFontColorSetValue* pfcsetText, CRect& rrcRect, int nLeft_x)
{
	...
	rcText.left = nLeft_x + XTP_DPI_X(5);
				...
				CDC* pDC, CThemeFontColorSetValue* pfcsetText, CRect& rrcRect, int nRight_x)
{
	...
	CRect rcText = rcEvent;
	rcText.right = nRight_x - XTP_DPI_X(5);
				...
				CXTPCalendarDayViewDay* pDayViewDay, const CRect& rcDay)
{
	...
	int nWidthH = bIsToday ? XTP_DPI_X(2) : XTP_DPI_X(1);

	if (bIsOffice2010Theme)
		nWidthH = XTP_DPI_X(1);

	CRect rcGroups = rcDay;
	rcGroups.DeflateRect(nWidthH, 0, nWidthH, XTP_DPI_Y(1));
	return rcGroups;
}
													...
													CDC* pDC, const CRect& rcRect)
{
	...
	{
		int nDayWidth = rcRect.Width() - XTP_DPI_X(4);
	...
	else
	{
		...
		for (int i = 0; i < 7; i++)
		{
			CString strWDLong = CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SDAYNAME1 + i), 100);
			CString strWDShort = CXTPCalendarUtils::GetLocaleString(XTPToUIntChecked(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(i + 1)), 100);
		...
		if (nCenterTextWidth >= nWDLong_max + XTP_DPI_X(4))
		{
			m_nWeekDayFormat = 2;
		}
		else if (nCenterTextWidth >= nWDShort_max + XTP_DPI_X(4))
		{
			m_nWeekDayFormat = 1;
		}
		else
...
void CXTPCalendarThemeOffice2007::CTODayViewDayHeader::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	{
		int nPage = XTPToIntChecked(si.nPage); //Current visible width
	...
	else if (!pThemeX->GetCalendarControl()->m_bFullDateMode)
	{
		...
		{
			LCTYPE lctWDName1 = XTPToUIntChecked(m_nWeekDayFormat > 1 
				? LOCALE_SDAYNAME1 : CXTPCalendarUtils::GetDayOfWeekLocaleStringId(1));
	...
	if (pThemeX->GetCalendarControl()->m_bFullDateMode)
	{
		...
		if (!sFmt.IsEmpty())
		{
			...
			if (sFmt.Find(_T("%a, %b %d")) > -1)
			{
				CString strDayName = CXTPCalendarUtils::GetLocaleString(
					XTPToUIntChecked(LOCALE_SABBREVDAYNAME1 + ((dtDay.GetDayOfWeek() - 2 + 7) % 7)), 100);
			
				CString strMonthName = CXTPCalendarUtils::GetLocaleString(
					XTPToUIntChecked(LOCALE_SABBREVMONTHNAME1 + (dtDay.GetMonth() - 1)), 100);
				
				strTextCenter = strDayName + _T(", ") + strMonthName + dtDay.Format(_T(" %d"));
			}
			else
			{
...
int CXTPCalendarThemeOffice2007::CTOCellDynBackground::_GetEventBusyStatusRank(CXTPCalendarEvent* pEvent)
{
	...
	const int cnRankSize = 5;
	ASSERT(
		xtpCalendarBusyStatusFree < cnRankSize &&
		xtpCalendarBusyStatusElsewhere < cnRankSize &&
		xtpCalendarBusyStatusTentative < cnRankSize &&
		xtpCalendarBusyStatusBusy < cnRankSize &&
		xtpCalendarBusyStatusOutOfOffice < cnRankSize);

	static int sarBSrank[cnRankSize] = { 0, 0, 0, 0 };
	if (sarBSrank[0] == 0)
	{
		sarBSrank[xtpCalendarBusyStatusFree] = 1; // min rank
		sarBSrank[xtpCalendarBusyStatusElsewhere] = 2;
		sarBSrank[xtpCalendarBusyStatusTentative] = 3;
		sarBSrank[xtpCalendarBusyStatusOutOfOffice] = 4;
		sarBSrank[xtpCalendarBusyStatusBusy] = 5; // max rank
	}
...
void CXTPCalendarThemeOffice2007::CTODayViewDayGroupAllDayEvents::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	if (bIsOffice2010Theme)
		rcRectBk.bottom -= XTP_DPI_Y(1);
	else
		rcRectBk.bottom -= XTP_DPI_Y(2);
		...
		const CRect& rcRect, int nEventPlaceNumber)
{
	...
	if ((nMDEFlags & xtpCalendarMultiDayFirst) != 0)
		pDayViewEvent->m_rcEvent.left += XTP_DPI_X(8);

	if ((nMDEFlags & xtpCalendarMultiDayLast) != 0)
		pDayViewEvent->m_rcEvent.right -= XTP_DPI_X(8);
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	{
		pThemeX->_DrawEventLTHighlight(pDC, CPoint(rcEvent.left + XTP_DPI_X(nSelOffset), rcEvent.top + XTP_DPI_Y(nSelOffset)),
			rcEvent.right - XTP_DPI_X(nSelOffset), grclrBk, cnCornerSize / 2, rcEvent.bottom);
	}
	...
	if ((nMDEFlags & xtpCalendarMultiDayFirst) == 0)
	{
		...
		CXTPCalendarTheme::DrawRectPartly(pDC, rcSQ, clrBorder, bSelected, grclrBk, CRect(XTP_DPI_X(1), XTP_DPI_Y(1), 0, XTP_DPI_Y(1)));

		pThemeX->_DrawEventLTHighlight(pDC, CPoint(rcEvent.left + XTP_DPI_X(nSelOffset), rcEvent.top + XTP_DPI_Y(nSelOffset)),
			rcEvent.right - XTP_DPI_X(nSelOffset), grclrBk, 0, rcEvent.bottom - XTP_DPI_Y(nSelOffset));
	...
	else if (bSelected)
	{
		// draw left Gripper
		CRect rcGripper;
		rcGripper.top = rcEvent.top + rcEvent.Height() / 2 - XTP_DPI_Y(cnGripperSize) / 2;
		rcGripper.bottom = rcGripper.top + XTP_DPI_Y(cnGripperSize);
		rcGripper.left = rcEvent.left - XTP_DPI_X(1);
		rcGripper.right = rcGripper.left + XTP_DPI_X(cnGripperSize);
	...
	if ((nMDEFlags & xtpCalendarMultiDayLast) == 0)
	{
		...
		CXTPCalendarTheme::DrawRectPartly(pDC, rcSQ, clrBorder, bSelected, grclrBk, CRect(0, XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1)));

		pThemeX->_DrawEventLTHighlight(pDC, CPoint(rcSQ.left, rcEvent.top + XTP_DPI_Y(nSelOffset)),
			rcEventMax.right - XTP_DPI_X(nSelOffset), grclrBk, 0, INT_MAX, FALSE);
		if (!bHideArrow)
		{
			CRect rcArrow;
	...
	else if (bSelected)
	{
		...
		rcGripper.left = rcGripper.right - XTP_DPI_X(cnGripperSize);
	...
	if (pThemeX->GetCalendarControl()->IsMarkupEnabled())
	{
		...
		if (pUIElement && pMarkupContext)
		{
			...
			if (rcIconsMax.Height() > szIcons.cy)
				rcIconsMax.bottom -= (rcIconsMax.Height() - szIcons.cy) / 2 + XTP_DPI_Y(1);

			rcText.right = max(rcText.left,  rcText.right - szIcons.cx - XTP_DPI_X(2));
	...
	if (!bMarkupDrawn)
	{
		...
		if (nTIcx + XTP_DPI_X(5) < rcText.Width())
		{
			int ncx_2 = (rcText.Width() - nTIcx) / 2 - XTP_DPI_X(2);
			rcText.left += ncx_2;
			rcText.right = rcText.left + szText.cx + XTP_DPI_X(2);

			rcIconsMax.right -= ncx_2;
		}
		else
		{
			rcText.right -= szIcons.cx + XTP_DPI_X(2);
		}
	...
	rcCategoryIcons.left = rcText.right + XTP_DPI_X(1);
...
const int CXTPCalendarThemeOffice2007::CTODayViewEvent::nCategoryIconWidth = XTP_DPI_X(6);
const int CXTPCalendarThemeOffice2007::CTODayViewEvent::nCategoryIconOffset = XTP_DPI_X(1);
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_SingleDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	if (!bShortEvent)
	{
		...
		if (DrawShowAsStatus(pDC, pViewEvent, pThemeX, rcEvent, lShowAsBlockWidth))
		{
			...
			pThemeX->_DrawEventLTHighlight(pDC, CPoint(rcText.left, rcEvent.top + XTP_DPI_Y(nSelOffset)), rcEvent.right - XTP_DPI_X(nSelOffset), grclrBk);
		}
		else
		{
			pThemeX->_DrawEventLTHighlight(pDC, CPoint(rcEvent.left + XTP_DPI_X(nSelOffset), rcEvent.top + XTP_DPI_Y(nSelOffset)),
	...
	{
		CRect rcBS = rcEvent;
		rcBS.right = rcEventX.left + XTP_DPI_X(6);
		...
		if (pBSBrush)
		{
			...
			CXTPDCSaveState dcState(pDC);
			pDC->SetTextColor(clrBorderNotSel);
			pDC->SetBkColor(RGB(255, 255, 255));
			pDC->FillRect(&rcBSfill, pBSBrush);
		}

		// draw borders
		int nBorderWidth = bSelected ? XTP_DPI_X(2) : XTP_DPI_X(1);
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBS, clrBorder, nBorderWidth, CRect(XTP_DPI_X(1), XTP_DPI_Y(1), 0, 0));

		rcBS.right = rcEventX.left + XTP_DPI_X(1);
		rcBS.top += nBorderWidth;
		if (pBSBrush)
		{
			CXTPCalendarTheme::DrawRectPartly(pDC, rcBS, clrBorderNotSel, XTP_DPI_X(1), CRect(0, 0, XTP_DPI_X(1), 0));
		}

		rcBS.right += bSelected ? XTP_DPI_X(1) : 0;
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBS, clrBorder, nBorderWidth, CRect(0, 0, 0, XTP_DPI_Y(1)));

		rcText.left = rcEventX.left;

		if (pBSBrush)
		{
			pThemeX->_DrawEventLTHighlight(pDC, CPoint(rcEventX.left + XTP_DPI_X(1), rcEvent.top + XTP_DPI_Y(nSelOffset)),
				rcEvent.right - XTP_DPI_X(nSelOffset), grclrBk);
		}
		else
		{
			pThemeX->_DrawEventLTHighlight(pDC, CPoint(rcEvent.left + XTP_DPI_X(nSelOffset), rcEvent.top + XTP_DPI_Y(nSelOffset)),
				rcEvent.right - XTP_DPI_X(nSelOffset), grclrBk, 0, rcTimeframe.bottom - XTP_DPI_Y(nSelOffset));
		}
	}
	...
	if (bSelected)
	{
		...
		rcGripper.left = rcEvent.left + rcEvent.Width() / 2 - XTP_DPI_X(cnGripperSize) / 2;
		rcGripper.right = rcGripper.left + XTP_DPI_X(cnGripperSize);

		// draw top Gripper
		rcGripper.top = rcEvent.top - XTP_DPI_Y(1); //XTP_DPI_Y(cnGripperSize) / 2 + 1;
		rcGripper.bottom = rcGripper.top + XTP_DPI_Y(cnGripperSize);
		...
		// draw bottom Gripper
		rcGripper.bottom = rcEvent.bottom + XTP_DPI_Y(1); //XTP_DPI_Y(cnGripperSize) / 2 - 1;
		rcGripper.top = rcGripper.bottom - XTP_DPI_Y(cnGripperSize);
	...
	{
		rcIconsMax.bottom -= XTP_DPI_Y(4);
	}
	else
	{
		if (rcTopRow.Height() - XTP_DPI_X(4) > szIcons.cy)
		{
		...
		rcText.right = max(rcText.right - szIcons.cx - XTP_DPI_X(2), rcText.left + rcTopRow.Height() - XTP_DPI_Y(2));
	}
...
//test case >>
		...
		rcText.right = rcEventX.right - XTP_DPI_X(5); //<< 2010 >>
...
{
	CRect rcCategoryIcons = rcText;
	rcCategoryIcons.top = rcCategoryIcons.bottom - min(XTP_DPI_Y(20), (rcTopRow.Height() - XTP_DPI_Y(6)));
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_SingleDay::DrawShadow(CDC* pDC, CXTPCalendarDayViewEvent* pViewEvent)
{
	...
	BOOL bShortEvent = rcTimeframe.Height() + XTP_DPI_Y(1) < rcTopRow.Height();
	// --- Calculate event rectangle ---
	CRect rcRect(rcEvent);
	if (bShortEvent)
		rcRect.left += XTP_DPI_X(5);
	...
	rcBLShadow.left -= XTP_DPI_X(5);
	oShadow.DrawShadowRectPart(rcBLShadow, oShadow.xtpShadow2007BottomLeft);

	CRect rcBRShadow(rcVShadow.left, rcHShadow.top, rcVShadow.right, rcHShadow.bottom);
	oShadow.DrawShadowRectPart(rcBRShadow, oShadow.xtpShadow2007BottomRight);

	CRect rcCShadow(rcVShadow.left - XTP_DPI_X(3), rcHShadow.top - XTP_DPI_Y(3), rcVShadow.left, rcHShadow.top);
	oShadow.DrawShadowRectPart(rcCShadow, oShadow.xtpShadow2007Center);
									...
									int nLTCorner, int nLBy, BOOL bRCorner)
{
	...
	CRect rcTop(ptLT.x + nLTCorner, ptLT.y, nRTx, ptLT.y + XTP_DPI_Y(1));
	if (bRCorner)
		rcTop.right -= cnCornerSize / 2 + XTP_DPI_X(1);

	pDC->FillSolidRect(&rcTop, clrTop);

	if (nLBy != INT_MAX)
	{
		CRect rcLLVert(ptLT.x, ptLT.y + nLTCorner, ptLT.x + XTP_DPI_X(1), nLBy - nLTCorner);
		rcLLVert.top += nLTCorner ? 0 : XTP_DPI_Y(1);
		...
		CXTPPenDC autoPen(pDC->GetSafeHdc(), PS_SOLID, XTP_DPI_X(1), clrLight);
	...
	if (bRCorner)
	{
		...
		CXTPPenDC autoPen(pDC->GetSafeHdc(), PS_SOLID, XTP_DPI_X(1), clrLight2);
		pDC->MoveTo(rcTop.right, rcTop.top);
		pDC->LineTo(rcTop.right + cnCornerSize / 2 + XTP_DPI_X(1), rcTop.top + cnCornerSize / 2 + XTP_DPI_Y(1));
	}
}
...
void CXTPCalendarThemeOffice2007::CTOEvent::InitBusyStatusDefaultColors()
{
	...
	CXTPResourceManager::AssertValid(XTPResourceManager()->LoadBitmap(&m_bmpElsewherePattern, XTP_IDB_CALENDAR_ELSEWHERE_PATTERN_2007));
	...
	CXTPResourceManager::AssertValid(XTPResourceManager()->LoadBitmap(&m_bmpTentativePattern, XTP_IDB_CALENDAR_TENTATIVE_PATTERN_2007));
							...
							CDC* pDC, const CRect& rcRect, int nEventPlaceNumber)
{
	...
	int nEventsAreaX = pViewEvent->m_rcEvent.Width() - XTP_DPI_X(5);
	...
	if (nEventWidth <= XTP_DPI_X(5))
	{
		while (nPaceCount > 1 && nEventWidth <= XTP_DPI_X(5))
		{
			nPaceCount--;
	...
	BOOL bShortEvent = nEvent_dy + XTP_DPI_Y(1) < rcTopRow.Height();
	nEvent_dy = max(XTP_DPI_Y(5), nEvent_dy);
	...
	pViewEvent->m_rcEventMax.top = max(pViewEvent->m_rcEvent.top - XTP_DPI_Y(5), rcRect.top);
	pViewEvent->m_rcEventMax.bottom = min(pViewEvent->m_rcEvent.bottom + XTP_DPI_Y(5), rcRect.bottom);

	// adjust subject area
	pViewEvent->m_rcText.CopyRect(pViewEvent->m_rcEvent);

	pViewEvent->m_rcText.left += XTP_DPI_X(10); // correct to busy area
	pViewEvent->m_rcText.left += XTP_DPI_X(4);
	...
	pViewEvent->m_rcTimeframe.left = pViewEvent->m_rcEvent.left + XTP_DPI_X(1);
	pViewEvent->m_rcTimeframe.right = pViewEvent->m_rcTimeframe.left + XTP_DPI_X(4);

	// set expanded signs for day view
	CRect rcEvents = rcRect;

	if (pViewEvent->m_rcEvent.top >= rcEvents.bottom - XTP_DPI_Y(10))
		pViewEvent->GetViewGroup()->SetExpandDown();

	if (pViewEvent->m_rcEvent.bottom <= rcEvents.top + XTP_DPI_Y(10))
		...
		CDC* pDC, const CRect& rcRect, int nEventPlaceNumber)
{
	...
	pViewEvent->m_rcEvent.left = pViewEvent->m_rcEventMax.left + XTP_DPI_X(4) + XTP_DPI_X(2);
	pViewEvent->m_rcEvent.right = pViewEvent->m_rcEventMax.right - XTP_DPI_X(5) - XTP_DPI_X(2);
	...
	pViewEvent->m_rcText.CopyRect(pViewEvent->m_rcEvent);
	pViewEvent->m_rcText.DeflateRect(XTP_DPI_X(nDyx) + XTP_DPI_X(3), XTP_DPI_Y(3), XTP_DPI_X(nDyx) + XTP_DPI_X(2), XTP_DPI_Y(3));
	...
	pViewEvent->m_rcSubjectEditor.top += XTP_DPI_Y(1);
	pViewEvent->m_rcSubjectEditor.bottom -= XTP_DPI_Y(1);
}
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekDayHeader::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	for (int i = 0; i < 7; i++)
	{
		CString sWDlong = CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SDAYNAME1 + i), 100);
		CString sWDshort = CXTPCalendarUtils::GetLocaleString(XTPToUIntChecked(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(i + 1)), 100);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekDayHeader::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	for (int nCol = 0; nCol < nColsCount; nCol++)
	{
		...
		int nWeekDay = XTPToIntChecked(arWeekDays[nCol]);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekHeader::GetWeekHeaderRect(CXTPCalendarThemeOffice2007* pThemeX, CXTPCalendarMonthViewDay* pViewDay, CRect& rcWHeader)
{
	...
	rcWHeader.left += XTP_DPI_X(3);
	rcWHeader.top = dayLayout.m_rcDayHeader.bottom - XTP_DPI_Y(1);
		...
		LPCTSTR pcszCenterText, int* pnWidth)
{
	...
	rcWHeaderTextVert.DeflateRect(XTP_DPI_X(1), XTP_DPI_Y(3), XTP_DPI_X(1), XTP_DPI_Y(3));
	...
	if (szText.cx + XTP_DPI_X(7) < nRectSize)
	{
							...
							CString& rstrShort, CString& rstrLong, int nShort1Long2 )
{
	...
	{
		CString strMonth1 = CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SABBREVMONTHNAME1 + dtWDay1.GetMonth() - 1), 100);
		CString strMonth6 = CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SABBREVMONTHNAME1 + dtWDay6.GetMonth() - 1), 100);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	if (bIsToday)
	{
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrTodayBorder, XTP_DPI_X(2), CRect(XTP_DPI_X(1), 0, XTP_DPI_X(1), XTP_DPI_Y(1)));
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(1), CRect(0, 0, XTP_DPI_X(1), 0));
	}
	else
	{
		...
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(1), CRect(nIsLBorder, 0, XTP_DPI_X(1), 0));
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(2), CRect(0, 0, 0, XTP_DPI_Y(1)));
	}
		...
		if (pViewEvent && pViewEvent->IsVisible())
		{
			if (pViewEvent->IsMultidayEvent())
			{
				...
				else
				{
					...
					BOOL bDrawStartTime = 0 != (dwOpt & xtpCalendarOptMonthViewShowStartTimeAlways);
					BOOL bDrawEndTime = bShowEndTime && 0 != (dwOpt & xtpCalendarOptMonthViewShowEndTimeAlways);
	...
	if (bDraftMode && pDC->IsPrinting())
	{
		...
		rcPrint.DeflateRect(XTP_DPI_X(3), XTP_DPI_Y(3));

		CXTPFont font;
		font.CreatePointFont(70, _T("Arial"));
		CXTPFontDC autoFont(pDC, &font, RGB(0,0,0));
...
void CXTPCalendarThemeOffice2007::CTOMonthViewDayHeader::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	{
		CString strMonth = CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SABBREVMONTHNAME1 + nMonth - 1), 100);
																		...
																		CXTPCalendarViewEvent* pViewEvent, BOOL MultiDayFirstPart, BOOL MultiDayLastPart, BOOL fStartTimeAsClock, BOOL fEndTimeAsClock)
{
	...
	BOOL bDrawStartEndTimes = szStart.cx + szEnd.cx + XTP_DPI_X(12) < rcEventRect.Width()/2;

	if (!bDrawStartEndTimes && !bDrawStartTime && !bDrawEndTime)
		return;

	UINT uAlign = DT_VCENTER | DT_SINGLELINE;
	rcStart.left = min(rcStart.left + XTP_DPI_X(6), rcStart.right);
	rcStart.right = min(rcStart.left + szStart.cx + XTP_DPI_X(2), rcStart.right);
	if (bDrawStartTime)
	{
	...
	rcEnd.right = max(rcEnd.right - XTP_DPI_X(6), rcEventRect.left);
	rcEnd.left = max(rcEnd.right - szEnd.cx - XTP_DPI_X(2), rcEventRect.left);
	if (bDrawEndTime)
	{
		if (fEndTimeAsClock)
...
void CXTPCalendarThemeOffice2007::CTOMonthViewEvent_MultiDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	else if (bSelected)
	{
		...
		rcGripper.top = rcEvent.top + rcEvent.Height() / 2 - XTP_DPI_Y(cnGripperSize) / 2;
		rcGripper.bottom = rcGripper.top + XTP_DPI_Y(cnGripperSize);
		rcGripper.right = rcEvent.right + XTP_DPI_X(1);
		rcGripper.left = rcGripper.right - XTP_DPI_X(cnGripperSize);
	...
	pViewEvent->m_rcSubjectEditor.CopyRect(rcText);
	pViewEvent->m_rcSubjectEditor.top = rcEvent.top + XTP_DPI_Y(2);
	pViewEvent->m_rcSubjectEditor.bottom = rcEvent.bottom - XTP_DPI_Y(2);
	pViewEvent->AdjustSubjectEditor();
...
BOOL CXTPCalendarThemeOffice2007::CTOEvent::DrawShowAsStatus(CDC* pDC, CXTPCalendarViewEvent* pViewEvent, CXTPCalendarThemeOffice2007* pThemeX, const CRect& rcEvent, LONG& lShowAsBlockWidth)
{
	...
	CRect rcBS = rcEvent;
	rcBS.right = min(rcBS.left + XTP_DPI_X(12), rcEvent.right);
	...
	rcBS.right++;
	rcBS.left = min(rcBS.left + XTP_DPI_X(5), rcBS.right);

	CXTPCalendarTheme::DrawRectPartly(pDC, rcBS, clrBorder, bSelected, grclrBk, CRect(0, XTP_DPI_Y(1), 0, XTP_DPI_Y(1)));

	rcBS.right = rcBS.left + XTP_DPI_X(1);
	pDC->FillSolidRect(&rcBS, clrBorder);

	lShowAsBlockWidth = XTP_DPI_X(6);
	return TRUE;
}
		...
		CDC* pDC, const CRect& rcEventRect, CXTPCalendarMonthViewEvent* pViewEvent)
{
	...
	DWORD dwOpt = XTP_SAFE_GET3(GetTheme(), GetCalendarControl(), GetCalendarOptions(), dwAdditionalOptions, 0);
	BOOL bShowEndTime = XTP_SAFE_GET2(pViewEvent, GetCalendarControl(), MonthView_IsShowEndDate(), TRUE);
	BOOL bDrawStartTime = szStart.cx + XTP_DPI_X(6) < rcEventRect.Width() / 2 && //||
		(dwOpt & xtpCalendarOptMonthViewShowStartTimeAlways);
	BOOL bDrawEndTime = bShowEndTime && szStart.cx + szEnd.cx + XTP_DPI_X(12) < rcEventRect.Width()/2 && //||
	...
	if (bDrawStartTime)
	{
		if (pViewEvent->IsTimeAsClock())
		{
			...
			rcStart.left += iOffset + XTP_DPI_X(5);
			rcStart.right = rcStart.left;
		...
		{
			rcStart.left = min(rcStart.left + XTP_DPI_X(4), rcStart.right);
			rcStart.right = min(rcStart.left + szStart.cx + XTP_DPI_X(6), rcStart.right);
	...
	{
		rcStart.right = rcStart.left + XTP_DPI_X(5);
	}
	...
	if (bDrawEndTime)
	{
		if (pViewEvent->IsTimeAsClock())
		{
			...
			rcEnd.left += iOffset;
			rcEnd.right = rcEnd.left + XTP_DPI_X(5);
		}
		else
		{
			rcEnd.left = rcStart.right;
			rcEnd.right = min(rcEnd.left + szEnd.cx + XTP_DPI_X(6), rcEnd.right);
		...
		CDC* pDC, const CRect& rcTextRect, CXTPCalendarMonthViewEvent* pViewEvent)
{
	...
	{
		rcText.right = max(rcIconsMax.left, rcIconsMax.right - szIcons.cx) - XTP_DPI_X(2);
	}
		...
		CDC* pDC, const CRect& rcRect, int nEventPlaceNumber)
{
	...
	// adjust subject area
	int nDyx = pViewEvent->IsSelected() ? XTP_DPI_X(5) : XTP_DPI_X(2);

	pViewEvent->m_rcText.CopyRect(pViewEvent->m_rcEvent);
	pViewEvent->m_rcText.DeflateRect(nDyx + XTP_DPI_X(3), XTP_DPI_Y(3), nDyx + XTP_DPI_X(2), XTP_DPI_Y(3));
...
void CXTPCalendarThemeOffice2007::CTOWeekViewDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	if (bIsToday)
	{
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrTodayBorder, XTP_DPI_X(2), CRect(XTP_DPI_X(1), 0, 0, XTP_DPI_Y(1)));
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrTodayBorder, XTP_DPI_X(1), CRect(0, 0, XTP_DPI_X(1), 0));
	...
	else
	{
		...
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(1), CRect(nIsLBorder, 0, nIsRBorder, 0));
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(2), CRect(0, 0, 0, XTP_DPI_Y(1)));
	}
...
void CXTPCalendarThemeOffice2007::CTOWeekViewEvent_MultiDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	{
		CRect rcSQ = rcEventMax;
		rcSQ.left += min(XTP_DPI_X(1), rcSQ.Width());

		rcSQ.right = min(rcEvent.left + cnCornerSize, rcEventMax.right);
		CXTPCalendarTheme::DrawRectPartly(pDC, rcSQ, clrBorder, bSelected, grclrBk, CRect(0, XTP_DPI_Y(1), 0, XTP_DPI_Y(1)));
	...
	if ((nMDEFlags & xtpCalendarMultiDayLast) == 0)
	{
		CRect rcSQ = rcEventMax;
		rcSQ.left = max(rcEvent.right - cnCornerSize, rcEventMax.left);
		rcSQ.right -= min(XTP_DPI_X(2), rcSQ.Width());
		CXTPCalendarTheme::DrawRectPartly(pDC, rcSQ, clrBorder, bSelected, grclrBk, CRect(0, XTP_DPI_Y(1), 0, XTP_DPI_Y(1)));
		...
		CDC* pDC, const CRect& rcEventRect, CXTPCalendarWeekViewEvent* pViewEvent)
{
	...
	BOOL bDrawStartTime = 0 != (dwOpt & xtpCalendarOptWeekViewShowStartTimeAlways);
	BOOL bDrawEndTime = bShowEndTime && 0 != (dwOpt & xtpCalendarOptWeekViewShowEndTimeAlways);
...
BEGIN_INTERFACE_MAP(CXTPCalendarThemeOffice2007, CXTPCalendarTheme)
	INTERFACE_PART(CXTPCalendarThemeOffice2007, XTPDIID_ICalendarThemeOffice2007, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarThemeOffice2007, XTPDIID_ICalendarThemeOffice2007)
