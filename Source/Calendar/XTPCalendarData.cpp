...
//===========================================================================
IMPLEMENT_DYNAMIC(CXTPCalendarData, CXTPCmdTarget)
...
BOOL CXTPCalendarData::AddEventLabel(int nID, CString strName, COLORREF crLabel)
{
	if (m_pLabelList)
	{
		m_pLabelList->Add(new CXTPCalendarEventLabel(
			nID, crLabel, strName));

		m_pLabelList->SetDefaultValues(FALSE);
...
BOOL CXTPCalendarData::AddEventCategory(UINT uID, CString strName, COLORREF crBorder, COLORREF crBkBase)
{
	if (m_pEventCategories)
	{
		m_pEventCategories->Add(new CXTPCalendarEventCategory(
			uID, strName, crBorder, crBkBase));

		m_pEventCategories->SetDefaultValues(FALSE);
...
void CXTPCalendarData::ClearEventCategoryList()
{
	if (m_pEventCategories)
	{
		m_pEventCategories->RemoveAll();
	}
}

void CXTPCalendarData::ClearEventLabelList()
{
	if (m_pLabelList)
	{
...
void CXTPCalendarData::RestoreEventCategoryList()
{
	if (m_pEventCategories)
	{
		m_pEventCategories->SetDefaultValues(TRUE);
		m_pEventCategories->InitDefaultValues();
	}
}

void CXTPCalendarData::RestoreEventLabelList()
{
	if (m_pLabelList)
	{
		m_pLabelList->SetDefaultValues(TRUE);
		m_pLabelList->InitDefaultValues();
	}
}
...
void CXTPCalendarData::UpdateEventCategories(COLORREF* pClrBkBase, COLORREF* pClrBorder, int nCount)
{
	m_pEventCategories->UpdateDefaultValues(pClrBkBase, pClrBorder, nCount);
}
...
CXTPCalendarData* CXTPCalendarData::CreateDataProvider(XTPCalendarDataProvider eDataProvider)
{
	...
	case xtpCalendarDataProviderCustom:
		...
		break;
	case xtpCalendarDataProviderUnknown:
	default:
		ASSERT(FALSE);
	}
...
BEGIN_DISPATCH_MAP(CXTPCalendarData, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPCalendarData, CXTPCmdTarget)
	INTERFACE_PART(CXTPCalendarData, XTPDIID_CalendarDataProvider, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarData, XTPDIID_CalendarDataProvider)
...
{
	CXTPCalendarEvent* pEvent = DYNAMIC_DOWNCAST(CXTPCalendarEvent, 
		CXTPCalendarEvent::FromIDispatchSafe(Event));
	if (!pEvent)
	{
