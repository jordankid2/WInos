...
void CTOPrevNextEventButton::RefreshMetrics(BOOL /*bRefreshChildren*/)
{
	...
	m_rcBitmapBorder.SetStandardValue(CRect(XTP_DPI_X(1), XTP_DPI_Y(8), XTP_DPI_X(1), XTP_DPI_Y(8)));
	...
	m_strText.SetStandardValue(XTPResourceManager()->LoadString(
		XTPToUInt(m_bPrev ? XTP_IDS_CALENDAR_PREVAPPOINTMENT : XTP_IDS_CALENDAR_NEXTAPPOINTMENT)));
}

CSize CTOPrevNextEventButton::GetSize(const CRect* prcRect)
{
	CSize szSize(XTP_DPI_X(23), XTP_DPI_Y(172));

	if (prcRect)
	{
		if (prcRect->Height() < XTP_DPI_Y(35) ||
			prcRect->Width() < szSize.cx * 2 + XTP_DPI_X(1))
		{
			szSize.cx = szSize.cy = 0;
		}
		else if (szSize.cy + XTP_DPI_Y(70) > prcRect->Height())
		{
			szSize.cy = XTP_DPI_Y(34);
		}
	}
...
void CTOPrevNextEventButton::Draw(CXTPCalendarView* /*pObject*/, CDC* pDC)
{
	...
	rcArrDest.OffsetRect(XTP_DPI_X(9), XTP_DPI_Y(12));
	...
	if (rcText.Height() > XTP_DPI_X(100))
	{
		rcText.DeflateRect(XTP_DPI_X(1), XTP_DPI_Y(3), XTP_DPI_X(1), XTP_DPI_Y(3));
