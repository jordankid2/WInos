...
void CXTPCalendarWeekViewEvent::AdjustLayout(CDC* pDC, const CRect& rcEventMax, int nEventPlaceNumber)
{
	...
	if (!IsMultidayEvent())
	{
		...
		if (IsTimeAsClock())
		{
			nTimeCellWidth = XTP_DPI_X(2) + XTP_SAFE_GET2(GetView(), GetPaintManager(), GetClockSize().cx, 0);
		}
		else
		{
			CString strTime = XTP_SAFE_GET1(GetView(), GetItemTextEventTimeMax(), _T(""));
			nTimeCellWidth = XTP_DPI_X(2) + XTP_SAFE_GET3(GetView(), GetPaintManager(), GetWeekViewEventPart(), GetTextExtent(pDC, strTime).cx, 0);
		}
			...
			(dwOpt & (xtpCalendarOptWeekViewShowStartTimeAlways | xtpCalendarOptWeekViewShowEndTimeAlways)))
		{
			...
			if (IsShowEndTime() &&
				(m_rcText.Width() - nText > XTP_DPI_X(1) || (dwOpt & xtpCalendarOptWeekViewShowEndTimeAlways)))
			{
