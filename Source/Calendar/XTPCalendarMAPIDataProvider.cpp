...
#ifndef MIDL_PASS
#define MIDL_PASS // skips 'WCHAR' redefinition.
#endif
...
{
	// layout: [w:buf-size][w:str-len][ascii-char: count=len]
	SIZE_T nStrLenW = _tcslen(pcszString);
	SIZE_T nStrSize = nStrLenW*2 + 1;
	...
	SIZE_T nBuffer = nStrLen + 1;

	xtpAddDataT<WORD>(rarData, (WORD)nBuffer);
	xtpAddDataT<WORD>(rarData, (WORD)nStrLen);

	SIZE_T nSize = XTPToSizeTChecked(rarData.GetSize());
	rarData.SetSize(XTPToIntPtrChecked(nSize + nStrLen), 4096);
...
public:
	...
	public:
		...
		void SortByNewStart();
	protected:
		typedef int (AFX_CDECL* T_CompareFunc)(const CXTPRcException* pExc1, const CXTPRcException* pExc2);
		void _Sort(T_CompareFunc);

		static int AFX_CDECL CompareAsc_ByOrigStart(const CXTPRcException* pExc1, const CXTPRcException* pExc2);
		static int AFX_CDECL CompareAsc_ByNewStart(const CXTPRcException* pExc1, const CXTPRcException* pExc2);
	};
...
int AFX_CDECL CXTPCalendarMAPI_Recurrence::CXTPRcExceptions::CompareAsc_ByOrigStart(const CXTPRcException* pExc1, const CXTPRcException* pExc2)
{
...
int AFX_CDECL CXTPCalendarMAPI_Recurrence::CXTPRcExceptions::CompareAsc_ByNewStart(const CXTPRcException* pExc1, const CXTPRcException* pExc2)
{
						...
						)
{
	...
	for (i = 0; i < nCount; i++)
	{
		...
		if (RCExc.wFlags & xtpMAPIRcED_ReminderTime)
		{
			...
			RCExc.nReminderTime = xtpGetNextDataT<int>(pExcData, pMaxPtr);
		}
		...
		if (RCExc.wFlags & xtpMAPIRcED_BusyStatus)
		{
			...
			RCExc.nBusyStatus = xtpGetNextDataT<int>(pExcData, pMaxPtr);
		}
		...
		{
			// layout: [dw] -
			RCExc.nLabel = xtpGetNextDataT<int>(pExcData, pMaxPtr);
		}
						...
						const CXTPCalendarMAPI_Recurrence::CXTPRcData& RCData)
{
	...
	switch (RCData.wType)
	{
		...
		case xtpMAPIRcType_Yearly:
		{
			...
			else if (RCData.ucType2 == xtpMAPIRcType2_MonYearly_Nth)
			{
				...
				opt.m_YearNth.nWhichDayMask = XTPToIntChecked(RCData.dwWeekDayMask);
	...
	case xtpMAPIRcEnd_Number:
		...
		rpEnd.m_nEndAfterOccurrences = XTPToIntChecked(RCData.dwOccurrences);
		break;
	...
	dtOccStartTime.SetTime(XTPToIntChecked(RCData.dwOccStartTime / 60),
		XTPToIntChecked(RCData.dwOccStartTime % 60), 0);

	pPattern->SetStartTime(dtOccStartTime);

	int nDuration = XTPToIntChecked(RCData.dwOccEndTime - RCData.dwOccStartTime);
	ASSERT(nDuration >= 0);
									...
									CXTPCalendarEvent* pMaster)
{
	...
	switch (opt.m_nRecurrenceType)
	{
		case xtpCalendarRecurrenceDaily:
		{
			...
			else
			{
				...
				rRCData.dwInterval = XTPToUIntChecked(opt.m_Daily.nIntervalDays * 24 * 60);
			}
		}
		break;
		...
		case xtpCalendarRecurrenceWeekly:
		{
			...
			rRCData.dwInterval = XTPToUIntChecked(opt.m_Weekly.nIntervalWeeks);
			rRCData.dwWeekDayMask = XTPToUIntChecked(opt.m_Weekly.nDayOfWeekMask);
		}
		break;
	...
	default:
		ASSERT(!"Unexpected recurrence type");
		break;
	}
	...
	case xtpCalendarPatternEndAfterOccurrences:
		...
		rRCData.dwOccurrences = XTPToUIntChecked(rpEnd.m_nEndAfterOccurrences);
	...
	rRCData.dwOccStartTime = XTPToUIntChecked(dtOccStartTime.GetHour() * 60 + dtOccStartTime.GetMinute());
...
BOOL CXTPCalendarMAPI_Recurrence::RCDataToBin(CXTPRcData& RCData, CByteArray& rarData)
{
	...
	xtpAddDataT<int>(rarData, nExcCount);
	...
	for (i = 0; i < nExcCount; i++)
	{
		...
		{
			// layout: [dw] - bool(0|1)
			xtpAddDataT<BOOL>(rarData, rce.bIsMeeting);
		}
		if (rce.wFlags & xtpMAPIRcED_ReminderTime)
		{
			// layout: [dw] minutes before start
			xtpAddDataT<int>(rarData, rce.nReminderTime);
		}
		if (rce.wFlags & xtpMAPIRcED_IsReminder)
		{
			// layout: [dw] - bool(0|1)
			xtpAddDataT<BOOL>(rarData, rce.bIsReminder);
		}
		...
		{
			// layout: [dw] - busy status [0..3] = XTPCalendarEventBusyStatus
			xtpAddDataT<int>(rarData, rce.nBusyStatus);
		}
		if (rce.wFlags & xtpMAPIRcED_IsAllDay)
		{
			// layout: [dw] - bool(0|1)
			xtpAddDataT<BOOL>(rarData, rce.bIsAllDay);
		}
		if (rce.wFlags & xtpMAPIRcED_Label)
		{
			// layout: [dw] -
			xtpAddDataT<int>(rarData, rce.nLabel);
		}
	}
...
DWORD CXTPCalendarMAPI_Recurrence::_CalcRCShiftX(CXTPCalendarRecurrencePattern* pPattern)
{
	...
	if (opt.m_nRecurrenceType == xtpCalendarRecurrenceDaily && !opt.m_Daily.bEveryWeekDayOnly)
	{
		...
		dwRCShiftX = XTPToUInt(nDay % nIntervalDays);
		...
		opt.m_nRecurrenceType == xtpCalendarRecurrenceWeekly)
	{
		...
		dwRCShiftX = XTPToUInt((nWeek * 7 - 1) % (nIntervalWeeks * 7));
		dwRCShiftX *= 24 * 60; // convert to minutes
	}
	else
	{
...
STDMETHODIMP CXTPCalendarMAPIDataProvider::OpenInbox(
	LPMDB lpMDB, LPMAPIFOLDER *lpInboxFolder, 
	LPMAPIFOLDER *lpCalendarFolder, CXTPCalendarMAPIDataProvider* pProvider)
{
	if (!lpMDB || !pProvider)
		return E_POINTER;
...
STDMETHODIMP CXTPCalendarMAPIDataProvider::OpenDefaultMessageStore(
	...
	LPMDB * lpMDB, CXTPCalendarMAPIDataProvider* pProvider)
{
...
BOOL CXTPCalendarMAPIDataProvider::DoCreate_Event(CXTPCalendarEvent* pEvent, DWORD& rdwNewEventID)
{
	...
	rdwNewEventID = GetEventID(XTPToUIntChecked(arSearchKey.GetSize()), (LPENTRYID)arSearchKey.GetData(), TRUE);
	//-----------------------------------------------------------------------
...
void CXTPCalendarMAPIDataProvider::GetMAPILogonParams(LPTSTR& rpszProfile, LPTSTR& rpszPassword, 
	FLAGS& ruFlags)
{
	rpszProfile = NULL;
	rpszPassword = NULL;
	...
	{
		DWORD dwFlags = 0;
		SCANF_S(strFlags, _T("%lx"), &dwFlags);
...
LPSRestriction AFX_CDECL CXTPCalendarMAPIDataProvider::BuildBinaryRestriction(
	ULONG cbSize, LPBYTE lpData, ULONG ulPropTag)
{
	static SRestriction sres;
...
void CXTPCalendarMAPIDataProvider::_SetEventRuntimeProps(CXTPCalendarCustomProperties* pEventProps, 
	LPMESSAGE pMessage)
{
...
CXTPCalendarEventPtr CXTPCalendarMAPIDataProvider::ImportEvent(LPMESSAGE pMessage)
{
	...
	DWORD dwEventID = GetEventID(XTPToUIntChecked(arSearchKey.GetSize()), (LPENTRYID)arSearchKey.GetData(), TRUE);
...
void CXTPCalendarMAPIDataProvider::_SetMAPIEventGlobalPropsIfNeed(
	LPMESSAGE pMessage, CXTPCalendarEvent* pEvent)
{
	if (!pMessage || !pEvent)
	{
		ASSERT(FALSE);
...
BOOL CXTPCalendarMAPIDataProvider::UpdateMAPIEvent(
	LPMESSAGE pMessage, CXTPCalendarEvent* pEvent)
{
	if (!pMessage || !pEvent)
	{
	...
	VERIFY(_setPropTagVal_int(pMessage, nPropTag, nTmp));
	VERIFY(_setPropTagVal_int(pMessage, PR_SENSITIVITY, XTPToInt(nTmp ? SENSITIVITY_PRIVATE : SENSITIVITY_NONE)));
...
UINT AFX_CDECL CXTPCalendarMAPIDataProvider::_getStreamSize(IStream* pStream)
{
	ASSERT(pStream);
	if (!pStream)
	{
...
int AFX_CDECL CXTPCalendarMAPIDataProvider::_getSimpleMAPITypeSize(int nType)
{
	switch (nType)
	{
...
BOOL CXTPCalendarMAPIDataProvider::_setPropTagVal_int(
	LPMESSAGE pMessage, ULONG ulPropTag, int nValue)
{
	int nType = XTPToInt(PROP_TYPE(ulPropTag));
	...
	ULONG ulPropTag, LPCTSTR pcszValue)
{
	...
	{
		SIZE_T nStrBuffSize = _tcslen(pcszValue) * sizeof(TCHAR) + 2;
		SIZE_T nBufSize = sizeof(SPropValue) + nStrBuffSize + 4;

		SCODE sc = this->MAPIAllocateBuffer(XTPToULongChecked(nBufSize), (void**)&pPropVal);
		if (sc != S_OK || !pPropVal)
		{
			return FALSE;
		}
...
BOOL CXTPCalendarMAPIDataProvider::_setPropTagVal_UtcTime(
	LPMESSAGE pMessage, ULONG ulPropTag, const SYSTEMTIME& stTime)
{
	int nType = XTPToInt(PROP_TYPE(ulPropTag));
	...
	if (nType == PT_SYSTIME) // FILETIME
	{
		...
		ASSERT(sizeof(PropVal.Value.ft) == sizeof(ft));
		memcpy(&PropVal.Value.ft, &ft, sizeof(ft));
	}
	else
	{
		ASSERT(FALSE);
	...
	ULONG ulPropTag, CByteArray& rData)
{
	...
	{
		int nType = XTPToInt(PROP_TYPE(pProp->ulPropTag));
		SIZE_T nTypeSize = XTPToSizeTChecked(_getSimpleMAPITypeSize(nType));
		...
		else
		{
			if (nType == PT_STRING8) /* Null terminated 8-bit character string */
			{
				...
				nTypeSize = strlen(pProp->Value.lpszA) + 1;
			}
			else if (nType == PT_UNICODE) /* Null terminated Unicode string */
			{
				pData_src = (void*)pProp->Value.lpszW;
				nTypeSize = wcslen(pProp->Value.lpszW) * 2 + 2;
			}
		...
		rData.SetSize(XTPToIntPtrChecked(nTypeSize));
...
SYSTEMTIME CXTPCalendarMAPIDataProvider::_getPropVal_UtcTime(
	LPMESSAGE pMessage, ULONG ulPropTag)
{
...
HRESULT CXTPCalendarMAPIDataProvider::_deletePropTag(
	LPMESSAGE pMessage, ULONG ulPropTag)
{
	CUIntArray arPropsTags;
...
HRESULT CXTPCalendarMAPIDataProvider::_deletePropsTags(
	LPMESSAGE pMessage, CUIntArray& arPropsTags)
{
	...
	SIZE_T nTagsCount = XTPToSizeTChecked(arPropsTags.GetSize());
	SPropTagArray* pPorpTagsArray = NULL;

	SIZE_T nBufSize = sizeof(SPropTagArray) + nTagsCount * sizeof(ULONG)+4;
	SCODE sc = this->MAPIAllocateBuffer(XTPToULongChecked(nBufSize), (void**)&pPorpTagsArray);
	...
	//---------------------------------------------
	pPorpTagsArray->cValues = XTPToULongChecked(nTagsCount);
	for (INT_PTR i = 0; i < XTPToIntPtrChecked(nTagsCount); i++)
	{
...
CXTPCalendarEventsPtr CXTPCalendarMAPIDataProvider::ImportAllEvents(
...
CXTPMAPIBinary CXTPCalendarMAPIDataProvider::GetSearchKey(ULONG cbEntryID, 
	LPENTRYID lpEntryID)
{
	...
	if (SUCCEEDED(hrID))
	{
		keySearch.Set(XTPToULongChecked(arSearchKey.GetSize()), arSearchKey.GetData());
	}
...
{
	m_cb = eid.m_cb;
	m_arBytes.SetSize(XTPToIntPtrChecked(m_cb));
...
BOOL AFX_CDECL CXTPMAPIBinary::IsBinaryEqual(
	const CXTPMAPIBinary& eid1, const CXTPMAPIBinary& eid2)
{
	...
	return 0 == memcmp(
		...
		XTPToSizeTChecked(eid1.m_arBytes.GetSize()));
}
...
ULONG CXTPCalendarMAPIDataProvider::CMAPIPropIDMap::_GetPropTagFromID_ex(
	LPMESSAGE pMessage,
	const XTP_MAPI_PROP_NAME& propNameEx)
{
	ASSERT(pMessage);
	...
	if (SUCCEEDED(hRes) && pPropName)
	{
		...
		pPropName->Kind.lID = XTPToLongChecked(propNameEx.m_ulID);
...
ULONG CXTPCalendarMAPIDataProvider::CMAPIPropIDMap::GetPropIDByTag(
	LPMESSAGE pMessage, ULONG ulPropTag)
{
	ULONG ulID = 0;
