...
void CXTPCalendarResource::OleSetDataProvider(LPDISPATCH pDispDataProvider, BOOL bCloseDataProviderWhenDestroy)
{
	...
	CXTPCalendarData* pData = DYNAMIC_DOWNCAST(CXTPCalendarData, 
		CXTPCalendarData::FromIDispatchSafe(pDispDataProvider));
	ASSERT(pData);
	if (!pData)
	{
		AfxThrowOleException(E_INVALIDARG);
...
{
	CXTPCalendarResource* pResource = DYNAMIC_DOWNCAST(CXTPCalendarResource, 
		CXTPCalendarResource::FromIDispatchSafe(pResourceDisp));
	if (!pResource)
	{
		AfxThrowOleException(E_INVALIDARG);
	}
