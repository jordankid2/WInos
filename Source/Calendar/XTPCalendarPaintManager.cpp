...
CXTPCalendarPaintManager::CXTPCalendarPaintManager()
{
	...
	m_pDayViewTimeScaleHeaderPart = (CDayViewTimeScaleHeaderPart*)AddViewPart(new CDayViewTimeScaleHeaderPart(m_pControlPart));
	m_pDayViewTimeScaleCellPart = (CDayViewTimeScaleCellPart*)AddViewPart(new CDayViewTimeScaleCellPart(m_pDayViewTimeScaleHeaderPart));
	m_pDayViewCellPart = (CDayViewCellPart*)AddViewPart(new CDayViewCellPart(m_pControlPart));
	m_pDayViewWorkCellPart = (CDayViewWorkCellPart*)AddViewPart(new CDayViewWorkCellPart(m_pDayViewCellPart));
	m_pDayViewNonworkCellPart = (CDayViewNonworkCellPart*)AddViewPart(new CDayViewNonworkCellPart(m_pDayViewCellPart));
	m_pDayViewHeaderPart = (CDayViewHeaderPart*)AddViewPart(new CDayViewHeaderPart(m_pControlPart));
	m_pDayViewGroupHeaderPart = (CDayViewGroupHeaderPart*)AddViewPart(new CDayViewGroupHeaderPart(m_pControlPart));
	m_pDayViewAllDayEventsPart = (CDayViewAllDayEventsPart*)AddViewPart(new CDayViewAllDayEventsPart(m_pControlPart));
	m_pDayViewAllDayEventScrollIconsPart = (CDayViewAllDayEventScrollIconsPart*)AddViewPart(new CDayViewAllDayEventScrollIconsPart(m_pControlPart));
	...
	m_pMonthViewHeaderPart = (CMonthViewHeaderPart*)AddViewPart(new CMonthViewHeaderPart(m_pControlPart));
	m_pMonthViewEventPart = (CMonthViewEventPart*)AddViewPart(new CMonthViewEventPart(m_pControlPart));
	m_pTimeLineViewPart = (CTimeLineViewPart*)AddViewPart(new CTimeLineViewPart(m_pControlPart));
	m_pTimeLineViewTimeScalePart_Day = (CTimeLineViewTimeScalePart_Day*)AddViewPart(new CTimeLineViewTimeScalePart_Day(m_pControlPart));
	m_pTimeLineViewTimeScalePart_Week = (CTimeLineViewTimeScalePart_Week*)AddViewPart(new CTimeLineViewTimeScalePart_Week(m_pControlPart));
	m_pTimeLineViewTimeScalePart_Month = (CTimeLineViewTimeScalePart_Month*)AddViewPart(new CTimeLineViewTimeScalePart_Month(m_pControlPart));
}
...
void CXTPCalendarPaintManager::DrawHeader(CDC* pDC, CRect& rcHeader, BOOL bIsSelected, BOOL bIsCurrent)
{
	DrawHeader(pDC, rcHeader, bIsSelected, bIsCurrent, m_grclrToday, m_clrUnderLineHdr);
}

void CXTPCalendarPaintManager::DrawHeader(CDC* pDC, CRect& rcHeader, BOOL bIsSelected, BOOL bIsCurrent, const CXTPPaintManagerColorGradient& grclr, COLORREF clrUnderLineHdr)
{
	ASSERT(pDC);
	...
	if (pDC->GetSafeHdc())
	{
		...
		if (bIsCurrent)
		{
			...
			pDC->FillSolidRect(rcHeader.left, rcHeader.bottom - XTP_DPI_Y(1),
				rcHeader.Width(), XTP_DPI_X(1), clrUnderLineHdr);
		}
...
void CXTPCalendarPaintManager::UpdateGlyphs()
{
	...
	int nBmpID = XTPDpiHelper()->SelectDpiSpecific(
		XTP_IDB_CALENDAR_EXPANDSIGNUP_100,
		XTP_IDB_CALENDAR_EXPANDSIGNUP_125,
		XTP_IDB_CALENDAR_EXPANDSIGNUP_150,
		XTP_IDB_CALENDAR_EXPANDSIGNUP_200);
	CXTPResourceManager::AssertValid(XTPResourceManager()->LoadBitmap(&bmpSignUp, XTPToUInt(nBmpID)));

	nBmpID = XTPDpiHelper()->SelectDpiSpecific(
		XTP_IDB_CALENDAR_EXPANDSIGNDOWN_100,
		XTP_IDB_CALENDAR_EXPANDSIGNDOWN_125,
		XTP_IDB_CALENDAR_EXPANDSIGNDOWN_150,
		XTP_IDB_CALENDAR_EXPANDSIGNDOWN_200);
	CXTPResourceManager::AssertValid(XTPResourceManager()->LoadBitmap(&bmpSignDown, XTPToUInt(nBmpID)));

	nBmpID = XTPDpiHelper()->SelectDpiSpecific(
		XTP_IDB_CALENDAR_EVENT_GLYPHS_100,
	...
	BOOL bRes = m_ilGlyphs.Create(szGlyphs.cx / glypsCount, szGlyphs.cy, ILC_COLOR24 | ILC_MASK, 0, 1);
	nBmpID = XTPDpiHelper()->SelectDpiSpecific(
		XTP_IDB_CALENDAR_EVENT_GLYPHS_100,
		XTP_IDB_CALENDAR_EVENT_GLYPHS_125,
		XTP_IDB_CALENDAR_EVENT_GLYPHS_150,
		XTP_IDB_CALENDAR_EVENT_GLYPHS_200);
	CXTPResourceManager::AssertValid(XTPResourceManager()->LoadBitmap(&bmpGlyphs, XTPToUInt(nBmpID)));
...
void CXTPCalendarPaintManager::CreateGlyphsFont()
{
	...
	if (!m_pGlyphsFont)
	{
		TRACE(_T("WARNING! Calendar Paint Manager: could not create 'MS Outlook' font, standard bitmaps will be used.\n"));
	}
}
...
void CXTPCalendarPaintManager::RefreshMetrics()
{
    ...
    m_clrTLVEventBar			.SetStandardValue(::GetSysColor(COLOR_ACTIVECAPTION));
    m_clrTLVEventBarLine		.SetStandardValue(::GetSysColor(COLOR_INACTIVECAPTION));
    m_clrTLVTimeScaleBackground .SetStandardValue(::GetSysColor(COLOR_BTNFACE));
    m_clrTLVTimeScaleBorder		.SetStandardValue(::GetSysColor(COLOR_3DDKSHADOW));
    m_clrTLVTimeScaleHighlight	.SetStandardValue(::GetSysColor(COLOR_HIGHLIGHT));
    m_clrTLVTimeScaleText		.SetStandardValue(::GetSysColor(COLOR_WINDOWTEXT));
    m_clrTLVText				.SetStandardValue(::GetSysColor(COLOR_WINDOWTEXT));
    m_clrTLVBack				.SetStandardValue(::GetSysColor(COLOR_WINDOW));
    m_clrTLVSelectedText		.SetStandardValue(::GetSysColor(COLOR_HIGHLIGHTTEXT));
    m_clrTLVSelectedBack		.SetStandardValue(::GetSysColor(COLOR_3DSHADOW));
...
const CSize CXTPCalendarPaintManager::GetExpandSignSize()
{
	...
	{
		int idResource = XTPDpiHelper()->SelectDpiSpecific(
			XTP_IDB_CALENDAR_EXPANDSIGNDOWN_100,
			XTP_IDB_CALENDAR_EXPANDSIGNDOWN_125, 
			XTP_IDB_CALENDAR_EXPANDSIGNDOWN_150,
			XTP_IDB_CALENDAR_EXPANDSIGNDOWN_200);
	...
	static const CPoint arrHalfHours[] =
		{
			...
			CPoint(XTP_DPI_X(3), XTP_DPI_Y(-3)),
			CPoint(XTP_DPI_X(4), XTP_DPI_Y(-1)),
			CPoint(XTP_DPI_X(5), XTP_DPI_Y(1)),
			CPoint(XTP_DPI_X(3), XTP_DPI_Y(3)),
			CPoint(XTP_DPI_X(1), XTP_DPI_Y(5)),
			CPoint(XTP_DPI_X(-1), XTP_DPI_Y(5)),
			CPoint(XTP_DPI_X(-3), XTP_DPI_Y(3)),
...
int CXTPCalendarPaintManager::DrawIcons(CDC* pDC, CXTPCalendarViewEvent* pViewEvent)
{
	...
	BOOL bUseFont = XTP_SAFE_GET3(pViewEvent, GetCalendarControl(), GetCalendarOptions(), bUseOutlookFontGlyphs, FALSE);
...
void CXTPCalendarPaintManager::InitBusyStatusDefaultColors()
{
	...
	CXTPResourceManager::AssertValid(XTPResourceManager()->LoadBitmap(&m_bmpElsewherePattern, XTP_IDB_CALENDAR_ELSEWHERE_PATTERN));
	...
	CXTPResourceManager::AssertValid(XTPResourceManager()->LoadBitmap(&m_bmpTentativePattern, XTP_IDB_CALENDAR_TENTATIVE_PATTERN));
	...
	CXTPCalendarViewPartBrushValue* pBVal_Elsewhere = _GetAtGrow_BusyStatusBrushValue(xtpCalendarBusyStatusElsewhere, TRUE);
...
CXTPCalendarViewEventSubjectEditor* CXTPCalendarPaintManager::StartEditSubject(CXTPCalendarViewEvent* pViewEvent, CRect& rcEditor)
{
	UNREFERENCED_PARAMETER(pViewEvent);
	UNREFERENCED_PARAMETER(rcEditor);

	CXTPCalendarViewEventSubjectEditor* pEditor = new CXTPCalendarViewEventSubjectEditor(m_pControl);
	return pEditor;
}

void CXTPCalendarPaintManager::SetEditFont(CXTPCalendarViewEvent* pViewEvent, CXTPCalendarViewEventSubjectEditor* pEditor)
{
	if (!pViewEvent)
	{
		ASSERT(FALSE);
...
void CXTPCalendarPaintManager::PartRefresh(CMonthViewEventPart* pViewPart)
{
	...
	pViewPart->m_grclrClassicSelDay.SetStandardValue(pViewPart->m_clrBackground, RGB(217, 214, 202));
...
void CXTPCalendarPaintManager::PartRefresh(CTimeLineViewPart* pViewPart)
{
	...
	LPCTSTR pszBmpName = XTPDpiHelper()->SelectDpiSpecific(
		_T("XTP_IDB_CALENDAR_TLV_EVENT_100"),
...
void CXTPCalendarPaintManager::PartDraw(CDayViewEventPart* pViewPart, CDC* pDC, CXTPCalendarDayViewEvent* pViewEvent)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pViewEvent);
	...
	if (pDC->GetSafeHdc() && pViewPart && pViewEvent && pViewEvent->GetEvent())
	{
		...
		pDC->FillSolidRect(rcBorder.left, rcBorder.top, rcBorder.Width(), XTP_DPI_Y(1), m_clrWindowText);
		pDC->FillSolidRect(rcBorder.left - XTP_DPI_X(1), rcBorder.bottom - XTP_DPI_Y(bMultiDay ? 1 : 0),
			rcBorder.Width() + XTP_DPI_X(1), XTP_DPI_Y(1), m_clrWindowText);

		if (!bMultiDay)
		{
			pDC->FillSolidRect(rcBorder.right, rcBorder.top, XTP_DPI_X(1), rcBorder.Height() + XTP_DPI_Y(1), m_clrWindowText);
		}

		if (bSelected && bMultiDay)
		{
			pDC->FillSolidRect(rcEvent.left, rcEvent.top + XTP_DPI_Y(1), rcEvent.Width(), rcEvent.Height() - XTP_DPI_Y(2), m_clrButtonFace);
			pDC->FillSolidRect(rcEvent.left, rcEvent.top - XTP_DPI_Y(1), rcEvent.Width() + XTP_DPI_X(1), XTP_DPI_Y(1), m_clrWindowText);
			pDC->FillSolidRect(rcEvent.left, rcEvent.bottom, rcEvent.Width() + XTP_DPI_X(1), XTP_DPI_Y(1), m_clrWindowText);
		}
		...
		if (!bMarkupDrawn)
		{
			...
			if (nMDEFlags & xtpCalendarMultiDayFirst)
			{
				...
				pDC->FillSolidRect(rcEvent.left, rcEvent.top, XTP_DPI_X(1), rcEvent.Height(), m_clrWindowText);

				rcEvent.left++;
				if (rcClockRect.Width() > 0)
				{
					DrawClock(pDC, pEvent->GetStartTime(), rcClockRect, crBackground, xtpCalendarClockAlignLeft);
				}
			}
			if (nMDEFlags & xtpCalendarMultiDayLast)
			{
				pDC->FillSolidRect(rcEvent.right, rcEvent.top, XTP_DPI_X(1), rcEvent.Height(), m_clrWindowText);
			...
			if (!bMultiDay)
			{
					...
					pViewEvent->GetTimeframeRect().bottom == LONG_MAX)
				{
					POINT arBusyPoints[] =
					{
						...
						{ rcEvent.left + XTP_DPI_X(7), rcEvent.top + XTP_DPI_Y(7) },
						{ rcEvent.left, rcEvent.top }
					};
				...
				uAlignWordBreak = XTPToUInt(dwAdditionalOpt & xtpCalendarOptDayViewNoWordBreak ? DT_WORD_ELLIPSIS : DT_WORDBREAK);
			...
			bOutOfBorders = nY_need > rcText.Height() || nX_need > rcText.Width() || bTimeOutOfBorders;
		}
...
void CXTPCalendarPaintManager::PartDrawHourCell(CDayViewTimeScaleCellPart* pViewPart, CDC* pDC, CRect rc, CString strText, BOOL bFillBackground, BOOL bCurrentTime)
{
	...
	if (pDC->GetSafeHdc() && pViewPart)
	{
		...
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top, XTP_DPI_X(1), rc.Height(), pViewPart->m_clrBorder);
		...
		pDC->FillSolidRect(rc.left, rc.bottom - XTP_DPI_Y(1), rc.Width(), XTP_DPI_X(1), pViewPart->m_clrBorder);
...
void CXTPCalendarPaintManager::PartDrawBigHourCell(CDayViewTimeScaleCellPart* pViewPart, CDC* pDC, CRect rc, CString strHour, CString strMin, int nRowPerHour, BOOL bFillBackground, BOOL bCurrentTime)
{
	UNREFERENCED_PARAMETER(bCurrentTime);
	...
	if (pDC->GetSafeHdc() && pViewPart)
	{
		...
		{
			// draw minutes text
			pViewPart->DrawText(pDC, strMin, rcMinutesText, DT_VCENTER | DT_LEFT | DT_SINGLELINE | DT_NOCLIP);
		}

		// draw small lines
		if (nRowPerHour > 2)
		{
			BOOL bShowMinutes = XTP_SAFE_GET3(this, m_pControl, GetCalendarOptions(), bDayView_TimeScaleShowMinutes, FALSE);
			CString strMinEx;

			for (int i = 1; i < nRowPerHour; i++)
			{
				int y = rc.top + i * rc.Height() / nRowPerHour;
				pDC->FillSolidRect(rcMinutesText.left, y - XTP_DPI_Y(1), rcMinutesText.Width(), XTP_DPI_X(1), pViewPart->m_clrBorder);
				...
				if (bShowMinutes)
				{
					...
					pViewPart->DrawText(pDC, strMinEx, rcMinEx, DT_TOP | DT_LEFT | DT_SINGLELINE | DT_NOCLIP);
				}
			}
		}
		...
		pDC->FillSolidRect(rcLine.left, rcLine.bottom - XTP_DPI_Y(1), rcLine.Width(), XTP_DPI_Y(1), pViewPart->m_clrBorder);
	}
}

void CXTPCalendarPaintManager::PartDraw(CDayViewTimeScaleHeaderPart* pViewPart, CDC* pDC, CXTPCalendarDayView* /*pView*/, CRect rc, CString strText)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
...
void CXTPCalendarPaintManager::PartDrawNowLine(CDayViewTimeScaleHeaderPart* pViewPart, CDC* pDC, CXTPCalendarDayView* pView, CRect rc, int y, BOOL bDrawBk)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pView);
...
void CXTPCalendarPaintManager::PartDraw(CDayViewCellPart* pViewPart, CDC* pDC, CXTPCalendarDayViewGroup* pViewGroup, CRect rc, const XTP_CALENDAR_DAYVIEWCELL_PARAMS& cellParams)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pViewGroup);
	...
	if (pDC->GetSafeHdc() && pViewPart && pViewGroup)
	{
		...
		pDC->FillSolidRect(rc.left + (iLeftMargin - XTP_DPI_X(1)) + nDayDeviderX, rc.top, XTP_DPI_X(1), rc.Height(), clrBorder);
		...
		if (dtTime.GetMinute() == 0 && dtTime.GetHour() != 0)
			pDC->FillSolidRect(rc.left, rc.top - XTP_DPI_Y(1), rc.Width(), XTP_DPI_Y(1), pViewPart->m_clrHour);

		pDC->FillSolidRect(rc.left, rc.bottom - XTP_DPI_Y(1), rc.Width(), XTP_DPI_Y(1), pViewPart->m_clrShadow);
	}
}

void CXTPCalendarPaintManager::PartDraw(CDayViewAllDayEventsPart* pViewPart, CDC* pDC, CXTPCalendarDayViewGroup* pViewGroup, CRect rc, BOOL bSelected)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pViewGroup);
	...
	if (pDC->GetSafeHdc() && pViewPart && pViewGroup && pViewGroup->GetViewDay())
	{
		...
		if (bSelected)
		{
			...
			pDC->FillSolidRect(rc.left, rc.bottom - XTP_DPI_Y(1), rc.Width(), XTP_DPI_Y(1), m_clr3DShadow);
		}
		else
		{
			pDC->FillSolidRect(rc, pViewPart->GetBackgroundColor());
		...
		if (pViewGroup->GetViewDay()->GetDayIndex() || pViewGroup->GetGroupIndex())
			pDC->FillSolidRect(rc.left, rc.top + XTP_DPI_Y(1), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(2), clrBorder);
	}
}

void CXTPCalendarPaintManager::PartDraw(CDayViewHeaderPart* pViewPart, CDC* pDC, CXTPCalendarDayViewDay* pViewDay, CRect rc, CString strText)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pViewDay);
	...
	{
		BOOL bIsToday = XTP_SAFE_GET1(pViewPart->GetCalendarControl(), IsToday(pViewDay->GetDayDate()), FALSE);
		...
		pDC->FillSolidRect(rc.left, rc.top, rc.Width(), XTP_DPI_X(1), pViewPart->m_clrTopLeftBorder);
		pDC->FillSolidRect(rc.left, rc.top + XTP_DPI_Y(bFirstDay ? 0: 2), XTP_DPI_X(1), rc.Height() -  XTP_DPI_Y(bFirstDay ? 0 : 3), pViewPart->m_clrTopLeftBorder);
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(bLastDay ? 0: 2), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(bLastDay ? 0 : 3), m_clr3DShadow);
...
void CXTPCalendarPaintManager::PartDraw(CDayViewAllDayEventScrollIconsPart* pViewPart, CDC* pDC, CXTPCalendarDayViewGroup* pViewGroup, BOOL bDrawUp, BOOL bDrawDown, const CRect& rcIconUp, const CRect& rcIconDown, BOOL bHighlightUp, BOOL bHighlightDown)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
...
void CXTPCalendarPaintManager::PartDraw(CDayViewGroupHeaderPart* pViewPart, CDC* pDC, CXTPCalendarDayViewGroup* pViewGroup, CRect rc, CString strText)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pViewGroup);
	...
	if (pDC->GetSafeHdc() && pViewPart && pViewGroup && pViewGroup->GetViewDay())
	{
		...
		pDC->FillSolidRect(rc.left, rc.top, rc.Width(), XTP_DPI_Y(1), pViewPart->m_clrTopLeftBorder);
		pDC->FillSolidRect(rc.left, rc.top + XTP_DPI_Y(bFirstGroup ? 0: 2), XTP_DPI_X(1), rc.Height() -  XTP_DPI_Y(bFirstGroup ? 0 : 3), pViewPart->m_clrTopLeftBorder);
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(bLastGroup ? 0: 2), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(bLastGroup ? 0 : 3), m_clr3DShadow);
		...
		if (pViewDay->GetCalendarControl()->m_bMultiColorScheduleMode)
		{
			...
			if (ind == 1)
				...
				pViewPart->SetTextColor(RGB((GetRValue(clr) + 64 * ind) % 255, (GetGValue(clr) + 64 * ind) % 255, (GetBValue(clr) + 64 * ind) % 255));
		}
...
void CXTPCalendarPaintManager::PartDrawDayHeader(CWeekViewPart* pViewPart, CDC* pDC, CRect rcDay, int nHeaderHeight, CString strHeader, BOOL bIsCurrent, BOOL bIsSelected)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	...
	if (pDC->GetSafeHdc() && pViewPart)
	{
		...
		rcHeader.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(1), rcHeader.Height() - nHeaderHeight - XTP_DPI_Y(1));
		...
		pViewPart->SetTextColor(bIsSelected ? pViewPart->m_clrTextHighLightColor : pViewPart->m_clrTextNormalColor);
		pViewPart->DrawText(pDC, strHeader, rcText, DT_VCENTER | DT_RIGHT | DT_SINGLELINE);
	}
}

void CXTPCalendarPaintManager::PartDraw(CWeekViewPart* pViewPart, CDC* pDC, CXTPCalendarWeekView* pWeekView)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pWeekView);
	...
	if (pDC->GetSafeHdc() && pViewPart && pWeekView)
	{
		...
		for (int nDay = 0; nDay < 7; nDay++)
		{
			...
			pViewPart->DrawDayHeader(pDC, pWDay->GetDayRect(), nDayHeaderHeight, strDate, bIsCurrent, bIsSelected);
		}
	}
}

void CXTPCalendarPaintManager::PartDrawBorder(CWeekViewEventPart* pViewPart, CDC* pDC, CRect rcView, CXTPCalendarWeekViewEvent* pWeekViewEvent)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	...
	if (pDC->GetSafeHdc() && pViewPart && pWeekViewEvent && pWeekViewEvent->GetEvent())
	{
		...
		if (bAllDay ||
				...
				nMDEFlags & xtpCalendarMultiDayMiddle)
		{
			pDC->FillSolidRect(rcView.left, rcView.top , rcView.Width(), nBorderWidth, pViewPart->m_clrTextNormalColor);
			pDC->FillSolidRect(rcView.left, rcView.bottom - nBorderWidth, rcView.Width(), nBorderWidth, pViewPart->m_clrTextNormalColor);
		}
		if (nMDEFlags & xtpCalendarMultiDayFirst)
		{
			pDC->FillSolidRect(rcView.left, rcView.top, XTP_DPI_X(1), rcView.Height(), pViewPart->m_clrTextNormalColor);
		}
		if (nMDEFlags & xtpCalendarMultiDayLast)
		{
			pDC->FillSolidRect(rcView.right, rcView.top, XTP_DPI_X(1), rcView.Height(), pViewPart->m_clrTextNormalColor);
		}
	}
}

int CXTPCalendarPaintManager::PartDrawTimes(CWeekViewEventPart* pViewPart, CDC* pDC, CXTPCalendarWeekViewEvent* pWeekViewEvent)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pWeekViewEvent);
	...
	if (pDC->GetSafeHdc() && pViewPart && pWeekViewEvent && pWeekViewEvent->GetEvent())
	{
		...
		COLORREF clrBackground = pWeekViewEvent->IsSelected() ? m_clrHighlight : pViewPart->m_clrBackground;
		...
		else
		{
			...
			if (bShowStartTime)
				pViewPart->DrawLineEx(pDC, strStartTime, &rcStart, DT_RIGHT | DT_VCENTER, DT_LEFT | DT_VCENTER);
		}
		...
		if (pWeekViewEvent->IsShowEndTime())
		{
			...
			else
			{
				...
				if (bShowEndTime)
					pViewPart->DrawLineEx(pDC, strEndTime, &rcEnd, DT_RIGHT | DT_VCENTER, DT_LEFT | DT_VCENTER);
			}
		}
	}
...
void CXTPCalendarPaintManager::PartDrawSubj(CWeekViewEventPart* pViewPart, CDC* pDC, CXTPCalendarWeekViewEvent* pWeekViewEvent)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pWeekViewEvent);
	...
	if (pDC->GetSafeHdc() && pViewPart && pWeekViewEvent && pWeekViewEvent->GetEvent())
	{
		...
		if (pWeekViewEvent->IsMultyDayTextAlign() || pViewPart->GetTextExtent(pDC, strEventText).cx >= rcText.Width())
			nAlign = DT_LEFT;
...
void CXTPCalendarPaintManager::PartDraw(CWeekViewEventPart* pViewPart, CDC* pDC, CXTPCalendarWeekViewEvent* pWeekViewEvent)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pWeekViewEvent);
...
void CXTPCalendarPaintManager::PartDrawGrid(CMonthViewGridPart* pViewPart, CDC* pDC, CXTPCalendarMonthView* pMonthView)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pMonthView);

	if (pDC->GetSafeHdc() && pViewPart && pMonthView && pMonthView->GetGrid() && pMonthView->GetCalendarControl())
	{
		...
		for (int nWeek = 0; nWeek < nWeeksCount; nWeek++)
		{
			for (int nDayIdx = 0; nDayIdx < 7; nDayIdx++)
			{
				...
				{
					pDC->FillSolidRect(rcB.left, rcB.bottom, rcB.Width(), XTP_DPI_X(1), clrGridLine);
				}
				...
				COLORREF clrBackground = nMonth % 2 == nWhiteMonthColorIdx ? pViewPart->m_clrBackground : pViewPart->m_clrBackground2;
				if (nDayIdx == 0)
				{
					rcDay.left--;
				}
...
void CXTPCalendarPaintManager::PartDraw(CMonthViewHeaderPart* pViewPart, CDC* pDC, CXTPCalendarMonthView* pMonthView, CRect rc, int nCollIndex, CString strText)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pMonthView);
	...
	if (pDC->GetSafeHdc() && pViewPart && pMonthView && pMonthView->GetGrid())
	{
		...
		pDC->FillSolidRect(rc.left, rc.top + XTP_DPI_Y(bFirstDay ? 0: 2), XTP_DPI_X(1), rc.Height() -  XTP_DPI_Y(bFirstDay ? 1 : 3), pViewPart->m_clrTopLeftBorder);
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(bLastDay ? 0: 2), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(bLastDay ? 1 : 3), m_clr3DShadow);
...
void CXTPCalendarPaintManager::PartDrawDayDate(CMonthViewEventPart* pViewPart, CDC* pDC, CRect rc, BOOL bToday, BOOL bSelected, BOOL bFirstMonthDay, CString strText)
{
	...
	if (pDC->GetSafeHdc() && pViewPart)
	{
		...
		if (bSelected)
		{
			DrawHeader(pDC, rc, bSelected, FALSE,
				pViewPart->m_grclrClassicSelDay, pViewPart->m_clrClassicUnderline);
		}
...
void CXTPCalendarPaintManager::PartDrawEvent(CMonthViewEventPart* pViewPart, CDC* pDC, CXTPCalendarMonthViewEvent* pViewEvent)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pViewEvent);
	...
	if (pDC->GetSafeHdc() && pViewPart && pViewEvent && pViewEvent->GetEvent())
	{
		...
		BOOL bHideTimes = XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), bMonthView_HideTimes, FALSE);
		...
		if (nMDEFlags & xtpCalendarMultiDayFMLmask)
		{
			...
			pDC->FillSolidRect(rcEvent.left, rcEvent.top, rcEvent.Width(), nBY, pViewPart->m_clrMultiDayEventFrameColor);
			pDC->FillSolidRect(rcEvent.left, rcEvent.bottom - nBY, rcEvent.Width(), nBY, pViewPart->m_clrMultiDayEventFrameColor);

			if (nMDEFlags & xtpCalendarMultiDayFirst)
			{
				pDC->FillSolidRect(rcEvent.left, rcEvent.top, XTP_DPI_X(1), rcEvent.Height(), pViewPart->m_clrMultiDayEventFrameColor);
			}
			if (nMDEFlags & xtpCalendarMultiDayLast)
			{
				pDC->FillSolidRect(rcEvent.right, rcEvent.top, XTP_DPI_X(1), rcEvent.Height(), pViewPart->m_clrMultiDayEventFrameColor);
			}
		...
		pViewPart->DrawText(pDC, strText, rcText, uAlign | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
		pDC->SetTextColor(clrOldTextColor);
...
void CXTPCalendarPaintManager::PartDraw(CTimeLineViewTimeScalePart_Day* pViewPart, CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineView* pView)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pView);
	...
	if (pDC->GetSafeHdc() && pViewPart && pView && pView->GetTimeScale())
	{
		...
		for (int nP = 0; nP < 100; nP++, dtDay += spDay)
		{
			...
			CPoint ptTR(rcDayLabel.left + szLabel.cx + XTP_DPI_X(4), rcDayLabel.bottom - XTP_DPI_Y(9));
			...
			POINT arTRPts[] ={ {ptTR.x, ptTR.y}, {ptTR.x + XTP_DPI_X(9), ptTR.y}, {ptTR.x + XTP_DPI_X(4), ptTR.y + XTP_DPI_Y(5) } };
			CRgn rgnTR;
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (bBold)
			{
				...
				BOOL bSelected = 
					dtStroke.GetYear()  == dtNow.GetYear()  &&
				...
				if (bSelected)
				{
					...
					pViewPart->DrawText(pDC, strHour, &rcLabel, DT_CENTER | DT_SINGLELINE | DT_VCENTER, pView->m_clrSelectedText, m_pSelFont);
				}
				else
				{
					pViewPart->DrawText(pDC, strHour, &rcLabel, DT_CENTER | DT_SINGLELINE | DT_VCENTER, pView->m_clrTimeScaleText);
				}
			}
		}
	}
...
void CXTPCalendarPaintManager::PartDraw(CTimeLineViewTimeScalePart_Week* pViewPart, CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineView* pView)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pView);
	...
	if (pDC->GetSafeHdc() && pViewPart && pView && pView->GetTimeScale())
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			CString strWD = CXTPCalendarUtils::GetLocaleString(XTPToUIntChecked(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(nWD)));
			...
			if (bSelected)
			{
				...
				pViewPart->DrawText(pDC, strLabel, &rcLabel, DT_CENTER|DT_SINGLELINE|DT_VCENTER, pView->m_clrSelectedText, m_pSelFont);
			}
			else
			{
				pViewPart->DrawText(pDC, strLabel, &rcLabel, DT_CENTER|DT_SINGLELINE|DT_VCENTER, pView->m_clrTimeScaleText);
			}
		}
	}
}
...
void CXTPCalendarPaintManager::PartDraw(CTimeLineViewTimeScalePart_Month* pViewPart, CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineView* pView)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pView);
	...
	if (pDC->GetSafeHdc() && pViewPart && pView && pView->GetTimeScale())
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			//CString strWD = CXTPCalendarUtils::GetLocaleString(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(nWD));
			TCHAR szWD1[8] = {0};
...
void CXTPCalendarPaintManager::PartDraw(CTimeLineViewTimeScalePart_WorkWeek* pViewPart, CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineView* pView)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pView);
	...
	if (pDC->GetSafeHdc() && pViewPart && pView && pView->GetTimeScale())
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			pViewPart->DrawText(pDC, strLabel, &rcLabel, DT_LEFT|DT_SINGLELINE|DT_VCENTER, pView->m_clrTimeScaleText);
		}
	}
}

void CXTPCalendarPaintManager::PartDrawGroup(CTimeLineViewPart* pViewPart, CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineViewGroup* pGroup)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pGroup);
...
void CXTPCalendarPaintManager::PartDrawEvent(CTimeLineViewPart* pViewPart, CDC* pDC, const CRect& rcEvents, CXTPCalendarTimeLineViewEvent* pViewEvent)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pViewEvent);
	...
	if (pDC->GetSafeHdc() && pViewPart && pViewEvent)
	{
		...
		CRect rcB1 = rcEvent;
		rcB1.top += XTP_DPI_Y(1);
		rcB1.bottom = rcB1.top + XTP_DPI_Y(7);
		rcB1.left += XTP_DPI_X(1);
		rcB1.right = rcB1.left + pViewEvent->GetEventDurationInPixels();

		pDC->FillSolidRect(rcB1, pViewEvent->GetView()->m_clrEventBar);

		rcB1.bottom = rcB1.top + XTP_DPI_Y(1);

		pDC->FillSolidRect(rcB1, pViewEvent->GetView()->m_clrEventBarLine);

		CSize szIcon(0, 0);
		if (pViewPart->m_pimgEvent)
		{
			szIcon = pViewPart->m_pimgEvent->GetExtent();

			CRect rcIcon = rcEvent;
			rcIcon.left += XTP_DPI_X(1);
		...
		rcText.left = rcEvent.left + szIcon.cx + XTP_DPI_X(5);
		rcText.right = rcEvent.right + szIcon.cx + XTP_DPI_X(5);
...
CFont& CXTPCalendarPaintManager::PartGetTextFont(CXTPCalendarViewPart* pViewPart, CXTPCalendarViewPartFontValue& fntText)
{
	while (pViewPart != NULL)
	{
...
void CXTPCalendarPaintManager::PartGetParams(CDayViewCellPart* pViewPart, CXTPCalendarDayViewGroup* pViewGroup, XTP_CALENDAR_DAYVIEWCELL_PARAMS* rCellParams)
{
	if (pViewGroup)
	{
		rCellParams->clrBackgroundColor =  rCellParams->bSelected ? m_clrHighlight : pViewPart->GetBackgroundColor();
	}
}

//-- CALENDAR PART ADJUST METHODS

void CXTPCalendarPaintManager::PartCalcWidth(CDayViewTimeScaleCellPart* pViewPart, CDC* pDC, const CString& strHour, const CString& strMin, int nHourHeight, int& nWidth)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
...
void CXTPCalendarPaintManager::PartAdjustTimeFont(CDayViewTimeScaleCellPart* pViewPart, CDC* pDC, CRect rcCell)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
...
CSize CXTPCalendarPaintManager::PartCalcEventSize(CTimeLineViewPart* pViewPart, CDC* pDC, CXTPCalendarTimeLineViewEvent* pViewEvent)
{
	ASSERT(pDC);
	ASSERT(pViewPart);
	ASSERT(pViewEvent);
	...
	if (pDC->GetSafeHdc() && pViewPart && pViewEvent)
	{
		...
		if (!pView) return CSize(0, 0);

		CXTPResourceImage* pImage = pViewPart->m_pimgEvent;
		if (!pImage) return CSize(0, 0);
...
void CXTPCalendarPaintManager::CControlPart::RefreshMetrics() {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartRefresh(this); }
}
...
void CXTPCalendarPaintManager::CDayViewEventPart::OnDraw(CDC* pDC, CXTPCalendarDayViewEvent* pViewEvent) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pViewEvent); }
}
...
void CXTPCalendarPaintManager::CDayViewTimeScaleCellPart::CalcWidth(CDC* pDC, const CString& strHour, const CString& strMin, int nHourHeight, int& nWidth) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartCalcWidth(this, pDC, strHour, strMin, nHourHeight, nWidth); }
}

void CXTPCalendarPaintManager::CDayViewTimeScaleCellPart::AdjustTimeFont(CDC* pDC, CRect rcCell) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartAdjustTimeFont(this, pDC, rcCell); }
}
...
void CXTPCalendarPaintManager::CDayViewTimeScaleCellPart::DrawHourCell(CDC* pDC, CRect rc, CString strText, BOOL bFillBackground, BOOL bCurrentTime /*= FALSE*/) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawHourCell(this, pDC, rc, strText, bFillBackground, bCurrentTime); }
}
...
void CXTPCalendarPaintManager::CDayViewTimeScaleCellPart::DrawBigHourCell(CDC* pDC, CRect rc, CString strHour, CString strMin, int nRowPerHour, BOOL bFillBackground, BOOL bCurrentTime /*= FALSE*/) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawBigHourCell(this, pDC, rc, strHour, strMin, nRowPerHour, bFillBackground, bCurrentTime); }
}
...
void CXTPCalendarPaintManager::CDayViewTimeScaleHeaderPart::OnDraw(CDC* pDC, CXTPCalendarDayView* pView, CRect rc, CString strText) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pView, rc, strText); }
}

void CXTPCalendarPaintManager::CDayViewTimeScaleHeaderPart::DrawNowLine(CDC* pDC, CXTPCalendarDayView* pView, CRect rc, int y, BOOL bDrawBk) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawNowLine(this, pDC, pView, rc, y, bDrawBk); }
}

void CXTPCalendarPaintManager::CDayViewTimeScaleHeaderPart::DrawExpandSigns(CDC* pDC, CXTPCalendarDayView* pView, const CRect& rcTSHours)
{
	ASSERT(pView);
...
void CXTPCalendarPaintManager::CDayViewCellPart::GetParams(CXTPCalendarDayViewGroup* pViewGroup, XTP_CALENDAR_DAYVIEWCELL_PARAMS* rCellParams) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartGetParams(this, pViewGroup, rCellParams); }
}
...
void CXTPCalendarPaintManager::CDayViewCellPart::OnDraw(CDC* pDC, CXTPCalendarDayViewGroup* pViewGroup, CRect rc, const XTP_CALENDAR_DAYVIEWCELL_PARAMS& cellParams) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pViewGroup, rc, cellParams); }
}
...
void CXTPCalendarPaintManager::CDayViewAllDayEventsPart::OnDraw(CDC* pDC, CXTPCalendarDayViewGroup* pViewGroup, CRect rc, BOOL bSelected) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pViewGroup, rc, bSelected); }
}
...
void CXTPCalendarPaintManager::CDayViewHeaderPart::OnDraw(CDC* pDC, CXTPCalendarDayViewDay* pViewDay, CRect rc, CString strText) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pViewDay, rc, strText); }
}

void CXTPCalendarPaintManager::CDayViewAllDayEventScrollIconsPart::OnDraw(CDC* pDC, CXTPCalendarDayViewGroup* pViewGroup, BOOL bDrawUp, BOOL bDrawDown, const CRect& rcIconUp, const CRect& rcIconDown, BOOL bHighlightUp, BOOL bHighlightDown) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pViewGroup, bDrawUp, bDrawDown, rcIconUp, rcIconDown, bHighlightUp, bHighlightDown); }
}

void CXTPCalendarPaintManager::CDayViewGroupHeaderPart::OnDraw(CDC* pDC, CXTPCalendarDayViewGroup* pViewGroup, CRect rc, CString strText) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pViewGroup, rc, strText); }
}
...
void CXTPCalendarPaintManager::CWeekViewPart::DrawDayHeader(CDC* pDC, CRect rcDay, int nHeaderHeight, CString strHeader, BOOL bIsCurrent, BOOL bIsSelected) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawDayHeader(this, pDC, rcDay, nHeaderHeight, strHeader, bIsCurrent, bIsSelected); }
}

void CXTPCalendarPaintManager::CWeekViewPart::OnDraw(CDC* pDC, CXTPCalendarWeekView* pWeekView) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pWeekView); }
}

void CXTPCalendarPaintManager::CWeekViewEventPart::DrawBorder(CDC* pDC, CRect rcView, CXTPCalendarWeekViewEvent* pWeekViewEvent) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawBorder(this, pDC, rcView, pWeekViewEvent); }
}

int CXTPCalendarPaintManager::CWeekViewEventPart::DrawTimes(CDC* pDC, CXTPCalendarWeekViewEvent* pWeekViewEvent) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { return m_pPaintManager->PartDrawTimes(this, pDC, pWeekViewEvent); } return 0;
}

void CXTPCalendarPaintManager::CWeekViewEventPart::DrawSubj(CDC* pDC, CXTPCalendarWeekViewEvent* pWeekViewEvent) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawSubj(this, pDC, pWeekViewEvent); }
}

void CXTPCalendarPaintManager::CWeekViewEventPart::OnDraw(CDC* pDC, CXTPCalendarWeekViewEvent* pWeekViewEvent) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pWeekViewEvent); }
}

void CXTPCalendarPaintManager::CMonthViewGridPart::OnDrawGrid(CDC* pDC, CXTPCalendarMonthView* pMonthView) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawGrid(this, pDC, pMonthView); }
}
...
void CXTPCalendarPaintManager::CMonthViewHeaderPart::OnDraw(CDC* pDC, CXTPCalendarMonthView* pMonthView, CRect rc, int nCollIndex, CString strText) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, pMonthView, rc, nCollIndex, strText); }
}
...
void CXTPCalendarPaintManager::CMonthViewEventPart::OnDrawDayDate(CDC* pDC, CRect rc, BOOL bToday, BOOL bSelected, BOOL bFirstMonthDay, CString strText) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawDayDate(this, pDC, rc, bToday, bSelected, bFirstMonthDay, strText); }
}

void CXTPCalendarPaintManager::CMonthViewEventPart::OnDrawEvent(CDC* pDC, CXTPCalendarMonthViewEvent* pViewEvent) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawEvent(this, pDC, pViewEvent); }
}
...
void CXTPCalendarPaintManager::CTimeLineViewTimeScalePart_Day::Draw(CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineView* pView) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, rcRect, pView); }
}

void CXTPCalendarPaintManager::CTimeLineViewTimeScalePart_Week::Draw(CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineView* pView) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, rcRect, pView); }
}

void CXTPCalendarPaintManager::CTimeLineViewTimeScalePart_Month::Draw(CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineView* pView) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, rcRect, pView); }
}

void CXTPCalendarPaintManager::CTimeLineViewTimeScalePart_WorkWeek::Draw(CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineView* pView) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDraw(this, pDC, rcRect, pView); }
}
...
void CXTPCalendarPaintManager::CTimeLineViewPart::DrawGroup(CDC* pDC, const CRect& rcRect, CXTPCalendarTimeLineViewGroup* pGroup) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawGroup(this, pDC, rcRect, pGroup); }
}

void CXTPCalendarPaintManager::CTimeLineViewPart::DrawEvent(CDC* pDC, const CRect& rcEvents, CXTPCalendarTimeLineViewEvent* pViewEvent) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { m_pPaintManager->PartDrawEvent(this, pDC, rcEvents, pViewEvent); }
}

CSize CXTPCalendarPaintManager::CTimeLineViewPart::CalcEventSize(CDC* pDC, CXTPCalendarTimeLineViewEvent* pViewEvent) {
	ASSERT(m_pPaintManager); if (m_pPaintManager) { return m_pPaintManager->PartCalcEventSize(this, pDC, pViewEvent); } return CSize(0);
}
