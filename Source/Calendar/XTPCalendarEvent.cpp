...
IMPLEMENT_DYNAMIC(CXTPCalendarEvent, CXTPCmdTarget)
...
BEGIN_DISPATCH_MAP(CXTPCalendarEvent, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPCalendarEvent, CXTPCmdTarget)
	INTERFACE_PART(CXTPCalendarEvent, XTPDIID_CalendarEvent, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCalendarEvent, XTPDIID_CalendarEvent)
...
{
	CXTPCalendarRecurrencePattern* pPattern = DYNAMIC_DOWNCAST(CXTPCalendarRecurrencePattern, 
		CXTPCalendarRecurrencePattern::FromIDispatchSafe(Pattern));
	if (!pPattern)
	{
...
{
	CXTPCalendarEvent* pSourceEvent = DYNAMIC_DOWNCAST(CXTPCalendarEvent, 
		CXTPCalendarEvent::FromIDispatchSafe(SourceEvent));
	if (!pSourceEvent)
	{
		AfxThrowOleException(E_INVALIDARG);
	}
