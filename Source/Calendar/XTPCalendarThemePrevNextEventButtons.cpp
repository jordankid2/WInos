...
{
	if (GetPrevEventButtonPartX())
		GetPrevEventButtonPartX()->Draw((CXTPCalendarView*)NULL, pDC);

	if (GetNextEventButtonPartX())
		GetNextEventButtonPartX()->Draw((CXTPCalendarView*)NULL, pDC);
}

void CTOPrevNextEventButtons::Draw(CCmdTarget* pObject, CDC* pDC)
{
	if (NULL != pObject)
	{
		CXTPCalendarView* pView = DYNAMIC_DOWNCAST(CXTPCalendarView, pObject);
		if (NULL != pView)
		{
			Draw(pView, pDC);
		}
...
void CTOPrevNextEventButtons::CDaysMap::AdjustMiddleDay(long nDay)
{
	...
	int nSize = XTPToIntChecked(GetSize());
	...
	{
		memset(GetData(), 0, XTPToSizeTChecked(nSize));
		m_nDaysOffset += nDay - nMiddleDay0;
	}
	else if (nDay < nMiddleDay0)
	{
		MEMMOVE_S(GetData() + nShift, GetData(), XTPToSizeTChecked(nCount2));
		memset(GetData(), 0, XTPToSizeTChecked(nShift));
		m_nDaysOffset -= nShift;
	}
	else
	{
		MEMMOVE_S(GetData(), GetData() + nShift, XTPToSizeTChecked(nCount2));
		memset(GetData() + nCount2, 0, XTPToSizeTChecked(nShift));
		m_nDaysOffset += nShift;
	}
}
...
void CTOPrevNextEventButtons::CDaysMap::_GrowArrayIfNeed(long nDay)
{
		...
		nNewSize < cnMaxArraySize)
	{
		...
		if (nShift)
		{
			...
			MEMMOVE_S(GetData() + nShift, GetData(), XTPToSizeTChecked(nCount2));
			memset(GetData(), 0, XTPToSizeTChecked(min(nShift, nNewSize)));
		}
...
//  if (nDays > (int)m_nCacheSizeMax_days)
	...
	SetSize(nDays);
	memset(GetData(), 0, XTPToSizeTChecked(nDays));
