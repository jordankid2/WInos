...
int CXTPCalendarUtils::ParceTimeDuration_min(LPCTSTR pcszTime)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		if (_tcsnicmp(strScale, arSIname[i], XTPToSizeTChecked(arSIname[i].GetLength())) == 0 ||
			_tcsnicmp(strScale, arSIname2[i], XTPToSizeTChecked(arSIname2[i].GetLength())) == 0)
		{
...
void AFX_CDECL CXTPCalendarUtils::StripFormat(CString& strFormat, int nIndex, int nRemove)
{
...
int AFX_CDECL CXTPCalendarUtils::GetDayOfWeekLocaleStringId(int nWeekDay)
{
	ASSERT(1 <= nWeekDay && nWeekDay <= 7);

	int nFirstID = XTPSystemVersion()->IsWinVistaOrGreater() 
		? LOCALE_SSHORTESTDAYNAME1 : LOCALE_SABBREVDAYNAME1;
	return nFirstID + (nWeekDay - 1);
}
