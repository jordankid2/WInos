...
BOOL AFX_CDECL GetEncoderClsid(const WCHAR *pFormat, CLSID *pClsid)
{
	UINT num, size;
...
LPCWSTR AFX_CDECL GetMimeType(LPCTSTR lpszFileName)
{
...
void AFX_CDECL CXTPGraphicUtils::SaveBitmapToFile(HBITMAP hBitmap, LPCTSTR lpszFileName) // static
{
...
Gdiplus::Status AFX_CDECL CXTPGraphicUtils::GdipCreateBitmapFromHICON_Fixed(HICON hicon, Gdiplus::Bitmap** bitmap)
{
...
Gdiplus::Status AFX_CDECL CXTPGraphicUtils::GdipCreateBitmapFromHICON_Fixed_Native(HICON hicon, Gdiplus::GpBitmap** bitmap)
{
	...
	do
	{
		...
		{
			returnStatus = Gdiplus::InvalidParameter;
			break;
		}
		...
		{
			returnStatus = Gdiplus::GenericError;
			break;
		}
		...
		if (!pbBitmapBits)
		{
			returnStatus = Gdiplus::OutOfMemory;
			break;
		}
		...
		stat = DllExports::GdipBitmapLockBits(*bitmap, &rect, Gdiplus::ImageLockModeWrite, PixelFormat32bppPARGB, &lockeddata);
