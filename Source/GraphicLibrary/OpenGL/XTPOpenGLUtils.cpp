...
void CXTPOpenGLUtils::DebugBox(const CXTPPoint3d& ptMin, const CXTPPoint3d& ptMax, BOOL bSolid /*= FALSE*/)
{
	glPushAttrib(GL_CURRENT_BIT);
...
void CXTPOpenGLUtils::Vertex(const CXTPPoint3d& ptVertext)
{
	glVertex3d(ptVertext.X, ptVertext.Y, ptVertext.Z);
}

void CXTPOpenGLUtils::Vertex(const CXTPPoint3d& ptVertext, const CXTPPoint3d& vNormal)
{
...
CXTPPoint3d CXTPOpenGLUtils::ComputeTriangleNormal(
...
{
	CXTPPoint3d vNormal;
	CXTPMathUtils::ComputeTriangleNormal(
		vA.X, vA.Y, vA.Z,
		vB.X, vB.Y, vB.Z,
		vC.X, vC.Y, vC.Z,
...
BOOL CXTPOpenGLUtils::Project(const CXTPPoint3d& pt3d, CPoint& pt2d, double* pdWinZ /*= NULL*/)
{
	...
	CXTPPoint3d ptWnd;
	if (GL_TRUE == gluProject(
		pt3d.X, pt3d.Y, pt3d.Z,
		modelMatrix, projMatrix, viewport,
		&ptWnd.X, &ptWnd.Y, &ptWnd.Z))
	{
...
BOOL CXTPOpenGLUtils::Unproject(const CPoint& pt2d, CXTPPoint3d& pt3d, double dWinZ /*= 0.*/)
{
	...
	CXTPPoint3d ptWnd;
	bSuccess = (GL_TRUE == gluUnProject(
		pt2d.x, viewport[3] - pt2d.y, dWinZ,
		modelMatrix, projMatrix, viewport,
		&pt3d.X, &pt3d.Y, &pt3d.Z));
