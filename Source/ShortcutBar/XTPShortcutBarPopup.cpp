...
void CXTPShortcutBarPopup::DoModal()
{
	...
	CRect rc(rcShortcutBar.right, rcShortcutBar.top, rcShortcutBar.right + m_nPopupWidth + m_szGap.cx * 2, rcShortcutBar.bottom);

	SetWindowPos(0, rc.left, rc.top, rc.Width(), rc.Height(), SWP_NOZORDER | SWP_SHOWWINDOW);

	::SetParent(m_hWndChild, m_hWnd);
	::SetWindowPos(m_hWndChild, 0, m_szGap.cx, m_szGap.cy, rc.Width() - m_szGap.cx * 2, rc.Height() - m_szGap.cy * 2, SWP_NOZORDER | SWP_SHOWWINDOW);
...
LRESULT CXTPShortcutBarPopup::OnNcHitTest(CPoint point)
{
	...
	CRect rcBorder(rectWindow.right - m_szGap.cx, rectWindow.top, rectWindow.right, rectWindow.bottom);
...
void CXTPShortcutBarPopup::OnSize(UINT nType, int cx, int cy)
{
	...
	::SetWindowPos(m_hWndChild, 0, m_szGap.cx, m_szGap.cy, cx - m_szGap.cx * 2, cy - m_szGap.cy * 2, SWP_NOZORDER);
