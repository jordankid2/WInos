...
IMPLEMENT_DYNAMIC(CXTPShortcutBarItem, CXTPCmdTarget);
...
CXTPShortcutBar::CXTPShortcutBar()
{
	...
	m_hSizeCursor = AfxGetApp()->LoadStandardCursor(IDC_SIZENS);
	...
	m_nMinClientHeight = XTP_DPI_Y(200);
	m_bAllowResize = TRUE;
	...
	m_nMinimizedWidth = XTP_DPI_X(32);
	m_bMinimized = FALSE;
...
void CXTPShortcutBar::AllowMinimize(BOOL bAllowMinimize, int nMinimizedWidth /*= 0*/)
{
	if (0 == nMinimizedWidth)
	{
		nMinimizedWidth = XTP_DPI_X(32);
	}
...
void CXTPShortcutBar::SetTheme(XTPShortcutBarPaintTheme paintTheme)
{
	...
	case xtpShortcutThemeOffice2013:
		...
		break;
	case xtpShortcutThemeVisualStudio2015:
		m_pPaintManager = new CXTPShortcutBarVisualStudio2015Theme();
		break;
	}
...
void CXTPShortcutBar::OnExpandButtonDown(CXTPShortcutBarItem* pExpandButton)
{
	...
	pPopup->SetIconSize(XTP_DPI(CSize(16, 16)));
	pPopup->SetButtonSize(XTP_DPI(CSize(23, 23)));
	...
	pControlAddOrRemove->GetCommandBar()->SetIconSize(XTP_DPI(CSize(16, 16)));
	pControlAddOrRemove->GetCommandBar()->SetButtonSize(XTP_DPI(CSize(23, 23)));
...
BEGIN_INTERFACE_MAP(CXTPShortcutBarItem, CXTPCmdTarget)
	INTERFACE_PART(CXTPShortcutBarItem, XTPDIID_IShortcutBarItem, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPShortcutBarItem, XTPDIID_IShortcutBarItem)

BEGIN_DISPATCH_MAP(CXTPShortcutBarItem, CXTPCmdTarget)
...
CXTPShortcutBarItem* AFX_CDECL CXTPShortcutBarItem::FromDispatch(LPDISPATCH pDisp)
{
	CXTPShortcutBarItem* pInst = NULL;
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPShortcutBarItem, 
			CXTPShortcutBarItem::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
