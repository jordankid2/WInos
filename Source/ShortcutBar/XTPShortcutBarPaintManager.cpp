...
void CXTPShortcutBarPaintManager::CreateFonts()
{
	...
	STRCPY_S(lf.lfFaceName, LF_FACESIZE, m_strOfficeFont.IsEmpty() ? _T("Arial") : m_strOfficeFont);
...
void CXTPShortcutBarPaintManager::DrawExpandButton(CDC* pDC, CRect rc)
{
	...
	if (m_pExpandIcon)
	{
		m_pExpandIcon->Draw(pDC, ptImage, XTP_DPI(CSize(16, 16)));
	}
}
...
void CXTPShortcutBarPaintManager::DrawNavigationItem(CDC* pDC, CXTPShortcutBarItem* pItem)
{
	...
	CRect rcText(CPoint((rc.left + rc.right - sz.cy) / 2, 
		min(rc.bottom - XTP_DPI_Y(3), (rc.top + rc.bottom + sz.cx) / 2)), CSize(sz.cy, sz.cx));
...
void CXTPShortcutBarPaintManager::FillPopupBack(CDC* pDC, CRect rc, CSize szGap)
{
	if (pDC)
	{
		...
		pDC->Draw3dRect(szGap.cx - XTP_DPI_X(1), szGap.cy - XTP_DPI_Y(1), 
			rc.Width() - szGap.cx * 2 + XTP_DPI_X(2), rc.Height() - szGap.cy * 2 + XTP_DPI_Y(2), m_clrPopupFrame, m_clrPopupFrame);
	}
}
...
{
	return AxCreateOleFont(&m_xtpFontText, this, (LPFNFONTCHANGED)&CXTPShortcutBarPaintManager::OleSetFont);
}
