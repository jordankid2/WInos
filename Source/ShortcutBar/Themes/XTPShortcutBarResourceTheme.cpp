// XTPShortcutBarResourceTheme.cpp : implementation of the CXTPShortcutBarResourceTheme class.
//
...
CXTPShortcutBarResourceTheme::CXTPShortcutBarResourceTheme()
{
	...
	m_rcCaptionPadding = XTP_DPI(CRect(8, 0, 8, 0));
	m_rcItemCaptionPadding = XTP_DPI(CRect(8, 0, 8, 0));
}
...
void CXTPShortcutBarResourceTheme::RefreshMetrics()
{
	...
	CXTPResourceImage* pImage = XTPLoadImage(_T("SHORTCUTBARITEMS"));
	if (!pImage)
		return;
	...
	LOGFONT lf;
	m_xtpFontPaneItemCaption.GetLogFont(&lf);
...
void CXTPShortcutBarResourceTheme::DrawPaneCaptionMinimizeButton(CDC* pDC, CXTPShortcutBarPane* pPane)
{
	...
	if (!rcMinimizeButton.IsRectEmpty())
	{
		...
		CXTPResourceImage* pItems  = pImages->LoadFile(_T("SHORTCUTBARITEMS"));
		CXTPResourceImage* pGlyphs = pImages->LoadFile(_T("SHORTCUTBARGLYPHS"));

		if (!pItems || !pGlyphs)
		{
			CXTPShortcutBarOffice2003Theme::DrawPaneCaptionMinimizeButton(pDC, pPane);
			return;
		}
		...
		if (pPane->IsMinimizeButtonHighlighted() || pPane->IsMinimizeButtonPressed())
		{
			...
			pItems->DrawImage(pDC, rcMinimizeButton, pItems->GetSource(nState, 4));
		}

		CRect rcSrc(pGlyphs->GetSource(pPane->GetShortcutBar()->IsShortcutBarMinimized() ? 0 : 1, 5));

		CRect rcDest(CPoint((rcMinimizeButton.left + rcMinimizeButton.right - XTP_DPI_X(rcSrc.Width())) / 2,
			(rcMinimizeButton.top + rcMinimizeButton.bottom - XTP_DPI_Y(rcSrc.Height())) / 2), XTP_DPI(rcSrc.Size()));

		pGlyphs->DrawImage(pDC, rcDest, rcSrc);
	}
}
	...
	else
	{
		if (bDraw)
		{
			...
			if (!rcMinimizeButton.IsRectEmpty())
			{
				...
				rcText.right = rcMinimizeButton.left - XTP_DPI_X(2);
			}
...
int CXTPShortcutBarResourceTheme::DrawPaneItemCaption(CDC* pDC, CXTPShortcutBarPaneItem* pItem, BOOL bDraw)
{
	if (bDraw)
	{
		...
		if (pItem->IsExpandable())
		{
			...
			if (!m_bFlatStyle)
			{
				CRect rcDest(CPoint(rc.right - 2 - XTP_DPI_X(rcSrc.Width()),
					(rc.top + rc.bottom + 2 - XTP_DPI_Y(rcSrc.Height())) / 2),
					XTP_DPI(rcSrc.Size()));
...
void CXTPShortcutBarResourceTheme::FillShortcutItemEntry(CDC* pDC, CXTPShortcutBarItem* pItem)
{
	...
	if (m_bFlatStyle)
	{
		if (!pItem->IsExpanded())
		{
			...
			{
				pImage = pImageSmall;
				rc.DeflateRect(0, XTP_DPI_Y(2), 0, XTP_DPI_Y(2));
			}
		}
...
int CXTPShortcutBarResourceTheme::DrawGripper(CDC* pDC, CXTPShortcutBar* pShortcutBar, BOOL bDraw)
{
	if (bDraw)
	{
		...
		if (m_bFlatStyle)
		{
			rc.DeflateRect(0, 0, 0, XTP_DPI_Y(2));
		}
	...
	return XTP_DPI_Y(m_bFlatStyle ? 7 : 9);
}
