// XTPShortcutBarAnimation.cpp : implementation file
//
...
CXTPShortcutBarAnimation::CAnimateInfo::CAnimateInfo()
{
	hbmSrc = 0;
	pSrcBits = 0;
	hbmSrcBack = 0;
...
CXTPShortcutBarAnimation::CXTPShortcutBarAnimation(CXTPShortcutBar* pShortcutBar)
{
	m_bInvalidate = FALSE;
	m_pParent = pShortcutBar;
	m_nTimer = 0;
...
CXTPShortcutBarAnimation::~CXTPShortcutBarAnimation()
{
	RemoveAnimations();
}

void CXTPShortcutBarAnimation::RemoveAnimations()
{
...
void CXTPShortcutBarAnimation::AlphaBlendU(PBYTE pDest, PBYTE pSrcBack, int cx, int cy, PBYTE pSrc, BYTE byAlpha)
{
...
BOOL CXTPShortcutBarAnimation::IsAnimationEnabled() const
{
	return m_bAnimation;
}

void CXTPShortcutBarAnimation::AnimateRect(CClientDC& dc, CAnimateInfo* pai)
{
...
void CXTPShortcutBarAnimation::OnAnimate()
{
...
void CXTPShortcutBarAnimation::RemoveIntersections(LPCRECT rcPaint, BOOL bAddExclude)
{
	for (int i = 0; i < m_arrAnimation.GetSize(); i++)
	{
...
void CXTPShortcutBarAnimation::AddAnimation(CClientDC& dc, CAnimateInfo* pai)
{
...
void CXTPShortcutBarAnimation::OnDestroy()
{
	m_bmpCache.DeleteObject();
	RemoveAnimations();
}

void CXTPShortcutBarAnimation::RedrawRect(LPCRECT lpRect, BOOL bAnimate)
{
	if (lpRect == NULL)
	{
		m_bmpCache.DeleteObject();
	...
	m_pParent->OnDraw(&dcDraw, rcPaint);

	dcDraw.SelectClipRgn(NULL);
	rgn.DeleteObject();
...
void CXTPShortcutBarAnimation::OnPaint(CPaintDC& paintDC)
{
	CXTPClientRect rc(m_pParent);
	...
	if (!m_bDoubleBuffer && !IsAnimationEnabled())
	{
		...
		m_pParent->OnDraw(&memDC, rcPaint);
