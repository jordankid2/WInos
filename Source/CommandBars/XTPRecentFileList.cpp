...
IMPLEMENT_DYNAMIC(CXTPRecentFileList, CXTPCmdTarget)
...
{
	m_pItems = new CXTPRecentFileListItem*[XTPToUIntChecked(nSize)];
...
void CXTPRecentFileList::WriteList()
{
	...
	LPTSTR pszEntry = new TCHAR[XTPToUIntChecked(max(20, m_strEntryFormat.GetLength() + 7))];
...
void CXTPRecentFileList::ReadList()
{
	...
	for (int iMRU = 0; iMRU < m_nSize; iMRU++)
	{
		...
		if (!m_arrNames[iMRU].IsEmpty())
		{
			...
			wsprintf(pszEntry, _T("Pinned%d"), iMRU + 1);
			m_pItems[iMRU]->m_bPinned = (0 != pApp->GetProfileInt(m_strSectionName, pszEntry, FALSE));

			wsprintf(pszEntry, _T("IconId%d"), iMRU + 1);
			m_pItems[iMRU]->m_nIconId = XTPToInt(pApp->GetProfileInt(m_strSectionName, pszEntry, -1));
...
BEGIN_DISPATCH_MAP(CXTPRecentFileListItem, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPRecentFileListItem, CXTPCmdTarget)
	INTERFACE_PART(CXTPRecentFileListItem, XTPDIID_RecentFileListItem, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPRecentFileListItem, XTPDIID_RecentFileListItem)
...
BEGIN_DISPATCH_MAP(CXTPRecentFileList, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPRecentFileList, CXTPCmdTarget)
	INTERFACE_PART(CXTPRecentFileList, XTPDIID_RecentFileList, Dispatch)
END_INTERFACE_MAP()
...
IMPLEMENT_OLETYPELIB_EX(CXTPRecentFileList, XTPDIID_RecentFileList)
