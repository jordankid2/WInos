...
void CXTPControlGalleryItem::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	...
	{
		pGalleryPaintManager->DrawItem(pDC, rcItem, bEnabled, bSelected, bPressed, bChecked, this, pGallery);
	}
}
...
CXTPImageManagerIcon* CXTPControlGalleryItem::GetImage()
{
	...
	return m_pItems->GetImageManager()->GetImage(XTPToUInt(GetImageIndex() >= 0 ? GetImageIndex() : m_nId), m_nImageWidth);
}
...
BEGIN_DISPATCH_MAP(CXTPControlGalleryItem, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPControlGalleryItem, "ImageIndex", 2, OleGetImageIndex, OleSetImageIndex, VT_I4)
