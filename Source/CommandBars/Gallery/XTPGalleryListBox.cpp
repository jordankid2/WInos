...
void CXTPGalleryListBox::RecalcLayout()
{
	...
	if (GetPaintManager() != NULL
		&& GetPaintManager()->GetCurrentTheme() != XTPPaintManager()->GetCurrentTheme())
	{
		SetPaintManager(CXTPPaintManager::CreateTheme(XTPPaintManager()->GetCurrentTheme()));
	}
...
BOOL CXTPGalleryListBox::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (message == WM_LBUTTONDOWN || message == WM_RBUTTONDOWN || message == WM_MBUTTONDOWN)
	{
		...
		AxGetParentOleControl(this)->FireMouseDown(
			message == WM_LBUTTONDOWN ? LEFT_BUTTON : message == WM_RBUTTONDOWN ? RIGHT_BUTTON : MIDDLE_BUTTON,
	...
	{
		CPoint point((DWORD)lParam);
		AxGetParentOleControl(this)->FireMouseUp(
			message == WM_LBUTTONUP ? LEFT_BUTTON : message == WM_RBUTTONUP ? RIGHT_BUTTON : MIDDLE_BUTTON,
			AxShiftState(), point.x, point.y);
	}
...
void CXTPRecentFileListBoxPinItem::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	...
	if (pIcon)
	{
		...
		pIcon->Draw(pDC, CPoint((rcItem.left + rcItem.right - sz.cx) / 2,
			(rcItem.top + rcItem.bottom - sz.cy) / 2), sz);
	}
}
	...
	: m_bUseShellIcon(FALSE)
{
	...
	m_strCaption = m_pRecentItem ? m_pRecentItem->GetPathName() : _T("");
...
void CXTPRecentFileListBoxItem::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	...
	if (pImage)
	{
		...
		pImage->Draw(pDC, pt, XTP_DPI(CSize(32, 32)));
	}
	else
	{
		DWORD dwFlags = SHGFI_LARGEICON | SHGFI_ICON;
		LPCWSTR lpszFile = XTP_CT2CW(m_strCaption);
		DWORD dwFileAttr = 0;
		...
		if (m_bUseShellIcon)
		{
			...
			lpszFile = XTP_CT2CW(::PathFindExtension(m_strCaption));
		}

		SHFILEINFO sfi;
		if (SUCCEEDED(::SHGetFileInfo(XTP_CW2CT(lpszFile), dwFileAttr, 
			&sfi, sizeof(SHFILEINFO), dwFlags)) && sfi.hIcon)
		{
			DrawIconEx(pDC->GetSafeHdc(), rcItem.left + XTP_DPI_X(7), rcItem.top + XTP_DPI_Y(2), 
				sfi.hIcon, XTP_DPI_X(32), XTP_DPI_X(32), 0, 0, DI_NORMAL);
...
void CXTPRecentFileListBox::SetUseShellIcon(BOOL bUseShellIcon)
{
	...
	{
		CXTPRecentFileListBoxItem* pItem = DYNAMIC_DOWNCAST(CXTPRecentFileListBoxItem, m_pItems->GetItem(i));
		if (NULL != pItem)
		{
