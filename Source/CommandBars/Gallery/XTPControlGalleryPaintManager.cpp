...
void CXTPControlGalleryPaintManager::DrawLabel(CDC* pDC, CXTPControlGalleryItem* pLabel, CRect rcItem)
{
	...
	pDC->FillSolidRect(rcItem.left, rcItem.bottom - XTP_DPI_Y(1), 
		rcItem.Width(), XTP_DPI_Y(1), pPaintManager->GetXtremeColor(XPCOLOR_3DSHADOW));
...
void CXTPControlGalleryPaintManager::DrawPopupScrollBar(CDC* pDC, CXTPControlGallery* pGallery)
{
	...
	COLORREF clr = GetItemTextColor(IS_PART_ENABLED(nState), IS_PART_SELECTED(nState), IS_PART_PRESSED(nState), FALSE, xtpBarTypeNormal);
	...
	clr = GetItemTextColor(IS_PART_ENABLED(nState), IS_PART_SELECTED(nState), IS_PART_PRESSED(nState), FALSE, xtpBarTypeNormal);
...
COLORREF CXTPControlGalleryPaintManager::GetItemTextColor(BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked, XTPBarType barType)
{
	return m_pPaintManager->GetRectangleTextColor(bSelected, bPressed, bEnabled, bChecked, FALSE, barType, xtpBarPopup);
}

void CXTPControlGalleryPaintManager::DrawItemBack(CDC* pDC, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked, XTPBarType barType)
{
	m_pPaintManager->DrawGalleryItemBack(pDC, rcItem, bEnabled, bSelected, bPressed, bChecked, barType);
}

void CXTPControlGalleryPaintManager::DrawItem(CDC* pDC, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked, CXTPControlGalleryItem* pGalleryItem, CXTPControlGallery* pGallery)
{
	ASSERT(pGallery);
	ASSERT(pGalleryItem);
	...
	if (pMarkupUIElement)
	{
		...
		DrawItemBack(pDC, rcItem, bEnabled, bSelected, bPressed, bChecked, bListStyle ? xtpBarTypePopup : xtpBarTypeNormal);
		COLORREF clrTextColor = GetItemTextColor(bEnabled, bSelected, bListStyle ? FALSE : bPressed, bChecked, bListStyle ? xtpBarTypePopup : xtpBarTypeNormal);

		XTPMarkupSetDefaultFont(XTPMarkupElementContext(pMarkupUIElement),
			(HFONT)m_pPaintManager->GetRegularFont()->GetSafeHandle(), clrTextColor);
	...
	BOOL bDrawImgeAndText = (pImage != NULL && !strCaption.IsEmpty()) &&
	...
	if (bDrawImgeAndText && pGallery->GetStyle() == xtpButtonIconAndCaptionBelow)
	{
		...
		COLORREF clrTextColor = GetItemTextColor(bEnabled, bSelected, FALSE, FALSE, xtpBarTypePopup);
	...
	else if (bDrawImgeAndText)
	{
		...
		if (bChecked)
		{
			CRect rcCheckBox(rcItem.left + n2x, rcItem.top + n2y, rcItem.left + n2x + nImageBoxWidth, rcItem.bottom - n2y);
			DrawItemBack(pDC, rcCheckBox, bEnabled, FALSE, FALSE, TRUE, xtpBarTypePopup);
		}

		CPoint pt(rcItem.left + n2x + (nImageBoxWidth - szImage.cx) / 2, (rcItem.top + rcItem.bottom - szImage.cy) / 2);
		...
		if (pGallery->GetStyle() == xtpButtonCaptionAndDescription)
		{
			...
			pDC->DrawText(strCaption, rcText, DT_END_ELLIPSIS | DT_SINGLELINE | DT_TOP | DT_NOPREFIX);
		...
		{
			CXTPFontDC font(pDC, m_pPaintManager->GetRegularFont());
			pDC->DrawText(strCaption, rcText, DT_END_ELLIPSIS | DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
		}
	}
	else if (pImage)
	{
	...
	else
	{
		...
		COLORREF clrTextColor = GetItemTextColor(bEnabled, bSelected, FALSE, bChecked, xtpBarTypePopup);
