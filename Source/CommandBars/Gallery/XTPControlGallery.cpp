...
void CXTPControlGallery::PerformMouseMove(CPoint point)
{
	...
	if (m_bPressed && (m_ptPressed != CPoint(0, 0)) && (m_nSelected != -1) &&
		(abs(m_ptPressed.x - point.x) > XTP_DPI_X(4) || abs(m_ptPressed.y - point.y) > XTP_DPI_Y(4)))
	{
		m_ptPressed = CPoint(0, 0);
...
{
	m_nWidth = CXTPControlGallery::m_bAutoScaleDpi ? XTP_DPI_X(szControl.cx) : szControl.cx;
	m_nHeight = CXTPControlGallery::m_bAutoScaleDpi ? XTP_DPI_Y(szControl.cy) : szControl.cy;
}
...
void CXTPControlGallery::AdjustExcludeRect(CRect& rc, BOOL bVertical)
{
	...
	{
		rc.SetRect(m_rcControl.left - XTP_DPI_X(1), m_rcControl.top, m_rcControl.left - XTP_DPI_X(1), m_rcControl.top);
	}
}
...
{
	CXTPControlGalleryItems* pItems = DYNAMIC_DOWNCAST(CXTPControlGalleryItems, 
		CXTPControlGalleryItems::FromIDispatchSafe(lpDispatch));
	if (NULL == pItems)
	{
...
{
	CXTPControlGalleryItem* pItem = DYNAMIC_DOWNCAST(CXTPControlGalleryItem, 
		CXTPControlGalleryItem::FromIDispatchSafe(lpSelected));
