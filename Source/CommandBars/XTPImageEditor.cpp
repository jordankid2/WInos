...
void CXTPImageEditorPicture::Init(UINT nID, CXTPImageEditorDlg* pParentWnd)
{
	...
	SetWindowPos(&CWnd::wndTop, rc.left, rc.top, sz.cx + XTP_DPI_X(1), sz.cy + XTP_DPI_Y(1), SWP_NOMOVE);
	...
	for (int x = 0; x < rc.Width(); x++)
	{
		for (int y = 0; y < rc.Height(); y++)
		{
			...
			if (bAlpha)
			{
				...
				DWORD nAlpha = (clr & 0xFF000000) >> 24;
				DWORD nColor = XTPToDWORD((((x + 3) / 4) + ((y + 3) / 4)) % 2 == 0 ? 255 : 235);
...
void AFX_CDECL CXTPImageEditorPicture::CopyBitmap(CAlphaBitmap* pBitmapCopyTo, CAlphaBitmap* pBitmapCopyFrom, CSize sz, CPoint ptOffset, COLORREF clrTransparentFrom, COLORREF clrTransparentTo, CDC* pDC)
{
	CDC dc;
...
void CXTPImageEditorPicker::OnLButtonDown(UINT /*nFlags*/, CPoint /*point*/)
{
	...
	GetParent()->SendMessage(WM_NOTIFY, XTPToWPARAM(mHDR.idFrom), XTPToLPARAM(&mHDR));
}
...
void CXTPImageEditorPreview::OnPaint()
{
	...
	CRect rcButton(XTP_DPI_X(10), (rc.Height() - m_szPicture.cy) / 2, 
		XTP_DPI_X(10) + m_szPicture.cx, (rc.Height() - m_szPicture.cy) / 2 + m_szPicture.cy);
	CRect rcFrame(rcButton);
	rcFrame.InflateRect(XTP_DPI_X(3), XTP_DPI_Y(3));
...
CXTPImageEditorDlg::CDlgToolBar::CDlgToolBar()
{
	SetIconSize(XTP_DPI(CSize(16, 16)));
	SetButtonSize(XTP_DPI(CSize(23, 23)));
}
	...
	: m_bCommandBarsEditor(bCommandBarsEditor)
{
	...
	m_pSelected = NULL;
	m_szPicture = XTP_DPI(CSize(16, 16));
...
void CXTPImageEditorDlg::GetIcon(CXTPImageManagerIconHandle* pHandle)
{
	...
	if (m_wndPicture.m_pPictureBitmap)
	{
		...
		else
		{
			...
			pHandle->Attach(imgList.ExtractIcon(0), FALSE);
		}
	}
}
...
void CXTPImageEditorDlg::SetIcon(CXTPImageManagerIconHandle& icon)
{
	...
	if (iconHandle.IsAlpha())
	{
		...
		FixAlphaLayer(lpBits, XTPToUIntChecked(m_szPicture.cx * m_szPicture.cy));
...
BOOL CXTPImageEditorDlg::OnInitDialog()
{
	...
	CSize sz = m_wndToolbarTools.CalcDockingLayout(XTP_DPI_X(23) * 4 + XTP_DPI_X(7), LM_HORZDOCK | LM_HORZ | LM_COMMIT);
	m_wndToolbarTools.SetWindowPos(&CWnd::wndTop, rcTools.left - XTP_DPI_X(7), rcTools.top - XTP_DPI_Y(4), sz.cx, sz.cy, 0);
	...
	sz = m_wndToolbarMove.CalcDockingLayout(XTP_DPI_X(23) * 4 + XTP_DPI_X(7), LM_HORZDOCK | LM_HORZ | LM_COMMIT);
	m_wndToolbarMove.SetWindowPos(&CWnd::wndTop, rcMove.left - XTP_DPI_X(7), rcMove.top - XTP_DPI_Y(4), sz.cx, sz.cy, 0);
	...
	{
		CRect rcItem(CPoint(rcColors.left, rcColors.top - XTP_DPI_Y(4)), XTP_DPI(CSize(11, 11)));
		rcItem.OffsetRect(colors[i].ptOffset.x * XTP_DPI_X(13), colors[i].ptOffset.y * XTP_DPI_Y(13));
