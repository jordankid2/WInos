...
{
	int nHeight = (0 == m_nCustomHeight
		? GetMessageBarPaintManager()->GetHeight()
		: m_nCustomHeight);
	return nHeight;
}
...
void CXTPMessageBar::RecalcLayout(CDC* pDC)
{
	...
	m_rcContent = CRect(CPoint(rcMargin.left + rcPadding.left, (rcClient.Height() - szContent.cy) / 2), szContent);
	...
	for (i = 0; i < m_arrButtons.GetSize(); i++)
	{
		...
		pButton->m_rcButton = CRect(CPoint(nLeft + rcPadding.right, (rcClient.Height() - szButton.cy) /2), szButton);
		nLeft = pButton->m_rcButton.right;
	}

	int nRight = rcClient.right - rcMargin.right - rcPadding.right - (pCloseButton ? XTP_DPI_X(18) : 0);
	if (nLeft > nRight)
	{
...
INT_PTR CXTPMessageBar::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	if (m_pHotButton)
	{
		nHit = XTPToIntPtrChecked(m_pHotButton->m_nID);
