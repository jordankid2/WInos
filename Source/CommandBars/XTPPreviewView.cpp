...
CXTPControlPageSelector::CXTPControlPageSelector()
{
	...
	m_szItem = XTP_DPI(CSize(32, 32));
}

void CXTPControlPageSelector::OnDrawItem(CDC* pDC, CRect rcItem, COLORREF clrText)
{
	rcItem.DeflateRect(XTP_DPI_X(3), XTP_DPI_Y(4), XTP_DPI_X(3), XTP_DPI_Y(4));

	for (int y = rcItem.top, i = 0; y < rcItem.bottom; y += XTP_DPI_Y(3), i++)
	{
...
CXTPPreviewView::CXTPPreviewView(int nPageCount /*=9*/)
{
	...
	m_pPageInfo2 = new PAGE_INFO[XTPToUIntChecked(nPageCount)];
...
static const UINT nDefaultZooms[] = { 10, 25, 50, 75, 100, 150, 200, 500 };

int CXTPPreviewView::GetZoomState() const
{
	return XTPToIntChecked(m_nZoomState);
}
...
int CXTPPreviewView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	ASSERT (m_pToolBar); // dialog bar must be valid.
	...
	m_pCommandBar->SetButtonSize(XTP_DPI(CSize(23, 23)));
	m_pCommandBar->SetIconSize(XTP_DPI(CSize(16, 16)));
	...
	pControlGotoPage->SetWidth(XTP_DPI_X(100));
	...
	pControlZoom->ModifyListBoxStyle(0, LBS_OWNERDRAWFIXED | LBS_HASSTRINGS);

	pControlZoom->SetWidth(XTP_DPI_X(100));
...
{
	return m_pCommandBar->GetControls()->FindControl(XTPToInt(nCommandID));
}
...
void CXTPPreviewView::DoZoom(UINT nPage, CPoint point)
{
	if (m_nZoomState == XTP_ZOOM_WHOLEPAGE)
	{
		...
		if (bShowBar)
		{
			...
			info.nMin = XTPToInt(m_pPreviewInfo->GetMinPage());
			info.nMax = XTPToInt(m_pPreviewInfo->GetMaxPage());
			info.nPage = 1;
...
#define PREVIEW_MARGIN  XTP_DPI_X(8)
#define PREVIEW_PAGEGAP XTP_DPI_X(8)
...
void CXTPPreviewView::SetScaledSize(UINT nPage)
{
	...
	switch (m_nZoomState)
	{
		...
		case XTP_ZOOM_PAGEWIDTH:
			...
			pRatio->cx = windowSize.cx - PREVIEW_MARGIN * 2 - XTP_DPI_X(6);
			pRatio->cy = pSize->cx;
			break;
		...
		case 100:
			if (bPaperLarger)
			{
				pRatio->cx = XTP_DPI_X(1);
				pRatio->cy = XTP_DPI_Y(1);
			}
			else
			{
		...
		default:
			pRatio->cx = XTPToLongChecked(m_nZoomState);
			pRatio->cy = XTP_DPI_Y(100);
			break;
	}
	...
	pRect->SetRect(PREVIEW_MARGIN, PREVIEW_MARGIN,
		scaledSize.cx + PREVIEW_MARGIN + XTP_DPI_X(3),
		scaledSize.cy + PREVIEW_MARGIN + XTP_DPI_Y(3));

	if (m_nZoomState == XTP_ZOOM_WHOLEPAGE)
	{
		pRect->OffsetRect((windowSize.cx - pRect->Size().cx) / 2 - XTP_DPI_X(1), 
			(windowSize.cy - pRect->Size().cy) / 2 - XTP_DPI_Y(1));

		// we need to offset the page multiple times
		int local = XTPToIntChecked(nPage) % m_Across;
		while (local-- >= 1)
		{
			pRect->OffsetRect(m_PageOffset.x, 0);
		}

		local = XTPToIntChecked(nPage) / m_Across;
		while (local-- > 0)
		{
...
void CXTPPreviewView::SetupScrollbar()
{
	...
	if (m_pPreviewInfo->GetMaxPage() < 0x8000 && m_pPreviewInfo->GetMaxPage() - m_pPreviewInfo->GetMinPage() <= 32767U)
	{
		...
		info.fMask = SIF_PAGE|SIF_RANGE;
		info.nMin = XTPToIntChecked(m_pPreviewInfo->GetMinPage());
		info.nMax = XTPToIntChecked(__min(m_pPreviewInfo->GetMaxPage(), m_nPages));
		info.nPage = 1; // correct? MSDN: Specifies the page size, in device units

		if (info.nMin == 1 && info.nMax == 1)
...
void CXTPPreviewView::OnGotoPageKillFocus(NMHDR* pNMHDR, LRESULT* /*pRes*/)
{
	...
	int niCurrentPage = _ttoi(strText);
	if (0 <= niCurrentPage)
	{
		UINT nCurrentPage = XTPToUInt(niCurrentPage);
...
void CXTPPreviewView::OnZoomCombo(NMHDR* pNMHDR, LRESULT* pRes)
{
	...
	if (0 <= nCurSel)
	{
		...
		{
			SetZoomState(XTPToUInt(nCurSel), 0, CPoint(0, 0));
		}
		else if (nCurSel > 1)
		{
			SetZoomState(XTPToUInt(pControl->GetItemData(nCurSel)), 0, CPoint(0, 0));
		}
	}
	else
	{
		...
		{
			SetZoomState(XTPToUInt(nZoom), 0, CPoint(0, 0));
...
void CXTPPreviewView::SetNumPreviewPages(int nAcross, int nDown)
{
	...
	m_Across = nAcross;
	m_Down = nDown;
	m_nPages = XTPToUIntChecked(nAcross * nDown);
