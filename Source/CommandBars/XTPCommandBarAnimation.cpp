...
void CXTPCommandBarAnimation::RedrawRect(LPCRECT lpRect, BOOL bAnimate)
{
	...
	else
	{
		...
		if (NULL != pai->hbmSrc && NULL != pai->hbmSrcBack
			&& NULL != pai->pSrcBits && NULL != pai->pSrcBackBits)
		{
			CDC dcSrc;
			...
			if (memcmp(pai->pSrcBits, pai->pSrcBackBits, XTPToSizeTChecked(rcPaint.Width() * rcPaint.Height() * 4)) == 0)
			{
				delete pai;
			}
			else
