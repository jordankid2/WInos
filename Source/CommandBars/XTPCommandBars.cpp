...
CXTPCommandBar* CXTPCommandBarsContextMenus::Add(UINT nIDBar, UINT nIDMenuResource, int nSubMenu)
{
	...
	CString strCaption;
	XTPResourceManager()->GetMenuLocaleString(&menu, XTPToUIntChecked(nSubMenu), strCaption, MF_BYPOSITION);
...
IMPLEMENT_DYNCREATE(CXTPCommandBars, CXTPCmdTarget)
...
CXTPDockBar* CXTPCommandBars::CanDock(CPoint pt, CXTPDockBar* pMainDock) const
{
	...
	if (pMainDock)
	{
		...
		if (pMainDock->m_dwStyle & CBRS_ORIENT_HORZ &&
			rc.top - XTP_DPI_Y(20) < pt.y && rc.bottom + XTP_DPI_Y(20) > pt.y && rc.left - XTP_DPI_X(20) < pt.x && rc.right + XTP_DPI_X(20) > pt.x)
			return pMainDock;
		if (pMainDock->m_dwStyle & CBRS_ORIENT_VERT &&
			rc.left - XTP_DPI_X(20) < pt.x && rc.right + XTP_DPI_X(20) > pt.x && rc.top - XTP_DPI_Y(20) < pt.y && rc.bottom + XTP_DPI_Y(20) > pt.y)
			return pMainDock;
	}
	for (int i = 0; i < 4; i++)
	{
...
void CXTPCommandBars::ShowKeyboardTips(CXTPCommandBar* pCommandBar, int nLevel)
{
	...
	for (i = (int)m_keyboardTips.arr.GetSize() - 1; i >= 0; i--)
	{
		...
		if (pWnd->m_strPrefix.IsEmpty())
		{
			...
			if (XTPSystemVersion()->GetMaxCharSize() > 1 && 
#ifndef _UNICODE
				_istlead(static_cast<BYTE>(pWnd->m_strTip[0]))
#else
				_istlead(static_cast<WORD>(pWnd->m_strTip[0]))
#endif
				)
	...
	for (i = 0; i < nCount; i++)
	{
		...
		pWnd->CreateEx(DWORD((bLayered ? WS_EX_LAYERED : 0) | WS_EX_TOOLWINDOW), AfxRegisterWndClass(CS_SAVEBITS), 0, WS_POPUP, rc, pCommandBar, 0);
...
BOOL CXTPCommandBars::OnFrameWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (message == WM_COMMAND)
	{
		...
		if (LOWORD(wParam) == ID_HELP && GetHelpTrackingId() > 0)
		{
			...
			{
				GetSite()->SendMessage(WM_COMMANDHELP, 0, XTPToLPARAM(HID_BASE_COMMAND +  GetHelpTrackingId()));
				return TRUE;
			}
		}
	}
	...
	{
		if ((wParam == 0 || LOWORD(wParam) != 0) && (wParam & 0xFFF0) != SC_KEYMENU && !IsCustomizeMode())
			ClosePopups();
	}

	if ((message == WM_SETFOCUS) || (message == WM_ACTIVATE))
	{

		CXTPRibbonBar* pRibbonBar = DYNAMIC_DOWNCAST(CXTPRibbonBar, GetMenuBar());

		if (pRibbonBar)
		{
			if (pRibbonBar->IsBackstageViewVisible())
				return TRUE;
		}
	}
...
CXTPControl* CXTPCommandBars::FindControl(XTPControlType type, UINT nId, BOOL bVisible, BOOL bRecursive) const
{
	...
	for (int i = GetCount() - 1; i >= 0; i--)
	{
		...
		{
			CXTPControl* pControl = pBar->GetControls()->FindControl(type, XTPToInt(nId), bVisible, bRecursive);
...
BOOL CXTPCommandBars::TrackPopupMenu(CXTPPopupBar* pPopup, UINT nFlags, int x, int y, CWnd* pWnd, LPCRECT rcExclude, CWnd* pWndOwner)
{
	...
	pPopup->m_bExecOnRButton = (0 != (nFlags & TPM_RIGHTBUTTON));
	pPopup->m_bIgnoreUpdateHandler = (0 != (nFlags & TPM_NONOTIFY));
	...
	BOOL bRecurse = (0 != (nFlags & TPM_RECURSE));
...
void CXTPCommandBars::SetSite(CWnd* pFrame)
{
	...
	m_bRightToLeft = (0 != (m_pFrame->GetExStyle() & WS_EX_LAYOUTRTL));
}
...
BOOL CXTPCommandBars::IsControlHidden(CXTPControl* pControl)
{
	...
	BOOL bCommandHidden = IsCommandHidden(XTPToUInt(pControl->GetID()));
...
void CXTPCommandBars::EnableFrameTheme(BOOL bEnable)
{
	...
	CXTPCommandBarsFrameHook* pFrameHook = NULL;

	if (!bEnable)
	{
		if (IsEnableFrameTheme())
		{
			pFrameHook = m_pFrameHook;
			BOOL bInternalFrameHook = m_bInternalFrameHook;

			if (!m_bBeingDestructed)
			{
				m_pFrameHook->DisableFrameTheme();
			}

			if (bInternalFrameHook)
			{
				SetFrameHook(NULL);
	...
	else if (!IsEnableFrameTheme())
	{
		m_pFrameHook = new CXTPCommandBarsFrameHook();;
		pFrameHook = m_pFrameHook;
		m_pFrameHook->EnableFrameTheme(this);
		ASSERT(m_pFrameHook == pFrameHook && "Internal frame hook reference is lost");

		bRecalcLayout = TRUE;
		m_bInternalFrameHook = TRUE;
	}
...
BEGIN_DISPATCH_MAP(CXTPCommandBarsContextMenus, CXTPCmdTarget)
...
IMPLEMENT_OLETYPELIB_EX(CXTPCommandBarsContextMenus, XTPDIID_ICommandBarsContextMenus)
