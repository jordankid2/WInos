...
void CXTPControlButtonColor::Draw(CDC* pDC)
{
	...
	{
		CRect rcButton(CPoint(m_rcControl.left + XTP_DPI_X(2), m_rcControl.CenterPoint().y - XTP_SELECTOR_SIZE / 2), CSize(XTP_SELECTOR_SIZE, XTP_SELECTOR_SIZE));
...
CXTPControlColorSelector::CXTPControlColorSelector()
{
	...
	if (ghExtendedColorsResourceInstance != hCurrentResourceInstance)
	{
		...
		if (ghExtendedColorsResourceInstance != hCurrentResourceInstance)
		{
			...
			for (int i = 0; i < nCount; i++)
			{
				XTPResourceManager()->LoadString(&gExtendedColors[i].strTip, gExtendedColors[i].toolTipID);
			}
...
INT_PTR CXTPControlColorSelector::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	INT_PTR nHit = XTPToIntPtrChecked(gExtendedColors[nIndex].toolTipID);

	CXTPToolTipContext::FillInToolInfo(pTI, GetParent()->GetSafeHwnd(), GetRect(nIndex), nHit, gExtendedColors[nIndex].strTip);
...
void CXTPControlColorSelector::Draw(CDC* pDC)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (i == m_nSelected || gExtendedColors[i].clrButton == m_clr)
		{
			...
			GetPaintManager()->DrawRectangle(pDC, rcButton, i == m_nSelected, bPressed, GetEnabled(), gExtendedColors[i].clrButton == m_clr, FALSE, xtpBarTypeNormal, xtpBarPopup);
		}
