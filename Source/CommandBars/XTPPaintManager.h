...
public:
	...
	//-----------------------------------------------------------------------
	CXTPPaintManagerFont() : m_bStandardFont(TRUE) {}
...
public:
	...
	//     barType     - Parent's bar type
	//-----------------------------------------------------------------------
	virtual void DrawGalleryItemBack(CDC* pDC, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked, XTPBarType barType);
	...
	virtual void DrawExpandGlyph(CDC* pDC, CXTPControl* pButton, COLORREF clr, BOOL bVertical); // <combine CXTPPaintManager::DrawExpandGlyph@CDC*@CXTPControl*@CPoint@CPoint@CPoint@COLORREF>
	virtual void DrawHiddenGlyph(CDC* pDC, CXTPControl* pButton, COLORREF clr, BOOL bVertical); // <combine CXTPPaintManager::DrawExpandGlyph@CDC*@CXTPControl*@CPoint@CPoint@CPoint@COLORREF>
...
public:
	...
	void Line(CDC* pDC, int x0, int y0, int x1, int y1, const int nPen); //<combine CXTPPaintManager::Line@CDC*@CPoint@CPoint>
	void Line(CDC* pDC, int x0, int y0, int x1, int y1, COLORREF clrPen); //<combine CXTPPaintManager::Line@CDC*@CPoint@CPoint>
...
//{{AFX_CODEJOCK_PRIVATE
public:
	_XTP_DEPRECATE("The function is no longer supported. Use GetInstanceAddRef and ReleaseInstance instead.")
	...
	virtual CSize GetStretchIconSizeDPIAwareness(CXTPImageManagerIcon* pImage, CSize szIcon, CXTPControl* pControl, CSize szAvailable);
...
protected:
	...
	virtual CSize DrawControlText(CDC* pDC, CXTPControl* pButton, CRect rcText, BOOL bDraw, BOOL bTriangled, CSize szImage, BOOL bDrawImage);
	...
	static int AFX_CDECL CalcEntityHeight(
		int nFontHeight,
		int nFactor,
		int nMinHeight = XTP_COMMANDBAR_MIN_CONTROL_HEIGHT,
		int nCustomDPI = 0);
	static int AFX_CDECL CalcTextHeight(
		int nFontHeight,
		int nFactor = XTP_COMMANDBAR_TEXT_HEIGHT_FACTOR,
		int nCustomDPI = 0);
	static int AFX_CDECL CalcCtrlHeight(
		int nFontHeight,
...
protected:
	...
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CFont, m_fontRegular, m_xtpFontRegular, GetRegularFontHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CFont, m_fontRegularBold, m_xtpFontRegularBold, GetRegularBoldFontHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CFont, m_fontVert, m_xtpFontVert, GetVertFontHandle);
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CFont, m_fontVertBold, m_xtpFontVertBold, GetVertBoldFontHandle);
	...
	//{{AFX_CODEJOCK_PRIVATE
	_XTP_DEPRECATE("The member is obsolete. Use GetGlobalPaintManager/SetGlobalPaintManager instead")
...
{
	return this == NULL ? CFont ::FromHandle((HFONT)::GetStockObject(DEFAULT_GUI_FONT)): &m_xtpFontRegular;
}
...
AFX_INLINE BOOL CXTPPaintManager::IsCaptionbarInBackstageMode() const
{
	return m_bIsCaptionBarInBackstageMode;
}

AFX_INLINE void CXTPPaintManager::SetCaptionbarBackstageMode(BOOL bMode)
{
	m_bIsCaptionBarInBackstageMode = bMode;
}

AFX_INLINE int CXTPPaintManager::GetBackstageModeMenuWidth() const
{
	return m_nBackstageMenuWidth;
}

AFX_INLINE void CXTPPaintManager::SetBackstageModeMenuWidth(int nWidth)
{
	m_nBackstageMenuWidth = nWidth;
}

AFX_INLINE int CXTPPaintManager::GetBackstageSysButtonTop() const
{
	return m_nBackstageSysButtonTop;
}

AFX_INLINE void CXTPPaintManager::SetBackstageSysButtonTop(int nTop)
{
	m_nBackstageSysButtonTop = nTop;
}

AFX_INLINE int CXTPPaintManager::GetBackstageSysButtonBottom() const
{
	return m_nBackstageSysButtonBottom;
}

AFX_INLINE void CXTPPaintManager::SetBackstageSysButtonBottom(int nBottom)
{
