...
CXTPCommandBarEditCtrl::CRichEditContext::CRichEditContext()
{
	...
	if (XTPLoadWindowClass(xtpWindowClassRichEdit,
		xtpWindowClassVersionHighest,
		&nVersion, &wc))
	{
...
BOOL CXTPCommandBarEditCtrl::ShowContextMenu(CXTPControl* pControl, CPoint point)
{
	...
	UINT nAlign = UINT(pControl->GetParent()->GetExStyle() & (WS_EX_LAYOUTRTL | WS_EX_RIGHT) ? TPM_RIGHTALIGN : TPM_LEFTALIGN);
	int nCommand = CXTPCommandBars::TrackPopupMenu(pPopupBar, UINT(TPM_NONOTIFY | TPM_RECURSE | TPM_RETURNCMD | nAlign), point.x, point.y, this, NULL, this);
...
BOOL CXTPCommandBarEditCtrl::CreateEdit(DWORD dwStyle, CWnd* pParentWnd)
{
	...
	{
		if (!CWnd::Create(GetRichEditContext().m_strClassName, 0,
			dwStyle | ES_NOOLEDRAGDROP, CRect(0, 0, 0, 0), pParentWnd, 0))
		{
			return FALSE;
		}
...
void CXTPControlEdit::SetRect(CRect rcControl)
{
	...
	if (m_pEdit)
	{
		...
		rcControl.DeflateRect(
			m_nLabelWidth + XTP_DPI_X(1 + 3), XTP_DPI_Y(3),
			XTP_DPI_X(3) + (m_bShowSpinButtons ? (XTP_DPI_X(18) - 1) : 0) + (m_nBuddyButtonId > 0 ? XTP_DPI_X(18) : 0), XTP_DPI_Y(3));
...
void CXTPControlEdit::DrawEditText(CDC* pDC, CRect rcText)
{
	if (m_nEditIconId > 0)
	{
		...
		if (pIcon)
		{
			...
			if (GetPaintManager()->IsDPIIconsScallingEnabled(this))
			{
				sz = XTP_DPI(sz);
			}
...
void CXTPControlEdit::DeflateEditRect(CRect& rcControl)
{
	if (m_nEditIconId > 0)
	{
		...
		if (pIcon)
		{
			...
			if (GetPaintManager()->IsDPIIconsScallingEnabled(this))
			{
				nIconWidth = XTP_DPI_X(nIconWidth);
			}
			rcControl.left += nIconWidth + XTP_DPI_X(2);
		}
	}
}
...
CRect CXTPControlEdit::GetSpinButtonsRect() const
{
	...
	CRect rc(GetRect());
	return CRect(rc.right - XTP_DPI_X(18), rc.top + XTP_DPI_Y(1), rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1));
}
...
CRect CXTPControlEdit::GetBuddyButtonRect() const
{
	...
	CRect rc(GetRect());
	rc = CRect(rc.right - XTP_DPI_X(18 + 1), rc.top + XTP_DPI_Y(1), rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1));
...
BEGIN_DISPATCH_MAP(CXTPControlEdit, CXTPControl)
	...
	DISP_PROPERTY_EX_ID(CXTPControlEdit, "EditHandle", 63, OleGetEditHandle, SetNotSupported, VT_HANDLE)
	...
	DISP_PROPERTY_EX_ID(CXTPControlEdit, "ShellAutoComplete", 69, OleGetShellAutoComplete, OleSetShellAutoComplete, VT_I4)

	DISP_PROPERTY_EX_ID(CXTPControlEdit, "EditIconId", 70, OleGetEditIconId, OleSetEditIconId, VT_I4)
	...
	DISP_PROPERTY_EX_ID(CXTPControlEdit, "BuddyButtonId", 81, OleGetBuddyButtonId, OleSetBuddyButtonId, VT_I4)
