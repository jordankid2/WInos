...
CXTPMiniToolBar::CXTPMiniToolBar()
{
	...
	m_szButtonSpacing = XTP_DPI(CSize(0, 3));
}
...
void CXTPMiniToolBar::UpdateOpacity()
{
	...
	{
		if (nLen < XTP_DPI_X(17)) nOpacity = XTP_DPI_X(17) + (XTP_DPI_X(16) - nLen) * 14;
	}
	else
	{
		if (nLen < XTP_DPI_X(90)) nOpacity = MulDiv(255, XTP_DPI_X(90) - nLen, XTP_DPI_X(90));
	}
	...
	if ((m_nOpacity != nOpacity))
	{
		...
		{
			SetWindowPos(0, 0, 0, 0, 0, UINT(SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER 
				| SWP_NOOWNERZORDER | (nOpacity > 20 ? SWP_SHOWWINDOW : SWP_HIDEWINDOW)));
		}
		...
		while (pos)
		{
			...
			{
				pShadow->SetWindowPos(0, 0, 0, 0, 0, UINT(SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER 
					| SWP_NOOWNERZORDER | (nOpacity > 20 ? SWP_SHOWWINDOW : SWP_HIDEWINDOW)));
			}
		}
	}
	...
	if (nLen > XTP_DPI_X(m_bActivated ? 180 : 110))
	{
		m_bTracking = FALSE;
	}
}
...
BOOL CXTPMiniToolBar::TrackMiniBar(UINT nFlags, int x, int y)
{
	...
	m_bExecOnRButton = (0 != (nFlags & TPM_RIGHTBUTTON));
	m_popupFlags = DWORD((nFlags & TPM_RIGHTALIGN) ? xtpPopupLeft | xtpPopupUp : xtpPopupUp);
...
BOOL CXTPMiniToolBar::TrackPopupMenu(CXTPPopupBar* pPopup, UINT nFlags, int x, int y)
{
	...
	const int uMenuAndToolbarIdent = XTP_DPI_X(15); // px
...
IMPLEMENT_OLETYPELIB_EX(CXTPMiniToolBar, XTPDIID_MiniToolBar)

BEGIN_INTERFACE_MAP(CXTPMiniToolBar, CXTPPopupToolBar)
	INTERFACE_PART(CXTPMiniToolBar, XTPDIID_MiniToolBar, Dispatch)
END_INTERFACE_MAP()
