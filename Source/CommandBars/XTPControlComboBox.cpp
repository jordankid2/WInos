...
CListBox* CXTPControlComboBox::GetListBoxCtrl() const
{
	CXTPControlComboBoxList* pComboBoxList = DYNAMIC_DOWNCAST(CXTPControlComboBoxList, m_pCommandBar);
	return XTP_ASSERT_CHECK(NULL != pComboBoxList) ? pComboBoxList->GetListBoxCtrl() : NULL;
}
...
BOOL CXTPControlComboBox::OnHookMouseWheel(UINT /*nFlags*/, short zDelta, CPoint /*pt*/)
{
	...
	((CXTPControlComboBoxPopupBar*)m_pCommandBar)->ProcessHookKeyDown(this, UINT(zDelta > 0 ? VK_UP : VK_DOWN), 0);
	return TRUE;
}
...
void CXTPControlComboBox::SetRect(CRect rcControl)
{
	...
	if (m_pEdit)
	{
		...
		rcControl.DeflateRect(m_nLabelWidth + XTP_DPI_X(1 + 3), XTP_DPI_Y(3), m_nThumbWidth + XTP_DPI_X(3), XTP_DPI_Y(3));
...
void CXTPControlComboBox::ShowHideEditControl()
{
	if (m_pEdit && m_pEdit->GetSafeHwnd())
	{
		m_pEdit->SetWindowPos(0, 0, 0, 0, 0, UINT(SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE
...
{
	return m_nLabelWidth + m_nThumbWidth + XTP_DPI_X(5);
...
void CXTPControlComboBox::DeflateEditRect(CRect& rcControl)
{
	if (m_nEditIconId > 0)
	{
		CXTPImageManagerIcon* pIcon = m_pParent->GetImageManager()->GetImage(XTPToUInt(m_nEditIconId), 0);
		if (pIcon)
			rcControl.left += XTP_DPI_X(pIcon->GetWidth()) + XTP_DPI_X(2);
	}
}
...
void CXTPControlComboBox::DrawEditText(CDC* pDC, CRect rcText)
{
	...
	{
		CRect rcFocus(rcText.left - XTP_DPI_X(1), rcText.top + XTP_DPI_Y(1),
			rcText.right - XTP_DPI_X(1), rcText.bottom - XTP_DPI_Y(1));
	...
	if (m_nEditIconId > 0)
	{
		...
		if (pIcon)
		{
			...
			pIcon->Draw(pDC, CPoint(rcText.left, rcText.CenterPoint().y - sz.cy / 2),
				GetEnabled() ? pIcon->GetIcon() : pIcon->GetDisabledIcon(), sz);
			rcText.left += sz.cx + XTP_DPI_X(2);
		}
	}
...
{
	CString strText;
	GetLBText(XTPToInt(lpDrawItemStruct->itemID), strText);
	...
	if (nListIconId != -1)
	{
		...
		if (pImage)
		{
			...
			pImage->Draw(pDC, CPoint(rc.left + XTP_DPI_X(2), (rc.top + rc.bottom - sz.cy) / 2),
				pImage->GetIcon(bSelected ? xtpImageHot : xtpImageNormal), sz);
			rc.left += sz.cx + XTP_DPI_X(2);
		}
	}
...
void CXTPControlComboBox::DeleteItem(long nIndex)
{
	if (nIndex < GetCount())
	{
		GetListBoxCtrl()->DeleteString(XTPToUIntChecked(nIndex));
	}
}

void CXTPControlComboBox::SetLabelWidth(int nLabelWidth)
{
...
void CXTPControlComboBox::SetThumbWidth(int nThumbWidth)
{
...
void CXTPControlComboBox::SetEditIconId(int nId)
{
...
BEGIN_INTERFACE_MAP(CXTPControlComboBox, CXTPControlPopup)
	INTERFACE_PART(CXTPControlComboBox, XTPDIID_ICommandBarComboBox, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPControlComboBox, XTPDIID_ICommandBarComboBox)
