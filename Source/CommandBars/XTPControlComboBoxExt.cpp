...
void CXTPControlSizeComboBox::EnumFontSizes(CDC& dc, LPCTSTR pFontName)
{
	...
	::EnumFontFamilies(dc.m_hDC, pFontName,
		reinterpret_cast<FONTENUMPROC>(reinterpret_cast<LPVOID>(EnumSizeCallBack)), XTPToLPARAM(this));
}
...
void CXTPControlFontComboBoxList::CFontDescHolder::EnumFontFamilies()
{
	if (m_arrayFontDesc.GetSize() == 0)
	{
		...
		BOOL bWin4 = XTPSystemVersion()->IsWinNT4OrGreater();
		CWindowDC dc(CWnd::GetDesktopWindow());
		if (bWin4)
		{
			::EnumFontFamiliesEx(dc, &lf,
				reinterpret_cast<FONTENUMPROC>(reinterpret_cast<LPVOID>(EnumFamScreenCallBackEx)), XTPToLPARAM(this), NULL);
		}
		else
		{
			::EnumFontFamilies(dc, NULL,
				reinterpret_cast<FONTENUMPROC>(reinterpret_cast<LPVOID>(EnumFamScreenCallBack)), XTPToLPARAM(this));
		}
	}
}
	...
	NEWTEXTMETRICEX* /*lpntm*/, int FontType, LPVOID pThis)
{
	...
	if (FontType & RASTER_FONTTYPE)
		return 1;
	DWORD dwData = DWORD((FontType & TRUETYPE_FONTTYPE) ? TT_FONT : 0);
...
CXTPControlFontComboBox::CXTPControlFontComboBox(DWORD dwStyleListBox)
{
	...
	((CXTPControlFontComboBoxList*)m_pCommandBar)->EnumFontFamiliesEx(0 != dwStyleListBox);

	SetWidth(XTP_DPI_X(150));
	SetDropDownWidth(XTP_DPI_X(250));
	SetDropDownListStyle();
...
{
	m_dwStyleListBox = XTPToDWORD(dwStyleListBox);

	CMapStringToPtr map;

	GetListBoxCtrl()->ResetContent();

	CXTPControlFontComboBoxList::CFontDescHolder& s_fontHolder = CXTPSingleton<CXTPControlFontComboBoxList::CFontDescHolder>::Instance();
	...
	for (i = 0; i < nCount; i++)
	{
		...
		if (nIndex >= 0) //no error
			GetListBoxCtrl()->SetItemDataPtr(nIndex, pDesc);
	}
}
...
void CXTPControlFontComboBoxList::Copy(CXTPCommandBar* pCommandBar, BOOL bRecursive)
{
	if (this != pCommandBar)
	{
		...
		EnumFontFamiliesEx(0 != m_dwStyleListBox);
	}
}
