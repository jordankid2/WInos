...
public:
	...
	void OnExecute()
	{
		...
		int nId = m_nId == XTP_ID_MENUBAR_CLOSE ? SC_CLOSE :
			m_nId == XTP_ID_MENUBAR_RESTORE ? SC_RESTORE : SC_MINIMIZE;

		::PostMessage(hWndChild, WM_SYSCOMMAND, XTPToWPARAM(nId), 0);
	}
};
...
public:
	...
	void SetMDISysMenu(HWND hWndActiveChild, HMENU hDocMenu)
	{
		...
		if (hIcon)
			m_hIcon = CXTPImageManagerIcon::ScaleToFit(hIcon, CXTPImageManagerIcon::GetExtent(hIcon), XTP_DPI_X(16));
		...
		for (UINT i = 0; i < XTPToUInt(pMenu->GetMenuItemCount()); i++)
		{
			UINT nItemID = pMenu->GetMenuItemID(XTPToInt(i));
			if (nItemID == 0)
				bSeparator = TRUE;
			else
			{
				...
				if (nItemID >= 0xF000 && nItemID < 0xF1F0)
				{
					...
					m_pCommandBar->GetControls()->Add(pControl, XTPToInt(nItemID));
					...
					pControl->SetItemDefault(::GetMenuDefaultItem(pMenu->m_hMenu, TRUE, GMDI_USEDISABLED) == i);

					pControl->SetEnabled(!(pMenu->GetMenuState(i, MF_BYPOSITION) & MF_DISABLED));
				}
				else
				{
					pControl = m_pCommandBar->GetControls()->AddMenuItem(pMenu, XTPToInt(i));
				}
	...
	void Draw(CDC* pDC)
	{
		if (m_hIcon)
		{
			pDC->DrawState(GetRect().TopLeft(), XTP_DPI(CSize(16, 16)), m_hIcon, (UINT)DSS_NORMAL, HBRUSH(0));
		}
	}
...
void CXTPMenuBar::RefreshSysButtons()
{
	...
	LONG lStyle = hWndActiveChild ? GetWindowLong(hWndActiveChild, GWL_STYLE) : 0;

	CXTPControl* pButton = m_pControls->FindControl(XTP_ID_MENUBAR_SYSMENU);
	if (bMax && (m_dwFlags & xtpFlagAddMDISysPopup))
	{
		CControlMDISysMenuPopup* pSysButton = static_cast<CControlMDISysMenuPopup*>(pButton);
		HMENU hDocMenuButton = pButton ? pSysButton->m_hDocMenu : 0;

		HMENU hDocMenu = ::GetSystemMenu(hWndActiveChild, FALSE);
		if (hDocMenu && ::IsMenu(hDocMenu))
		{
			if (hDocMenuButton != hDocMenu)
			{
				if (pButton)
				{
					pSysButton->SetMDISysMenu(hWndActiveChild, hDocMenu);
					DelayRedraw();
				}
				else
				{
	...
	if (!pButton && bMax && (lStyle & WS_MINIMIZEBOX) && (lStyle & WS_SYSMENU) && (!(m_dwFlags & xtpFlagHideMinimizeBox)))
	{
		AddSysButton(new CControlMDIButton(), XTP_ID_MENUBAR_MINIMIZE, _T("0"));
	}
	...
	if (!pButton && bMax && (lStyle & WS_MAXIMIZEBOX) && (lStyle & WS_SYSMENU) && (!(m_dwFlags & xtpFlagHideMaximizeBox)))
	{
		AddSysButton(new CControlMDIButton(), XTP_ID_MENUBAR_RESTORE, _T("2"));
	}
	else if (pButton && !bMax)
	{
		m_pControls->Remove(pButton);
	...
	if (!pButton && bMax && (lStyle & WS_SYSMENU) && (!(m_dwFlags & xtpFlagHideClose)))
	{
		AddSysButton(new CControlMDIButton(), XTP_ID_MENUBAR_CLOSE, _T("r"));
	}
	else if (pButton && !bMax)
	{
		m_pControls->Remove(pButton);
...
BEGIN_DISPATCH_MAP(CXTPMenuBarMDIMenuInfo, CXTPCmdTarget)
...
IMPLEMENT_OLETYPELIB_EX(CXTPMenuBarMDIMenuInfo, XTPDIID_IMenuBarMDIMenu)
...
BEGIN_DISPATCH_MAP(CXTPMenuBarMDIMenus, CXTPCmdTarget)
...
IMPLEMENT_OLETYPELIB_EX(CXTPMenuBarMDIMenus, XTPDIID_IMenuBarMDIMenus)
...
IMPLEMENT_OLETYPELIB_EX(CXTPMenuBar, XTPDIID_IMenuBar)

BEGIN_INTERFACE_MAP(CXTPMenuBar, CXTPToolBar)
	INTERFACE_PART(CXTPMenuBar, XTPDIID_IMenuBar, Dispatch)
END_INTERFACE_MAP()
