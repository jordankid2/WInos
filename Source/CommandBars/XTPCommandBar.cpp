...
BOOL CXTPCommandBar::SetSelected(int nSelected, BOOL bKeyboard)
{
	...
	if (m_nSelected != -1)
	{
		...
		if (!bKeyboard && m_bTrackOnHover) SetTimer(XTP_TID_HOVER, XTPToUIntChecked(s_nHoverDelay), NULL);
	}
	else
	{
...
BOOL CXTPCommandBar::SetPopuped(int nPopuped, BOOL bKeyboard)
{
	...
	if (nPopuped != -1)
	{
		...
		{
			SetTimer(XTP_TID_EXPANDED_POPUP, XTPToUIntChecked(s_nExpandDelay), NULL);
		}
...
void CXTPCommandBar::OnUpdateCmdUI()
{
	...
	{
		pControl = m_pControls->GetAt(XTPToIntChecked(state.m_nIndex));
		...
		state.m_nID = XTPToUInt(pControl->GetID());
...
void CXTPCommandBar::CustomizeDrawMarker(CDC* pDC)
{
	...
	{
		pDC->PatBlt(m_rcMarker.left, m_rcMarker.top, XTP_DPI_X(1), m_rcMarker.Height(), PATINVERT);
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(1), m_rcMarker.top + XTP_DPI_Y(1), XTP_DPI_X(1), m_rcMarker.Height() - XTP_DPI_Y(2), PATINVERT);
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(2), m_rcMarker.top + XTP_DPI_Y(2), m_rcMarker.Width() - XTP_DPI_X(4), XTP_DPI_Y(2), PATINVERT);
		pDC->PatBlt(m_rcMarker.right - XTP_DPI_X(1), m_rcMarker.top, XTP_DPI_X(1), m_rcMarker.Height(), PATINVERT);
		pDC->PatBlt(m_rcMarker.right - XTP_DPI_X(2), m_rcMarker.top + XTP_DPI_Y(1), XTP_DPI_X(1), m_rcMarker.Height() - XTP_DPI_Y(2), PATINVERT);

	}
	else
	{
		pDC->PatBlt(m_rcMarker.left, m_rcMarker.top, m_rcMarker.Width(), XTP_DPI_Y(1), PATINVERT);
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(1), m_rcMarker.top + XTP_DPI_Y(1), m_rcMarker.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), PATINVERT);
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(2), m_rcMarker.top + XTP_DPI_Y(2), XTP_DPI_X(2), m_rcMarker.Height() - XTP_DPI_Y(4), PATINVERT);
		pDC->PatBlt(m_rcMarker.left, m_rcMarker.bottom - XTP_DPI_Y(1), m_rcMarker.Width(), XTP_DPI_Y(1), PATINVERT);
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(1), m_rcMarker.bottom - XTP_DPI_Y(2), m_rcMarker.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), PATINVERT);
	}
...
void CXTPCommandBar::OnPaintManagerDestroyed(CXTPPaintManager* pPaintManager)
{
	if (m_pPaintManager == pPaintManager)
	{
		m_pPaintManager->Unsubscribe(this);
		m_pPaintManager = NULL; // must not be released at this point
	}
}
	...
	CRect& rcMarker, int& nDropIndex, BOOL& bDropAfter)
{
			...
			if (nOffset == -1 || nOffset > abs(rc.CenterPoint().y - point.y))
			{
				...
				if (bDropAfter)
				{
					if (i >= nCount - 1 || GetControl(i + 1)->GetBeginGroup())
						rcMarker.SetRect(rc.left, rc.bottom - XTP_DPI_Y(6), rc.right, rc.bottom);
					else
						rcMarker.SetRect(rc.left, rc.bottom, rc.right, rc.bottom + XTP_DPI_Y(6));
				}
				else
				{
					rcMarker.SetRect(rc.left, rc.top, rc.right, rc.top + XTP_DPI_Y(6));
			...
			if (nOffset == -1 || nOffset > abs(rc.CenterPoint().x - point.x))
			{
				...
				if (bDropAfter)
				{
					if (i >= nCount - 1 || GetControl(i + 1)->GetBeginGroup() || GetControl(i + 1)->GetWrap())
						rcMarker.SetRect(rc.right - XTP_DPI_X(6), rc.top, rc.right, rc.bottom);
					else
						rcMarker.SetRect(rc.right, rc.top, rc.right + XTP_DPI_X(6), rc.bottom);
				}
				else
				{
					rcMarker.SetRect(rc.left, rc.top, rc.left + XTP_DPI_X(6), rc.bottom);
				}
			}
		}
	...
	if (pControlMarker == NULL)
	{
		...
		{
			rcMarker.SetRect(rc.left + XTP_DPI_X(2), rc.top + XTP_DPI_Y(2), rc.left + XTP_DPI_X(8), rc.bottom - XTP_DPI_Y(2));
			bDropAfter = TRUE;
		}
		nDropIndex = -1;
		m_bVerticalMarker = FALSE;
...
LRESULT CXTPCommandBar::OnHelpHitTest(WPARAM, LPARAM lParam)
{
	...
	CXTPControl* pControl = m_pControls->HitTest(CPoint(lParam));

	if (pControl != NULL)
		return XTPToLRESULT(HID_BASE_COMMAND + pControl->GetHelpId());

	UINT nID = _AfxGetDlgCtrlID(m_hWnd);
	return XTPToLRESULT(nID != 0 ? HID_BASE_CONTROL + nID : 0);
}
...
BEGIN_DISPATCH_MAP(CXTPCommandBar, CWnd)
	...
	DISP_PROPERTY_ID(CXTPToolBar, "CustomizeDialogPresent", 36, m_bCustomizeDialogPresent, VT_BOOL)
	DISP_PROPERTY_ID(CXTPCommandBar, "TextOrientation", 40, m_nTextOrientation, VT_I4)

	DISP_FUNCTION_ID(CXTPCommandBar, "GetIconWidth",  43, OleGetIconWidth, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCommandBar, "GetIconHeight", 44, OleGetIconHeight, VT_I4, VTS_NONE)
	
END_DISPATCH_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPCommandBar, XTPDIID_ICommandBar)
...
void CXTPCommandBar::OleSetSelectedControl(LPDISPATCH lpDispatch)
{
	...
	CXTPControl* pControl = DYNAMIC_DOWNCAST(CXTPControl, CXTPControl::FromIDispatchSafe(lpDispatch));
	if (pControl)
	{
...
int CXTPCommandBar::OleGetIconWidth()
{
	return m_szIcons.cx;
}

int CXTPCommandBar::OleGetIconHeight()
{
	return m_szIcons.cy;
}
...
CXTPCommandBar* AFX_CDECL CXTPCommandBar::FromDispatch(LPDISPATCH pDisp)
{
	CXTPCommandBar* pInst = NULL;
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPCommandBar, CXTPCmdTarget::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
...
BEGIN_INTERFACE_MAP(CXTPCommandBar, CWnd)
	INTERFACE_PART(CXTPCommandBar, IID_IAccessible, ExternalAccessible)
	XTP_DECLARE_CMDTARGETPROVIDER_INTERFACE_PART(CXTPCommandBar)
#ifdef _XTP_ACTIVEX
	INTERFACE_PART(CXTPCommandBar, XTPDIID_ICommandBar, Dispatch)
#endif
END_INTERFACE_MAP()

XTP_IMPLEMENT_CMDTARGETPROVIDER_INTERFACE(CXTPCommandBar)
