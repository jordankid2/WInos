...
void CXTPControlCustom::ShowHideChildControl()
{
	...
	if (m_hwndControl)
	{
		::SetWindowPos(m_hwndControl, 0, 0, 0, 0, 0, UINT(SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE
...
BEGIN_DISPATCH_MAP(CXTPControlCustom, CXTPControl)
	DISP_PROPERTY_EX_ID(CXTPControlCustom, "Handle", dispidHandle, OleGetHandle, OleSetHandle, VT_I4)
...
{
	return (long)m_hwndControl;
}

void CXTPControlCustom::OleSetHandle(long nHandle)
{
	if (nHandle && IsWindow((HWND)nHandle))
	{
		SetControl(CWnd::FromHandle((HWND)nHandle));
	}
}
