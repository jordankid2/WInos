...
IMPLEMENT_DYNCREATE(CXTPControls, CXTPCmdTarget)
...
CXTPControl* CXTPControls::AddMenuItem(CMenu* pMenu, int nIndex)
{
	...
	CString strCaption;
	XTPResourceManager()->GetMenuLocaleString(pMenu, XTPToUIntChecked(nIndex), strCaption, MF_BYPOSITION);
	...
	if (pPopupMenu)
	{
		...
		{
			nID = XTPToUIntChecked(_ttoi(strCaption.Mid(iPos + 1)));
	...
	if (pControl == NULL)
	{
		...
		::GetMenuItemInfo(pMenu->m_hMenu, XTPToUIntChecked(nIndex), TRUE, &info);

		pControl->SetTag(info.dwItemData);
	}

	CXTPControlAction* pAction = pCommandBars->FindAction(XTPToInt(nID));
	...
	{
		if (pAction == NULL) pAction = pCommandBars->CreateAction(XTPToInt(nID));
	...
	if (pSite)
	{
		...
		pSite->SendMessage(WM_XTP_AFTERCREATECONTROL, 0, XTPToLPARAM(&cs));
	}
	...
	switch (controlType)
	{
		...
		case xtpControlButtonPopup:
		{
			...
			if (nId > 0)
			{
				...
				if (menu.LoadMenu(XTPToUInt(nId)))
				{
					pPopupMenu = menu.GetSubMenu(0);
...
long CXTPControls::GetNext(long nIndex, int nDirection, BOOL bKeyboard, BOOL bSkipTemporary, BOOL bSkipCollapsed) const
{
	...
	DWORD nSkipVisible = DWORD(xtpHideScroll | (!bSkipCollapsed ? xtpHideExpand : 0));
...
{
	int nResult = 0, x = 0;
	BOOL bVert = (0 != (dwMode & LM_VERTDOCK));
	BOOL bFirst = TRUE;
...
{
	DWORD nFlags = DWORD(bIgnoreWraps ? 0 : xtpHideWrap);
	int nCount = 0;
	for (int i = 0; i < GetCount(); i++)
	{
...
CSize CXTPControls::_CalcSmartLayoutToolBar(CDC* pDC, XTPBUTTONINFO* pData, DWORD& dwMode)
{
	...
	int nResultHeight = szLargeIcon.cy + XTP_DPI_Y(7) + m_pParent->GetPaintManager()->GetSplitDropDownHeight();
...
CSize CXTPControls::_WrapSmartLayoutToolBar(CDC* pDC, XTPBUTTONINFO* pData, int nWidth, DWORD& dwMode)
{
	...
	for (int i = 0; i < GetCount(); i++)
	{
		...
		BOOL bDrawImage = GetAt(i)->GetIconId() > 0 ?
			GetAt(i)->GetImageManager()->IsPrimaryImageExists(XTPToUInt(GetAt(i)->GetIconId()), szLargeIcon.cx) != NULL: FALSE;
...
CSize CXTPControls::CalcDynamicSize(CDC* pDC, int nLength, DWORD dwMode, const CRect& rcBorder, int nWidth)
{
	...
	{
		XTPBUTTONINFO* pData = new XTPBUTTONINFO[XTPToUIntChecked(GetCount())];
		for (int j = 0; j < GetCount(); j++)
		{
			CXTPControl* pControl = GetAt(j);
			pData[j].Attach(pDC, pControl);

			if (pControl->GetID() == XTP_ID_CUSTOMIZE_ADDORREMOVE)
			{
				rcBorders = XTP_DPI_RECT(CRect(3,3,3,3));
			}
		}
		...
		if ((m_pParent->GetFlags() & xtpFlagSmartLayout))
		{
			...
			{
				sizeResult = _WrapSmartLayoutToolBar(pDC, pData, nLength - rcBorders.top - rcBorders.bottom, dwMode);
			}
			else
			{
				sizeResult = _WrapSmartLayoutToolBar(pDC, pData, nLength - rcBorders.right - rcBorders.left, dwMode);
			}
		...
		else
		{
			...
			{
				_WrapToolBar(pData, nLength - rcBorders.right - rcBorders.left, dwMode);
			}
			else if (dwMode & LM_VERTDOCK)
			{
				_WrapToolBar(pData, nLength - rcBorders.top - rcBorders.bottom, dwMode);
			}
			else
			{
				_SizeFloatableBar(pData, nLength - ((dwMode & LM_LENGTHY) ? rcBorders.top + rcBorders.bottom : rcBorders.right + rcBorders.left), dwMode);
			}


			sizeResult = _CalcSize(pData, 0 != (dwMode & LM_VERTDOCK));
		}

		if (nWidth > 0)
		{
			if (dwMode & LM_HORZDOCK)
				sizeResult.cy = max(nWidth - rcBorders.top - rcBorders.bottom, sizeResult.cy);
			else
				sizeResult.cx = max(nWidth - rcBorders.right - rcBorders.left, sizeResult.cx);
		}

		if (dwMode & LM_STRETCH)
		{
			if (dwMode & LM_VERTDOCK)
				sizeResult.cy = max(nLength - rcBorders.top - rcBorders.bottom, sizeResult.cy);
			else
				sizeResult.cx = max(nLength - rcBorders.right - rcBorders.left, sizeResult.cx);

		}
		_AdjustBorders(pData, sizeResult, dwMode, rcBorders);

		if (dwMode & LM_COMMIT)
		{
			if (!(m_pParent->GetFlags() & xtpFlagSmartLayout))
				_MoveRightAlligned(pData, sizeResult, rcBorders, dwMode);
	...
	{
		sizeResult = XTP_DPI(CSize(23, 22)) + CSize(rcBorders.left + rcBorders.right, rcBorders.top + rcBorders.bottom);
...
CSize CXTPControls::CalcPopupSize(CDC* pDC, int nLength, int nMRUWidth, const CRect& rcBorder)
{
	...
	nWidth = max(XTP_DPI_X(24), nWidth);
	nHeight = (nHeight == 0 ? XTP_DPI_Y(22) : nHeight);
...
BEGIN_DISPATCH_MAP(CXTPControlActions, CXTPCmdTarget)
	DISP_FUNCTION_ID(CXTPControlActions, "Count", dispidCount, OleGetItemCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPControlActions, "Item", 3, OleGetItem, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPControlActions, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
	DISP_FUNCTION_ID(CXTPControlActions, "Add", dispidAdd, OleAdd, VT_DISPATCH, VTS_I4 VTS_BSTR VTS_BSTR VTS_BSTR VTS_BSTR)
	DISP_FUNCTION_ID(CXTPControlActions, "Find", DISPID_VALUE, OleFind, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPControlActions, "DeleteAll", 5, RemoveAll, VT_EMPTY, VTS_NONE)
END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPControlActions, CXTPCmdTarget)
	INTERFACE_PART(CXTPControlActions, XTPDIID_ICommandBarActions, Dispatch)
//  //INTERFACE_PART(CXTPControlActions, IID_IEnumVARIANT, EnumVARIANT)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPControlActions, XTPDIID_ICommandBarActions)

IMPLEMENT_ENUM_VARIANT(CXTPControlActions)

long CXTPControlActions::OleGetItemCount()
{
	return (long)GetCount();
}

LPDISPATCH CXTPControlActions::OleGetItem(long nIndex)
{
	if (nIndex >= 0 && nIndex < GetCount())
	{
		CXTPControlAction* pItem = GetAt(nIndex);
...
LPDISPATCH CXTPControlActions::OleFind(int nId)
{
	return XTPGetDispatch(FindAction(nId));
}

LPDISPATCH CXTPControlActions::OleAdd(int nId, LPCTSTR lpszCaption, LPCTSTR lpszTooltipText, LPCTSTR lpszDescriptionText, LPCTSTR lpszCategory)
{
	...
	pAction->SetTooltip(lpszTooltipText);
	pAction->SetDescription(lpszDescriptionText);
	pAction->SetCategory(lpszCategory);

	return XTPGetDispatch(pAction);
}
...
BEGIN_DISPATCH_MAP(CXTPControls, CXTPCmdTarget)
	DISP_FUNCTION_ID(CXTPControls, "Count", dispidCount, OleGetItemCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPControls, "Item", DISPID_VALUE, OleItem, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPControls, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
	DISP_FUNCTION_ID(CXTPControls, "Add", dispidAdd, OleAdd, VT_DISPATCH, VTS_I4 VTS_I4 VTS_BSTR VTS_VARIANT VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPControls, "Parent", dispidParent, OleParent, VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPControls, "Find", dispidFindControl, OleFindControl, VT_DISPATCH, VTS_VARIANT VTS_VARIANT VTS_VARIANT VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPControls, "DeleteAll", 5, RemoveAll, VT_EMPTY, VTS_NONE)

	DISP_FUNCTION_ID(CXTPControls, "CreateOriginalControls", 6, CreateOriginalControls, VT_EMPTY, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPControls, "OriginalControls", 7, OleGetOriginalControls, SetNotSupported, VT_DISPATCH)

	DISP_FUNCTION_ID(CXTPControls, "AddControl", 8, OleAddControl, VT_EMPTY, VTS_DISPATCH VTS_VARIANT)

END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPControls, CXTPCmdTarget)
	INTERFACE_PART(CXTPControls, XTPDIID_ICommandBarControls, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPControls, XTPDIID_ICommandBarControls)

LPDISPATCH CXTPControls::OleParent()
{
	return m_pParent ? m_pParent->GetIDispatch(TRUE) : 0;
}
long CXTPControls::OleGetItemCount()
{
	return (long)GetCount();
}

LPDISPATCH CXTPControls::OleItem(long nIndex)
{
	return OleGetItem(nIndex - 1);
}

LPDISPATCH CXTPControls::OleGetItem(long nIndex)
{
	if (nIndex >= 0 && nIndex < GetCount())
	{
...
LPDISPATCH CXTPControls::OleGetOriginalControls()
{
	return m_pOriginalControls ? m_pOriginalControls->GetIDispatch(TRUE) : NULL;
}


IMPLEMENT_ENUM_VARIANT(CXTPControls)

void CXTPControls::OleAddControl(LPDISPATCH Control, const VARIANT& varBefore)
{
	int nBefore = VariantToLong(&varBefore) - 1;
	CXTPControl* pControl = DYNAMIC_DOWNCAST(CXTPControl, CXTPControl::FromIDispatchSafe(Control));
	if (!pControl)
		return;
...
LPDISPATCH CXTPControls::OleAdd(long controlType, int nId, LPCTSTR lpszCaption, const VARIANT& varBefore, const VARIANT& varTemporary)
{
	int nBefore = VariantToLong(&varBefore) - 1;
...
LPDISPATCH CXTPControls::OleFindControl(const VARIANT& varType, const VARIANT& varId, const VARIANT& varVisible, const VARIANT& varRecursive)
{
	BOOL bVisible = VariantToBool(&varVisible, FALSE);
	BOOL bRecursive = VariantToBool(&varRecursive, FALSE);
	XTPControlType nType = (XTPControlType)VariantToLong(&varType, xtpControlError);
	int nId = VariantToLong(&varId, -1);

	CXTPControl* pControl = FindControl(nType, nId, bVisible, bRecursive);
	return pControl ? pControl->GetIDispatch(TRUE) : NULL;
}
#endif
