...
CSize CXTPVisualStudio6Theme::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar, BOOL bDraw)
{
	...
	if (pBar->GetPosition() == xtpBarRight || pBar->GetPosition() == xtpBarLeft)
	{
		if (pDC && bDraw)
		{
			...
			Draw3dRect(pDC, CRect(XTP_DPI_X(3), XTP_DPI_Y(7), rc.right - XTP_DPI_X(3), XTP_DPI_Y(10)), COLOR_BTNHILIGHT, COLOR_3DSHADOW);
		}
	...
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom)
	{
		if (pDC && bDraw)
		{
			...
			Draw3dRect(pDC, CRect(XTP_DPI_X(7), XTP_DPI_Y(3), XTP_DPI_X(10), rc.bottom - XTP_DPI_Y(3)), COLOR_BTNHILIGHT, COLOR_3DSHADOW);
		}
