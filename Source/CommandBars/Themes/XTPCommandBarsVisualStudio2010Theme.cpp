// XTPCommandBarsVisualStudio2010Theme.cpp : implementation of the CXTPVisualStudio2010Theme class.
//
...
CXTPVisualStudio2010Theme::CXTPVisualStudio2010Theme()
{
	...
	m_nToolbarSeparatorMargin = XTP_DPI_X(2);
}
...
void CXTPVisualStudio2010Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	if (barPosition == xtpBarPopup && barType == xtpBarTypePopup && bSelected && !bChecked && !bPressed)
	{
		if (m_bLunaTheme)
		{
			XTPDrawHelpers()->GradientFill(pDC, CRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1)), m_grcLunaSelected, FALSE);
		}
		else
		{
			pDC->FillSolidRect(CRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1)), GetXtremeColor(XPCOLOR_HIGHLIGHT));
		}
...
void CXTPVisualStudio2010Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition, int nID)
{
	CXTPCommandBarsOffice2003Theme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition, nID);
}

BOOL CXTPVisualStudio2010Theme::DrawRectangle(CDC* pDC, CXTPControl* pButton)
{
	return CXTPCommandBarsOffice2003Theme::DrawRectangle(pDC, pButton);
}
	...
	if (bDraw && pBar->GetType() == xtpBarTypePopup)
	{
		...
		if (!pControl->GetWrap())
			HorizontalLine(pDC, rcControl.left - XTP_DPI_X(1) + GetPopupBarGripperWidth(pBar) + m_nPopupBarTextPadding,
				rcControl.top - XTP_DPI_Y(2), rcControl.right + XTP_DPI_X(1), m_clrMenuSeparator);
		else
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(2), rcRow.top - XTP_DPI_Y(1), rcRow.bottom + XTP_DPI_Y(1), m_clrMenuSeparator);
...
void CXTPVisualStudio2010Theme::FillCommandBarEntry(CDC* pDC, CXTPCommandBar* pBar)
{
	...
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom)
	{
		...
		rcFill.left += XTP_DPI_X(2);

		pDC->FillSolidRect(rcFill.left + XTP_DPI_X(1), rcFill.top, rcFill.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), m_clrCommandBar.clrDark);
		pDC->FillSolidRect(rcFill.left, rcFill.top + XTP_DPI_Y(1), rcFill.Width(), rcFill.Height() - XTP_DPI_X(2), m_clrCommandBar.clrDark);
		pDC->FillSolidRect(rcFill.left + XTP_DPI_X(1), rcFill.bottom - XTP_DPI_Y(1), rcFill.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), m_clrCommandBar.clrDark);
	...
	else if (pBar->GetPosition() == xtpBarLeft || pBar->GetPosition() == xtpBarRight)
	{
		...
		CRect rcFill = rc;
		rcFill.top += XTP_DPI_Y(2);
		rcFill.left += XTP_DPI_X(1);

		pDC->FillSolidRect(rcFill.left + XTP_DPI_X(1), rcFill.top, rcFill.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), m_clrCommandBar.clrDark);
		pDC->FillSolidRect(rcFill.left, rcFill.top + XTP_DPI_Y(1), rcFill.Width(), rcFill.Height() - XTP_DPI_Y(2), m_clrCommandBar.clrDark);
		pDC->FillSolidRect(rcFill.left + XTP_DPI_X(1), rcFill.bottom - XTP_DPI_Y(1), rcFill.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), m_clrCommandBar.clrDark);
