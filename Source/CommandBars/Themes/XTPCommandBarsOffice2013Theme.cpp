...
void CXTPCommandBarsOffice2013Theme::RefreshMetrics()
{
    ...
    m_clrWorkspace.clrDark  .SetStandardValue(XTPIniColor(_T("CommandBars"),            _T("WorkspaceDark"),       RGB(240, 240, 240)));
    m_clrWorkspace.clrLight .SetStandardValue(XTPIniColor(_T("CommandBars"),            _T("WorkspaceLight"),      RGB(255, 255, 255)));
    m_clrGlyph              .SetStandardValue(XTPIniColor(_T("CommandBars"),            _T("Glyph"),               RGB(119, 119, 119)));
    m_clrGlyphHighlight     .SetStandardValue(XTPIniColor(_T("CommandBars"),            _T("GlyphHighlight"),      RGB( 68,  68,  68)));
    m_clrGlyphPushed        .SetStandardValue(XTPIniColor(_T("CommandBars"),            _T("GlyphPushed"),         RGB( 68,  68,  68)));
    m_clrGlyphDisabled      .SetStandardValue(XTPIniColor(_T("CommandBars"),            _T("GlyphDisabled"),       RGB(177, 177, 177)));
    m_clrEditTextNormal     .SetStandardValue(XTPIniColor(_T("CommandBars"),            _T("EditText"),            RGB(  0,   0,   0)));
    m_clrEditTextHighlighted.SetStandardValue(XTPIniColor(_T("CommandBars"),            _T("EditTextHighlight"),   RGB(  0,   0,   0)));
    m_clrEditTextDisabled   .SetStandardValue(XTPIniColor(_T("CommandBars"),            _T("EditTextDisabled"),    RGB(152, 152, 152)));
    m_clrMessageBarText     .SetStandardValue(XTPIniColor(_T("CommandBars.MessageBar"), _T("Text"),                RGB(119, 119, 119)));
	m_clrFloatingGripper    .SetStandardValue(XTPIniColor(_T("CommandBars.Toolbar"),    _T("GripperFloating"),     RGB(255, 255, 255)));
	m_clrFloatingGripperText.SetStandardValue(XTPIniColor(_T("CommandBars.Toolbar"),    _T("GripperFloatingText"), RGB( 68,  68,  68)));
	m_clrCaptionButtonFill.SetStandardValue(XTPIniColor(_T("CommandBars.Frame"), _T("CaptionButtonActiveFillColor"), RGB(119, 119, 119)));
...
BOOL CXTPCommandBarsOffice2013Theme::DrawRectangle(CDC* pDC, CXTPControl* pButton)
{
	...
	if (pButton->GetID() == XTP_ID_TOOLBAR_EXPAND)
	{
		...
		if (pButton->GetPopuped() || pButton->GetSelected())
		{
			Rectangle(pDC, pButton->GetRect(),
				bFloating? m_csToolbar.clrBorderHighlight: m_csToolbar.clrBackHighlight, m_csToolbar.clrBackHighlight);
		}
		else
		{
			Rectangle(pDC, pButton->GetRect(),
				m_csToolbar.clrToolbarBack, m_csToolbar.clrToolbarBack);
		}
		return TRUE;
	}
	else if (pButton->GetID() == XTP_ID_CUSTOMIZE_ADDORREMOVE)
	{
		...
		{
			Rectangle(pDC, pButton->GetRect(),
				m_csToolbar.clrBorderCheckedHighlight, m_csToolbar.clrBackHighlight);
		}
		else
		{
			Rectangle(pDC, pButton->GetRect(),
				m_csMenu.clrBorder, m_csMenu.clrBack);
		}
		return TRUE;
	}
...
void CXTPCommandBarsOffice2013Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition, int nID)
{
	CXTPCommandBarsModernUITheme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition, nID);
}
...
void CXTPCommandBarsOffice2013Theme::FillCommandBarEntry(CDC *pDC, CXTPCommandBar *pBar)
{
	...
	else if (pBar->GetPosition() == xtpBarPopup)
	{
		...
		if (pBarParent && (pBarParent->GetType() == xtpBarTypeMenuBar) && (pBarParent->GetPosition() != xtpBarPopup))
		{
...
CSize CXTPCommandBarsOffice2013Theme::DrawControlEdit(CDC* pDC, CXTPControlEdit* pControlEdit, BOOL bDraw)
{
	...
	if (!bEnabled)
	{
		...
		COLORREF clrBorder = IsKeyboardSelected(bSelected) ? m_csComboBox.clrBorderHighlight : m_csComboBox.clrBorderDisabled;
		pDC->Draw3dRect(rc, clrBorder, clrBorder);
	}
	else
	{
		pDC->FillSolidRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), rc.Width() - XTP_DPI_X(pControlEdit->IsSpinButtonsVisible() ? 18 : 1),
	...
	rcText.DeflateRect(pControlEdit->GetLabelWidth() + XTP_DPI_X(3), XTP_DPI_Y(1), XTP_DPI_X(3), XTP_DPI_Y(1));
	...
	if (pControlEdit->IsSpinButtonsVisible())
	{
		...
		if(bEnabled)
		{
			...
			{
				pDC->FillSolidRect(rcSpin.left, rcSpin.top, XTP_DPI_X(1), rcSpin.Height(), m_csComboBox.clrDropDownBorderDropped);
				pDC->FillSolidRect(rcSpin.left, rcBottom.top, rcSpin.Width(), XTP_DPI_Y(1), m_csComboBox.clrDropDownBorderDropped);
			}
		}
		else
		{
		...
		COLORREF clrSpinArrow = bEnabled ? m_csComboBox.clrDropDownMarker : m_csComboBox.clrDropDownMarkerDisabled;
	...
	if (pControlEdit->IsBuddyButtonVisible())
	{
		...
		DrawRectangle(pDC, rcBuddyButton, bSelected == XTP_EDITBUDDYBUTTON, bIsPressed == XTP_EDITBUDDYBUTTON, bEnabled,  FALSE, FALSE, xtpBarTypeNormal, xtpBarTop);

		CXTPImageManager* pImageManager = pControlEdit->GetParent()->GetImageManager();

		CXTPImageManagerIcon* pIcon = pImageManager->GetImage(XTPToUInt(pControlEdit->GetBuddyButtonId()));

		if (pIcon)
		{
			CSize sz(pIcon->GetExtent());
			if (IsDPIIconsScallingEnabled(pControlEdit))
			{
				sz = XTP_DPI(sz);
			}
			pIcon->Draw(pDC, CPoint((rcBuddyButton.left + rcBuddyButton.right - sz.cx) / 2, (rcBuddyButton.top + rcBuddyButton.bottom - sz.cy) / 2),
				pIcon->GetIcon(!bEnabled ? xtpImageDisabled : pControlEdit->GetSelected() == XTP_EDITBUDDYBUTTON ?
					xtpImageHot : xtpImageNormal), sz);
		}
	}
...
void CXTPCommandBarsOffice2013Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	...
	else
	{
		...
		{
			Rectangle(pDC, rc, pColorSet->clrBorderChecked, (bChecked == 2) ? pColorSet->clrBack : pColorSet->clrBackChecked);
		}
		else if (bChecked && bSelected && !bPressed)
		{
			Rectangle(pDC, rc, pColorSet->clrBorderCheckedHighlight, pColorSet->clrBackCheckedHighlight);
		}
...
void DrawDot(CDC* pDC, CPoint pt)
{
	...
	pDC->SetPixel(pt, RGB(195,195,195)); pt.Offset(0,1);
	pDC->SetPixel(pt, RGB(135,135,135)); pt.Offset(0,1);
	pDC->SetPixel(pt, RGB(195,195,195)); pt.Offset(1,-2);

	pDC->SetPixel(pt, RGB(135,135,135)); pt.Offset(0,1);
	pDC->SetPixel(pt, RGB(119,119,119)); pt.Offset(0,1);
	pDC->SetPixel(pt, RGB(135,135,135)); pt.Offset(1,-2);
...
void CXTPCommandBarsOffice2013Theme::DrawDashedLine(CDC *pDC, int x, int y, int z, COLORREF clr, BOOL bHorzBar)
{
...
CSize CXTPCommandBarsOffice2013Theme::DrawCommandBarGripper(CDC *pDC, CXTPCommandBar *pBar, BOOL bDraw)
{
	if(bDraw) ASSERT_VALID(pDC);
	ASSERT_VALID(pBar);
	...
	if (bDraw ? pBar->GetShowGripper() : TRUE)
	{
		...
		if (barPos == xtpBarRight || barPos == xtpBarLeft
			|| barPos == xtpBarTop || barPos == xtpBarBottom)
		{
			...
			if (bDraw && !pBar->IsDialogBar())
			{
				if (barPos == xtpBarRight || barPos == xtpBarLeft)
				{
					...
					ptDash.x += XTP_DPI_X(2); ptDash.y += XTP_DPI_Y(2); nDashZ -= XTP_DPI_X(1);
					DrawDashedLine(pDC, ptDash.x, ptDash.y, nDashZ, m_csToolbar.clrGripper, FALSE);
		...
		else if (barPos == xtpBarFloating)
		{
			...
			rcBar.SetRect(XTP_DPI_X(3), XTP_DPI_Y(3), rcBar.right - XTP_DPI_X(3), XTP_DPI_Y(3) + max(XTP_DPI_Y(15), sz.cy));
			if (pDC && bDraw)
			{
				pDC->SetBkMode(TRANSPARENT);
				pDC->FillSolidRect(rcBar, m_clrFloatingGripper);
				pDC->SetTextColor(m_clrFloatingGripperText);
				pDC->DrawText(pBar->GetTitle(), CRect(XTP_DPI_X(5), XTP_DPI_Y(3), rcBar.right - 2 * rcBar.Size().cy, rcBar.bottom), DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
			}
			...
			if (bDraw && !pBar->IsDialogBar() && pBar->GetType() == xtpBarTypePopup)
			{
				...
				DrawPopupBarGripper(pDC, rcBorders.left - XTP_DPI_X(1), nTop,
					GetPopupBarGripperWidth(pBar), rcBar.bottom - nTop - rcBorders.bottom);
			}
		}
		else
		{
...
CSize CXTPCommandBarsOffice2013Theme::DrawControlComboBox(CDC* pDC, CXTPControlComboBox* pControlCombo, BOOL bDraw)
{
	...
	CRect rcDropDown(rcComboBox.right - pControlCombo->GetThumbWidth(), rcComboBox.top + 1, rcComboBox.right - 1, rcComboBox.bottom - 1);
	pDC->FillSolidRect(rcDropDown, clrDropDownBackground);
	
	if (bEnabled)
		CXTPDrawHelpers::DrawLine(pDC, rcDropDown.left, rcDropDown.top, rcDropDown.left, rcDropDown.bottom, clrDropDownBorder);
...
COLORREF CXTPCommandBarsOffice2013Theme::GetGlyphColor(CXTPControl* pControl)
{
	...
	BOOL bHilite = (pControl->GetID() == XTP_ID_TOOLBAR_EXPAND || pControl->GetID() == XTP_ID_CUSTOMIZE_ADDORREMOVE);
...
COLORREF CXTPCommandBarsOffice2013Theme::GetSeparatorColor(CXTPControl* pControl, BOOL bDark)
{
	...
	if (barType == xtpBarTypeMenuBar || 
		barType == xtpBarTypePopup)
	{
		CXTPColorSetStateToolbar* pColorSet = DYNAMIC_DOWNCAST(CXTPColorSetStateToolbar, GetButtonColorSet(barType));
		if (pColorSet)
		{
			return bDark ? pColorSet->clrSeparator.clrDark : pColorSet->clrSeparator.clrLight;
		}
		CXTPColorSetStateMenu* pColorSetM = DYNAMIC_DOWNCAST(CXTPColorSetStateMenu, GetButtonColorSet(barType));
		if (pColorSetM)
		{
	...
	if (barType == xtpBarTypeNormal || 
		barType == xtpBarTypeRibbon)
	{
		CXTPColorSetStateToolbar* pColorSet = DYNAMIC_DOWNCAST(CXTPColorSetStateToolbar, GetButtonColorSet(xtpBarTypeNormal));
		if (pColorSet)
		{
			return bDark ? pColorSet->clrSeparator.clrDark : pColorSet->clrSeparator.clrLight;
		}
...
void CXTPCommandBarsOffice2013Theme::DrawControlPopupGlyph(CDC* pDC, CXTPControl* pButton)
{
	...
	rcArrow.left = rcArrow.right - (pButton->GetType() == xtpControlSplitButtonPopup
		? m_nSplitButtonPopupWidth : rcArrow.Height());
	rcArrow.DeflateRect(XTP_DPI_X(4), XTP_DPI_Y(4), XTP_DPI_X(4), XTP_DPI_Y(4));

	XTPPrimitiveSymbol nSymbol = CXTPDrawHelpers::IsContextRTL(pDC)
...
void CXTPCommandBarsOffice2013Theme::DrawDropDownGlyph(CDC* pDC, CXTPControl* pControl, CPoint pt, BOOL bSelected, BOOL bPopuped, BOOL bEnabled, BOOL bVert)
{
...
void CXTPCommandBarsOffice2013Theme::DrawPopupBarGripper(CDC* pDC, int x, int y, int cx, int cy, BOOL bExpanded)
{
	UNREFERENCED_PARAMETER(bExpanded);
	pDC->FillSolidRect(x, y, cx, cy, m_csMenu.clrGripper);
}

CSize CXTPCommandBarsOffice2013Theme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	if (!bDraw)
	{
	...
	if (pBar->GetType() == xtpBarTypePopup)
	{
		if (!pControl->GetWrap())
		{
			HorizontalLine(pDC, rcControl.left - XTP_DPI_X(1) + GetPopupBarGripperWidth(pBar) + m_nPopupBarTextPadding,
		...
		{
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(2), rcRow.top - XTP_DPI_Y(1), rcRow.bottom + XTP_DPI_Y(1), clrDark);
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(1), rcRow.top - XTP_DPI_Y(1), rcRow.bottom + XTP_DPI_Y(1), clrLight);
		}
	}
	else if (pBar->GetPosition() != xtpBarPopup && IsVerticalPosition(pBar->GetPosition()))
	{
		...
		{
			VerticalLine(pDC, rcRow.right + XTP_DPI_X(2), rcRow.top + XTP_DPI_Y(4), rcRow.bottom, clrDark);
			VerticalLine(pDC, rcRow.right + XTP_DPI_X(3), rcRow.top + XTP_DPI_Y(4), rcRow.bottom, clrLight);
		}
	}
	else
	{
		if (!pControl->GetWrap())
		{
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(4), rcRow.top + XTP_DPI_Y(2), rcRow.bottom, clrDark);
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(3), rcRow.top + XTP_DPI_Y(2), rcRow.bottom, clrLight);
		}
		else
		{
...
void CXTPCommandBarsOffice2013Theme::DrawExpandGlyph(CDC* pDC, CXTPControl* pButton, CPoint pt0, CPoint pt1, CPoint pt2, COLORREF /*clr*/)
{
...
void CXTPCommandBarsOffice2013Theme::DrawHiddenGlyph(CDC* pDC, CXTPControl* pButton, COLORREF /*clr*/, BOOL bVertical)
{
	COLORREF clrGlyph = GetGlyphColor(pButton);
	CXTPPaintManager::DrawHiddenGlyph(pDC, pButton, clrGlyph, bVertical);
}

void CXTPCommandBarsOffice2013Theme::DrawExpandGlyph(CDC* pDC, CXTPControl* pButton, COLORREF clr, BOOL bVertical)
{
	CXTPCommandBarsModernUITheme::DrawExpandGlyph(pDC, pButton, clr, bVertical);
}

void CXTPCommandBarsOffice2013Theme::DrawCheckMark(CDC* pDC, CRect rcCheck, BOOL bIsRadio, BOOL bEnabled, BOOL bSelected)
{
...
void CXTPCommandBarsOffice2013Theme::GetCheckMarkColors(COLORREF& clrMark, COLORREF& clrBack, COLORREF& clrBorder, BOOL bSelected, BOOL bPressed, BOOL bEnabled)
{
	if (!bEnabled)
	{
...
CSize CXTPCommandBarsOffice2013Theme::DrawControlCheckBoxMark(CDC* pDC, CXTPControl* pButton, CRect rc, BOOL bDraw)
{
	...
	if ((bOnQuickAccessAboveRibbon) && (m_themeCurrent != xtpThemeVisualStudio2015))		
	{
...
CSize CXTPCommandBarsOffice2013Theme::DrawControlCheckBoxMark(CDC* pDC, CRect rc, BOOL bDraw, BOOL bSelected, BOOL bPressed, BOOL bChecked, BOOL bEnabled)
{
	if (m_bThemedCheckBox && m_themeButton->IsAppThemed())
	{
		return CXTPPaintManager::DrawControlCheckBoxMark(pDC, rc, bDraw, bSelected, bPressed, bChecked, bEnabled);
...
CSize CXTPCommandBarsOffice2013Theme::DrawControlRadioButtonMark(CDC* pDC, CRect rc, BOOL bDraw, BOOL bSelected, BOOL bPressed, BOOL bChecked, BOOL bEnabled)
{
	if (m_bThemedCheckBox && m_themeButton->IsAppThemed())
	{
	...
	if (bDraw)
	{
		...
		if (bChecked)
		{
			AlphaEllipse(pDC, CRect(
				rc.left + XTP_DPI_X(4), rc.top + XTP_DPI_Y(4),
				rc.left + XTP_DPI_X(9), rc.top + XTP_DPI_Y(9)), clrMark, clrMark);
		}
	}
...
void CXTPCommandBarsOffice2013Theme::DrawSplitButtonFrame(CDC* pDC, CXTPControl* pButton, CRect rcButton)
{
	...
	else if (bSelected)
	{
		...
		{
			if (bOnQuickAccessAboveRibbon)
				pDC->FillSolidRect(rcPartA, bPressed ? m_clrQuickAccessButtonPressedBack : m_clrQuickAccessButtonHotBack);
			else
	...
	{
		if (bOnQuickAccessAboveRibbon)
			Rectangle(pDC, rcButton, m_clrQuickAccessButtonHotBack, (bChecked == 2) ? pColorSet->clrBack : m_clrQuickAccessButtonHotBack);
		else
		Rectangle(pDC, rcButton, pColorSet->clrBorderChecked, (bChecked == 2) ? pColorSet->clrBack : pColorSet->clrBackChecked);
	}
...
CXTPColorSetStateButton* CXTPCommandBarsOffice2013Theme::GetButtonColorSet(XTPBarType barType, BOOL bTextColor /*=FALSE*/)
{
	...
	case xtpBarTypeMenuBar:  // CXTPMenuBar
		pColorSet = bTextColor ? (CXTPColorSetStateButton*)&m_csMenu : (CXTPColorSetStateButton*)&m_csToolbar;
		break;
	}
...
COLORREF CXTPCommandBarsOffice2013Theme::GetControlTextColor(CXTPControl* pButton)
{
	...
	BOOL isOnTabBar = (pButton->GetFlags() & xtpFlagRibbonTabBackground) && (!pButton->GetSelected());
	if (!isOnTabBar || pButton->GetPopuped())
		return CXTPPaintManager::GetControlTextColor(pButton);
	return m_csRibbon.clrTabTextInactive;
}

COLORREF CXTPCommandBarsOffice2013Theme::GetRectangleTextColor(BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	// CXTPPopupBar.
	if (barType == xtpBarTypePopup)
	{
		return !bEnabled ? m_csMenu.clrTextDisabled : (bSelected ? m_csMenu.clrTextHighlight : m_csMenu.clrText);
	}
...
{
	BOOL isOnTabBar = (pButton->GetFlags() & xtpFlagRibbonTabBackground) && (!pButton->GetSelected());
	CXTPDrawHelpers::DrawFontText(pDC, _T("Marlett"), 80,
		pButton->GetParameter(), pButton->GetParameter().GetLength(),
		pButton->GetRect(), DT_SINGLELINE | DT_CENTER | DT_VCENTER,
		isOnTabBar? m_csRibbon.clrTabTextInactive : m_csMenu.clrText);
}

BOOL CXTPCommandBarsOffice2013Theme::SetOfficeBackImage(CXTPCommandBars* pCommandBars, XTPBackImage nBackImage /*=XTPNone*/)
{
	CXTPRibbonOffice2013Theme* pRibbonTheme =
		DYNAMIC_DOWNCAST(CXTPRibbonOffice2013Theme, m_pRibbonPaintManager);
...
BOOL AFX_CDECL XTPSetOfficeSetBackImage(CXTPCommandBars* pCommandBars, XTPBackImage nBackImage /*=XTPNone*/)
{
	if (!pCommandBars)
		return FALSE;
...
void CXTPCommandBarsOffice2013Theme::DrawFrameBorder(CDC* pDC, CRect rc, int szBorderThickness, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	UNREFERENCED_PARAMETER(barPosition);
	...
	if (!bEnabled)
	{
		...
		{
			if (!IsKeyboardSelected(bSelected))
				Draw3dRect(pDC, rc, szBorderThickness, pColorSet->clrBackDisabled, pColorSet->clrBackDisabled);
		}
		else if (IsKeyboardSelected(bSelected))
		{
			CRect rcFill(rc);
	...
	else if (bPopuped)
	{
		...
		{
			Draw3dRect(pDC, rc, szBorderThickness, pColorSet->clrBorderChecked, pColorSet->clrBackChecked);
		}
	}
	...
	else
	{
		if (bChecked && !bSelected && !bPressed)
		{
			Draw3dRect(pDC, rc, szBorderThickness, pColorSet->clrBorderChecked, (bChecked == 2) ? pColorSet->clrBack : pColorSet->clrBackChecked);
		}
		else if (bChecked && bSelected && !bPressed)
		{
			Draw3dRect(pDC, rc, szBorderThickness, pColorSet->clrBorderCheckedHighlight, pColorSet->clrBackCheckedHighlight);
		}
		else if (IsKeyboardSelected(bPressed) || (bSelected && bPressed))
		{
			Draw3dRect(pDC, rc, szBorderThickness, pColorSet->clrBorderPushed, pColorSet->clrBackPushed);
		}
		else if (bSelected || bPressed)
		{
			Draw3dRect(pDC, rc, szBorderThickness, pColorSet->clrBorderHighlight, pColorSet->clrBackHighlight);
		}
	}
}

void CXTPCommandBarsOffice2013Theme::DrawGalleryItemBack(CDC* pDC, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked, XTPBarType barType)
{
	int borderThickness = XTP_DPI_X(4);
	DrawFrameBorder(pDC, rcItem, borderThickness, bSelected, bPressed, bEnabled, bChecked, FALSE, barType, xtpBarPopup);
}
