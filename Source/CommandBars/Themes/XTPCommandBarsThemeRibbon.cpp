...
{
	return DWORD(IsCompositeRect2(pControl->GetParent(), pControl->GetRect()) ? XTP_DI_COMPOSITE : 0);
}
...
void CXTPRibbonTheme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
...
void CXTPRibbonTheme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition, int nID)
{
	CXTPResourceTheme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition, nID);
}
...
void CXTPRibbonTheme::DrawSplitButtonFrame(CDC* pDC, CXTPControl* pButton, CRect rcButton)
{
	...
	if (bSmallSize)
	{
		...
		CPoint pt = CPoint(rcButton.right - m_nSplitButtonDropDownWidth / 2 - XTP_DPI_X(1), rcButton.CenterPoint().y);
...
{
	XTPDrawHelpers()->GradientFill(pDC, CRect(CPoint(x + XTP_DPI_X(1), y), CSize(cx - XTP_DPI_X(1), cy)),
...
void CXTPRibbonTheme::FillPopupLabelEntry(CDC* pDC, CRect rc)
{
	...
	pDC->FillSolidRect(rc.left, rc.bottom - XTP_DPI_Y(1), rc.Width(), XTP_DPI_Y(1), RGB(197, 197, 197));
}
	...
	if (controlType == xtpButtonExpandToolbar)
	{
		...
		else
		{
			...
			if (pBar->IsDwmEnabled())
			{
				...
				CRect rcDest(CPoint((rc.right + rc.left - XTP_DPI_X(rcSrc.Width())) / 2, (rc.top + rc.bottom - XTP_DPI_Y(rcSrc.Height())) / 2), XTP_DPI(rcSrc.Size()));
			...
			else
			{
				...
				if (!bVert) rc.OffsetRect(-XTP_DPI_X(1), 0);
				else rc.OffsetRect(0, -XTP_DPI_Y(1));
				DrawExpandSymbols(pDC, IsVerticalPosition(pBar->GetPosition()), 
					CRect(CPoint(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1)), rc.Size()),
...
CSize CXTPRibbonTheme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	if (pBar->GetType() == xtpBarTypePopup)
	{
		...
		if (!pControl->GetWrap())
		{
			int nLeft = rcControl.left - XTP_DPI_X(1) + GetPopupBarGripperWidth(pBar) + m_nPopupBarTextPadding;
			HorizontalLine(pDC, nLeft, rcControl.top - XTP_DPI_Y(2), rcControl.right, m_clrMenuPopupSeparator);
			HorizontalLine(pDC, nLeft, rcControl.top - XTP_DPI_Y(2) + 1, rcControl.right, RGB(245, 245, 245));
		}
		else
		{
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(2), pControl->GetRowRect().top - XTP_DPI_Y(1), 
	...
	else if (bDraw && pBar->GetPosition() == xtpBarTop && pBar->IsDwmEnabled() && !pControl->GetWrap())
	{
		...
		if (pImage)
		{
			pImage->DrawImage(pDC, CRect(rcControl.left - XTP_DPI_X(4), rcRow.top + m_nToolbarSeparatorMargin + XTP_DPI_Y(1), 
				rcControl.left - XTP_DPI_X(1), rcRow.bottom - m_nToolbarSeparatorMargin + XTP_DPI_Y(1)), pImage->GetSource());
		}
		return 0;
	}
	else
...
CSize CXTPRibbonTheme::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar, BOOL bDraw)
{
	if (bDraw && pDC && pBar->GetPosition() == xtpBarTop && pBar->IsDwmEnabled())
	{
		...
		{
			if (!m_bFlatGripper) FillCompositeRect(pDC, CRect(XTP_DPI_X(7), y + XTP_DPI_Y(1), XTP_DPI_X(9), y + XTP_DPI_Y(3)), GetXtremeColor(COLOR_WINDOW));
			FillCompositeRect(pDC, CRect(XTP_DPI_X(6), y, XTP_DPI_X(8), y + XTP_DPI_Y(2)), GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
...
void CXTPRibbonTheme::DrawStatusBarGripper(CDC* pDC, CRect rcClient)
{
	...
	CRect rcSrc(0, 0, pImage->GetWidth(), pImage->GetHeight());
	CRect rcGripper(rcClient.right - XTP_DPI_X(rcSrc.Width()), rcClient.bottom - XTP_DPI_Y(rcSrc.Height()) - XTP_DPI_Y(4),
		rcClient.right, rcClient.bottom - XTP_DPI_Y(4));
...
int CXTPRibbonTheme::DrawStatusBarSwitchPane(CDC* pDC, CRect rcItem, CXTPStatusBarSwitchPane* pPane)
{
	...
	if (pDC)
	{
		for (int i = 0; i < nCount; i++)
		{
			...
			if (pIcon)
			{
				CSize sz(XTP_DPI(pIcon->GetExtent()));
				CPoint pt((rcItem.left + rcItem.right - sz.cx) / 2, (rcItem.top + rcItem.bottom - sz.cy) / 2);
				pIcon->Draw(pDC, pt, pIcon->GetIcon(!bEnabled ? xtpImageDisabled : pSwitch->bChecked ? xtpImageChecked : xtpImageNormal), sz);
			}
...
CSize CXTPRibbonTheme::DrawControlEdit(CDC* pDC, CXTPControlEdit* pControlEdit, BOOL bDraw)
{
	...
	if (bPopupBar) pDC->SetTextColor(GetXtremeColor(UINT(!pControlEdit->GetEnabled() ? XPCOLOR_MENUBAR_GRAYTEXT: bSelected ? XPCOLOR_HIGHLIGHT_TEXT : XPCOLOR_MENUBAR_TEXT)));
	else pDC->SetTextColor(GetXtremeColor(UINT(!pControlEdit->GetEnabled() ? XPCOLOR_TOOLBAR_GRAYTEXT : XPCOLOR_TOOLBAR_TEXT)));
	...
	if (pControlEdit->IsSpinButtonsVisible())
	{
		...
		CRect rcTop(rc.right - XTP_DPI_X(18), rc.top + XTP_DPI_Y(1), rc.right - XTP_DPI_X(1), rc.CenterPoint().y);
		...
		pImage = XTP_UNDPI_X(rcTop.Width()) > 12 && XTP_UNDPI_Y(rcTop.Height()) > 12 ? LoadImage(_T("CONTROLGALLERYSCROLLARROWGLYPHS")) :
		...
		pImage->DrawImage(pDC, CRect(CPoint((rcTop.left + rcTop.right - sz.cx) / 2, (rcTop.top + rcTop.bottom + XTP_DPI_Y(1) - sz.cy) / 2), sz), rcSrc, CRect(0, 0, 0, 0), 0xFF00FF);
		...
		pImage->DrawImage(pDC, CRect(CPoint((rcBottom.left + rcBottom.right - sz.cx) / 2, (rcBottom.top + rcBottom.bottom - sz.cy) / 2), sz), rcSrc, CRect(0, 0, 0, 0), 0xFF00FF);
	...
	if (pControlEdit->IsBuddyButtonVisible())
	{
		...
		if (pIcon)
		{
			...
			pIcon->Draw(pDC, CPoint((rcBuddyButton.left + rcBuddyButton.right - sz.cx) / 2, (rcBuddyButton.top + rcBuddyButton.bottom - sz.cy) / 2),
				pIcon->GetIcon(!bEnabled ? xtpImageDisabled : pControlEdit->GetSelected() == XTP_EDITBUDDYBUTTON ? xtpImageHot : xtpImageNormal), sz);
		}
	}
...
void CXTPRibbonTheme::SetRoundRectRegion(CWnd* pWnd)
{
	...
	RECT rgn[] =
	{
		{XTP_DPI_X(1), 0, cx - XTP_DPI_X(1), XTP_DPI_Y(1)},
...
void CXTPRibbonTheme::DrawControlPopupGlyph(CDC* pDC, CXTPControl* pButton)
{
	...
	CPoint pt = CPoint(rcButton.right - XTP_DPI_X(14), (rcButton.top + rcButton.bottom - XTP_DPI_Y(rcSrc.Height())) / 2);
...
void CXTPRibbonTheme::DrawPopupResizeGripper(CDC* pDC, CRect rcGripper, int nFlags)
{
	...
	if (nFlags == 2)
	{
		...
		CRect rcDest(CPoint(rcGripper.CenterPoint().x - XTP_DPI_X(rc.Width()) / 2,
			rcGripper.bottom - XTP_DPI_Y(rc.Height())), XTP_DPI(rc.Size()));
...
void CXTPRibbonTheme::DrawPopupBarCheckMark(CDC* pDC, CRect rc, BOOL bEnabled, COLORREF clr)
{
	...
	CRect rcDest(CPoint((rc.left + rc.right - XTP_DPI_X(rcSrc.Width())) / 2,
		(rc.top + rc.bottom - XTP_DPI_Y(rcSrc.Height())) / 2), XTP_DPI(rcSrc.Size()));
