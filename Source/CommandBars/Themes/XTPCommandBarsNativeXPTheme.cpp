...
void CXTPCommandBarsNativeXPTheme::RefreshMetrics()
{
	...
	COLORREF clr;
	if (m_themeToolbar->IsAppThemeActive() && SUCCEEDED(m_themeToolbar->GetThemeColor(0, 0, TMT_TEXTCOLOR, &clr)))
		m_arrColor[COLOR_BTNTEXT].SetStandardValue(clr);


	m_clrSelectedText = m_arrColor[COLOR_BTNTEXT];
	if (m_themeToolbar->IsAppThemeActive() && SUCCEEDED(m_themeToolbar->GetThemeColor(TP_BUTTON, TS_HOT, TMT_TEXTCOLOR, &clr)))
		m_clrSelectedText = clr;

	m_clrPressedText = m_arrColor[COLOR_BTNTEXT];
	if (m_themeToolbar->IsAppThemeActive() && SUCCEEDED(m_themeToolbar->GetThemeColor(TP_BUTTON, TS_PRESSED, TMT_TEXTCOLOR, &clr)))
...
void CXTPCommandBarsNativeXPTheme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	if (!IsThemeEnabled())
	{
		CXTPCommandBarsOffice2000Theme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled , bChecked , bPopuped, barType, barPosition);
		return;
	}
	...
	if (barType == xtpBarTypePopup && !bChecked)
	{
		if (bSelected || bPressed)
		{
			...
			{
				bDrawn = SUCCEEDED(m_themeMenu->DrawThemeBackground(pDC->GetSafeHdc(), XTP_MP_POPUPITEM, 2, rc, NULL));
			}
	...
	if (m_bSelectImageInPopupBar && barPosition == xtpBarPopup && barType == xtpBarTypePopup && bChecked && m_themeMenu->IsAppThemeActive())
	{
		m_themeMenu->DrawThemeBackground(pDC->GetSafeHdc(), XTP_MP_POPUPCHECKBACKGROUND, bEnabled ? 2 : 1, rc, NULL);
		return;
	}
...
void CXTPCommandBarsNativeXPTheme::DrawPopupBarRadioMark(CDC* pDC, CRect rc, BOOL bEnabled, COLORREF clr)
{
	ASSERT_VALID(pDC);

	if (m_themeMenu->IsAppThemeActive())
	{
		m_themeMenu->DrawThemeBackground(pDC->GetSafeHdc(), XTP_MP_POPUPCHECK, bEnabled ? 3 : 4, rc, NULL);
	}
	else
	{
...
void CXTPCommandBarsNativeXPTheme::DrawPopupBarCheckMark(CDC* pDC, CRect rc, BOOL bEnabled, COLORREF clr)
{
	if (m_themeMenu->IsAppThemeActive())
	{
		if (FAILED(m_themeMenu->DrawThemeBackground(pDC->GetSafeHdc(), XTP_MP_POPUPCHECK, bEnabled ? 1 : 2, rc, NULL)))
...
int CXTPCommandBarsNativeXPTheme::GetPopupBarGripperWidth(CXTPCommandBar* pBar)
{
	...
	if (m_themeMenu->IsAppThemed()) nWidth += XTP_DPI_X(3);
	return nWidth;
}
...
void CXTPCommandBarsNativeXPTheme::FillCommandBarEntry(CDC* pDC, CXTPCommandBar* pBar)
{
	...
	if (pBar->GetPosition() == xtpBarPopup)
	{
		if (m_themeMenu->IsAppThemeActive())
		{
			if (SUCCEEDED(m_themeMenu->DrawThemeBackground(pDC->GetSafeHdc(), XTP_MP_POPUPBORDERS, 0, &rc, NULL)))
			{
				rc.DeflateRect(XTP_DPI_X(3), XTP_DPI_Y(3));
				m_themeMenu->DrawThemeBackground(pDC->GetSafeHdc(), XTP_MP_POPUPBACKGROUND, 0, &rc, NULL);
				return;
			}
		}
		if (m_bFlatMenus)
	...
	else if (pBar->GetPosition() == xtpBarFloating)
	{
		...
		{
			pDC->FillSolidRect(rc, GetXtremeColor(UINT(pBar->GetType() == xtpBarTypePopup ? COLOR_MENU : COLOR_3DFACE)));
			m_themeToolbar->DrawThemeBackground(pDC->GetSafeHdc(), 0, 0, &rc, NULL);
		}

		m_themeWindow->DrawThemeBackground(pDC->GetSafeHdc(), WP_SMALLFRAMELEFT, CS_ACTIVE, CRect(rc.left, rc.top, rc.left + XTP_DPI_X(3), rc.bottom), 0);
		m_themeWindow->DrawThemeBackground(pDC->GetSafeHdc(), WP_SMALLFRAMERIGHT, CS_ACTIVE, CRect(rc.right - XTP_DPI_X(3), rc.top, rc.right, rc.bottom), 0);
		m_themeWindow->DrawThemeBackground(pDC->GetSafeHdc(), WP_SMALLFRAMEBOTTOM, CS_ACTIVE, CRect(rc.left, rc.bottom - XTP_DPI_Y(3), rc.right, rc.bottom), 0);
	}
	else
	{
...
CSize CXTPCommandBarsNativeXPTheme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	...
	if (pBar->GetType() == xtpBarTypePopup)
	{
		if (pBar->GetPosition() == xtpBarPopup && m_themeMenu->IsAppThemeActive() && !pControl->GetWrap())
		{
			m_themeMenu->DrawThemeBackground(pDC->GetSafeHdc(), XTP_MP_POPUPSEPARATOR, 0, CRect(rcControl.left + GetPopupBarGripperWidth(pControl->GetParent()), rcControl.top - 8, rcControl.right, rcControl.top - 2), NULL);
			return 0;
		}
		else
		{
	...
	{
		if (!pControl->GetWrap())
			m_themeToolbar->DrawThemeBackground(pDC->GetSafeHdc(), TP_SEPARATORVERT, 0, CRect(rcRow.left, rcControl.top - XTP_DPI_Y(6), rcRow.right, rcControl.top), NULL);
		else
			m_themeToolbar->DrawThemeBackground(pDC->GetSafeHdc(), TP_SEPARATOR, 0, CRect(rcRow.right, rcRow.top + XTP_DPI_Y(4), rcRow.right + XTP_DPI_X(5), rcRow.bottom), NULL);
	}
	else
	{
		if (!pControl->GetWrap())
			m_themeToolbar->DrawThemeBackground(pDC->GetSafeHdc(), TP_SEPARATOR, 0, CRect(rcControl.left - XTP_DPI_X(6), rcRow.top, rcControl.left, rcRow.bottom), NULL);
		else
			m_themeToolbar->DrawThemeBackground(pDC->GetSafeHdc(), TP_SEPARATORVERT, 0, CRect(rcRow.left + XTP_DPI_X(4), rcRow.top - XTP_DPI_Y(5), rcRow.right, rcRow.top), NULL);
	}
	return 0;
}

void CXTPCommandBarsNativeXPTheme::DrawPopupBarGripper(CDC* pDC, int x, int y, int cx, int cy, BOOL /*bExpanded*/)
{
	if (m_themeMenu->IsAppThemeActive())
	{
...
CSize CXTPCommandBarsNativeXPTheme::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar, BOOL bDraw)
{
	if (!IsThemeEnabled())
	{
	...
	if (pBar->GetType() == xtpBarTypePopup && pBar->GetPosition() == xtpBarPopup && m_themeMenu->IsAppThemeActive())
	{
		rc.DeflateRect(XTP_DPI_X(3), XTP_DPI_Y(3));
	...
	else if (pBar->GetPosition() == xtpBarFloating)
	{
		...
		if (pDC && bDraw)
		{
			...
			pDC->DrawText(pBar->GetTitle(), CRect(XTP_DPI_X(5), XTP_DPI_Y(3), rc.right + XTP_DPI_X(3) - 2 * rc.Size().cy, rc.bottom), DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
		}
...
void CXTPCommandBarsNativeXPTheme::AdjustExcludeRect(CRect& rc, CXTPControl* pControl, BOOL bVertical)
{
	if (!IsThemeEnabled())
	{
...
void CXTPCommandBarsNativeXPTheme::DrawSplitButtonFrame(CDC* pDC, CXTPControl* pButton, CRect rcButton)
{
	if (!IsThemeEnabled())
	{
...
void CXTPCommandBarsNativeXPTheme::DrawControlEditSpin(CDC* pDC, CXTPControlEdit* pControlEdit)
{
	...
	m_themeSpin->DrawThemeBackground(pDC->GetSafeHdc(), SPNP_UP,
		!bEnabled ? UPS_DISABLED : bSelected && bPressed == XTP_EDITSPIN_UP ? UPS_PRESSED :
		bSelected == XTP_EDITSPIN_UP ? UPS_HOT : UPS_NORMAL, rcTop, NULL);

	m_themeSpin->DrawThemeBackground(pDC->GetSafeHdc(), SPNP_DOWN,
		!bEnabled ? DNS_DISABLED : bSelected && bPressed == XTP_EDITSPIN_DOWN ? DNS_PRESSED :
		bSelected == XTP_EDITSPIN_DOWN ? DNS_HOT : DNS_NORMAL, rcBottom, NULL);
}

void CXTPCommandBarsNativeXPTheme::DrawControlEditFrame(CDC* pDC, CRect rc, BOOL bEnabled, BOOL bSelected)
{
	ASSERT_VALID(pDC);
...
void CXTPCommandBarsNativeXPTheme::DrawControlComboBoxButton(CDC* pDC, CRect rcBtn, BOOL bEnabled, BOOL bSelected, BOOL bDropped)
{
	if (!m_themeCombo->IsAppThemed())
	{
		CXTPCommandBarsOffice2000Theme::DrawControlComboBoxButton(pDC, rcBtn, bEnabled, bSelected, bDropped);
		return;
	}
...
CSize CXTPCommandBarsNativeXPTheme::DrawSpecialControl(CDC* pDC, XTPSpecialControl controlType, CXTPControl* pButton, CXTPCommandBar* pBar, BOOL bDraw, LPVOID lpParam)
{
	...
	return CXTPCommandBarsOffice2000Theme::DrawSpecialControl(pDC, controlType, pButton, pBar, bDraw, lpParam);
}

COLORREF CXTPCommandBarsNativeXPTheme::GetRectangleTextColor(BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	...
	if (!IsThemeEnabled() || barType == xtpBarTypePopup || barPosition == xtpBarPopup)
		return CXTPCommandBarsOffice2000Theme::GetRectangleTextColor(bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition);
...
void CXTPCommandBarsNativeXPTheme::DrawControlMDIButton(CDC* pDC, CXTPControlButton* pButton)
{
	...
	{
		int nPartID = pButton->GetID() == XTP_ID_MENUBAR_RESTORE ? WP_MDIRESTOREBUTTON :
			pButton->GetID() == XTP_ID_MENUBAR_MINIMIZE ? WP_MDIMINBUTTON : WP_MDICLOSEBUTTON;

		int nState = !pButton->GetEnabled() ? SBS_DISABLED : pButton->GetPressed() ? SBS_PUSHED :
			pButton->GetSelected() ? SBS_HOT : SBS_NORMAL;

		if (m_themeWindow->DrawThemeBackground(*pDC, nPartID, nState, pButton->GetRect(), NULL) == S_OK)
			return;
	}
