...
CXTPCommandBarsThemeMsMoney::CXTPCommandBarsThemeMsMoney(COLORREF clrGradientLight, COLORREF clrGradientDark, COLORREF clrLineLight, COLORREF clrLineDark)
{
...
CSize CXTPCommandBarsThemeMsMoney::DrawControlPopupParent(CDC* pDC, CXTPControl* pButton, BOOL bDraw)
{
	...
	if (bDraw)
	{
		...
		if (bChecked)
		{
			CRect rcCheck (CPoint(rcButton.left - XTP_DPI_X(2), rcButton.top + rcButton.Height()/2 - XTP_DPI_Y(10)), 
				XTP_DPI(CSize(20, 20)));
		...
		if (controlType == xtpControlPopup)
		{
			...
			CRect rc(ptCenter.x - sz.cx, ptCenter.y - sz.cy, ptCenter.x + sz.cx, ptCenter.y + sz.cy);
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayRight, rc, clrText);
		}

		rcText.SetRect(rcButton.left + XTP_DPI_X(16), rcButton.top, rcButton.right, rcButton.bottom);
		rcShortcut.SetRect(rcButton.left + XTP_DPI_X(16) + XTP_DPI_X(4), rcButton.top, rcButton.right - XTP_DPI_X(19), rcButton.bottom);

	}

	pDC->DrawText(pButton->GetCaption(), &rcText, UINT(DT_SINGLELINE | DT_VCENTER | (!bDraw ? DT_CALCRECT : DT_END_ELLIPSIS)));

	if (controlType == xtpControlButton && pButton->GetShortcutText().GetLength() != 0 )
		pDC->DrawText(pButton->GetShortcutText(), &rcShortcut, UINT(DT_SINGLELINE | DT_VCENTER | (!bDraw ? DT_CALCRECT : DT_RIGHT)));

	return CSize(XTP_DPI_X(16) + XTP_DPI_X(12) + XTP_DPI_X(32) + rcText.Width() + rcShortcut.Width(), max(XTP_DPI_Y(17), m_nTextHeight - XTP_DPI_Y(4)));
}


CSize CXTPCommandBarsThemeMsMoney::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	if (!bDraw)
	{
...
CSize CXTPCommandBarsThemeMsMoney::DrawControlToolBarParent(CDC* pDC, CXTPControl* pButton, BOOL bDraw)
{
	...
	case xtpControlButton:
		{
			...
			if (bDraw)
			{
				...
				DrawControlText(pDC, pButton, rcText, TRUE, FALSE, TRUE, controlType == xtpControlButtonPopup);

				if (pButton->GetIconId() != 0)
				{
					CPoint pt = CPoint(rcButton.CenterPoint().x - szIcon.cx / 2, rcButton.top + XTP_DPI_Y(3));
					CXTPImageManagerIcon* pImage = XTPImageManager()->GetImage(XTPToUInt(pButton->GetIconId()), szIcon.cx);
					DrawImage(pDC, pt, CSize(szIcon.cx, 0), pImage, bSelected, bPressed, bEnabled, bChecked, FALSE);
				}
			}
			else
			{
				CSize szText = DrawControlText(pDC, pButton, CXTPEmptyRect(), FALSE, FALSE, TRUE, controlType == xtpControlButtonPopup);
...
void CXTPCommandBarsThemeMsMoney::AdjustExcludeRect(CRect& rc, CXTPControl* pControl, BOOL bVertical)
{
...
{
	if (pBar->GetType() == xtpBarTypePopup) return XTP_DPI(CRect(1, 1, 1, 1));
	if (pBar->GetType() == xtpBarTypeNormal) return XTP_DPI(CRect(0, 5, 0, 5));
...
void CXTPCommandBarsThemeMsMoney::DrawImage(CDC* pDC, CPoint pt, CSize /*sz*/, CXTPImageManagerIcon* pImage, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, BOOL  )
{
	if (!bEnabled)
	{
		pImage->Draw(pDC, pt, pImage->GetIcon(xtpImageDisabled));
	}
	else if (bPopuped || bChecked)
	{
		pImage->Draw(pDC, pt, pImage->GetIcon(bChecked && (bSelected || bPressed) ? xtpImageHot : xtpImageNormal));
	}
	else
	{
		pImage->Draw(pDC, pt, pImage->GetIcon(bSelected || bPressed ? xtpImageHot : xtpImageNormal));
	}
}



CSize CXTPCommandBarsThemeMsMoney::DrawSpecialControl(CDC* pDC, XTPSpecialControl controlType, CXTPControl* pButton, CXTPCommandBar* pBar, BOOL bDraw, LPVOID lpParam)
{
	if (controlType == xtpButtonExpandToolbar)
	{
		...
		else
		{
			...
			if (!IsVerticalPosition(pBar->GetPosition()))
			{
				CRect rc(rcButton.left, rcButton.bottom - rcButton.Width(), rcButton.right, rcButton.bottom);
				XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayDown, rc, clrText);

				if (bHiddenExists)
				{
					rc.SetRect(rcButton.right - rcButton.Height(), rcButton.top, rcButton.right, rcButton.bottom);
					XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolShiftRight, rc, clrText);
				}
			} else
			{
				CRect rc(rcButton.left, rcButton.top, rcButton.left + rcButton.Height(), rcButton.bottom);
				XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayDown, rc, clrText);
