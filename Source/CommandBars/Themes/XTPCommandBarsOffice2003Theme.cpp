	...
	case xtpSystemThemeAero:
		{
			...
			static const COLORREF clrValues[] =
			{
...
void CXTPCommandBarsOffice2003Theme::DrawPopupBarGripper(CDC* pDC, int x, int y, int cx, int cy, BOOL bExpanded)
{
...
void CXTPCommandBarsOffice2003Theme::FillCommandBarEntry(CDC* pDC, CXTPCommandBar* pBar)
{
	...
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom)
	{
		...
		if (pWnd)
		{
			...
			ExcludeCorners(pDC, CRect(rc.left + XTP_DPI_X(3), rc.top + XTP_DPI_Y(1), rc.right, rc.bottom - XTP_DPI_Y(1)));
		}
		...
		pDC->FillSolidRect(rcFill.left, rcFill.bottom, rcFill.Width(), rc.Height() - rcFill.Height() - XTP_DPI_Y(2), m_clrCommandBar.clrDark);

		pDC->FillSolidRect(rc.left + XTP_DPI_X(5), rc.bottom - XTP_DPI_Y(1), rc.Width() - XTP_DPI_X(7), XTP_DPI_Y(1), m_clrToolbarShadow);
	...
	else if (pBar->GetPosition() == xtpBarLeft || pBar->GetPosition() == xtpBarRight)
	{
		...
		if (pWnd)
		{
			...
			ExcludeCorners(pDC, CRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(3), rc.right - XTP_DPI_X(1), rc.bottom));
		}
		...
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(5) , XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(7), m_clrToolbarShadow);
	...
	else if (pBar->GetPosition() == xtpBarFloating && HasFloatingBarGradientEntry(pBar))
	{
		...
		CRect rcFill(rc.left + XTP_DPI_X(2), rc.top + XTP_DPI_Y(2) + max(XTP_DPI_Y(15), sz.cy), rc.right - XTP_DPI_X(2), rc.bottom - XTP_DPI_Y(2));
...
CSize CXTPCommandBarsOffice2003Theme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	...
	if (pBar->GetPosition() == xtpBarRight || pBar->GetPosition() == xtpBarLeft)
	{
		if (!pControl->GetWrap())
		{
			Line(pDC, rcRow.left + m_nToolbarSeparatorMargin, rcControl.top - XTP_DPI_Y(4), 
				rcRow.right - m_nToolbarSeparatorMargin, rcControl.top - XTP_DPI_Y(4), XPCOLOR_SEPARATOR);
			if (!m_bPlainSeparators)
			{
				Line(pDC, rcRow.left + m_nToolbarSeparatorMargin + XTP_DPI_X(1), rcControl.top - XTP_DPI_Y(3),
					rcRow.right - m_nToolbarSeparatorMargin + XTP_DPI_X(1), rcControl.top - XTP_DPI_Y(3), COLOR_3DHIGHLIGHT);
			}
		}
		else
		{
			...
			Line(pDC, rcRow.right + XTP_DPI_X(2) + 1, rcRow.top + m_nToolbarSeparatorMargin + XTP_DPI_Y(1), 
	...
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom || bSkipWrapSeparators)
	{
		if (!pControl->GetWrap())
		{
			Line(pDC, rcControl.left - XTP_DPI_X(4), rcRow.top + m_nToolbarSeparatorMargin, 
				rcControl.left - XTP_DPI_X(4), rcRow.bottom - m_nToolbarSeparatorMargin, XPCOLOR_SEPARATOR);
			if (!m_bPlainSeparators)
			{
				Line(pDC, rcControl.left - XTP_DPI_X(4) + 1, rcRow.top + m_nToolbarSeparatorMargin + XTP_DPI_Y(1),
					rcControl.left - XTP_DPI_X(4) + 1, rcRow.bottom - m_nToolbarSeparatorMargin + XTP_DPI_Y(1), COLOR_3DHIGHLIGHT);
			}
		}
		else if (!bSkipWrapSeparators)
		{
			Line(pDC, rcRow.left + m_nToolbarSeparatorMargin, rcRow.top - XTP_DPI_Y(4), 
				rcRow.right, rcRow.top - XTP_DPI_Y(4), XPCOLOR_SEPARATOR);
			Line(pDC, rcRow.left + m_nToolbarSeparatorMargin + XTP_DPI_X(1), rcRow.top - XTP_DPI_Y(4) + 1, 
				rcRow.right, rcRow.top - XTP_DPI_Y(4) + 1, COLOR_3DHIGHLIGHT);
		}
	}
	else return CXTPCommandBarsOfficeXPTheme::DrawCommandBarSeparator(pDC, pBar, pControl, bDraw);

	return 0;
}

CSize CXTPCommandBarsOffice2003Theme::DrawDialogBarGripper(CDC* pDC, CXTPDialogBar* pBar, BOOL bDraw)
{
	...
	if (pDC && bDraw)
	{
		...
		for (int y = rcGripper.top + XTP_DPI_Y(4); y < rcGripper.bottom - XTP_DPI_Y(5); y += XTP_DPI_Y(4))
		{
			pDC->FillSolidRect(CRect(XTP_DPI_X(4 + 2), y + XTP_DPI_Y(1), XTP_DPI_X(6 + 2), y + XTP_DPI_Y(3)), GetXtremeColor(COLOR_WINDOW));
			pDC->FillSolidRect(CRect(XTP_DPI_X(3 + 2), y, XTP_DPI_X(5 + 2), y + XTP_DPI_Y(2)), GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
	}
...
CSize CXTPCommandBarsOffice2003Theme::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar, BOOL bDraw)
{
	...
	{
		if (!pDC || !bDraw) return CSize(0, XTP_DPI_Y(8) + XTP_DPI_Y(1));
		...
		{
			if (!m_bFlatGripper) pDC->FillSolidRect(x + szGripper05.cx, y2, szGripper.cx, szGripper.cy, GetXtremeColor(COLOR_WINDOW));
			pDC->FillSolidRect(x, y1, szGripper.cx, szGripper.cy, GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
	}
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom)
	{
		if (!pDC || !bDraw) return CSize(XTP_DPI_X(6), 0);
		...
		{
			if (!m_bFlatGripper) pDC->FillSolidRect(x2, y + szGripper05.cy, szGripper.cx, szGripper.cy, GetXtremeColor(COLOR_WINDOW));
			pDC->FillSolidRect(x1, y, szGripper.cx, szGripper.cy, GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
	}
...
void CXTPCommandBarsOffice2003Theme::RectangleEx(CDC* pDC, CRect rc, int nColorBorder, BOOL bHoriz, CXTPPaintManagerColorGradient& color)
{
	GradientFill(pDC, rc, color.clrLight, color.clrDark, bHoriz);
	Draw3dRect(pDC, rc, nColorBorder, nColorBorder);
}

void CXTPCommandBarsOffice2003Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	if (bPopuped)
	{
	...
	{
		CXTPCommandBarsOfficeXPTheme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition);
	}
}

void CXTPCommandBarsOffice2003Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition, int nID)
{
	CXTPCommandBarsOfficeXPTheme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition, nID);
}
...
void CXTPCommandBarsOffice2003Theme::DrawExpandSymbols(CDC* pDC, BOOL bVertical, CRect rc, BOOL bHiddenExists, COLORREF clr)
{
...
CSize CXTPCommandBarsOffice2003Theme::DrawSpecialControl(CDC* pDC, XTPSpecialControl controlType, CXTPControl* pButton, CXTPCommandBar* pBar, BOOL bDraw, LPVOID lpParam)
{
	if (controlType == xtpButtonExpandToolbar)
	{
		if (!bDraw)
		{
			...
			if (!IsVerticalPosition(pBar->GetPosition()))
				pButton->SetRect(CRect(szBar.cx - XTP_DPI_X(11) - XTP_DPI_X(1), XTP_DPI_Y(1), szBar.cx - 0, szBar.cy - 0));
			else
				pButton->SetRect(CRect(0, szBar.cy - XTP_DPI_Y(11) - XTP_DPI_Y(1), szBar.cx - 0, szBar.cy - 0));
		}
		else
		{
			...
			if (!IsVerticalPosition(pBar->GetPosition()))
			{
				pDC->ExcludeClipRect(rc.right - XTP_DPI_X(1), rc.top, rc.right, rc.top + XTP_DPI_Y(1));
				pDC->ExcludeClipRect(rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1), rc.right, rc.bottom);
				pDC->ExcludeClipRect(rc.left, rc.top + XTP_DPI_Y(1), rc.left + XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1));
			}
			else
			{
				pDC->ExcludeClipRect(rc.left, rc.bottom - XTP_DPI_Y(1), rc.left + XTP_DPI_X(1), rc.bottom);
				pDC->ExcludeClipRect(rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1), rc.right, rc.bottom);
				pDC->ExcludeClipRect(rc.left + XTP_DPI_X(1), rc.top, rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(1));
			}
			...
			COLORREF clrText = GetXtremeColor(UINT(pButton->GetPopuped() ? XPCOLOR_TOOLBAR_TEXT: pButton->GetSelected() ? XPCOLOR_HIGHLIGHT_TEXT : XPCOLOR_TOOLBAR_TEXT));
			BOOL bHiddenExists = *(BOOL*)lpParam;

			DrawExpandSymbols(pDC, IsVerticalPosition(pBar->GetPosition()), CRect(CPoint(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1)), rc.Size()) , bHiddenExists, GetXtremeColor(COLOR_BTNHIGHLIGHT));
	...
	return CXTPCommandBarsOfficeXPTheme::DrawSpecialControl(pDC, controlType, pButton, pBar, bDraw, lpParam);
}
...
BOOL CXTPCommandBarsOffice2003Theme::LoadModernToolbarIcons(UINT nIDResource, UINT* pCommands, int nCount, CSize szIcon)
{
	HBITMAP hbm = GetModernBitmap(nIDResource);
	if (!hbm)
		return FALSE;
...
CRect CXTPCommandBarsOffice2003Theme::GetCommandBarBorders(CXTPCommandBar* pBar)
{
	...
	else return CXTPCommandBarsOfficeXPTheme::GetCommandBarBorders(pBar);
}

CSize CXTPCommandBarsOffice2003Theme::DrawTearOffGripper(CDC* pDC, CRect rcGripper, BOOL bSelected, BOOL bDraw)
{
	if (bDraw)
	{
		...
		{
			pDC->FillSolidRect(CRect(x + XTP_DPI_X(1), XTP_DPI_Y(4 + 1), x + XTP_DPI_X(3), XTP_DPI_Y(6 + 1)), GetXtremeColor(COLOR_BTNHIGHLIGHT));
			pDC->FillSolidRect(CRect(x, XTP_DPI_Y(3 + 1), x + XTP_DPI_X(2), XTP_DPI_Y(5 + 1)), GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
	}
