...
void CXTPCommandBarsOffice2000Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	if (m_bSelectImageInPopupBar && barPosition == xtpBarPopup && barType == xtpBarTypePopup && bChecked)
	{
...
void CXTPCommandBarsOffice2000Theme::DrawImage(CDC* pDC, CPoint pt, CSize sz, CXTPImageManagerIcon* pImage, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, BOOL)
{
	if (!bEnabled)
	{
		if (!m_iconsInfo.bUseDisabledIcons && !pImage->IsAlpha())
		{
			pImage->DrawMono(pDC, pt + XTP_DPI(CPoint(1, 1)), pImage->GetIcon(), sz, GetXtremeColor(COLOR_3DHILIGHT));
	...
	else if ((bSelected && !bPressed) || (!bSelected && bPressed == TRUE))
	{
		...
		pImage->Draw(pDC, CPoint(pt.x + XTP_DPI_X(1), pt.y + XTP_DPI_Y(1)), pImage->GetPressedIcon(), sz);
}
...
void CXTPCommandBarsOffice2000Theme::DrawControlEditSpin(CDC* pDC, CXTPControlEdit* pControlEdit)
{
	...
	CRect rcTop(rcSpin.left + XTP_DPI_X(1), rcSpin.top + XTP_DPI_Y(1), rcSpin.right - XTP_DPI_X(1), rcSpin.CenterPoint().y);
	CRect rcBottom(rcSpin.left + XTP_DPI_X(1), rcTop.bottom, rcSpin.right - XTP_DPI_X(1), rcSpin.bottom - XTP_DPI_Y(1));
...
CSize CXTPCommandBarsOffice2000Theme::DrawControlEdit(CDC* pDC, CXTPControlEdit* pControlEdit, BOOL bDraw)
{
	if (!bDraw)
	{
...
void CXTPCommandBarsOffice2000Theme::DrawControlEditFrame(CDC* pDC, CRect rc, BOOL bEnabled, BOOL bSelected)
{
	if (!bEnabled)
	{
		rc.DeflateRect(1, 1);
...
void CXTPCommandBarsOffice2000Theme::DrawControlComboBoxButton(CDC* pDC, CRect rcBtn, BOOL bEnabled, BOOL bSelected, BOOL bDropped)
{
	...
	if (bEnabled)
	{
		...
		if (bDropped)
		{
			pDC->FillSolidRect(rcBtn.left - XTP_DPI_X(1), rcBtn.top, XTP_DPI_X(1), rcBtn.Height(), GetXtremeColor(COLOR_3DFACE));
...
CSize CXTPCommandBarsOffice2000Theme::DrawControlComboBox(CDC* pDC, CXTPControlComboBox* pControlCombo, BOOL bDraw)
{
	if (!bDraw)
	{
	...
	CRect rcText(rc.left + XTP_DPI_X(3), rc.top + XTP_DPI_Y(1), rc.right - rcBtn.Width() + XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1));
...
CSize CXTPCommandBarsOffice2000Theme::DrawSpecialControl(CDC* pDC, XTPSpecialControl controlType, CXTPControl* pButton, CXTPCommandBar* pBar, BOOL bDraw, LPVOID lpParam)
{
...
CSize CXTPCommandBarsOffice2000Theme::GetPopupBarImageSize(CXTPCommandBar* pBar)
{
	...
	CSize szImage(max(szIcon.cx + XTP_DPI_X(4), m_nTextHeight), max(szIcon.cy + XTP_DPI_Y(4), m_nTextHeight));
	return szImage;
}

CSize CXTPCommandBarsOffice2000Theme::DrawControlPopupParent(CDC* pDC, CXTPControl* pButton, BOOL bDraw)
{
	...
	if (bDraw)
	{
		...
		CRect rcCheck (CPoint(rcImageRect.left + rcImageRect.Width()/2 - XTP_DPI_X(10), rcImageRect.top + rcImageRect.Height()/2 - XTP_DPI_Y(10)), XTP_DPI(CSize(20, 20)));
		...
		if (pButton->GetExpanded())
		{
			...
			{
				DrawPopupBarGripper(pDC, rcButton.left - XTP_DPI_X(1), rcButton.top, nGripperWidth + XTP_DPI_X(1), rcButton.Height(), TRUE);
			}

		}


		COLORREF clrText = GetRectangleTextColor(bSelected, FALSE, bEnabled, FALSE, FALSE, xtpBarTypePopup, xtpBarPopup);
		...
		if (bChecked)
		{
			DrawRectangle(pDC, rcImageRect, bSelected, FALSE, bEnabled, bChecked, FALSE, xtpBarTypePopup, xtpBarPopup);
		...
		if (bChecked && (bDoubleGripper || !pImage))
		{
			...
			if (controlType != xtpControlRadioButton)
				DrawPopupBarCheckMark(pDC, rcCheck, bEnabled, GetXtremeColor(UINT(!bEnabled ? COLOR_GRAYTEXT : m_nPopupBarText)));
			else
				DrawPopupBarRadioMark(pDC, rcCheck, bEnabled, GetXtremeColor(UINT(!bEnabled ? COLOR_GRAYTEXT : m_nPopupBarText)));
		}
		...
		if (IsPopupControlType(controlType))
		{
			...
			if (pButton->GetType() == xtpControlSplitButtonPopup)
			{
				if (!bPopuped || !bSelected || !bEnabled)
				{
					...
					VerticalLine(pDC, rcButton.right - XTP_DPI_X(20), rcButton.top + XTP_DPI_Y(1), rcButton.bottom - XTP_DPI_Y(1), !bEnabled ? clrText : GetXtremeColor(XTPToUIntChecked(bSelected ? COLOR_3DSHADOW : m_nPopupBarText)));
				}
				else
				{
					rcSelection.right -= XTP_DPI_X(20);
					Draw3dRect(pDC, CRect(rcButton.right - XTP_DPI_X(20), rcButton.top, rcButton.right, rcButton.bottom), COLOR_3DSHADOW, COLOR_BTNHILIGHT);
					if (bSelected && !m_bSelectImageInPopupBar) DrawRectangle(pDC, rcSelection, bSelected, FALSE, TRUE, FALSE, FALSE, xtpBarTypePopup, xtpBarPopup);
				}

				COLORREF clr = m_bSelectImageInPopupBar || !bPopuped || !bSelected ? clrText : GetXtremeColor(XTPToUIntChecked(m_nPopupBarText));
...
COLORREF CXTPCommandBarsOffice2000Theme::GetRectangleTextColor(BOOL bSelected, BOOL /*bPressed*/, BOOL bEnabled, BOOL bChecked, BOOL /*bPopuped*/, XTPBarType barType, XTPBarPosition barPosition)
{
	if (barType == xtpBarTypePopup)
	{
		return GetXtremeColor(XTPToUIntChecked(!bEnabled ? COLOR_GRAYTEXT: bSelected && !bChecked ? COLOR_HIGHLIGHTTEXT : m_nPopupBarText));
	}

	UINT nColorFace = barPosition == xtpBarPopup ? XTPToUIntChecked(m_nPopupBarText) : COLOR_BTNTEXT;
...
void CXTPCommandBarsOffice2000Theme::DrawSplitButtonFrame(CDC* pDC, CXTPControl* pButton, CRect rcButton)
{
	...
	DrawRectangle(pDC, CRect(rcButton.left, rcButton.top, rcButton.right - XTP_DPI_X(12), rcButton.bottom),
		bSelected || bPopuped, bPressed, bEnabled, bChecked, FALSE, xtpBarTypeNormal, pButton->GetParent()->GetPosition());

	CPoint pt = CPoint(rcButton.right - m_nSplitButtonDropDownWidth / 2, rcButton.CenterPoint().y);

	if ((bSelected || bPressed || bPopuped) && bEnabled)
		DrawRectangle(pDC, CRect(rcButton.right - m_nSplitButtonDropDownWidth, rcButton.top, rcButton.right, rcButton.bottom),
		TRUE, FALSE, bEnabled, FALSE, bPopuped, xtpBarTypeNormal, pButton->GetParent()->GetPosition());
...
CSize CXTPCommandBarsOffice2000Theme::DrawDialogBarGripper(CDC* pDC, CXTPDialogBar* /*pBar*/, BOOL bDraw)
{
	CSize sz(XTP_DPI_X(6), max(XTP_DPI_Y(22), m_nTextHeight + XTP_DPI_Y(4)));

	if (pDC && bDraw)
	{
		Draw3dRect(pDC, CRect(XTP_DPI_X(3), XTP_DPI_Y(3), XTP_DPI_Y(6), sz.cy), COLOR_BTNHILIGHT, COLOR_3DSHADOW);
	}
...
CSize CXTPCommandBarsOffice2000Theme::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar, BOOL bDraw)
{
	if (pBar->IsDialogBar())
	...
	{
		if (pDC && bDraw)
			Draw3dRect(pDC, CRect(XTP_DPI_X(3), XTP_DPI_Y(3), rc.right - XTP_DPI_X(3), XTP_DPI_Y(6)), COLOR_BTNHILIGHT, COLOR_3DSHADOW);
		return CSize(0, XTP_DPI_Y(8));
	}
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom)
	{
		if (pDC && bDraw)
			Draw3dRect(pDC, CRect(XTP_DPI_X(3), XTP_DPI_Y(3), XTP_DPI_X(6), rc.bottom - XTP_DPI_Y(3)), COLOR_BTNHILIGHT, COLOR_3DSHADOW);
		return CSize(XTP_DPI_X(6), 0);
	}
	return 0;
}

CSize CXTPCommandBarsOffice2000Theme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	if (!bDraw)
	{
	...
	{
		if (!pControl->GetWrap())
			Draw3dRect(pDC, CRect(rcControl.left + XTP_DPI_X(11), rcControl.top - XTP_DPI_Y(5), rcControl.right - XTP_DPI_X(11), rcControl.top - XTP_DPI_Y(3)), COLOR_3DSHADOW, COLOR_BTNHILIGHT);
		else
			Draw3dRect(pDC, CRect(rcControl.left - XTP_DPI_X(5), rcRow.top + XTP_DPI_Y(3), rcControl.left - XTP_DPI_X(3), rcRow.bottom - XTP_DPI_Y(3)), COLOR_3DSHADOW, COLOR_BTNHILIGHT);
	}
	else if (pBar->GetPosition() != xtpBarPopup && IsVerticalPosition(pBar->GetPosition()))
	{
		if (!pControl->GetWrap())
			Draw3dRect(pDC, CRect(rcRow.left, rcControl.top - XTP_DPI_Y(4), rcRow.right, rcControl.top - XTP_DPI_Y(2)), COLOR_3DSHADOW, COLOR_BTNHILIGHT);
		else
			Draw3dRect(pDC, CRect(rcRow.right + XTP_DPI_X(2), rcRow.top + XTP_DPI_Y(4), rcRow.right + XTP_DPI_X(4), rcRow.bottom), COLOR_3DSHADOW, COLOR_BTNHILIGHT);
	}
	else
	{
		if (!pControl->GetWrap())
			Draw3dRect(pDC, CRect(rcControl.left - XTP_DPI_X(4), rcRow.top, rcControl.left - XTP_DPI_X(2), rcRow.bottom), COLOR_3DSHADOW, COLOR_BTNHILIGHT);
		else
			Draw3dRect(pDC, CRect(rcRow.left + XTP_DPI_X(4), rcRow.top - XTP_DPI_Y(4), rcRow.right, rcRow.top - XTP_DPI_Y(2)), COLOR_3DSHADOW, COLOR_BTNHILIGHT);
	}
	return 0;
}

void CXTPCommandBarsOffice2000Theme::AdjustExcludeRect(CRect& rc, CXTPControl* pControl, BOOL bVertical)
{
...
CSize CXTPCommandBarsOffice2000Theme::DrawListBoxControl(CDC* pDC, CXTPControl* pButton, CRect rcButton, BOOL bSelected, BOOL bDraw, CXTPCommandBars* pCommandBars)
{
	...
	if (bDraw)
	{
		...
		CRect rcSelection(rcButton.left + XTP_DPI_X(1), rcButton.top, rcButton.right - XTP_DPI_X(1), rcButton.bottom);
		XTPControlType controlType = pButton->GetType();
		XTPButtonStyle buttonStyle = pButton->GetStyle();
		...
		CRect rcText(rcButton.left + szImage.cx + XTP_DPI_X(4), rcButton.top, rcButton.right, rcButton.bottom);
		...
		{
			Line(pDC, rcButton.right - XTP_DPI_X(20), rcButton.top + XTP_DPI_Y(1), rcButton.right - XTP_DPI_X(20), rcButton.bottom - XTP_DPI_Y(1), bSelected ? COLOR_HIGHLIGHTTEXT : COLOR_BTNTEXT);
		}
		...
		if (controlType == xtpControlComboBox || controlType == xtpControlEdit)
		{
			...
			CRect rcCombo(rcButton.left + szImage.cx + XTP_DPI_X(4) + rcText.Width() + XTP_DPI_X(5), rcButton.top, rcButton.right, rcButton.bottom);
...
CSize CXTPCommandBarsOffice2000Theme::DrawTearOffGripper(CDC* pDC, CRect rcGripper, BOOL bSelected, BOOL bDraw)
{
	if (bDraw)
	{
		CRect rc(rcGripper.left, rcGripper.top, rcGripper.right, rcGripper.bottom - XTP_DPI_Y(1));
		pDC->FillSolidRect(rc, GetXtremeColor(UINT(bSelected ? COLOR_ACTIVECAPTION : COLOR_INACTIVECAPTION)));
	}
...
void CXTPCommandBarsOffice2000Theme::DrawPopupResizeGripper(CDC* pDC, CRect rcGripper, int nFlags)
{
	...
	else
	{
		...
		x-= 4;
		pDC->FillSolidRect(CRect(x + XTP_DPI_X(1), XTP_DPI_X(1) + y, x + XTP_DPI_X(3), XTP_DPI_Y(3) + y), GetXtremeColor(COLOR_WINDOW));
		pDC->FillSolidRect(CRect(x, 0 + y, x + XTP_DPI_X(2), XTP_DPI_Y(2) + y), GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
