...
// XTPCommandBarsResourceTheme.cpp : implementation of the CXTPResourceTheme class.
//
...
#include "CommandBars/Gallery/XTPControlGalleryResourceTheme.h"
...
void CXTPResourceTheme::RefreshMetrics()
{
	...
	m_nMessageBarHeight = XTP_DPI_Y(GetImages()->GetImageInt(_T("MessageBar"), _T("MessageBarHeight")));
	m_szMessageBarDefaultButtonSize = XTP_DPI(GetImages()->GetImageSize(_T("MessageBar"), _T("MessageBarDefaultButtonSize")));
...
void CXTPResourceTheme::FillWorkspace(CDC* pDC, CRect rc, CRect rcExclude)
{
	...
	if (NULL != pImage)
	{
		...
		CRect rcTopLeft(rc);
		rcTopLeft.bottom = rcTopLeft.top + XTP_DPI_Y(rcSrc.Height());
		rcTopLeft.right = rcTopLeft.left + max(rcTopLeft.Width(), XTP_DPI_X(rcSrc.Width()));

		pImage->DrawImage(pDC, rcTopLeft, rcSrc, CRect(rcSrc.Width() - 1, 0, 0, 0));

		CRect rcFill(rc.left, rc.top + XTP_DPI_Y(rcSrc.Height()), rc.right, rc.bottom);
