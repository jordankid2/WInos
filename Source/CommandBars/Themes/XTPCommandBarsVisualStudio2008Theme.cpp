// XTPCommandBarsVisualStudio2008Theme.cpp : implementation of the CXTPVisualStudio2008Theme class.
//
...
{
	m_nPopupBarTextPadding = XTP_DPI_X(10);
	...
	case xtpSystemThemeAero:
		{
			...
			m_pStatusBarPaintManager->m_clrBackground = RGB(160,160,180);
...
void CXTPVisualStudio2008Theme::DrawPopupBarGripper(CDC* pDC, int x, int y, int cx, int cy, BOOL bExpanded)
{
	...
	pDC->FillSolidRect(x + cx, y, XTP_DPI_X(1), cy, GetXtremeColor(XPCOLOR_SEPARATOR));
	pDC->FillSolidRect(x + cx + XTP_DPI_X(1), y, XTP_DPI_X(1), cy, GetXtremeColor(COLOR_3DHILIGHT));
}
...
{
	pDC->FillSolidRect(rc.left + XTP_DPI_X(3), rc.top, rc.Width() - XTP_DPI_X(6), XTP_DPI_Y(1), clrBorder);
	pDC->FillSolidRect(rc.left + XTP_DPI_X(3), rc.bottom - XTP_DPI_Y(1), rc.Width() - XTP_DPI_X(6), XTP_DPI_Y(1), clrBorder);
	pDC->FillSolidRect(rc.left, rc.top + XTP_DPI_Y(3), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(6), clrBorder);
	pDC->FillSolidRect(rc.right - XTP_DPI_X(1) , rc.top + XTP_DPI_Y(3), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(6), clrBorder);

	AlphaPixel(pDC, rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), clrBorder, 255);
	AlphaPixel(pDC, rc.left + XTP_DPI_X(1), rc.top, clrBorder, 60);
	AlphaPixel(pDC, rc.left + XTP_DPI_X(2), rc.top, clrBorder, 120);
	AlphaPixel(pDC, rc.left, rc.top + XTP_DPI_Y(1), clrBorder, 60);
	AlphaPixel(pDC, rc.left, rc.top + XTP_DPI_Y(2), clrBorder, 120);

	AlphaPixel(pDC, rc.right - XTP_DPI_X(2), rc.top + XTP_DPI_Y(1), clrBorder, 255);
	AlphaPixel(pDC, rc.right - XTP_DPI_X(3), rc.top, clrBorder, 120);
	AlphaPixel(pDC, rc.right - XTP_DPI_X(2), rc.top, clrBorder, 60);
	AlphaPixel(pDC, rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), clrBorder, 60);
	AlphaPixel(pDC, rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(2), clrBorder, 120);

	AlphaPixel(pDC, rc.left + XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(2), clrBorder, 255);
	AlphaPixel(pDC, rc.left + XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1), clrBorder, 60);
	AlphaPixel(pDC, rc.left + XTP_DPI_X(2), rc.bottom - XTP_DPI_Y(1), clrBorder, 120);
	AlphaPixel(pDC, rc.left, rc.bottom - XTP_DPI_Y(2), clrBorder, 60);
	AlphaPixel(pDC, rc.left, rc.bottom - XTP_DPI_Y(3), clrBorder, 120);


	AlphaPixel(pDC, rc.right - XTP_DPI_X(2), rc.bottom - XTP_DPI_Y(2), clrBorder, 255);
	AlphaPixel(pDC, rc.right - XTP_DPI_X(3), rc.bottom - XTP_DPI_Y(1), clrBorder, 120);
	AlphaPixel(pDC, rc.right - XTP_DPI_X(2), rc.bottom - XTP_DPI_Y(1), clrBorder, 60);
	AlphaPixel(pDC, rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(2), clrBorder, 60);
	AlphaPixel(pDC, rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(3), clrBorder, 120);
}
...
void CXTPVisualStudio2008Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	...
	{
		XTPDrawHelpers()->GradientFill(pDC, CRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1)), m_clrMenuSelection, FALSE);
	...
	if (barType == xtpBarTypeMenuBar && bSelected && !bChecked && !bPressed && bEnabled)
	{
		...
		pDC->FillSolidRect(rc.left + XTP_DPI_X(1), rc.top, rc.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), clrShadow);
		pDC->FillSolidRect(rc.left + XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1), rc.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), clrShadow);
		pDC->FillSolidRect(rc.left, rc.top + XTP_DPI_Y(1), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(2), clrShadow);
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(2), clrShadow);
		rc.DeflateRect(1, 1);
		...
		pDC->SetPixel(rc.left, rc.bottom - XTP_DPI_Y(1), clrPixel);
		pDC->SetPixel(rc.right - XTP_DPI_X(1), rc.top, clrPixel);
		pDC->SetPixel(rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1), clrPixel);
...
void CXTPVisualStudio2008Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition, int nID)
{
	CXTPVisualStudio2005Theme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition, nID);
}

BOOL CXTPVisualStudio2008Theme::DrawRectangle(CDC* pDC, CXTPControl* pButton)
{
	return CXTPVisualStudio2005Theme::DrawRectangle(pDC, pButton);
}
...
CSize CXTPVisualStudio2008Theme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	...
	if (!pControl->GetWrap())
	{
		HorizontalLine(pDC, rcControl.left - XTP_DPI_X(1) + GetPopupBarGripperWidth(pBar) + m_nPopupBarTextPadding, rcControl.top - XTP_DPI_Y(2), rcControl.right + XTP_DPI_X(1), GetXtremeColor(XPCOLOR_SEPARATOR));
		HorizontalLine(pDC, rcControl.left - XTP_DPI_X(1) + GetPopupBarGripperWidth(pBar) + m_nPopupBarTextPadding, rcControl.top - XTP_DPI_Y(1), rcControl.right + XTP_DPI_X(1), GetXtremeColor(COLOR_3DHILIGHT));
	}
	else
	{
		VerticalLine(pDC, rcControl.left - XTP_DPI_X(2), rcRow.top - XTP_DPI_Y(1), rcRow.bottom + XTP_DPI_X(1), GetXtremeColor(XPCOLOR_SEPARATOR));
		VerticalLine(pDC, rcControl.left - XTP_DPI_X(1), rcRow.top - XTP_DPI_Y(1), rcRow.bottom + XTP_DPI_X(1), GetXtremeColor(COLOR_3DHILIGHT));
	}
