...
void CXTPCommandBarsOfficeXPTheme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition, int nID)
{
	UNREFERENCED_PARAMETER(nID);
	DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition);
}

void CXTPCommandBarsOfficeXPTheme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
...
void CXTPCommandBarsOfficeXPTheme::DrawImage(CDC* pDC, CPoint pt, CSize sz, CXTPImageManagerIcon* pImage, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, BOOL)
{
	if (!bEnabled)
	{
	...
	else if ((bSelected && !bPressed) || (!bSelected && bPressed == TRUE))
	{
		...
		{
			pImage->Draw(pDC, CPoint(pt.x + XTP_DPI_X(1), pt.y + XTP_DPI_Y(1)), pImage->GetShadowIcon(), sz);
			pImage->Draw(pDC, CPoint(pt.x - XTP_DPI_X(1), pt.y - XTP_DPI_Y(1)), pImage->GetHotIcon(), sz);
		}
		else
		{
...
CSize CXTPCommandBarsOfficeXPTheme::DrawControlEdit(CDC* pDC, CXTPControlEdit* pControlEdit, BOOL bDraw)
{
	if (!bDraw)
	{
	...
	if (!bEnabled)
	{
		...
		COLORREF clrBorder = GetXtremeColor(UINT(IsKeyboardSelected(bSelected) ? XPCOLOR_HIGHLIGHT_BORDER : XPCOLOR_DISABLED));
		pDC->Draw3dRect(rc, clrBorder, clrBorder);
	}
	else
	{
		pDC->FillSolidRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), rc.Width() - XTP_DPI_X(pControlEdit->IsSpinButtonsVisible() ? 18 : 2),
	...
	if (pControlEdit->IsSpinButtonsVisible())
	{
		...
		else
		{
			...
			pDC->FillSolidRect(rcSpin.left, rcSpin.top, XTP_DPI_X(1), rcSpin.Height(), GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER));
			pDC->FillSolidRect(rcSpin.left, rcBottom.top, rcSpin.Width(), XTP_DPI_Y(1), GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER));
		}
	...
	if (pControlEdit->IsBuddyButtonVisible())
	{
		...
		DrawRectangle(pDC, rcBuddyButton, bSelected == XTP_EDITBUDDYBUTTON, bPressed == XTP_EDITBUDDYBUTTON, bEnabled,  FALSE, FALSE, xtpBarTypeNormal, xtpBarTop);
...
CSize CXTPCommandBarsOfficeXPTheme::DrawControlComboBox(CDC* pDC, CXTPControlComboBox* pControlCombo, BOOL bDraw)
{
	if (!bDraw)
	{
	...
	CRect rcText(rc.left + XTP_DPI_X(3), rc.top + XTP_DPI_Y(1), rc.right - rcBtn.Width(), rc.bottom - XTP_DPI_Y(1));
	...
	{
		CRect rcEntry(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), rc.right - nThumb + XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1));
		...
		if (bSelected)
		{
			...
			DrawRectangle(pDC, rcBtn, TRUE, bDropped, TRUE, FALSE, FALSE, xtpBarTypeNormal, pControlCombo->GetParent()->GetPosition());
	...
	DrawComboExpandMark(pDC, rcBtn, GetXtremeColor(UINT(!pControlCombo->GetEnabled() ? XPCOLOR_DISABLED: bDropped ? XPCOLOR_PUSHED_TEXT: bSelected ? XPCOLOR_HIGHLIGHT_TEXT : XPCOLOR_TOOLBAR_TEXT)));

	return 0;
}


void CXTPCommandBarsOfficeXPTheme::DrawPopupBarGripper(CDC* pDC, int x, int y, int cx, int cy, BOOL bExpanded)
{
	pDC->FillSolidRect(x, y, cx, cy, GetXtremeColor(UINT(bExpanded ? XPCOLOR_MENUBAR_EXPANDED : XPCOLOR_TOOLBAR_FACE)));
}
...
void CXTPCommandBarsOfficeXPTheme::DrawSplitButtonPopup(CDC* pDC, CXTPControl* pButton)
{
	...
	if (!bPopuped || !bSelected || !bEnabled)
		VerticalLine(pDC, rcButton.right - m_nSplitButtonPopupWidth, rcButton.top + XTP_DPI_Y(1), rcButton.bottom - XTP_DPI_Y(1),
...
void CXTPCommandBarsOfficeXPTheme::DrawControlPopupGlyph(CDC* pDC, CXTPControl* pButton)
{
	...
	rc.left = rc.right - (pButton->GetType() == xtpControlSplitButtonPopup 
		? m_nSplitButtonPopupWidth : rc.Height());
	rc.DeflateRect(XTP_DPI_X(4), XTP_DPI_Y(4), XTP_DPI_X(4), XTP_DPI_Y(4));

	BOOL bSelected = pButton->GetSelected(), bPopuped = pButton->GetPopuped(), bEnabled = pButton->GetEnabled();
	COLORREF clr = (pButton->GetType() == xtpControlSplitButtonPopup && bEnabled && bPopuped && bSelected
...
CSize CXTPCommandBarsOfficeXPTheme::DrawControlPopupParent(CDC* pDC, CXTPControl* pButton, BOOL bDraw)
{
	...
	CSize szImage(max(szIcon.cx + XTP_DPI_X(4), m_nTextHeight + XTP_DPI_Y(4)), max(szIcon.cy + XTP_DPI_Y(6), m_nTextHeight));
	...
	if (bDraw)
	{
		...
		{
			DrawPopupBarGripper(pDC, rcButton.left - XTP_DPI_X(1), rcButton.top, nGripperWidth, rcButton.Height(), TRUE);
		}
		...
		if (pButton->GetType() == xtpControlLabel)
		{
			...
			int nLabelGripper = rcButton.left > nGripperWidth || !m_bOffsetPopupLabelText ? 0 : nGripperWidth;
		...
		if (bChecked)
		{
			DrawRectangle(pDC, rcCheck, bSelected, FALSE, bEnabled, bChecked, FALSE, xtpBarTypePopup, xtpBarPopup, pButton->GetID());
		}

		if (pImage)
		{
			szIcon = GetStretchIconSizeDPIAwareness(pImage, szIcon, pButton, rcImageRect.Size());
		...
		if (bChecked && (bDoubleGripper || !pImage))
		{
			...
			DrawCheckMark(pDC, rcCheck, (controlType == xtpControlRadioButton), bEnabled, bSelected);
		}
...
void CXTPCommandBarsOfficeXPTheme::DrawCheckMark(CDC* pDC, CRect rcCheck, BOOL bIsRadio, BOOL bEnabled, BOOL /*bSelected*/)
{
	COLORREF clr = !bEnabled ? GetXtremeColor(XPCOLOR_DISABLED) : 0;

	if (bEnabled && !XTPColorManager()->LongColor(0, GetXtremeColor(XPCOLOR_HIGHLIGHT_CHECKED), 1, 50))
	{
		clr = RGB(255, 255, 255);
	}
...
COLORREF CXTPCommandBarsOfficeXPTheme::GetRectangleTextColor(BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	UINT nColorFace = UINT(barPosition == xtpBarPopup ? XPCOLOR_MENUBAR_TEXT : XPCOLOR_TOOLBAR_TEXT);

	if (barType == xtpBarTypePopup)
	{
		return GetXtremeColor(UINT(!bEnabled ? XPCOLOR_MENUBAR_GRAYTEXT :
			bSelected ? XPCOLOR_HIGHLIGHT_TEXT : XPCOLOR_MENUBAR_TEXT));
	}

	COLORREF clrText = GetXtremeColor(UINT(!bEnabled ? XPCOLOR_TOOLBAR_GRAYTEXT :
...
void CXTPCommandBarsOfficeXPTheme::FillIntersectRect(CDC* pDC, CXTPPopupBar* pPopupBar, COLORREF clr)
{
	if (pPopupBar && pPopupBar->GetControlPopup())
	{
		...
		if (rcIntersect.IntersectRect(rcWindow, rcExclude))
		{
			...
			if (rcIntersect.Width() > XTP_DPI_X(1)) rcIntersect.DeflateRect(XTP_DPI_X(1), 0);
			if (rcIntersect.Height() > XTP_DPI_Y(1)) rcIntersect.DeflateRect(0, XTP_DPI_Y(1));
...
void CXTPCommandBarsOfficeXPTheme::FillCommandBarEntry(CDC* pDC, CXTPCommandBar* pBar)
{
	...
	if (pBar->GetPosition() == xtpBarFloating)
	{
		...
		COLORREF clrBrush =  GetXtremeColor(UINT(pBar->GetType() == xtpBarTypePopup ? XPCOLOR_MENUBAR_FACE :
...
CSize CXTPCommandBarsOfficeXPTheme::DrawDialogBarGripper(CDC* pDC, CXTPDialogBar* /*pBar*/, BOOL bDraw)
{
...
CSize CXTPCommandBarsOfficeXPTheme::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar, BOOL bDraw)
{
	if (pBar->IsDialogBar())
	...
	if (pBar->GetPosition() == xtpBarPopup && pBar->GetType() == xtpBarTypePopup)
	{
		...
		DrawPopupBarGripper(pDC, rcBorders.left - XTP_DPI_X(1), rcBorders.top, GetPopupBarGripperWidth(pBar), rc.bottom - rcBorders.top - rcBorders.bottom);
	}
	if (pBar->GetPosition() == xtpBarFloating)
	{
...
CSize CXTPCommandBarsOfficeXPTheme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	if (!bDraw)
	{
	...
	if (pBar->GetType() == xtpBarTypePopup)
	{
		if (!pControl->GetWrap())
			HorizontalLine(pDC, rcControl.left - XTP_DPI_X(1) + GetPopupBarGripperWidth(pBar) + m_nPopupBarTextPadding,
				rcControl.top - XTP_DPI_Y(2), rcControl.right + XTP_DPI_X(1), GetXtremeColor(XPCOLOR_SEPARATOR));
		else
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(2), rcRow.top - XTP_DPI_Y(1), rcRow.bottom + XTP_DPI_Y(1), GetXtremeColor(XPCOLOR_SEPARATOR));
	}
	else if (pBar->GetPosition() != xtpBarPopup && IsVerticalPosition(pBar->GetPosition()))
	{
		if (!pControl->GetWrap())
			HorizontalLine(pDC, rcRow.left, rcControl.top - XTP_DPI_Y(4), rcRow.right, GetXtremeColor(XPCOLOR_SEPARATOR));
		else
			VerticalLine(pDC, rcRow.right + XTP_DPI_X(2), rcRow.top + XTP_DPI_Y(4), rcRow.bottom, GetXtremeColor(XPCOLOR_SEPARATOR));
	}
	else
	{
		if (!pControl->GetWrap())
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(4), rcRow.top, rcRow.bottom, GetXtremeColor(XPCOLOR_SEPARATOR));
		else
			HorizontalLine(pDC, rcRow.left, rcRow.top - XTP_DPI_Y(4), rcRow.right, GetXtremeColor(XPCOLOR_SEPARATOR));
	}
	return 0;
}

void CXTPCommandBarsOfficeXPTheme::AdjustExcludeRect(CRect& rc, CXTPControl* pControl, BOOL bVertical)
{
...
CSize CXTPCommandBarsOfficeXPTheme::DrawListBoxControl(CDC* pDC, CXTPControl* pButton, CRect rcButton, BOOL bSelected, BOOL bDraw, CXTPCommandBars* pCommandBars)
{
	...
	CSize szImage(max(szIcon.cx + XTP_DPI_X(4), m_nTextHeight + XTP_DPI_X(4)), max(szIcon.cy + XTP_DPI_Y(6), m_nTextHeight));
	...
	if (bDraw)
	{
		...
		if (pImage)
		{
			szIcon = GetStretchIconSizeDPIAwareness(pImage, szIcon, pButton, szImage);
		...
		COLORREF clrText = GetRectangleTextColor(bSelected, FALSE, TRUE, FALSE, FALSE, xtpBarTypePopup, xtpBarPopup);
		...
		CRect rcLabelText(rcButton.left + szImage.cx + XTP_DPI_X(8), rcButton.top, rcButton.right, rcButton.bottom);
		...
		{
			Line(pDC, rcButton.right - XTP_DPI_X(20), rcButton.top + XTP_DPI_Y(1), rcButton.right - XTP_DPI_X(20), rcButton.bottom - XTP_DPI_Y(1), bSelected ? XPCOLOR_HIGHLIGHT_BORDER : COLOR_3DSHADOW);
		...
		if (controlType == xtpControlComboBox || controlType == xtpControlEdit)
		{
			...
			CRect rcCombo(rcButton.left + szImage.cx + XTP_DPI_X(8) + rcText.Width() + XTP_DPI_X(5), rcButton.top, rcButton.right, rcButton.bottom);
			...
			{
				if (!bSelected) pDC->FillSolidRect(rcThumb.left + XTP_DPI_X(2), rcThumb.top + XTP_DPI_Y(2), rcThumb.Width() - XTP_DPI_X(4), rcThumb.Height() - XTP_DPI_Y(4), GetXtremeColor(COLOR_3DFACE));
				...
				DrawComboExpandMark(pDC, rcThumb, GetXtremeColor(UINT(bSelected ? XPCOLOR_HIGHLIGHT_TEXT : XPCOLOR_TOOLBAR_TEXT)));
			}
		}
...
CSize CXTPCommandBarsOfficeXPTheme::DrawTearOffGripper(CDC* pDC, CRect rcGripper, BOOL bSelected, BOOL bDraw)
{
	if (bDraw)
	{
		CRect rc(XTP_DPI_X(1), XTP_DPI_Y(1), rcGripper.right + XTP_DPI_X(1), rcGripper.Height() + XTP_DPI_Y(1));
...
void CXTPCommandBarsOfficeXPTheme::DrawPopupResizeGripper(CDC* pDC, CRect rcGripper, int nFlags)
{
	...
	if (nFlags == 2)
	{
		...
		{
			pDC->FillSolidRect(CRect(x + XTP_DPI_X(1), XTP_DPI_Y(4) + rcGripper.top, x + XTP_DPI_X(3), XTP_DPI_Y(6) + rcGripper.top), GetXtremeColor(COLOR_WINDOW));
			pDC->FillSolidRect(CRect(x, XTP_DPI_Y(3) + rcGripper.top, x + XTP_DPI_X(2), XTP_DPI_Y(5) + rcGripper.top), GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
	}
	else
	{
		...
		pDC->FillSolidRect(CRect(x + XTP_DPI_X(1), XTP_DPI_Y(1) + y, x + XTP_DPI_X(3), XTP_DPI_Y(3) + y), GetXtremeColor(COLOR_WINDOW));
		pDC->FillSolidRect(CRect(x, 0 + y, x + XTP_DPI_X(2), XTP_DPI_Y(2) + y), GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
...
CSize CXTPCommandBarsOfficeXPTheme::DrawControlCheckBoxMark(CDC* pDC, CXTPControl* pButton, CRect rc, BOOL bDraw)
{
...
CSize CXTPCommandBarsOfficeXPTheme::DrawControlCheckBoxMark(CDC* pDC, CRect rc, BOOL bDraw, BOOL bSelected, BOOL bPressed, BOOL bChecked, BOOL bEnabled)
{
	...
	if (bDraw)
	{
		...
		if (bChecked)
		{
			...
			CXTPPenDC pen(pDC->GetSafeHdc(), GetXtremeColor(UINT(!bEnabled || bChecked == 2 ? XPCOLOR_GRAYTEXT : COLOR_BTNTEXT)));
...
CSize CXTPCommandBarsOfficeXPTheme::DrawControlRadioButtonMark(CDC* pDC, CRect rc, BOOL bDraw, BOOL bSelected, BOOL bPressed, BOOL bChecked, BOOL bEnabled)
{
	if (m_bThemedCheckBox && m_themeButton->IsAppThemed())
	...
	if (bDraw)
	{
		...
		{
			AlphaEllipse(pDC, rc, GetXtremeColor(UINT(bSelected || bPressed ? XPCOLOR_HIGHLIGHT_BORDER : XPCOLOR_3DSHADOW)),
				GetXtremeColor(UINT(bSelected && bPressed ? XPCOLOR_HIGHLIGHT_PUSHED :
				bPressed || bSelected ? XPCOLOR_HIGHLIGHT : COLOR_WINDOW)));
		}

		if (bChecked)
		{
			COLORREF clrFace = GetXtremeColor(UINT(!bEnabled || bChecked == 2 ? XPCOLOR_GRAYTEXT : COLOR_BTNTEXT));
			AlphaEllipse(pDC, CRect(
				rc.left + XTP_DPI_X(4), rc.top + XTP_DPI_Y(4),
				rc.left + XTP_DPI_X(9), rc.top + XTP_DPI_Y(9)), clrFace, clrFace);
		}
	}
...
void CXTPCommandBarsOfficeXPTheme::DrawSplitButtonFrame(CDC* pDC, CXTPControl* pButton, CRect rcButton)
{
	BOOL bPressed = pButton->GetPressed(), bSelected = pButton->GetSelected(), bEnabled = pButton->GetEnabled(), bChecked = pButton->GetChecked(),
	...
	if (pButton->GetStyle() == xtpButtonIconAndCaptionBelow)
	{
		...
		{
			if (bVert)
				pDC->FillSolidRect(rcButton.left + nSplitDropDownHeight, rcButton.top, XTP_DPI_X(1), rcButton.Height(), GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER));
			else
				pDC->FillSolidRect(rcButton.left, rcButton.bottom - nSplitDropDownHeight - XTP_DPI_Y(1), rcButton.Width(), XTP_DPI_Y(1), GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER));
		}
	...
	if ((bSelected || bPressed) && !bPopuped && bEnabled)
		DrawRectangle(pDC, CRect(rcButton.right - m_nSplitButtonDropDownWidth - XTP_DPI_X(1), rcButton.top, rcButton.right, rcButton.bottom),
