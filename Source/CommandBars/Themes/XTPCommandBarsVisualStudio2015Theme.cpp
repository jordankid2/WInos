// XTPCommandBarsVisualStudio2015Theme.cpp : implementation of the CXTPCommandBarsVisualStudio2015Theme class.
//
...
#include "CommandBars/Gallery/XTPControlGalleryVisualStudio2015Theme.h"
...
#include "CommandBars/Frame/XTPFrameThemeVisualStudio2015.h"
...
#include "CommandBars/StatusBar/XTPStatusBarThemeVisualStudio2015.h"
...
IMPLEMENT_DYNAMIC(CXTPCommandBarsVisualStudio2015Theme, CXTPCommandBarsOffice2013Theme)

CXTPCommandBarsVisualStudio2015Theme::CXTPCommandBarsVisualStudio2015Theme()
{
	// Frame paint manager
	SAFE_DELETE(m_pFramePaintManager);
	m_pFramePaintManager = new CXTPFrameThemeVisualStudio2015(this);

	// Progress paint manager
	SAFE_DELETE(m_pProgressPaintManager);
	m_pProgressPaintManager = new CXTPProgressThemeVisualStudio2015(this);

	// Slider paint manager
	SAFE_DELETE(m_pSliderPaintManager);
	m_pSliderPaintManager = new CXTPSliderThemeVisualStudio2015(this);

	// Message bar paint manager
	SAFE_DELETE(m_pMessageBarPaintManager);
	m_pMessageBarPaintManager = new CXTPMessageBarThemeVisualStudio2015(this);

	// Status bar paint manager
	SAFE_DELETE(m_pStatusBarPaintManager);
	m_pStatusBarPaintManager = new CXTPStatusBarThemeVisualStudio2015(this);

	// Ribbon paint manager
	SAFE_DELETE(m_pRibbonPaintManager);
	m_pRibbonPaintManager = new CXTPRibbonThemeVisualStudio2015(this);

	// Gallery paint manager
	SAFE_DELETE(m_pGalleryPaintManager);
	m_pGalleryPaintManager = new CXTPControlGalleryVisualStudio2015Theme(this);

	// Scrollbar paint manager
	SAFE_DELETE(m_pScrollBarPaintManager);
	m_pScrollBarPaintManager = new CXTPScrollBarThemeVisualStudio2015();
...
void CXTPCommandBarsVisualStudio2015Theme::SetStateColor(COLORREF clrState /*=COLORREF_NULL*/)
{
	CXTPFrameThemeVisualStudio2015* pFramePaintManager = static_cast<CXTPFrameThemeVisualStudio2015*>(m_pFramePaintManager);
	pFramePaintManager->SetStateColor(clrState);

	CXTPStatusBarThemeVisualStudio2015* pStatusBarPaintManager = static_cast<CXTPStatusBarThemeVisualStudio2015*>(m_pStatusBarPaintManager);
	pStatusBarPaintManager->SetStateColor(clrState);
}

COLORREF CXTPCommandBarsVisualStudio2015Theme::GetStateColor() const
{
	const CXTPFrameThemeVisualStudio2015* pFramePaintManager = static_cast<const CXTPFrameThemeVisualStudio2015*>(m_pFramePaintManager);
	COLORREF clr1 =  pFramePaintManager->GetStateColor();

	const CXTPStatusBarThemeVisualStudio2015* pStatusBarPaintManager = static_cast<const CXTPStatusBarThemeVisualStudio2015*>(m_pStatusBarPaintManager);
	COLORREF clr2 = pStatusBarPaintManager->GetStateColor();

	return clr1 == clr2 ? clr1 : COLORREF_NULL;
}

void CXTPCommandBarsVisualStudio2015Theme::DrawSplitButtonFrame(CDC* pDC, CXTPControl* pButton, CRect rcButton)
{
	...
	else
	{
		...
		DrawRectangle(pDC, rcButton, bSelected, bPressed, bEnabled, bChecked, bPopuped, pButton->GetParent()->GetType(), pButton->GetParent()->GetPosition());
		...
		if (bEnabled && bSelected && !bPopuped && !bPressed && bChecked )
		{
			...
			VerticalLine(pDC, rcButton.right - m_nSplitButtonDropDownWidth - 1,
				rcButton.top, rcButton.bottom, clrLine);
		}
...
void CXTPCommandBarsVisualStudio2015Theme::DrawCheckRect(CDC* pDC, CRect rc, COLORREF clrFore, COLORREF clrBack)
{
	ASSERT_VALID(pDC);
...
void CXTPCommandBarsVisualStudio2015Theme::DrawGalleryItemBack(CDC* pDC, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked, XTPBarType barType)
{
	if (barType == xtpBarTypePopup)
	{
		if (!bEnabled) // disabled
		{
			Rectangle(pDC, rcItem,
				m_csComboBox.clrBackDisabled, m_csComboBox.clrBackDisabled);
			return;
		}
		else if (!bSelected && !bPressed) // normal state
		{
			Rectangle(pDC, rcItem,
				m_csComboBox.clrBack, m_csComboBox.clrBack);
			return;
		}
		else if (bSelected && !bPressed) // hover
		{
			Rectangle(pDC, rcItem,
				m_csToolbar.clrBorderHighlight, m_csToolbar.clrBackHighlight);
			return;
		}
		else if (bSelected && bPressed) // pressed
		{
			Rectangle(pDC, rcItem,
				m_csToolbar.clrBorderPushed, m_csToolbar.clrBackPushed);
			return;
		}
	}
...
COLORREF CXTPCommandBarsVisualStudio2015Theme::GetControlTextColor(CXTPControl* pButton)
{
	ASSERT_VALID(pButton);
...
void CXTPCommandBarsVisualStudio2015Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition, int nID)
{
	...
	if (bChecked && nID == 0)
	{
		...
		if (!bEnabled) // disabled
		{
			DrawCheckRect(pDC, rc,
				m_csCheckMark.clrCheckDisabled, m_csCheckMark.clrBackDisabled);
			return;
		}
		else if (!bSelected && !bPressed) // normal state
		{
			DrawCheckRect(pDC, rc,
				m_csCheckMark.clrCheck, m_csCheckMark.clrBack);
			return;
		}
		else if (bSelected && !bPressed) // hover
		{
			DrawCheckRect(pDC, rc,
				m_csCheckMark.clrCheckHighlight, m_csCheckMark.clrBackHighlight);
			return;
		}
		else if (bSelected && bPressed) // pressed
		{
			DrawCheckRect(pDC, rc,
				m_csCheckMark.clrCheckPushed, m_csCheckMark.clrBackPushed);
			return;
		}
	}
	else
...
void CXTPCommandBarsVisualStudio2015Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
...
BOOL CXTPCommandBarsVisualStudio2015Theme::DrawRectangle(CDC* pDC, CXTPControl* pButton)
{
	return CXTPCommandBarsOffice2013Theme::DrawRectangle(pDC, pButton);
}

BOOL AFX_CDECL XTPSetVSStateColor(CXTPCommandBars* pCommandBars, COLORREF clrState /*=COLORREF_NULL*/)
{
	if (!pCommandBars)
		return FALSE;

	CXTPCommandBarsVisualStudio2015Theme* pPaintManager = 
		DYNAMIC_DOWNCAST(CXTPCommandBarsVisualStudio2015Theme, pCommandBars->GetPaintManager());
...
CSize CXTPCommandBarsVisualStudio2015Theme::DrawCommandBarGripper(CDC *pDC, CXTPCommandBar *pBar, BOOL bDraw)
{
	CSize szGripper(0, 0);
		...
		xtpBarBottom == pBar->GetPosition())
	{
		...
		if (bDraw)
		{
			...
			DrawDashedLine(pDC, ptDash.x, ptDash.y, nDashZ, GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER), TRUE);

			ptDash.x += XTP_DPI_X(2); ptDash.y += XTP_DPI_Y(2); nDashZ -= XTP_DPI_X(2);
			DrawDashedLine(pDC, ptDash.x, ptDash.y, nDashZ, GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER), TRUE);
...
CSize CXTPCommandBarsVisualStudio2015Theme::DrawControlComboBox(CDC* pDC, CXTPControlComboBox* pControlCombo, BOOL bDraw)
{
