...
void CXTPCommandBarsVisualStudio2012Theme::DrawDashedLine(CDC *pDC, int x, int y1, int y2, COLORREF clr)
{
...
CSize CXTPCommandBarsVisualStudio2012Theme::DrawControlComboBox(CDC* pDC, CXTPControlComboBox* pControlCombo, BOOL bDraw)
{
	...
	CRect rcDropDown(rcComboBox.right - nThumb, rcComboBox.top + XTP_DPI_Y(1), rcComboBox.right - XTP_DPI_X(1), rcComboBox.bottom - XTP_DPI_Y(1));
	CRect rcText(rcComboBox.left + XTP_DPI_X(3), rcComboBox.top + XTP_DPI_Y(1), rcComboBox.right - rcDropDown.Width(), rcComboBox.bottom - XTP_DPI_Y(1));
...
void AFX_CDECL CXTPCommandBarsVisualStudio2012Theme::FillRect(CDC* pDC, CRect rc, BOOL bCheckMark, COLORREF clrFore, COLORREF clrBack)
{
	if (bCheckMark)
	{
...
void CXTPCommandBarsVisualStudio2012Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition, int nID)
{
...
CSize CXTPCommandBarsVisualStudio2012Theme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pControl, BOOL bDraw)
{
	...
	if (pBar->GetType() == xtpBarTypePopup)
	{
		if (!pControl->GetWrap())
		{
			HorizontalLine(pDC, rcControl.left - XTP_DPI_X(1) + GetPopupBarGripperWidth(pBar) + m_nPopupBarTextPadding,
				rcControl.top - XTP_DPI_Y(2), rcControl.right + XTP_DPI_X(1), m_pColorSet->m_clrSeparator);
			HorizontalLine(pDC, rcControl.left - XTP_DPI_X(1) + GetPopupBarGripperWidth(pBar) + m_nPopupBarTextPadding,
				rcControl.top - XTP_DPI_Y(1), rcControl.right + XTP_DPI_X(1), m_pColorSet->m_clrSeparatorHighlight);
		}
		else
		{
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(2), rcRow.top - XTP_DPI_Y(1), rcRow.bottom + XTP_DPI_Y(1), m_pColorSet->m_clrSeparator);
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(1), rcRow.top - XTP_DPI_Y(1), rcRow.bottom + XTP_DPI_Y(1), m_pColorSet->m_clrSeparatorHighlight);
		}
	}
	else if (pBar->GetPosition() != xtpBarPopup && IsVerticalPosition(pBar->GetPosition()))
	{
		if (!pControl->GetWrap())
		{
			HorizontalLine(pDC, rcRow.left, rcControl.top - XTP_DPI_Y(4), rcRow.right, m_pColorSet->m_clrSeparator);
			HorizontalLine(pDC, rcRow.left, rcControl.top - XTP_DPI_Y(3), rcRow.right, m_pColorSet->m_clrSeparatorHighlight);
		}
		else
		{
			VerticalLine(pDC, rcRow.right + XTP_DPI_X(2), rcRow.top + XTP_DPI_Y(4), rcRow.bottom, m_pColorSet->m_clrSeparator);
			VerticalLine(pDC, rcRow.right + XTP_DPI_X(3), rcRow.top + XTP_DPI_Y(4), rcRow.bottom, m_pColorSet->m_clrSeparatorHighlight);
		}
	}
	else
	{
		if (!pControl->GetWrap())
		{
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(4), rcRow.top + XTP_DPI_Y(2), rcRow.bottom, m_pColorSet->m_clrSeparator);
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(3), rcRow.top + XTP_DPI_Y(2), rcRow.bottom, m_pColorSet->m_clrSeparatorHighlight);
		}
		else
		{
			HorizontalLine(pDC, rcRow.left, rcRow.top - XTP_DPI_Y(4), rcRow.right, m_pColorSet->m_clrSeparator);
			HorizontalLine(pDC, rcRow.left, rcRow.top - XTP_DPI_Y(3), rcRow.right, m_pColorSet->m_clrSeparatorHighlight);
		}
	}
	return 0;
}
...
void CXTPCommandBarsVisualStudio2012Theme::DrawExpandGlyph(CDC* pDC, CXTPControl* pButton, CPoint pt0, CPoint pt1, CPoint pt2, COLORREF /*clr*/)
{
	...
	if (pButton->GetPopuped())
	{
		HorizontalLine(pDC, pt2.x - XTP_DPI_X(2), pt0.y - XTP_DPI_Y(3), pt1.x + XTP_DPI_X(1), m_pColorSet->m_clrGlyphPushed);
	...
	else
	{
		...
		{
			HorizontalLine(pDC, pt2.x - XTP_DPI_X(2), pt0.y - XTP_DPI_Y(3), pt1.x + XTP_DPI_X(1), m_pColorSet->m_clrGlyphHighlighted);
			Triangle(pDC, pt0, pt1, pt2, m_pColorSet->m_clrGlyphHighlighted);
		}
		else
		{
			HorizontalLine(pDC, pt2.x - XTP_DPI_X(2), pt0.y - XTP_DPI_Y(3), pt1.x + XTP_DPI_X(1), m_pColorSet->m_clrGlyph);
			Triangle(pDC, pt0, pt1, pt2, m_pColorSet->m_clrGlyph);
		}
	}
}

void CXTPCommandBarsVisualStudio2012Theme::DrawExpandGlyph(CDC* pDC, CXTPControl* pButton, COLORREF clr, BOOL bVertical)
{
