...
BOOL CXTPTabClientWnd::CWorkspace::DrawIcon(CDC* pDC, CPoint pt, CXTPTabManagerItem* pItem, BOOL bDraw, CSize& szIcon) const
{
	if (pItem == m_pNewWindowItem)
	{
		...
		CXTPImageManagerIcon* pIcon = pImageManager->GetImage(XTPToUIntChecked(m_pTabClientWnd->m_nNewWindowTabCommand));
	...
	if (pItem->GetImageIndex() != -1)
	{
		...
		CXTPImageManagerIcon* pIcon = pImageManager->GetImage(XTPToUIntChecked(pItem->GetImageIndex()));
		if (pIcon) pIcon->Draw(pDC, pt, szIcon);
...
#else
		m_pTabClientWnd->GetParent()->SendMessage(WM_COMMAND, XTPToWPARAM(m_pTabClientWnd->m_nNewWindowTabCommand));
#endif
