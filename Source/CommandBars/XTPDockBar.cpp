...
public:
	void Sort()
	{
		qsort(GetData(), XTPToSizeT(GetSize()), sizeof(DOCK_INFO), CompareFunc);
	}
...
int CXTPDockBar::_GetMode(BOOL bHorz, CXTPToolBar* pBar)
{
	...
	DWORD dwMode = DWORD(bHorz ? LM_HORZ | LM_HORZDOCK : LM_VERTDOCK);
	...
	if ((!m_pCommandBars->IsCustomizeMode() || m_pCommandBars->IsQuickCustomizeMode()) && pBar->GetFlags() & xtpFlagHideWrap)
		return XTPToInt(dwMode | LM_HIDEWRAP);

	return XTPToInt(dwMode);
}
...
int CXTPDockBar::_AdjustRow(CToolBarArray& arrRow, CPoint pt, int nLength, BOOL bHorz, AFX_SIZEPARENTPARAMS* lpLayout, int& nRemove)
{
	...
	for (nPos = 0; nPos < arrRow.GetSize(); nPos++)
	{
		...
		CSize sizeBar = pBar->CalcDockingLayout(nLength, XTPToDWORD(_GetMode(bHorz, pBar)));
	...
	if (nLen > nLength)
	{
		...
		for (nPos = 0; nPos < arrInfo.GetSize(); nPos++)
		{
			...
			{
				CSize sz = pBar->CalcDockingLayout(1, XTPToDWORD(_GetMode(bHorz, pBar)));
	...
	for (nPos = 0; nPos < arrInfo.GetSize(); nPos++)
	{
		CXTPToolBar* pBar = arrInfo[nPos].pBar;
		CRect& rect = arrInfo[nPos].rcBar;
		CSize sizeBar = pBar->CalcDockingLayout(bHorz ? rect.Width() : rect.Height(), XTPToDWORD(_GetMode(bHorz, pBar)));
	...
	for (nPos = 0; nPos < arrInfo.GetSize(); nPos++)
	{
		CXTPToolBar* pBar = arrInfo[nPos].pBar;
		CRect& rect = arrInfo[nPos].rcBar;
		DWORD nMode = XTPToDWORD(_GetMode(bHorz, pBar)) | LM_COMMIT;
...
{
	BOOL bHorz = (0 != (nMode & LM_HORZ));
	BOOL bStretch = (0 != (nMode & LM_STRETCH));
...
void CXTPDockBar::OnSize(UINT nType, int cx, int cy)
{
	...
	{
		CXTPCommandBarsFrameHook* pFrameHook = m_pCommandBars->GetFrameHook();
		if (NULL != pFrameHook)
		{
			pFrameHook->UpdateDwmClientArea();
		}
	}
}
...
LRESULT CXTPDockBar::OnSizeParent(WPARAM, LPARAM lParam)
{
	...
	if (dwStyle & WS_VISIBLE)
	{
		...
		DWORD dwMode = DWORD(lpLayout->bStretch ? LM_STRETCH : 0);
...
int CXTPDockBar::Insert(CXTPToolBar* pBarIns, CRect /*rect*/, CPoint ptMid)
{
	...
	int nTotalWidth = 0;
	//int nPosInsBefore = 0;
	BOOL bHorz = (0 != (m_dwStyle & CBRS_ORIENT_HORZ));
