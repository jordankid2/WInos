...
CXTPPopupBar::CXTPPopupBar()
{
	...
	m_bShowShadow = TRUE;
	m_rcBorders = XTP_DPI(CRect(2, 4, 2, 4));
	m_bExpanding = FALSE;
...
BOOL CXTPPopupBar::Create()
{
	...
	UINT nClassStyle = UINT((!pCommandBars || pCommandBars->GetCommandBarsOptions()->bUseSystemSaveBitsStyle) 
		? CS_SAVEBITS | CS_DBLCLKS : CS_DBLCLKS);
	CWnd* pSite = GetSite();
	...
	if (!CreateEx(XTPToDWORD(WS_EX_TOOLWINDOW | nLayoutRTL), _T("XTPPopupBar"), 0, 
		MFS_SYNCACTIVE | WS_POPUP | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, CRect(0, 0, 0, 0), pSite, 0))
		return FALSE;
...
XTPAnimationType CXTPPopupBar::GetAnimationType() const
{
	...
	if (pCommandBars)
	{
		...
		if (xtpAnimateWindowsDefault == animationType)
		{
			BOOL bEnabled = FALSE;
		...
		else if (xtpAnimateRandom == animationType)
		{
			switch (RAND_S() % 3)
			{
				case 0:
...
void CXTPPopupBar::RecalcSizeLayout()
{
	if (!m_bDynamicLayout && GetSafeHwnd() && IsTrackingMode())
	{
		...
		BOOL bRTL = (0 != (GetExStyle() & WS_EX_LAYOUTRTL));
...
CSize CXTPPopupBar::CalcDynamicLayout(int nLength, DWORD /*nMode*/)
{
	...
	if (m_bResizable)
	{
		szResizeGripper.cy = XTP_DPI_Y(m_bResizable != 2 ? 11 : 8);
...
CRect CXTPPopupBar::CalculatePopupRect(CPoint ptPopup, CSize sz)
{
	...
	rcWork.bottom -= XTP_DPI_Y(5);  // for shadow
...
void CXTPPopupBar::UpdateLocation(CSize sz)
{
	...
	SetWindowPos(&CWnd::wndTopMost, rc.left, rc.top, rc.Width(), rc.Height(), 
		UINT(SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_SHOWWINDOW | (IsWindowVisible() ? SWP_NOZORDER : 0)));
}
...
void CXTPPopupBar::AdjustScrolling(LPSIZE lpSize, BOOL bDown, BOOL bInvalidate)
{
	...
	int nScrollHeight = XTP_DPI_Y(16);
	int i;
...
void CXTPPopupBar::SwitchTearOffTracking(BOOL bShow, CPoint point)
{
	...
	if (!bShow)
	{
		...
		CPoint pt(point.x - m_szTearOffBar.cx / 2, point.y - XTP_DPI_Y(10));
...
void CXTPPopupBar::TrackTearOff()
{
	...
	while (CWnd::GetCapture() == this)
	{
		...
		{
			pt = msg.pt;
			BOOL bState = !(pt.x > rcGripper.right + XTP_DPI_X(10) || pt.x < rcGripper.left - XTP_DPI_X(10) ||
				pt.y > rcGripper.bottom + XTP_DPI_Y(10) || pt.y < rcGripper.top - XTP_DPI_Y(10));
			if (bState != bShow)
			{
				bShow = bState;
			...
			if (m_pTearOffBar != NULL)
			{
				CPoint point(pt.x - m_szTearOffBar.cx / 2, pt.y - XTP_DPI_Y(10));
...
void CXTPPopupBar::DrawCommandBar(CDC* pDC, CRect rcClipBox)
{
	...
	if (m_scrollInfo.bScroll)
	{
		...
		{
			CRect rc = m_scrollInfo.btnUp.m_rc;
			rc.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2));
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayDown, rc, GetXtremeColor(COLOR_GRAYTEXT));
		}
		if (!m_scrollInfo.btnDown.m_rc.IsRectEmpty())
		{
			CRect rc = m_scrollInfo.btnDown.m_rc;
			rc.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2));
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayUp, rc, GetXtremeColor(COLOR_GRAYTEXT));
		}
	}
...
BOOL CXTPPopupBar::SetSelected(int nSelected, BOOL bKeyboard)
{
	...
	if (m_nSelected != -1 && !bKeyboard && (GetControl(m_nSelected)->GetID() == XTP_ID_POPUPBAR_EXPAND))
	{
		...
		{
			SetTimer(TID_EXPANDHOVER, XTPToUInt(s_nExpandHoverDelay), NULL);
		}
	}
	else
	{
...
{
	CXTPControl* pControl = fByPos ? m_pControls->GetAt(XTPToIntChecked(uItem)) : m_pControls->FindControl(
		xtpControlError, XTPToIntChecked(uItem), TRUE, FALSE);
	if (pControl)
	{
...
UINT CXTPPopupBar::GetDefaultItem(UINT /*gmdiFlags*/, BOOL fByPos)
{
	for (int i = 0; i < GetControlCount(); i++)
	{
		...
		{
			return fByPos ? XTPToUIntChecked(pControl->GetIndex()) : XTPToUInt(pControl->GetID());
		}
	}
	return (UINT)-1;
}
...
BEGIN_DISPATCH_MAP(CXTPPopupBar, CXTPCommandBar)
	DISP_PROPERTY_ID(CXTPPopupBar, "DoubleGripper", 30, m_bDoubleGripper, VT_BOOL)
END_DISPATCH_MAP()
