...
void CXTPControlComboBoxList::CreateListBox()
{
	...
	{
		CreateEx(WS_EX_CLIENTEDGE | WS_EX_TOOLWINDOW, _T("LISTBOX"), _T(""),
			WS_POPUP | WS_VSCROLL | WS_CLIPCHILDREN | dwStyle, CRect(0, 0, 0, XTP_DPI_Y(50)), 0, 0);
	}
	...
	if (m_pCommandBars)
	{
		if (m_nListIconId != -1)
		{
			...
			if (pIcon)
			{
				GetListBoxCtrl()->SetItemHeight(-1, XTPToUIntChecked(XTP_DPI_Y(pIcon->GetHeight()) + XTP_DPI_Y(4)));
			}
		}
		else
		{
			...
			if (0 != lf.lfHeight)
			{
				GetListBoxCtrl()->SetItemHeight(-1, XTPToUIntChecked(abs(lf.lfHeight) + XTP_DPI_Y(4)));
			}
		}
	}
}
...
void CXTPControlComboBoxList::OnNcPaint()
{
	...
	dc.Draw3dRect(rc, pPaintManager->GetXtremeColor(XPCOLOR_EDITCTRLBORDER), pPaintManager->GetXtremeColor(XPCOLOR_EDITCTRLBORDER));

	rc.DeflateRect(1, 1);
	dc.Draw3dRect(rc, pPaintManager->GetXtremeColor(COLOR_WINDOW), pPaintManager->GetXtremeColor(COLOR_WINDOW));
}
...
{
	return static_cast<CListBox*>(static_cast<CWnd*>(
			const_cast<CXTPControlComboBoxList*>(this)));
}
...
BOOL CXTPControlComboBoxList::Popup(CXTPControlPopup* pControlPopup, BOOL /*bSelectFirst*/)
{
	...
	int nHeight = min (((CXTPControlComboBox*)m_pControlPopup)->m_nDropDownItemCount,
		max(1, GetListBoxCtrl()->GetCount())) * nItemHeight + XTP_DPI_Y(4);
...
void CXTPControlComboBoxList::Copy(CXTPCommandBar* pCommandBar, BOOL bRecursive)
{
	if (this != pCommandBar)
	{
		CXTPControlComboBoxList* pComboBoxList = DYNAMIC_DOWNCAST(CXTPControlComboBoxList, pCommandBar);
