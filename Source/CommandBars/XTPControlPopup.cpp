...
BOOL CXTPControlPopup::OnLButtonDblClk(CPoint point)
{
	...
	if (pPopupBar && (m_controlType == xtpControlPopup || m_controlType == xtpControlButtonPopup))
	{
		...
		{
			int nDefID = XTPToInt(pPopupBar->GetDefaultItem(0, TRUE));
			CXTPControl* pControl = pPopupBar->GetControl(nDefID);
			pControl->OnExecute();
			return TRUE;
		}
...
BEGIN_INTERFACE_MAP(CXTPControlPopup, CXTPControlButton)
	INTERFACE_PART(CXTPControlPopup, XTPDIID_ICommandBarPopup, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPControlPopup, XTPDIID_ICommandBarPopup)
