...
void CXTPStatusBarThemeResource::RefreshMetrics()
{
	...
	m_clrTextGray    = XTPIniColor(_T("StatusBar"), _T("StatusBarTextGray"), ::GetSysColor(COLOR_GRAYTEXT));
	m_clrGripperLite = XTPIniColor(_T("StatusBar"), _T("GripperLite"),       RGB(255, 255, 255));
	m_clrGripperDark = XTPIniColor(_T("StatusBar"), _T("GripperDark"),       ::GetXtremeColor(XPCOLOR_SEPARATOR));
}
...
void CXTPStatusBarThemeResource::DrawStatusBarPaneBorder(CDC* pDC, CRect rc, CXTPStatusBarPane* /*pPane*/, BOOL bGripperPane)
{
	if (!bGripperPane)
	{
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top, XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(2), m_clrGripperDark);
		pDC->FillSolidRect(rc.right, rc.top, XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(2), m_clrGripperLite);
	}
}
...
void CXTPStatusBarThemeResource::DrawStatusBarGripper(CDC* pDC, CRect rcClient)
{
	...
	CRect rcGripper(rcClient.right - XTP_DPI_X(rcSrc.Width()) - szMargin.cx, rcClient.bottom - XTP_DPI_Y(rcSrc.Height()) - szMargin.cy,
