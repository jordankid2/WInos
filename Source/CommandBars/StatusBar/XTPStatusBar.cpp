...
CXTPStatusBar::CXTPStatusBar()
{
	...
	m_cyTopBorder = XTP_DPI_Y(2);
	m_cxLeftBorder = 0;
	m_cxRightBorder = 0;
	m_cyBottomBorder = 0;
	m_cxPaneSpacing = XTP_DPI_X(2);

	m_bCustomizable = FALSE;

	m_nMinHeight = XTP_DPI_Y(20);
...
{
	if (m_xtpFontStatus.GetSafeHandle())
		return &m_xtpFontStatus;
...
void CXTPStatusBar::SetFont(CFont* pFont, BOOL bRedraw /* = TRUE */)
{
	...
	if (pFont)
	{
		...
		m_xtpFontStatus.CreateFontIndirect(&lf);
	}
...
{
	ASSERT(pParentWnd);   // must have a parent
...
LRESULT CXTPStatusBar::OnNcHitTest(CPoint point)
{
	if (m_bShowSizeGripper && !::IsZoomed(::GetParent(m_hWnd)))
	{
		...
		if (point.y >= rc.bottom - XTP_DPI_Y(3) && GetPaintManager()->BaseTheme() == xtpThemeRibbon && CXTPCommandBarsFrameHook::m_nMsgQueryFrameHook != 0)
		{
...
void CXTPStatusBar::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	nm.hdr.idFrom = XTPToUIntPtrChecked(GetDlgCtrlID());
	nm.hdr.hwndFrom = GetSafeHwnd();
	nm.dwItemSpec = XTPToDWORDChecked(m_pHighlightedPane ? m_pHighlightedPane->GetIndex() : ~0);
	nm.pt = point;
...
void CXTPStatusBar::OnNcCalcSize(BOOL /*bCalcValidRects*/, NCCALCSIZE_PARAMS* lpncsp)
{
	...
	lpncsp->rgrc[0].top += rect.top - __min(rect.top, XTP_DPI_Y(2));
...
void CXTPStatusBar::CStatusCmdUI::Enable(BOOL bOn)
{
	...
	CXTPStatusBar* pStatusBar = DYNAMIC_DOWNCAST(CXTPStatusBar, m_pOther);
	ASSERT(pStatusBar != NULL);
	if (!pStatusBar)
		return;
	ASSERT(m_nIndex < m_nIndexMax);

	CXTPStatusBarPane* pPane = pStatusBar->GetPane(XTPToIntChecked(m_nIndex));
...
void CXTPStatusBar::CStatusCmdUI::SetCheck(int nCheck) // "checking" will pop out the text
{
	...
	UINT nNewStyle = pStatusBar->GetPaneStyle(XTPToIntChecked(m_nIndex)) & ~SBPS_POPOUT;
	if (nCheck != 0)
		nNewStyle |= SBPS_POPOUT;
	pStatusBar->SetPaneStyle(XTPToIntChecked(m_nIndex), nNewStyle);
}
...
void CXTPStatusBar::CStatusCmdUI::SetText(LPCTSTR lpszText)
{
	...
	pStatusBar->SetPaneText(XTPToIntChecked(m_nIndex), lpszText);
}
...
void CXTPStatusBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
	CStatusCmdUI state;
	state.m_pOther = this;
	state.m_nIndexMax = XTPToUIntChecked(GetPaneCount());
	for (state.m_nIndex = 0; state.m_nIndex < state.m_nIndexMax;
		state.m_nIndex++)
	{
		state.m_nID = GetPane(XTPToIntChecked(state.m_nIndex))->m_nID;
...
LRESULT CXTPStatusBar::OnIdleUpdateCmdUI(WPARAM wParam, LPARAM)
{
	...
	BOOL bVis = (0 != (GetStyle() & WS_VISIBLE));
	UINT swpFlags = 0;
...
INT_PTR CXTPStatusBar::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	if (pPane)
	{
		...
		nHit = XTPToIntPtrChecked(pPane->GetID());
...
#ifndef _XTP_ACTIVEX
	pPane;
	GetParent()->SendMessage(WM_COMMAND, XTPToWPARAM(Id));
#else
...
void CXTPStatusBar::LoadState(LPCTSTR lpszProfileName)
{
	...
	if (m_bCustomizable)
	{
		...
		while (!strValue.IsEmpty())
		{
			...
			int nID = 0, bVisible = 0;
			int nResult = SCANF_S(strPane, _T("%i=%i"), &nID, &bVisible);

			CXTPStatusBarPane* pPane = (nResult == 2) && (nID != 0) ? FindPane(XTPToUInt(nID)) : NULL;
			if (pPane)
			{
...
BEGIN_DISPATCH_MAP(CXTPStatusBar, CControlBar)
...
BEGIN_INTERFACE_MAP(CXTPStatusBar, CControlBar)
	INTERFACE_PART(CXTPStatusBar, XTPDIID_IStatusBar, Dispatch)
	...
	XTP_DECLARE_CMDTARGETPROVIDER_INTERFACE_PART(CXTPStatusBar)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPStatusBar, XTPDIID_IStatusBar)
XTP_IMPLEMENT_CMDTARGETPROVIDER_INTERFACE(CXTPStatusBar)

IMPLEMENT_CONNECTION_EX(CXTPStatusBar, XTPDIID_IStatusBarEvents)
...
void CXTPStatusBar::OleSetFont(LPFONTDISP pFontDisp)
{
	...
	CFont font;
	if (AxCreateFontFromDispatch(&font, pFontDisp))
	{
		SetFont(&font);
	}
	else
