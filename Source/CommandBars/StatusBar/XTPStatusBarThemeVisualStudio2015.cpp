// XTPStatusBarThemeVisualStudio2015.cpp
//
...
CXTPStatusBarThemeVisualStudio2015::CXTPStatusBarThemeVisualStudio2015(CXTPPaintManager *pPaintManager)
	: CXTPStatusBarThemeOffice2013(pPaintManager)
{
}

void CXTPStatusBarThemeVisualStudio2015::RefreshMetrics()
{
	CXTPStatusBarThemeOffice2013::RefreshMetrics();

	m_clrGripperLite.SetStandardValue(XTPColorManager()->LightColor(m_clrBackground, GetXtremeColor(COLOR_WINDOW), 50));
	m_clrGripperDark.SetStandardValue(XTPColorManager()->LightColor(m_clrBackground, GetXtremeColor(COLOR_WINDOWTEXT), 75));
}

void CXTPStatusBarThemeVisualStudio2015::SetStateColor(COLORREF clrState)
{
	...
	else
	{
		...
		m_clrGripperLite         = XTPColorManager()->LightColor(clrState, GetXtremeColor(COLOR_WINDOW), 50);
		m_clrGripperDark         = XTPColorManager()->LightColor(clrState, GetXtremeColor(COLOR_WINDOWTEXT), 75);
...
COLORREF CXTPStatusBarThemeVisualStudio2015::GetStateColor() const
{
	return (m_clrTextGray == m_clrBackground
		&& m_clrTextGray == m_clrButtonBack
		&& m_clrTextGray == m_clrGripperLite
		&& m_clrTextGray == m_clrGripperDark
		&& m_clrTextGray == m_clrButtonBackPushed
		&& m_clrTextGray == m_clrButtonBackHighlight
		&& m_clrTextGray == m_clrButtonBackChecked ? m_clrTextGray : COLORREF_NULL);
}

void CXTPStatusBarThemeVisualStudio2015::DrawStatusBarGripper(CDC *pDC, CRect rcClient)
{
