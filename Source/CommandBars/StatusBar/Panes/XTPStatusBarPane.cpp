...
IMPLEMENT_DYNAMIC(CXTPStatusBarPane, CXTPCmdTarget)
...
CXTPStatusBarPane::CXTPStatusBarPane()
{
	...
	m_rcPadding.SetRect(XTP_DPI_X(3), XTP_DPI_Y(1), XTP_DPI_X(3), XTP_DPI_Y(1));
...
int CXTPStatusBarPane::GetBestFit()
{
	...
	if (pIcon) cxText += XTP_DPI_X(pIcon->GetWidth()) + (!m_strText.IsEmpty() ? XTP_DPI_X(2) : 0);
...
{
	ASSERT(m_pStatusBar);
	return (m_pStatusBar && m_xtpFontText.m_hObject == NULL) ?
		m_pStatusBar->GetFont(): &m_xtpFontText;
}
...
void CXTPStatusBarPane::SetTextFont(PLOGFONT pLogfText)
{
	...
	m_xtpFontText.CreateFontIndirect(pLogfText);
...
{
	if (m_nIconIndex != -1)
		return m_pStatusBar->GetImageManager()->GetImage(XTPToUIntChecked(m_nIconIndex), 0);
...
BEGIN_DISPATCH_MAP(CXTPStatusBarPane, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPStatusBarPane, "Dark", 26, IsDark, SetDark, VT_BOOL)
...
BEGIN_INTERFACE_MAP(CXTPStatusBarPane, CXTPCmdTarget)
	INTERFACE_PART(CXTPStatusBarPane, XTPDIID_StatusBarPane, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPStatusBarPane, XTPDIID_StatusBarPane)
...
{
	return m_xtpFontText.GetSafeHandle() ? AxCreateOleFont(&m_xtpFontText, this, (LPFNFONTCHANGED)&CXTPStatusBarPane::OleSetFont)
