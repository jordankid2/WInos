...
{
	m_clrText               .SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("TextColor"),           RGB(255, 255, 255)));
	m_clrTextGray           .SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("TextGrayColor"),       RGB( 43,  87, 154)));
	m_clrBackground         .SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("BackgroundColor"),     RGB( 43,  87, 154)));
	m_clrButtonBack         .SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("ButtonBack"),          RGB( 43,  87, 154)));
	m_clrButtonBackChecked  .SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("ButtonBackChecked"),   RGB( 25,  71, 138)));
	m_clrButtonBackPushed   .SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("ButtonBackPushed"),    RGB( 25,  71, 138)));
	m_clrButtonBackHighlight.SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("ButtonBackHighlight"), RGB( 62, 109, 181)));
	m_clrButtonTextChecked  .SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("ButtonTextChecked"),   RGB( 68,  68,  68)));
	m_clrButtonTextPushed   .SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("ButtonTextPushed"),    RGB( 68,  68,  68)));
	m_clrButtonTextHighlight.SetStandardValue(XTPIniColor(_T("CommandBars.StatusBar"), _T("ButtonTextHighlight"), RGB( 68,  68,  68)));
	...
	else
	{
		for (int i=0; i<pPane->GetSwitchCount(); i++)
		{
			...
			BOOL bEnabled = pPane->IsEnabled() && pSwitch->bEnabled;
				 ...
				 if (pSwitch->bPressed)     pDC->FillSolidRect(rcItem, m_clrButtonBackPushed);
			else if (pSwitch->bHighlighted) pDC->FillSolidRect(rcItem, m_clrButtonBackHighlight);
			else if (pSwitch->bChecked)     pDC->FillSolidRect(rcItem, m_clrButtonBackChecked);
			else                            pDC->FillSolidRect(rcItem, m_clrButtonBack);
