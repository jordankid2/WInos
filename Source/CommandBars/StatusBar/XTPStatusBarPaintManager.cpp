...
{
	m_clrText       .SetStandardValue(XTPIniColor(_T("StatusBar"), _T("TextColor"),       ::GetSysColor(COLOR_BTNTEXT)));
	m_clrTextGray   .SetStandardValue(XTPIniColor(_T("StatusBar"), _T("TextGrayColor"),   ::GetSysColor(COLOR_GRAYTEXT)));
	m_clrBackground .SetStandardValue(XTPIniColor(_T("StatusBar"), _T("BackgroundColor"), ::GetSysColor(COLOR_BTNFACE)));
	m_clrGripperLite.SetStandardValue(XTPIniColor(_T("StatusBar"), _T("GripperLite"),     ::GetSysColor(COLOR_BTNHIGHLIGHT)));
	m_clrGripperDark.SetStandardValue(XTPIniColor(_T("StatusBar"), _T("GripperDark"),     ::GetSysColor(COLOR_BTNSHADOW)));
}

void CXTPStatusBarPaintManager::FillStatusBar(CDC* pDC, CXTPStatusBar* pBar)
{
	if (m_pPaintManager->m_bThemedStatusBar && m_pPaintManager->m_themeStatusBar->IsAppThemeActive())
	{
...
void CXTPStatusBarPaintManager::DrawStatusBarGripper(CDC* pDC, CRect rcClient)
{
	if (m_pPaintManager->m_bThemedStatusBar && m_pPaintManager->m_themeStatusBar->IsAppThemeActive())
	{
		CRect rcGripper(rcClient.right - XTP_DPI_X(18), rcClient.top, rcClient.right, rcClient.bottom);
	...
	else
	{
		...
		for (i = 0; i < 3; i++)
		{
			...
			pDC->MoveTo(rcClient.Width() - 3 - i * szGripperStep.cx, rcClient.Height() - szGripperMargin.cy);
			pDC->LineTo(rcClient.Width() - szGripperMargin.cx, rcClient.Height() - 3 - i * szGripperStep.cy);

			pDC->MoveTo(rcClient.Width() - 4 - i * szGripperStep.cx, rcClient.Height() - szGripperMargin.cy);
			pDC->LineTo(rcClient.Width() - szGripperMargin.cx, rcClient.Height() - 4 - i * szGripperStep.cy);
		}

		CXTPPenDC penBorderWhite(*pDC, m_clrGripperLite);
		for (i = 0; i < 3; i++)
		{
			pDC->MoveTo(rcClient.Width() - 5 - i * szGripperStep.cx, rcClient.Height() - szGripperMargin.cy);
			pDC->LineTo(rcClient.Width() - szGripperMargin.cx, rcClient.Height() - 5 - i * szGripperStep.cy);
		}
	}
}
...
COLORREF CXTPStatusBarPaintManager::GetTextColor(CXTPStatusBarPane *pPane)
{
	return pPane->IsButton() && (pPane->IsHighlighted() || pPane->IsPressed()) ?
		m_pPaintManager->GetRectangleTextColor(pPane->IsHighlighted(), pPane->IsPressed(), pPane->IsEnabled(), FALSE, FALSE, xtpBarTypeNormal, xtpBarBottom) : m_clrText;
}
...
void CXTPStatusBarPaintManager::DrawStatusBarPaneEntry(CDC* pDC, CRect rcItem, CXTPStatusBarPane* pPane)
{
	...
	COLORREF clrText = !bEnabled ? pPane->GetTextGrayColor() != (COLORREF)-1 ? pPane->GetTextGrayColor(): m_clrTextGray: pPane->GetTextColor() != (COLORREF)-1 ? pPane->GetTextColor() : clrNormalText;
...
int CXTPStatusBarPaintManager::DrawStatusBarSwitchPane(CDC* pDC, CRect rcItem, CXTPStatusBarSwitchPane* pPane)
{
	...
	if (pDC)
	{
		for (int i = 0; i < nCount; i++)
		{
			...
			if (pIcon)
			{
				CSize szSize(XTP_DPI(pIcon->GetExtent()));
				CPoint pt((rcItem.left + rcItem.right - szSize.cx) / 2,
					(rcItem.top + rcItem.bottom - szSize.cy) / 2);
				pIcon->Draw(pDC, pt, pIcon->GetIcon(!bEnabled ? xtpImageDisabled : pSwitch->bChecked ? xtpImageChecked : xtpImageNormal), szSize);
			}
