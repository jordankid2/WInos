...
BOOL CXTPCustomizeCommandsPage::AddCategories(UINT nIDResource, BOOL bListSubItems)
{
	...
	{
		CString strCategory;
		if (XTPResourceManager()->GetMenuLocaleString(&menu, XTPToUInt(i), strCategory, MF_BYPOSITION) > 0)
		{
...
void CXTPCustomizeCommandsPage::OnCategoriesSelectionChanged()
{
	...
	XTP_COMMANDBARS_CATEGORYINFO* pInfo = GetCategoryInfo(XTPToIntChecked(m_lstCategories.GetItemData(nIndex)));
	...
	for (int i = 0; i < pInfo->pControls->GetCount(); i++)
	{
		...
		m_lstCommands.SendMessage(LB_INSERTSTRING, XTPToWPARAM(m_lstCommands.GetCount()),
