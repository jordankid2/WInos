...
void CXTPGroupLine::OnPaint()
{
	...
	dc.DrawText(strText, rc, UINT(DT_SINGLELINE | DT_VCENTER |
	...
	{
		dc.FillSolidRect(rcLine.left, rcLine.CenterPoint().y, rcLine.Width(), XTP_DPI_Y(1), clrLine);
	}
	else
	{
		dc.FillSolidRect(rcLine.left, rcLine.CenterPoint().y, rcLine.Width(), XTP_DPI_Y(1), GetSysColor(COLOR_3DSHADOW));
		dc.FillSolidRect(rcLine.left, rcLine.CenterPoint().y + XTP_DPI_Y(1), rcLine.Width(), XTP_DPI_Y(1), GetSysColor(COLOR_3DHIGHLIGHT));
	}
}
...
CXTPCustomizeOptionsPage::CXTPCustomizeOptionsPage(CXTPCustomizeSheet* pSheet) : CXTPPropertyPage(CXTPCustomizeOptionsPage::IDD)
{
...
{
	CString strPrompt;
	CXTPResourceManager::AssertValid(XTPResourceManager()->LoadString(&strPrompt, XTP_IDS_CONFIRM_RESETDATA));
