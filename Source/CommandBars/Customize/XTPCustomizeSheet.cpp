...
class CXTPCustomizeSheet::CCustomizeEdit : public CXTPControlEdit
{
	...
	void SendCommand(BOOL bTrackLost)
	{
		...
		tagNMCONTROL.hdr.idFrom = XTPToUIntPtrChecked(m_nId);
...
CXTPCustomizeSheet::CXTPCustomizeSheet(CXTPCommandBars* pCommandBars, UINT iSelectPage)
{
	...
	VERIFY(m_pCustomIcons->SetIcons(bmp, pIDs, XTPToIntChecked(nCount), CSize(16, 16), xtpImageNormal));
	delete[] pIDs;
...
void CXTPCustomizeSheet::ContextMenu(CPoint point)
{
	if (m_pContextMenu)
	{
		...
		CXTPCommandBars::TrackPopupMenu(m_pContextMenu, UINT(TPM_RECURSE | (bLayourRTL ? TPM_RIGHTALIGN : 0)), point.x, point.y, (CWnd*)m_pCommandBars->GetSite(), NULL, this);
	}
}
...
BOOL CXTPCustomizeSheet::FilterMouse(WPARAM wParam, LPARAM lParam)
{
	...
	if (((m_pWndParent->GetStyle() & WS_CHILD) == 0) && (!IsDescendant(m_pWndParent->m_hWnd, ::GetForegroundWindow())))
	{
		SetForegroundWindow();
		SetFocus();
	}
...
void CXTPCustomizeSheet::OnCustomizeEditImage()
{
	...
	if (pControl && pControl->GetIconId() != 0)
	{
		...
		CXTPImageManagerIcon* pImage = m_pCommandBars->GetImageManager()->GetImage(XTPToUInt(pControl->GetIconId()), szIcon.cx);
		...
		if (ed.DoModal() == IDOK)
		{
			...
			{
				pControl->m_nCustomIconId = XTPToInt(m_pCommandBars->GetImageManager()->AddCustomIcon(hIcon));
			}
			else
			{
...
void CXTPCustomizeSheet::OnCustomizeCopyImage()
{
	...
	if (pControl && pControl->GetIconId() != 0)
	{
		CXTPImageManagerIcon* pImage = m_pCommandBars->GetImageManager()->GetImage(XTPToUInt(pControl->GetIconId()));
		if (pImage)
		{
			CXTPImageManagerIconHandle hIcon;
	...
	if (wParam == XTP_ID_CUSTOMIZE_CHANGEBUTTONIMAGE)
	{
		...
		if (pControl)
		{
			...
			if (pIconSet && pIconSet->GetIcons()->GetCount() == 1)
			{
				...
				pControl->m_nCustomIconId = XTPToInt(m_pCommandBars->GetImageManager()->AddCustomIcon(pIcon->GetIcon()));
				pControl->GetParent()->OnRecalcLayout();
			}
		}
...
void CXTPCustomizeSheet::OnUpdateCustomizeName(CCmdUI* pCmdUI)
{
	...
	if (NULL != pCommandBar)
	{
		CXTPControl* pEditControl = pCommandBar->GetControls()->GetAt(XTPToIntChecked(pCmdUI->m_nIndex));
