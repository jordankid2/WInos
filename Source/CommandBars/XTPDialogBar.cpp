...
void CXTPDialogBar::CControlButtonHide::Draw(CDC* pDC)
{
	...
	XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolClose, m_rcControl, GetPaintManager()->GetControlTextColor(this));
}
...
CSize CXTPDialogBar::CalcSize(int nLength, DWORD dwMode, CSize szClient)
{
	...
	{
		pHide->SetRect(CRect(CPoint(nRightBorder - XTP_DPI_X(16), m_rcBorders.top + szBar.cy / 2 - XTP_DPI_Y(8)), XTP_DPI(CSize(16, 16))));
		nRightBorder -= XTP_DPI_X(16);
	}

	CControlCaptionPopup* pCaption = (CControlCaptionPopup*)m_pControls->FindControl(XTP_ID_DIALOGBAR_CAPTION);
	if (!pCaption)
	{
		pCaption = (CControlCaptionPopup*)m_pControls->Add(new CControlCaptionPopup(), XTP_ID_DIALOGBAR_CAPTION, _T(""), -1, TRUE);
	...
	pCaption->SetRect(CRect(m_rcBorders.left + szBar.cx, nCaptionTop, nRightBorder, nCaptionTop + nCaptionHeight));
...
{
	CMenu menu;
	VERIFY(menu.LoadMenu(XTPToUInt(nMenuResourceID)));
...
BEGIN_DISPATCH_MAP(CXTPDialogBar, CXTPToolBar)
	...
	DISP_PROPERTY_EX_ID(CXTPDialogBar, "CaptionPopupBar", 102, OleGetCaptionPopupBar, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDialogBar, "ChildHandle", 103, OleGetChildHandle, OleSetChildHandle, VT_HANDLE)
