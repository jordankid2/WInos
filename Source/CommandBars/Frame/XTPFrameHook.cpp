...
{
	return m_pCommandBars ? m_pCommandBars->GetPaintManager() : (m_bUseGlobalPaintManager 
		? XTPPaintManager() : m_pPaintManager);
}
...
void CXTPCommandBarsFrameHook::RepositionCaptionButtons()
{
	...
	for (int i = 0; i < m_pCaptionButtons->GetCount(); i++)
	{
		...
		{
			CSize szButton = GetFramePaintManager()->GetCaptionButtonSize(XTPToUInt(pControl->GetID()), m_hwndSite);
...
void CXTPCommandBarsFrameHook::OnFrameShadowDisabled(
...
int CXTPCommandBarsFrameHook::OnHookMessage(HWND hWnd, UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult)
{
	...
	if (nMessage == WM_ACTIVATE && NULL != GetShadow())
	{
		...
		if (NULL != pFramePaintManager)
		{
			...
			pFramePaintManager->OnShadowedFrameActiveStateChanged(
				bActivating, GetSite(), GetShadow());
		}
	}
	...
	if (nMessage == WM_NCHITTEST && IsCaptionVisible() && !m_bDwmEnabled)
	{
		...
		{
			if ((point.x < XTP_DPI_X(7) + GetSystemMetrics(SM_CXSMICON)) && (GetSiteStyle() & WS_SYSMENU))
				lResult = HTSYSMENU;
			else
				lResult = HTCAPTION;
		}
	...
	if (nMessage == WM_SYSCOMMAND && !m_bDwmEnabled && (wParam & 0xFFF0) == SC_MINIMIZE && GetSiteStyle() & WS_CHILD)
	{
		lResult = XTPHookManager()->Default();
...
void CXTPCommandBarsFrameHook::DisableShadow()
{
	...
	if (NULL != pShadow)
	{
		XTPFrameShadowManager()->DisableShadow(GetSite());
	}
}
