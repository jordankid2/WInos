...
void CXTPFrameThemeModernUI::LoadParts(HZIP hZip, CXTPMarkupContext *pMarkupContext)
{
	...
	// Frame caption buttons
	bMarkupLoaded = m_pButtonMinimize->LoadPart(hZip, _T("Frame/ButtonMinimize.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pButtonMaximize->LoadPart(hZip, _T("Frame/ButtonMaximize.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pButtonRestore->LoadPart(hZip, _T("Frame/ButtonRestore.xaml") , pMarkupContext);
	ASSERT(bMarkupLoaded);
...
void CXTPFrameThemeModernUI::RefreshMetrics()
{
	...
	m_clrFrameBorderInactive.SetStandardValue(pImages->GetImageColor(_T("Frame.Inactive"), _T("BorderColor")));
	m_clrFrameBorderActive  .SetStandardValue(pImages->GetImageColor(_T("Frame.Active"),   _T("BorderColor")));
