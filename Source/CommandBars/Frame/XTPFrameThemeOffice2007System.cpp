...
{
	// Caption bar
	BOOL bMarkupLoaded = m_pCaptionActive->LoadPart(hZip, _T("RibbonFrameCaptionActive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pCaptionInactive->LoadPart(hZip, _T("RibbonFrameCaptionInactive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	// Frame border
	bMarkupLoaded = m_pBorderLeftActive->LoadPart(hZip, _T("Frame/BorderLeftActive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderLeftInactive->LoadPart(hZip, _T("Frame/BorderLeftInactive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderLeftCaptionActive->LoadPart(hZip, _T("Frame/BorderLeftCaptionActive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderLeftCaptionInactive->LoadPart(hZip, _T("Frame/BorderLeftCaptionInactive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderTopActive->LoadPart(hZip, _T("Frame/BorderTopActive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderTopInactive->LoadPart(hZip, _T("Frame/BorderTopInactive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderRightActive->LoadPart(hZip, _T("Frame/BorderRightActive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderRightInactive->LoadPart(hZip, _T("Frame/BorderRightInactive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderRightCaptionActive->LoadPart(hZip, _T("Frame/BorderRightCaptionActive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderRightCaptionInactive->LoadPart(hZip, _T("Frame/BorderRightCaptionInactive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderBottomActive->LoadPart(hZip, _T("Frame/BorderBottomActive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pBorderBottomInactive->LoadPart(hZip, _T("Frame/BorderBottomInactive.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	// Frame caption
	bMarkupLoaded = m_pCaptionButtonSelected->LoadPart(hZip, _T("Frame/CaptionButtonSelected.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pCaptionButtonPressed->LoadPart(hZip, _T("Frame/CaptionButtonPressed.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGlyphMinimize9->LoadPart(hZip, _T("Frame/GlyphMinimize9.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGlyphMinimize11->LoadPart(hZip, _T("Frame/GlyphMinimize11.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGlyphMaximize9->LoadPart(hZip, _T("Frame/GlyphMaximize9.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGlyphMaximize11->LoadPart(hZip, _T("Frame/GlyphMaximize11.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGlyphRestore9->LoadPart(hZip, _T("Frame/GlyphRestore9.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGlyphRestore11->LoadPart(hZip, _T("Frame/GlyphRestore11.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);
...
HRGN CXTPFrameThemeOffice2007System::CalcFrameRegion(CXTPCommandBarsFrameHook *pFrameHook, CSize sz) const
{
	...
	SIZE_T nSizeBottomRect = bHasStatusBar ? sizeof(rgnRibbonBottomFrame) : sizeof(rgnSimpleBottomFrame);
...
void CXTPFrameThemeOffice2007System::DrawFrameCaptionButton(CDC *pDC, CRect rc, int nId, BOOL bSelected, BOOL bPressed, BOOL bActive)
{
	...
	switch(nId)
	{
	case SC_MINIMIZE: pGlyph = (nGlyphSize == 11 ? m_pGlyphMinimize11 : m_pGlyphMinimize9); break;
	case SC_MAXIMIZE: pGlyph = (nGlyphSize == 11 ? m_pGlyphMaximize11 : m_pGlyphMaximize9); break;
