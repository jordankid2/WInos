...
HRGN CXTPRibbonOffice2007FrameTheme::CalcFrameRegion(CXTPCommandBarsFrameHook *pFrameHook, CSize sz) const
{
	...
	RECT rgnTopFrame[] =
	{
		{ XTP_DPI_X(4), 0, cx - XTP_DPI_X(4), XTP_DPI_Y(1) },
...
void CXTPRibbonOffice2007FrameTheme::DrawFrame(CDC *pDC, CXTPCommandBarsFrameHook *pFrameHook)
{
	...
	if (bHasStatusBar)
	{
		if (!m_bFlatFrame)
		{
			...
			if (pImage)
			{
				...
				CRect rcLight(rc.left + XTP_DPI_X(1), rc.bottom - nStatusHeight - nBottomBorder, rc.left + nLeftBorder, rc.bottom - nBottomBorder);
			...
			if (pImage)
			{
				...
				CRect rcDark(rc.right - nRightBorder, rc.bottom - nStatusHeight - nBottomBorder, rc.right - XTP_DPI_X(1), rc.bottom - nBottomBorder);
				pImage->DrawImage(pDC, rcDark, rcSrc, CRect(0, 0, 0, 0));
			}
		}
		else
		{
			CRect rcLight(rc.left + XTP_DPI_X(1), rc.bottom - nStatusHeight - nBottomBorder, rc.left + nLeftBorder, rc.bottom - nBottomBorder + XTP_DPI_Y(1));
			pDC->FillSolidRect(rcLight, bActive ? m_clrFrameBorderActive2 : m_clrFrameBorderInactive2);

			CRect rcDark(rc.right - nRightBorder, rc.bottom - nStatusHeight - nBottomBorder, rc.right - XTP_DPI_X(1), rc.bottom - nBottomBorder + XTP_DPI_Y(1));
			pDC->FillSolidRect(rcDark, bActive ? m_clrFrameBorderActive2 : m_clrFrameBorderInactive2);

			if (nBottomBorder > XTP_DPI_Y(1))
			{
				pDC->FillSolidRect(rc.left + nLeftBorder - XTP_DPI_X(1), rc.bottom - nStatusHeight - nBottomBorder, XTP_DPI_X
					(1), nStatusHeight, bActive ? m_clrFrameBorderActive3 : m_clrFrameBorderInactive3);
				pDC->FillSolidRect(rc.right - nRightBorder, rc.bottom - nStatusHeight - nBottomBorder, 
					XTP_DPI_X(1), nStatusHeight, bActive ? m_clrFrameBorderActive3 : m_clrFrameBorderInactive3);
	...
	if (bHasStatusBar || m_bRoundedCornersAlways)
	{
		...
		if (nRightBorder > XTP_DPI_X(3))
		{
			...
			{
				rcSrc = pImage->GetSource(bActive ? 0 : 1, 2);
				CRect rcBottomRight(rc.right - XTP_DPI_X(rcSrc.Width()), rc.bottom - XTP_DPI_Y(rcSrc.Height()), rc.right, rc.bottom);
...
void CXTPRibbonOffice2007FrameTheme::DrawFrameCaptionButton(CDC *pDC, CRect rc, int nId, BOOL bSelected, BOOL bPressed, BOOL bActive)
{
	...
	int nGlyphSize = 23;
	if (rc.Width() < XTP_DPI_X(27)) nGlyphSize = 17;
	if (rc.Width() < XTP_DPI_X(15)) nGlyphSize = 13;
