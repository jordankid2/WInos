...
CXTPFramePaintManager::CXTPFramePaintManager(CXTPPaintManager* pPaintManager)
	...
	, m_szMDIClildWindowCaptionButtonSize(::GetSystemMetrics(SM_CYSIZE), ::GetSystemMetrics(SM_CYSIZE))
{
...
{
	m_rcIconMargin          = XTP_DPI(XTPIniRect(_T("CommandBars.Frame"), _T("IconMargin"), XTP_UNDPI(m_rcIconMargin)));
	m_rcCaptionButtonMargin = XTP_DPI(XTPIniRect(_T("CommandBars.Frame"), _T("CaptionButtonMargin"), XTP_UNDPI(m_rcCaptionButtonMargin)));
	m_szCaptionButtonSize = XTP_DPI(XTPIniSize(_T("CommandBars.Frame"), _T("CaptionButtonSize"), XTP_UNDPI(m_szCaptionButtonSize)));
	m_nFrameCaptionHeight = XTP_DPI_Y(XTPIniInt(_T("CommandBars.Frame"), _T("CaptionHeight"), XTP_UNDPI_Y(m_nFrameCaptionHeight)));
	m_bCaptionCenterAligned = XTPIniInt(_T("CommandBars.Frame"), _T("CaptionCenterAligned"), m_bCaptionCenterAligned);
}
...
void CXTPFramePaintManager::DrawCaptionText(CDC *pDC, CRect rcCaptionText, CWnd *pSite, BOOL bActive)
{
	...
	{
		pDC->SetTextColor(!bActive ? m_clrFrameCaptionTextInactiveBSMode : m_clrFrameCaptionTextActiveBSMode);
		if (rcCaptionText.left < m_pPaintManager->GetBackstageModeMenuWidth())
		{
			rcCaptionText.left = m_pPaintManager->GetBackstageModeMenuWidth() + XTP_DPI_X(5);
			rcCaptionText.right += (m_pPaintManager->GetBackstageModeMenuWidth() + XTP_DPI_X(5));
			CRect rc;
...
void CXTPFramePaintManager::OnShadowedFrameActiveStateChanged(
	BOOL bActive,
	CWnd* pFrame,
