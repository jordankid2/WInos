...
{
	m_clrFrameCaptionActive   = XTPIniColor(_T("Frame.Active"),   _T("CaptionColor"), RGB(239, 239, 242));
	m_clrFrameCaptionInactive = XTPIniColor(_T("Frame.Inactive"), _T("CaptionColor"), RGB(239, 239, 242));
}
...
CXTPFrameShadow* CXTPFrameThemeVisualStudio2012::CreateShadow(CWnd* pFrame)
{
	...
	if (XTPFrameShadowManager()->IsShadowEnabled(pFrame))
	{
		pShadow = XTPFrameShadowManager()->GetShadow(pFrame);

		if (pShadow)
		{
			pShadow->SetColor(VS2012ActiveShadowColor);
			pShadow->SetOuterSize(XTPToUInt(XTP_DPI_Y(5)));
			pShadow->SetInnerSize(XTPToUInt(XTP_DPI_Y(3)));
			pShadow->SetOffset(0);
			pShadow->SetTransparency(0x80);
		}
	}
	else
	{
		pShadow = XTPFrameShadowManager()
			->EnableShadow(pFrame, VS2012ActiveShadowColor,
			XTPToUInt(XTP_DPI_Y(5)), XTPToUInt(XTP_DPI_Y(3)), 0, 0x80);
	}

	return pShadow;
}

void CXTPFrameThemeVisualStudio2012::OnShadowedFrameActiveStateChanged(
	BOOL bActive,
	CWnd* pFrame,
