...
{
	XTP_SHORTCUTMANAGER_ACCEL* pAccels = new XTP_SHORTCUTMANAGER_ACCEL[XTPToUIntChecked(GetCount())];
...
void CXTPShortcutManagerAccelTable::CopyAccelTable(HACCEL hAccelTable)
{
	...
	LPACCEL lpAccel = new ACCEL[XTPToUInt(nAccelSize)];
...
#ifndef _XTP_ACTIVEX
			SendMessage(hWnd, WM_COMMAND, XTPToWPARAM(accel->cmd), 0);
#else
...
CString CXTPShortcutManager::CKeyHelper::GetLocalKeyNameText(UINT uiVirtKey)
{
	...
	::GetKeyNameText(XTPToLong(nScanCode), szBuffer, BUFFER_LEN);
...
UINT CXTPShortcutManager::AccelKeyToVirtualKey(TCHAR chAccel)
{
	int nLayoutCount = GetKeyboardLayoutList(0, 0);
	if (nLayoutCount >= 1)
	{
		...
		for (int i = 0; i < nLayoutCount; i++)
		{
			short key = VkKeyScanEx(chAccel, arrLayoutList[i]);
			if (key < 0)
				continue;
			WORD chKey = LOWORD(key);
			chKey = WORD(chKey & 0xFF);
			chKey = static_cast<WORD>(ToUpper(static_cast<TCHAR>(chKey)));
			return chKey;
		}
	}
...
void CXTPShortcutManager::LoadShortcuts(LPCTSTR lpszProfileName)
{
	...
	UINT nVersion = AfxGetApp()->GetProfileInt(lpszProfileName, _T("AcceleratorsVersion"), 0);
	...
	if (nVersion == 0)
	{
		...
		{
			int nAccelSize = XTPToIntChecked(uiSize / sizeof(ACCEL));
			ASSERT (lpAccel != NULL);
	...
	else if (nVersion == 1)
	{
		...
		if (AfxGetApp()->GetProfileBinary(lpszProfileName,  _T("Accelerators"), (LPBYTE*) &lpAccel, &uiSize))
		{
			int nAccelSize = XTPToIntChecked(uiSize / sizeof(XTP_SHORTCUTMANAGER_ACCEL));
			ASSERT (lpAccel != NULL);
...
{
	CString str = m_pKeyNameText->Translate(XTPToUIntChecked(nAccel)) + _T('+');
...
void CXTPShortcutManager::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->IsStoring())
	{
		...
		if (pPX->IsAllowBlobValues())
		{
			...
			pPX->WriteCount(XTPToDWORDChecked(nAccelSize));
		...
		else
		{
			...
			POSITION posEnum = pEnumerator->GetPosition(XTPToDWORDChecked(nAccelSize));
...
BEGIN_INTERFACE_MAP(CXTPShortcutManager, CXTPCmdTarget)
	INTERFACE_PART(CXTPShortcutManager, XTPDIID_IKeyBindings, Dispatch)
	INTERFACE_PART(CXTPShortcutManager, IID_IConnectionPointContainer, ConnPtContainer)
END_INTERFACE_MAP()

BEGIN_DISPATCH_MAP(CXTPShortcutManager, CXTPCmdTarget)
...
IMPLEMENT_OLETYPELIB_EX(CXTPShortcutManager, XTPDIID_IKeyBindings)

IMPLEMENT_CONNECTION_EX(CXTPShortcutManager, XTPDIID_IKeyBindingsEvents)
...
BEGIN_DISPATCH_MAP(CXTPShortcutManagerAccel, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPShortcutManagerAccel, CXTPCmdTarget)
	INTERFACE_PART(CXTPShortcutManagerAccel, XTPDIID_IKeyBinding, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPShortcutManagerAccel, XTPDIID_IKeyBinding)
