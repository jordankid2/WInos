...
#include "CommandBars/XTPDockState.h"
...
CXTPToolBar::CToolBarInfo::CToolBarInfo()
{
	...
	m_bDialogBar = FALSE;
	m_szDialogBarFloatingClient = m_szDialogBarDockingClient = XTP_DPI(CSize(200, 200));
}
...
void CXTPToolBar::CToolBarInfo::DoPropExchange(CXTPPropExchange* pPX, CXTPDockState* pDockState)
{
	...
	PX_DPI_Rect(pPX, _T("MRUDockPos"), m_rectMRUDockPos, CRect(0, 0, 0, 0));
	PX_DPI_Point(pPX, _T("MRUFloatPos"), m_ptMRUFloatPos, CPoint(0, 0));
	...
	if (pDockState->GetVersion() > _XTP_SCHEMA_87)
	{
		...
		if (m_bDialogBar)
		{
			PX_DPI_Size(pPX, _T("DialogBarDockingClientSize"), m_szDialogBarDockingClient, CSize(0, 0));
			PX_DPI_Size(pPX, _T("DialogBarFloatingClientSize"), m_szDialogBarFloatingClient, CSize(0, 0));
		}
	}
}
...
BOOL CXTPToolBar::CToolBarInfo::SaveState(LPCTSTR lpszProfileName, int nIndex)
{
	...
	pApp->WriteProfileInt(szSection, _xtpBarID, XTPToInt(m_nBarID));
	if (!m_bVisible)
	...
	if (m_nMRUWidth != 32767)
		pApp->WriteProfileInt(szSection, _xtpMRUWidth, XTPToInt(m_nMRUWidth));
...
BOOL CXTPToolBar::CToolBarInfo::LoadState(LPCTSTR lpszProfileName, int nIndex, CXTPDockState* pDockState)
{
	...
	m_bVisible = !!pApp->GetProfileInt(szSection, _xtpVisible, TRUE);
	m_bFloating = !!pApp->GetProfileInt(szSection, _xtpFloating, FALSE);
	m_pointPos = CPoint(
		XTPToInt(pApp->GetProfileInt(szSection, _xtpXPos, -1)),
		XTPToInt(pApp->GetProfileInt(szSection, _xtpYPos, -1)));
	...
	m_rectMRUDockPos = CRect(
		XTPToInt(pApp->GetProfileInt(szSection, _xtpMRUDockLeftPos, 0)),
		XTPToInt(pApp->GetProfileInt(szSection, _xtpMRUDockTopPos, 0)),
		XTPToInt(pApp->GetProfileInt(szSection, _xtpMRUDockRightPos, 0)),
		XTPToInt(pApp->GetProfileInt(szSection, _xtpMRUDockBottomPos, 0)));
	pDockState->ScaleRectPos(m_rectMRUDockPos);

	m_ptMRUFloatPos = CPoint(
		XTPToInt(pApp->GetProfileInt(szSection, _xtpMRUFloatXPos, 0)),
		XTPToInt(pApp->GetProfileInt(szSection, _xtpMRUFloatYPos, 0)));
	pDockState->ScalePoint(m_ptMRUFloatPos);

	m_bDialogBar = (0 != pApp->GetProfileInt(szSection, _xtpDialogBar, FALSE));
	if (m_bDialogBar)
	{
		m_szDialogBarDockingClient.cx = XTPToLong(pApp->GetProfileInt(szSection, _T("DialogBarDockingClientCx"), m_szDialogBarDockingClient.cx));
		m_szDialogBarDockingClient.cy = XTPToLong(pApp->GetProfileInt(szSection, _T("DialogBarDockingClientCy"), m_szDialogBarDockingClient.cy));
		m_szDialogBarFloatingClient.cx = XTPToLong(pApp->GetProfileInt(szSection, _T("DialogBarFloatingClientCx"), m_szDialogBarFloatingClient.cx));
		m_szDialogBarFloatingClient.cy = XTPToLong(pApp->GetProfileInt(szSection, _T("DialogBarFloatingClientCy"), m_szDialogBarFloatingClient.cy));
	}
...
void CXTPDockState::LoadState(LPCTSTR lpszProfileName)
{
	...
	TCHAR szSection[256];
	wsprintf(szSection, _xtpSummarySection, lpszProfileName);
	UINT nBars = pApp->GetProfileInt(szSection, _xtpBars, 0);

	CSize size;
	size.cx = XTPToLongChecked(pApp->GetProfileInt(szSection, _xtpScreenCX, 0));
	size.cy = XTPToLongChecked(pApp->GetProfileInt(szSection, _xtpScreenCY, 0));
	SetScreenSize(size);
	...
	for (UINT i = 0; i < nBars; i++)
	{
		...
		pInfo->LoadState(lpszProfileName, XTPToInt(i), this);
	}
}
...
void CXTPDockState::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->IsStoring())
	{
		...
		PX_DPI_Size(&secDockState, _T("ScreenSize"), sz, CSize(0, 0));
	...
	else
	{
		...
		CSize size;
		PX_DPI_Size(&secDockState, _T("ScreenSize"), size, CSize(0, 0));
		SetScreenSize(size);
...
// get state info
	...
	pInfo->m_nMRUWidth = XTPToUIntChecked(m_nMRUWidth);
...
{
	UINT nFlags = UINT((pInfo->m_bFloating ? 0 : SWP_NOSIZE) | SWP_NOACTIVATE | SWP_NOZORDER);

	if (!pInfo->m_bFloating && GetDockBar() == NULL)
		nFlags |= SWP_NOMOVE;

	m_nMRUWidth = XTPToIntChecked(pInfo->m_nMRUWidth);
...
BOOL CXTPCommandBars::GetControlsFileName(CString& strFileName, LPCTSTR lpszProfileName)
{
...
#endif
#ifdef _XTP_ACTIVEX
	if (g_pGlobalSettings == 0 || ((CCommandBarsGlobalSettings*)g_pGlobalSettings)->m_strEXEName.IsEmpty())
	{
		VERIFY(::GetModuleFileName(GetModuleHandle(0), szBuff, _MAX_PATH));

		CString strLower(strFileName);
		strLower.MakeLower();
		if (strLower.Find(_T("vb6.exe")) != -1)
			return FALSE;
	...
	{
		strFileName = ((CCommandBarsGlobalSettings*)g_pGlobalSettings)->m_strPath + _T("\\") +
			((CCommandBarsGlobalSettings*)g_pGlobalSettings)->m_strEXEName + _T(".exe-") + lpszProfileName;
	}
...
void CXTPCommandBars::LoadDockBarsState(LPCTSTR lpszProfileName)
{
	...
	for (int nIndex = 0; nIndex < 4; nIndex++)
	{
		...
		if (nBars > 1)
		{
			...
			{
				TCHAR buf[16];
				wsprintf(buf, _xtpBar, XTPToInt(i));
				...
				pDock->m_arrBars.InsertAt(XTPToInt(i), pBar);
			}
		}
	}
...
void CXTPCommandBars::DoPropExchange(CXTPPropExchange* pPX, XTP_COMMANDBARS_PROPEXCHANGE_PARAM* pParam)
{
	...
	else
	{
		...
		if (pParam->bSerializeControls)
		{
			...
			{
				BOOL bDirtyStateBeforeRestore = m_pOptions->bDirtyState;

				RestoreCommandBarList(&lstCommandBars, pParam->bLoadSilent);

				if (!bDirtyStateBeforeRestore && m_pOptions->bDirtyState)
				{
					bExplicitlyDirty = TRUE;
				}
...
BOOL CXTPCommandBars::SaveCommandBarList(CXTPPropExchange* pPX, CXTPCommandBarList* pCommandBarList)
{
	try
	{
		...
		long nCount = pCommandBarList->GetCount();
...
{
	// Call detailed implementation
	MergeToolBar(pCommandBar, bSilent, 0);
}

void CXTPToolBar::MergeToolBar(
	...
	int nDefaultConfirmationChoice)
{
		...
		&& GetControls()->GetOriginalControls()->Compare(pLastControls) == FALSE)
	{
		...
		if (pLastControls->Compare(pCommandBar->GetControls()) == FALSE)
		{
			int nChoice = nDefaultConfirmationChoice;
			if (!bSilent && IDYES != nChoice && IDNO != nChoice)
			{
				...
				nChoice = XTPResourceManager()->ShowMessageBox(strMessage, MB_YESNO | MB_ICONQUESTION);
			}
...
void CXTPCommandBars::RestoreCommandBarList(CXTPCommandBarList* pCommandBarList, BOOL bSilent)
{
	for (int i = 0; i < pCommandBarList->GetCount(); i++)
	{
		...
		if (pToolBar)
		{
			...
			if (pExistsBar)
			{
				int nDefaultConfirmationChoice = 0;
				if (m_bDefaultResetCustomizedToolbarChoiceEnabled)
				{
					nDefaultConfirmationChoice = (m_bDefaultResetCustomizedToolbarChoice ? IDYES : IDNO);
				}
				pExistsBar->MergeToolBar(pCommandBar, bSilent, nDefaultConfirmationChoice);
			}
			else
			{
...
void CXTPCommandBar::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_DPI_X_Int(pPX, _T("MRUWidth"), m_nMRUWidth, 0);
	...
	{
		PX_DPI_Size(pPX, _T("ButtonSize"), m_szButtons, CSize(0, 0));
...
void CXTPPopupBar::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_DPI_X_Int(pPX, _T("TearOffWidth"), m_nTearOffWidth, 0);

	if (pPX->GetSchema() > _XTP_SCHEMA_85)
	{
		PX_DPI_Rect(pPX, _T("Borders"), m_rcBorders, CRect(2, 4, 2, 4));
...
void CXTPControlAction::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->GetSchema() > _XTP_SCHEMA_1624)
	{
		PX_String(pPX, _T("CustomShortcutText"), m_strCustomShortcutText, _T(""));
	}
	else
	{
...
void CXTPControl::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (m_nCustomIconId > 0)
	{
		...
		pCommandBars->GetImageManager()->DoPropExchange(XTPToUInt(m_nCustomIconId), &secCustomIcon);
	}
	...
	if (pPX->GetSchema() > _XTP_SCHEMA_103)
	{
		...
		PX_DPI_Y_Int(pPX, _T("Height"), m_nHeight, 0);
...
void CXTPControlPopup::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->IsStoring())
	{
		long dwCommandBarID = m_pCommandBar ? XTPToLongChecked(m_pCommandBar->m_nBarID) : 0;
