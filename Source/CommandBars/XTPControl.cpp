...
LRESULT CXTPControl::NotifySite(CWnd* pSite, UINT code, NMXTPCONTROL* pNM)
{
	...
	pNM->hdr.code = code ;
	pNM->hdr.idFrom = XTPToUIntPtrChecked(GetID());
	pNM->hdr.hwndFrom = 0;
	pNM->pControl = this;
...
void CXTPControl::OnExecute()
{
	...
	if (pCommandBars)
	{
		if (m_nId) pCommandBars->SetCommandUsed(XTPToUInt(m_nId));
...
void CXTPControl::OnInvertTracker(CDC* pDC, CRect rect)
{
	...
	pDC->InvertRect(CRect(rect.left, rect.top + XTP_DPI_Y(2), rect.left + XTP_DPI_X(2), rect.bottom - XTP_DPI_Y(2)));
	pDC->InvertRect(CRect(rect.right - XTP_DPI_X(2), rect.top + XTP_DPI_Y(2), rect.right, rect.bottom - XTP_DPI_Y(2)));
...
void CXTPControl::OnCustomizeMouseMove(CPoint point)
{
	if (IsCustomizeResizeAllow() && m_rcControl.PtInRect(point) 
		&& ((point.x - m_rcControl.left <= XTP_DPI_X(2)) || (m_rcControl.right - point.x <= XTP_DPI_X(2))))
	{
...
{
	if (m_pParent && m_rcControl.PtInRect(point) && ((point.x - m_rcControl.left <= XTP_DPI_X(2)) || (m_rcControl.right - point.x <= XTP_DPI_X(2))))
	{
		CXTPCommandBars* pCommandBars = m_pParent->GetCommandBars();
		ASSERT(pCommandBars);
...
{
	CXTPControlAction* pAction = DYNAMIC_DOWNCAST(CXTPControlAction, CXTPControlAction::FromIDispatchSafe(lpDisp));
	if (NULL == pAction)
	{
