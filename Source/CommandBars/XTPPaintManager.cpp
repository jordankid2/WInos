...
void CXTPPaintManagerFont::SetCustomFont(BOOL bCustomFont)
{
...
#ifdef _XTP_INCLUDE_MARKUP
		, CXTPSingletonDependencies<
			CXTPSingleton<CXTPGdiPlus>
		>
#endif
	>
{
...
private:
	CGlobalPaintManagerPtr() : m_pPaintManager(NULL)
	{
	}
...
void CXTPPaintManager::RefreshMetrics()
{
	...
	if (m_pScrollBarPaintManager)  m_pScrollBarPaintManager->RefreshMetrics();
	if (m_pGalleryPaintManager)    m_pGalleryPaintManager->RefreshMetrics();
	if (m_pSliderPaintManager)     m_pSliderPaintManager->RefreshMetrics();
	if (m_pProgressPaintManager)   m_pProgressPaintManager->RefreshMetrics();
	if (m_pFramePaintManager)      m_pFramePaintManager->RefreshMetrics();
 	if (m_pStatusBarPaintManager)  m_pStatusBarPaintManager->RefreshMetrics();
	if (m_pMessageBarPaintManager) m_pMessageBarPaintManager->RefreshMetrics();
...
#ifdef _XTP_INCLUDE_RIBBON
	if (m_pRibbonPaintManager) m_pRibbonPaintManager->RefreshMetrics();
#endif
}
...
{
#if defined(_XTP_ACTIVEX) && defined(_XTP_INCLUDE_RIBBON)
	...
	switch(paintTheme)
	{
		case xtpThemeOfficeXP:
			pInstance = new CXTPCommandBarsOfficeXPTheme();
			break;
		case xtpThemeOffice2003:
			pInstance = new CXTPCommandBarsOffice2003Theme();
			break;
		...
		case xtpThemeNativeWinXP:
			pInstance = new CXTPCommandBarsNativeXPTheme();
			break;
		case xtpThemeVisualStudio6:
			pInstance = new CXTPVisualStudio6Theme();
			break;
		case xtpThemeVisualStudio2005:
			pInstance = new CXTPVisualStudio2005Theme();
			break;
		case xtpThemeVisualStudio2008:
			pInstance = new CXTPVisualStudio2008Theme();
			break;
		case xtpThemeVisualStudio2010:
			pInstance = new CXTPVisualStudio2010Theme();
			break;
		...
		case xtpThemeCustom:
		default:
			pInstance = new CXTPCommandBarsOffice2000Theme();
			break;
	}
...
void CXTPPaintManager::SetTheme(XTPPaintTheme paintTheme)
{
	...
	if (XTPResourceImages()->m_strIniFileName.Find(_T("OFFICE2010")) != -1)
	{
		...
		pPaintManager->m_pMessageBarPaintManager = new CXTPMessageBarThemeOffice2010(XTPPaintManager());
...
{
	Rectangle(pDC, rc,
		(nPen == -1 ? COLORREF_NULL : GetXtremeColor(XTPToUIntChecked(nPen))),
...
void CXTPPaintManager::Draw3dRect(CDC* pDC, LPCRECT rc, int frameThickness, COLORREF clrPen, COLORREF clrBrush)
{
	if (clrBrush != COLORREF_NULL)
	{
		CRect r(rc);
...
{
	XTPPrimitiveDrawer()->DrawSymbol(pDC, bLayoutRTL 
		? xtpPrimitiveSymbolCheckMarkRTL : xtpPrimitiveSymbolCheckMark, rc, clr);
}
...
void CXTPPaintManager::DrawPopupBarRadioMark(CDC* pDC, CRect rcCheck, BOOL /*bEnabled*/, COLORREF clr)
{
	...
	CRect rcRadio(pt.x - XTP_DPI_X(3), pt.y - XTP_DPI_Y(3), pt.x + XTP_DPI_X(3), pt.y + XTP_DPI_Y(3));
...
void CXTPPaintManager::DrawControlText3(CDC* pDC, CXTPControl* pButton, const CString& str, const CString& strClear, CRect rcText, BOOL bVert, BOOL bCentered)
{
	...
	if (!bVert)
	{
		if (bEmbossed)
		{
			...
			pDC->DrawText(strClear, rcText + XTP_DPI(CPoint(1, 1)), nFormat | DT_VCENTER | DT_NOPREFIX);
	...
	else
	{
		if (bEmbossed)
		{
			COLORREF clrText = pDC->GetTextColor();
			pDC->SetTextColor(GetXtremeColor(COLOR_3DHIGHLIGHT));
			pDC->DrawText(strClear, rcText + XTP_DPI(CPoint(1, 1)), nFormat | DT_NOPREFIX | DT_NOCLIP);
			pDC->SetTextColor(clrText);
		}
...
void CXTPPaintManager::DrawDropDownGlyph(CDC* pDC, CXTPControl* pControl, CPoint pt, BOOL bSelected, BOOL bPopuped, BOOL bEnabled, BOOL bVert)
{
	...
	XTPPrimitiveDrawer()->DrawSymbol(pDC, symb,
		CRect(pt.x - size, pt.y - size, pt.x + size, pt.y + size), clr);
}
...
XTPPrimitiveSymbol CXTPPaintManager::GetControlSymbol(XTPBarPosition barPosition, BOOL bVert) const
{
	...
	{
		case xtpBarTop:
			symb = xtpPrimitiveSymbolPlayDown;
			break;
		case xtpBarBottom:
			symb = xtpPrimitiveSymbolPlayUp;
			break;
		case xtpBarLeft:
			symb = xtpPrimitiveSymbolPlayRight;
			break;
		case xtpBarRight:
			symb = xtpPrimitiveSymbolPlayLeft;
			break;
		case xtpBarFloating:
		case xtpBarPopup:
		case xtpBarListBox:
		case xtpBarNone:
		default:
			symb = bVert ? xtpPrimitiveSymbolPlayLeft : xtpPrimitiveSymbolPlayDown;
			break;
	};
...
CSize CXTPPaintManager::DrawControlText2(CDC* pDC, CXTPControl* pButton, CRect rcText, BOOL bDraw, BOOL bVert, BOOL bTriangled)
{
	...
	if (!bVert)
	{
		...
		//Added 2px offset on first row for all buttons (with one or two text rows) to fix issue under high DPI.
		...
		int nSecondRowWidth = pDC->GetTextExtent(strSecondRowClear).cx + (bTriangled ? XTP_COMMANDBAR_TRIANGLE_SIZE : 0);
		...
		if (bTriangled)
		{
			CPoint pt = CPoint(strSecondRow.IsEmpty() 
				? rcText.CenterPoint().x - XTP_DPI_X(1)
				: rcSecondRowText.left + nSecondRowWidth - XTP_COMMANDBAR_TRIANGLE_SIZE / 2, rcSecondRowText.CenterPoint().y);
			if (strSecondRow.IsEmpty())
			{
	...
	{
		CRect rcSecondRowText(rcText.left + XTP_DPI_X(1), rcText.top, rcText.CenterPoint().x + XTP_DPI_X(1), rcText.bottom);
		CRect rcFirstRowText(rcText.CenterPoint().x, rcText.top, rcText.right - XTP_DPI_X(1), rcText.bottom);

		CSize szText = pDC->GetTextExtent (strFirstRowClear);
		rcFirstRowText.left = rcFirstRowText.right - (rcFirstRowText.Width() - szText.cy + XTP_DPI_X(1)) / 2;
		rcFirstRowText.top = rcFirstRowText.top + (rcFirstRowText.Height() - szText.cx + XTP_DPI_Y(1)) / 2;
		...
		rcSecondRowText.left = rcSecondRowText.right - (rcSecondRowText.Width() - szText.cy + XTP_DPI_Y(1)) / 2;
		rcSecondRowText.top = rcSecondRowText.top + (rcSecondRowText.Height() - szText.cx + XTP_DPI_X(1)) / 2;
		DrawControlText3(pDC, pButton, strSecondRow, strSecondRowClear, rcSecondRowText, bVert, FALSE);

		if (bTriangled)
		{
			CPoint pt = CPoint(
				rcSecondRow.CenterPoint().x - XTP_DPI_X(2),
...
CSize CXTPPaintManager::DrawControlText(CDC* pDC, CXTPControl* pButton, CRect rcText, BOOL bDraw, BOOL bVert, BOOL bCentered, BOOL bTriangled)
{
	...
	if (!bDraw)
	{
		...
		if (bTriangled) sz.cx += m_bOffice2007Padding ? XTP_COMMANDBAR_TRIANGLE_SIZE : XTP_COMMANDBAR_TRIANGLE_SIZE + XTP_DPI_X(2);
		return bVert ? CSize(sz.cy, sz.cx) : sz;
	}
...
void CXTPPaintManager::SetFontHeight(int nFontHeight)
{
	...
	if (m_pFramePaintManager)
	{
		...
		{
			m_pFramePaintManager->m_fntFrameCaption.SetCustomFont(NULL);
		}
	}	
}
...
void CXTPPaintManager::UpdateFonts()
{
	...
	BOOL bUseClearType = m_bClearTypeTextQuality && XTPSystemVersion()->IsClearTypeTextQualitySupported();
...
void CXTPPaintManager::DrawGalleryItemBack(CDC* pDC, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked, XTPBarType barType)
{
	DrawRectangle(pDC, rcItem, bSelected, bPressed, bEnabled, bChecked, FALSE, barType, xtpBarPopup);
}
...
void CXTPPaintManager::DrawCommandBarGroupSeparator(CDC* pDC, CXTPCommandBar* pBar, CXTPControl* pLeftControl, CXTPControl* pRightControl, BOOL bDraw /*= TRUE*/)
{
	UNREFERENCED_PARAMETER(pDC);
...
CSize CXTPPaintManager::DrawControlEdit(CDC* pDC, CXTPControlEdit* pControlEdit, BOOL bDraw)
{
	if (bDraw)
	{
		...
		else
		{
			...
			if (pControlEdit->IsImageVisible())
			{
				...
				szIcon = GetStretchIconSizeDPIAwareness(pImage, szIcon, pControlEdit, rc.Size());
	...
	int nHeight = m_nEditHeight > 0 ? m_nEditHeight : pDC->GetTextExtent(_T(" "), 1).cy + XTP_DPI_Y(7);
...
CSize CXTPPaintManager::DrawControlComboBox(CDC* pDC, CXTPControlComboBox* pControlCombo, BOOL bDraw)
{
	if (bDraw)
	{
		...
		else
		{
			...
			if (pControlCombo->IsImageVisible())
			{
				...
				szIcon = GetStretchIconSizeDPIAwareness(pImage, szIcon, pControlCombo, rc.Size());
	...
	CXTPFontDC font(pDC, GetIconFont());
	int nHeight = m_nEditHeight > 0 ? m_nEditHeight : max(XTP_DPI_Y(20), pDC->GetTextExtent(_T(" "), 1).cy + XTP_DPI_Y(7));

	pControlCombo->SetThumbWidth(max(XTP_DPI_X(15), MulDiv(nHeight, XTP_DPI_X(9), XTP_DPI_X(14)) + XTP_DPI_X(1)));
...
XTPBarTextOrientation CXTPPaintManager::GetControlTextOrientation(CXTPControl* pControl)
{
	...
	return (pControl->GetParent()->GetPosition() == xtpBarLeft || pControl->GetParent()->GetPosition() == xtpBarRight) ? xtpBarTextVertical : xtpBarTextHorizontal;
}

CFont* CXTPPaintManager::GetControlFont(CXTPControl* pControl)
{
	return GetControlTextOrientation(pControl) == xtpBarTextVertical ? (pControl->IsItemDefault() ? &m_xtpFontVertBold: &m_xtpFontVert) : (pControl->IsItemDefault() ? &m_xtpFontRegularBold : &m_xtpFontRegular);
}
...
CFont* CXTPPaintManager::GetCommandBarFont(CXTPCommandBar* pCommandBar)
{
	...
	if (nTextOrientation == xtpBarTextAuto)
	{
		nTextOrientation = pCommandBar->GetPosition() == xtpBarLeft || pCommandBar->GetPosition() == xtpBarRight ? xtpBarTextVertical : xtpBarTextHorizontal;
	}

	return nTextOrientation == xtpBarTextVertical ? &m_xtpFontVert : &m_xtpFontRegular;
}

void CXTPPaintManager::DrawExpandGlyph(CDC* pDC, CXTPControl* pButton, CPoint pt0, CPoint pt1, CPoint pt2, COLORREF clr)
{
	ASSERT(pDC);
	UNREFERENCED_PARAMETER(pButton);
...
void CXTPPaintManager::DrawExpandGlyph(
	CDC* pDC,
	CXTPControl* pButton,
	COLORREF clr,
	BOOL bVertical)
{
	...
	XTPPrimitiveDrawer()->DrawSymbol(pDC, bVertical ? xtpPrimitiveSymbolPlayDown 
		: xtpPrimitiveSymbolPlayRight, rc, clr);
}
...
CSize CXTPPaintManager::DrawSpecialControl(CDC* pDC, XTPSpecialControl controlType, CXTPControl* pButton, CXTPCommandBar* pBar, BOOL bDraw, LPVOID lpParam)
{
	if (controlType == xtpButtonExpandToolbar)
	{
		if (!bDraw)
		{
			...
			if (!IsVerticalPosition(pBar->GetPosition()))
				pButton->SetRect(CRect(szBar.cx - XTP_DPI_X(11) - XTP_DPI_X(2), XTP_DPI_Y(2), szBar.cx - XTP_DPI_X(2), szBar.cy - XTP_DPI_Y(2)));
			else
				pButton->SetRect(CRect(XTP_DPI_X(2), szBar.cy - XTP_DPI_Y(11) - XTP_DPI_Y(2), szBar.cx - XTP_DPI_X(2), szBar.cy - XTP_DPI_Y(2)));
		}
		else
		{
	...
	else if (controlType == xtpButtonHideFloating)
	{
		...
		COLORREF crClose = (!pButton->GetPressed() && !pButton->GetSelected()
			? m_clrFloatingGripperText : GetControlTextColor(pButton));
...
CSize CXTPPaintManager::DrawControlText(CDC* pDC, CXTPControl* pButton, CRect rcText, BOOL bDraw, BOOL bTriangled, CSize szImage, BOOL bDrawImage)
{
	...
	if (!bVert)
	{
		if (bDrawImage)
		{
			...
			return CSize(XTP_DPI_X(m_bOffice2007Padding ? 4 : 3) + sz.cx + szImage.cx, max(sz.cy + XTP_DPI_Y(6), szImage.cy));
		}
		else
		{
...
CSize CXTPPaintManager::DrawControlCheckBoxMark(CDC* pDC, CXTPControl* pButton, CRect rc, BOOL bDraw)
{
...
CSize CXTPPaintManager::DrawControlCheckBoxMark(CDC* pDC, CRect rc, BOOL bDraw, BOOL bSelected, BOOL bPressed, BOOL bChecked, BOOL bEnabled)
{
	if (bDraw)
	{
		...
		{
			pDC->DrawFrameControl(rc, UINT(DFC_BUTTON), UINT(DFCS_BUTTONCHECK |
...
CSize CXTPPaintManager::DrawControlRadioButtonMark(CDC* pDC, CRect rc, BOOL bDraw, BOOL bSelected, BOOL bPressed, BOOL bChecked, BOOL bEnabled)
{
	if (bDraw)
	{
		...
		{
			pDC->DrawFrameControl(rc, UINT(DFC_BUTTON), UINT(DFCS_BUTTONRADIO |
				(!bEnabled || bChecked == 2 ? DFCS_INACTIVE : 0) | (bChecked ? DFCS_CHECKED : 0) |
				(bPressed ? DFCS_PUSHED : 0)));
		}
	}
...
CSize CXTPPaintManager::DrawControlPopupParent(CDC* pDC, CXTPControl* pButton, BOOL bDraw)
{
	...
	if (bDraw)
	{
		...
		rcShortcut.SetRect(rcButton.left + nGripperWidth + XTP_DPI_X(8), rcButton.top, rcButton.right - XTP_DPI_X(19), rcButton.bottom);
	...
	{
		{
			...
			if (bDraw)
			{
				rcShortcut.bottom = rcText.bottom = rcText.top + pDC->GetTextExtent(_T(" "), 1).cy + XTP_DPI_Y(10);
			}
			DrawPopupBarText(pDC, strCaption, rcText, UINT(DT_END_ELLIPSIS | (!bShowKeyboardCues ? DT_NOPREFIX : 0)), bDraw, bSelected, bEnabled);

			if (controlType == xtpControlButton && pButton->GetCustomShortcutText().GetLength() != 0)
				DrawPopupBarText(pDC, pButton->GetCustomShortcutText(), rcShortcut, DT_RIGHT, bDraw, bSelected, bEnabled);
			else if (controlType == xtpControlButton && pButton->GetShortcutText().GetLength() != 0)
		...
		if (bDraw)
		{
			CRect rcDescription(rcText.left, rcText.bottom - XTP_DPI_Y(2), rcButton.right - XTP_DPI_X(10), rcButton.bottom);
			pDC->DrawText(pButton->GetDescription(),rcDescription, UINT(DT_WORDBREAK | DT_TOP | DT_NOPREFIX));
		}
		else
		{
			...
			int nWidth = nGripperWidth + m_nPopupBarTextPadding + rcText.Width() +  rcShortcut.Width() + XTP_DPI_X(32);

			return GetControlSize(pButton, CSize(max(nWidth, nGripperWidth + m_nPopupBarTextPadding + szDescription.cx / 2 + XTP_DPI_X(32)),
				max(szImage.cy, rcDescription.Height() + rcText.Height() + XTP_DPI_Y(19))), FALSE);
		}
		return 0;
	}
	...
	return GetControlSize(pButton, CSize(nGripperWidth + m_nPopupBarTextPadding + rcText.Width() +  rcShortcut.Width() + XTP_DPI_X(32), szImage.cy), FALSE);
}
...
CSize CXTPPaintManager::GetStretchIconSizeDPIAwareness(CXTPImageManagerIcon* pImage, CSize szIcon, CXTPControl* pControl, CSize szAvailable)
{
	if (pImage == NULL)
		return szIcon;

	BOOL bDPIIconsScallingEnabled = IsDPIIconsScallingEnabled(pControl);
	const int nDpiScallingButtonMarginX = XTP_DPI_X(2);
	const int nDpiScallingButtonMarginY = XTP_DPI_Y(2);
	
	if (bDPIIconsScallingEnabled)
	{
		szIcon.cx = XTP_DPI_X(szIcon.cx);
		szIcon.cy = XTP_DPI_Y(szIcon.cy);
		...
		szIcon = GetStretchIconSize(pImage, szIcon);
	}
	else
	{
...
int AFX_CDECL CXTPPaintManager::CalcTextHeight(
	int nFontHeight,
...
// only available if CommandBar available
BOOL CXTPPaintManager::IsDPIIconsScallingEnabled(CXTPControl *pButton)
{
	BOOL bEnableIconsScalling = FALSE;

	if (!pButton)
		return bEnableIconsScalling;

	CXTPCommandBar *pCommandBar = pButton->GetCommandBar();

	if (!pCommandBar || !pCommandBar->GetCommandBars())
		pCommandBar = pButton->GetParent();

	if (pCommandBar && pCommandBar->GetCommandBars())
	{
		bEnableIconsScalling = pCommandBar->GetCommandBars()->GetCommandBarsOptions()->GetToolbarsIconsScalling();
	}
...
CSize CXTPPaintManager::DrawControlToolBarParent(CDC* pDC, CXTPControl* pButton, BOOL bDraw)
{
	...
	{
		szButton = CSize(max(XTP_DPI_X(42), GetControlHeight() * 2 - XTP_DPI_X(3)),
			m_nEditHeight > 0 ? m_nEditHeight * 3 : GetSplitDropDownHeight() + szIcon.cy + XTP_DPI_Y(7));
	}
	else if (pButton->GetWidth() > 0 || pButton->GetHeight() > 0)	// individual custom size
	{
		szButton = GetControlSize(pButton, szButton, bVert);
	}
	else if (pButton->GetParent() && pButton->GetParent()->m_szButtons != CSize(0))	// common custom size
	{
	...
	szIcon = GetStretchIconSizeDPIAwareness(pImage, szIcon, pButton, szButton);
	...
	if (controlType == xtpControlCheckBox || controlType == xtpControlRadioButton)
	{
		...
		if (bDraw)
		{
			...
			if (!bVert)
			{
				rcCheck = CRect(CPoint(rcButton.left + XTP_DPI_X(3), (rcButton.bottom + rcButton.top - szCheckBox.cy)/2), szCheckBox);
				rcButton.left += szCheckBox.cx + XTP_DPI_X(5);
			}
			else
			{
				rcCheck = CRect(CPoint(rcButton.CenterPoint().x - szCheckBox.cx / 2, rcButton.top + XTP_DPI_Y(3)), szCheckBox);
	...
	if (buttonStyle == xtpButtonIconAndCaptionBelow)
	{
		...
		if (bDraw)
		{
			if (bDrawImage)
			{
				CPoint pt = bVert ? CPoint(rcButton.left + XTP_DPI_X(3) + nSplitDropDownHeight, rcButton.CenterPoint().y - szIcon.cy / 2) :
					CPoint(rcButton.CenterPoint().x - szIcon.cx / 2, rcButton.top + XTP_DPI_Y(2));
	...
	if (bDraw && pImage)
	{
		CPoint pt = !bVert ? CPoint(rcButton.left + XTP_DPI_X(3), rcButton.CenterPoint().y - szIcon.cy / 2) :
			CPoint(rcButton.CenterPoint().x - szIcon.cx / 2, rcButton.top + XTP_DPI_Y(3));

		if (controlType == xtpControlSplitButtonPopup) pt.x = rcButton.left + XTP_DPI_X(3);
		if (controlType == xtpControlPopup && bDrawImage && !bDrawText) pt.x = rcButton.left + XTP_DPI_X(3);
	...
	case xtpControlPopup:
		{
			...
			else
			{
				if (!bDrawText)
				{
					...
					return GetControlSize(pButton, CSize(szButton.cy + XTP_DPI_X(7), szButton.cy), bVert);
				}
	...
	case xtpControlSplitButtonPopup:
		{
			if (!bDrawImage) szButton = CSize(XTP_COMMANDBAR_MIN_CONTROL_HEIGHT, XTP_COMMANDBAR_MIN_CONTROL_HEIGHT);
	...
	case xtpControlGallery:
		{
			...
			return GetControlSize(pButton, DrawControlText(pDC, pButton, rcButton, bDraw, FALSE, bDrawImage ? szIcon : szButton, bDrawImage), bVert);
		}
...
CSize CXTPPaintManager::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar, BOOL bDraw)
{
	if (pBar->GetPosition() == xtpBarFloating)
	{
		...
		rc.SetRect(XTP_DPI_X(3), XTP_DPI_Y(3), rc.right - XTP_DPI_X(3), XTP_DPI_Y(3) + max(XTP_DPI_Y(15), sz.cy));
		if (pDC && bDraw)
		{
			pDC->SetBkMode(TRANSPARENT);
			pDC->FillSolidRect(rc, m_clrFloatingGripper);
			pDC->SetTextColor(m_clrFloatingGripperText);
			pDC->DrawText(pBar->GetTitle(), CRect(XTP_DPI_X(5), XTP_DPI_Y(3), rc.right - 2* rc.Size().cy, rc.bottom), DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
		}
		return rc.Size();
...
void CXTPPaintManager::DrawControlMDIButton(CDC* pDC, CXTPControlButton* pButton)
{
	...
	DrawTextApi(pDC, strText, pButton->GetRect(), DT_SINGLELINE | DT_CENTER | DT_VCENTER | DT_NOPREFIX, pButton->HasDwmCompositedRect());
...
BEGIN_DISPATCH_MAP(CXTPPaintManager, CXTPCmdTarget)
	...
	DISP_PROPERTY_NOTIFY_ID(CXTPPaintManager, "ClearTypeTextQuality", 4, m_bClearTypeTextQuality, OleRefreshMetrics, VT_BOOL)
	...
	DISP_FUNCTION_ID(CXTPPaintManager, "FillWorkspace", 7, OleFillWorkspace, VT_EMPTY, VTS_HANDLE VTS_I4 VTS_I4 VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPPaintManager, "DrawRectangle", 8, OleDrawRectangle, VT_EMPTY,
		VTS_HANDLE VTS_I4 VTS_I4 VTS_I4 VTS_I4 VTS_BOOL VTS_BOOL VTS_BOOL VTS_BOOL VTS_BOOL VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPPaintManager, "GetRectangleTextColor", 9, OleGetRectangleTextColor, VT_COLOR,
		VTS_BOOL VTS_BOOL VTS_BOOL VTS_BOOL VTS_BOOL VTS_I4 VTS_I4)
	...
	DISP_PROPERTY_ID(CXTPPaintManager, "SelectImageInPopupBar", 12, m_bSelectImageInPopupBar, VT_BOOL)
	...
	DISP_FUNCTION_ID(CXTPPaintManager, "LoadFrameIconFromResource", 16, OleLoadFrameIcon, VT_EMPTY, MODULE_HANDLE_TYPE VTS_BSTR VTS_I4 VTS_I4)
END_DISPATCH_MAP()
...
void CXTPPaintManager::OleLoadFrameIcon(MODULE_HANDLE Module, LPCTSTR pszFileName, long Width, long Height)
{
	...
	hIcon = (HICON)::LoadImage((HMODULE)Module, pszFileName, IMAGE_ICON, Width, Height, LR_LOADFROMFILE);
...
{
	FillWorkspace(CDC::FromHandle((HDC)hDC), CRect(x, y, x + cx, y + cy), CRect(0, 0, 0, 0));
}

void CXTPPaintManager::OleDrawRectangle(OLE_HANDLE hDC, int x, int y, int cx, int cy, BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	DrawRectangle(CDC::FromHandle((HDC)hDC), CRect(x, y, x + cx, y + cy), bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition);
}

COLORREF CXTPPaintManager::OleGetRectangleTextColor(BOOL bSelected, BOOL bPressed, BOOL bEnabled, BOOL bChecked, BOOL bPopuped, XTPBarType barType, XTPBarPosition barPosition)
{
	return GetRectangleTextColor(bSelected, bPressed, bEnabled, bChecked, bPopuped, barType, barPosition);
