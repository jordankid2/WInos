...
void CXTPSliderThemeOffice2007System::DrawScrollBar(CDC *pDC, CXTPScrollBase *pScrollBar)
{
	...
	if (!pSBInfo->fVert)
	{
		...
		CRect rcTrackDest(CPoint(rcTrack.left, (rcTrack.top + rcTrack.bottom - XTP_DPI_Y(2)) / 2), CSize(rcTrack.Width(), XTP_DPI_Y(2)));

		pDC->FillSolidRect(rcTrackDest.left, rcTrackDest.top, rcTrackDest.Width(), XTP_DPI_Y(1), clrShadow);
		pDC->FillSolidRect(rcTrackDest.left, rcTrackDest.top + XTP_DPI_Y(1), rcTrackDest.Width(), XTP_DPI_Y(1), clrLight);
		...
		if (!pTicks || (pSBInfo->posMax <= pSBInfo->posMin))
		{
			...
			pDC->FillSolidRect(rcTrackTickDest.left, rcTrackTickDest.top, XTP_DPI_X(1), XTP_DPI_Y(4), clrShadow);
			pDC->FillSolidRect(rcTrackTickDest.left, rcTrackTickDest.top + XTP_DPI_Y(5), XTP_DPI_X(1), XTP_DPI_Y(2), clrShadow);
			pDC->FillSolidRect(rcTrackTickDest.left + XTP_DPI_X(1), rcTrackTickDest.top + XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(7), clrLight);
		}
		else
		{
			...
			for (int i = 0; i < pTicks->nCount; i++)
			{
				...
				{
					CRect rcTrackTickDest(CPoint(rcTrackDest.left + (int)dPos - XTP_DPI_X(1),
						(rcTrackDest.top + rcTrackDest.bottom - XTP_DPI_Y(8)) / 2), XTP_DPI(CSize(2, 8)));
		...
		pDC->FillSolidRect(ptCenter.x - XTP_DPI_X(4), ptCenter.y - XTP_DPI_Y(1), XTP_DPI_X(8), XTP_DPI_Y(2), clrShadow);
		...
		pDC->FillSolidRect(ptCenter.x - XTP_DPI_X(4), ptCenter.y - XTP_DPI_Y(1), XTP_DPI_X(8), XTP_DPI_Y(2), clrShadow);
		pDC->FillSolidRect(ptCenter.x - XTP_DPI_X(1), ptCenter.y - XTP_DPI_Y(4), XTP_DPI_X(2), XTP_DPI_Y(8), clrShadow);

		if (bEnabled)
		{
			CRect rc(CPoint((rcBtnTrack.left + rcBtnTrack.right - XTP_DPI_X(8)) / 2,
				(rcBtnTrack.top + rcBtnTrack.bottom - XTP_DPI_Y(13)) / 2), XTP_DPI(CSize(8, 13)));

			POINT pts[] = {rc.left, rc.top, rc.right, rc.top,
				rc.right,  rc.bottom - XTP_DPI_Y(4), rc.left + XTP_DPI_X(4), rc.bottom, rc.left, rc.bottom - XTP_DPI_Y(4), rc.left, rc.top};
