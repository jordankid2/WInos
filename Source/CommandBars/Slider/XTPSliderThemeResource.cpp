...
void CXTPSliderResourceTheme::DrawScrollBar(CDC* pDC, CXTPScrollBase* pGallery)
{
	...
	if (!pSBInfo->fVert)
	{
		...
		CRect rcTrackDest(CPoint(rcTrack.left, (rcTrack.top + rcTrack.bottom - XTP_DPI_Y(pImage->GetHeight())) / 2),
		...
		{
			CRect rcTrackTickDest(CPoint((rcTrackDest.left + rcTrackDest.right - XTP_DPI_X(pImage->GetWidth())) / 2,
		...
		else
		{
			...
			for (int i = 0; i < pTicks->nCount; i++)
			{
				...
				{
					CRect rcTrackTickDest(CPoint(rcTrackDest.left + (int)dPos - XTP_DPI_X(pImage->GetWidth()) / 2,
						(rcTrackDest.top + rcTrackDest.bottom - XTP_DPI_Y(pImage->GetHeight())) / 2),
						XTP_DPI(pImage->GetExtent()));
		...
		if (bEnabled)
		{
			...
			CRect rcBtnTrackDest(CPoint((rcBtnTrack.left + rcBtnTrack.right - XTP_DPI_X(rcImgSrc.Width())) / 2,
