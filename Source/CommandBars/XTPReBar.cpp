...
const LPCTSTR CXTPReBar::m_lpszStateInfoEntry = _T("RebarStateInfo");
...
CXTPReBarBase::CXTPReBarBase()
{
	...
	m_iComCtlVersion = XTPSystemVersion()->GetComCtlVersion();
}
...
BOOL CXTPReBarBase::_AddBar(CWnd* pBar, REBARBANDINFO* pRBBI)
{
	...
	pRBBI->cxMinChild = size.cx + (m_iComCtlVersion < VERSION_IE401 ? XTP_DPI_X(4) : 0);
...
BOOL CXTPReBar::AddToolBar(CXTPToolBar* pToolBar, DWORD dwStyle)
{
	...
	if (pToolBar->GetDockBar())
	{
		...
		pToolBar->SetFlags(0, xtpFlagAlignAny | xtpFlagFloating);
	...
	UINT nID = XTPToUInt(pToolBar->GetBarID());

	int nCount = XTPToInt(DefWindowProc(RB_GETBANDCOUNT, 0, 0));
	REBARBANDINFO rbBand;
	rbBand.cbSize = sizeof(rbBand);
	...
	VERIFY(DefWindowProc(RB_SETBANDINFO, XTPToWPARAM(nCount - 1), XTPToLPARAM(&rbBand)));
...
void CXTPReBar::DeleteToolBar(CXTPToolBar* pToolBar)
{
	...
	if (nBand != -1)
	{
		DefWindowProc(RB_DELETEBAND, XTPToWPARAM(nBand), 0);
	}
}
...
CXTPToolBar* CXTPReBar::GetToolBar(int nBand)
{
	...
	VERIFY(DefWindowProc(RB_GETBANDINFO, XTPToWPARAM(nBand), XTPToLPARAM(&rbBand)));
...
{
	CSize szResult = CXTPReBarBase::CalcFixedLayout(bStretch, bHorz);
	Invalidate(FALSE);
	...
	for (int nTemp = nCount; nTemp--;)
	{
		...
		if (pToolBar)
		{
			...
			CSize sz = pToolBar->CalcDockingLayout(rc.Width(), DWORD(LM_HORZDOCK | LM_HORZ | LM_COMMIT 
				| (pToolBar->GetFlags() & xtpFlagStretched ? LM_STRETCH : LM_HIDEWRAP)));

			REBARBANDINFO rbbi;
			rbbi.cbSize = sizeof(rbbi);
			rbbi.fMask = RBBIM_CHILDSIZE | RBBIM_IDEALSIZE;
			rbbi.cxIdeal = XTPToUIntChecked(sz.cx);
			rbbi.cxMinChild = XTPToUIntChecked(szMin.cx);
			rbbi.cyMinChild = XTPToUIntChecked(sz.cy);
			VERIFY(DefWindowProc(RB_SETBANDINFO, XTPToWPARAM(nTemp), XTPToLPARAM(&rbbi)));
...
void CXTPReBar::LoadState(LPCTSTR lpszProfileName)
{
	...
	if (!strValue.IsEmpty())
	{
		...
		for (int nBand = 0; nBand < nCount; nBand++)
		{
			...
			UINT nID, cx, nStyle;
#pragma warning(push)
#pragma warning(disable : 4774) // 'swscanf_s' : format string expected in argument 2 is not a string literal
			int nResult = SCANF_S(strBandState, m_lpszStateInfoFormat, &nID, &cx, &nStyle);
#pragma warning(pop)
			ASSERT(nResult == 3);
			UNUSED(nResult);

			DefWindowProc(RB_MOVEBAND, XTPToWPARAM(DefWindowProc(RB_IDTOINDEX, nID, 0)), nBand);
			VERIFY(DefWindowProc(RB_GETBANDINFO, XTPToWPARAM(nBand), XTPToLPARAM(&rbbi)));
			rbbi.cx = cx;
			rbbi.fStyle = (rbbi.fStyle & ~(RBBS_HIDDEN | RBBS_BREAK)) | nStyle;
			VERIFY(DefWindowProc(RB_SETBANDINFO, XTPToWPARAM(nBand), XTPToLPARAM(&rbbi)));
