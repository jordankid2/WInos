...
class CControlButtonCustomize: public CXTPControlButton
{
	DECLARE_XTP_CONTROL(CControlButtonCustomize);
...
IMPLEMENT_XTP_CONTROL(CControlButtonCustomize, CXTPControlButton);
...
INT_PTR CXTPCommandBar::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	if (pCommandBars && bToolBarAccelTips)
	{
		if (strShortcut.IsEmpty())
		{
			...
			if (!pCommandBars->GetShortcutManager()->FindDefaultAccelerator(pControl->GetID(), strShortcut))
				strShortcut.Empty();
		}
...
BOOL CXTPToolBar::Create(CWnd* pParentWnd, BOOL bFloat)
{
	if (!IsWindow(m_hWnd))
	{
		...
		CPoint pt = m_pDockContext ? m_pDockContext->m_ptMRUFloatPos : XTP_DPI(CPoint(100, 100));

		CXTPCommandBars* pCommandBars = GetCommandBars();
		DWORD dwExStyle = DWORD(pCommandBars && pCommandBars->IsLayoutRTL() ? WS_EX_LAYOUTRTL : 0);

		if (!CWnd::CreateEx(dwExStyle, _T("XTPToolBar"), m_strTitle, dwStyle, CRect(pt, CSize(0, 0)), pParentWnd, 0))
			return FALSE;
	...
	if (m_bShowExpandButton)
	{
		...
		if (bShowExpandButtonAlways)
		{
			if (dwMode & LM_HORZDOCK)
				rcBorder.right += XTP_DPI_X(11);
			else
				rcBorder.bottom += XTP_DPI_Y(11);
		}
		else
		{
			if (dwMode & LM_COMMIT)
			...
			else
			{
				...
				{
					if (dwMode & LM_HORZDOCK)
						sz.cx = max(sz.cx, szMin.cx + XTP_DPI_X(11));
					else
						sz.cy = max(sz.cy, szMin.cy + XTP_DPI_Y(11));
				}
...
CSize CXTPToolBar::CalcDynamicLayout(int nLength, DWORD nMode)
{
	...
	CRect rcBorder(GetBorders());
	rcBorder.top += szBar.cy + XTP_DPI_Y(1);
...
LRESULT CXTPToolBar::OnFloatStatus(WPARAM wParam, LPARAM)
{
	...
	if (bSyncActivate)
	{
		...
		if ((wParam & FS_ACTIVATE) && !IsWindowVisible() && IsTopParentActive())
		{
			...
			LONG lStyle = ::GetWindowLong(::GetForegroundWindow(), GWL_STYLE);
			if ((lStyle & WS_VISIBLE) && !(lStyle & WS_MINIMIZE))
	...
	if (wParam & (FS_SHOW | FS_HIDE))
	{
		SetWindowPos(NULL, 0, 0, 0, 0,
			UINT(((wParam & FS_SHOW) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW) | SWP_NOZORDER |
...
void CXTPToolBar::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
	...
	if (nHitTest >= HTSIZEFIRST && nHitTest <= HTSIZELAST)
	{
		...
		m_pCommandBars->ClosePopups();
		m_pDockContext->StartResize(XTPToIntChecked(nHitTest), point);
		return;
	}
...
CSize CXTPCommandBar::GetButtonSize() const
{
	...
	return CSize(szIcon.cx + XTP_DPI_X(6), szIcon.cy + XTP_DPI_Y(6));
}
...
BOOL AFX_CDECL CXTPToolBar::DoUpdateCustomizeMenuControl(
	...
	CCmdTarget* pTarget)
{
	...
	if (0 == pControl->GetID())
	{
		CControlButtonCustomize* pButton = DYNAMIC_DOWNCAST(CControlButtonCustomize, pControl);
		if (NULL != pButton)
		{
			ASSERT(NULL != pButton->m_pControl);
			CXTPControl* controls[] = { pButton, pButton->m_pControl };
			bHandled = pTarget->OnCmdMsg(
				XTPToUInt(pButton->m_pControl->GetID()),
				XTP_CN_UPDATE_CUSTOMIZE_COMMAND_UI,
				controls,
				NULL);
		}
	}
...
BEGIN_DISPATCH_MAP(CXTPToolBar, CXTPCommandBar)
	DISP_PROPERTY_ID(CXTPToolBar, "ContextMenuPresent", 15, m_bContextMenuPresent, VT_BOOL)
	DISP_PROPERTY_ID(CXTPToolBar, "ShowExpandButton", 33, m_bShowExpandButton, VT_BOOL)
	DISP_FUNCTION_ID(CXTPToolBar, "Reset", 35, OleReset, VT_EMPTY, VTS_NONE)
	DISP_PROPERTY_ID(CXTPToolBar, "Closeable", 24, m_bCloseable, VT_BOOL)
	DISP_FUNCTION_ID(CXTPToolBar, "Delete", 14, OleDelete, VT_EMPTY, VTS_NONE)
	DISP_PROPERTY_ID(CXTPToolBar, "CustomizeDialogPresent", 36, m_bCustomizeDialogPresent, VT_BOOL)
END_DISPATCH_MAP()

void CXTPToolBar::OleReset()
{
	Reset(FALSE);
}

void CXTPToolBar::OleDelete()
{
