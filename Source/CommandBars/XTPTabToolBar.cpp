...
CXTPTabManagerItem* CXTPTabToolBar::InsertCategory(int nItem, LPCTSTR lpszItem, UINT* pButtons, int nCount, BOOL bLoadIcons)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		else
		{
			...
			if (pControl == NULL)
			{
				pControl = m_pControls->Add(controlType, XTPToInt(nID));
				if (pControl)
				{
					pControl->SetStyle(buttonStyle);
					if (controlType == xtpControlPopup) pControl->SetIconId(XTPToInt(nID));
				}
			}
			else m_pControls->Add(pControl, XTPToInt(nID));
...
void CXTPTabToolBar::BeforeCustomizeControlAdd(CXTPControl* pControl)
{
...
#ifdef _XTP_ACTIVEX
		if (pItem) FireEvent(1, EVENT_PARAM(VTS_DISPATCH), XTPGetDispatch(pItem, FALSE));
#endif
	}
}
...
BOOL CXTPTabToolBar::OnBeforeItemClick(CXTPTabManagerItem* pItem)
{
	...
	FireEvent(4, EVENT_PARAM(VTS_DISPATCH VTS_PVARIANT), XTPGetDispatch(pItem, FALSE), &vtCanceled);
...
CSize CXTPTabToolBar::CalcDynamicLayout(int nLength, DWORD nMode)
{
	...
	CSize szBar = CXTPToolBar::GetPaintManager()->GetDefaultCommandBarSize(this);

	CRect rcBorderOriginal(XTP_DPI_X(3), XTP_DPI_Y(3) + szBar.cy + XTP_DPI_Y(1), XTP_DPI_X(3), XTP_DPI_Y(3));
	...
	if (pHide)
	{
		...
		pHide->SetRect(CRect(nLeft - szBar.cy, XTP_DPI_Y(3), nLeft, XTP_DPI_Y(3) + szBar.cy));
		nLeft -= szBar.cy;
	...
	if (pPopupExp && (nLeft - szBar.cy) > 0 && bShowExpandButtonAlways)
	{
		pPopupExp->SetWrap(TRUE);
		pPopupExp->SetHideFlags(xtpNoHide);
		pPopupExp->SetRect(CRect(nLeft - szBar.cy, XTP_DPI_Y(3), nLeft, XTP_DPI_Y(3) + szBar.cy));
	}
...
void CXTPTabToolBar::OnCustomizeDrop(CXTPControl* pDataObject, DROPEFFECT& dropEffect, CPoint ptDrop, CPoint ptDrag)
{
	...
	CXTPControl* pDragControl = pCommandBars ? DYNAMIC_DOWNCAST(CXTPControl, pCommandBars->GetDragControl()) : NULL;

	if (pDragControl && GetSelectedItem())
	{
		pDragControl->SetCategory(GetSelectedItem()->GetCaption());
		pDragControl->SetHideFlag(xtpHideRibbonTab, FALSE);
	}
...
BOOL CXTPTabToolBar::DrawIcon(CDC* pDC, CPoint pt, CXTPTabManagerItem* pItem, BOOL bDraw, CSize& szIcon) const
{
	...
	CXTPImageManagerIcon* pImage = GetImageManager()->GetImage(
		XTPToUIntChecked(pItem->GetImageIndex()), szIcon.cx);
...
BEGIN_DISPATCH_MAP(CXTPTabToolBar, CXTPToolBar)
	DISP_FUNCTION_ID(CXTPTabToolBar, "InsertCategory", 100, OleInsertCategory, VT_DISPATCH, VTS_I4 VTS_BSTR)
	DISP_PROPERTY_EX_ID(CXTPTabToolBar, "CategoryCount", 101, OleGetItemCount, SetNotSupported, VT_I4)
	DISP_FUNCTION_ID(CXTPTabToolBar, "Category", DISPID_VALUE, OleGetItem, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPTabToolBar, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
	DISP_FUNCTION_ID(CXTPTabToolBar, "RemoveCategory", 102, OleRemoveCategory, VT_EMPTY, VTS_I4)
	DISP_PROPERTY_EX_ID(CXTPTabToolBar, "TabPaintManager", 103, OleGetTabPaintManager, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_ID(CXTPTabToolBar, "MinimumWidth", 104, m_nMinWidth, VT_I4)
	DISP_FUNCTION_ID(CXTPTabToolBar, "UpdateTabs", 105, OleUpdateTabs, VT_EMPTY, VTS_NONE)
	DISP_PROPERTY_ID(CXTPTabToolBar, "AllowReorder", 106, m_bAllowReorder, VT_BOOL)
END_DISPATCH_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPTabToolBar, XTPDIID_ITabToolBar)

BEGIN_INTERFACE_MAP(CXTPTabToolBar, CXTPToolBar)
	INTERFACE_PART(CXTPTabToolBar, XTPDIID_ITabToolBar, Dispatch)
	//INTERFACE_PART(CXTPTabToolBar, IID_IEnumVARIANT, EnumVARIANT)
	INTERFACE_PART(CXTPTabToolBar, IID_IConnectionPointContainer, ConnPtContainer)
END_INTERFACE_MAP()

IMPLEMENT_ENUM_VARIANT(CXTPTabToolBar)

IMPLEMENT_CONNECTION_EX(CXTPTabToolBar, XTPDIID_ITabToolBarEvents)

LPDISPATCH CXTPTabToolBar::OleInsertCategory(int nIndex, LPCTSTR lpszTitle)
{
	CXTPTabManagerItem* pItem = InsertCategory(nIndex, lpszTitle, NULL, 0, FALSE);
	return pItem->GetIDispatch(TRUE);
}

long CXTPTabToolBar::OleGetItemCount()
{
	return GetItemCount();
}

LPDISPATCH CXTPTabToolBar::OleGetItem(int nIndex)
{
	CXTPTabManagerItem* pItem =  GetItem(nIndex);

	return pItem ? pItem->GetIDispatch(TRUE) : NULL;
}

void CXTPTabToolBar::OleRemoveCategory(int nIndex)
{
	DeleteItem(nIndex);
}

LPDISPATCH CXTPTabToolBar::OleGetTabPaintManager()
{
	return m_pTabPaintManager->GetIDispatch(TRUE);
}

void CXTPTabToolBar::OleUpdateTabs()
{
	UpdateTabs();
}
