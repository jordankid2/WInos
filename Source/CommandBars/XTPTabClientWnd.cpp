...
void CXTPTabClientWnd::SetLayoutRTL(BOOL bRightToLeft)
{
	...
	{
		((CSingleWorkspace*)GetWorkspace(0))->ModifyStyleEx(
			DWORD(m_bRightToLeft ? 0 : WS_EX_LAYOUTRTL),
...
BOOL CXTPTabClientWnd::Attach(CXTPCommandBars* pCommandBars, CMDIFrameWnd* pParentFrame, BOOL bEnableGroups)
{
	...
	if (bEnableGroups)
	{
		::SetClassLong(m_hWnd, GCL_STYLE, XTPToLong(::GetClassLong(m_hWnd, GCL_STYLE) | CS_DBLCLKS));
	}
...
void CXTPTabClientWnd::CheckCommandBarsTheme()
{
	...
	m_pPaintManager->m_bClearTypeTextQuality = pCommandBars->GetPaintManager()->m_bClearTypeTextQuality;
	m_pPaintManager->m_bBoldSelected = FALSE;
	m_pPaintManager->m_rcClientMargin.SetRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2));
	...
	switch (themeCommandBars)
	{
		...
		case xtpThemeVisualStudio2012Dark:
		{
			...
			m_pPaintManager->SetAppearanceSet(pTheme);
			m_pPaintManager->SetColor((themeCommandBars == xtpThemeVisualStudio2012Dark) ? xtpTabColorVisualStudio2012Dark : xtpTabColorVisualStudio2012);
		}
		break;
		...
		case xtpThemeRibbon:
		{
			...
			m_pPaintManager->m_bOneNoteColors = FALSE;
			m_pPaintManager->m_bHotTracking = TRUE;
			m_pPaintManager->m_rcClientMargin.SetRect(XTP_DPI_X(3), XTP_DPI_Y(2), XTP_DPI_X(3), XTP_DPI_Y(4));

			if (pCommandBars->GetMenuBar() && pCommandBars->GetMenuBar()->GetType() == xtpBarTypeRibbon)
			{
			...
			else
			{
				...
				m_pPaintManager->m_rcButtonMargin.SetRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2));
			}
		}
		break;
	...
	while (pChildWnd)
	{
		...
		if (pFoundItem != NULL)
		{
			if ((pChildWnd->GetStyle() & WS_VISIBLE) == WS_VISIBLE)
			{
				...
				if (m_bAutoUpdateTabCaption) pFoundItem->SetCaption(GetItemText(pChildWnd));
...
void CXTPTabClientWnd::OnFillBackground(CDC* pDC, CRect rc)
{
	...
	if (pCommandBars)
	{
		...
		{
			m_pPaintManager->GetAppearanceSet()->DrawWorkspacePart(pDC, rc, xtpTabWorkspacePartBack);
		}
	}
	else
	{
...
void CXTPTabClientWnd::ReorderWorkspace(CPoint pt, CXTPTabManagerItem* pItem)
{
	...
	m_pFocusWorkspace = NULL;
	m_nFocusedAction = XTP_TABCLIENT_ACTION_CANCEL;

	if (rc.PtInRect(pt))
	{
		m_nFocusedAction = XTP_TABCLIENT_ACTION_POPUP;
		...
		for (int i = 0; i < GetWorkspaceCount(); i++)
		{
			...
			if (pWorkspace->GetHeaderRect().PtInRect(pt))
			{
				...
				m_nFocusedAction = XTP_TABCLIENT_ACTION_ATTACH;
				break;
			}
			...
			if ((GetWorkspaceCount() == 1 || m_bHorizSplitting) && bSelfAvail)
			{
				...
				if (nOffset > 0 && nOffset < XTP_DPI_X(30))
				{
					rcGroup = pWorkspace->GetControlRect();
					rcGroup.top = rcGroup.CenterPoint().y;

					m_pFocusWorkspace = pWorkspace;
					m_nFocusedAction = XTP_TABCLIENT_ACTION_INSERTHORIZ;
					break;
				}
			...
			if ((GetWorkspaceCount() == 1 || !m_bHorizSplitting) && bSelfAvail)
			{
				...
				if (nOffset > 0 && nOffset < XTP_DPI_X(30))
				{
					...
					m_pFocusWorkspace = pWorkspace;
					m_nFocusedAction = XTP_TABCLIENT_ACTION_INSERTVERT;
					break;
				}
			}
		}
...
void CXTPTabClientWnd::OnWorkspaceCommand(UINT nID)
{
	...
	switch (nID)
	{
		case XTP_ID_WORKSPACE_NEWVERTICAL:
			DoWorkspaceCommand(pItem, (CWorkspace*)pItem->GetTabManager(), XTP_TABCLIENT_ACTION_INSERTVERT);
			break;

		case XTP_ID_WORKSPACE_NEWHORIZONTAL:
			DoWorkspaceCommand(pItem, (CWorkspace*)pItem->GetTabManager(), XTP_TABCLIENT_ACTION_INSERTHORIZ);
			break;

		case XTP_ID_WORKSPACE_MOVEPREVIOUS:
			{
				int nIndex = FindIndex(pItem->GetTabManager());
				DoWorkspaceCommand(pItem, GetWorkspace(nIndex - 1), XTP_TABCLIENT_ACTION_ATTACH);
			}
			break;
...
void CXTPTabClientWnd::DoWorkspaceCommand(CXTPTabManagerItem* pItem, CWorkspace* pFocusWorkspace, int nAction)
{
	...
	switch (nAction)
	{
		case XTP_TABCLIENT_ACTION_ATTACH:
			{
				...
				if (pFocusWorkspace != pItem->GetTabManager())
				{
					...
					int nInsert = pFocusWorkspace->GetItemCount() + (pFocusWorkspace->m_pNewWindowItem ? -1 : 0);
		...
		case XTP_TABCLIENT_ACTION_INSERTHORIZ:
		case XTP_TABCLIENT_ACTION_INSERTVERT:
			{
				NormalizeWorkspaceSize();
				...
				m_bHorizSplitting = (nAction == XTP_TABCLIENT_ACTION_INSERTHORIZ);
				break;
			}
	}
...
BOOL CXTPTabClientWnd::PreTranslateMessage(MSG* pMsg)
{
	if (m_bEnableGroups)
	{
			...
			pMsg->message == WM_RBUTTONDBLCLK)
		{
			...
			if (IsAttached() && GetParentFrame()->SendMessage(WM_XTP_PRETRANSLATEMOUSEMSG, (WPARAM)pMsg->message, pMsg->lParam))
				return TRUE;
		}
#endif
	}
	...
	if (!m_bEnableGroups && GetCommandBars())
	{
			...
			message == WM_MDITILE)
		{
			...
			while (hWnd)
			{
				...
				{
					CXTPCommandBarsFrameHook* pFrame = (CXTPCommandBarsFrameHook*)
						::SendMessage(hWnd, CXTPCommandBarsFrameHook::m_nMsgQueryFrameHook, 0, 0);
...
BEGIN_DISPATCH_MAP(CXTPTabClientWnd, CWnd)
	...
	DISP_PROPERTY_EX_ID(CXTPTabClientWnd, "AllowReorder", 3, IsAllowReorder, SetAllowReorder, VT_BOOL)
	...
	DISP_FUNCTION_ID(CXTPTabClientWnd, "HitTest", 9, OleHitTest, VT_DISPATCH, VTS_XPOS_PIXELS VTS_YPOS_PIXELS)
	DISP_FUNCTION_ID(CXTPTabClientWnd, "Selected", 10, OleGetSelected, VT_DISPATCH, VTS_NONE)

	DISP_FUNCTION_ID(CXTPTabClientWnd, "DoWorkspaceCommand", 11, OnWorkspaceCommand, VT_EMPTY, VTS_I4)
	DISP_FUNCTION_ID(CXTPTabClientWnd, "IsWorkspaceCommandEnabled", 12, OleIsWorkspaceCommandEnabled, VT_BOOL, VTS_I4)
	...
	DISP_PROPERTY_EX_ID(CXTPTabClientWnd, "ToolTipContext", 14, OleGetToolTipContext, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_ID(CXTPTabClientWnd, "UseSplitterTracker", 15, m_bUseSplitterTracker, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTabClientWnd, "HorizGroupSplitting", 19, OleGetHorizGroupSplitting, OleSetHorizGroupSplitting, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTabClientWnd, "GroupCount", 20, OlegGetGroupCount, SetNotSupported, VT_I4)
	...
	DISP_FUNCTION_ID(CXTPTabClientWnd, "CreateGroup", 23, OleCreateGroup, VT_EMPTY, VTS_DISPATCH VTS_BOOL)
	DISP_FUNCTION_ID(CXTPTabClientWnd, "MoveToGroup", 24, OleMoveToGroup, VT_EMPTY, VTS_DISPATCH VTS_I4)
	...
	DISP_PROPERTY_PARAM_ID(CXTPTabClientWnd, "GroupSplitter", 27, OleGetGroupSplitter, OleSetGroupSplitter, VT_R8, VTS_I4)
	DISP_PROPERTY_ID(CXTPTabClientWnd, "IgnoreFlickersOnActivate", 28, m_bIgnoreFlickersOnActivate, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTabClientWnd, "hWnd", DISPID_HWND, OleGetHwnd, SetNotSupported, VT_HANDLE)

	DISP_PROPERTY_ID(CXTPTabClientWnd, "AutoUpdateTabCaption", 29, m_bAutoUpdateTabCaption, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTabClientWnd, "EnableMarkup", 30, OleGetEnableMarkup, OleSetEnableMarkup, VT_BOOL)
	DISP_FUNCTION_ID(CXTPTabClientWnd, "ShowNewWindowTab", 31, ShowNewWindowTab, VT_EMPTY, VTS_BOOL VTS_I4)
...
{
	CXTPTabManagerItem* pItem = FindItem((HWND)hWnd);
	return pItem ? pItem->GetIDispatch(TRUE) : 0;
}

int CXTPTabClientWnd::OleFindGroup(LPDISPATCH Item)
{
	CXTPTabManagerItem* pItem = DYNAMIC_DOWNCAST(CXTPTabManagerItem, CXTPTabManagerItem::FromIDispatchSafe(Item));
	if (!pItem)
	{
		return -1;
	}
...
void CXTPTabClientWnd::OleCreateGroup(LPDISPATCH Item, BOOL Horiz)
{
	...
	if (pItem)
	{
		DoWorkspaceCommand(pItem, GetWorkspace(GetWorkspaceCount() - 1), Horiz ? XTP_TABCLIENT_ACTION_INSERTHORIZ : XTP_TABCLIENT_ACTION_INSERTVERT);
	}
}
...
void CXTPTabClientWnd::OleMoveToGroup(LPDISPATCH Item, int nGroup)
{
	...
	DoWorkspaceCommand(pItem, pDest, XTP_TABCLIENT_ACTION_ATTACH);
}
