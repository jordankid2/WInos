...
void CXTPControlListBox::SetRect(CRect rcControl)
{
	...
	m_pListBox->SetWindowPos(0, 0, 0, 0, 0, UINT(SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER 
		| SWP_NOACTIVATE | (!IsVisible() ? SWP_HIDEWINDOW : SWP_SHOWWINDOW)));
}
...
void CXTPControlListBox::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_DPI_X_Int(pPX, _T("Width"), m_nWidth, 100);
	...
	if (pPX->IsStoring())
	{
		int nCount = m_pListBox->m_hWnd ? m_pListBox->GetCount() : 0;
	...
	else
	{
		...
		{
			CString strSection;
			strSection.Format(_T("Item%i"), XTPToInt(i));
...
void CXTPControlListBoxCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (!bOutside)
	{
		SetCurSel(XTPToIntChecked(nItem));
		SetSel(XTPToIntChecked(nItem), TRUE);
...
CSize CXTPControlStatic::GetSize(CDC* pDC)
{
	...
	if (bVert) return CSize(max(XTP_DPI_X(22), sz.cx), max(m_nWidth, sz.cy));

	return CSize(max(m_nWidth, sz.cx), max(XTP_DPI_Y(22), sz.cy));
