...
{
	if (m_xtpFontEdit.GetSafeHandle())
	{
		m_xtpFontEdit.GetLogFont(&lf);
		return TRUE;
	}
...
BOOL CXTPSyntaxEditPropertiesPageFont::ReadRegistryValues()
{
	...
	if (!m_xtpFontEdit.CreateFontIndirect(&lf))
		return FALSE;
...
void CXTPSyntaxEditPropertiesPageFont::UpdateSampleFont()
{
	if (m_xtpFontEdit.GetSafeHandle())
	{
		m_txtSample.SetFont(&m_xtpFontEdit);
		m_txtSampleSel.SetFont(&m_xtpFontEdit);
	}
}
...
void CXTPSyntaxEditPropertiesPageFont::OnSelChangeComboNames()
{
	...
	STRCPY_S(lf.lfFaceName, LF_FACESIZE, m_csName);
