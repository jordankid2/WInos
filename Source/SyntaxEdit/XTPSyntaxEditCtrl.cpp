...
void CXTPSyntaxEditCtrl::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	else if (nChar != VK_BACK && nChar != VK_ESCAPE && !(bCtrlKey && !bAltKey) && !bProcessed)
	{
		...
		if (m_pSelection->IsSelExist())
		{
			if (!m_pBuffer->GetOverwriteFlag() || nChar != VK_RETURN)
			{
...
void CXTPSyntaxEditCtrl::InsertString(LPCTSTR szText, int iRow, int iCol, BOOL bDeleteSelection)
{
	...
	m_pSelection->Reset_disp(nCurDocRow, m_nDispCol);
	SetCurCaretPos(nCurDocRow, m_nDispCol, FALSE/*, FALSE*/);
}
...
int CXTPSyntaxEditCtrl::MoveCurrentVisibleRowUp(int nCount)
{
	...
	nNewVisRow = nOldDocRow - nDocDiff;
	SetCurrentDocumentRow(nNewVisRow);
	m_nTopRow -= nDocDiff;
...
DROPEFFECT CXTPSyntaxEditCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
...
BOOL CXTPSyntaxEditCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect, CPoint point)
{
	UNREFERENCED_PARAMETER(pDataObject);
