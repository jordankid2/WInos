...
IMPLEMENT_DYNCREATE(CXTPSyntaxEditBufferManager, CXTPCmdTarget)
...
//---------------------------------------------------------------------------
int AFX_CDECL _Compare_BYTE(const void* pb1, const void* pb2)
{
...
CXTPSyntaxEditBufferManager::CXTPSyntaxEditBufferManager()
{
	...
	m_ptrDefaultLexConfigurationManager = new CXTPSyntaxEditConfigurationManager();
	SetLexConfigurationManager(m_ptrDefaultLexConfigurationManager);
...
CXTPSyntaxEditBufferManager::~CXTPSyntaxEditBufferManager()
{
	...
	if(m_ptrDefaultLexConfigurationManager != NULL)
	{
		m_ptrDefaultLexConfigurationManager->Close();
		m_ptrDefaultLexConfigurationManager = NULL;
	}
	...
	CXTPSyntaxEditConfigurationManager* pMan)
{
	...
	if(pMan != m_ptrDefaultLexConfigurationManager)
	{
		m_ptrDefaultLexConfigurationManager->Close();
		m_ptrDefaultLexConfigurationManager = NULL;
	}
...
BEGIN_INTERFACE_MAP(CXTPSyntaxEditBufferManager, CXTPCmdTarget)
	INTERFACE_PART(CXTPSyntaxEditBufferManager, XTPDIID_SyntaxEditDataManager, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPSyntaxEditBufferManager, XTPDIID_SyntaxEditDataManager)

BEGIN_DISPATCH_MAP(CXTPSyntaxEditBufferManager, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPSyntaxEditBufferManager, "FileExt", 1, OleGetFileExt, OleSetFileExt, VT_BSTR)
END_DISPATCH_MAP()

BSTR CXTPSyntaxEditBufferManager::OleGetFileExt()
{
	return GetFileExt().AllocSysString();
}

void CXTPSyntaxEditBufferManager::OleSetFileExt(LPCTSTR pcszFileExt)
{
	if (GetFileExt().CompareNoCase(pcszFileExt) != 0)
	{
		SetFileExt(pcszFileExt);
	}
}
