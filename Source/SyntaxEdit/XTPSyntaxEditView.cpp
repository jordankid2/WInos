...
{
	GetEditCtrl().OnDragOver(pDataObject, dwKeyState, point);
...
BOOL CXTPSyntaxEditView::OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect, CPoint point)
{
	if (m_bFilesDragging)
	{
	...
	GetEditCtrl().OnDrop(pDataObject, dropEffect, point);
...
BEGIN_INTERFACE_MAP(CXTPSyntaxEditViewPrintOptions, CXTPPrintOptions)
	INTERFACE_PART(CXTPSyntaxEditViewPrintOptions, XTPDIID_SyntaxEditPrintOptions, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPSyntaxEditViewPrintOptions, XTPDIID_SyntaxEditPrintOptions)
