...
{
	CSize sz(XTP_DPI_X(nSize), XTP_DPI_Y(nSize));
...
void CXTPComboBoxTheme::DrawBackground(CDC *pDC, CXTPComboBox* pComboBox)
{
	...
	if (bRightAlign)
		FillRect(pDC->GetSafeHdc(), CRect(rc.left, rc.top, rc.left + GetSystemMetrics(SM_CXVSCROLL) + XTP_DPI_X(3), rc.bottom), hBrush);
	else
		FillRect(pDC->GetSafeHdc(), CRect(rc.right - GetSystemMetrics(SM_CXVSCROLL) - XTP_DPI_X(3), rc.top, rc.right, rc.bottom), hBrush);
}
