...
#include "Controls/Combo/Themes/XTPComboBoxThemeResource.h"
#include "Controls/Combo/Themes/XTPComboBoxThemeDefault.h"
#include "Controls/Combo/Themes/XTPComboBoxThemeOffice2000.h"
#include "Controls/Combo/Themes/XTPComboBoxThemeOffice2003.h"
#include "Controls/Combo/Themes/XTPComboBoxThemeOfficeXPFlat.h"
...
#include "Controls/Combo/Themes/XTPComboBoxThemeVisualStudio2012.h"
...
LRESULT CXTPComboBox::OnSetTheme(WPARAM wParam, LPARAM /*lParam*/)
{
...
BOOL CXTPComboBox::SetTheme(XTPControlTheme nTheme)
{
	...
	switch (nTheme)
	{
		...
		case xtpControlThemeVisualStudio2015:
			m_pTheme = new CXTPComboBoxThemeVisualStudio2015();
			break;
	...
	m_bUseVisualStyle = (
		nTheme == xtpControlThemeDefault || 
		nTheme == xtpControlThemeNativeWinXP);
...
BOOL CXTPComboBox::OnDropDown()
{
	Invalidate();
	return FALSE;
}

BOOL CXTPComboBox::OnCloseUp()
{
	Invalidate();
	return FALSE;
}
...
void CXTPComboBox::SetBackColor(COLORREF crBack)
{
	if (m_pTheme)
    {
        m_pTheme->SetBackColor(crBack);
    }
}

COLORREF CXTPComboBox::GetBackColor() const
{
    return m_pTheme ? m_pTheme->GetBackColor() : GetXtremeColor(COLOR_WINDOW);
}

void CXTPComboBox::SetTextColor(COLORREF crText)
{
	if (m_pTheme)
    {
        m_pTheme->SetTextColor(crText);
    }
}

COLORREF CXTPComboBox::GetTextColor() const
{
	return m_pTheme ? m_pTheme->GetTextColor() : GetXtremeColor(COLOR_WINDOWTEXT);
}
