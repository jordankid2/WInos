	...
	else
	{
		if (bEnabled)
		{
			...
			if (m_bHighlight && !bPressed && bSelected)
			{
				...
				arRect[0] = rcBtn;
				arRect[0].bottom = arRect[0].top + rcBtn.Height()/2;
				pDC->FillSolidRect(&arRect[0], m_grcButtonHilite.clrLight);

				arRect[1] = rcBtn;
				arRect[1].top += rcBtn.Height()/2;
				pDC->FillSolidRect(&arRect[1], m_grcButtonHilite.clrDark);
			}
			else
			{
