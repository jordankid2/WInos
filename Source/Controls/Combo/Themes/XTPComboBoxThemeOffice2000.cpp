...
void CXTPComboBoxThemeOffice2000::DrawBackground(CDC *pDC, CXTPComboBox* pComboBox)
{
	...
	else if (pComboBox->GetFlatStyle() && !bEnabled)
	{
	...
	else
	{
		...
		pDC->Draw3dRect(rc.left + 1, rc.top + 1, 
...
void CXTPComboBoxThemeOffice2000::DrawButton(CDC *pDC, CXTPComboBox* pComboBox, CRect rcBtn)
{
	...
	else if (!(pComboBox->GetFlatStyle() && !bEnabled))
	{
		...
		pDC->Draw3dRect(rcBtn.left + 1, rcBtn.top + 1, 
			rcBtn.Width() - 2, rcBtn.Height() - 2,
			GetXtremeColor(bPressed ? COLOR_3DSHADOW : COLOR_3DHILIGHT),
			GetXtremeColor(bPressed ? COLOR_3DHILIGHT : COLOR_3DSHADOW));
	}
}
