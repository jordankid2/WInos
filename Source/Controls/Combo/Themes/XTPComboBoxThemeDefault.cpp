...
BOOL CXTPComboBoxThemeDefault::DrawComboBox(CDC* pDC, CXTPComboBox* pComboBox)
{
	if (!m_themeComboBox->IsAppThemeReady())
	{
		...
		if (bRightAlign)
			FillRect(pDC->GetSafeHdc(), CRect(rc.left, rc.top, rc.left + XTP_DPI_X(3) + nThumbWidth, rc.bottom), hBrush);
		else
			FillRect(pDC->GetSafeHdc(), CRect(rc.right - nThumbWidth - XTP_DPI_X(3), rc.top, rc.right, rc.bottom), hBrush);
