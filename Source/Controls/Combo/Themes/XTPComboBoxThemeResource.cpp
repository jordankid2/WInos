...
BOOL CXTPComboBoxThemeResource::DrawComboBox(CDC* pDC, CXTPComboBox* pComboBox)
{
	...
	else
	{
		...
		if (bRightAlign)
		{
			FillRect(pDC->GetSafeHdc(), CRect(rc.left, rc.top,
				rc.left + GetSystemMetrics(SM_CXVSCROLL) + XTP_DPI_X(3), rc.bottom), hBrush);
		}
		else
		{
			FillRect(pDC->GetSafeHdc(), CRect(rc.right - GetSystemMetrics(SM_CXVSCROLL) - XTP_DPI_X(2), rc.top, 
				rc.right, rc.bottom), hBrush);
		}
		...
		int x = rcBtn.left + ((rcBtn.Width() - XTP_DPI_X(5)) / 2);
		int y = rcBtn.top + ((rcBtn.Height() - XTP_DPI_Y(4)) / 2) + 1;

		CRect rcArrow(x, y, x + XTP_DPI_X(5), y + XTP_DPI_Y(4));
