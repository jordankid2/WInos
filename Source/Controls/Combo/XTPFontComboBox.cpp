...
int CXTPFontEnum::GetMaxWidth()
{
	CWindowDC dc(NULL);
	CFont font;
	CFont* p = dc.SelectObject(CXTPFontCtrlBase::CreateUIFont(font) ? &font : &XTPAuxData().xtpFont);
...
{
	VERIFY(m_ilFontType.Create(XTP_IDB_FONTTYPE,
		XTP_DPI_Y(FontTypeWidth), 1, RGB(255,0,255)));
}
...
// 	BOOL bFocus    = ((itemState & ODS_FOCUS)    == ODS_FOCUS);
	...
	if (pTheme)
	{
		clrBack = bDisabled ? pTheme->m_clrBackDisabled : bSelected ? pTheme->m_clrBackHighlight : pTheme->m_clrBackground;
		clrText = bDisabled ? pTheme->m_clrTextDisabled : bSelected ? pTheme->m_clrTextHighlight : pTheme->m_clrText;
	}
	else
	{
		clrBack = bDisabled ? GetXtremeColor(COLOR_3DFACE)   : bSelected ? GetXtremeColor(COLOR_HIGHLIGHT)     : GetXtremeColor(COLOR_WINDOW);
		clrText = bDisabled ? GetXtremeColor(COLOR_GRAYTEXT) : bSelected ? GetXtremeColor(COLOR_HIGHLIGHTTEXT) : GetXtremeColor(COLOR_WINDOWTEXT);
	}
...
//	}
	...
	if (pLF->dwType & TRUETYPE_FONTTYPE)
	{
		...
		m_ilFontType.Draw(pDC, 1, rType.TopLeft(), ILD_NORMAL);
...
void CXTPFontCtrlBase::OnMeasureItem(LPMEASUREITEMSTRUCT lpMIS)
{
	...
	lpMIS->itemHeight = GetFontItemHeight();
	lpMIS->itemWidth = 0;
}
...
void CXTPFontComboBox::InitControl(LPCTSTR lpszFaceName, UINT nWidth/*= 0*/, BOOL bEnable/*= TRUE*/)
{
	...
	{
		SetDroppedWidth(XTP_DPI_X(FontTypeWidth) + XTP_DPI_X(6)
	...
	if (!m_ilFontType.m_hImageList)
	{
		VERIFY(m_ilFontType.Create(
			XTP_IDB_FONTTYPE, XTP_DPI_X(FontTypeWidth), 1, RGB(255, 0, 255)));
	}
