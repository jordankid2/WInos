...
void CXTPColorPicker::DrawColorPicker(CDC* pDC)
{
	...
	COLORREF clrText = bEnabled ? m_pTheme->m_crText : ::GetSysColor(COLOR_GRAYTEXT);
	...
	XTPPrimitiveDrawer()->DrawSymbol(pDC,
		xtpPrimitiveSymbolPlayDown, rcArrow, clrText);
...
BOOL CXTPColorPicker::SetThemeColors(XTP_PICK_BUTTON* pColors /*=NULL*/, int nCount /*=10*/, LPCTSTR lpszTitle /*=NULL*/, UINT* pStringIDs /*=NULL*/, int ppShadeMatrix[6][10] /*=NULL*/)
{
...
BOOL CXTPColorPicker::SetThemeColors(XTP_PICK_BUTTON* pColors, int nCount, LPCTSTR lpszTitle, LPCTSTR* ppszStrings, int ppShadeMatrix[6][10])
{
	CXTPColorSelectorCtrl csc;
...
BOOL CXTPColorPicker::SetStandardColors(XTP_PICK_BUTTON* pColors, int nCount, int nCols /*=10*/, LPCTSTR lpszTitle /*=NULL*/)
{
	CXTPColorSelectorCtrl csc;
...
void CXTPColorPicker::OnXtpShowPopup()
{
	...
	default:
		pColorPopup->SetTheme(new CXTPColorSelectorCtrlTheme());
		break;
	}
