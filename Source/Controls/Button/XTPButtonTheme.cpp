...
void CXTPButtonTheme::CalcRects(CDC* pDC, CXTPButton* pButton, CRect* pButtonText, UINT* pDrawFlags, CRect* pButtonIcon)
{
	...
	if (pButton->IsDropDownStyle() && pButton->GetPushButtonStyle() != xtpButtonDropDownNoGlyph)
	{
		rcItem.right -= XTP_DPI_X(12) + (pButton->GetPushButtonStyle() == xtpButtonSplitDropDown ?
...
void CXTPButtonTheme::DrawPushButtonDropDown(CDC* pDC, CXTPButton* pButton)
{
	...
	switch (pButton->GetPushButtonStyle())
	{
		...
		case xtpButtonSplitDropDown:
		{
			...
			COLORREF crBorderShadow = !bEnabled ? m_crTextDisabled : IsVisualThemeUsed(pButton) 
				?  GetSysColor(COLOR_BTNSHADOW) : m_crBorderShadow;

			pDC->FillSolidRect(rcItem.left, rcItem.top, XTP_DPI_X(1), rcItem.Height(), crBorderShadow);
			pDC->FillSolidRect(rcItem.left + XTP_DPI_X(1), rcItem.top, XTP_DPI_X(1), rcItem.Height(), GetSysColor(COLOR_BTNHIGHLIGHT));
	...
	if (m_bToolbarStyle)
	{
		...
		if (pIconManager && pIconManager->GetIcon())
		{
			CXTPImageManagerIconHandle& icon = (pButton->IsHighlighted() || bSelected) ?  
											( pButton->IsPushed() ? pIconManager->GetPressedIcon() : pIconManager->GetHotIcon() )
											: pIconManager->GetIcon();
											
			UINT nFlags = !pButton->IsWindowEnabled() ? (DSS_NORMAL | DSS_DISABLED) : DSS_NORMAL;
...
void CXTPButtonTheme::DrawGroupBox(CDC* pDC, CXTPButton* pButton)
{
	...
	if (pButton->GetBorderStyle() == xtpGroupBoxSingleLine)
	{
		if (IsVisualThemeUsed(pButton))
		{
			...
			pDC->FillSolidRect(rcGroupBox.left, rcGroupBox.top, rcGroupBox.Width(), XTP_DPI_Y(1), clrLine);
		}
		else
		{
