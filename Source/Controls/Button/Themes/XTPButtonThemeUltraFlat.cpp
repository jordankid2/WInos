...
void CXTPButtonThemeUltraFlat::DrawGroupBox(CDC* pDC, CXTPButton* pButton)
{
	...
	{
		pDC->FillSolidRect(rcGroupBox.left, rcGroupBox.top, rcGroupBox.Width(), XTP_DPI_Y(1), m_crBorderShadow);
	}
	else
	{
...
void CXTPButtonThemeUltraFlat::DrawCheckBoxMark(CDC* pDC, CXTPButton* pButton)
{
	...
	if (bChecked)
	{
		...
		XTPPrimitiveDrawer()->DrawSymbol(pDC, CXTPDrawHelpers::IsContextRTL(pDC) ? xtpPrimitiveSymbolCheckMarkRTL : xtpPrimitiveSymbolCheckMark, rc, clrCheck);
	}
}
...
void CXTPButtonThemeUltraFlat::DrawRadioButtonMark(CDC* pDC, CXTPButton* pButton)
{
	...
	if (bChecked)
	{
		...
		AlphaEllipse(pDC, CRect(rc.left + XTP_DPI_X(4), rc.top + XTP_DPI_Y(4), 
			rc.left + XTP_DPI_X(9), rc.top + XTP_DPI_Y(9)), clrCheck, clrCheck);
	}
}
