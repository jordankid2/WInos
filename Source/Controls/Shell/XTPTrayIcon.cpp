...
void CXTPTrayIcon::SetDefaultValues()
{
	...
	if (XTPSystemVersion()->IsWinXP())
	{
		m_niData.cbSize -= sizeof(HICON);
	}
	...
	// must be set to some guid on Windows 7
	// but we're not using NIF_GUID at the moment
	// just zero it, more info at http://msdn.microsoft.com/en-us/library/windows/desktop/bb773352(v=vs.85).aspx
	ZeroMemory(&m_niData.guidItem, sizeof(m_niData.guidItem));
	m_niData.hBalloonIcon = NULL;
...
bool CXTPTrayIcon::ShowBalloonTip(LPCTSTR lpszInfo, LPCTSTR lpszInfoTitle/*= NULL*/, DWORD dwInfoFlags/*= NIIF_NONE*/, UINT uTimeout/*= 10*/, HICON hBaloonIcon /*= NULL*/)
{
	bool bResult = false;
	...
	if (IsShellVersion5() && lpszInfo)
	{
		...
		m_niData.uTimeout = (uTimeout * 1000); // convert time to milliseconds
		m_niData.dwInfoFlags = dwInfoFlags;

		if (hBaloonIcon && XTPSystemVersion()->IsWinVistaOrGreater())
		{
