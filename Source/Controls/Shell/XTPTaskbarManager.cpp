...
CXTPTaskbarManagerJumpList::CXTPTaskbarManagerJumpList()
{
	m_pcdl = NULL;
	m_poaRemoved = NULL;
	m_sAppID.Empty(); // Added by CPN
	m_uiCategory = 0; // Added by CPN
...
void CXTPTaskbarManagerJumpList::BuildList()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (!m_sAppID.IsEmpty())
		{
			m_pcdl->SetAppID(XTP_CT2CW(m_sAppID));
		}
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				if (m_uiCategory != 0)
				{
					hr = m_pcdl->AppendKnownCategory((KNOWNDESTCATEGORY)m_uiCategory);
				}
			}
...
BEGIN_DISPATCH_MAP(CXTPTaskbarManagerJumpList, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPTaskbarManagerJumpList, CXTPCmdTarget)
	INTERFACE_PART(CXTPTaskbarManagerJumpList, XTPDIID_TaskbarManagerJumpList, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPTaskbarManagerJumpList, XTPDIID_TaskbarManagerJumpList)
