...
{
	ASSERT(pEditWnd);              // Must be valid
	ASSERT(pEditWnd->GetParent());
	...
	{
		hPopupIcon = XTPResourceManager()->LoadIcon(MAKEINTRESOURCE(XTP_IDS_ARROW), XTP_DPI_SIZE(CSize(16, 16)));
		dwStyle |= BS_ICON;
	}
...
#pragma warning(disable : 4127) // C4127: conditional expression is constant
		if (sizeof(OPENFILENAME) < 88 && XTPSystemVersion()->IsWin2KOrGreater() && dlg.m_ofn.lStructSize < 88)
#pragma warning(pop)
		{
