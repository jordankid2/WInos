...
BOOL CXTPEditThemeDefault::DrawNcBordersDefault(CDC* pDC, CRect rcWnd, CXTPEdit* pEdit)
{
	...
	{
		CRect rcClient(rcWnd.left + XTP_DPI_X(1), rcWnd.top + XTP_DPI_Y(1), rcWnd.right - XTP_DPI_X(1), rcWnd.bottom - XTP_DPI_Y(1));
		pDC->ExcludeClipRect(rcClient);

		if (SUCCEEDED(m_winTheme.DrawThemeBackground(*pDC, 0, pEdit->IsWindowEnabled() ? ETS_NORMAL : ETS_DISABLED, &rcWnd, NULL)))
		{
...
void CXTPEditThemeDefault::DrawNcScrollBack(CDC* pDC, CRect rcWnd, CXTPEdit* pEdit)
{
	...
	if ((pEdit->GetStyle() & (WS_VSCROLL | WS_HSCROLL)) == (WS_VSCROLL | WS_HSCROLL))
	{
		...
		rcWnd.SetRect(rcWnd.right - ::GetSystemMetrics(SM_CXVSCROLL), rcWnd.bottom - ::GetSystemMetrics(SM_CYHSCROLL), rcWnd.right, rcWnd.bottom);
...
void CXTPEditThemeDefault::DrawBuddyBorders(CWnd* pBuddy, CDC* pDC, CRect rc, int nAlignemnt, CXTPEdit* pEdit)
{
	ASSERT_VALID(pDC);
UNREFERENCED_PARAMETER(pBuddy);
	...
	{
		pDC->DrawEdge(rc, EDGE_SUNKEN, (nAlignemnt == UDS_ALIGNRIGHT) ? 
			BF_TOP | BF_BOTTOM | BF_RIGHT : BF_TOP | BF_BOTTOM | BF_LEFT);
	}
}
