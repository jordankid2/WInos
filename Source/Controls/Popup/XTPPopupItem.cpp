...
IMPLEMENT_DYNCREATE(CXTPPopupItem, CXTPCmdTarget)
...
{
	ASSERT(m_pControl);
	return (m_pControl && m_xtpFontText.m_hObject == NULL) ?
		&m_pControl->GetPaintManager()->m_xtpFontText : &m_xtpFontText;
}
...
void CXTPPopupItem::CalculateHeight()
{
	...
	if (pIcon)
	{
		m_rcItem.bottom = m_rcItem.top + XTP_DPI_Y(pIcon->GetHeight());
		return;
	}
...
void CXTPPopupItem::CalculateWidth()
{
	...
	if (pIcon)
	{
		m_rcItem.right = m_rcItem.left + XTP_DPI_X(pIcon->GetWidth());
		return;
	}
...
BEGIN_INTERFACE_MAP(CXTPPopupItem, CXTPCmdTarget)
	INTERFACE_PART(CXTPPopupItem, XTPDIID_IPopupControlItem, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPopupItem, XTPDIID_IPopupControlItem)

BEGIN_DISPATCH_MAP(CXTPPopupItem, CXTPCmdTarget)
