...
BOOL CXTPColorSelectorCtrl::SetColors(CXTPPickBtnArray& arColors, const XTP_PICK_BUTTON* pColors, const int nCount)
{
	ASSERT(pColors != NULL);
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		{
			CString strTip;
			CXTPResourceManager::AssertValid(XTPResourceManager()->LoadString(&strTip, btn.toolTipID));
...
CString CXTPColorSelectorCtrl::GetThemeTip(LPCTSTR lpszColor, LPCTSTR lpszDesc, int nShade)
{
	...
	{
		CString strAdjust;
		strAdjust.LoadString((nShade > 0) ? XTP_IDS_LIGHTER : XTP_IDS_DARKER);
...
BOOL CXTPColorSelectorCtrl::SetThemeColors(CXTPPickBtnArray& arColors, int nCols /*=10*/, LPCTSTR lpszTitle /*=NULL*/)
{
...
BOOL CXTPColorSelectorCtrl::SetThemeColors(XTP_PICK_BUTTON* pColors, int nCount, LPCTSTR lpszTitle, CStringArray& arStrings, int ppShadeMatrix[6][10])
{
	ASSERT(NULL != pColors);
	ASSERT(nCount == 10);
	...
	for (int r = 0; r < 6; ++r) // 6 rows.
	{
		...
		{
			XTP_PICK_BUTTON btn = {0};
			btn.clrButton = CXTPDrawHelpers::AdjustBrightness(pColors[c].clrButton, ppShadeMatrix[r][c]);
			...
			if (btn.toolTipID != 0)
			{
				CString strTip;
				CXTPResourceManager::AssertValid(XTPResourceManager()->LoadString(&strTip, btn.toolTipID));
				STRCPY_S(btn.szTip, _countof(btn.szTip), GetThemeTip(strTip, arStrings[c], ppShadeMatrix[r][c]));
			}
			else
			{
				STRCPY_S(btn.szTip, _countof(btn.szTip), GetThemeTip(pColors[c].szTip, arStrings[c], ppShadeMatrix[r][c]));
			}
...
BOOL CXTPColorSelectorCtrl::SetThemeColors(XTP_PICK_BUTTON* pColors /*=NULL*/, int nCount /*=10*/, LPCTSTR lpszTitle /*=NULL*/, UINT* pStringIDs /*=NULL*/, int ppShadeMatrix[6][10] /*=NULL*/)
{
	if (pColors == NULL)
	{
...
BOOL CXTPColorSelectorCtrl::SetThemeColors(XTP_PICK_BUTTON* pColors, int nCount, LPCTSTR lpszTitle, LPCTSTR* ppszStrings, int ppShadeMatrix[6][10])
{
...
BOOL CXTPColorSelectorCtrl::SetStandardColors(CXTPPickBtnArray& arColors, int nCols /*=10*/, LPCTSTR lpszTitle /*=NULL*/)
{
...
BOOL CXTPColorSelectorCtrl::SetStandardColors(XTP_PICK_BUTTON* pColors /*=NULL*/, int nCount /*=10*/, int nCols /*=10*/, LPCTSTR lpszTitle /*=NULL*/)
{
	if (pColors == NULL)
	{
...
int CXTPColorSelectorCtrl::OnCreate(LPCREATESTRUCT lpCS)
{
	...
	CRect rect;
	m_pTheme->CreateAutoButton(
		this, nIndex, x, y, cx, bChecked);
	m_pTheme->CreateColorButtons(
		this, nIndex, x, y, cx, bChecked);
	m_pTheme->CreateRecentColorButtons(
		this, nIndex, x, y, cx, bChecked);
	m_pTheme->CreateMoreButton(
		this, nIndex, x, y, cx, bChecked);
...
BOOL CXTPColorSelectorCtrl::SetTheme(XTPControlTheme eTheme)
{
	...
	switch (eTheme)
	{
		...
		default:
			bReturn = SetTheme(new CXTPColorSelectorCtrlTheme());
			break;
	}
...
void AFX_CDECL CXTPColorSelectorCtrl::SetRecentColors(const CUIntArray& arRecentColors, LPCTSTR lpszTitle /*=NULL*/)
{
...
void CXTPColorSelectorCtrlTheme::FillBackground(CDC* pDC, CXTPColorSelectorCtrl* pOwner, const CRect& rect)
{
	UNREFERENCED_PARAMETER(pOwner);
	pDC->FillSolidRect(&rect, m_crBack);
}

void CXTPColorSelectorCtrlTheme::DrawColorCell(CDC* pDC, CXTPColorSelectorCtrl* pOwner, CXTPColorSelectorCtrl::COLOR_CELL* pColorCell, BOOL bHilite, BOOL bPressed)
{
	...
	COLORREF crBorder = ((pColorCell->dwStyle & CPS_XTP_NOFILL) || ((pOwner->GetPopupStyle() & CPS_XTP_COLORBORDERS) == 0)) ?
		m_crBorder : CXTPDrawHelpers::AdjustBrightness(pColorCell->clr, -25);
	...
	if (nLen)
	{
		...
		pDC->DrawText(strText, nLen, rcText, nFormat);
	}
}
...
void CXTPColorSelectorCtrlTheme::DrawColorSelector(CDC* pDC, CXTPColorSelectorCtrl* pOwner)
{
	...
	if (pOwner->HasCaptions())
	{
		if (pOwner->GetThemeColors().GetSize() > 0 || pOwner->GetStandardColors().GetSize() > 0)
		{
			...
			{
				pDC->DrawText(pOwner->GetThemeTitle(),
					&m_rcThemeText, DT_VCENTER | DT_LEFT | DT_SINGLELINE);
			}
			if (pOwner->GetStandardColors().GetSize())
			{
				pDC->DrawText(pOwner->GetStandardTitle(),
					&m_rcStandardText, DT_VCENTER | DT_LEFT | DT_SINGLELINE);
			}
		}
		else
		{
			...
			{
				pDC->DrawText(pOwner->GetCustomTitle(),
					&m_rcStandardText, DT_VCENTER | DT_LEFT | DT_SINGLELINE);
			}
		...
		if (pOwner->GetPopupStyle() & CPS_XTP_MORECOLORS && pOwner->GetPopupStyle() & CPS_XTP_RECENTCOLORS)
		{
			if (pOwner->m_arRecentColors.GetSize() > 0)
			{
				pDC->DrawText(CXTPColorSelectorCtrl::m_strTitleRecent,
					&m_rcRecentText, DT_VCENTER | DT_LEFT | DT_SINGLELINE);
			}
		}
	}
	...
	for (pos = pOwner->GetColorCellList().GetHeadPosition(); pos;)
	{
		...
		BOOL bHighlighted = (((nIndex == pOwner->GetCurSel()) && (pOwner->GetPressed() == -1)) || (nIndex == pOwner->GetPressed()));
...
CSize CXTPColorSelectorCtrlTheme::CalcSize(CXTPColorSelectorCtrl* pOwner)
{
	...
	sz.cx = pOwner->GetBorderRect().left + (m_sizeButton.cx * pOwner->GetNumCols()) + pOwner->GetBorderRect().right;
	...
	if (pOwner->GetThemeColors().GetSize() > 0 || pOwner->GetStandardColors().GetSize() > 0)
	{
		...
		if (pOwner->GetThemeColors().GetSize())
		{
			sz.cy += CalcHeight(static_cast<int>(pOwner->GetThemeColors().GetSize()), pOwner->GetNumCols(), m_sizeButton.cy);
		...
		if (pOwner->GetStandardColors().GetSize())
		{
			sz.cy += CalcHeight(static_cast<int>(pOwner->GetStandardColors().GetSize()), pOwner->GetNumCols(), m_sizeButton.cy);
	...
	else
	{
		...
		{
			int nColors = (pOwner->GetExtendedColors() == NULL) ? _countof(_ExtendedColors) : pOwner->GetExtendedCount();
		...
		(pOwner->GetPopupStyle() & CPS_XTP_RECENTCOLORS) != 0)
	{
		...
		{
			int nColors = min(static_cast<int>(CXTPColorSelectorCtrl::m_arRecentColors.GetSize()), pOwner->GetNumCols());
			sz.cy += CalcHeight(nColors, pOwner->GetNumCols(), m_sizeButton.cy);
...
BOOL CXTPColorSelectorCtrlTheme::CreateAutoButton(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
	ASSERT_VALID(pOwner);
	...
	int nResID = (pOwner->GetPopupStyle() & CPS_XTP_NOFILL) ? XTP_IDS_CLR_NOFILL : XTP_IDS_CLR_AUTOMATIC;
	...
	pColorCell->rect     = CRect(x + pOwner->GetBorderRect().left, y, cx - pOwner->GetBorderRect().right, y + m_nAutoButton);
	...
	pOwner->GetToolTipCtrl().AddTool(pOwner, strColorToolTip, pColorCell->rect, nResID);
...
BOOL CXTPColorSelectorCtrlTheme::CreateExtendedButtons(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
	ASSERT_VALID(pOwner);

	// Create the color pick buttons.
	XTP_PICK_BUTTON* pickBtn = (pOwner->GetPopupStyle() & CPS_XTP_EXTENDED) ? (pOwner->GetExtendedColors() ? pOwner->GetExtendedColors() : _ExtendedColors) : _StandardColors;
	int nColors = (pOwner->GetPopupStyle() & CPS_XTP_EXTENDED) ? (pOwner->GetExtendedColors() ? pOwner->GetExtendedCount() : _countof(_ExtendedColors)) : _countof(_StandardColors);

	if (pOwner->HasCaptions() && ((pOwner->GetPopupStyle() & CPS_XTP_EXTENDED) == 0 || !pOwner->GetCustomTitle().IsEmpty()))
	{
	...
	CRect rcButton(x + pOwner->GetBorderRect().left, y, x + pOwner->GetBorderRect().left + m_sizeButton.cx, y + m_sizeButton.cy);
	...
	for (int i = 0; i < nColors; i++)
	{
		...
		{
			y += m_sizeButton.cy;
			rcButton = CRect(x + pOwner->GetBorderRect().left, y, x + pOwner->GetBorderRect().left + m_sizeButton.cx, y + m_sizeButton.cy);
		}
		nIndex++;
		...
		{
			CString strTip;
			CXTPResourceManager::AssertValid(XTPResourceManager()->LoadString(&strTip, pickBtn[i].toolTipID));
			pOwner->GetToolTipCtrl().AddTool(pOwner, strTip, pColorCell->rect, pickBtn[i].toolTipID);
		}
		...
		if ((pOwner->GetCurColor() == clrColor) && !bChecked)
		{
			bChecked = true;
			pColorCell->bChecked = true;
			pOwner->SetCurSel(pColorCell->nIndex);
		}
...
BOOL CXTPColorSelectorCtrlTheme::CreateThemeButtons(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
	ASSERT_VALID(pOwner);
	...
	if (pOwner->GetThemeColors().GetSize())
	{
		...
		for (int i = 0; i < pOwner->GetThemeColors().GetSize(); i++)
		{
			...
			pOwner->GetColorCellList().AddTail(pColorCell);
			pOwner->GetToolTipCtrl().AddTool(pOwner, pOwner->GetThemeColors()[i].szTip, pColorCell->rect, 1);
	...
	if (pOwner->GetStandardColors().GetSize())
	{
		...
		for (int i = 0; i < pOwner->GetStandardColors().GetSize(); i++)
		{
			...
			pOwner->GetColorCellList().AddTail(pColorCell);
			pOwner->GetToolTipCtrl().AddTool(pOwner, pOwner->GetStandardColors()[i].szTip, pColorCell->rect, 1);
...
BOOL CXTPColorSelectorCtrlTheme::CreateColorButtons(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
	ASSERT_VALID(pOwner);
...
BOOL CXTPColorSelectorCtrlTheme::CreateRecentColorButtons(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
	ASSERT_VALID(pOwner);
	...
	for (int i = 0; i < nColors; i++)
	{
		...
		CString strColor;
		strColor.Format(_T("R:%d, G:%d, B:%d"), GetRValue(clrColor), GetGValue(clrColor), GetBValue(clrColor));
		pOwner->GetToolTipCtrl().AddTool(pOwner, (clrColor == COLORREF_NULL) ? _T("Null") : strColor, pColorCell->rect, 1);
...
BOOL CXTPColorSelectorCtrlTheme::CreateMoreButton(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
	ASSERT_VALID(pOwner);
...
{
	m_sizeButton = XTP_DPI(CSize(16,16));
...
void CXTPColorSelectorCtrlThemeOffice2013::RefreshMetrics(CXTPColorSelectorCtrl* pOwner)
{
    ...
    m_crBorderHilight = XTPIniColor(_T("Controls.ColorPicker"), _T("BorderHighlight"), RGB(242, 148,  54));
    m_crBorderPressed = XTPIniColor(_T("Controls.ColorPicker"), _T("BorderPressed"),   RGB(239,  72,  16));
    m_crBorderInside  = XTPIniColor(_T("Controls.ColorPicker"), _T("BorderInside"),    RGB(255, 226, 148));
    m_crBack          = XTPIniColor(_T("Controls.ColorPicker"), _T("Back"),            RGB(255, 255, 255));
    m_crBackPressed   = XTPIniColor(_T("Controls.ColorPicker"), _T("BackPressed"),     RGB(163, 189, 227));
    m_crBackHilight   = XTPIniColor(_T("Controls.ColorPicker"), _T("BackHighlight"),   RGB(213, 225, 242));
    m_crBackChecked   = XTPIniColor(_T("Controls.ColorPicker"), _T("BackChecked"),     RGB(194, 213, 242));
    m_crPopupBorder   = XTPIniColor(_T("Controls.ColorPicker"), _T("PopupBorder"),     RGB(198, 198, 198));
...
void CXTPColorSelectorCtrlThemeOffice2013::DrawBorder(CDC* pDC, CRect rcItem, CRect rcBorder, COLORREF crBorder)
{
	if (rcBorder.left == 1)
	{
		pDC->FillSolidRect(rcItem.left, rcItem.top,
			XTP_DPI_X(1), rcItem.Height(), crBorder);
	}
	if (rcBorder.top == 1)
	{
		pDC->FillSolidRect(rcItem.left + XTP_DPI_X(1), rcItem.top,
			rcItem.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), crBorder);
	}
	if (rcBorder.right == 1)
	{
		pDC->FillSolidRect(rcItem.right - XTP_DPI_X(1), rcItem.top,
			XTP_DPI_X(1), rcItem.Height(), crBorder);
	}
...
void CXTPColorSelectorCtrlThemeOffice2013::DrawColor(CDC* pDC, CXTPColorSelectorCtrl* pOwner, CXTPColorSelectorCtrl::COLOR_CELL* pColorCell, CRect rcItem, BOOL bChecked, BOOL bHilite, BOOL bPressed)
{
	ASSERT_VALID(pDC);
	...
	{
		COLORREF crBorder = ((pColorCell->dwStyle & CPS_XTP_NOFILL) || ((pOwner->GetPopupStyle() & CPS_XTP_COLORBORDERS) == 0)) ?
			m_crBorder : CXTPDrawHelpers::AdjustBrightness(pColorCell->clr, -15);

		DrawBorder(pDC, rcItem, pColorCell->rcBorder, crBorder);
	}
}

void CXTPColorSelectorCtrlThemeOffice2013::DrawColorCell(CDC* pDC, CXTPColorSelectorCtrl* pOwner, CXTPColorSelectorCtrl::COLOR_CELL* pColorCell, BOOL bHilite, BOOL bPressed)
{
	ASSERT_VALID(pDC);
	ASSERT(NULL != pColorCell);
...
void CXTPColorSelectorCtrlThemeOffice2013::DrawColorSelector(CDC* pDC, CXTPColorSelectorCtrl* pOwner)
{
	pDC->SetBkMode(TRANSPARENT);
...
BOOL CXTPColorSelectorCtrlThemeOffice2013::CreateAutoButton(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
	y -= m_nMargin;
...
BOOL CXTPColorSelectorCtrlThemeOffice2013::CreateExtendedButtons(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
...
BOOL CXTPColorSelectorCtrlThemeOffice2013::CreateThemeButtons(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
	ASSERT_VALID(pOwner);
...
BOOL CXTPColorSelectorCtrlThemeOffice2013::CreateRecentColorButtons(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
...
BOOL CXTPColorSelectorCtrlThemeOffice2013::CreateMoreButton(CXTPColorSelectorCtrl* pOwner, int& nIndex, const int x, int& y, const int cx, bool& bChecked)
{
	y += m_nMargin;
...
void CXTPColorSelectorCtrlThemeVisualStudio2015::RefreshMetrics(CXTPColorSelectorCtrl* pOwner)
{
	...
	m_crBorderHilight = XTPIniColor(_T("Controls.ColorPicker"), _T("BorderHighlight"), RGB( 49, 154, 255));
	m_crBorderPressed = XTPIniColor(_T("Controls.ColorPicker"), _T("BorderPressed"),   RGB(  0, 121, 206));
    m_crBorderInside  = XTPIniColor(_T("Controls.ColorPicker"), _T("BorderInside"),    RGB(206, 223, 247));
	...
	m_crBackHilight   = XTPIniColor(_T("Controls.ColorPicker"), _T("BackHighlight"),   RGB(206, 223, 247));
	m_crBackChecked   = XTPIniColor(_T("Controls.ColorPicker"), _T("BackChecked"),     RGB(239, 239, 247));
	m_crPopupBorder   = XTPIniColor(_T("Controls.ColorPicker"), _T("PopupBorder"),     RGB(156, 158, 189));
	m_crText          = XTPIniColor(_T("Controls.ColorPicker"), _T("Text"),            RGB( 24,  28,  24));
	m_crTextPressed   = XTPIniColor(_T("Controls.ColorPicker"), _T("TextPressed"),     RGB(255, 255, 255));
...
void CXTPColorSelectorCtrlThemeVisualStudio2015::DrawColorCell(CDC* pDC, CXTPColorSelectorCtrl* pOwner, CXTPColorSelectorCtrl::COLOR_CELL* pColorCell, BOOL bHilite, BOOL bPressed)
{
	ASSERT(NULL != pColorCell);

	if (pColorCell->dwStyle == CPS_XTP_PICKBOX)
	{
		CXTPColorSelectorCtrlThemeOffice2013::DrawColorCell(pDC, pOwner, pColorCell, bHilite, bPressed);
	}
	else
	{
		ASSERT_VALID(pDC);
