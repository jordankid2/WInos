...
//  m_bMouseWasClickedSoDoNotShowTip = FALSE;
	...
	m_pointTipOffsetPos = XTP_DPI(CPoint(0, 20)); //20 = mouse cursor height
...
void CXTPTipWindow::InitializeSize(const CPoint& point, BOOL bCenterHorz /*FALSE*/)
{
	...
	m_rcScreenWindow.right = point.x + nWidth + XTP_DPI_X(2); //+2 for better view
	...
	if ((m_dwTipStyle & TWS_XTP_DROPSHADOW) && !m_bSystemShadow)
		m_rcShadow.OffsetRect(XTP_DPI_X(5), XTP_DPI_Y(5));
}
...
void CXTPTipWindow::DrawShadowRect(CDC* pDC, const CRect& rcShadow)
{
	...
	else
	{
		...
		int cx = rcShadow.Width() + XTP_DPI_X(5);
		int cy = rcShadow.Height() + XTP_DPI_Y(5);
		...
		for (x = 5; x < cx; ++x)
		{
			...
			for (y = 5; y < cy; ++y)
			{
				CPoint pt(XTP_DPI_X(x), XTP_DPI_Y(y));
				int iAlpha = 0;
...
void CXTPTipWindow::OnPaint()
{
	...
	// Draw the title text.
	CFont* pOldFont = dc.SelectObject(&XTPAuxData().xtpFontBold);
	dc.TabbedTextOut(m_sizeMargin.cx + XTP_DPI_X(1), m_sizeMargin.cy, m_strTitle, m_nTabPositions,
		m_pnTabStopPositions, m_sizeMargin.cx + XTP_DPI_X(1));

	// Draw the description text.
	dc.SelectObject(&XTPAuxData().xtpFont);

	if (m_arStrings.GetSize() == 0)
		dc.TabbedTextOut(m_sizeMargin.cx + XTP_DPI_X(1), m_sizeMargin.cy + ((m_strTitle.GetLength() > 0) ? m_sizeTitle.cy + m_nLineSpace : 0),
		m_strDescrip, m_nTabPositions, m_pnTabStopPositions, m_sizeMargin.cx + XTP_DPI_X(1));
	else
	{
		...
		int nStrings;
		for (nStrings = 0; nStrings < m_arStrings.GetSize(); ++nStrings)
			dc.TabbedTextOut(m_sizeMargin.cx + XTP_DPI_X(1),
			m_sizeMargin.cy + (nStrings * sizeDesc.cy) + ((m_strTitle.GetLength() > 0) ? m_sizeTitle.cy + m_nLineSpace : 0),
			m_arStrings[nStrings], m_nTabPositions, m_pnTabStopPositions, m_sizeMargin.cx + XTP_DPI_X(1));
