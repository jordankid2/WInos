...
void CXTPPopupThemeMSN::DrawBackground(CDC* pDC, CXTPPopupControl* pControl, CRect rcClient)
{
	...
	rcBackground = CRect(rcClient.left, rcBackground.bottom, rcClient.right, rcClient.top + XTP_DPI_Y(23));
	XTPDrawHelpers()->GradientFill(pDC, rcBackground, RGB(255, 255, 255), RGB(207, 221, 244), FALSE);

	rcBackground = CRect(rcClient.left, rcBackground.bottom, rcClient.right, rcClient.top + XTP_DPI_Y(40));
