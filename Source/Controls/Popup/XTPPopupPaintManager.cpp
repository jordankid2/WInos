...
void CXTPPopupPaintManager::DrawItemIcon(CDC* pDC, CXTPPopupItem* pItem)
{
	...
	if (pItem->IsPressed() && pItem->IsSelected())
	{
		...
		pIcon->Draw(pDC, rc.TopLeft(), pIcon->GetCheckedIcon(), XTP_DPI(pIcon->GetExtent()));
	}
	else if (pItem->IsSelected() || pItem->IsPressed())
	{
		if (pItem->IsButton())
		...
		pIcon->Draw(pDC, rc.TopLeft(), pIcon->GetHotIcon(), XTP_DPI(pIcon->GetExtent()));
	}
	else
	{
		pIcon->Draw(pDC, rc.TopLeft(), pIcon->GetIcon(), XTP_DPI(pIcon->GetExtent()));
	}
}
