	...
	COLORREF clrDefault/*= CLR_DEFAULT*/)
{
	...
	{
		SetBorders(XTP_DPI_X(4), XTP_DPI_Y(4), XTP_DPI_X(6), XTP_DPI_Y(6));
	}
	else
	{
		SetBorders(XTP_DPI_X(4), XTP_DPI_Y(4), XTP_DPI_X(10), XTP_DPI_Y(10));
	}
...
int CXTPColorPopup::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	BOOL bThinBorder = GetTheme()->m_bThinBorder;

	if (bThinBorder)
	{
		ModifyStyle(WS_DLGFRAME | WS_THICKFRAME | WS_BORDER, 0, 0);
		ModifyStyleEx(WS_EX_WINDOWEDGE | WS_EX_DLGMODALFRAME, WS_EX_STATICEDGE, SWP_FRAMECHANGED);
	}
	else
	{
	...
	if (bThinBorder && m_bDisplayShadow)
	{
...
void CXTPColorPopup::OnNcPaint()
{
	if (GetTheme()->m_bThinBorder)
	{
		...
		dc.Draw3dRect(0, 0, rc.Width(), rc.Height(),
			GetTheme()->m_crPopupBorder, GetTheme()->m_crPopupBorder);
	}
	else
	{
		Default();
