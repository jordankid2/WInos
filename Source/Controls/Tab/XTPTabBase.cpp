...
BOOL CXTPTabBase::Init()
{
	if (::IsWindow(m_pTabCtrl->GetSafeHwnd()))
	{
		m_pTabCtrl->SetFont(&XTPAuxData().xtpFont);
...
void CXTPTabBase::OnAddPadding(CString& strLabelText)
{
	if (!strLabelText.IsEmpty())
	{
		...
		{
			pfontNormal = &XTPAuxData().xtpFontVert;
			pfontSelect = &XTPAuxData().xtpFontVertBold;
		}
		else
		{
			pfontNormal = &XTPAuxData().xtpFont;
			pfontSelect = &XTPAuxData().xtpFontBold;
		}
...
void CXTPTabBase::GetChildRect(CRect& rcChild) const
{
	...
	if (dwStyle & TCS_VERTICAL)
	{
		...
		{
			rcChild.top += XTP_DPI_Y(TABVIEW_BORDER);
			rcChild.left += XTP_DPI_X(TABVIEW_BORDER);
			rcChild.right -= XTP_DPI_X(TABVIEW_BORDER) + cx;
			rcChild.bottom -= XTP_DPI_Y(TABVIEW_BORDER);
		}
		...
		{
			rcChild.top += XTP_DPI_Y(TABVIEW_BORDER);
			rcChild.left += XTP_DPI_X(TABVIEW_BORDER) + cx;
			rcChild.right -= XTP_DPI_X(TABVIEW_BORDER);
			rcChild.bottom -= XTP_DPI_Y(TABVIEW_BORDER);
		}
	}
	...
	else
	{
		...
		if (dwStyle & TCS_BOTTOM)
		{
			...
			rcChild.left += XTP_DPI_X(TABVIEW_BORDER);
			rcChild.right -= XTP_DPI_X(TABVIEW_BORDER);
			rcChild.bottom -= XTP_DPI_Y(TABVIEW_BORDER) + cy;
		}

		// Top
		else
		{
			rcChild.top += XTP_DPI_Y(TABVIEW_BORDER) + cy;
			rcChild.left += XTP_DPI_X(TABVIEW_BORDER);
			rcChild.right -= XTP_DPI_X(TABVIEW_BORDER);
			rcChild.bottom -= XTP_DPI_Y(TABVIEW_BORDER);
		}
...
CWnd* CXTPTabExBase::CreateTabView(CRuntimeClass* pViewClass, CDocument* pDocument, CCreateContext* pContext)
{
		...
		&& pViewClass->IsDerivedFrom(RUNTIME_CLASS(CView)))
	{
		...
		BOOL bFound = FALSE;
		for (POSITION pos = pDocument->GetFirstViewPosition(); NULL != pos;)
		{
			if (pDocument->GetNextView(pos) == pView)
			{
				bFound = TRUE;
				break;
			}
...
BOOL CXTPTabExBase::UpdateTabLabel(int iTab, CXTPTcbItem* pMember, LPCTSTR lpszLabel)
{
	...
	STRCPY_S(szText, _countof(szText), strLabel);
...
{
	int len = pDC->GetTextExtent(sLabel).cx + XTP_DPI_X(6) + XTP_DPI_X(6);
	if (sLabel.GetLength() == 1) ++len;
	if (bHasIcon)
	{
		len += XTP_DPI_X(16) + XTP_DPI_X(6);
	}
	return len;
}
...
void CXTPTabExBase::Condense()
{
	...
	const int iMinTabWidth = tm.tmAveCharWidth * 6 + XTP_DPI_X(12);
