...
void CXTPTabBaseTheme::RefreshMetrics(CTabCtrl* /*pTab*/)
{
	...
	m_themeTab->OpenThemeData(NULL, L"TAB");
}
...
void CXTPTabBaseTheme::FillHeader(CDC* pDC, CXTPTabBase* pTabCtrlBase, CRect rcHeader)
{
	...
	if (m_bUseWinThemes && m_themeTab->IsAppThemeReady())
	{
		...
		if (IsRight(dwStyle))
		{
			rc.right = rcHeader.left + XTP_DPI_X(1);
		}
		...
		{
			rc.bottom = rcHeader.top + XTP_DPI_Y(1);
		}
		else
		{
	...
	if (IsRight(dwStyle))
	{
		pDC->FillSolidRect(rcHeader.left - XTP_DPI_X(2), rcHeader.top,
			XTP_DPI_X(1), rcHeader.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DSHADOW));
		pDC->FillSolidRect(rcHeader.left - XTP_DPI_X(1), rcHeader.top,
			XTP_DPI_X(1), rcHeader.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DDKSHADOW));
	}
	else if (IsLeft(dwStyle))
	{
		pDC->FillSolidRect(rcHeader.right, rcHeader.top,
			XTP_DPI_X(1), rcHeader.Height(), GetXtremeColor(COLOR_3DHILIGHT));
	}
	else if (IsBottom(dwStyle))
	{
		pDC->FillSolidRect(rcHeader.left + XTP_DPI_X(1), rcHeader.top - XTP_DPI_Y(2),
			rcHeader.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), GetXtremeColor(COLOR_3DSHADOW));
		pDC->FillSolidRect(rcHeader.left, rcHeader.top - XTP_DPI_Y(1),
			rcHeader.Width(), XTP_DPI_Y(1), GetXtremeColor(COLOR_3DDKSHADOW));
	}
	else
	{
		pDC->FillSolidRect(rcHeader.left, rcHeader.bottom,
			rcHeader.Width() - XTP_DPI_X(1), XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));
	}
}
...
void CXTPTabBaseTheme::VerticalLine(CDC* pDC, int x, int y, int nLength, COLORREF clr)
{
	if (clr != COLORREF_NULL)
		pDC->FillSolidRect(x, y, XTP_DPI_X(1), nLength, clr);
}

void CXTPTabBaseTheme::HorizontalLine(CDC* pDC, int x, int y, int nLength, COLORREF clr)
{
	if (clr != COLORREF_NULL)
		pDC->FillSolidRect(x, y, nLength, XTP_DPI_Y(1), clr);
}
...
void CXTPTabBaseTheme::FillTabFace(CDC* pDC, CTabCtrl* pTabCtrl, CRect rcItem, BOOL bSelected)
{
	...
	if (IsRight(dwStyle))
	{
		if (bSelected)
		{
			rcItem.InflateRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(1), XTP_DPI_Y(2));

			// paint the selected tab background.
			pDC->FillSolidRect(&rcItem, GetBackColor());

			HorizontalLine(pDC, rcItem.left, rcItem.top, rcItem.Width() - XTP_DPI_X(1), GetXtremeColor(COLOR_3DHILIGHT));
			HorizontalLine(pDC, rcItem.left, rcItem.top + XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DFACE));
			Pixel(pDC, rcItem.right - XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));

			VerticalLine(pDC, rcItem.right, rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(4), GetXtremeColor(COLOR_3DDKSHADOW));
			VerticalLine(pDC, rcItem.right - XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(4), GetXtremeColor(COLOR_3DSHADOW));

			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.bottom - XTP_DPI_Y(2), rcItem.Width() - XTP_DPI_X(1), GetXtremeColor(COLOR_3DSHADOW));
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.bottom - XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DDKSHADOW));
			Pixel(pDC, rcItem.right - XTP_DPI_X(1), rcItem.bottom - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DDKSHADOW));
		}
		else
		{
			// paint the selected tab background.
			...
			HorizontalLine(pDC, rcItem.left, rcItem.top, rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DHILIGHT));
			HorizontalLine(pDC, rcItem.left, rcItem.top + XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DFACE));
			Pixel(pDC, rcItem.right - XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));

			VerticalLine(pDC, rcItem.right - XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(4), GetXtremeColor(COLOR_3DDKSHADOW));
			VerticalLine(pDC, rcItem.right - XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(4), GetXtremeColor(COLOR_3DSHADOW));

			HorizontalLine(pDC, rcItem.left, rcItem.bottom - XTP_DPI_Y(2), rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DSHADOW));
			HorizontalLine(pDC, rcItem.left, rcItem.bottom - XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DDKSHADOW));
			Pixel(pDC, rcItem.right - XTP_DPI_X(2), rcItem.bottom - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DDKSHADOW));
		}
	}
	else if (IsLeft(dwStyle))
	{
		if (bSelected)
		{
			rcItem.InflateRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(1), XTP_DPI_Y(1));

			// paint the selected tab background.
			pDC->FillSolidRect(&rcItem, GetBackColor());

			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.top, rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DHILIGHT));
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DFACE));
			Pixel(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));

			VerticalLine(pDC, rcItem.left, rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(3), GetXtremeColor(COLOR_3DHILIGHT));
			VerticalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(3), GetXtremeColor(COLOR_3DFACE));

			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.bottom - XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DSHADOW));
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.bottom, rcItem.Width() - XTP_DPI_X(3), GetXtremeColor(COLOR_3DDKSHADOW));
			Pixel(pDC, rcItem.left + XTP_DPI_X(1), rcItem.bottom - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DDKSHADOW));
		}
		else
		{
			// paint the selected tab background.
			...
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.top, rcItem.Width() - XTP_DPI_X(1), GetXtremeColor(COLOR_3DHILIGHT));
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DFACE));
			Pixel(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));

			VerticalLine(pDC, rcItem.left, rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(4), GetXtremeColor(COLOR_3DHILIGHT));
			VerticalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(4), GetXtremeColor(COLOR_3DFACE));

			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.bottom - XTP_DPI_Y(2), rcItem.Width() - XTP_DPI_X(1), GetXtremeColor(COLOR_3DSHADOW));
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.bottom - XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(2), GetXtremeColor(COLOR_3DDKSHADOW));
			Pixel(pDC, rcItem.left + XTP_DPI_X(1), rcItem.bottom - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DDKSHADOW));
		}
	}
	else if (IsBottom(dwStyle))
	{
		if (bSelected)
		{
			rcItem.InflateRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_X(1));

			// paint the selected tab background.
			pDC->FillSolidRect(&rcItem, GetBackColor());

			VerticalLine(pDC, rcItem.left, rcItem.top, rcItem.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));
			VerticalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top, rcItem.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DFACE));
			Pixel(pDC, rcItem.left + XTP_DPI_X(1), rcItem.bottom - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));

			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.bottom, rcItem.Width() - XTP_DPI_X(4), GetXtremeColor(COLOR_3DDKSHADOW));
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.bottom - XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(4), GetXtremeColor(COLOR_3DSHADOW));

			VerticalLine(pDC, rcItem.right - XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(1), rcItem.Height() - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DDKSHADOW));
			VerticalLine(pDC, rcItem.right - XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(1), rcItem.Height() - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DSHADOW));
			Pixel(pDC, rcItem.right - XTP_DPI_X(2), rcItem.bottom - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DDKSHADOW));
		}
		else
		{
			// paint the selected tab background.
			...
			VerticalLine(pDC, rcItem.left, rcItem.top, rcItem.Height() - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DHILIGHT));
			VerticalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top, rcItem.Height() - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DFACE));
			Pixel(pDC, rcItem.left + XTP_DPI_X(1), rcItem.bottom - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DHILIGHT));

			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.bottom - XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(4), GetXtremeColor(COLOR_3DDKSHADOW));
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.bottom - XTP_DPI_Y(2), rcItem.Width() - XTP_DPI_X(4), GetXtremeColor(COLOR_3DSHADOW));

			VerticalLine(pDC, rcItem.right - XTP_DPI_X(1), rcItem.top - XTP_DPI_Y(1), rcItem.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DDKSHADOW));
			VerticalLine(pDC, rcItem.right - XTP_DPI_X(2), rcItem.top - XTP_DPI_Y(1), rcItem.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DSHADOW));
			Pixel(pDC, rcItem.right - XTP_DPI_X(2), rcItem.bottom - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DDKSHADOW));
		}
	}
	else
	{
		if (bSelected)
		{
			...
			VerticalLine(pDC, rcItem.left, rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));
			VerticalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DFACE));
			Pixel(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));

			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.top, rcItem.Width() - XTP_DPI_X(4), GetXtremeColor(COLOR_3DHILIGHT));
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(1), rcItem.Width() - XTP_DPI_X(4), GetXtremeColor(COLOR_3DFACE));

			VerticalLine(pDC, rcItem.right - XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(3), GetXtremeColor(COLOR_3DDKSHADOW));
			VerticalLine(pDC, rcItem.right - XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(3), GetXtremeColor(COLOR_3DSHADOW));
			Pixel(pDC, rcItem.right - XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(1), GetXtremeColor(COLOR_3DDKSHADOW));
		}
		else
		{
			// paint the selected tab background.
			...
			VerticalLine(pDC, rcItem.left, rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DHILIGHT));
			VerticalLine(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DFACE));
			Pixel(pDC, rcItem.left + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));

			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.top, rcItem.Width() - XTP_DPI_X(4), GetXtremeColor(COLOR_3DHILIGHT));
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.top + 1, rcItem.Width() - XTP_DPI_X(4), GetXtremeColor(COLOR_3DFACE));

			VerticalLine(pDC, rcItem.right - XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DDKSHADOW));
			VerticalLine(pDC, rcItem.right - XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(2), rcItem.Height() - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DSHADOW));
			Pixel(pDC, rcItem.right - XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(1), GetXtremeColor(COLOR_3DDKSHADOW));
		}
	}
}
...
void CXTPTabBaseTheme::DrawTabIcon(CDC* pDC, CTabCtrl* pTabCtrl, CRect& rcItem, int iItem)
{
	...
	if (pTabCtrl->GetItem(iItem, &tci))
	{
		...
		if (pImageList && tci.iImage >= 0)
		{
			...
			if (IsHorz(dwStyle))
			{
				point.Offset(XTP_DPI_X(m_iEdge) * 2, XTP_DPI_Y(IsTop(dwStyle) ? m_iEdge : 1));
			}
			else
			{
				point.Offset(XTP_DPI_X(IsLeft(dwStyle) ? m_iEdge : 1), XTP_DPI_Y(m_iEdge) * 2);
			}
			...
			CSize szImage(info.rcImage.right - info.rcImage.left,
				info.rcImage.bottom - info.rcImage.top);
			CSize szScaledImage;
			if (IsHorz(dwStyle))
			{
				szScaledImage.cy = rcItem.Height() - (point.y - rcItem.top) * 2;
				szScaledImage.cx = MulDiv(szScaledImage.cy, szImage.cx, szImage.cy);
			}
			else
			{
				szScaledImage.cx = rcItem.Width() - (point.x - rcItem.left) * 2;
				szScaledImage.cy = MulDiv(szScaledImage.cx, szImage.cy, szImage.cx);
			}

			CRect rcImage(point, szScaledImage);
			CXTPDrawHelpers::StretchImageListImage(pDC, rcImage, pImageList, tci.iImage);

			if (IsHorz(dwStyle))
			{
				rcItem.left = rcImage.right + XTP_DPI_X(m_iEdge);
			}
			else
			{
				rcItem.top = rcImage.bottom + XTP_DPI_Y(m_iEdge);
			}
		}
	}
}
...
void CXTPTabBaseTheme::DrawTabText(CDC* pDC, CTabCtrl* pTabCtrl, CRect& rcItem, int iItem, BOOL bSelected, BOOL bBoldFont)
{
	...
	if (pTabCtrl->GetItem(iItem, &tci))
	{
		...
		if (nLen > 0)
		{
			...
			DWORD dwStyle = GetTabStyle(pTabCtrl);
			CXTPFontDC font(pDC, (bBoldFont && bSelected) ? (IsHorz(dwStyle) ? &XTPAuxData().xtpFontBold : &XTPAuxData().xtpFontVertBold) 
				: (IsHorz(dwStyle) ? &XTPAuxData().xtpFont : &XTPAuxData().xtpFontVert));
			...
			else
			{
				...
				rcItem.left = rcItem.right - (rcItem.Width() - sz.cy + XTP_DPI_Y(1)) / 2;
				rcItem.top = rcItem.top + (rcItem.Height() - sz.cx + XTP_DPI_X(1)) / 2;
...
CRect CXTPTabBaseTheme::GetHeaderRect(CXTPTabBase* pTabCtrlBase)
{
	...
	{
		rcHeader.bottom = (rcHeader.top + rcItem.Height()) * pTabCtrl->GetRowCount() + XTP_DPI_Y(m_iEdge);
	}
	return rcHeader;
}
...
void CXTPTabBaseThemeOfficeXP::DrawTabIcon(CDC* pDC, CTabCtrl* pTabCtrl, CRect& rcItem, int iItem)
{
	...
	if (pTabCtrl->GetItem(iItem, &tci))
	{
		...
		if (pImageList && tci.iImage >= 0)
		{
			...
			if (IsHorz(dwStyle))
			{
				point.Offset(XTP_DPI_X(m_iEdge) * 2, XTP_DPI_Y(m_iEdge) / 2);
			}
			else
			{
				point.Offset(XTP_DPI_X(IsRight(dwStyle) ? 1 : 2), m_iEdge * 2);
			}
			...
			{
				rcItem.top = rcImage.top + XTP_DPI_Y(m_iEdge);
			}
		}
	}
}
...
void CXTPTabBaseThemeOfficeXP::FillHeader(CDC* pDC, CXTPTabBase* pTabCtrlBase, CRect rcHeader)
{
	...
	else if (IsBottom(dwStyle))
	{
		pDC->FillSolidRect(rcHeader.left, rcHeader.top,
			rcHeader.Width(), XTP_DPI_Y(1), GetXtremeColor(COLOR_WINDOWFRAME));
	}
	else
	{
		pDC->FillSolidRect(rcHeader.left, rcHeader.bottom - XTP_DPI_Y(1),
			rcHeader.Width(), XTP_DPI_Y(1), GetXtremeColor(COLOR_3DFACE));

		pDC->FillSolidRect(rcHeader.left, rcHeader.bottom - XTP_DPI_Y(2),
			rcHeader.Width(), XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));
	}
}
...
void CXTPTabBaseThemeOfficeXP::FillTabFace(CDC* pDC, CTabCtrl* pTabCtrl, CRect rcItem, BOOL bSelected)
{
	...
	if (IsRight(dwStyle))
	{
		if (bSelected)
		{
			rcItem.InflateRect(XTP_DPI_X(1), XTP_DPI_Y(-3), XTP_DPI_X(-1), XTP_DPI_Y(1));

			// paint the selected tab background.
			pDC->FillSolidRect(&rcItem, GetBackColor());

			HorizontalLine(pDC, rcItem.left, rcItem.top, rcItem.Width(), GetXtremeColor(COLOR_3DHILIGHT));
			VerticalLine(pDC, rcItem.right, rcItem.top, rcItem.Height() + XTP_DPI_Y(1), GetXtremeColor(COLOR_WINDOWFRAME));
			HorizontalLine(pDC, rcItem.left, rcItem.bottom, rcItem.Width(), GetXtremeColor(COLOR_WINDOWFRAME));
		}
		else
		{
			HorizontalLine(pDC, rcItem.left + XTP_DPI_X(2), rcItem.bottom + XTP_DPI_Y(3), 
				rcItem.Width() - XTP_DPI_X(4), GetXtremeColor(COLOR_3DSHADOW));
		}
	}
	else if (IsLeft(dwStyle))
	{
	...
	else if (IsBottom(dwStyle))
	{
		if (bSelected)
		{
			rcItem.InflateRect(XTP_DPI_X(-2), 0, XTP_DPI_X(1), XTP_DPI_Y(1));

			// paint the selected tab background.
			pDC->FillSolidRect(&rcItem, GetBackColor());

			VerticalLine(pDC, rcItem.left, rcItem.top + XTP_DPI_Y(1), rcItem.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));
			HorizontalLine(pDC, rcItem.left, rcItem.bottom - XTP_DPI_Y(1), rcItem.Width(), GetXtremeColor(COLOR_WINDOWFRAME));
			VerticalLine(pDC, rcItem.right, rcItem.top + XTP_DPI_Y(1), rcItem.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_WINDOWFRAME));
		}
		else
		{
			VerticalLine(pDC, rcItem.right + XTP_DPI_X(1), rcItem.top + XTP_DPI_Y(2), 
				rcItem.Height() - XTP_DPI_Y(3), GetXtremeColor(COLOR_3DSHADOW));
		}
	}
	else
	{
		if (bSelected)
		{
			rcItem.InflateRect(XTP_DPI_X(-2), XTP_DPI_Y(1), XTP_DPI_X(2), XTP_DPI_Y(2));

			// paint the selected tab background.
			pDC->FillSolidRect(&rcItem, GetBackColor());

			VerticalLine(pDC, rcItem.left, rcItem.top + XTP_DPI_Y(1), rcItem.Height() - XTP_DPI_Y(2), GetXtremeColor(COLOR_3DHILIGHT));
			HorizontalLine(pDC, rcItem.left, rcItem.top, rcItem.Width(), GetXtremeColor(COLOR_3DHILIGHT));
			VerticalLine(pDC, rcItem.right, rcItem.top, rcItem.Height() - XTP_DPI_Y(1), GetXtremeColor(COLOR_WINDOWFRAME));
		}
		else
		{
			VerticalLine(pDC, rcItem.right + XTP_DPI_X(2), rcItem.top + XTP_DPI_Y(2), 
				rcItem.Height() - XTP_DPI_Y(3), GetXtremeColor(COLOR_3DSHADOW));
		}
	}
}
...
COLORREF CXTPTabBaseThemeOfficeXP::FillButtons(CDC* pDC, CXTPTabCtrlButtons* pButton)
{
	...
	{
		pDC->FillSolidRect(0, 0, rcClient.right, XTP_DPI_Y(1), GetXtremeColor(COLOR_BTNTEXT));
	}
	else
	{
		pDC->FillSolidRect(0, rcClient.bottom - XTP_DPI_Y(1), 
			rcClient.right, XTP_DPI_Y(1), GetXtremeColor(COLOR_3DHILIGHT));
	}
...
void CXTPTabBaseThemeOfficeXP::DrawButton(CDC* pDC, CXTPTabCtrlButton* pButton, COLORREF clrButton)
{
	...
	if (wStyle & DB_CROSS)
	{
		XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolClose, rc, clrButton);
	}
	else if (wStyle & DB_LEFT)
	{
		CPoint pts[] =
		{
			CPoint(rc.right - XTP_DPI_X(5), rc.top + XTP_DPI_Y(2)),
			CPoint(rc.right - XTP_DPI_X(9), rc.top + XTP_DPI_Y(6)),
			CPoint(rc.right - XTP_DPI_X(5), rc.top + XTP_DPI_Y(10))
		};
		pDC->Polygon(pts, 3);
	}
	else if (wStyle & DB_RIGHT)
	{
		CPoint pts[] =
		{
			CPoint(rc.left + XTP_DPI_X(4), rc.top + XTP_DPI_Y(2)),
			CPoint(rc.left + XTP_DPI_X(8), rc.top + XTP_DPI_Y(6)),
			CPoint(rc.left + XTP_DPI_X(4), rc.top + XTP_DPI_Y(10))
		};
		pDC->Polygon(pts, 3);
	}
...
void CXTPTabBaseThemeOffice2003::FillHeader(CDC* pDC, CXTPTabBase* pTabCtrlBase, CRect rcHeader)
{
	...
	{
		pDC->FillSolidRect(rcHeader.left - XTP_DPI_X(1), rcHeader.top,
			XTP_DPI_X(1), rcHeader.Height(), m_clrDarkShadow);
	}
	else if (IsLeft(dwStyle))
	{
		pDC->FillSolidRect(rcHeader.right, rcHeader.top,
	...
	{
		pDC->FillSolidRect(rcHeader.left, rcHeader.top - XTP_DPI_Y(1),
			rcHeader.Width(), XTP_DPI_Y(1), m_clrDarkShadow);
	}
	else
	{
		pDC->FillSolidRect(rcHeader.left, rcHeader.bottom - XTP_DPI_Y(2),
...
void CXTPTabBaseThemeOffice2003::FillTabFace(CDC* pDC, CTabCtrl* pTabCtrl, CRect rcItem, BOOL bSelected)
{
	...
	if (IsRight(dwStyle))
	{
		COLORREF clr = GradientFillTabFace(pDC, pTabCtrl, CRect(rc.left - (bSelected ? XTP_DPI_X(1) : 0), 
			rc.top + XTP_DPI_Y(1), rc.right, rc.bottom), bSelected);

		HorizontalLine(pDC, rc.left, rc.top, rc.Width() - XTP_DPI_X(1), m_clrDarkShadow);
		Pixel(pDC, rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), m_clrDarkShadow);

		VerticalLine(pDC, rc.right - 0, rc.top + XTP_DPI_Y(2), rc.Height() - XTP_DPI_Y(3), m_clrDarkShadow);

		HorizontalLine(pDC, rc.left, rc.bottom, rc.Width() - XTP_DPI_X(1), m_clrDarkShadow);
		Pixel(pDC, rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1), m_clrDarkShadow);

		if (bSelected)
		{
			VerticalLine(pDC, rc.left - XTP_DPI_X(2), rcClient.top, rcClient.Height(), clr);
		}
	}
	else if (IsLeft(dwStyle))
	{
		COLORREF clr = GradientFillTabFace(pDC, pTabCtrl, CRect(rc.left + XTP_DPI_X(1), 
			rc.top + XTP_DPI_Y(1), rc.right + (bSelected ? XTP_DPI_X(1) : 0), rc.bottom), bSelected);

		HorizontalLine(pDC, rc.left + XTP_DPI_X(2), rc.top, rc.Width() - XTP_DPI_X(2), m_clrDarkShadow);
		Pixel(pDC, rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), m_clrDarkShadow);

		VerticalLine(pDC, rc.left, rc.top + XTP_DPI_Y(2), rc.Height() - XTP_DPI_Y(3), m_clrDarkShadow);

		HorizontalLine(pDC, rc.left + XTP_DPI_X(2), rc.bottom, rc.Width() - XTP_DPI_X(1), m_clrDarkShadow);
		Pixel(pDC, rc.left + XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1), m_clrDarkShadow);

		if (bSelected)
		{
			VerticalLine(pDC, rc.right + XTP_DPI_X(1), rcClient.top, rcClient.Height(), clr);
		}
	}
	else if (IsBottom(dwStyle))
	{
		...
		COLORREF clr = GradientFillTabFace(pDC, pTabCtrl, CRect(rc.left + XTP_DPI_X(1), rc.top - (bSelected ? XTP_DPI_X(1) : 0), rc.right, rc.bottom - XTP_DPI_Y(1)), bSelected);

		VerticalLine(pDC, rc.left, rc.top, rc.Height() - XTP_DPI_Y(1), m_clrDarkShadow);
		Pixel(pDC, rc.left + XTP_DPI_X(1), rc.top + rc.Height() - XTP_DPI_Y(1), m_clrDarkShadow);

		HorizontalLine(pDC, rc.left + XTP_DPI_X(2), rc.bottom, rc.Width() - XTP_DPI_X(3), m_clrDarkShadow);

		VerticalLine(pDC, rc.right, rc.top, rc.Height() - XTP_DPI_Y(1), m_clrDarkShadow);
		Pixel(pDC, rc.left + rc.Width() - XTP_DPI_X(1), rc.top + rc.Height() - XTP_DPI_Y(1), m_clrDarkShadow);

		if (bSelected)
		{
			HorizontalLine(pDC, rcClient.left, rc.top - XTP_DPI_Y(2), rcClient.Width(), clr);
		}
	}
	else
	{
		rc.InflateRect(0, XTP_DPI_Y(2), 0, 0);

		COLORREF clr = GradientFillTabFace(pDC, pTabCtrl, CRect(rc.left + XTP_DPI_X(1), 
			rc.top + XTP_DPI_Y(1), rc.right, rc.bottom + (bSelected ? XTP_DPI_Y(1) : 0)), bSelected);

		VerticalLine(pDC, rc.left, rc.top + XTP_DPI_Y(2), rc.Height() - XTP_DPI_Y(2), m_clrDarkShadow);
		Pixel(pDC, rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), m_clrDarkShadow);

		HorizontalLine(pDC, rc.left + XTP_DPI_X(2), rc.top, rc.Width() - XTP_DPI_X(3), m_clrDarkShadow);

		VerticalLine(pDC, rc.right, rc.top + XTP_DPI_Y(2), rc.Height() - XTP_DPI_Y(2), m_clrDarkShadow);
		Pixel(pDC, rc.left + rc.Width() - XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), m_clrDarkShadow);

		if (bSelected)
		{
			HorizontalLine(pDC, rcClient.left, rc.bottom + XTP_DPI_Y(1), rcClient.Width(), clr);
		}
	}
}
