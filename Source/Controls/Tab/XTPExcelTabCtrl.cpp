...
, m_bSubclassed(TRUE)
{
	...
	m_nClientWidth = 0;
	m_nClientHeight = 0;
	m_pNormFont = &XTPAuxData().xtpFont;
	m_pBoldFont = &XTPAuxData().xtpFontBold;
	...
	m_cx = ::GetSystemMetrics(SM_CXHSCROLL) - XTP_DPI_X(1);
...
BOOL CXTPExcelTabCtrl::SetTheme(XTPControlTheme eTheme)
{
	...
	switch (eTheme)
	{
		case xtpControlThemeOfficeXP:         m_pTheme = new CXTPExcelTabCtrlThemeOfficeXP; break;
		case xtpControlThemeOffice2003:       m_pTheme = new CXTPExcelTabCtrlThemeOffice2003; break;
		case xtpControlThemeOffice2013:       m_pTheme = new CXTPExcelTabCtrlThemeOffice2013; break;
		case xtpControlThemeVisualStudio2015: m_pTheme = new CXTPExcelTabCtrlThemeVisualStudio2015; break;
		default:                              m_pTheme = new CXTPExcelTabCtrlTheme; break;
	}

	m_wndHScrollBar.SetTheme(eTheme);
	RefreshMetrics();
...
void CXTPExcelTabCtrl::SetGripperPosition(int x, bool bPercent)
{
	...
	m_rectGripper.right = m_rectGripper.left + XTP_DPI_X(GRIPPER_WIDTH);

	if (m_cy < m_iTabHeight)
	{
		m_rectSB_H.left = m_rectGripper.right;
		m_rectSB_H.top = m_rectTabs.bottom - m_iTabHeight + ((m_iTabHeight - m_cy) / 2);
		m_rectSB_H.right = m_rectTabs.right - m_cx;
		m_rectSB_H.bottom = m_rectSB_H.top + m_cy;
		m_rectGripper.top = m_rectSB_H.top;
		m_rectGripper.bottom = m_rectSB_H.bottom;
	}
	else
	{
...
int CXTPExcelTabCtrl::GetTabWidth(int nItem) const
{
	...
	return size.cx + m_iTabHeight + (m_iTabHeight / 2);
}
...
void CXTPExcelTabCtrl::OnLeftArrow()
{
	...
	CPoint Point(GetTotalArrowWidth() + XTP_DPI_X(1) + GetOverlap(), XTP_DPI_Y(1));
...
{
	m_nOffset = GetTotalTabWidth() - GetTotalTabAreaWidth() + XTP_DPI_X(1);
	EnableButtons();
	InvalidateTabs();
}
...
void CXTPExcelTabCtrl::OnDraw(CDC* pDC)
{
	...
	m_pTheme->FillBackground(pDC, rectPaint, (m_dwStyle & FTS_XTP_BOTTOM) != 0);

	// Exclude the arrow buttons from being repainted.
	const int iTotalArrowWidth = GetTotalArrowWidth();
	CRect rcArrows(rectPaint);
	rcArrows.top += XTP_DPI_Y(1);
	rcArrows.right = rcArrows.left + iTotalArrowWidth - XTP_DPI_X(1);
	...
	if (rectPaint.right >= rcArrows.right)
	{
		...
		CBitmap bitmap;
		bitmap.CreateCompatibleBitmap(pDC, nTotalTabWidth, m_iTabHeight);
		CBitmap* pOldBitmap = tabDC.SelectObject(&bitmap);

		// Fill the tab background color.
		m_pTheme->FillTabsBackground(&tabDC, nTotalTabWidth, m_iTabHeight);
		...
		pDC->BitBlt(iTotalArrowWidth, rectPaint.top, GetTotalTabAreaWidth(),
			m_iTabHeight, &tabDC, m_nOffset, 0, SRCCOPY);
		// cleanup.
...
void CXTPExcelTabCtrl::RecalcLayout()
{
	...
	{
		m_rectTabs.top = m_nClientHeight - m_iTabHeight;
		m_rectViews.top = 0;
	}
	else
	{
		m_rectTabs.top = 0;
		m_rectViews.top = m_iTabHeight;
	}
	m_rectTabs.bottom = m_rectTabs.top + m_iTabHeight;
	m_rectViews.bottom = m_rectViews.top + m_nClientHeight - m_iTabHeight;
	...
	if (m_dwStyle & FTS_XTP_HASARROWS)
	{
		int iTop = m_rectTabs.top + XTP_DPI_Y(1);
		int iBottom = m_rectTabs.bottom;

		if (m_cy < m_iTabHeight)
		{
			iTop = m_rectTabs.bottom - m_iTabHeight + ((m_iTabHeight - m_cy) / 2);
			iBottom = iTop + m_cy;
		}

		CRect btnRects[] =
		{
			CRect(0, iTop, m_cx, iBottom),
			CRect(m_cx, iTop, m_cx * 2, iBottom),
			CRect(m_cx * 2, iTop, m_cx * 3, iBottom),
			CRect(m_cx * 3, iTop, m_cx * 4, iBottom)
		};
...
{
	return ((m_iTabHeight / 2) + XTP_DPI_Y(2));
}
...
void CXTPExcelTabCtrl::SetTabHeight(int iTabHeight)
{
	if ((iTabHeight >= m_cy) && (m_iTabHeight != iTabHeight))
	{
...
int CXTPExcelTabCtrl::GetTabHeight() const
{
	return m_iTabHeight;
}
