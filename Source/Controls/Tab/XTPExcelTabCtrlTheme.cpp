...
int CXTPExcelTabCtrlTheme::DrawTab(CDC* pDC, CXTPExcelTabCtrl* pTabCtrl, const CPoint& pt, bool bSelected, CXTPTcbItem* pTcbItem)
{
	...
	if (pTabCtrl->GetTabStyle() & FTS_XTP_SQUARETABS)
	{
		int y[] =
		{
			bBottom ? pt.y : pt.y + iHeight - XTP_DPI_Y(1),
	...
	else
	{
		POINT points[] =
		{
			{ pt.x, pt.y + iHeight - XTP_DPI_Y(1) },
			{ pt.x + iBase - XTP_DPI_X(1), pt.y },
			{ pt.x + szText.cx + iWidth - iBase - XTP_DPI_X(1), pt.y },
			{ pt.x + szText.cx + iWidth - XTP_DPI_X(1), pt.y + iHeight - XTP_DPI_Y(1) }
		};
...
void CXTPExcelTabCtrlTheme::DrawButtonBack(CDC* pDC, CRect& rect, const CXTPExcelTabCtrlButtonState& state)
{
...
void CXTPExcelTabCtrlTheme::DrawButton(CDC* pDC, CXTPExcelTabCtrl* /*pTabCtrl*/, const CXTPExcelTabCtrlButtonState& state)
{
	...
	case xtpArrowIconLeft:
		...
		break;
	case xtpArrowIconLeftHome:
		nSymbol = xtpPrimitiveSymbolInjectLeft;
		break;
	case xtpArrowIconRight:
		nSymbol = xtpPrimitiveSymbolPlayRight;
		rc.InflateRect(XTP_DPI_X(1), XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1));
		break;
	case xtpArrowIconRightHome:
		nSymbol = xtpPrimitiveSymbolInjectRight;
		break;
	}
...
void CXTPExcelTabCtrlTheme::FillBackground(CDC* pDC, CRect& rect, BOOL bBottom)
{
	...
	pDC->FillSolidRect(0, bBottom? rect.top: rect.bottom - XTP_DPI_Y(1),
		rect.Width(), XTP_DPI_Y(1), m_clr3DShadow);
}
...
void CXTPExcelTabCtrlThemeOfficeXP::DrawButtonBack(CDC* pDC, CRect& rect, const CXTPExcelTabCtrlButtonState& state)
{
...
void CXTPExcelTabCtrlThemeOffice2003::DrawButtonBack(CDC* pDC, CRect& rect, const CXTPExcelTabCtrlButtonState& state)
{
	if (state.m_bEnabled && state.m_bHilight)
	{
		if (state.m_bPressed)
...
void CXTPExcelTabCtrlThemeOffice2013::RefreshMetrics(CXTPExcelTabCtrl* pTab)
{
	...
	COLORREF crTabFaceNormal   = XTPIniColor(_T("Controls.ExcelTabCtrl"), _T("TabFaceNormal"),   RGB(245, 245, 245));
	COLORREF crTabFaceSelected = XTPIniColor(_T("Controls.ExcelTabCtrl"), _T("TabFaceSelected"), RGB(255, 255, 255));
	COLORREF crTabBorder       = XTPIniColor(_T("Controls.ExcelTabCtrl"), _T("TabBorder"),       RGB(212, 212, 212));
	COLORREF crTextNormal      = XTPIniColor(_T("Controls.ExcelTabCtrl"), _T("TextNormal"),      RGB(152, 152, 152));
	COLORREF crTextSelected    = XTPIniColor(_T("Controls.ExcelTabCtrl"), _T("TextSelected"),    RGB( 43,  87, 154));
	COLORREF crTextDisabled    = XTPIniColor(_T("Controls.ExcelTabCtrl"), _T("TextDisabled"),    RGB(212, 212, 212));
...
int CXTPExcelTabCtrlThemeOffice2013::DrawTab(CDC* pDC, CXTPExcelTabCtrl* pTabCtrl, const CPoint& pt, bool bSelected, CXTPTcbItem* pTcbItem)
{
	if (pDC == NULL || pTcbItem == NULL || !::IsWindow(pTabCtrl->GetSafeHwnd()))
		return -1;
	...
	if (pTabCtrl->GetTabStyle() & FTS_XTP_SQUARETABS)
	{
		...
		if (bSelected)
		{
			...
			// fill tab and draw text.
			pDC->FillSolidRect(rcText.left - iBase, y[0], szText.cx + iBase * 2, y[1] - iGap, m_clrWindow);		
			pDC->DrawText(pTcbItem->szTabLabel, rcText, DT_CENTER|DT_VCENTER);
			
			// tab borders
			pDC->MoveTo(x[0], y[0]);
			pDC->LineTo(x[0], y[1] - iGap);
			pDC->MoveTo(x[1], y[0]);
			pDC->LineTo(x[1], y[1] - iGap);
			
			pen.Color(crFore);
			pDC->MoveTo(x[0], y[1]- iGap);
			pDC->LineTo(x[1] + XTP_DPI_X(1), y[1] - iGap);
			pDC->MoveTo(x[0], y[1] - (iGap - XTP_DPI_Y(1)));
			pDC->LineTo(x[1] + XTP_DPI_X(1), y[1] - (iGap - XTP_DPI_Y(1)));
		}
		else
		{
			// draw text.
			...
			// tab border
			pDC->MoveTo(x[0], y[0] + XTP_DPI_Y(4));
			pDC->LineTo(x[0], y[1] - XTP_DPI_Y(2));
		}
	}
	else
	{
...
void CXTPExcelTabCtrlThemeOffice2013::DrawButtonBack(CDC* pDC, CRect& rect, const CXTPExcelTabCtrlButtonState& state)
{
...
void CXTPExcelTabCtrlThemeOffice2013::DrawButton(CDC* pDC, CXTPExcelTabCtrl* /*pTabCtrl*/, const CXTPExcelTabCtrlButtonState& state)
{
	CRect rc(state.m_rect);
	if (rc.IsRectEmpty()) return;
	...
	CSize sz = rc.Size();
	XTPPrimitiveDrawer()->DrawSymbol(pDC, nSymbol, rc, state.m_bEnabled ? m_clrWindowText : m_clrBtnTextGray);
}

void CXTPExcelTabCtrlThemeOffice2013::FillBackground(CDC* pDC, CRect& rect, BOOL bBottom)
{
	pDC->FillSolidRect(rect, m_clr3DFace);	
	pDC->FillSolidRect(rect.left, bBottom? rect.top: rect.bottom - XTP_DPI_Y(1),
		rect.Width(), XTP_DPI_Y(1), m_clrBorder3DShadow);
}
