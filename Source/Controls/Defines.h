...
const DWORD CPS_XTP_NOFILL          = 0x0001; // CXTPColorSelectorCtrl style displays a "No Fill" button rather than the default Automatic Color button.
const DWORD CPS_XTP_EXTENDED        = 0x0002; // CXTPColorSelectorCtrl style displays 40 extended colors rather than the default 16 colors.
const DWORD CPS_XTP_MORECOLORS      = 0x0004; // CXTPColorSelectorCtrl style displays a "More Colors" button which will display a CXTPColorDialog.
const DWORD CPS_XTP_PICKBOX         = 0x0008; // CXTPColorSelectorCtrl style internal style used to define a color picker button.
const DWORD CPS_XTP_NOAUTOMATIC     = 0x0010; // CXTPColorSelectorCtrl style does not display an "Automatic Color" button.
const DWORD CPS_XTP_RECENTCOLORS    = 0x0020; // CXTPColorSelectorCtrl style displays a recently used color list for the control.
const DWORD CPS_XTP_RIGHTALIGN      = 0x0040; // CXTPColorSelectorCtrl style displays the color picker popup window right aligned.
const DWORD CPS_XTP_COLORBORDERS    = 0x0080; // CXTPColorSelectorCtrl style displays color borders in color.

const DWORD CPS_XTP_SHOWHEXVALUE    = 0x0100; // CXTPColorDialog style displays hex value of the color.
const DWORD CPS_XTP_SHOW3DSELECTION = 0x0200; // CXTPColorDialog style displays a 3D border around the color selection box.
const DWORD CPS_XTP_SHOWEYEDROPPER  = 0x0400; // CXTPColorDialog style displays an eye dropper selection tool.
...
#define ON_WM_XTP_TAGCLOUDSELCHANGE() \
	{ WM_XTP_TAGCLOUDSELCHANGE, 0, 0, 0, AfxSig_vw, \
		(AFX_PMSG)(AFX_PMSGW)(static_cast< void (AFX_MSG_CALL CWnd::*)(XTP_TAGCLOUDTAG*) > (&OnTagCloudSelChange)) },
//}}AFX_CODEJOCK_PRIVATE
...
#define ON_WM_XTP_TAGCLOUDRIGHTCLK() \
	{ WM_XTP_TAGCLOUDRIGHTCLK, 0, 0, 0, AfxSig_vw, \
		(AFX_PMSG)(AFX_PMSGW)(static_cast< void (AFX_MSG_CALL CWnd::*)(XTP_TAGCLOUDTAG*) > (&OnTagCloudRightClk)) },
//}}AFX_CODEJOCK_PRIVATE
...
#define ON_WM_XTP_TAGCLOUDDBLCLK() \
	{ WM_XTP_TAGCLOUDDBLCLK, 0, 0, 0, AfxSig_vw, \
		(AFX_PMSG)(AFX_PMSGW)(static_cast< void (AFX_MSG_CALL CWnd::*)(XTP_TAGCLOUDTAG*) > (&OnTagCloudDblClk)) },
//}}AFX_CODEJOCK_PRIVATE
