...
void CXTPSearchOptionsCtrl::OnPaint()
{
	...
	if (m_bExpanded)
	{
		pOldFont = memDC.SelectObject(&XTPAuxData().xtpFont);
		...
		rcCaption.top += XTP_DPI_Y(1);
		rcCaption.bottom -= XTP_DPI_Y(1);
		memDC.FillSolidRect(rcCaption, GetXtremeColor(COLOR_HIGHLIGHT));
	}
	else
	{
		pOldFont = memDC.SelectObject(&XTPAuxData().xtpFontULine);
...
bool CXTPSearchOptionsView::CResizeWnd::IsGroupBox()
{
	...
	if (0 != ::GetClassName(m_hWndChild, szClassName, _countof(szClassName)))
	{
...
HBRUSH CXTPSearchOptionsView::OnCtlColor(CDC* pDC, CWnd* /*pWnd*/, UINT /*nCtlColor*/)
{
	...
	if (NULL != m_xtpBrush.GetSafeHandle() && clr != m_clrBrush)
	{
		m_xtpBrush.DeleteObject();
	}

	if (NULL == m_xtpBrush.GetSafeHandle())
	{
		m_xtpBrush.CreateSolidBrush(clr);
		m_clrBrush = clr;
	}
