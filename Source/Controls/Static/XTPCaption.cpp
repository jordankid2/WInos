...
: m_pTheme(NULL)
{
	...
	m_nOffset = 0;
	m_nBorder = 0;
	m_sizeIcon.cx = __min(XTPAuxData().cxSmIcon, XTP_DPI_X(16));
	m_sizeIcon.cy = __min(XTPAuxData().cySmIcon, XTP_DPI_Y(16));
	m_dwExStyle = 0L;
...
{
	ASSERT (pChild);      // must be valid.
	ASSERT (pNotifyWnd);  // must be valid.
...
CRect CXTPCaption::GetTextRect() const
{
	...
	size.cx += tm.tmAveCharWidth + XTP_DPI_X(12);
	size.cy = rcClient.Height();

	int iWidth = rcClient.Width() - (m_hIcon ? XTP_DPI_X(18) : 0);
	if (size.cx > iWidth)
		size.cx = iWidth;
...
CRect CXTPCaption::GetButtonRect() const
{
	...
	CSize szIcon(XTP_DPI_X(16), XTP_DPI_Y(15));
	CXTPClientRect rcClient(this);
