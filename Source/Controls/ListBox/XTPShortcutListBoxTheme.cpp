...
#include "Controls/ListBox/XTPShortcutListBoxTheme.h"
...
void CXTPShortcutListBoxTheme::DrawItem(CXTPShortcutListBox* pMenuListBox, LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (pCI->m_pIcon && 0 < pCI->m_pIcon->GetWidth() && 0 < pCI->m_pIcon->GetHeight())
	{
...
void CXTPShortcutListBoxTheme::DrawImage(CDC* pDC, CRect& rcIcon, CXTPImageManagerIcon* pImage, BOOL bEnabled, BOOL /*bSelected*/, BOOL /*bPressed*/)
{
	...
	CSize szIcon = XTP_DPI(pImage->GetExtent());

	if (bEnabled)
	{
		pImage->Draw(pDC, pt, szIcon);
	}
	else
	{
		if (pImage->HasDisabledIcon() || pImage->IsAlpha())
		{
			pImage->Draw(pDC, pt, pImage->GetDisabledIcon(), szIcon);
		}
		else
		{
...
void CXTPShortcutListBoxThemeOfficeXP::DrawImage(CDC* pDC, CRect& rcIcon, CXTPImageManagerIcon* pImage, BOOL bEnabled, BOOL bSelected, BOOL bPressed)
{
	...
	if (bEnabled)
	{
		...
		{
			pImage->Draw(pDC, pt, pImage->GetFadedIcon(), szIcon);
		}
		else if (bSelected && !bPressed)
		{
			pImage->Draw(pDC, CPoint(pt.x + 1, pt.y + 1), pImage->GetShadowIcon(), szIcon);
			pImage->Draw(pDC, CPoint(pt.x - 1, pt.y - 1), pImage->GetHotIcon(), szIcon);
		}
		else
		{
			pImage->Draw(pDC, rcIcon.TopLeft(), szIcon);
		}
	}
	else
	{
		...
		{
			pImage->DrawMono(pDC, pt, pImage->GetIcon(), szIcon, m_crItemTextDisabled);
		}
	}
}
