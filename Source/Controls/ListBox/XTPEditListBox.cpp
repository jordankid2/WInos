...
void CXTPEditListBox::EditListItem(BOOL bNewItem)
{
	...
	rcItem.InflateRect(-XTP_DPI_X(2), XTP_DPI_Y(2));
...
void CXTPEditListBox::Initialize(bool bAutoFont/*= true*/)
{
	...
	ASSERT(m_pParentWnd); // must be valid.
	...
	rcWindow.top += XTP_DPI_Y(BTN_OFFSET) + ::GetSystemMetrics(SM_CXEDGE);
...
void CXTPEditListBox::RecalcLayout()
{
	if (HasToolbar() && ::IsWindow(m_editGroup.m_hWnd))
	{
		...
		rcWindow.top -= XTP_DPI_Y(BTN_OFFSET);
		rcWindow.bottom = rcWindow.top + XTP_DPI_Y(BTN_OFFSET);
...
void CXTPEditListBoxToolBar::MoveButtons()
{
	...
	rcClient.left = rcClient.right - XTP_DPI_X(BTN_OFFSET);
	...
	for (int i = _countof(m_arButton)-1; i >= 0 ; --i)
	{
		if (::IsWindow(m_arButton[i].GetSafeHwnd()))
		{
			...
			rcClient.OffsetRect(-XTP_DPI_X(BTN_OFFSET), 0);
		}
		else
		{
...
void CXTPEditListBoxToolBar::OnNcPaint()
{
	...
	if (wrpTreeView.IsAppThemeActive() && (GetOwner()->GetExStyle() & WS_EX_CLIENTEDGE))
	{
		CXTPWindowRect rc(this);
		CXTPClientRect rcClient(this);

		COLORREF clr = GetXtremeColor(COLOR_BTNSHADOW);
		if (rc.Width() == rcClient.Width() + XTP_DPI_X(2) &&
			SUCCEEDED(wrpTreeView.GetThemeColor(0, 0, TMT_BORDERCOLOR, &clr)))
		{
			CWindowDC dc(this);

			dc.FillSolidRect(0, 0, rc.Width(), XTP_DPI_Y(1), clr);
			dc.FillSolidRect(0, 0, XTP_DPI_X(1), rc.Height(), clr);
			dc.FillSolidRect(rc.Width() - XTP_DPI_X(1), 0, XTP_DPI_X(1), rc.Height(), clr);
			dc.FillSolidRect(XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(1), 
				rc.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), GetXtremeColor(COLOR_3DFACE));
			return;
		}
	}
