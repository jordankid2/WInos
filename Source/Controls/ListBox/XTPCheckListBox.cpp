...
CXTPCheckListBox::CCheckListState::CCheckListState(BOOL bListBox3D)
{
	...
	BOOL bScaled = ScaleBitmap(&bitmap, &bitmapScaled);
	m_hbitmapCheck = (HBITMAP)(bScaled ? bitmapScaled.Detach() : bitmap.Detach());

	// As far as we cannot know the valid themed part size upfront we assume its default value
	// which might not be correct but better than nothing.
	m_sizeCheckThemed.cx = GetSystemMetrics(SM_CXMENUCHECK);
	m_sizeCheckThemed.cy = GetSystemMetrics(SM_CYMENUCHECK);
}
...
bool CXTPCheckListBox::PreDrawItemThemed(CDC* pDC, DRAWITEMSTRUCT &drawItem, int nCheck, int cyItem)
{
	...
	if (m_themeHelper->IsAppThemeReady())
	{
		...
		{
			m_checkListState.m_sizeCheckThemed = size;
...
void CXTPCheckListBox::PreDrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	drawItem.rcItem.left = drawItem.rcItem.left + (m_bLastItemDrawnThemed
		? m_checkListState.m_sizeCheckThemed.cx : m_checkListState.m_sizeCheck.cx) + 3;
