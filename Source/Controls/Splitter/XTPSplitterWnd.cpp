// XTPSplitterWnd.cpp : implementation file
//
...
, m_bClipStyles(FALSE)
{
	m_cxSplitter = m_cxSplitterGap = XTP_DPI_X(6);
	m_cySplitter = m_cySplitterGap = XTP_DPI_Y(6);
...
BOOL CXTPSplitterWnd::SetTheme(XTPControlTheme eTheme)
{
	...
	case xtpControlThemeVisualStudio2015:
		pTheme = new CXTPSplitterWndThemeVisualStudio2015();
		break;
	...
	case xtpControlThemeOffice2013:
		pTheme = new CXTPSplitterWndThemeOffice2013();
		break;
	...
	{
	case xtpControlThemeOffice2000:
		SetScrollBarTheme(xtpScrollBarThemeOffice2000);
		break;

	case xtpControlThemeOfficeXP:
		SetScrollBarTheme(xtpScrollBarThemeOfficeXP);
		break;

	case xtpControlThemeOffice2003:
		SetScrollBarTheme(xtpScrollBarThemeOffice2003);
		break;

	case xtpControlThemeOffice2013:
		SetScrollBarTheme(xtpScrollBarThemeOffice2013);
		break;
	...
	case xtpControlThemeVisualStudio2015:
		...
		break;
	default:
		SetScrollBarTheme(xtpScrollBarThemeWindowsDefault);
		break;
	}
...
void CXTPSplitterWnd::DrawTracker(const CRect& rect, CBrush* pBrush)
{
	...
	else
	{
		...
		if (bVert)
		{
			if (rcTracker.Width() != XTP_DPI_X(4))
			{
				rcTracker.left = rcTracker.right - XTP_DPI_X(4);
			}
		}
		else
		{
			if (rcTracker.Height() != XTP_DPI_Y(4))
			{
				rcTracker.bottom = rcTracker.top + XTP_DPI_Y(4);
			}
		}
...
CXTPSplitterWndEx::CXTPSplitterWndEx()
{
	m_cxBorder = 0;
	m_cyBorder = 0;
	m_cxSplitter = m_cxSplitterGap = XTP_DPI_X(4);
	m_cySplitter = m_cySplitterGap = XTP_DPI_Y(4);
	m_cyTopBorderGap = XTP_DPI_Y(6);
