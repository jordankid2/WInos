...
{
	// set splitter colors.
	m_crBack.SetStandardValue(XTPIniColor(_T("Splitter"), _T("BoxBack"), ::GetSysColor(COLOR_3DFACE)));
	m_crOuterBorder.SetStandardValue(XTPIniColor(_T("Splitter"), _T("OuterBorderLight"), ::GetSysColor(COLOR_3DHILIGHT)), XTPIniColor(_T("Splitter"), _T("OuterBorderDark"), ::GetSysColor(COLOR_3DSHADOW)) );
	m_crInnerBorder.SetStandardValue(XTPIniColor(_T("Splitter"), _T("InnerBorderLight"), ::GetSysColor(COLOR_3DFACE)), XTPIniColor(_T("Splitter"), _T("InnerBorderDark"), ::GetSysColor(COLOR_WINDOWFRAME)));
...
void CXTPSplitterWndTheme::DrawSplitter(CDC* pDC, CXTPSplitterWnd* pSplitter, int nType, const CRect& rectArg)
{
	...
	case splitBorder:
		{
			COLORREF crTopLeft = !pSplitter->IsBorderVisible() ? m_crBack : m_crOuterBorder.clrDark;
			COLORREF crBottomRight = !pSplitter->IsBorderVisible() ? m_crBack : m_crOuterBorder.clrLight;
			pDC->Draw3dRect(rect, crTopLeft, crBottomRight);

			rect.InflateRect(-CX_BORDER, -CY_BORDER);

			crTopLeft = !pSplitter->IsBorderVisible() ? m_crBack : m_crInnerBorder.clrDark;
			crBottomRight = !pSplitter->IsBorderVisible() ? m_crBack : m_crInnerBorder.clrLight;
			pDC->Draw3dRect(rect, crTopLeft, crBottomRight);
...
void CXTPSplitterWndThemeOffice2013::RefreshMetrics()
{
	...
	m_crBack.SetStandardValue(
		XTPIniColor(_T("Splitter"), _T("BoxBack"), RGB(255,255,255)));

	m_crOuterBorder.SetStandardValue(
		XTPIniColor(_T("Splitter"), _T("OuterBorderLight"), RGB(255,255,255)),
		XTPIniColor(_T("Splitter"), _T("OuterBorderDark"),  RGB(255,255,255)));

	m_crInnerBorder.SetStandardValue(
		XTPIniColor(_T("Splitter"), _T("InnerBorderLight"), RGB(212,212,212)),
		XTPIniColor(_T("Splitter"), _T("InnerBorderDark"),  RGB(212,212,212)));
