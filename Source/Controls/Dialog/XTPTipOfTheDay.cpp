...
CXTPTipOfTheDay::CXTPTipOfTheDay(LPCTSTR lpszTipFile/*= NULL*/, CWnd* pParent/*= NULL*/)
{
	...
	m_pLightBulbIcon = 0;
	{
		...
		iconHandle.Attach(CXTPImageManagerIcon::LoadBitmapFromResource(MAKEINTRESOURCE(XTP_IDI_LIGHTBULB), NULL), FALSE);
...
CXTPTipOfTheDay::~CXTPTipOfTheDay()
{
	...
	if (m_xtpFontTitle.GetSafeHandle())
		m_xtpFontTitle.DeleteObject();

	if (m_xtpFontTip.GetSafeHandle())
		m_xtpFontTip.DeleteObject();
...
void CXTPTipOfTheDay::OnPaint()
{
	...
	if (m_bClassicStyle)
	{
		...
		rcBorder.DeflateRect(XTP_DPI_X(10), XTP_DPI_Y(10), XTP_DPI_X(10), 0);

		dc.FillSolidRect(rcBorder.left + XTP_DPI_X(50), rcBorder.top, 
			rcBorder.Width() - XTP_DPI_X(50), rcBorder.Height(), GetXtremeColor(COLOR_WINDOW));
		dc.FillSolidRect(rcBorder.left, rcBorder.top, XTP_DPI_X(50), 
			rcBorder.Height(), RGB(128, 128, 128));
		...
		if (m_pLightBulbIcon)
		{
			m_pLightBulbIcon->Draw(&dc, CPoint(rcBorder.left + XTP_DPI_X(8), rcBorder.top + XTP_DPI_Y(8)),
				XTP_DPI(m_pLightBulbIcon->GetExtent()));
		}

		// Draw the title text.
		CXTPFontDC font(&dc, m_pFontTitle);
		dc.SetTextColor(GetXtremeColor(COLOR_WINDOWTEXT));
		dc.TextOut(XTP_DPI_X(52) + XTP_DPI_X(10) + XTP_DPI_X(5), XTP_DPI_Y(14) + XTP_DPI_Y(5), m_strTipTitle);
	}
	else
	{
		...
		if (m_pLightBulbIcon)
		{
			m_pLightBulbIcon->Draw(&dc, XTP_DPI(CPoint(8, 8)), XTP_DPI(m_pLightBulbIcon->GetExtent()));
		}

		// Draw the title text.
		CXTPFontDC font(&dc, m_pFontTitle);
		dc.SetTextColor(RGB(0x00, 0x33, 0x99));
		dc.TextOut(XTP_DPI_X(52), XTP_DPI_Y(14), m_strTipTitle);
...
BOOL CXTPTipOfTheDay::OnInitDialog()
{
	...
	if (m_pFontTitle == NULL && m_pFontTip == NULL)
	{
		...
		m_xtpFontTip.CreateFontIndirect(&ncm.lfMenuFont);

		ncm.lfMenuFont.lfHeight = ::MulDiv(-12, ::GetDeviceCaps(dc.m_hDC, LOGPIXELSY), 72);

		if (_tcscmp(ncm.lfMenuFont.lfFaceName, _T("Segoe UI")) != 0)
			ncm.lfMenuFont.lfWeight = FW_BOLD;
		m_xtpFontTitle.CreateFontIndirect(&ncm.lfMenuFont);

		m_pFontTip = &m_xtpFontTip;
		m_pFontTitle = &m_xtpFontTitle;
	}
	...
	m_rcBorder.bottom = rcBorder.bottom - XTP_DPI_Y(1);

	if (!m_bClassicStyle)
	{
		m_rcTipText.SetRect(XTP_DPI_X(52), XTP_DPI_Y(44), 
			m_rcBorder.right - XTP_DPI_X(6), m_rcBorder.bottom - XTP_DPI_Y(6));
	}
	else
	{
		m_rcTipText.SetRect(XTP_DPI_X(52) + XTP_DPI_X(10) + XTP_DPI_X(5), XTP_DPI_Y(44) + XTP_DPI_Y(5),
			m_rcBorder.right - XTP_DPI_X(6) - XTP_DPI_X(10), m_rcBorder.bottom - XTP_DPI_Y(6));
	}
