...
void CXTPTaskDialogClient::RefreshMetrics()
{
	...
	m_ptMargin     = XTPDlu2Pix(XTP_DPI_X(xtpDluMargin), XTP_DPI_X(xtpDluMargin));
	m_ptPadding    = XTPDlu2Pix(XTP_DPI_X(xtpDluPadding), XTP_DPI_X(xtpDluPadding));

	m_ptBtnSpacing = CPoint(XTPDlu2Pix(XTP_DPI_X(xtpDluBtnSpacing), XTP_DPI_X(xtpDluBtnSpacing)).x, 0);

	if (!m_bThemeReady)
		m_ptBtnSpacing = CPoint(m_ptMargin.x, 2);

	m_nMinClientWidth = XTPDlu2Pix(XTP_DPI_X(xtpDluMinClient), 0).x - m_ptMargin.x * 2;
...
BOOL CXTPTaskDialogClient::CreateLinkButtons()
{
	...
	for (UINT i = 0; i < m_pConfig->cButtons; ++i)
	{
		...
		if (IsLinkButtonsUsed())
		{
			...
			pTheme->SetTitleFont(&m_xtpFontLarge);
...
BOOL CXTPTaskDialogClient::CreateInputBox()
{
	...
	m_wndInputBox.SetFont(&m_xtpFont);
...
BOOL CXTPTaskDialogClient::CreateRadioButtons()
{
	...
	for (UINT i = 0; i < m_pConfig->cRadioButtons; ++i)
	{
		...
		CPoint point(XTP_DPI_X(10), XTP_DPI_Y(2));
...
BOOL CXTPTaskDialogClient::CreateVerifyExpandButtons()
{
	...
	if (!m_strExpandedInformation.IsEmpty())
	{
		...
		CSize sizeButton = CalcButtonSize(strButtonSize, NULL, XTPDlu2Pix(XTP_DPI_X(xtpDluMaxVerify),0).x);
	...
	if (!m_strVerificationText.IsEmpty())
	{
		...
		CPoint point(XTP_DPI_X(10), XTP_DPI_Y(2));
		CSize sizeButton = CalcButtonSize(strButtonText, &point, XTPDlu2Pix(XTP_DPI_X(xtpDluMaxVerify),0).x);
...
#ifdef _XTP_INCLUDE_MARKUP
		CXTPButton* pXtpButton = DYNAMIC_DOWNCAST(CXTPButton, pButton);
		if (NULL != pXtpButton && NULL != pXtpButton->GetMarkupContext())
		{
			pXtpButton->GetMarkupContext()->AddHandler(
				CXTPMarkupHyperlink::m_pClickEvent,
				CreateMarkupClassDelegate(this, &CXTPTaskDialogClient::OnMarkupHyperlinkClick));
		}
#endif
...
BOOL CXTPTaskDialogClient::CreateClient(BOOL bMoveWindow)
{
	...
	{
		CXTPEmptyRect rect;
		m_wndContent.Create(rect, m_strContent.m_strCaption, &m_xtpFont, this);
		m_wndExpandedInformation.Create(rect, m_strExpandedInformation.m_strCaption, &m_xtpFont, this);
		m_wndFooter.Create(rect, m_strFooter.m_strCaption, &m_xtpFont, this);
	}
...
HICON CXTPTaskDialogClient::LoadSysIcon(PCWSTR lpwzIcon, CSize sizeIcon)
{
	...
	if (m_bUseSysIcons)
	{
		...
		if (modUser32Dll.LoadLibrary(_T("user32.dll")))
		{
			hNewIcon = XTPResourceManager()->CreateIconFromResource(modUser32Dll, MAKEINTRESOURCE(dwID), sizeIcon);
		}
	}
...
void CXTPTaskDialogClient::CreateIcons()
{
	...
	{
		m_pIconLinkGlyph = new CXTPImageManagerIcon(0, XTP_DPI_X(20), XTP_DPI_X(20));
		...
		if (bRTLLayout)
		{
			hIconGlyph.Attach(CXTPImageManagerIcon::InvertAlphaBitmap(hBitmap), FALSE);
			DeleteObject(hBitmap);
		}
		else
		{
			hIconGlyph.Attach(hBitmap, FALSE);
		}
		...
		if (bRTLLayout)
		{
			hIconGlyphHot.Attach(CXTPImageManagerIcon::InvertAlphaBitmap(hBitmap), FALSE);
			DeleteObject(hBitmap);
		}
		else
		{
			hIconGlyphHot.Attach(hBitmap, FALSE);
		}
...
void CXTPTaskDialogClient::CreateFonts()
{
	...
	if (m_bThemeReady)
	{
		...
		m_xtpFont.CreateFontIndirect(&ncm.lfMenuFont);

		ncm.lfMenuFont.lfHeight = ::MulDiv(-12, ::GetDeviceCaps(dc.m_hDC, LOGPIXELSY), 72);

		if (_tcscmp(ncm.lfMenuFont.lfFaceName, _T("Segoe UI")) != 0)
			ncm.lfMenuFont.lfWeight = FW_BOLD;
		m_xtpFontLarge.CreateFontIndirect(&ncm.lfMenuFont);
	}
	else
	{
		m_xtpFont.CreateFontIndirect(&ncm.lfMenuFont);
		m_xtpFontLarge.CreateFontIndirect(&ncm.lfCaptionFont);
	}
}
...
CSize CXTPTaskDialogClient::CalcButtonSize(CString strButton, CPoint* pMarginButton /*=NULL*/, int nMaxWidth /*=0*/)
{
	...
	if (!strCaptText.IsEmpty())
	{
		...
		if (nMaxWidth == 0)
			XTPCalcIdealTextSize(strCaptText, sizeCaptText, m_xtpFont, XTP_DPI_X(330), XTP_DPI_X(7));
		else
			XTPCalcTextSize(strCaptText, sizeCaptText, m_xtpFont);
	}
	...
	if (!strNoteText.IsEmpty())
	{
		...
		if (nMaxWidth == 0)
			XTPCalcIdealTextSize(strNoteText, sizeNoteText, m_xtpFont, XTP_DPI_X(330), XTP_DPI_X(7));
		else
			XTPCalcTextSize(strNoteText, sizeNoteText, m_xtpFont);
	}
...
int CXTPTaskDialogClient::CalcLinkButtonIdealWidth(CString strButton, int cxWidth)
{
	...
	else
	{
		...
		if (XTPCalcIdealTextSize(strCaptText, sizeText, m_xtpFontLarge, XTP_DPI_X(120), XTP_DPI_X(30)))
		{
...
CSize CXTPTaskDialogClient::CalcLinkButtonSize(CXTPButton* pButton, CString strButton, int cxMaxWidth)
{
	...
	{
		XTPMarkupSetDefaultFont(pButton->GetMarkupContext(), (HFONT)m_xtpFontLarge.GetSafeHandle(), COLORREF_NULL);

		sizeCaptText = XTPMarkupMeasureElement(pButton->GetMarkupUIElement(), cxMaxWidth, INT_MAX);

	}
	else if (XTPCalcTextSize(strCaptText, sizeCaptText, m_xtpFontLarge, &bWordWrap))
	{
	...
	if (XTPCalcTextSize(strNoteText, sizeNoteText, m_xtpFont, &bWordWrap))
	{
		// if the string spans multiple rows, set the max width.
		if (bWordWrap)
...
{
	int x = m_rcCommandButtons.left, y = m_rcCommandButtons.top + m_ptMargin.y - XTP_DPI_X(1);
...
{
	int nTop = m_rcLinkButtons.top + m_ptMargin.x + m_ptPadding.y / 2 + XTP_DPI_Y(2);
...
void CXTPTaskDialogClient::SetVerifyButtonPos()
{
	...
	if (IsWindow(m_pBtnVerify) && m_mapWndSize.Lookup(m_pBtnVerify, sizeVerify))
	{
		...
		rcButton.left = m_ptMargin.x + XTP_DPI_X(3);
...
int CXTPTaskDialogClient::CalcIdealTextWidth(const CStringMarkup& strItemText, int nMinWidth, BOOL bLargeFont)
{
	...
	if (strItemText.m_pUIElement)
	{
		if (XTPCalcIdealTextSize(m_pMarkupContext, strItemText.m_pUIElement, sizeText,
			bLargeFont? m_xtpFontLarge: m_xtpFont,
			XTP_DPI_X(bLargeFont ? 275 : 330),
			XTP_DPI_X(bLargeFont ? 14 : 7)))
		{
...
CRect CXTPTaskDialogClient::CalcTextRect(const CStringMarkup& strItemText, int nWidth, BOOL bLargeFont)
{
	...
	if (strItemText.m_pUIElement)
	{
		XTPMarkupSetDefaultFont(m_pMarkupContext, bLargeFont? m_xtpFontLarge: m_xtpFont, COLORREF_NULL);
	...
	else
	{
		...
		if (XTPCalcTextSize(strItemText, sizeText, bLargeFont? m_xtpFontLarge: m_xtpFont))
		{
			rcItemText.SetRect(0, 0, nWidth, sizeText.cy);
		}
	}
...
CRect CXTPTaskDialogClient::CalcCommandRect(int nWidth)
{
	...
	rcCommand.bottom += m_ptMargin.y * 2 - XTP_DPI_Y(2);
...
CSize CXTPTaskDialogClient::CalcDynamicLayout(int nIdealWidth)
{
	...
	m_rcContent.OffsetRect(m_ptMargin.x + nMainIconMargin,
		m_rcMainInstruction.bottom + m_ptMargin.y + XTP_DPI_Y(1));

	int nLinkTop = m_rcContent.bottom + m_ptPadding.y;

	if (nLinkTop < m_ptMargin.y * 4 - XTP_DPI_Y(3))
		nLinkTop = m_ptMargin.y * 4 - XTP_DPI_Y(3);
	...
	if (m_arrRadioButtons.GetSize() > 0)
	{
		...
		for (int i = 0; i < m_arrRadioButtons.GetSize(); i++)
		{
			...
			if (m_mapWndSize.Lookup(pButton, sizeButton))
			{
				...
				nLinkTop += sizeButton.cy + XTP_DPI_Y(3);
			}
		}
	...
	{
		m_rcRadioButtons = CRect(m_rcContent.left + XTP_DPI_X(1), nLinkTop, m_rcContent.right, nLinkTop);
	}
	...
	m_rcLinkButtons = CRect(m_rcContent.left + XTP_DPI_X(1), nLinkTop + XTP_DPI_Y(1), 
		m_ptMargin.y + nIdealWidth - XTP_DPI_X(1), nLinkTop + XTP_DPI_Y(1));
	...
	if (!m_strFooter.IsEmpty())
	{
		...
		m_rcFooter.OffsetRect(m_ptMargin.x + nFooterIconMargin - XTP_DPI_X(1), 
			nBottom + m_ptMargin.y + XTP_DPI_Y(1));

		nBottom += m_rcFooter.Height() + 2 * m_ptMargin.y + XTP_DPI_Y(1);
	}
...
BOOL CXTPTaskDialogClient::ResizeParentToFit(CSize szClient, BOOL bMoveWindow)
{
	...
	CPoint ptPos((rcParent.left + rcParent.right - szClient.cx) / 2 - XTP_DPI_X(3),
		(rcParent.top + rcParent.bottom - szClient.cy) / 2 - XTP_DPI_Y(12));
...
void CXTPTaskDialogClient::DrawBackground(CDC* pDC, CRect rect)
{
	...
	if (m_bThemeReady)
	{
		...
		{
			pDC->FillSolidRect(rect.left, m_rcCommandButtons.top, rect.Width(), XTP_DPI_Y(1), m_cr3DLight);
		}

		if (!m_rcFooter.IsRectEmpty())
		{
			pDC->FillSolidRect(rect.left, m_rcCommandButtons.bottom, 
				rect.Width(), XTP_DPI_Y(1), m_cr3DLight);
			pDC->FillSolidRect(rect.left, m_rcCommandButtons.bottom + XTP_DPI_Y(1), 
				rect.Width(), XTP_DPI_Y(1), m_cr3DHighLight);
		}

		if (m_bExpanded && !m_strExpandedInformation.IsEmpty() && IsExpandFooterArea() && !m_rcExpandedInformation.IsRectEmpty())
		{
			pDC->FillSolidRect(rect.left, m_rcExpandedInformation.top - m_ptMargin.y,  
				rect.Width(), XTP_DPI_Y(1), m_cr3DLight);
			pDC->FillSolidRect(rect.left, m_rcExpandedInformation.top - m_ptMargin.y + XTP_DPI_Y(1), 
				rect.Width(), XTP_DPI_Y(1), m_cr3DHighLight);
		}
	}
	else
	{
		...
		{
			pDC->FillSolidRect(rect.left, m_rcCommandButtons.bottom + XTP_DPI_Y(1), 
				rect.Width(), XTP_DPI_Y(1), m_cr3DShadow);
		}
		if (m_bExpanded && !m_strExpandedInformation.IsEmpty() && IsExpandFooterArea() && !m_rcExpandedInformation.IsRectEmpty())
		{
			pDC->FillSolidRect(rect.left, m_rcExpandedInformation.top - m_ptMargin.y,  
...
void CXTPTaskDialogClient::DrawMainInstruction(CDC* pDC)
{
	...
	DrawText(pDC, m_strMainInstruction, m_rcMainInstruction, m_crMainText, &m_xtpFontLarge);
}
...
void CXTPTaskDialogClient::DrawContent(CDC* pDC)
{
	...
	{
		DrawText(pDC, m_strContent, m_rcContent, m_crContentText, &m_xtpFont);
	}
}
...
void CXTPTaskDialogClient::DrawExpandedInfo(CDC* pDC)
{
	...
	{
		DrawText(pDC, m_strExpandedInformation, m_rcExpandedInformation, IsExpandFooterArea() ? m_crFooterText : m_crContentText, &m_xtpFont);
	}
}
...
void CXTPTaskDialogClient::DrawFootNote(CDC* pDC)
{
	...
	if (m_hIconFooter)
	{
		DrawIcon(pDC, m_hIconFooter, CPoint(m_ptMargin.x + XTP_DPI_X(2), m_rcFooter.top), m_sizeIconFooter);
	}

	if (!::IsWindow(m_wndFooter.m_hWnd))
	{
		DrawText(pDC, m_strFooter, m_rcFooter, m_crFooterText, &m_xtpFont);
	}
}
