	...
	: CXTPTaskDialogFrame(&m_config)
{
	...
	m_config.hInstance = XTPGetInstanceHandle();
...
void CXTPTaskDialog::SetWidth(int cxWidth, BOOL bPixelToDLU /*=FALSE*/)
{
	...
	m_config.cxWidth = XTPToUIntChecked(cxWidth);
}
...
INT_PTR CXTPTaskDialog::DoModal()
{
	...
	// You can only use comctl32.dll task dialog when system is Windows Vista or later.
	if (bUseComCtl32 && XTPSystemVersion()->IsWinVistaOrGreater())
	{
		...
		if (modComCtl32Dll.LoadLibrary(_T("ComCtl32.dll"))
			&& modComCtl32Dll.GetProcAddress((FARPROC*)&pfnTaskDialogIndirect, "TaskDialogIndirect", MAKELONG(0, 6)))
		{
...
{
	return CreateFromResourceParam(XTPGetInstanceHandle(), nIDResource, lpszDialog, NULL, 0);
}
...
BOOL CXTPTaskDialog::CreateFromResourceParam(CXTPPropExchangeXMLNode* pPX, LPCTSTR const* rglpsz, int nString)
{
	...
	SetWidth(XTP_DPI_X(GetPropExchangeInt(pPX, _T("Width"))), TRUE);
