...
COLORREF AFX_CDECL CXTPColorBase::HLStoRGB(double h, double l, double s)
{
	return CXTPDrawHelpers::HSLtoRGB(h, s, l);
}

void AFX_CDECL CXTPColorBase::RGBtoHSL(COLORREF color, double* h, double* s, double* l)
{
...
void CXTPColorWnd::DrawCrossHair(CDC* pDC)
{
	...
	COLORREF crStick = (m_eHasFocus == focusColorWheel ? RGB(0xff, 0xff, 0xff) : RGB(0x00, 0x00, 0x00));

	CRect rcStick(x - XTP_DPI_X(5), y - XTP_DPI_Y(1),
		x - XTP_DPI_X(10), y + XTP_DPI_Y(1));
	pDC->FillSolidRect(rcStick, crStick);

	rcStick.SetRect(x + XTP_DPI_X(5), y - XTP_DPI_Y(1),
		x + XTP_DPI_X(10), y + XTP_DPI_Y(1));
	pDC->FillSolidRect(rcStick, crStick);

	rcStick.SetRect(x - XTP_DPI_X(1), y - XTP_DPI_Y(5),
		x + XTP_DPI_X(1), y - XTP_DPI_Y(10));
	pDC->FillSolidRect(rcStick, crStick);
...
{
	GetClientRect(&rect);
	rect.DeflateRect(0, XTP_DPI_Y(4));
	rect.right = rect.left + XTP_DPI_X(14);
}
...
void CXTPColorLum::DrawLuminanceBar(CDC* pDC)
{
	...
	for (int y = 0; y < cy; y++)
	{
		...
		pDC->FillSolidRect(rcBar.left, y + rcBar.top, cx, XTP_DPI_Y(1), clr);
	}
}
...
void CXTPColorLum::DrawSliderArrow(CDC* pDC)
{
	...
	CSize sz(XTP_DPI_X(16), XTP_DPI_Y(16));
	CRect rcArrow(x, y - sz.cy / 2, x + sz.cx, y + sz.cy / 2);
	XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayLeft, rcArrow,
		m_eHasFocus == focusLumination ? RGB(0x00, 0x00, 0x00) : GetXtremeColor(COLOR_3DSHADOW));
}
