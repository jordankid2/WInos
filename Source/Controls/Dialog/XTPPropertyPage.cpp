...
BOOL CXTPPropertyPage::LoadTemplate(HGLOBAL& hTemplate, HINSTANCE& hInst)
{
	...
	if ((m_psp.dwFlags & PSP_DLGINDIRECT) && m_psp.pResource)
	{
		...
		hInst = XTPGetInstanceHandle();
	}
	else
	{
		HRSRC hResource = XTPResourceManager()->FindResource(hInst, m_psp.pszTemplate, RT_DIALOG);
		if (!hResource)
			return FALSE;
...
BOOL CXTPPropertyPage::LoadTemplate()
{
	...
	BOOL bResult = MeasureTemplate(m_totalDev);
	return bResult;
}

BOOL CXTPPropertyPage::MeasureTemplate(SIZE& szDlg)
{
	BOOL bResult = FALSE;
	HGLOBAL hTemplate = NULL;
	HINSTANCE hInstance = NULL;
	...
	do
	{
		if (!LoadTemplate(hTemplate, hInstance) || NULL == hTemplate)
			break;

		DLGTEMPLATE* lpDialogTemplate = (DLGTEMPLATE*)GlobalLock(hTemplate);
		if (NULL == lpDialogTemplate)
			break;
		...
		DWORD dwNewStyle = ((lSaveStyle &~ WS_VISIBLE) 
			& (DS_LOCALEDIT | WS_CLIPCHILDREN | DS_SETFONT | DS_FIXEDSYS))
		...
		LPCTSTR lpDlgParentClassName = AfxRegisterWndClass(
			CS_VREDRAW | CS_HREDRAW,
			::LoadCursor(NULL, IDC_ARROW),
			reinterpret_cast<HBRUSH>(GetStockObject(WHITE_BRUSH)),
			NULL);
		if (NULL == lpDlgParentClassName)
			break;

		if (!wndDlgParent.CreateEx(0, lpDlgParentClassName, NULL, 0, 0, 0, 0, 0, NULL, 0))
			break;

		bResult = dlgTest.CreateIndirect(lpDialogTemplate, &wndDlgParent);
		if (!bResult)
			break;

		CSize sz;
		CXTPWindowRect rc(&dlgTest);
		szDlg = rc.Size();
		m_totalDev = szDlg;
	...
	if (::IsWindow(dlgTest))
		dlgTest.DestroyWindow();

	if (::IsWindow(wndDlgParent))
		wndDlgParent.DestroyWindow();
...
void CXTPPropertyPage::SetModified(BOOL bChanged)
{
...
void CXTPPropertyPage::OnNcPaint()
{
	...
	if (m_pSheet->GetPageBorderStyle() == xtpPageBorderFrame)
	{
		...
		wrpTreeView.OpenThemeData(NULL, L"LISTBOX");
...
void CXTPPropertyPageStaticCaption::OnPaint()
{
	...
	CXTPFontDC font(&dc, &XTPAuxData().xtpFontBold);
	dc.DrawText(strText, rc, DT_VCENTER | DT_SINGLELINE);
}

void CXTPPropertyPage::UpdatePage(void)
{
