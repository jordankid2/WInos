...
namespace XTPTaskDialogAPI
{
	...
	_XTP_EXT_CLASS BOOL AFXAPI XTPCalcIdealTextSize(const CString& strText, CSize& sizeText, CFont& font, int dluMax /*=xtpDluMax*/, int dluGrow /*=xtpDluAdd*/)
	{
		...
		else if (sizeExtent.cx > sizeText.cx)
		{
			CSize sizeMin = sizeText;
			XTPCalcTextSize(strText, sizeText, font);

			if (sizeText.cx > sizeMin.cx)
			{
				return TRUE;
			}
		}
	...
	_XTP_EXT_CLASS BOOL AFXAPI XTPCalcIdealTextSize(CXTPMarkupContext* pMarkupContext, CXTPMarkupUIElement* pUIElement, CSize& sizeText, CFont& font, int dluMax /*=xtpDluMax*/, int dluGrow /*=xtpDluAdd*/)
	{
		XTPMarkupSetDefaultFont(pMarkupContext, (HFONT)font.GetSafeHandle(), (COLORREF)-1);

		CSize sizeExtent = XTPMarkupMeasureElement(pUIElement, INT_MAX, INT_MAX);
		CSize sizeMax = sizeExtent;
		...
		sizeExtent = XTPMarkupMeasureElement(pUIElement, sizeText.cx, INT_MAX);

		if (sizeExtent.cx == sizeText.cx && sizeExtent.cy == sizeMax.cy && sizeExtent.cx < sizeMax.cx)
			sizeExtent = sizeMax;
