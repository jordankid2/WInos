...
int CXTPTaskDialogLinkCtrl::DrawTextPartText(CDC* pDC, CString strBuffer, int x, int y, LINKITEM* pItem)
{
	...
	pDC->DrawText(strBuffer, CRect(x, y, x + sz.cx + XTP_DPI_X(10), y + sz.cy), DT_TOP | DT_LEFT | DT_NOPREFIX | DT_SINGLELINE);
...
{
	CXTPFontDC fontDC(pDC, pItem && m_xtpFontUL.GetSafeHandle() ? &m_xtpFontUL : GetFont(), pItem ? m_crTextLink :m_crText);
...
void CXTPTaskDialogLinkCtrl::PreSubclassWindow()
{
	...
	if (m_bPreSubclassWindow)
	{
		...
		if (m_arrLinks.GetSize() > 0 && GetFont())
		{
			...
			lf.lfUnderline = 1;
			m_xtpFontUL.DeleteObject();
			m_xtpFontUL.CreateFontIndirect(&lf);
		}
		Invalidate(FALSE);
	}
}
...
// CXTPTaskButtonTheme
//===========================================================================
const int xtpDluMargin           = 5;   // non-client margin.


CXTPTaskButtonTheme::CXTPTaskButtonTheme(BOOL bThemeReady)
{
	m_bThemeReady = bThemeReady;
	m_ptMargin = XTPDlu2Pix(XTP_DPI_X(xtpDluMargin), XTP_DPI_Y(xtpDluMargin));
	m_pFontTitle = NULL;
...
CXTPTaskButtonTheme::CXTPTaskButtonTheme()
{
	...
	m_bThemeReady = m_themeButton->IsAppThemeActive();
	m_ptMargin = XTPDlu2Pix(XTP_DPI_X(xtpDluMargin), XTP_DPI_Y(xtpDluMargin));
	m_pFontTitle = NULL;
	m_bOffsetHiliteText = FALSE;
	m_nBorderWidth = XTP_DPI_X(3);
...
BOOL CXTPTaskButtonTheme::DrawWinThemeBackground(CDC* pDC, CXTPButton* pButton)
{
	if (m_bThemeReady)
	{
		...
		if (m_themeButton->IsAppThemeActive() && XTPSystemVersion()->IsWinVistaOrGreater())
		{
		...
		if (bSelected)
		{
			...
			rcItem.DeflateRect(1, 1);
			XTPDrawHelpers()->DrawLine(pDC, rcItem.left, rcItem.top, rcItem.left + rcItem.Width(), rcItem.top, m_crBorder3DShadow);

			rcItem.top += XTP_DPI_Y(1);
...
void CXTPTaskButtonTheme::DrawButtonIcon(CDC* pDC, CXTPButton* pButton)
{
	...
	if (pIcon != NULL)
	{
		...
		CPoint point(XTP_DPI_X(7), m_ptMargin.y - pIcon->GetHeight() / 2 + size.cy / 2);
...
void CXTPTaskButtonThemeExpando::DrawButton(CDC* pDC, CXTPButton* pButton)
{
	...
	if (::GetFocus() == pButton->GetSafeHwnd() && ((pButton->SendMessage(WM_QUERYUISTATE) & UISF_HIDEFOCUS) == 0))
	{
		...
		rcText.InflateRect(XTP_DPI_X(1), 0);
		pDC->DrawFocusRect(rcText);
	}

	rcItem.right = rcItem.left + XTP_DPI_X(18);
	rcItem.bottom = rcItem.top + min(XTP_DPI_Y(20), rcItem.Height());

	CPoint pt(rcItem.right / 2 - XTP_DPI_X(1), rcItem.bottom / 2 - XTP_DPI_Y(3));
	...
	if (m_themeButton->IsAppThemeActive())
	{
		...
		themeTaskDialog.OpenThemeData(NULL, L"TASKDIALOG");

		if (themeTaskDialog.IsAppThemeActive())
		{
			CRect rcButton(0, 0, XTP_DPI_X(19), XTP_DPI_Y(21));
	...
	XTPPrimitiveDrawer()->DrawSymbol(pDC, !pButton->GetChecked()
		? xtpPrimitiveSymbolPlayDown : xtpPrimitiveSymbolPlayUp,
		rcItem, GetXtremeColor(COLOR_BTNTEXT));
}
...
void CXTPTaskDialogProgressCtrl::OnPaint()
{
	...
	int nSpace = XTP_DPI_X(bVista ? 0 : 2);
	rc.left = m_nMarqueePos + XTP_DPI_X(1);
...
void CXTPTaskDialogProgressCtrl::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 101 && m_bMarquee)
	{
		...
		m_nMarqueePos += (dxBlock + nSpace) * XTP_DPI_X(m_nMarqueeDelay > 50 ? 1 : 2);
