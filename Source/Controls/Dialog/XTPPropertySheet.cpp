...
CXTPPropertyPage* CXTPPropertySheet::GetPage(int nPage) const
{
	return nPage >= 0 && nPage < m_arrPages.GetSize() ? m_arrPages.GetAt(nPage) : NULL;
}

CXTPPropertyPage* CXTPPropertySheet::GetActivePage() const
{
	return m_hWnd ? m_pActivePage : GetPage(m_psh.nStartPage);
}

int CXTPPropertySheet::GetActiveIndex() const
{
...
BOOL CXTPPropertySheet::AddButton(UINT nIDCaption, UINT nID)
{
	...
	pButton->SendMessage(WM_SETFONT, (WPARAM)(HFONT)m_xtpFontNormal, FALSE);
...
void CXTPPropertySheet::MoveButtons(AFX_SIZEPARENTPARAMS* pLayout, CSize szClient)
{
	...
	CClientDC dc(this);
	CXTPFontDC font(&dc, &m_xtpFontNormal);
...
void CXTPPropertySheet::CreateFont(CXTPPropertyPage* pActivePage)
{
	...
	SendMessage(WM_SETFONT, (WPARAM)(HFONT)m_xtpFontNormal, FALSE);
}
...
void CXTPPropertySheet::UpdatePages()
{
