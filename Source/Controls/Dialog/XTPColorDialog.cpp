...
void CXTPColorDialog::OnPaint()
{
	...
	rect.top = rcBtnCancel.bottom + m_nGapSize 
		+ (::IsWindow(m_wndHexEdit.m_hWnd) ? m_nTextHeight + m_nGapSize : 0)
		+ m_nTextHeight + m_nGapSize;
	...
	CRect rcText(
		rect.left, rect.top - m_nGapSize - m_nTextHeight,
		rect.right, rect.top - m_nGapSize);
	...
	rcText.SetRect(
		rect.left, rect.bottom + m_nGapSize,
		rect.right, rect.bottom + m_nGapSize + m_nTextHeight);
...
int CXTPColorDialog::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (m_dwStyle & CPS_XTP_SHOWHEXVALUE)
	{
		if (!m_wndHexEdit.CreateEx(WS_EX_STATICEDGE, _T("EDIT"), RGBtoHex(m_clrNew),
			WS_CHILD | WS_VISIBLE | ES_READONLY | ES_CENTER, CRect(0, 0, 0, 0), this, AFX_IDC_CHANGE))
		{
