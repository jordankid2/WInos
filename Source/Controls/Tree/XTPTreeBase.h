...
public:
	...
	//     IsFindValid
	//-----------------------------------------------------------------------
	virtual HTREEITEM FindItemExact(LPCTSTR lpszSearch, BOOL bCaseSensitive = FALSE, BOOL bDownDir = TRUE, HTREEITEM hItem = NULL);
...
protected:
	...
	//     has no expander.
	//-----------------------------------------------------------------------
	CRect GetItemExpanderRect(HTREEITEM hItem, CDC* pDC, const CRect& rcIcon, const CRect& rcStateIcon) const;
	...
	_XTP_DEPRECATE("The method is deprecated and will be removed in future versions. Use 'DrawItem(CDC *pDC, HTREEITEM hItem, const COLORREF crBack)' instead.")
	...
	//-----------------------------------------------------------------------
	void DrawDarkThemeExpander(CDC *pDC, COLORREF crBack, CRect rcItem, CRect rcExpander, int nPart, int nState) const;
	...
	//     hItem          - Handle of the tree item to start searching from, NULL to use
	//     the currently selected tree item.
	//     bExactlySameText - TRUE if item label and input search string should have exactly the same text. If this parameter
	//     is TRUE the value of bWholeWord is not take in account.
	// Returns:
	//     The handle to the item, or returns NULL.
	// See Also:
	//     IsFindValid
	//-----------------------------------------------------------------------
	virtual HTREEITEM FindItemImpl(LPCTSTR lpszSearch, BOOL bCaseSensitive = FALSE, BOOL bDownDir = TRUE, BOOL bWholeWord = FALSE, HTREEITEM hItem = NULL, BOOL bExactlySameStr = FALSE);
...
protected:
	...
	BOOL m_bDoDefaultItemDrawing; // FALSE by default, which means an item will be drawn by framework. 
							    // If TRUE, items will be drawn in a standard way.
