...
void CXTPTreeBase::SetTheme(int nTheme)
{
	...
	case xtpControlThemeOffice2013:
		{
			m_pTheme = new CXTPTreeOffice2013Theme();
		}
		break;
	case xtpControlThemeVisualStudio2015:
		{
			m_pTheme = new CXTPTreeVisualStudio2015Theme();
		}
		break;
	default:
		{
...
CRect CXTPTreeBase::GetItemIconRect(HTREEITEM hItem) const
{
	...
	if (pImageList)
	{
		...
		{
			ImageList_GetIconSize((*pImageList).m_hImageList, reinterpret_cast<int*>(&szImage.cx), reinterpret_cast<int*>(&szImage.cy));
		}
		...
		if (bRes)
		{
			...
			if (0 != nStateImage)
			{
				...
				if (pImageListState->GetImageInfo(nStateImage, &ImageInfo))
				{
					...
					rcStateIcon.top += (rcStateIcon.Height() / 2 - szStateImageSize.cy / 2); // vertical alignment center
	...
	const CRect& rcStateIcon) const
{
	...
	if (!rcIcon.IsRectEmpty())
	{
		...
		{
			rcExpander.left = rcExpander.left - (m_pTreeCtrl->GetIndent() + partSize.cx) / 2;
		}
	}
	else
	{
...
void CXTPTreeBase::DrawItem(CDC *pDC, HTREEITEM hItem, const COLORREF crBack)
{
	...
	UINT nItemState = m_pTreeCtrl->GetItemState(hItem, TVIS_SELECTED | TVIS_EXPANDED | TVIS_CUT | TVIS_OVERLAYMASK);
	...
	if (NULL != pImageList && !rcIcon.IsRectEmpty())
	{
		...
		CPoint or(0,0);
		CSize sz(0, 0);
		pImageList->DrawIndirect(pDC, curImage,
			rcIcon.TopLeft(), sz, or, fl, SRCCOPY, CLR_DEFAULT, GetTreeBackColor());
	}
	...
	if (dwTreeStyle & TVS_HASLINES)
	{
		...
		DrawDottedLine(pDC, CPoint(nLineX, nVLineTop + nVLineHalfSize), CPoint(nLineRight, nVLineTop + nVLineHalfSize));
	...
	if ((dwTreeStyle & TVS_HASBUTTONS) && m_pTreeCtrl->ItemHasChildren(hItem)) // item has children
	{
		...
		if (m_pWindowsTreeTheme->IsAppThemeActive())
		{
			...
			if (IsExplorerTheme() && XTPSystemVersion()->IsWinVistaOrGreater()) // no glow effect on Windows XP
			{
				...
				// when enabled HDPI and height of expander more than height of item, we need to reduce size of expander, for correct show of hot item
			...
			if (m_pTheme->IsThemeDark())
			{
				...
				else
				{
					...
					BYTE r = (BYTE)(255 - GetRValue(bg)), g = (BYTE)(255 - GetGValue(bg)), b = (BYTE)(255 - GetBValue(bg));
					...
					m_pWindowsTreeTheme->DrawThemeBackground(dcMem.GetSafeHdc(), nPart, nState, rr, NULL);
					pDC->BitBlt(rcExpander.left, rcExpander.top, rcExpander.Width(), rcExpander.Height(), &dcMem, 0, 0, NOTSRCCOPY);
					dcMem.DeleteDC();
				}
			}
			else
			{
				m_pWindowsTreeTheme->DrawThemeBackground(
					pDC->GetSafeHdc(), nPart, nState, rcExpander, NULL);
			}
		}
		else
		{
			CBrush brBkColor(GetTreeBackColor()), brPlusMinus(GetTreeTextColor()), brBorder(GetTreeTextColor());
...
void CXTPTreeBase::DoPaint(CDC& dc, BOOL bInternal)
{
	...
	while (hItem && iVisibleCount--)
	{
		...
		// draw the focus rect.
		if ((uState & TVIS_FOCUSED) && (m_pTheme && m_pTheme->IsDrawFocusRect()) && !IsFullRowSelectionEnabled())
		{
...
void CXTPTreeBase::DrawDarkThemeExpander(CDC *pDC, COLORREF crBack, CRect rcItem, CRect rcExpander, int nPart, int nState) const
{
	ASSERT_VALID(pDC);
		...
		for (int y = 0; y < rc.Height(); y++)
		{
			...
			else
			{
				if (nState == GLPS_OPENED || nState == XTP::HGLPS_OPENED)
					dcMem.SetPixel(x, y, nPart == XTP::TVP_HOTGLYPH ? RGB(85, 211, 249) : RGB(150, 150, 150));
				else
					dcMem.SetPixel(x, y, nPart == XTP::TVP_HOTGLYPH ? RGB(85, 211, 249) : RGB(200, 200, 200));
			}
		}
	}
	...
	pDC->BitBlt(rcExp.left, rcExp.top, rcExp.Width(), rcExp.Height(), &dcMem, rc.left + delta, rc.top + delta, SRCCOPY);
	dcMem.DeleteDC();
}