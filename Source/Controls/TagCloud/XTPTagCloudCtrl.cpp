...
CXTPTagCloudStyle::CXTPTagCloudStyle(int nScore, COLORREF clrText, COLORREF clrTextHover, COLORREF clrBorder, LOGFONT& lf)
	: m_nScore(nScore)
	, m_clrText(clrText)
...
INT_PTR CXTPTagCloudStyleManager::AddStyle(int nScore, COLORREF clrText, COLORREF clrTextHover, COLORREF clrBorder, LOGFONT& lf)
{
...
BOOL CXTPTagCloudStyleManager::SetColors(int nScore, COLORREF clrText, COLORREF clrTextHover, COLORREF clrBorder /*=COLORREF_NULL*/)
{
...
void CXTPTagCloudTagArray::Sort(XTPTagCloudSort nSort, BOOL bAsc)
{
	...
	case xtpTagCloudAlpha:
		qsort(ppData,  nSize, sizeof(ppData[0]), bAsc ? CompareNameAsc : CompareNameDesc);
		break;
	case xtpTagCloudNumeric:
		qsort(ppData,  nSize, sizeof(ppData[0]), bAsc ? CompareScoreAsc : CompareScoreDesc);
		break;
	}
}

int AFX_CDECL CXTPTagCloudTagArray::CompareName(XTP_TAGCLOUDTAG* pTagA, XTP_TAGCLOUDTAG* pTagB, BOOL bAsc)
{
...
int AFX_CDECL CXTPTagCloudTagArray::CompareScore(XTP_TAGCLOUDTAG* pTagA, XTP_TAGCLOUDTAG* pTagB, BOOL bAsc)
{
...
BOOL CXTPTagCloudCtrl::RegisterWindowClass(HINSTANCE hInstance /*= NULL*/)
{
	return XTPDrawHelpers()->RegisterWndClass(hInstance,
		XTPTAGCLOUDCTRL_CLASSNAME, CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW);
}
	...
	for (INT_PTR i = 0; i < arrTags.GetSize(); i++)
	{
		if (arrTags[i])
		{
			...
			if (pStyle)
			{
				...
				pDC->SetTextColor((m_pHover == arrTags[i]) ? pStyle->m_clrTextHover : pStyle->m_clrText);
				pDC->SetBkMode(TRANSPARENT);
				pDC->DrawText(arrTags[i]->strItem, arrTags[i]->rcItem, DT_VCENTER | DT_CENTER | DT_SINGLELINE);
				...
				else
				{
					...
					{
						pDC->Draw3dRect(arrTags[i]->rcItem,
							pStyle->m_clrBorder, pStyle->m_clrBorder);
					}
				}
			}
		}
...
void CXTPTagCloudCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (pHover != m_pHover)
	{
		...
		if (m_bTipEnabled)
		{
			...
			{
				CString strToolTip;
				strToolTip.Format(XTP_IDS_TAGCLOUD_TIP, m_pHover->strItem.operator LPCTSTR(), m_pHover->nScore);
...
BOOL CXTPTagCloudCtrl::OnNotify(XTP_TAGCLOUDTAG* pTag, UINT nEventCode)
{
	...
	if (nEventCode == WM_XTP_TAGCLOUDDBLCLK || 
		nEventCode == WM_XTP_TAGCLOUDRIGHTCLK || 
...
BEGIN_DISPATCH_MAP(CXTPTagCloudStyleManager, CXTPCmdTarget)
	DISP_FUNCTION_ID(CXTPTagCloudStyleManager, "InitDefaultStyles", 1, OleInitDefaultStyles, VT_EMPTY, VTS_NONE)
	...
	DISP_FUNCTION_ID(CXTPTagCloudStyleManager, "CreateStyle", 8, OleCreateStyle, VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPTagCloudStyleManager, "Count", 9, OleGetItemCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPTagCloudStyleManager, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
END_DISPATCH_MAP()
...
void CXTPTagCloudStyleManager::OleAdd(LPDISPATCH pStyle)
{
	...
	CTagCloudStyle* pTagStyle = DYNAMIC_DOWNCAST(CTagCloudStyle, CTagCloudStyle::FromIDispatchSafe(pTagStyleDisp));
	if (!pTagStyle) 
	{
		AfxThrowOleException(E_INVALIDARG);
	}
	...
	AddStyle(pTagStyle->m_nScore, pTagStyle->m_TextColor, pTagStyle->m_TextHoverColor, pTagStyle->m_BorderColor, logFont);
...
void CXTPTagCloudStyleManager::OleSetStyle(LPDISPATCH pStyle)
{
	...
	LOGFONT logFont;
	pTagStyle->m_Font.GetLogFont(&logFont);
	SetColors(pTagStyle->m_nScore, pTagStyle->m_TextColor, pTagStyle->m_TextHoverColor, pTagStyle->m_BorderColor);
