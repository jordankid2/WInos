// XTPTabPaintManagerTheme.cpp: implementation of the CXTPTabPaintManagerTheme class.
//
...
void CXTPTabPaintManagerTheme::DrawWorkspacePart(CDC* pDC, LPRECT lpRect, XTPTabWorkspacePart workspacePart)
{
	if (!m_pPaintManager->m_bStaticFrame)
	{
		...
		case xtpTabWorkspacePartBack:
			pDC->FillSolidRect(lpRect, GetColorSet()->m_clrTabClientBack);
			break;
		}
	}
	else
	{
		...
		{
		case xtpTabWorkspacePartBorder:
			pDC->Draw3dRect(XTP_DPI_X(1), 0, lpRect->right - lpRect->left - XTP_DPI_X(2),
				lpRect->bottom - lpRect->top - XTP_DPI_Y(1), GetXtremeColor(COLOR_3DFACE), GetXtremeColor(COLOR_3DFACE));
		...
		case xtpTabWorkspacePartVSplitter:
			...
			pDC->FillSolidRect(lpRect->right - XTP_DPI_X(1), lpRect->top, XTP_DPI_X(1), lpRect->bottom - lpRect->top, GetColorSet()->m_clrFrameBorder);
			break;

		case xtpTabWorkspacePartHSplitter:
			pDC->FillSolidRect(lpRect, GetColorSet()->m_clrControlFace);
			pDC->FillSolidRect(lpRect->left, lpRect->bottom - XTP_DPI_Y(1), lpRect->right - lpRect->left, XTP_DPI_Y(1), GetColorSet()->m_clrFrameBorder);
			break;
