...
void CXTPTabThemeVisualStudio2012::DrawWorkspacePart(CDC* pDC, LPRECT lpRect, XTPTabWorkspacePart workspacePart)
{
	CRect rc(lpRect);
	...
	case xtpTabWorkspacePartBorder:
		{
			...
			pDC->Draw3dRect(rc.left, rc.top, rc.Width(), rc.Height(), GetColorSet()->m_clrTabClientBack, GetColorSet()->m_clrTabClientBack);
	...
	case xtpTabWorkspacePartVSplitter:
		...
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top, XTP_DPI_X(1), rc.bottom - rc.top, GetColorSet()->m_clrFrameBorder);
		break;
		
	case xtpTabWorkspacePartHSplitter:
		pDC->FillSolidRect(rc, GetColorSet()->m_clrHeaderFace.clrLight);
		pDC->FillSolidRect(rc.left, rc.bottom - XTP_DPI_Y(1), rc.right - rc.left, XTP_DPI_Y(1), GetColorSet()->m_clrFrameBorder);
		break;
