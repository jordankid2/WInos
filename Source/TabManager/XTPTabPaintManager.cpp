	...
	: m_bPanelManager(bPanelManager)
{
	...
	m_szIcon = CSize(XTP_DPI_X(16), XTP_DPI_Y(16));
...
CXTPTabPaintManagerTheme* CXTPTabPaintManager::SetAppearance(XTPTabAppearanceStyle tabAppearance)
{
	...
	{
	case xtpTabAppearanceExcel2003:
	...
	case xtpTabAppearancePropertyPageFlat:
		...
		break;
	case xtpTabAppearanceFlat:
		pAppearanceSet = new CXTPTabThemeFlat();
		break;
	case xtpTabAppearanceStateButtons:
...
int CXTPTabPaintManager::DrawSingleButtonIconAndText(CDC* pDC, CXTPTabManagerItem* pItem, CRect rcItem, BOOL bDraw)
{
	...
	else
	{
		...
		{
			if (!bDrawText)
				return _GetButtonLength((bVertical ? XTP_DPI_Y(8) : XTP_DPI_X(8)) + (bVertical ? szIcon.cy : szIcon.cx), m_nMinTabWidth, m_nMaxTabWidth) ;
...
{
	return AxCreateOleFont(&m_xtpFontNormal, this, (LPFNFONTCHANGED)&CXTPTabPaintManager::OleSetFont);
}
