...
CSize CXTPTabManager::CNavigateButtonTabClose::GetSize() const
{
	return m_pManager->GetPaintManager()->m_szTabCloseButton;
}

void CXTPTabManager::CNavigateButtonClose::DrawEntry(CDC* pDC, CRect rc, COLORREF clr, BOOL bEnabled)
{
	XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolTabClose, rc, clr, COLORREF_NULL, !bEnabled);
}

void CXTPTabManager::CNavigateButtonArrowRight::DrawEntry(CDC* pDC, CRect rc, COLORREF clr, BOOL bEnabled)
{
	ASSERT(NULL != pDC);

	if (m_pManager->IsHorizontalPosition())
	{
		BOOL bRTL = (pDC->GetLayout() == LAYOUT_RTL);

		if(bEnabled)
			XTPPrimitiveDrawer()->DrawSymbol(pDC, !bRTL ? xtpPrimitiveSymbolTabPlayRight: xtpPrimitiveSymbolTabPlayLeft, rc, clr, COLORREF_NULL);
		else
			XTPPrimitiveDrawer()->DrawSymbol(pDC, !bRTL ? xtpPrimitiveSymbolTabPlayRightDisabled: xtpPrimitiveSymbolTabPlayLeftDisabled, rc, clr, COLORREF_NULL);
	}
	else
	{
		XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayDown, rc, clr, COLORREF_NULL, !bEnabled);
	}
}
...
void CXTPTabManager::CNavigateButtonArrowLeft::DrawEntry(CDC* pDC, CRect rc, COLORREF clr, BOOL bEnabled)
{
	ASSERT(NULL != pDC);
	...
	if (m_pManager->IsHorizontalPosition())
	{
		...
		if(bEnabled)
			XTPPrimitiveDrawer()->DrawSymbol(pDC, !bRTL ? xtpPrimitiveSymbolTabPlayLeft : xtpPrimitiveSymbolTabPlayRight, rc, clr, COLORREF_NULL);
		else
			XTPPrimitiveDrawer()->DrawSymbol(pDC, !bRTL ? xtpPrimitiveSymbolTabPlayLeftDisabled : xtpPrimitiveSymbolTabPlayRightDisabled, rc, clr, COLORREF_NULL);
	}
	else
	{
		XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayUp, rc, clr, COLORREF_NULL, !bEnabled);
	}
}
...
void CXTPTabManager::EnableTabThemeTexture(HWND hWnd, BOOL bEnable)
{
	...
	themeTabControl.OpenThemeData(hWnd, L"TAB");
...
BEGIN_DISPATCH_MAP(CXTPTabManagerItem, CXTPCmdTarget)
...
// Note: we add support for DIID_ITabControlItem to support typesafe binding
//  from VBA.  This IID must match the GUID that is attached to the
//  dispinterface in the .ODL file.

BEGIN_INTERFACE_MAP(CXTPTabManagerItem, CXTPCmdTarget)
	INTERFACE_PART(CXTPTabManagerItem, XTPDIID_ITabControlItem, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPTabManagerItem, XTPDIID_ITabControlItem)
