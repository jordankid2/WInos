...
#include "Common/XTPDisableNoisyWarnings.h"
...
{
	pDC->SetTextColor(!pItem->IsEnabled() ? m_clrTextDisabled: !pItem->GetTabManager()->IsActive() ? m_clrTextSelectedHighlighted :
...
void CXTPTabPaintManagerColorSet::FillStateButton(CDC *pDC, CRect rcItem, CXTPTabManagerItem *pItem)
{
	if (!pItem->IsSelected())
	{
		if (pItem->GetTabManager()->IsHorizontalPosition())
			...
			pDC->Draw3dRect(rcItem.right, rcItem.top + 2, 2, rcItem.Height() - 4, m_clrBorderHighlight, m_clrHeaderFace.clrLight);
		else
			...
			pDC->Draw3dRect(rcItem.left + 2, rcItem.bottom, rcItem.Width() - 4, 2, m_clrBorderHighlight, m_clrHeaderFace.clrLight);
	}
	else
	{
		if (m_pPaintManager->m_bOneNoteColors)
