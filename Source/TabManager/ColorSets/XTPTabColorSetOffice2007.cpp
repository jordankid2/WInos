...
void CXTPTabColorSetOffice2007::RefreshMetrics()
{
	...
	if (pImages->IsValid())
	{
		...
		m_clrFrameBorder.SetStandardValue(pImages->GetImageColor(_T("TabManager"), _T("FrameBorder"), RGB(59, 97, 156)));
		m_csPropertyPage2003.clrFrameBorder.SetStandardValue(m_clrFrameBorder);
		m_csPropertyPage.clrDarkShadow.SetStandardValue(pImages->GetImageColor(_T("TabManager"), _T("DarkShadow"), RGB(0, 53, 154)));
		m_csPropertyPage2003.clrDarkShadow.SetStandardValue(m_csPropertyPage.clrDarkShadow);

		m_csPropertyPage.clrShadow.SetStandardValue(pImages->GetImageColor(_T("TabManager"), _T("LightShadow"), RGB(117, 166, 241)));
		...
		m_clrHeaderFace.SetStandardValue(pImages->GetImageColor(_T("Ribbon"), _T("RibbonFace"), RGB(196, 218, 250)));

		m_clrAutoHideFace.SetStandardValue(m_clrHeaderFace);

		m_clrButtonNormal.SetStandardValue(pImages->GetImageColor(_T("Window"), _T("ButtonFace"), RGB(162, 192, 246)));
		m_clrButtonSelected.SetStandardValue(pImages->GetImageColor(_T("Window"), _T("HighlightChecked"), RGB(255, 194, 115)));
		m_clrButtonHighlighted.SetStandardValue(pImages->GetImageColor(_T("Window"), _T("HighlightSelected"), RGB(255, 240, 201)));
		m_clrNavigateButtonBorder.SetStandardValue(pImages->GetImageColor(_T("Window"), _T("WindowFrame")));

		m_clrTextHighlight.SetStandardValue(pImages->GetImageColor(_T("Ribbon"), _T("TabNormalText"), RGB(21, 66, 139)));
		m_clrTextNormal.SetStandardValue(pImages->GetImageColor(_T("Ribbon"), _T("TabNormalText"), RGB(21, 66, 139)));
		m_clrTextSelected.SetStandardValue(pImages->GetImageColor(_T("Ribbon"), _T("TabSelectedText"), RGB(21, 66, 139)));
