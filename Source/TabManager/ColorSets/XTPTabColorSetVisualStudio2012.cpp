...
void CXTPTabColorSetVisualStudio2012::FillNavigateButton(CDC *pDC, CXTPTabManagerNavigateButton *pButton, CRect &rc)
{
	...
	if (pButton->IsOwnerNull())
	{
		...
		{
			pDC->FillSolidRect(rc, bPressed ? m_clrActiveFilesBackPressed : m_clrActiveFilesBackHighlighted);
			pButton->DrawEntry(pDC, rc, bPressed ? m_clrActiveFilesGlyphPressed : m_clrActiveFilesGlyphHighlighted, bEnabled);
	...
	else
	{
		...
		if (bTabSelected)
		{
			if (bPressed || bHilighted)
			{
				if (bTabGroupActive)
				{
					pDC->FillSolidRect(rc, bPressed ? m_clrButtonSelectedPressed : m_clrButtonSelectedHighlighted);
					pButton->DrawEntry(pDC, rc, bPressed ? m_clrTextSelectedPressed : m_clrTextSelectedHighlighted, bEnabled);
				}
				else
				{
					pDC->FillSolidRect(rc, bPressed ? m_clrNonActiveButtonSelectedPressed : m_clrNonActiveButtonSelectedHighlighted);
					pButton->DrawEntry(pDC, rc, bPressed ? m_clrNonActiveTextSelectedPressed : m_clrNonActiveTextSelectedHighlighted, bEnabled);
				}
		...
		else
		{
			if (bPressed || bHilighted)
			{
				...
				pButton->DrawEntry(pDC, rc, bPressed ? m_clrTextPressed : m_clrTextHighlightClose, bEnabled);
			...
			else
			{
				...
				pButton->DrawEntry(pDC, rc, bTabHilighted ? m_clrTextHighlight : m_clrButtonNormal, bEnabled);
			}
		}
	}
}
