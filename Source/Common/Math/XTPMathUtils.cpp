...
void AFX_CDECL CXTPMathUtils::ComputeTriangleNormal(
...
double AFX_CDECL CXTPMathUtils::GetSegmentLength(
	double x1, double y1,
	double x2, double y2)
{
	double dx = x1 - x2;
...
BOOL AFX_CDECL CXTPMathUtils::HitTestLineSegment(
	double x1, double y1,
	double x2, double y2,
...
BOOL AFX_CDECL CXTPMathUtils::GetLinesIntersection(
	...
	double& yi)
{
	...
	if (0. != d)
	{
		...
		if (!(x < __min(x1, x2) || x > __max(x1, x2) ||
			x < __min(x3, x4) || x > __max(x3, x4) ||
			y < __min(y1, y2) || y > __max(y1, y2) ||
			y < __min(y3, y4) || y > __max(y3, y4)))
		{
			xi = x;
			yi = y;
			bIntersect = TRUE;
...
void AFX_CDECL CXTPMathUtils::ComputeVectorCrossProduct(
	double x1, double y1, double z1,
	double x2, double y2, double z2,
...
double AFX_CDECL CXTPMathUtils::ComputeVectorDotProduct(
	double x1, double y1, double z1,
	double x2, double y2, double z2)
{
