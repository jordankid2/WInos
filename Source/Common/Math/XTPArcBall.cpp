...
{
	return 0. < m_dWidthAdjustmentFactor
		&& 0. < m_dHeightAdjustmentFactor;
}
...
void CXTPArcBall::GetRotationQuaternion(double& xQuat, double& yQuat, double& zQuat, double& wQuat)
{
	...
	CXTPPoint3d vPerp;
	CXTPMathUtils::ComputeVectorCrossProduct(
		m_vBegin.X, m_vBegin.Y, m_vBegin.Z,
		m_vEnd.X, m_vEnd.Y, m_vEnd.Z,
		vPerp.X, vPerp.Y, vPerp.Z);
	...
	if (CXTPMathUtils::m_dEPS < dLength)
	{
		...
		yQuat = vPerp.Y;
		zQuat = vPerp.Z;
		wQuat = CXTPMathUtils::ComputeVectorDotProduct(
			m_vBegin.X, m_vBegin.Y, m_vBegin.Z,
			m_vEnd.X, m_vEnd.Y, m_vEnd.Z);
	}
	else
