...
{
	UINT nClassStyle = UINT((dwStyle & XTP_TTS_NOSHADOW ? 0 : CS_DROPSHADOW) | CS_SAVEBITS | CS_OWNDC);
...
CSize CXTPToolTipContextToolTip::GetToolSize(TOOLITEM* lpToolInfo)
{
	...
	if (bDrawImage)
	{
		...
		if (m_pContext->IsImageDPIScalingEnabled())
...
void CXTPToolTipContextToolTip::DrawEntry(CDC* pDC, TOOLITEM* lpToolInfo, CRect rc)
{
	...
	CRect rcTitle(rc.left + XTP_DPI_X(2), rc.top + XTP_DPI_Y(2), 
		rc.right - XTP_DPI_X(2), rc.bottom);
...
void CXTPToolTipContext::COfficeToolTip::DrawBackground(CDC* pDC, TOOLITEM* /*lpToolInfo*/, CRect rc)
{
...
void CXTPToolTipContext::COffice2013ToolTip::DrawBackground(CDC* pDC, TOOLITEM* /*lpToolInfo*/, CRect rc)
{
...
XTP_STDMETHODIMP CXTPToolTipContext::CHTMLToolTip::XDocHostUIHandler::GetHostInfo( DOCHOSTUIINFO* pInfo )
{
...
XTP_STDMETHODIMP CXTPToolTipContext::CHTMLToolTip::XDocHostUIHandler::EnableModeless(BOOL /*fEnable*/)
{
	return E_NOTIMPL;
}

XTP_STDMETHODIMP CXTPToolTipContext::CHTMLToolTip::XDocHostUIHandler::OnDocWindowActivate(BOOL /*fActivate*/)
{
	return E_NOTIMPL;
}

XTP_STDMETHODIMP CXTPToolTipContext::CHTMLToolTip::XDocHostUIHandler::OnFrameWindowActivate(BOOL /*fActivate*/)
{
	return E_NOTIMPL;
}
...
XTP_STDMETHODIMP CXTPToolTipContext::CHTMLToolTip::XDocHostUIHandler::TranslateAccelerator(LPMSG /*lpMsg*/,
	/* [in] */ const GUID __RPC_FAR* /*pguidCmdGroup*/,
	/* [in] */ DWORD /*nCmdID*/)
{
	return S_FALSE;
...
XTP_STDMETHODIMP CXTPToolTipContext::CHTMLToolTip::XDocHostUIHandler::GetOptionKeyPath(BSTR* /*pbstrKey*/, DWORD)
{
	return E_NOTIMPL;
}
...
CSize CXTPToolTipContext::CHTMLToolTip::GetToolSize(TOOLITEM* lpToolInfo)
{
	...
	if (m_wndBrowser.GetSafeHwnd() == 0)
	{
		...
		if (!m_wndBrowser.CreateControl(CLSID_WebBrowser, 0,
			WS_VISIBLE | WS_CHILD, XTP_DPI(CRect(0, 0, 200, 200)), this, AFX_IDW_PANE_FIRST))
		{
			return 0;
		}
...
#else
	m_wndBrowser.MoveWindow(XTP_DPI_X(3) + rcMargin.left, XTP_DPI_Y(3) + rcMargin.top, 
		nBestWidth, nBestHeight);
#endif
...
BOOL CXTPToolTipContext::IsImageDPIScalingEnabled() const
{
	return m_bImageDPIScaling;
}

void CXTPToolTipContext::EnableImageDPIScaling(BOOL bScale)
{
	m_bImageDPIScaling = bScale;
}
...
void CXTPToolTipContext::FillInToolInfo(TOOLINFO* pTI, HWND hWnd, CRect rect, INT_PTR nHit, const CString& strToolTip)
{
...
#endif
	{
		...
		pTI->lpszText = (LPTSTR) ::calloc(XTPToSizeTChecked(strToolTip.GetLength()) + 1, sizeof(TCHAR));
		if (pTI->lpszText != NULL) STRCPY_S(pTI->lpszText, XTPToSizeTChecked(strToolTip.GetLength()) + 1, (LPCTSTR)strToolTip);
	}
}
	...
	const CString& strToolTip, const CString& strTitle, const CString& strDescription, CXTPImageManager* pImageManager)
{
	...
	if (pTI != NULL && pTI->cbSize == sizeof(XTP_TOOLTIP_TOOLINFO_EX))
	{
		...
		{
			ptc->lpszDescription = (LPTSTR)::calloc(XTPToSizeTChecked(strDescription.GetLength()) + 1, sizeof(TCHAR));
			if (ptc->lpszDescription != NULL) STRCPY_S(ptc->lpszDescription, XTPToSizeTChecked(strDescription.GetLength()) + 1, (LPCTSTR)strDescription);

			ptc->lpszTitle = (LPTSTR)::calloc(XTPToSizeTChecked(strTitle.GetLength()) + 1, sizeof(TCHAR));
			if (ptc->lpszTitle != NULL) STRCPY_S(ptc->lpszTitle, XTPToSizeTChecked(strTitle.GetLength()) + 1, (LPCTSTR)strTitle);
		}
...
void CXTPToolTipContext::SetStyle(XTPToolTipStyle toolStyle)
{
	if (m_toolStyle != toolStyle && (toolStyle != xtpToolTipBalloon || IsBalloonStyleSupported()))
	{
		...
		ModifyToolTipStyle(
			DWORD(XTP_TTS_OFFICEFRAME | XTP_TTS_NOSHADOW),
			DWORD(m_toolStyle == xtpToolTipOffice ? XTP_TTS_OFFICEFRAME | XTP_TTS_NOSHADOW : 0));
		ModifyToolTipStyle(
			DWORD(XTP_TTS_OFFICE2007FRAME),
			DWORD(m_toolStyle == xtpToolTipResource ? XTP_TTS_OFFICE2007FRAME : 0));
		ModifyToolTipStyle(
			DWORD(XTP_TTS_OFFICE2013FRAME),
			DWORD(m_toolStyle == xtpToolTipOffice2013 ? XTP_TTS_OFFICE2013FRAME : 0));
	}
}
...
CWnd* CXTPToolTipContext::CreateToolTip(CWnd* pOwner)
{
	...
	case xtpToolTipOffice:
		pToolTip = new CXTPToolTipContextToolTip(this);
		break;

	case xtpToolTipOffice2013:
		pToolTip = new COffice2013ToolTip(this);
		break;

	case xtpToolTipRTF:
		pToolTip = new CRichEditToolTip(this);
		break;

	case xtpToolTipLuna:
		pToolTip = new CLunaToolTip(this);
		break;

	case xtpToolTipResource:
		pToolTip = new COfficeToolTip(this);
		break;

#ifndef _XTP_EXCLUDE_HTML
	case xtpToolTipHTML:
		pToolTip = new CHTMLToolTip(this);
		break;
#endif

	case xtpToolTipMarkup:
		pToolTip = new CMarkupToolTip(this);
		break;

	case xtpToolTipStandard:
	case xtpToolTipBalloon:
	default:
		pToolTip = new CStandardToolTip();
		break;
	}
	...
	if (NULL != pToolTip)
	{
		...
		{
			bCreated = static_cast<CXTPToolTipContextToolTip*>(pToolTip)->Create(pOwner,
				m_dwStyle | TTS_ALWAYSTIP | TTS_NOPREFIX);
		}
		else if (DYNAMIC_DOWNCAST(CToolTipCtrl, pToolTip))
		{
			bCreated = static_cast<CToolTipCtrl*>(pToolTip)->Create(pOwner, 
				m_dwStyle | TTS_ALWAYSTIP | TTS_NOPREFIX 
		...
		if (bTipInfoChanged || CRect(m_lastInfo.rect) != CRect(tiHit.rect))
		{
			...
			if (nHit != -1)
			{
				...
				if ((m_toolStyle != xtpToolTipRTF && m_toolStyle != xtpToolTipHTML && m_toolStyle != xtpToolTipMarkup) && ((m_dwComCtlVersion >= VERSION_IE5) || (m_toolStyle != xtpToolTipStandard)))
				{
					...
					{
						pToolTip->SendMessage(TTM_SETTITLE, static_cast<WPARAM>(m_nIconTitle), (LPARAM)(LPCTSTR)tc.lpszTitle);
...
BEGIN_DISPATCH_MAP(CXTPToolTipContext, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPToolTipContext, "ShowTitleAndDescription", 2, ShowTitleAndDescription, VT_EMPTY, VTS_BOOL VTS_I4)
	...
	DISP_PROPERTY_EX_ID(CXTPToolTipContext, "TipTextColor", 5, GetTipTextColor, SetTipTextColor, VT_COLOR)
	DISP_FUNCTION_ID(CXTPToolTipContext, "IsBalloonStyleSupported", 6, IsBalloonStyleSupported, VT_BOOL, VTS_NONE)
	DISP_FUNCTION_ID(CXTPToolTipContext, "SetMargin", 7, OleSetMargin, VT_EMPTY, VTS_I4 VTS_I4 VTS_I4 VTS_I4)
	DISP_PROPERTY_EX_ID(CXTPToolTipContext, "Font", DISPID_FONT, OleGetFont, OleSetFont, VT_DISPATCH)
	DISP_FUNCTION_ID(CXTPToolTipContext, "ShowImage", 8, OleShowImage, VT_EMPTY, VTS_BOOL VTS_I4)

	DISP_PROPERTY_EX_ID(CXTPToolTipContext, "ShowOfficeBorder", 9, OleGetShowOfficeBorder, OleSetShowOfficeBorder, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPToolTipContext, "ShowShadow", 10, OleGetShowShadow, OleSetShowShadow, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPToolTipContext, "ImageDPIScaling", 11, IsImageDPIScalingEnabled, EnableImageDPIScaling, VT_BOOL)
