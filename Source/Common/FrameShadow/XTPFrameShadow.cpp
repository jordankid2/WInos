...
static struct GdiplusCtx {
	GdiplusCtx() : Token(0), nInitCounter(0) {}
	ULONG_PTR Token;
	LONG nInitCounter;
	...
	, m_bBeingUpdated(FALSE)
{
	...
	if (1 == InterlockedIncrement(&g_gdiPlusCtx.nInitCounter))
	{
		g_gdiPlusCtx.StartupInput.SuppressBackgroundThread = TRUE;
		if (Gdiplus::Ok == GdiplusStartup(&g_gdiPlusCtx.Token, &g_gdiPlusCtx.StartupInput, &g_gdiPlusCtx.StartupOutput))
		{
			g_gdiPlusCtx.StartupOutput.NotificationHook(&g_gdiPlusCtx.Token);
		}
		else
		{
			g_gdiPlusCtx.Token = 0;
			InterlockedDecrement(&g_gdiPlusCtx.nInitCounter);
		}
	}
...
CXTPFrameShadow::~CXTPFrameShadow()
{
	...
	if (0 == InterlockedDecrement(&g_gdiPlusCtx.nInitCounter))
	{
		GdiplusShutdown(g_gdiPlusCtx.Token);
		g_gdiPlusCtx.Token = 0;
	}
}
	...
	BYTE bTransparency /*= 0x80*/)
{
	...
	if (IsShadowSupported())
	{
		...
		bSuccess = CWnd::CreateEx(exStyle, AfxRegisterWndClass(0),
			...
			CW_USEDEFAULT, CW_USEDEFAULT, m_hOwner, NULL);
		if (bSuccess)
		{
			m_bCreated = TRUE;
			m_crColor = crColor;
...
UINT CXTPFrameShadow::GetBlendingOptions(
...
void CXTPFrameShadow::SetBlendingOptions(
	UINT nCount,
	...
	const float* pPositions)
{
	if (0 < nCount)
	{
		...
		for (INT_PTR i = 0; i < XTPToIntPtrChecked(nCount); ++i)
		{
			...
			float fltCornerPosition = (0 < pPositions[i] && pPositions[i] < 1 ? 1 - pPositions[i] : pPositions[i]);
...
void CXTPFrameShadow::OnPaint()
{
	...
	m_pfnUpdateLayeredWindow(m_hWnd, dc, &ptWinPos, &clientSize,
		hdcMemory, &ptSrc, 0, &blendFunction, ULW_ALPHA);
...
void CXTPFrameShadow::OnFlagsChanged(
	CXTPFrameShadowManager* /*pManager*/,
	int nOldEffectiveFlags,
...
void CXTPFrameShadow::LoadDefaults()
{
	...
	if (0 < nCount)
	{
		...
		arrFactors.SetSize(nCount);
		arrPositions.SetSize(nCount);
...
void CXTPFrameShadow::ComputeData()
{
	...
	m_pPCD->startColor = Color(0xff, GetRValue(m_crColor), GetGValue(m_crColor), GetBValue(m_crColor));
	m_pPCD->endColor = Color(0, GetRValue(m_crColor), GetGValue(m_crColor), GetBValue(m_crColor));

	// Left side
	m_pPCD->leftRect = Rect(
		0, m_pPCD->size.cy,
		m_pPCD->size.cx, clientSize.cy - m_pPCD->size.cy * 2);
#ifdef _DEBUG
#pragma push_macro("new")
#undef new
#endif
	m_pPCD->leftBrush = new LinearGradientBrush(
		PointF(m_pPCD->leftRect.Width + BrushCorrection, 0),
		PointF(-BrushCorrection, 0.f),
...
#pragma pop_macro("new")
	...
	{
		m_pPCD->leftBrush->SetBlend(
			m_BlendingOptions.Factors.GetData(),
...
#undef new
#endif
	m_pPCD->topBrush = new LinearGradientBrush(
		PointF(0, m_pPCD->topRect.Height + BrushCorrection),
		PointF(0, -BrushCorrection),
		m_pPCD->startColor, m_pPCD->endColor);
#ifdef _DEBUG
#pragma pop_macro("new")
#endif
	...
	// Right side
	m_pPCD->rightRect = Rect(
		clientSize.cx - m_pPCD->size.cx, m_pPCD->size.cy,
		m_pPCD->size.cx, clientSize.cy - m_pPCD->size.cy * 2);
#ifdef _DEBUG
#pragma push_macro("new")
#undef new
	...
	m_pPCD->rightBrush = new LinearGradientBrush(
		PointF(m_pPCD->rightRect.X - BrushCorrection, 0),
		PointF(m_pPCD->rightRect.X + m_pPCD->rightRect.Width + BrushCorrection, 0),
		m_pPCD->startColor, m_pPCD->endColor);
#ifdef _DEBUG
#pragma pop_macro("new")
#endif
	...
	// Bottom side
	m_pPCD->bottomRect = Rect(
		m_pPCD->size.cx, clientSize.cy - m_pPCD->size.cy,
		clientSize.cx - m_pPCD->size.cx * 2, m_pPCD->size.cy);
#ifdef _DEBUG
#pragma push_macro("new")
#undef new
	...
	m_pPCD->bottomBrush = new LinearGradientBrush(
		PointF(0, m_pPCD->bottomRect.Y - BrushCorrection),
		PointF(0, m_pPCD->bottomRect.Y + m_pPCD->bottomRect.Height + BrushCorrection),
		m_pPCD->startColor, m_pPCD->endColor);
#ifdef _DEBUG
#pragma pop_macro("new")
#endif
	...
	// Background
	m_pPCD->backgroundRect = Rect(
		m_pPCD->size.cx, m_pPCD->size.cy,
		clientSize.cx - m_pPCD->size.cx * 2, clientSize.cy - m_pPCD->size.cy * 2);
#ifdef _DEBUG
#pragma push_macro("new")
#undef new
#endif
	m_pPCD->backgroundBrush = new LinearGradientBrush(
		Point(0, 0),
...
#pragma pop_macro("new")
	...
	{
		m_pPCD->backgroundBrush->SetBlend(
			m_BlendingOptions.BackgroundFactors.GetData(),
...
#pragma pop_macro("new")
	...
	{
		m_pPCD->leftTopBrush->SetBlend(
			m_BlendingOptions.CornerFactors.GetData(),
			m_BlendingOptions.CornerPositions.GetData(),
			(INT)m_BlendingOptions.CornerFactors.GetSize());
	}

	// Right top corner
	m_pPCD->rightTopRect = Rect(
		clientSize.cx - m_pPCD->size.cx * 2, 0,
		m_pPCD->size.cx * 2, m_pPCD->size.cy * 2);
...
#pragma pop_macro("new")
	...
	m_pPCD->rightTopBrush->SetCenterPoint(Point(
		m_pPCD->size.cx + m_pPCD->rightTopRect.X,
		m_pPCD->size.cy));
	m_pPCD->rightTopBrush->SetSurroundColors(&m_pPCD->endColor, &one);
	if (0 < m_BlendingOptions.CornerFactors.GetSize())
	{
		m_pPCD->rightTopBrush->SetBlend(
			m_BlendingOptions.CornerFactors.GetData(),
			m_BlendingOptions.CornerPositions.GetData(),
			(INT)m_BlendingOptions.CornerFactors.GetSize());
	}
...
#pragma pop_macro("new")
	...
	m_pPCD->rightBottomBrush->SetCenterPoint(Point(
		m_pPCD->size.cx + m_pPCD->rightBottomRect.X,
		m_pPCD->size.cy + m_pPCD->rightBottomRect.Y));
	m_pPCD->rightBottomBrush->SetSurroundColors(&m_pPCD->endColor, &one);
	if (0 < m_BlendingOptions.CornerFactors.GetSize())
	{
		m_pPCD->rightBottomBrush->SetBlend(
			m_BlendingOptions.CornerFactors.GetData(),
			m_BlendingOptions.CornerPositions.GetData(),
			(INT)m_BlendingOptions.CornerFactors.GetSize());
	}
	...
	// Left bottom corner
	m_pPCD->leftBottomRect = Rect(
		0, clientSize.cy - m_pPCD->size.cy * 2,
		m_pPCD->size.cx * 2, m_pPCD->size.cy * 2);
...
#pragma pop_macro("new")
	...
	m_pPCD->leftBottomBrush->SetCenterPoint(Point(m_pPCD->size.cx,
		m_pPCD->size.cy + m_pPCD->leftBottomRect.Y));
	m_pPCD->leftBottomBrush->SetSurroundColors(&m_pPCD->endColor, &one);
	if (0 < m_BlendingOptions.CornerFactors.GetSize())
	{
		m_pPCD->leftBottomBrush->SetBlend(
			m_BlendingOptions.CornerFactors.GetData(),
			m_BlendingOptions.CornerPositions.GetData(),
			(INT)m_BlendingOptions.CornerFactors.GetSize());
	}
...
void CXTPFrameShadow::Update(BOOL bReposition, BOOL bRedraw /*= TRUE*/)
{
	...
	{
		if (0 == m_bFreezeUpdatesCounter
			&& !m_bIsIconic
			&& ::IsWindowVisible(m_hOwner))
		{
			...
			if ((0 == (dwStyle & WS_MAXIMIZE)) && !m_bIsIconic)
			{
				if (bReposition)
				{
					...
					CSize szNewSize(
						rcWnd.Width() + XTPToIntChecked(m_nOuterSize) * 2,
						rcWnd.Height() + XTPToIntChecked(m_nOuterSize) * 2);

					UINT flags = SWP_NOREDRAW | SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOCOPYBITS;
					...
					::DeferWindowPos(hdwp, m_hOwner, m_hWnd, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE
						| SWP_NOOWNERZORDER | SWP_NOACTIVATE | SWP_NOREDRAW);
...
void CXTPFrameShadow::Drag(HitTestResult nHitTest, CPoint ptOffset, CSize szMinSize)
{
	...
	if (bProceed)
	{
		::SetWindowPos(m_hOwner, HWND_TOP, rcWnd.left, rcWnd.top,
			rcWnd.Width(), rcWnd.Height(), SWP_NOZORDER | SWP_DEFERERASE);
...
{
	ASSERT(::IsWindow(m_hOwner));
	CXTPWindowRect rcClip(m_hOwner);
	...
	if (XTPSystemVersion()->IsWinVistaOrGreater())
	{
		...
		{
			HMODULE hDwmApi = ::GetModuleHandle(_T("dwmapi.dll"));
			if (NULL != hDwmApi)
			{
				pfnDwmGetWindowAttribute = reinterpret_cast<PFNDwmGetWindowAttribute>(
...
BEGIN_DISPATCH_MAP(CXTPFrameShadow, CCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPFrameShadow, "OffsetX", 3, OleGetOffsetX, OleSetOffsetX, VT_XSIZE_PIXELS)
	DISP_PROPERTY_EX_ID(CXTPFrameShadow, "OffsetY", 4, OleGetOffsetY, OleSetOffsetY, VT_YSIZE_PIXELS)
	DISP_PROPERTY_EX_ID(CXTPFrameShadow, "Color", 5, OleGetColor, OleSetColor, VT_COLOR)
	DISP_PROPERTY_EX_ID(CXTPFrameShadow, "Transparency", 6, OleGetTransparency, OleSetTransparency, VT_I2)
	DISP_PROPERTY_EX_ID(CXTPFrameShadow, "ClipOwner", 15, OleGetClipOwner, OleSetClipOwner, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPFrameShadow, "BlendingFactors", 7, OleGetBlendingFactors, SetNotSupported, VT_VARIANT)
	DISP_PROPERTY_EX_ID(CXTPFrameShadow, "BlendingPositions", 8, OleGetBlendingPositions, SetNotSupported, VT_VARIANT)
	...
	DISP_FUNCTION_ID(CXTPFrameShadow, "SetBlendingOptions", 14, OleSetBlendingOptions, VT_EMPTY, VTS_VARIANT VTS_VARIANT)
END_DISPATCH_MAP()
