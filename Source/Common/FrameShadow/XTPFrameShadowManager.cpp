...
public:
		...
		, m_pfnWTSUnRegisterSessionNotification(NULL)
	{
		if (XTPSystemVersion()->IsWinXPOrGreater())
		{
			...
			if (NULL != m_hWtsapi32)
			{
				m_pfnWTSRegisterSessionNotification = XTPToFunctionPtr<PFNWTSRegisterSessionNotification>(
					::GetProcAddress(m_hWtsapi32, "WTSRegisterSessionNotification"));
				m_pfnWTSUnRegisterSessionNotification = XTPToFunctionPtr<PFNWTSUnRegisterSessionNotification>(
...
public:
	void Enable()
	{
		if (NULL != m_pfnWTSRegisterSessionNotification
			&& NULL != m_pfnWTSUnRegisterSessionNotification)
		{
			if (!::IsWindow(*this))
			{
				HWND hWnd = ::CreateWindowEx(0, AfxRegisterWndClass(0), _T("CXTPFrameShadowManager::CWtsEventListener"),
					WS_OVERLAPPED, -100, -100, -50, -50, NULL, 0, NULL, NULL);
				if (NULL != hWnd)
				{
					Attach(hWnd);
				}
...
void CXTPFrameShadowManager::SetFlags(int nFlags)
{
	...
	{
		m_nEffectiveFlags = xtpFrameShadowsDisabled 
			| xtpFrameShadowsInvisible 
	...
	BYTE bTransparency /*= 0*/)
{
	...
	if (IsShadowSupported() && 0 == (GetFlags() & xtpFrameShadowsDisabled))
	{
		...
		if (NULL == pShadow)
		{
			...
			if (pShadow->Create(
				(COLORREF)-1 != crColor ? crColor : m_crDefaultColor,
				...
				0 < bTransparency ? bTransparency : m_bDefaultTransparency))
			{
				...
				{
					pShadow->SetBlendingOptions(
						(UINT)m_DefaultBlendingOptions.Factors.GetSize(),
...
UINT CXTPFrameShadowManager::GetDefaultBlendingOptions(
	OUT OPTIONAL float* pFactors,
	OUT OPTIONAL float* pPositions) const
{
	...
	if (0 < nCount)
	{
		...
		if (NULL != pPositions)
		{
			memcpy(pPositions, m_DefaultBlendingOptions.Positions.GetData(), nCount * sizeof(float));
		}
	}
...
void CXTPFrameShadowManager::SetDefaultBlendingOptions(
	UINT nCount,
	const float* pFactors,
...
void CXTPFrameShadowManager::SetGlobalBlendingOptions(
	UINT nCount,
	const float* pFactors,
	const float* pPositions)
{
...
void CXTPFrameShadowManager::SetShadowThreadMessageHook(
...
BEGIN_DISPATCH_MAP(CXTPFrameShadowManager, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPFrameShadowManager, "DefaultOuterSize", 2, OleGetDefaultOuterSize, OleSetDefaultOuterSize, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPFrameShadowManager, "DefaultInnerSize", 3, OleGetDefaultInnerSize, OleSetDefaultInnerSize, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPFrameShadowManager, "DefaultOffsetX", 4, OleGetDefaultOffsetX, OleSetDefaultOffsetX, VT_XSIZE_PIXELS)
	DISP_PROPERTY_EX_ID(CXTPFrameShadowManager, "DefaultOffsetY", 5, OleGetDefaultOffsetY, OleSetDefaultOffsetY, VT_YSIZE_PIXELS)
	DISP_PROPERTY_EX_ID(CXTPFrameShadowManager, "DefaultColor", 6, OleGetDefaultColor, OleSetDefaultColor, VT_COLOR)
	DISP_PROPERTY_EX_ID(CXTPFrameShadowManager, "DefaultTransparency", 7, OleGetDefaultTransparency, OleSetDefaultTransparency, VT_I2)
	DISP_PROPERTY_EX_ID(CXTPFrameShadowManager, "DefaultClipOwner", 24, OleGetDefaultClipOwner, OleSetDefaultClipOwner, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPFrameShadowManager, "DefaultBlendingFactors", 8, OleGetDefaultBlendingFactors, SetNotSupported, VT_VARIANT)
	DISP_PROPERTY_EX_ID(CXTPFrameShadowManager, "DefaultBlendingPositions", 9, OleGetDefaultBlendingPositions, SetNotSupported, VT_VARIANT)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "IsShadowSupported", 10, IsShadowSupported, VT_BOOL, VTS_NONE)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "EnableShadow", 11, OleEnableShadow, VT_DISPATCH, VTS_HANDLE VTS_COLOR VTS_I4 VTS_I4 VTS_I4 VTS_I4 VTS_I2)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "DisableShadow", 12, OleDisableShadow, VT_EMPTY, VTS_HANDLE)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "IsShadowEnabled", 13, OleIsShadowEnabled, VT_BOOL, VTS_NONE)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "GetShadow", 14, OleGetShadow, VT_DISPATCH, VTS_HANDLE)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "FreezeUpdates", 15, OleFreezeUpdates, VT_EMPTY, VTS_BOOL)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "SetGlobalOuterSize", 16, OleSetGlobalOuterSize, VT_EMPTY, VTS_I4)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "SetGlobalInnerSize", 17, OleSetGlobalInnerSize, VT_EMPTY, VTS_I4)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "SetGlobalOffset", 18, OleSetGlobalOffset, VT_EMPTY, VTS_XSIZE_PIXELS VTS_YSIZE_PIXELS)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "SetGlobalColor", 19, OleSetGlobalColor, VT_EMPTY, VTS_COLOR)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "SetGlobalTransparency", 20, OleSetGlobalTransparency, VT_EMPTY, VTS_I2)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "SetGlobalClipOwner", 25, OleSetGlobalClipOwner, VT_EMPTY, VTS_BOOL)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "SetDefaultBlendingOptions", 21, OleSetDefaultBlendingOptions, VT_EMPTY, VTS_VARIANT VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "SetGlobalBlendingOptions", 22, OleSetGlobalBlendingOptions, VT_EMPTY, VTS_VARIANT VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPFrameShadowManager, "FrameShadows", 23, OleGetFrameShadows, VT_DISPATCH, VTS_NONE)
END_DISPATCH_MAP()
...
void CXTPFrameShadowManager::OleSetDefaultBlendingOptions(const VARIANT& factors, const VARIANT& positions)
{
...
void CXTPFrameShadowManager::OleSetGlobalBlendingOptions(const VARIANT& factors, const VARIANT& positions)
{
	if ((factors.vt & VT_ARRAY) == 0)
		AfxThrowOleException(E_INVALIDARG);
	if ((factors.vt & VT_R4) == 0)
...
LPDISPATCH CXTPFrameShadowManager::OleEnableShadow(OLE_HANDLE hWndOLE, OLE_HANDLE color, long outerSize, long innerSize, OLE_XSIZE_PIXELS offsetX, OLE_YSIZE_PIXELS offsetY, short transparancy)
{
	CPoint p(offsetX, offsetY);
	HWND hWnd = (HWND)hWndOLE;
...
{
	CXTPFrameShadow* shadow = XTPFrameShadowManager()->GetShadow((HWND)hWnd);
	if (shadow == NULL)
		return NULL;
