...
struct IXTPFrameShadowManagerEvents
{
	...
	//===========================================================================
	virtual void OnFrameShadowEnabled(
	...
	//		pShadow - Shadow pointer
	//===========================================================================
	virtual void OnFrameShadowDisabled(
		CXTPFrameShadowManager* pManager,
		HWND hwndOwner,
		CXTPFrameShadow* pShadow)
	{
	...
	, public CXTPObservable<CXTPFrameShadowManager, IXTPFrameShadowManagerEvents>
{
	...
	friend _XTP_EXT_CLASS CXTPFrameShadowManager* AFX_CDECL XTPFrameShadowManager();
...
public:
	...
	//     bTransparency - Optional. Frame shadow transparency (0 - fully transparent, 255 - fully opaque).
	...
	CXTPFrameShadow* EnableShadow(
		...
		UINT nInnerSize = 0,
		CPoint ptOffset = CPoint(0, 0),
		BYTE bTransparency = 0); // <combine CXTPFrameShadowManager::EnableShadow@CWnd*@COLORREF@UINT@UINT@CPoint@BYTE>
	...
	CXTPFrameShadow* GetShadow(CWnd* pWnd) const; // <combine CXTPFrameShadowManager::GetShadow@HWND@const>
	...
	//     if a default value is used in EnableShadow.
	// Parameters:
	//     bTransparency - A new default value to be used. 0 means a fully transparent shadow, 255 means fully opaque.
	// See Also:
	...
	//-----------------------------------------------------------------------
	UINT GetDefaultBlendingOptions(
	...
	//-----------------------------------------------------------------------
	void SetDefaultBlendingOptions(
		UINT nCount,
	...
	void SetDefaultClipOwner(BOOL bClipOwner); // <combine CXTPFrameShadowManager::GetDefaultClipOwner>
...
private:
	...
	afx_msg LPDISPATCH OleEnableShadow(OLE_HANDLE hWnd, OLE_HANDLE color, long outerSize, long innerSize, OLE_XSIZE_PIXELS offsetX = 0, OLE_YSIZE_PIXELS offsetY = 0, short transparancy = 0);
