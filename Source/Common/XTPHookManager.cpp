...
int CXTPHookManagerHookAble::OnHookMessage(HWND hWnd, UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult)
{
...
int CXTPHookManagerHookAble::OnPostHookMessage(HWND hWnd, UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult)
{
	// Do nothing by default
	UNREFERENCED_PARAMETER(hWnd);
	UNREFERENCED_PARAMETER(nMessage);
...
public:
	...
	BOOL OnPostHookMessage(HWND hWnd, UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult);
...
BOOL CXTPHookManager::CHookSink::OnPostHookMessage(HWND hWnd, UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult)
{
	ASSERT(NULL == m_hHook);
...
CXTPHookManager::CXTPHookManager() : m_pGuard(new CXTPSimpleCriticalSection)
{
	...
	if (0 <= nCode)
	{
		...
		if (WH_MIN <= idHook && idHook <= WH_MAX)
		{
			...
			if (m_WindowsHooks[WH_INDEX(idHook)].Lookup(GetCurrentThreadId(), pSink))
			{
				...
				SAFE_MANAGE_STATE(pSink->m_pModuleState);
#endif
				pSink->OnHookMessage(NULL, XTPToUIntChecked(nCode), wParam, lParam, lResult);
			}
		}
	}
	else
...
BOOL CXTPHookManager::SetHook(
	int idHook,
	CXTPHookManagerHookAble* pHook,
	DWORD dwThreadId /*= 0*/)
{
	ASSERT(NULL != pHook);
	ASSERT_IDHOOK(idHook);
...
CXTPShadowManager::CXTPShadowManager()
{
	...
	if (hLib)
	{
		m_pfnUpdateLayeredWindow = XTPToFunctionPtr<LPFNUPDATELAYEREDWINDOW>(::GetProcAddress(hLib, "UpdateLayeredWindow"));
	}
...
BOOL CXTPShadowManager::CShadowWnd::Create(BOOL bHoriz, CRect rcWindow)
{
	...
	if (m_hWnd == 0)
	{
		...
		UINT nClassStyle = m_pShadowsManager->m_bUseSystemSaveBitsStyle ? UINT(CS_SAVEBITS | CS_OWNDC) : 0;
...
BEGIN_MESSAGE_MAP(CXTPShadowManager::CShadowWnd, CWnd)
	...
	//   error C2327: 'CShadowWnd::OnEraseBkgnd' : member from enclosing class is not a type name, static, or enumerator
	//   error C2065: 'OnEraseBkgnd' : undeclared identifier
	//   error C2440: 'type cast' : cannot convert from 'int *' to 'int (__thiscall CWnd::*)(class CDC *)'
...
{
	return (XTPToUIntChecked(nRed * dFactor) << 16)
		+ (XTPToUIntChecked(nGreen * dFactor) << 8)
