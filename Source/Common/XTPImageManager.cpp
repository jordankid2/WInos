...
IMPLEMENT_DYNAMIC(CXTPImageManager, CXTPCmdTarget);
...
BOOL CXTPImageManager::BlendImages(HBITMAP hbmSrc1, BOOL bRTL1, HBITMAP hbmSrc2, BOOL bRTL2, HBITMAP hbmDst) const
{
	...
	dwWidthBytes = XTPToUIntChecked(bmDst.bmWidthBytes);
...
{
	ImageList_DrawEx(m_hImageList, XTPToIntChecked(pIcon->GetID() - m_nBaseCommand), pDC->GetSafeHdc(),
...
IMPLEMENT_DYNAMIC(CXTPImageManagerIcon, CXTPCmdTarget)
...
HBITMAP AFX_CDECL CXTPImageManagerIcon::LoadBitmapFromFile(LPCTSTR lpszFileName, BOOL* lbAlphaBitmap)
{
	...
	HBITMAP hBmp = (HBITMAP)LoadImage(0, lpszFileName, UINT(IMAGE_BITMAP), 0, 0,
		UINT(LR_DEFAULTSIZE | (bAlphaBitmap ? LR_CREATEDIBSECTION : 0) | LR_LOADFROMFILE));
...
HBITMAP CXTPImageManagerIcon::LoadAlphaBitmap(HMODULE hModule, LPCTSTR lpszResource)
{
		...
		SizeofResource(hModule, hResource) >= int(biSizeImage + sizeof(BITMAPINFOHEADER)))
	{
		...
		pBitmapInfo->bmiHeader.biSizeImage = XTPToUIntChecked(biSizeImage);
...
HBITMAP CXTPImageManagerIcon::LoadAlphaIcon(LPCTSTR pszFileName, int nWidth)
{
	...
	for (int i = 0; i < idh.idCount; i++)
	{
		...
		{
			if (!SetFilePointer(hFile, XTPToLongChecked(pEntries[i].dwImageOffset), NULL, FILE_BEGIN))
				continue;
			...
			pIconImage->icHeader.biSizeImage = XTPToUIntChecked(pIconImage->icHeader.biHeight * pIconImage->icHeader.biWidth * 4);
...
{
	DWORD dwSize = XTPToUInt(MAKELONG(szIcon.cx, szIcon.cy));
...
HICON CXTPImageManagerIconHandle::GetDpiScaled(int nDpi /*= 0*/) const
{
	HICON hIconScaled = NULL;

	if (NULL != m_hIcon)
	{
		CSize szSize = GetExtent();
		int nWidth = (0 != nDpi ? XTPDpiHelper()->ScaleX(szSize.cx, nDpi) : XTP_DPI_X(szSize.cx));
		hIconScaled = CXTPImageManagerIcon::ScaleToFit(m_hIcon, szSize, nWidth);
	}

	return hIconScaled;
}

BOOL CXTPImageManagerIconHandle::CreateIconFromResource(
	LPCTSTR lpszResourceName,
	CSize szIcon,
	BOOL bGroupResource /*= TRUE*/,
	XTPIconSizeMatching nSizeMatching /*= xtpIconSizeExact*/)
{
	if (bGroupResource)
	{
		...
		if (hInst)
			return CreateIconFromResource(hInst, lpszResourceName, szIcon, TRUE, nSizeMatching);
	}
	...
	XTPIconSizeMatching nSizeMatching /*= xtpIconSizeExact*/)
{
	...
	if (hRsrc != NULL && bGroupResource)
	{
		...
		WORD nMatchedID = 0;
		CSize szMatched;
		switch (nSizeMatching)
		{
		case xtpIconSizeAtLeast:
			szMatched = CSize(INT_MAX, INT_MAX);
			break;
		case xtpIconSizeAtMost:
			szMatched = CSize(INT_MIN, INT_MIN);
			break;
		case xtpIconSizeExact:
			// no relevant
			break;
		}
		...
		{
			CSize sz(lpGrpIconDir->idEntries[i].bWidth, lpGrpIconDir->idEntries[i].bHeight);
			switch (nSizeMatching)
			{
			case xtpIconSizeExact:
				if (sz.cx == szIcon.cx && sz.cy == szIcon.cy)
				{
				...
				break;
			case xtpIconSizeAtLeast:
				if (szIcon.cx <= sz.cx && szIcon.cy <= sz.cy
					&& sz.cx < szMatched.cx && sz.cy < szMatched.cy)
				{
					nMatchedID = lpGrpIconDir->idEntries[i].nID;
					szMatched = sz;
					bSizeMatched = TRUE;
				}
				break;
			case xtpIconSizeAtMost:
				if (sz.cx <= szIcon.cx && sz.cy <= szIcon.cy
					&& szMatched.cx < sz.cx && szMatched.cy < sz.cy)
				{
					nMatchedID = lpGrpIconDir->idEntries[i].nID;
					szMatched = sz;
					bSizeMatched = TRUE;
		...
		if (bSizeMatched)
		{
			if (CreateIconFromResource(hInst, MAKEINTRESOURCE(nMatchedID), szMatched, FALSE))
				return TRUE;
		}
	...
	if (lpIconImage->icHeader.biBitCount == 32)
	{
		...
		*pBitmapInfo = lpIconImage->icHeader;
		...
		pBitmapInfo->biSizeImage = XTPToUIntChecked(pBitmapInfo->biHeight * pBitmapInfo->biWidth * 4);
...
BOOL CXTPImageManagerIcon::GetBitmapBits(CDC& dcSrc, HBITMAP hBitmap, PBITMAPINFO& pBitmapInfo, LPVOID& pBits, UINT& nSize)
{
	...
	nSize = XTPToUIntChecked(bmi.bmiHeader.biHeight * bmi.bmiHeader.biWidth * 4);
	...
	if (GetDIBits(dcSrc, hBitmap, 0, XTPToUIntChecked(bmi.bmiHeader.biHeight),
		pBits, pBitmapInfo, UINT(DIB_RGB_COLORS)) == NULL)
	{
		FREE(pBits);
		FREE(pBitmapInfo);
		return FALSE;
...
BOOL CXTPImageManagerIcon::SetIcon(
	UINT nIDResourceIcon,
	int nWidth,
	int nHeight,
	XTPIconSizeMatching nSizeMatching /*= xtpIconSizeExact*/)
{
	...
	if (!m_hIcon.CreateIconFromResource(MAKEINTRESOURCE(nIDResourceIcon), 
		CSize(nWidth, nHeight), TRUE, nSizeMatching))
		return FALSE;

	CSize szIcon = m_hIcon.GetExtent();
...
void CXTPImageManagerIcon::CreateFadedIcon()
{
	...
	else if (hFaded.PreMultiply())
	{
		...
		int nCount = szIcon.cx * szIcon.cy;
...
BOOL CXTPImageManagerIcon::GetDIBBitmap(HBITMAP hBitmap, PBYTE& pBits, UINT& nBitsSize, PBITMAPINFO& pBitmapInfo, UINT& nBitmapInfoSize)
{
	...
	TRY
	{
		...
		if (GetDIBits(dcSrc, hBitmap, 0, XTPToUIntChecked(bmi.bmiHeader.biHeight), pBits,
			pBitmapInfo, UINT(DIB_RGB_COLORS)) == NULL)
			AfxThrowMemoryException();
	}
	CATCH (CMemoryException, e)
	{
...
void CXTPImageManagerIcon::DrawMono(CDC* pDC, CPoint pt, CXTPImageManagerIconHandle& hIcon, CSize szIcon, COLORREF clrBrush)
{
	...
	CBitmap bmp;
	if(bmp.CreateCompatibleBitmap(pDC, szIcon.cx, szIcon.cy))
	{
...
CXTPImageManagerIcon* CXTPImageManagerIconSet::CreateIcon(UINT nWidth)
{
	...
	if (m_mapIcons.Lookup(nWidth, pIcon))
	{
		...
		ASSERT(pIcon->m_nWidth == XTPToIntChecked(nWidth));
	}
	else
	{
		pIcon = new CXTPImageManagerIcon(m_nID, XTPToIntChecked(nWidth), 15, this);
...
CXTPImageManagerIcon* CXTPImageManagerIconSet::GetIcon(UINT nWidth, BOOL bScaled/* = TRUE*/)
{
...
{
	return &CXTPSingleton<CXTPImageManager>::Instance();
}
...
BOOL CXTPImageManager::SetIcon(const CXTPImageManagerIconHandle& hIcon, UINT nIDCommand, CSize szIcon, XTPImageState imageState)
{
	...
	if (m_mapImages.Lookup(nIDCommand, pIconSet))
	{
		...
		{
		case xtpImageNormal:
			pIconSet->SetIcon(hIcon, XTPToUIntChecked(szIcon.cx));
			break;
		case xtpImageDisabled:
			pIconSet->SetDisabledIcon(hIcon, XTPToUIntChecked(szIcon.cx));
			break;
		case xtpImageHot:
			pIconSet->SetHotIcon(hIcon, XTPToUIntChecked(szIcon.cx));
			break;
		case xtpImageChecked:
			pIconSet->SetCheckedIcon(hIcon, XTPToUIntChecked(szIcon.cx));
			break;
		case xtpImagePressed:
			pIconSet->SetPressedIcon(hIcon, XTPToUIntChecked(szIcon.cx));
			break;
		}
	}
...
BOOL CXTPImageManager::SplitBitmap(HBITMAP hbmSource, int nCount, HBITMAP* pDest) const
{
	...
	TRY
	{
		...
		pBits = (PINT)malloc(XTPToSizeT(nSize));
		if (pBits == NULL)
			AfxThrowMemoryException();
		...
		if (GetDIBits(dcSrc, hbmSource, 0, XTPToUIntChecked(bmi.bmiHeader.biHeight), pBits,
			pBitmapInfo, UINT(DIB_RGB_COLORS)) == NULL)
			AfxThrowMemoryException();
		...
		PINT* pDestBits = new PINT[XTPToUIntChecked(nCount)];
		for (i = 0; i < nCount; i++)
		{
...
BOOL CXTPImageManager::SetIcons(CBitmap& bmpIcons, UINT* pCommands, int nCount, CSize szIcon, XTPImageState imageState, BOOL bAlpha)
{
	...
	if (bAlpha)
	{
		...
		HBITMAP* pDestBitmap = new HBITMAP[XTPToUIntChecked(nIconCount)];
...
BOOL CXTPImageManager::SetImageList(HIMAGELIST hImageList, int nBaseCommand, BOOL bDestoryImageList)
{
	...
	pImageList->m_nBaseCommand = XTPToUIntChecked(nBaseCommand);
...
BOOL CXTPImageManager::SetImageList(UINT nBitmap, int cx, int nBaseCommand, COLORREF clrMask)
{
	...
	HIMAGELIST hImageList = ImageList_Create(cx, bmpInfo.bmHeight, 
		UINT((bAlpha ? ILC_COLOR32 : ILC_COLOR24) |
...
BOOL CXTPImageManager::IsPrimaryImageExists(UINT nCommand, int nWidth) const
{
	...
	if (pIconSet)
	{
		CXTPImageManagerIcon* pIcon = pIconSet->GetIcon(XTPToUIntChecked(nWidth), FALSE);
		if (pIcon)
			return TRUE;
	}
...
CXTPImageManagerIcon* CXTPImageManager::GetImage(UINT nCommand, int nWidth) const
{
	...
	if (pIconSet)
	{
		return pIconSet->GetIcon(XTPToUIntChecked(nWidth));
	}
...
//this assertion kill VB6
	...
	else
	{
		...
		while (posEnum)
		{
			...
			long nWidth = 16;
			PX_Long(&secIcon, _T("Width"), nWidth);

			CXTPImageManagerIcon* pIcon = new CXTPImageManagerIcon(nCommand, nWidth, 15, pIconSet);
			PX_Serialize(&secIcon, _T("Data"), pIcon);

			pIconSet->m_mapIcons.SetAt(XTPToUIntChecked(nWidth), pIcon);
		}
	}
#endif
}
...
void CXTPImageManager::AddIcons(CXTPImageManagerIconSet* pIconSetAdd)
{
	...
	while (pos)
	{
		...
		{
			pIcon  = new CXTPImageManagerIcon(nId, XTPToIntChecked(nWidth), pIconAdd->GetHeight());
		...
		int nBlackAndWhiteContrast)
{
	...
	DWORD dwWidthBytes = XTPToUInt(bmBitmap.bmWidthBytes);
...
CXTPImageManager* AFX_CDECL CXTPImageManager::FromDispatch(LPDISPATCH pDisp)
{
	CXTPImageManager* pInst = NULL;
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPImageManager, CXTPImageManager::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
...
BEGIN_DISPATCH_MAP(CXTPImageManagerIcon, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPImageManagerIcon, CXTPCmdTarget)
	INTERFACE_PART(CXTPImageManagerIcon, XTPDIID_ImageManagerIcon, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPImageManagerIcon, XTPDIID_ImageManagerIcon)
...
BEGIN_DISPATCH_MAP(CXTPImageManager, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPImageManager, CXTPCmdTarget)
	INTERFACE_PART(CXTPImageManager, XTPDIID_IImageManagerIcons, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPImageManager, XTPDIID_IImageManagerIcons)
...
BEGIN_DISPATCH_MAP(CXTPImageManagerIconSet, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPImageManagerIconSet, CXTPCmdTarget)
INTERFACE_PART(CXTPImageManagerIconSet, XTPDIID_ImageManagerIconSet, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPImageManagerIconSet, XTPDIID_ImageManagerIconSet)
...
BOOL ScaleBitmap(CBitmap *src, CBitmap *dst)
{
	ASSERT_VALID(src);
	ASSERT_VALID(dst);
