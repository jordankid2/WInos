...
#else
	#if (_MSC_VER > 1310) // VS2005
		_tsplitpath_s(szFullPath, szDrive, size_t(szDrive ? _MAX_DRIVE : 0),
			szDir, size_t(szDir ? _MAX_DIR : 0), szFileName,
			size_t(szFileName ? _MAX_FNAME : 0), szExt,
			size_t(szExt ? _MAX_EXT : 0));
	#else
