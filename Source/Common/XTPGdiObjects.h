...
//// Enable this in Debug mode and re-build ToolkitPro only for tracking unauthorized cached object changes.
...
#define XTP_DEPRECATE_GDI_MEMBER_IN_FAVOR_OF(memberName, inFavorOf) \
	_XTP_DEPRECATE("The member '" #memberName "' has been deprecated, using it may lead to undefined behavior. " \
		"Use '" #inFavorOf "' instead. You can disable this message by defining XTP_DISABLE_DEPRECATE_GDI_MEMBER globally.")
#else
#define XTP_DEPRECATE_GDI_MEMBER_IN_FAVOR_OF(what)
#endif

#define XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED_(oldClass, oldClassRef, oldClassCRef, oldMemberName, cachedMemberName, handleGetterName) \
	XTP_DEPRECATE_GDI_MEMBER_IN_FAVOR_OF(oldMemberName, cachedMemberName) \
	AFX_INLINE oldClassRef handleGetterName() { return *cachedMemberName.GetNativeGdiObject();  } \
	XTP_DEPRECATE_GDI_MEMBER_IN_FAVOR_OF(oldMemberName, cachedMemberName) \
	AFX_INLINE oldClassCRef handleGetterName() const { return *cachedMemberName.GetNativeGdiObject(); } \
	__declspec(property(get = handleGetterName)) oldClassRef oldMemberName;

#define XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(oldClass, oldMemberName, cachedMemberName, handleGetterName) \
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED_(oldClass, oldClass&, const oldClass&, oldMemberName, cachedMemberName, handleGetterName)

template<class GdiObject, class XtpObject> class CXTPGdiObjectPtrProxy;
class CXTPCachedGdiObjectReferenceBase;
template<class Descriptor, class GdiObject, class XtpCacheClass> class CXTPCachedGdiObjectReference;
class CXTPFont;
class CXTPGdiObject;
...
#pragma pop_macro("GetObject")
	...
	BOOL IsHandleOwnershipGuaranteed() const; // <combine CXTPGdiObject::SetHandleOwnershipNotGuaranteed>
...
public:
	...
	BOOL CreateDIBPatternBrush(const void* lpPackedDIB, UINT nUsage); // <combine CXTPBrush::CreateDIBPatternBrush@HGLOBAL@UINT>
...
public:
	...
	CXTPPen(int nPenStyle, int nWidth, const LOGBRUSH* pLogBrush,
		int nStyleCount = 0, const DWORD* lpStyle = NULL); // <combine CXTPPen::CXTPPen@int@int@COLORREF>
	...
	BOOL CreatePen(int nPenStyle, int nWidth, const LOGBRUSH* pLogBrush,
		int nStyleCount = 0, const DWORD* lpStyle = NULL); // <combine CXTPPen::CreatePen@int@int@COLORREF>
