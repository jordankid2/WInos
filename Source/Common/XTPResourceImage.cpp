...
BOOL CXTPResourceImage::ConvertToBits(HBITMAP hBitmap)
{
	...
	if (m_bAlpha)
	{
		...
		if (GetDIBits(dcSrc, hBitmap, 0, XTPToUIntChecked(bmi.bmiHeader.biHeight),
			pBits, pBitmapInfo, DIB_RGB_COLORS) == NULL)
		{
			free(pBits);
			free(pBitmapInfo);
			return FALSE;
...
void CXTPResourceImage::DrawImage(CDC* pDC, const CRect& rcDest, CRect rcSrc, CRect rcSizingMargins)
{
	...
	SIZE szLayerSize;
	HBITMAP hbmpLayer = m_pImages->AllocateLayerBitmap(
		XTPToUIntChecked(m_szExtent.cx), XTPToUIntChecked(m_szExtent.cy),
...
COLORREF CXTPResourceImages::GetImageColor(LPCTSTR lpszSectionName, LPCTSTR lpszKeyName, COLORREF clrDefault) const
{
	...
	int r = 0, g = 0, b = 0;
	if (3 != SCANF_S(strValue, _T("%i %i %i"), &r, &g, &b))
	{
		r = 0, g = 0, b = 0;
	}
...
CRect CXTPResourceImages::GetImageRect(LPCTSTR lpszSectionName, LPCTSTR lpszKeyName, CRect rcDefault) const
{
	...
	if (4 != SCANF_S(strValue, _T("%i %i %i %i"), &l, &t, &r, &b))
	{
		l = 0, r = 0, t = 0, b = 0;
	}
...
CSize CXTPResourceImages::GetImageSize(LPCTSTR lpszSectionName, LPCTSTR lpszKeyName, CSize szDefault) const
{
	CString strValue = GetImageValue(lpszSectionName, lpszKeyName);
	if (strValue.IsEmpty())
		return szDefault;
	...
	if (2 != SCANF_S(strValue, _T("%i %i"), &cx, &cy))
	{
		cx = 0, cy = 0;
	}
...
CString CXTPResourceImages::GetImageString(LPCTSTR lpszSectionName, LPCTSTR lpszKeyName, LPCTSTR lpszDef /*=NULL*/) const
{
	CString strValue = GetImageValue(lpszSectionName, lpszKeyName);
	if (strValue.IsEmpty())
		return lpszDef;
...
HBITMAP CXTPResourceImages::AllocateLayerBitmap(
	UINT cx,
	UINT cy,
	LPDWORD* lpBits /*= NULL*/,
	SIZE* pSize /*= NULL*/)
{
	...
	if (NULL == m_hbmLayer)
	{
		...
		m_hbmLayer = CXTPImageManager::Create32BPPDIBSection(NULL, 
			XTPToIntChecked(size), XTPToIntChecked(size), (LPBYTE*)&m_pLayerBits);
