...
AFX_INLINE HRESULT CXTPActiveScriptObjectContext::Invoke(
	VARIANT* pvtResult /*= NULL*/)
{
...
template<class A1>
AFX_INLINE HRESULT CXTPActiveScriptObjectContext::Invoke(
	A1 arg1, VARIANT* pvtResult /*= NULL*/)
{
	CComVariant va(arg1);
...
public:
	...
	//      engine for. Standard ProgIDs are JScript and VBScript. More
	//      language providers can be installed if necessary.
	//  pSite - An optional pointer to engine site interface that receieves
	//      notifications and requests from scripting engine.
...
private:
	...
	static void CALLBACK TimeoutProc(
		HWND hWnd,
		UINT uMsg,
		UINT_PTR idEvent,
		DWORD dwTime);
