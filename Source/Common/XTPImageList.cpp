...
BOOL CXTPImageList::Create(CXTPImageList& imagelist1, int nImage1, CXTPImageList& imagelist2, int nImage2, int dx, int dy)
{
...
BOOL CXTPImageList::Create(CBitmap* bitmap, int cx, int nGrow, COLORREF crMask)
{
	...
	if (bitmap)
	{
		...
		if (Resize(bitmap, &newBitmap))
		{
			...
			{
				HIMAGELIST imageList = ImageList_Create(XTP_DPI_X(cx), info.bmHeight, ILC_MASK | ILC_COLOR8, 1, nGrow);
				if (imageList)
				{
		...
		if (srcDC.CreateCompatibleDC(NULL) && destDC.CreateCompatibleDC(NULL))
		{
			...
			if (src->GetBitmap(&bmpInfo))
			{
				...
				if (dst->CreateCompatibleBitmap(&srcDC, width, height))
				{
					...
					if (destDC.StretchBlt(0, 0, width, height, &srcDC, 0, 0, bmpInfo.bmWidth - deltaWidth, bmpInfo.bmHeight, SRCCOPY))
					{
						bRet = TRUE;
					}
