...
void CXTPScrollBarThemeWindowsClassic::DrawButtons(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsV* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	...
	::DrawFrameControl(pDC->GetSafeHdc(), &pMetrics->rcArrowUp, XTPToUInt(DFC_SCROLL), 
		XTPToUInt(DFCS_SCROLLUP | (!pMetrics->bEnabled ? DFCS_INACTIVE : 0) | (nHtPressed == XTP_HTSCROLLUP ? DFCS_PUSHED : 0)));
	::DrawFrameControl(pDC->GetSafeHdc(), &pMetrics->rcArrowDown, XTPToUInt(DFC_SCROLL),
		XTPToUInt(DFCS_SCROLLDOWN | (!pMetrics->bEnabled ? DFCS_INACTIVE : 0) | (nHtPressed == XTP_HTSCROLLDOWN ? DFCS_PUSHED : 0)));
}

void CXTPScrollBarThemeWindowsClassic::DrawTracker(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsV* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
	...
	if (!pMetrics->rcTrack.IsRectEmpty() && !pMetrics->rcBtnTrack.IsRectEmpty())
	{
		...
		DrawEdge(pDC->GetSafeHdc(), &pMetrics->rcBtnTrack, EDGE_RAISED, (UINT)(BF_ADJUST | BF_RECT));
	}
}

void CXTPScrollBarThemeWindowsClassic::DrawButtons(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsH* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
	...
	::DrawFrameControl(pDC->GetSafeHdc(), &pMetrics->rcArrowLeft, XTPToUInt(DFC_SCROLL),
		XTPToUInt(DFCS_SCROLLLEFT | (!pMetrics->bEnabled ? DFCS_INACTIVE : 0) | (nHtPressed == XTP_HTSCROLLUP ? DFCS_PUSHED : 0)));
	::DrawFrameControl(pDC->GetSafeHdc(), &pMetrics->rcArrowRight, XTPToUInt(DFC_SCROLL), 
		XTPToUInt(DFCS_SCROLLRIGHT | (!pMetrics->bEnabled ? DFCS_INACTIVE : 0) | (nHtPressed == XTP_HTSCROLLDOWN ? DFCS_PUSHED : 0)));
}

void CXTPScrollBarThemeWindowsClassic::DrawTracker(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsH* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
