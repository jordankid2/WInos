...
#define GETPARTSTATE(ht, pressed, hot, normal, disabled) \
	(!m_State.bEnabled ? disabled : m_State.nPressedHt == ht ? pressed : m_State.nHotHt == ht ? hot : normal)
...
void CXTPScrollBarThemeVisualStudio2012::DrawScrollBar(CDC *pDC, CXTPScrollBase *pScroll)
{
	...
	m_State.nPressedHt = pSBTrack ? (pSBTrack->bTrackThumb || pSBTrack->fHitOld ? pSBInfo->ht : -1) : -1;
...
void CXTPScrollBarThemeVisualStudio2012::DrawBackground(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsV* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
...
void CXTPScrollBarThemeVisualStudio2012::DrawButtons(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsV* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
...
void CXTPScrollBarThemeVisualStudio2012::DrawArrows(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsV* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
	...
	{
		DrawArrowGlyph(pDC, pMetrics->rcArrowUp, xtpArrowGlyphUp, m_State.bEnabled, m_State.clrArrowDisabled);
		DrawArrowGlyph(pDC, pMetrics->rcArrowDown, xtpArrowGlyphDown, m_State.bEnabled, m_State.clrArrowDisabled);
	}
}

void CXTPScrollBarThemeVisualStudio2012::DrawThumbButton(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsV* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
	...
	pDC->FillSolidRect(pMetrics->rcBtnTrack, m_State.nPressedHt == XTP_HTSCROLLTHUMB ? m_State.clrBtnTrackPressed : m_State.nHotHt == XTP_HTSCROLLTHUMB ? m_State.clrBtnTrackHot : m_State.clrBtnTrackEnabled);
}

void CXTPScrollBarThemeVisualStudio2012::DrawBackground(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsH* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
	...
	CRect rcArrowRight(pSBInfo->pxDownArrow, pSBInfo->rc.top, pSBInfo->rc.right, pSBInfo->rc.bottom);

	pDC->FillSolidRect(rcArrowLeft, m_State.clrBackground);
	pDC->FillSolidRect(rcArrowRight, m_State.clrBackground);
}

void CXTPScrollBarThemeVisualStudio2012::DrawButtons(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsH* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
...
void CXTPScrollBarThemeVisualStudio2012::DrawArrows(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsH* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
	...
	{
		DrawArrowGlyph(pDC, rcArrowLeft, xtpArrowGlyphLeft, m_State.bEnabled, m_State.clrArrowDisabled);
		DrawArrowGlyph(pDC, rcArrowRight, xtpArrowGlyphRight, m_State.bEnabled, m_State.clrArrowDisabled);
	}
}

void CXTPScrollBarThemeVisualStudio2012::DrawThumbButton(CDC *pDC, CXTPScrollBase *pScroll, XTPScrollMetricsH* pMetrics)
{
	ASSERT(pDC);
	ASSERT(NULL != pScroll);
	ASSERT(NULL != pMetrics);
	...
	CRect rcArrowRight(pSBInfo->pxDownArrow, pSBInfo->rc.top, pSBInfo->rc.right, pSBInfo->rc.bottom);
	CRect rcTrack(rcArrowLeft.right, pSBInfo->rc.top, rcArrowRight.left, pSBInfo->rc.bottom);
	CRect rcLowerTrack(rcTrack.left, rcTrack.top, rcTrack.left + m_State.nBtnTrackPos, rcTrack.bottom);
	...
	pDC->FillSolidRect(rcBtnTrack, m_State.nPressedHt == XTP_HTSCROLLTHUMB ? m_State.clrBtnTrackPressed : m_State.nHotHt == XTP_HTSCROLLTHUMB ? m_State.clrBtnTrackHot : m_State.clrBtnTrackEnabled);
}
