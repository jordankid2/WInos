...
void CXTPScrollBarCtrl::SetTheme(XTPScrollBarTheme theme)
{
	...
	case xtpScrollBarThemeVisualStudio2015:
		m_pPaintManager  = new CXTPScrollBarThemeVisualStudio2015();
		m_scrollBarStyle = xtpScrollStyleVisualStudio2015;
		break;
	...
	case xtpScrollBarThemeWindowsDefault:
	case xtpScrollBarThemeOffice2010:
	default:
...
{
	m_spi.fVert = 0 != (GetStyle() & SBS_VERT);
	m_spi.fSizebox = 0 != (GetStyle() & (SBS_SIZEBOX | SBS_SIZEGRIP));
...
void CXTPScrollBarCtrl::DoScroll(int cmd, int pos)
{
	if(!m_spi.fSizebox)
	{
		GetParent()->SendMessage(
			XTPToUInt(m_spi.fVert ? WM_VSCROLL : WM_HSCROLL),
			XTPToWPARAM(MAKELONG(cmd, pos)),
