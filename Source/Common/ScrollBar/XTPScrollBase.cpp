...
CRect AFX_CDECL OffsetSourceRect(CRect rc, int nState)
{
...
int AFX_CDECL SBPosFromPx(XTP_SCROLLBAR_POSINFO *pSBInfo, int px)
{
	ASSERT(NULL != pSBInfo);
...
XTP_SLIDERTICKS::XTP_SLIDERTICKS(double* pTicks, int nCount)
{
	...
	this->nCount = nCount;
	this->pTicks = new double[XTPToUIntChecked(nCount)];
...
void CXTPScrollBase::CalcScrollBarInfo(LPRECT lprc, XTP_SCROLLBAR_POSINFO *pSBInfo, SCROLLINFO* pSI)
{
	...
	pSBInfo->pos = XTPToIntChecked(pSI->nPos);
	pSBInfo->page = pSI->nPage <= INT_MAX ? XTPToIntChecked(pSI->nPage) : 0;
	...
	if ((pSBInfo->page != 0) && (dwRange != 0))
	{
		int i = MulDiv(pSBInfo->pxDownArrow - pSBInfo->pxUpArrow,
			pSBInfo->page, XTPToIntChecked(dwRange));
...
void CXTPScrollBase::TrackBox(UINT message, CPoint point)
{
	...
	int cmsTimer = XTPToInt(GetDoubleClickTime()) / 10;
	...
	case WM_LBUTTONDOWN:
		pSBTrack->hTimerSB = 0;
		cmsTimer = XTPToInt(GetDoubleClickTime()) * 4 / 5;
	...
	case WM_MOUSEMOVE:
		if (fHit && fHitChanged)
		{
			m_nTimerElapse = cmsTimer;
			pSBTrack->hTimerSB = SetTimer(m_pSBTrack->hWndTrack, XTPToUInt(IDSYS_SCROLL), XTPToUIntChecked(m_nTimerElapse), NULL);
...
void CXTPScrollBase::ContScroll()
{
	...
	if (pSBTrack->fHitOld)
	{
		m_nTimerElapse = XTPToInt(GetDoubleClickTime()) / 10;
		pSBTrack->hTimerSB = SetTimer(m_pSBTrack->hWndTrack, XTPToUInt(IDSYS_SCROLL), XTPToUInt(m_nTimerElapse), NULL);
		DoScroll(XTPToInt(pSBTrack->cmdSB), 0);
	}
}
