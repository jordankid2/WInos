...
private:
	...
	static StdScrollBarId AFX_CDECL SBtoSBId(int nBar);
	static UINT AFX_CDECL SBIdToSBWs(StdScrollBarId nBar);
	static int AFX_CDECL SBIdToSB(StdScrollBarId nBar);
...
public:
	...
	// -----------------------------------------------------------------
	// Summary:
	//     Applies WS_CLIPCHILDREN and WS_CLIPSIBLINGS styles the the container
	//     owner and all its immediate children windows in order prevent 
	//     possible overlapping of children controls with scroll bars.
	// -----------------------------------------------------------------
	void OptimizeChildrenClipping()
	{
		m_Impl.OptimizeChildrenClipping();
	}
