...
AFX_CDECL CXTPScrollBarContainerImpl::SBtoSBId(int nBar)
{
	...
	NCCALCSIZE_PARAMS *lpncsp)
{
	if(HasStdBar(SbVert))
	{
		...
		if(m_pContainer->GetExStyle() & WS_EX_LAYOUTRTL)
		{
			...
			lpncsp->rgrc->left = m_pContainer->GetStyle() & WS_BORDER ? --lpncsp->rgrc->left : lpncsp->rgrc->left;
			lpncsp->rgrc->left = m_pContainer->GetExStyle() & WS_EX_CLIENTEDGE ? lpncsp->rgrc->left -= 2 : lpncsp->rgrc->left;
		}
		else
		{
			lpncsp->rgrc->right += cxVScroll;
			lpncsp->rgrc->right = m_pContainer->GetStyle() & WS_BORDER ? ++lpncsp->rgrc->right : lpncsp->rgrc->right;
			lpncsp->rgrc->right = m_pContainer->GetExStyle() & WS_EX_CLIENTEDGE ? lpncsp->rgrc->right += 2 : lpncsp->rgrc->right;
		}
	}
