...
CXTPActiveScriptObjectContext::CXTPActiveScriptObjectContext(
	CXTPActiveScriptEngine* pEngine,
	IDispatch* pObject)
	: m_nRefs(1)
	, m_pEngine(pEngine)
	, m_pObject(pObject)
...
HRESULT CXTPActiveScriptObjectContext::InvokeWithArgs(
	VARIANT* lpArguments /*= NULL*/,
	UINT nCount /*= 0*/,
...
CXTPActiveScriptObjectContext* CXTPActiveScriptObjectContext::GetContext(
	LPCWSTR lpszName)
{
...
HRESULT CXTPActiveScriptObjectContext::ProcessNamedItem(
	LPCWSTR lpszName,
	WORD wFlags,
	VARIANT* lpArguments,
	UINT nCount,
	VARIANT* pvtResult /*= NULL*/)
{
	...
	if (NULL != lpszName)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				hr = m_pObject->GetIDsOfNames(IID_NULL, (LPOLESTR*)&lpszName,
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = m_pObject->Invoke(dispId, IID_NULL, LOCALE_SYSTEM_DEFAULT,
			wFlags, &dispParams, pvtResult, &exceptionInfo, NULL);
...
template<>
inline BOOL AFXAPI CompareElements<LPCWSTR, LPCWSTR>(
	const LPCWSTR* pElement1,
...
HRESULT CXTPActiveScriptEngine::Initialize(
	LPCWSTR lpszEngineProgId,
	IActiveScriptSite* pSite /*= NULL*/)
{
	...
	do
	{
		...
		hr = CoCreateInstance(clsidScriptEngine, NULL, CLSCTX_ALL,
			IID_IActiveScript, (LPVOID*)&m_pEngine);
		if (FAILED(hr))
		{
			break;
		}
...
HRESULT CXTPActiveScriptEngine::LoadCode(LPCWSTR lpszCode)
{
	...
	if (SUCCEEDED(hr))
	{
		hr = m_pParser->ParseScriptText(lpszCode, NULL, NULL, NULL, 0,
			0, SCRIPTTEXT_ISVISIBLE, NULL, NULL);
	}
...
CXTPActiveScriptObjectContext* CXTPActiveScriptEngine::GetGlobalContext() const
{
	...
	{
		pContext = new CXTPActiveScriptObjectContext(
			const_cast<CXTPActiveScriptEngine*>(this), pDispScript);
	}

	return pContext;
}

HRESULT CXTPActiveScriptEngine::AddNamedItem(
	LPCWSTR lpszName,
...
XTP_STDMETHODIMP CXTPActiveScriptEngine::XActiveScriptSite::GetItemInfo(
	LPCOLESTR pstrName,
	DWORD dwReturnMask,
	IUnknown **ppiunkItem,
	ITypeInfo **ppti)
...
XTP_STDMETHODIMP CXTPActiveScriptEngine::XActiveScriptSite::OnScriptError(IActiveScriptError *pScriptError)
{
	METHOD_PROLOGUE(CXTPActiveScriptEngine, ActiveScriptSite);
...
BEGIN_DISPATCH_MAP(CXTPActiveScriptStdRuntime, CXTPCmdTarget)
	DISP_FUNCTION(CXTPActiveScriptStdRuntime, "setTimeout", OleSetTimeout, VT_I4, VTS_DISPATCH VTS_I4)
...
UINT CXTPActiveScriptStdRuntime::OleSetTimeout(
	LPDISPATCH lpDispCallback,
	UINT nTimeout)
{
	UINT_PTR nTimeoutId = 0;
	if (NULL != lpDispCallback)
	{
		...
		if (0 != nTimeoutId)
		{
			XTPAccessExclusive(TimeoutTaskRegistry::GetInstance())->Add(nTimeoutId,
				new TimeoutTask(*this, lpDispCallback, nTimeout));
		}
	}

	return XTPToUIntChecked(nTimeoutId);
}

void CXTPActiveScriptStdRuntime::OleClearTimeout(UINT nTimeoutId)
{
	TimeoutTask* pTask = XTPAccessExclusive(TimeoutTaskRegistry::GetInstance())
		->Withdraw(nTimeoutId);
	if (NULL != pTask)
	{
		KillTimer(NULL, nTimeoutId);
...
void CALLBACK CXTPActiveScriptStdRuntime::TimeoutProc(
	HWND hWnd,
	UINT uMsg,
	UINT_PTR idEvent,
	DWORD dwTime)
{
	...
	if (NULL != pTask)
	{
		...
		DISPPARAMS dispParams = { 0 };
		HRESULT hr = pTask->m_lpDispCallback->Invoke(
			DISPID_VALUE,
			IID_NULL,
			LOCALE_USER_DEFAULT,
			DISPATCH_METHOD,
