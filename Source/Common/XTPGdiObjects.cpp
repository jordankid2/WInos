...
#pragma XTP_MESSAGE("XTP_ENABLE_UNAUTHORIZED_CACHED_GDI_OBJECT_CHANGE_FAULT is defined, the application " \
...
inline void DestroyReferenceObject(T* pObj)
{
	...
	ASSERT(0 <= ::VirtualQuery(pObj, &memInfo, sizeof(memInfo)) && memInfo.Protect == PAGE_READWRITE);
...
AFX_INLINE void CXTPCachedGdiObjectReferenceBase::AssertUnaltered()
{
	...
	{
		TRACE(_T("Cached GDI object has been changed. Expected handle - 0x%pX, actual handle - 0x%pX"),
		...
		ASSERT(!"Cached GDI object has been changed which can cause undetermined behavior " \
			...
			"and consumer code. In order to determine the root cause of the issue re-compile ToolkitPro " \
			"with XTP_ENABLE_UNAUTHORIZED_CACHED_GDI_OBJECT_CHANGE_FAULT macro defined in Deubg mode and try " \
			"to re-produce it again when running in debugger. When XTP_ENABLE_UNAUTHORIZED_CACHED_GDI_OBJECT_CHANGE_FAULT is defined " \
			"any attempt to alter a cached GDI object will result in memory access violation. Once you get " \
			"this exception please report it with the copy of full stack trace and output window to support@codejock.com.");
	}
}
...
template<class Descriptor, class GdiObject, class XtpCache>
AFX_INLINE CXTPCachedGdiObjectReference<Descriptor, GdiObject, XtpCache>::CXTPCachedGdiObjectReference(
	CXTPGdiObjectCacheBase* pCache,
	HGDIOBJ hObject,
	Descriptor& descriptor)
...
template<class Descriptor, class GdiObject, class XtpCache>
AFX_INLINE XTPGDIDESCRIPTOR& CXTPCachedGdiObjectReference<Descriptor, GdiObject, XtpCache>::AccessDescriptorData() const
{
...
template<class Descriptor, class GdiObject, class XtpCache>
AFX_INLINE const GdiObject* CXTPCachedGdiObjectReference<Descriptor, GdiObject, XtpCache>::Get() const
{
	return static_cast<GdiObject*>(m_pObject);
}
...
template<class Descriptor, class GdiObject, class XtpCache>
void CXTPCachedGdiObjectReference<Descriptor, GdiObject, XtpCache>::EnableUnauthorizedCachedGdiHandleChangeFault(BOOL bEnable /*= TRUE*/)
{
	BOOL bProtect = (bEnable
	...
	if (bProtect)
	{
		...
		if (!::VirtualProtect(this, sizeof(*this), bEnable ? PAGE_READONLY : PAGE_READWRITE, &dwOldProtection))
		{
...
CXTPGdiObject::~CXTPGdiObject()
{
	...
	ASSERT("Implementation must call DeleteObject in its destructor."
		&& NULL == m_hDummyGdiObject);
#endif

	ASSERT("Implementation must call DeleteObject in its destructor."
		&& NULL == m_pCachedObject
...
{
	return NULL != m_pCachedObject
		? m_pCachedObject->GetGdiObject()
		: m_pDummyGdiObject;
}
...
{
	ASSERT(NULL != m_pDummyGdiObject);
	ASSERT("Internal GDI object has been changed"
...
template<>
AFX_INLINE BOOL AFXAPI CompareElements(
	const LOGFONT* logFont1,
	const LOGFONT* logFont2)
{
	...
	if (logFont1->lfHeight == logFont2->lfHeight
		&& logFont1->lfWidth == logFont2->lfWidth
...
template<>
AFX_INLINE UINT AFXAPI HashKey(const LOGFONT& logFont)
{
	UINT nHash = QuickSequenceHash(
		reinterpret_cast<const UINT*>(&logFont),
...
public:
	typedef CXTPCachedGdiObjectReference<XTPLOGFONTDESCRIPTOR, CFont, CXTPFontCache> XTPCachedFontReference;
	...
	XTPCachedFontReference* CreateFont(int nHeight, int nWidth, int nEscapement,
...
CXTPFontCache::XTPCachedFontReference* CXTPFontCache::CreatePointFont(
	int nPointSize,
	LPCTSTR lpszFaceName,
	CDC* pDC /*= NULL*/)
{
...
CXTPFontCache::XTPCachedFontReference* CXTPFontCache::CreatePointFontIndirect(
...
void CXTPFontCache::RemoveFromCache(const void* pDescriptor)
{
	...
	XTP_GUARD_EXCLUSIVE(CXTPGdiObjectCacheBase, static_cast<CXTPGdiObjectCacheBase*>(this))
	{
		const XTPLOGFONTDESCRIPTOR* pLogFontDescriptor = reinterpret_cast<const XTPLOGFONTDESCRIPTOR*>(pDescriptor);
...
{
	CXTPFontCache::XTPCachedFontReference* pCachedFont = XTPFontCache()->CreateFontIndirect(lpLogFont);
	SetCachedObject(pCachedFont);
	return NULL != pCachedFont;
}

BOOL CXTPFont::CreateFont(int nHeight, int nWidth, int nEscapement,
	int nOrientation, int nWeight, BYTE bItalic, BYTE bUnderline,
	BYTE cStrikeOut, BYTE nCharSet, BYTE nOutPrecision,
	BYTE nClipPrecision, BYTE nQuality, BYTE nPitchAndFamily,
	LPCTSTR lpszFacename)
{
	CXTPFontCache::XTPCachedFontReference* pCachedFont = XTPFontCache()->CreateFont(
		nHeight, nWidth, nEscapement, nOrientation, nWeight, bItalic,
		bUnderline, cStrikeOut, nCharSet, nOutPrecision, nClipPrecision,
...
{
	CXTPFontCache::XTPCachedFontReference* pCachedFont = XTPFontCache()->CreatePointFont(
		nPointSize, lpszFaceName, pDC);
	SetCachedObject(pCachedFont);
	return NULL != pCachedFont;
}
...
{
	CXTPFontCache::XTPCachedFontReference* pCachedFont = XTPFontCache()->CreatePointFontIndirect(lpLogFont, pDC);
	SetCachedObject(pCachedFont);
	return NULL != pCachedFont;
}
...
AFX_INLINE BOOL AFXAPI CompareElements(
	...
	const LOGBRUSH* logBrush2)
{
	ASSERT(NULL != logBrush1);
	ASSERT(NULL != logBrush2);

	return (logBrush1->lbStyle == logBrush2->lbStyle
		&& logBrush1->lbColor == logBrush2->lbColor
		&& logBrush1->lbHatch == logBrush2->lbHatch);
}

template<>
AFX_INLINE UINT AFXAPI HashKey(const LOGBRUSH& logBrush)
{
...
public:
	typedef CXTPCachedGdiObjectReference<XTPLOGBRUSHDESCRIPTOR, CBrush, CXTPBrushCache> XTPCachedBrushReference;
...
CXTPBrushCache::XTPCachedBrushReference* CXTPBrushCache::CreateHatchBrush(int nIndex, COLORREF crColor)
{
	LOGBRUSH lbr;
...
CXTPBrushCache::XTPCachedBrushReference* CXTPBrushCache::CreateBrushIndirect(const LOGBRUSH* lpLogBrush)
{
...
CXTPBrushCache::XTPCachedBrushReference* CXTPBrushCache::CreateDIBPatternBrush(HGLOBAL hPackedDIB, UINT nUsage)
{
...
CXTPBrushCache::XTPCachedBrushReference* CXTPBrushCache::CreateDIBPatternBrush(const void* lpPackedDIB, UINT nUsage)
{
...
void CXTPBrushCache::RemoveFromCache(const void* pDescriptor)
{
	...
	XTP_GUARD_EXCLUSIVE(CXTPGdiObjectCacheBase, static_cast<CXTPGdiObjectCacheBase*>(this))
	{
		const XTPLOGBRUSHDESCRIPTOR* pLogBrushDescriptor = reinterpret_cast<const XTPLOGBRUSHDESCRIPTOR*>(pDescriptor);
...
{
	CXTPBrushCache::XTPCachedBrushReference* pCachedBrush = XTPBrushCache()->CreateSolidBrush(crColor);
...
{
	CXTPBrushCache::XTPCachedBrushReference* pCachedBrush = XTPBrushCache()->CreateHatchBrush(nIndex, crColor);
	SetCachedObject(pCachedBrush);
	return NULL != pCachedBrush;
}
...
{
	CXTPBrushCache::XTPCachedBrushReference* pCachedBrush = XTPBrushCache()->CreateBrushIndirect(lpLogBrush);
	SetCachedObject(pCachedBrush);
	return NULL != pCachedBrush;
}
...
{
	CXTPBrushCache::XTPCachedBrushReference* pCachedBrush = XTPBrushCache()->CreatePatternBrush(pBitmap);
	SetCachedObject(pCachedBrush);
	return NULL != pCachedBrush;
}
...
{
	CXTPBrushCache::XTPCachedBrushReference* pCachedBrush = XTPBrushCache()->CreateDIBPatternBrush(hPackedDIB, nUsage);
	SetCachedObject(pCachedBrush);
	return NULL != pCachedBrush;
}
...
{
	CXTPBrushCache::XTPCachedBrushReference* pCachedBrush = XTPBrushCache()->CreateDIBPatternBrush(lpPackedDIB, nUsage);
	SetCachedObject(pCachedBrush);
	return NULL != pCachedBrush;
}
...
{
	CXTPBrushCache::XTPCachedBrushReference* pCachedBrush = XTPBrushCache()->CreateSysColorBrush(nIndex);
	SetCachedObject(pCachedBrush);
	return NULL != pCachedBrush;
}
...
struct XTPPENINFO : XTPGDIDESCRIPTOR
{
	...
	void Set(int nPenStyle, int nWidth, const LOGBRUSH* pLogBrush,
...
void XTPPENINFO::Set(int nPenStyle, int nWidth, const LOGBRUSH* pLogBrush,
	int nStyleCount /*= 0*/, const DWORD* lpStyle /*= NULL*/)
{
...
template<>
AFX_INLINE BOOL AFXAPI CompareElements(
	XTPPENINFO* const * pPenInfo1,
	XTPPENINFO* const * pPenInfo2)
{
...
template<>
AFX_INLINE UINT AFXAPI HashKey(XTPPENINFO* pPenInfo)
{
...
CXTPCachedPenReference* CXTPPenCache::CreatePen(
...
CXTPCachedPenReference* CXTPPenCache::CreatePen(
	int nPenStyle, int nWidth, const LOGBRUSH* pLogBrush,
	int nStyleCount /*= 0*/, const DWORD* lpStyle /*= NULL*/)
{
	CXTPCachedPenReference* pPenRef = NULL;
	XTP_GUARD_EXCLUSIVE(CXTPGdiObjectCacheBase, static_cast<CXTPGdiObjectCacheBase*>(this))
...
CXTPPen::CXTPPen(int nPenStyle, int nWidth, const LOGBRUSH* pLogBrush,
	int nStyleCount /*= 0*/, const DWORD* lpStyle /*= NULL*/)
	: m_hLogPenHandle(NULL)
{
	SetDummyGdiObject(m_dummyPen);
...
BOOL CXTPPen::CreatePen(int nPenStyle, int nWidth, const LOGBRUSH* pLogBrush,
	int nStyleCount /*= 0*/, const DWORD* lpStyle /*= NULL*/)
{
	CXTPCachedPenReference* pCachedPen = XTPPenCache()->CreatePen(nPenStyle, nWidth, pLogBrush, nStyleCount, lpStyle);
	SetCachedObject(pCachedPen);
	return NULL != pCachedPen;
}
