...
CXTPDrawHelpers::CXTPDrawHelpers()
{
	...
	if (m_hMsImgDll)
	{
		m_pfnFastGradientFill = XTPToFunctionPtr<PFNGRADIENTFILL>(::GetProcAddress(m_hMsImgDll, "GradientFill"));
		m_pfnAlphaBlend = XTPToFunctionPtr<PFNALPHABLEND>(::GetProcAddress(m_hMsImgDll, "AlphaBlend"));
		m_pfnTransparentBlt = XTPToFunctionPtr<PFNTRANSPARENTBLT>(::GetProcAddress(m_hMsImgDll, "TransparentBlt"));
	}
}
...
void CXTPDrawHelpers::GradientFillFast(CDC* pDC, LPCRECT lpRect, COLORREF crFrom, COLORREF crTo, BOOL bHorz)
{
	...
	GradientFill(*pDC, vert, 2, &gRect, 1, static_cast<ULONG>(bHorz ? GRADIENT_FILL_RECT_H : GRADIENT_FILL_RECT_V));
}
...
void AFX_CDECL CXTPDrawHelpers::DrawBorder(CDC* pDC, CRect rBorder, COLORREF* pOutsideClr, COLORREF* pInsideClr /*=NULL*/)
{
	...
	// draw outside border.
	pDC->FillSolidRect(rBorder.left, rBorder.top, XTP_DPI_X(1), rBorder.Height(), pOutsideClr[0]); // left border
	pDC->FillSolidRect(rBorder.left + XTP_DPI_X(1), rBorder.top, rBorder.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), pOutsideClr[1]); // top border
	pDC->FillSolidRect(rBorder.right - XTP_DPI_X(1), rBorder.top, XTP_DPI_X(1), rBorder.Height(), pOutsideClr[2]); // right border
	pDC->FillSolidRect(rBorder.left + XTP_DPI_X(1), rBorder.bottom - XTP_DPI_Y(1), rBorder.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), pOutsideClr[3]); // bottom border
	...
	// draw inside border.
	pDC->FillSolidRect(rBorder.left, rBorder.top, XTP_DPI_X(1), rBorder.Height(), pInsideClr[0]); // left border
	pDC->FillSolidRect(rBorder.left + XTP_DPI_X(1), rBorder.top, rBorder.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), pInsideClr[1]); // top border
	pDC->FillSolidRect(rBorder.right - XTP_DPI_X(1), rBorder.top, XTP_DPI_X(1), rBorder.Height(), pInsideClr[2]); // right border
	pDC->FillSolidRect(rBorder.left + XTP_DPI_X(1), rBorder.bottom - XTP_DPI_Y(1), rBorder.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), pInsideClr[3]); // bottom border
}
...
COLORREF CXTPDrawHelpers::AdjustBrightness(COLORREF clr, int nPercent)
{
	...
	return HSLtoRGB(dHue, dSat, (nPercent > 0) ?
		(dLum+(1.0-dLum)*((double)nPercent/100.0)) : (dLum*(1.0+((double)nPercent/100.0))));
}
...
static int CALLBACK XTPEnumFontFamExProc(
...
int AFX_CDECL CXTPDrawHelpers::DrawFontText(
	CDC* pDC,
	...
	COLORREF crBackColor /*= COLORREF_NULL*/)
{
	...
	if (0 != nFontHeight)
	{
		...
		{
			nResult = DrawFontText(pDC, &font, lpchText, nCount, 
				lpRect, uFormat, crForeColor, crBackColor);
		}
	}
	else
	{
				...
				if (0 < GetObject(fntTest, sizeof(fontData), &fontData))
				{
					...
					if (font.CreateFontIndirect(&fontData))
					{
						nResult = DrawFontText(pDC, &font, lpchText, nTextLen, lpRect,
							DT_CENTER | DT_VCENTER | DT_SINGLELINE, crForeColor, crBackColor);
					}
				}
			}
		}
...
DWORD CXTPDrawHelpers::GetLayout(HDC hDC)
{
	...
	if (NULL == s_pfnGetLayout)
	{
		...
		s_pfnGetLayout = XTPToFunctionPtr<PFNGDIGETLAYOUTPROC>(::GetProcAddress(hInstance, "GetLayout"));
...
void CXTPDrawHelpers::SetContextRTL(HDC hDC, BOOL bLayoutRTL)
{
	...
	if (s_pfn == (PFNGDISETLAYOUTPROC)-1)
	{
		...
		s_pfn = (hInst ? XTPToFunctionPtr<PFNGDISETLAYOUTPROC>(::GetProcAddress(hInst, "SetLayout")) : NULL);
	}

	if (s_pfn != NULL)
	{
		(*s_pfn)(hDC, DWORD(bLayoutRTL ? LAYOUT_RTL : 0));
	}
}
...
void CXTPDrawHelpers::Triangle(CDC* pDC, CPoint pt0, CPoint pt1, CPoint pt2, COLORREF clrPen, COLORREF clrBrush /*=COLORREF_NULL*/)
{
...
void CXTPDrawHelpers::DrawExpandTriangle(CDC* pDC, CPoint pt, BOOL bExpanded, COLORREF clrPen, COLORREF clrBrush /*=COLORREF_NULL*/)
{
	if (bExpanded)
	{
...
void CXTPDrawHelpers::DrawExpandArrow(CDC* pDC, CRect rc, BOOL bExpanded, COLORREF clrArrow)
{
	...
	if (pen.CreatePen(PS_SOLID, 1, clrArrow))
	{
		...
		if (pOldPen)
		{
			...
			{
				x -= width / 2;
				pDC->MoveTo(x, y);	pDC->LineTo(x + XTP_DPI_X(3), y);
				x++;
				...
				for (int dy = 1; dy <= height / 2; dy++)
				{
					...
					pDC->MoveTo(x, y + dy);		pDC->LineTo(x + dx, y + dy);
					pDC->MoveTo(x, y - dy);		pDC->LineTo(x + dx, y - dy);
					x++;
				}
			}
...
void CXTPDrawHelpers::GetWindowCaption(HWND hWnd, CString& strWindowText)
{
	...
	::DefWindowProc(hWnd, WM_GETTEXT, XTPToWPARAM(nLen + 1), XTPToLPARAM((LPCTSTR)strWindowText.GetBuffer(nLen)));
...
void CXTPDrawHelpers::DrawColorRect(CDC* pDC, int l, int t, int cx, int cy, COLORREF clrFill, COLORREF clrBorder)
{
...
void CXTPDrawHelpers::CreatePatternBrush(CBitmap& bmp, CBrush& brush, COLORREF clrFill, const void* pBitsBrush)
{
...
void CXTPDrawHelpers::DrawBrushRect(CDC* pDC, CRect rColor, COLORREF clrFill, COLORREF clrBorder, const void* pBitsBrush)
{
	HDC hDC = pDC->GetSafeHdc();
	if (hDC)
	{
...
int CXTPDrawHelpers::ComputeFittingFontHeight(CDC* pDC, LPCTSTR lpStr, int cch, const CRect& rc, CFont& fntTest)
{
	ASSERT(pDC);
	ASSERT(NULL != lpStr);
	ASSERT(0 <= cch);
...
BOOL AFX_CDECL CXTPDrawHelpers::StretchImageListImage(
	CDC* pDC,
	CRect rcDest,
	...
	int nImage)
{
	...
	if (NULL != hIcon) {
		bSuccess = ::DrawIconEx(*pDC, rcDest.left, rcDest.top, hIcon,
			rcDest.Width(), rcDest.Height(), 0, NULL, DI_NORMAL);
		DestroyIcon(hIcon);
	}
...
BOOL CXTPDrawHelpers::GetDefaultLogFont(LOGFONT* pLogFont, BOOL bModernFont /*=TRUE*/, BOOL bVertFont /*=FALSE*/)
{
	ASSERT(pLogFont);
...
void CXTPDrawHelpers::ReplaceColor(CDC* pDC, CRect rArea, COLORREF crFind, COLORREF crReplace)
{
	...
	// BitBlt to buffer dc so the monochrome white is set to fill color and the monochrome black is set to white
	// then BitBlt to pDC with buffer dc so that the replace color part is blackened out and all other colors remains same
	...
	// BitBlt to buffer dc so that the monochrome white is set to fill color and the monochrome black is set to black
	// then BitBlt to pDC with buffer dc so that all colors remains as they where except the blackened out part receives the fill color
...
void CXTPDpi::InitDpiAwareness()
{
	if (!m_bDpiAwarenessInitialized)
	{
		if (XTPSystemVersion()->IsWin81OrGreater())
		{
			...
			if (NULL != hShcoreDll)
			{
				m_pfnSetProcessDpiAwareness = XTPToFunctionPtr<PFNSetProcessDpiAwareness>(::GetProcAddress(hShcoreDll, "SetProcessDpiAwareness"));
				m_pfnGetProcessDpiAwareness = XTPToFunctionPtr<PFNGetProcessDpiAwareness>(::GetProcAddress(hShcoreDll, "GetProcessDpiAwareness"));
				m_pfnGetDpiForMonitor = XTPToFunctionPtr<PFNGetDpiForMonitor>(::GetProcAddress(hShcoreDll, "GetDpiForMonitor"));
			}

			HMODULE hUser32Dll = ::GetModuleHandle(_T("user32.dll"));
			if (NULL != hUser32Dll)
			{
				m_pfnMonitorFromWindow = XTPToFunctionPtr<PFNMonitorFromWindow>(::GetProcAddress(hUser32Dll, "MonitorFromWindow"));
			}
		}
		else if (XTPSystemVersion()->IsWinVistaOrGreater())
		{
			...
			if (NULL != hUser32Dll)
			{
				m_pfnSetProcessDPIAware = XTPToFunctionPtr<PFNSetProcessDPIAware>(::GetProcAddress(hUser32Dll, "SetProcessDPIAware"));
			}
		}
...
LRESULT CXTPDpi::OnWmCreate(
	HWND hWnd,
	WPARAM /*wParam*/,
	LPARAM /*lParam*/)
{
	if (NULL != m_pfnMonitorFromWindow
		&& NULL != m_pfnGetDpiForMonitor)
	{
...
LRESULT CXTPDpi::OnWmDpiChanged(
	HWND hWnd,
	WPARAM wParam,
	LPARAM lParam)
{
	...
	LRESULT* pResult)
{
	...
	ASSERT(m_bSupportsDpiChange && "This CXTPDpi object must not be used for handling DPI window messages");
...
{
	return dpiScaleAbove < dpi ? (v * dpi) / CXTPDpi::DefaultDpi : v;
}

template<class T>
inline T UnscaleDpi(T v, int dpi, int dpiScaleAbove)
{
	return dpiScaleAbove < dpi ? (v * CXTPDpi::DefaultDpi) / dpi : v;
}
...
inline T ScaleDpiX(T x, HDC hDC, int dpiScaleAbove)
{
	...
	return dpiScaleAbove < dpiX ? (x * dpiX) / CXTPDpi::DefaultDpi : x;
}
...
inline T ScaleDpiY(T y, HDC hDC, int dpiScaleAbove)
{
	...
	return dpiScaleAbove < dpiY ? (y * dpiY) / CXTPDpi::DefaultDpi : y;
}
...
{
	int dpiX = ::GetDeviceCaps(hDC, LOGPIXELSX);
	return dpiScaleAbove < dpiX ? (x * CXTPDpi::DefaultDpi) / dpiX : x;
}
...
{
	int dpiY = ::GetDeviceCaps(hDC, LOGPIXELSY);
	return dpiScaleAbove < dpiY ? (y * CXTPDpi::DefaultDpi) / dpiY : y;
}
...
int CXTPDpi::ScaleX(int x, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	if (iDpiScaleAbove < dpiX)
	{
		...
		{
			nScaled = (0 <= x
				? m_nPositiveDpiValues[x].x
				: m_nNegativeDpiValues[-x].x);
		}
		else
		{
...
int CXTPDpi::ScaleY(int y, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	if (iDpiScaleAbove < dpiY)
	{
		...
		{
			nScaled = (0 <= y
				? m_nPositiveDpiValues[y].y
				: m_nNegativeDpiValues[-y].y);
		}
		else
		{
...
unsigned int CXTPDpi::ScaleX(HDC hDC, unsigned int x, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	return ScaleDpiX(x, hDC, iDpiScaleAbove);
}

unsigned int CXTPDpi::ScaleY(HDC hDC, unsigned int y, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	return ScaleDpiY(y, hDC, iDpiScaleAbove);
}
...
unsigned int CXTPDpi::UnscaleX(HDC hDC, unsigned int x, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	return UnscaleDpiX(x, hDC, iDpiScaleAbove);
}

unsigned int CXTPDpi::UnscaleY(HDC hDC, unsigned int y, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	return UnscaleDpiY(y, hDC, iDpiScaleAbove);
}
...
void CXTPDpi::ScaleRect(RECT *pRect, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pRect->left = ScaleX(pRect->left, iDpiScaleAbove);
	pRect->right = ScaleX(pRect->right, iDpiScaleAbove);
	pRect->top = ScaleY(pRect->top, iDpiScaleAbove);
	pRect->bottom = ScaleY(pRect->bottom, iDpiScaleAbove);
}
...
void CXTPDpi::UnscaleRect(RECT *pRect, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pRect->left = UnscaleX(pRect->left, iDpiScaleAbove);
	pRect->right = UnscaleX(pRect->right, iDpiScaleAbove);
	pRect->top = UnscaleY(pRect->top, iDpiScaleAbove);
	pRect->bottom = UnscaleY(pRect->bottom, iDpiScaleAbove);
}
...
void CXTPDpi::ScaleRect(HDC hDC, RECT *pRect, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pRect->left = ScaleX(hDC, pRect->left, iDpiScaleAbove);
	pRect->right = ScaleX(hDC, pRect->right, iDpiScaleAbove);
	pRect->top = ScaleY(hDC, pRect->top, iDpiScaleAbove);
	pRect->bottom = ScaleY(hDC, pRect->bottom, iDpiScaleAbove);
}
...
void CXTPDpi::UnscaleRect(HDC hDC, RECT *pRect, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pRect->left = UnscaleX(hDC, pRect->left, iDpiScaleAbove);
	pRect->right = UnscaleX(hDC, pRect->right, iDpiScaleAbove);
	pRect->top = UnscaleY(hDC, pRect->top, iDpiScaleAbove);
	pRect->bottom = UnscaleY(hDC, pRect->bottom, iDpiScaleAbove);
}

RECT CXTPDpi::UnscaleRect(HDC hDC, const RECT& rc, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	RECT rcUnscaled = rc;
...
CRect CXTPDpi::ScaleRect(HDC hDC, const CRect& rc, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
...
CRect CXTPDpi::UnscaleRect(HDC hDC, const CRect& rc, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
...
void CXTPDpi::ScalePoint(POINT *pPoint, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pPoint->x = ScaleX(pPoint->x, iDpiScaleAbove);
	pPoint->y = ScaleY(pPoint->y, iDpiScaleAbove);
}
...
void CXTPDpi::UnscalePoint(POINT *pPoint, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pPoint->x = UnscaleX(pPoint->x, iDpiScaleAbove);
	pPoint->y = UnscaleY(pPoint->y, iDpiScaleAbove);
}
...
void CXTPDpi::ScalePoint(HDC hDC, POINT *pPoint, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pPoint->x = ScaleX(hDC, pPoint->x, iDpiScaleAbove);
	pPoint->y = ScaleY(hDC, pPoint->y, iDpiScaleAbove);
}

POINT CXTPDpi::ScalePoint(HDC hDC, const POINT& pt, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	POINT ptScaled = pt;
...
void CXTPDpi::UnscalePoint(HDC hDC, POINT *pPoint, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	ASSERT(NULL != pPoint);

	pPoint->x = UnscaleX(hDC, pPoint->x, iDpiScaleAbove);
	pPoint->y = UnscaleY(hDC, pPoint->y, iDpiScaleAbove);
}

POINT CXTPDpi::UnscalePoint(HDC hDC, const POINT& pt, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	POINT ptUnscaled = pt;
...
void CXTPDpi::ScalePoint(HDC hDC, CPoint *pPoint, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	ScalePoint(hDC, (POINT*)pPoint, iDpiScaleAbove);
}

CPoint CXTPDpi::ScalePoint(HDC hDC, const CPoint& pt, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	return ScalePoint(hDC, (const POINT&)pt, iDpiScaleAbove);
}

void CXTPDpi::UnscalePoint(HDC hDC, CPoint *pPoint, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	UnscalePoint(hDC, (POINT*)pPoint, iDpiScaleAbove);
}

CPoint CXTPDpi::UnscalePoint(HDC hDC, const CPoint& pt, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
...
CPoint CXTPDpi::Unscale(HDC hDC, const CPoint& pt, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	return UnscalePoint(hDC, pt, iDpiScaleAbove);
}
...
void CXTPDpi::ScaleSize(SIZE *pSize, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pSize->cx = ScaleX(pSize->cx, iDpiScaleAbove);
	pSize->cy = ScaleY(pSize->cy, iDpiScaleAbove);
}
...
void CXTPDpi::UnscaleSize(SIZE *pSize, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pSize->cx = UnscaleX(pSize->cx, iDpiScaleAbove);
	pSize->cy = UnscaleY(pSize->cy, iDpiScaleAbove);
}
...
void CXTPDpi::ScaleSize(HDC hDC, SIZE *pSize, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pSize->cx = ScaleX(hDC, pSize->cx, iDpiScaleAbove);
	pSize->cy = ScaleY(hDC, pSize->cy, iDpiScaleAbove);
}
...
void CXTPDpi::UnscaleSize(HDC hDC, SIZE *pSize, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	...
	pSize->cx = UnscaleX(hDC, pSize->cx, iDpiScaleAbove);
	pSize->cy = UnscaleY(hDC, pSize->cy, iDpiScaleAbove);
}

SIZE CXTPDpi::UnscaleSize(HDC hDC, const SIZE& sz, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
	SIZE szUnscaled = sz;
...
CSize CXTPDpi::ScaleSize(HDC hDC, const CSize& sz, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
...
CSize CXTPDpi::UnscaleSize(HDC hDC, const CSize& sz, int iDpiScaleAbove /*= CXTPDpi::DefaultDpi*/) const
{
...
template<class T> AFX_INLINE T SelectDpiSpecificImpl(T upToDpi120, T upToDpi144, T upToDpi192, T theLargest)
{
...
LPCTSTR CXTPDpi::SelectDpiSpecific(LPCTSTR upToDpi120, LPCTSTR upToDpi144, LPCTSTR upToDpi192, LPCTSTR theLargest) const
{
	return SelectDpiSpecificImpl(upToDpi120, upToDpi144, upToDpi192, theLargest);
}

void* CXTPDpi::SelectDpiSpecific(void* upToDpi120, void* upToDpi144, void* upToDpi192, void* theLargest) const
{
	return SelectDpiSpecificImpl(upToDpi120, upToDpi144, upToDpi192, theLargest);
}
	...
	, m_bCodejockFontLoadingAttempted(FALSE)
{
	...
	m_pfnAddFontMemResourceEx = XTPToFunctionPtr<PFNAddFontMemResourceEx>(::GetProcAddress(hGdi32, "AddFontMemResourceEx"));
	m_pfnRemoveFontMemResourceEx = XTPToFunctionPtr<PFNRemoveFontMemResourceEx>(::GetProcAddress(hGdi32, "RemoveFontMemResourceEx"));
}
...
CXTPPrimitiveDrawer::~CXTPPrimitiveDrawer()
{
	...
	if (NULL != m_hCodejockMemFont
		&& NULL != m_pfnRemoveFontMemResourceEx)
	{
...
void CXTPPrimitiveDrawer::InitFont()
{
	if (!m_bCodejockFontLoadingAttempted)
	{
		...
		if (!m_bCodejockFontLoadingAttempted)
		{
			...
			BYTE bQuality = (BYTE)(XTPDpiHelper()->IsDefaultDpi() && XTPSystemVersion()->IsWinNT4OrGreater()
			...
			if (m_fntCodejock.CreateFont(0, 0, 0, 0, 0, 0, 0, 0, DEFAULT_CHARSET,
				OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS, bQuality, FIXED_PITCH,
				_T("Codejock")))
			{
...
void CXTPPrimitiveDrawer::DrawSymbol(
	CDC* pDC,
	...
	HPEN hOutlinePen /*= NULL*/)
{
	...
	if (NULL != m_fntCodejock.m_hObject)
	{
		...
		int nFontSize = XTPDrawHelpers()->ComputeFittingFontHeight(pDC, strSymbol, 1, rc, m_fntCodejock);
		if (0 != nFontSize)
		{
			...
			if (font.CreateFontIndirect(&fontData))
			{
				...
				if (!bOutline)
				{
					XTPDrawHelpers()->DrawFontText(pDC, &font, strSymbol, 1, rcOut,
						DT_SINGLELINE | DT_NOCLIP, crForeColor, crBackColor);
				}
				else
				{
					if (COLORREF_NULL == crForeColor)
...
void AFX_CDECL CXTPPrintPageHeaderFooter::DoInsertHFFormatSpecifierViaMenu(
