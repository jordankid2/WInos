		...
		case VT_EX_RECT:
			{
				...
				{
					RECT rc = {0, 0, 0, 0};
					if (SCANF_S(strValue, _T("%li, %li, %li, %li"), &rc.left, &rc.top, &rc.right, &rc.bottom) != 4)
						return CopyPropValue(vtProp, pvProp, pvDefault);
...
template<class T>
static BOOL PX_Point3dImpl(CXTPPropExchange* pPX, LPCTSTR pszPropName,
	CXTPPoint3T<T>& ptValue, CXTPPoint3T<T> ptDefault,
	LPCTSTR pszFormat)
{
	ASSERT_POINTER(pPX, CXTPPropExchange);
...
BOOL AFX_CDECL PX_Point(CXTPPropExchange* pPX, LPCTSTR pszPropName, CXTPPoint3T<float>& ptValue, CXTPPoint3T<float> ptDefault)
{
	return PX_Point3dImpl(pPX, pszPropName, ptValue, ptDefault, _T("%f, %f, %f"));
}

BOOL AFX_CDECL PX_Point(CXTPPropExchange* pPX, LPCTSTR pszPropName, CXTPPoint3T<double>& ptValue, CXTPPoint3T<double> ptDefault)
{
	return PX_Point3dImpl(pPX, pszPropName, ptValue, ptDefault, _T("%f, %f, %f"));
}

BOOL AFX_CDECL PX_Point(CXTPPropExchange* pPX, LPCTSTR pszPropName, CXTPPoint3T<int>& ptValue, CXTPPoint3T<int> ptDefault)
{
...
HKEY ParseRegistryRoot(CString strKey, CString& strSection)
{
	...
	while (nIndex > 0)
	{
		...
		if (hKey == NULL)
		{
			if ((RegOpenKeyEx(hKeyRoot, strSectionName, 0, KEY_WRITE | KEY_READ,
		...
		else
		{
			...
			DWORD dw;
			if ((RegCreateKeyEx(hKey, strSectionName, 0, REG_NONE,
...
CXTPPropExchange* AFX_CDECL PropExchangeFromControl(LPDISPATCH lpPropExchage)
{
	...
	CXTPPropExchange* pPX = DYNAMIC_DOWNCAST(CXTPPropExchange, CXTPPropExchange::FromIDispatchSafe(lpDispatch));
	if (!pPX)
	{
		lpDispatch->Release();
		return NULL;
