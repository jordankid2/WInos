...
{
	if (m_pSharedData)
		return (CSharedData&)*m_pSharedData;
...
{
	SAFE_DELETE(m_pSharedData);
	m_pSharedData = new CSharedData();
}
...
CXTPWinThemeWrapper::CXTPWinThemeWrapper(BOOL bUseSkinFramework)
	: m_hTheme(NULL)
	, m_bUseSkinFramework(bUseSkinFramework)
	, m_pSharedData(0)
{
}
...
BOOL CXTPWinThemeWrapper::IsAppThemeActive() const
{
	return IsAppThemed() && ThemeDataOpen();
}
...
HRESULT CXTPWinThemeWrapper::BufferedPaintSetAlpha(HPAINTBUFFER hBufferedPaint, const RECT *prc, BYTE alpha)
{
	HRESULT hResult = E_FAIL;

	TRY_POINTER_CALL(BufferedPaintSetAlpha)
		hResult = POINTER_CALL(hBufferedPaint, prc, alpha);
	CATCH_POINTER_CALL()
...
void CXTPWinDwmWrapper::LogicalToPhysicalPoint(HWND hWnd, LPPOINT lpPoint)
{
	...
	PFNLOGICALTOPHYSICALPOINT pfnLogicalToPhysicalPoint = XTPToFunctionPtr<PFNLOGICALTOPHYSICALPOINT>(
		::GetProcAddress(hModule, "LogicalToPhysicalPoint"));
...
void CXTPWinDwmWrapper::SetProcessDPIAware()
{
	...
	PFNSETPROCESSDPIAWARE pfnSetProcessDPIAware = XTPToFunctionPtr<PFNSETPROCESSDPIAWARE>(
		::GetProcAddress(hModule, "SetProcessDPIAware"));
