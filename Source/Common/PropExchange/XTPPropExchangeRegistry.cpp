...
BOOL CXTPPropExchangeRegistry::IsSectionExists(LPCTSTR lpszSection)
{
	...
	if (RegOpenKeyEx(m_hSectionKey, lpszSection, 0, REGSAM(m_bLoading ? KEY_READ : KEY_WRITE), &hSectionKey) == ERROR_SUCCESS)
	{
...
BOOL CXTPPropExchangeRegistry::OnBeforeExchange()
{
	...
	if (!m_bInitialized)
	{
		...
		VERIFY(RegCreateKeyEx(m_hParentKey, m_strSectionName, 0, REG_NONE, REG_OPTION_NON_VOLATILE, 
			REGSAM(m_bLoading ? KEY_READ : KEY_WRITE), NULL, &m_hSectionKey, NULL) == ERROR_SUCCESS);
		...
		case VT_EX_RECT:
			{
				...
				if (lResult == ERROR_SUCCESS)
				{
					...
					LPBYTE lpBuffer = (LPBYTE)strValue.GetBuffer(XTPToIntChecked(dwCount / sizeof(TCHAR)));
