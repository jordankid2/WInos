...
BOOL CXTPPropExchangeXMLNode::LoadFromEncodedString(LPCSTR lpszState)
{
	...
	HRESULT hr = m_xmlDocPtr->raw_load(va, &bResult);
	VariantClear(&va);
...
BOOL CXTPPropExchangeXMLNode::LoadFromString(BSTR lpszState)
{
	...
	VARIANT_BOOL bResult;
	HRESULT hr = m_xmlDocPtr->raw_loadXML(lpszState, &bResult);
	if (FAILED(hr) || bResult == VARIANT_FALSE)
	{
		TraceParseError();
		return FALSE;
...
BOOL CXTPPropExchangeXMLNode::LoadFromFile(LPCTSTR strFileName)
{
	...
	VARIANT_BOOL bResult;
	HRESULT hr = m_xmlDocPtr->raw_load(COleVariant(strFileName), &bResult);
	if (FAILED(hr) || bResult == VARIANT_FALSE)
	{
		TraceParseError();
		return FALSE;
...
CString CXTPPropExchangeXMLNode::GetEncodedXML(BOOL bFormatted)
{
	...
	if (IsRootSection())
	{
		...
		if (FAILED(m_xmlDocPtr->raw_save(var)))
			return str;
...
void CXTPPropExchangeXMLNode::FormatFile(LPCTSTR lpszFileName)
{
	...
	#define WRITEINDENT\
		fileBuffer.Write(lpszBegin, XTPToUIntChecked(int(lpszCurrent - lpszBegin) - nWhiteSpace));\
	...
	fileBuffer.Write(lpszBegin, XTPToUIntChecked(int(lpszCurrent - lpszBegin) - 1));
	fileBuffer.Write(lpszCrLf, 2);
...
void CXTPPropExchangeXMLNode::UpdateDocumentEncoding()
{
	...
	if (SUCCEEDED(m_xmlDocPtr->raw_selectSingleNode(L"/node()", &ptrBaseInstruction)) && ptrBaseInstruction != 0)
	{
		BSTR bstrNodeName = 0;
	...
	if (!bInstructionExists && SUCCEEDED(m_xmlDocPtr->raw_createProcessingInstruction(L"xml", CT2BSTR(strInstruction), &ptrInstruction)))
	{
		CXTPDOMNodePtr xmlNodePtr;
		if (SUCCEEDED(m_xmlDocPtr->raw_selectSingleNode(L"/node()", &xmlNodePtr)))
		{
			m_xmlDocPtr->raw_insertBefore(ptrInstruction, _variant_t(xmlNodePtr.GetInterfacePtr()), 0);
		}
	}
}
...
HRESULT CXTPPropExchangeXMLNode::SaveToFile(LPCTSTR strFileName, BOOL bFormatted)
{
	...
	HRESULT hr = m_xmlDocPtr->raw_save(COleVariant(strFileName));
	if (FAILED(hr))
		return hr;
...
void CXTPPropExchangeXMLNode::EmptySection()
{
	...
	{
		m_xmlParentNode->raw_removeChild(m_xmlSectionNode, NULL);
		m_xmlSectionNode = 0;
		m_bInitialized = FALSE;
	}
}
...
CXTPDOMNodePtr CXTPPropExchangeXMLNode::GetProfileNode(LPCTSTR strSection)
{
	...
	{
		m_xmlDocPtr->raw_createElement(CT2BSTR(strSection), &xmlElementPtr);
		ASSERT(xmlElementPtr != 0);
		if (xmlElementPtr)
		{
			m_xmlParentNode->raw_appendChild(xmlElementPtr, &xmlNodePtr);
		}
	}
...
CXTPDOMNodePtr CXTPPropExchangeXMLNode::GetEntryNode(LPCTSTR lpszEntry, BOOL bCreate)
{
	...
	if (m_bCompactMode)
	{
		...
		HRESULT hr = map->raw_getNamedItem(CT2BSTR(lpszEntry), &xmlNodeEntryPtr);
		...
		if (FAILED(hr) || xmlNodeEntryPtr == NULL)
		{
			if (bCreate)
			{
				...
				m_xmlDocPtr->raw_createAttribute(CT2BSTR(lpszEntry), &xmlElementPtr);
				if (xmlElementPtr)
				{
					map->raw_setNamedItem(xmlElementPtr, &xmlNodeEntryPtr);
				}
			}
			else
			{
	...
	else
	{
		...
		HRESULT hr = xmlNodeSectionPtr->raw_selectSingleNode(CT2BSTR(lpszEntry), &xmlNodeEntryPtr);
		if (FAILED(hr) || xmlNodeEntryPtr == NULL)
		{
			if (bCreate)
			{
				...
				CXTPDOMElementPtr xmlElementPtr;
				m_xmlDocPtr->raw_createElement(CT2BSTR(lpszEntry), &xmlElementPtr);
				ASSERT(xmlElementPtr != 0);
				if (xmlElementPtr)
				{
					xmlNodeSectionPtr->raw_appendChild(xmlElementPtr, &xmlNodeEntryPtr);
				}
			}
			else
			{
...
BOOL CXTPPropExchangeXMLNode::PutSection(CXTPPropExchangeXMLNode* pNode)
{
	...
	if (FAILED(m_xmlSectionNode->raw_appendChild(pNode->m_xmlSectionNode, NULL)))
		return FALSE;
...
CXTPPropExchangeXMLNode* CXTPPropExchangeXMLNode::Clone(BOOL bRecursive)
{
	...
	if (FAILED(m_xmlSectionNode->raw_cloneNode(vbRecursive, &xmlNodePtr)))
		return NULL;
...
CXTPPropExchangeXMLNode* CXTPPropExchangeXMLNode::CreateSection(LPCTSTR pszSection)
{
	...
	m_xmlDocPtr->raw_createElement(CT2BSTR(pszSection), &xmlElementPtr);
	if (xmlElementPtr == 0)
	{
		ASSERT(0);
		return NULL;
	...
	if (FAILED(m_xmlSectionNode->raw_appendChild(xmlElementPtr, &xmlNodePtr)))
		return NULL;
...
{
public:
	CXMLEnumerator(CXTPPropExchangeXMLNode* pContainer, LPCTSTR lpszSection)
...
public:
	...
	POSITION GetPosition(DWORD dwCount = 0, BOOL /*bCompactMode*/ = TRUE)
	{
		...
		if (m_pContainer->IsLoading())
		{
			...
			if (FAILED(((CXTPPropExchangeXMLNode*)m_pContainer)->m_xmlSectionNode->raw_selectNodes(CT2BSTR(strNode), &m_xmlNodes)))
			{
				return NULL;
			}
	...
	CXTPPropExchange* GetNext(POSITION& pos)
	{
		...
		if (m_pContainer->IsStoring())
		{
			...
			pSection->m_xmlDocPtr->raw_createElement(CT2BSTR(m_strSectionName), &xmlElementPtr);
			ASSERT(xmlElementPtr != 0);
			if (xmlElementPtr)
			{
				((CXTPPropExchangeXMLNode*)m_pContainer)->m_xmlSectionNode->raw_appendChild(xmlElementPtr, &xmlNodePtr);
			}
	...
	int GetCount() const
	{
		long nCount = XTPToLongChecked(m_nCount);
