...
CXTPPropExchangeArchive::CSelfArchive::CSelfArchive(CFile* pFile, 
	UINT nMode, int nBufSize /*= 4096*/, void* lpBuf /*= NULL*/)
...
BOOL CXTPPropExchangeArchive::ExchangeObjectInstance(CObject*& pObject, CRuntimeClass* pObjectRuntime)
{
	...
	if (IsLoading())
	{
		...
		m_ar.m_pLoadArray->InsertAt(XTPToIntPtrChecked(m_ar.m_nMapCount++), pObject);
	}
	else
	{
		m_ar.MapObject(NULL);
