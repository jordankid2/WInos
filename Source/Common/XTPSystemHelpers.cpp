...
BOOL CXTPModuleHandle::GetModuleHandle(LPCTSTR lpszModuleName)
{
	...
	BOOL bObtained = (xtpModNone != m_eModuleState
		&& IsModule(lpszModuleName)
		&& EnsureModuleState());
	if (!bObtained)
	{
...
BOOL CXTPModuleHandle::GetVersionInfo()
{
	...
	if (EnsureModuleState())
	{
		...
		{
			m_pfnDllGetVersion = XTPToFunctionPtr<PFNDLLVERSIONINFO>(::GetProcAddress(m_hModule, "DllGetVersion"));
		}
...
void CXTPAccessible::AccessibleNotifyWinEvent(DWORD event, HWND hwnd, LONG idObject, LONG idChild)
{
	...
	{
		m_pNotifyWinEvent = XTPToFunctionPtr<LPFNNOTIFYWINEVENT>(::GetProcAddress(m_hUser32, "NotifyWinEvent"));
...
LRESULT CXTPAccessible::LresultFromObject(REFIID riid, WPARAM wParam, LPUNKNOWN punk)
{
	...
	{
		m_pLresultFromObject = XTPToFunctionPtr<LPFNLRESULTFROMOBJECT>(::GetProcAddress(m_hOleAcc, "LresultFromObject"));
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::QueryInterface(
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::GetTypeInfoCount(
	unsigned int*)
{
	return E_NOTIMPL;
}

XTP_STDMETHODIMP CXTPAccessible::XAccessible::GetTypeInfo(
	unsigned int, LCID, ITypeInfo**)
{
	return E_NOTIMPL;
}

XTP_STDMETHODIMP CXTPAccessible::XAccessible::GetIDsOfNames(
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::get_accChild(VARIANT varChild, IDispatch* FAR* ppdispChild)
{
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::get_accDescription(VARIANT varChild, BSTR FAR* pszDescription)
{
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::get_accHelpTopic(BSTR* pszHelpFile, VARIANT varChild, long* pidTopic)
{
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::get_accKeyboardShortcut(VARIANT varChild, BSTR* pszKeyboardShortcut)
{
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::get_accDefaultAction(VARIANT varChild, BSTR* pszDefaultAction)
{
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::accLocation(long* pxLeft, long* pyTop, long* pcxWidth, long* pcyHeight, VARIANT varChild)
{
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::accNavigate(long navDir, VARIANT varStart, VARIANT* pvarEndUpAt)
{
...
XTP_STDMETHODIMP CXTPAccessible::XAccessible::Invoke(
...
{
	return &CXTPSingleton<CXTPSoundManager>::Instance();
}
...
BOOL XTPLoadWindowClass_RichEdit(
	...
	LPWNDCLASS pWndClass = NULL)
{
	...
	case xtpWindowClassVersionAtLeast:
		...
		break;
	case xtpWindowClassVersionExact:
		bResult = versions[*pnVersion].Load(pWndClass);
		break;
	}
...
BOOL AFX_CDECL XTPLoadWindowClass(
...
static HRESULT ResolveRelativePath(
...
inline LPCTSTR GetUriResourceType(
	LPCTSTR lpszSource,
	const XTP_URI_DEFAULTS* pDefaults = NULL)
{
	LPCTSTR lpType = (NULL != pDefaults && NULL != pDefaults->res.lpType
		? pDefaults->res.lpType : XTPGetDefaultResourceTypeFromPath(lpszSource));

	return lpType;
}

static HRESULT ParseResUri(
	LPCTSTR lpszResPath,
	XTP_URI* pUri,
	const XTP_URI_DEFAULTS* pDefaults = NULL)
{
	...
	do
	{
		...
		// Parse FILE part.
		if(_T('.') == *lpszResPath
			&& ((lpszResPath + 2) == lpType || (lpszResPath + 2) == lpID))
		{
			if(0 == GetModuleFileName(NULL, pUri->res.szFilePath, _countof(pUri->res.szFilePath)))
			{
				hr = HRESULT_FROM_WIN32(GetLastError());
	...
	const XTP_URI_DEFAULTS* pDefaults /*= NULL*/)
{
	...
	if(NULL != lpszUri && NULL != pUri)
	{
		...
		if(0 == _tcsncmp(lpszUri, szFileProtocol, _countof(szFileProtocol) - 1))
		{
			...
			hr = ResolveRelativePath(
				lpszUri + _countof(szFileProtocol) - 1,
				pUri->file,
				_countof(pUri->file));
		}
...
static HRESULT CreateStreamOnResourceUri(
	const XTP_URI* pUri,
	IStream** ppStream)
{
	ASSERT(NULL != pUri);
	...
	do
	{
		// Obtain size and pointer to resource data.
		hModule = LoadLibraryEx(pUri->res.szFilePath, NULL, 
		...
		HRSRC hRes = FindResource(hModule, pUri->res.lpName, lpType);
		if(NULL == hRes)
		{
			strType = XTPFindResourceType(hModule, pUri->res.lpName);
			if (!strType.IsEmpty())
			{
				hRes = FindResource(hModule, pUri->res.lpName, strType);
			}
		...
		// Create stream on resource data.
		HGLOBAL hStmData = GlobalAlloc(GPTR, dwSize);
		if(NULL == hStmData)
		{
			hr = HRESULT_FROM_WIN32(GetLastError());
			break;
		...
		LPVOID lpStmData = GlobalLock(hStmData);
		if(NULL == lpStmData)
		{
			hr = HRESULT_FROM_WIN32(GetLastError());
			GlobalFree(hStmData);
		...
		memcpy(lpStmData, lpData, dwSize);
		GlobalUnlock(lpStmData);

		hr = CreateStreamOnHGlobal(hStmData, TRUE, ppStream);
		if(FAILED(hr))
		{
			GlobalFree(hStmData);
			break;
...
public:
	...
	{
		return m_pStream->AddRef();
	}

	STDMETHODIMP_(ULONG) Release()
	{
		ULONG nRefs = m_pStream->Release();
		if(0 == nRefs)
		{
			delete this;
		}
...
static HRESULT CreateStreamOnFileUri(
	const XTP_URI* pUri,
	IStream** ppStream)
{
	ASSERT(NULL != pUri);
	...
	do
	{
		hFile = CreateFile(pUri->file, GENERIC_READ, FILE_SHARE_READ,
		...
		hMem = GlobalAlloc(GPTR, fileSize.LowPart);
		if(NULL == hMem)
		{
			hr = HRESULT_FROM_WIN32(GetLastError());
			break;
...
HRESULT AFX_CDECL XTPCreateReadOnlyStreamOnUri(
	const XTP_URI* pUri,
	IStream** ppStream)
{
	ASSERT(NULL != pUri);
	...
	{
	case xtpUriFile:
		hr = CreateStreamOnFileUri(pUri, &pStream);
		break;
	case xtpUriRes:
		hr = CreateStreamOnResourceUri(pUri, &pStream);
		break;
	default:
		hr = E_INVALIDARG;
		break;
	...
	if(SUCCEEDED(hr))
	{
		*ppStream = new CReadOnlyStreamWrapper(pStream);
	}
...
BOOL AFX_CDECL XTPLoadUnicodeText(
	const void* pRawData,
	UINT cbRawData,
	...
	if(NULL != pRawData && NULL != lpUnicodeText && NULL != pchUnicodeText)
	{
			...
			&& 0xbf == pBOM[2])
		{
			...
			int cch = MultiByteToWideChar(CP_UTF8, 0,
				lpMbString, XTPToIntChecked(cchRaw),
		...
		else if(0 < cbRawData)
		{
			...
			int cch = MultiByteToWideChar(CP_ACP, 0, lpMbString,
				XTPToIntChecked(cchRaw), lpUnicodeText,
				XTPToIntChecked(*pchUnicodeText) - 1);
			if(0 < cch)
			{
				*pchUnicodeText = XTPToUInt(cch);
				lpUnicodeText[cch] = L'\0';
...
BOOL AFX_CDECL XTPIsDerivedFromRemote(const CRuntimeClass* pBaseClass, const CRuntimeClass* pClassThis)
{
	...
	, m_pfnModule32Next(NULL)
{
	...
	if (NULL != hKernel32)
	{
		...
		m_pfnModule32First = XTPToFunctionPtr<PFNMODULE32FIRSTNEXT>(
		...
		m_pfnModule32Next = XTPToFunctionPtr<PFNMODULE32FIRSTNEXT>(
			::GetProcAddress(hKernel32,
#ifndef _UNICODE
			"Module32Next"
#else
		...
		m_bInitialized = (NULL != m_pfnCreateToolhelp32Snapshot
			&& NULL != m_pfnThread32First
			&& NULL != m_pfnThread32Next
			&& NULL != m_pfnModule32First
