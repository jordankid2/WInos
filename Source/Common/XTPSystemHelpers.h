...
{
	friend class CXTPSingleton<CXTPSoundManager>;
...
// Summary: Parses URI.
// Parameters:
//     lpszUri - Pointer to null terminated URI.
//
...
//          the problem occured.
//-----------------------------------------------------------------------
_XTP_EXT_CLASS HRESULT AFX_CDECL XTPParseUri(
	LPCTSTR lpszUri,
	XTP_URI* pUri,
	const XTP_URI_DEFAULTS* pDefaults = NULL);

//-----------------------------------------------------------------------
// Summary: Creates a read-only stream on object identifier by URI specified.
// Parameters:
//     pUri - Pointer to object URI structure.
//     grfMode - STGM flags.
//     ppStream - Output pointer that receives stream pointer.
// Returns: S_OK if successful or a standard COM error code.
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
// Summary: Loads an arbitrary text as Unicode. If source text is no
//          in Unicode format, converts to Unicode depending on detected
//          text ncoding. Supported encodings: ANSI, UTF-8, Unicode.
// Parameters:
//     pRawData - Pointer to raw text data to load.
//     cbRawData - Size of raw text data in bytes.
//     lpUnicodeText - Pointer to a buffer large enough to hold converted
//                     to Unicode text.
//     pchUnicodeText - On input contains number of WCHARs available in
//                      lpUnicodeText buffer including terminating 0,
//                      on output - number of WCHARs placed to the buffer
//                      excluding terminating 0.
//-----------------------------------------------------------------------
_XTP_EXT_CLASS BOOL AFX_CDECL XTPLoadUnicodeText(
	const void* pRawData,
	UINT cbRawData,
	LPWSTR lpUnicodeText,
	UINT* pchUnicodeText);
...
enum XTPWindowClassVersionSelectionVerb
{
	...
	xtpWindowClassVersionAtLeast, // At least the specified window class version should be available.
	                              // The largest available is selected in result.
...
//-----------------------------------------------------------------------
// Summary:
//     Loads and verified a window class module of a specified version
//     according to version selection rule.
// Parameters:
//     nClass - One of predefined window class identifiers.
...
//     pWndClass - Optional. If not NULL and succeeded it gets filled
//                 with window class specific information.
// Returns:
//     TRUE if the class specified has been loaded and verified.
//-----------------------------------------------------------------------
_XTP_EXT_CLASS BOOL AFX_CDECL XTPLoadWindowClass(
	XTPWindowClass nClass,
...
AFX_INLINE BOOL AFX_CDECL XTPLoadWindowClass(
	...
	XTPWindowClassVersionSelectionVerb nVerb,
	int nVersion,
	LPWNDCLASS pWndClass = NULL) // <combine XTPLoadWindowClass@XTPWindowClass@XTPWindowClassVersionSelectionVerb@int*@LPWNDCLASS>
{
...
//-----------------------------------------------------------------------
// Summary: Initializes COM, increments and holds COM initialization reference
...
public:
	//-----------------------------------------------------------------------
	// Summary: Initializes COM for the calling thread. If COM has already been
	//          initialized, just increments COM reference counter.
	//-----------------------------------------------------------------------
	CXTPComInitializer()
	{
	...
	//-----------------------------------------------------------------------
	// Summary: Decrements COM reference counter and uninitializes COM for 
	//          the calling thread if its holds the last COM reference.
	//-----------------------------------------------------------------------
	~CXTPComInitializer()
	{
	...
	//-----------------------------------------------------------------------
	// Summary: Obtains COM initialization status code.
	// Returns: COM initialization status code.
	//-----------------------------------------------------------------------
...
//=======================================================================
// Summary:
//      Holds a list if observable objects to which an oberser is subscribed.
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//      Constructs subscription list for the observer provided.
	// Parameters:
...
//-----------------------------------------------------------------------
// Summary: Provides a basic implementation of the Observable design pattern.
// Parameters:
...
protected:
	//-----------------------------------------------------------------------
	// Summary: Constructs observable object.
	//-----------------------------------------------------------------------
	CXTPObservable();

	//-----------------------------------------------------------------------
	// Summary: Handles observable object destruction
	//-----------------------------------------------------------------------
	~CXTPObservable();

public:
	//-----------------------------------------------------------------------
	// Summary: Subscribes a unique observer for events.
	// Parameters:
	//     pObserver - A pointer to observer object to be subscribed to events.
	//     pSubcriptionList - An optional pointer to the subscriber list. If
	//         provided the observable object pointer is added to the subscription
	//         list.
	//-----------------------------------------------------------------------
	void Subscribe(Observer* pObserver, CXTPSubscriptionList<Owner, Observer>* pSubcriptionList = NULL);

	//-----------------------------------------------------------------------
	// Summary: Unsubscribes an observer from events.
	// Parameters:
	//     pObserver - A pointer to observer object to unsubscribed from events.
	// Remarks:
	//     If an observer has been previously subscribed with subscription list
	//     pointer provided, the subscription list is notified about observer
	...
	//-----------------------------------------------------------------------
	// Summary: Notifies all subscribed observers in the order of subscription
	//          about an event determined by observer's method to be called for
	//          each subscribed observer.
	// Parameters:
	//     method - A pointer to observer's method to be called for each 
	//              subscribed observer.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary: Notifies all subscribed observers in the reversed order of 
	//          subscription about an event determined by observer's method 
	...
	void NotifyDynamic(void(T::* method)(O*))
	{
		...
		for (INT_PTR i = 0; i < m_Observers.GetSize(); ++i)
		{
			if (!m_Observers[i].bRemove)
			{
				T* pCastedObserver = reinterpret_cast<T*>(static_cast<CXTPTypeIdProvider<Observer>*>(
					m_Observers[i].pObserver)->TryCastTo(specializedTypeId));
	...
	//-----------------------------------------------------------------------
	// Summary: Notifies all subscribed observers in the reversed order of subscription
	//          about an event determined by observer's method to be called for 
	//          each subscribed observer.
	// Parameters:
	//     method - A pointer to observer's method to be called for each 
	...
	//-----------------------------------------------------------------------
	// Summary: Notifies all subscribed observers in the order of subscription
	//          about an event determined by observer's method to be called 
	...
	void NotifyDynamic(void(T::* method)(O*, A1, A2), A1 arg1, A2 arg2)
	{
		...
		for (INT_PTR i = 0; i < m_Observers.GetSize(); ++i)
		{
			if (!m_Observers[i].bRemove)
			{
				...
				if (NULL != pCastedObserver)
				{
					(pCastedObserver->*method)(static_cast<O*>(static_cast<Owner*>(this)), arg1, arg2);
				}
			}
		}
	...
	void NotifyDynamic(void(T::* method)(O*, A1, A2, A3), A1 arg1, A2 arg2, A3 arg3)
	{
		...
		for (INT_PTR i = 0; i < m_Observers.GetSize(); ++i)
		{
			if (!m_Observers[i].bRemove)
			{
				...
				if (NULL != pCastedObserver)
				{
					(pCastedObserver->*method)(static_cast<O*>(static_cast<Owner*>(this)), arg1, arg2, arg3);
				}
			}
		}
...
AFX_INLINE CXTPSubscriptionList<Observable, Observer>::~CXTPSubscriptionList()
{
	...
	ASSERT("Unable to unsubscribe from all subscriptions. The dangling observer pointer can lead to unspecified behavior."
...
template<class Owner, class Observer>
AFX_INLINE CXTPObservable<Owner, Observer>::CXTPObservable() : m_nNotificationDepth(0) { }
...
template<class Owner, class Observer>
AFX_INLINE void CXTPObservable<Owner, Observer>::Subscribe(Observer* pObserver,
...
// Returns:
//     TRUE if the derived class has the base class as its parent.
// See Also:
//     XTP_DYNAMIC_DOWNCAST_REMOTE,XTPDynamicDownCastRemote,XTPIsKindOfRemote
//-----------------------------------------------------------------------
_XTP_EXT_CLASS BOOL AFX_CDECL XTPIsDerivedFromRemote(
...
//-----------------------------------------------------------------------
_XTP_EXT_CLASS BOOL AFX_CDECL XTPIsKindOfRemote(
...
//     the function returns the appropriate pointer. If it is not a legal cast, 
//     the function returns NULL.
// See Also:
//     XTP_DYNAMIC_DOWNCAST_REMOTE,XTPIsKindOfRemote,XTPIsDerivedFromRemote
//-----------------------------------------------------------------------
_XTP_EXT_CLASS CObject* AFX_CDECL XTPDynamicDownCastRemote(
	CRuntimeClass* pClass, CObject* pObject);
...
// Returns:
//     If the object referenced by the pointer is a "kind of" the identified class, 
//     the macro returns the appropriate pointer. If it is not a legal cast, 
...
//=======================================================================
// Summary:
//      A wrapper around Toolhelp32 API. API is loaded dynamically
//      and IsInitialized can be used for checking if API is supported.
...
public:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      The method should be used for checking if Toolhelp32 API
	//      is supported and can be used.
	// Returns:
	//      TRUE if Toolhelp32 API is supported and can be used.
	//-----------------------------------------------------------------------
