...
CXTPResourceManager::CXTPW2A::CXTPW2A(LPCWSTR lpwstr)
{
	...
	m_lpsrt = new CHAR[XTPToUIntChecked(nDestLen) + 1];
...
CXTPResourceManager::CXTPA2W::CXTPA2W(LPCSTR lptstr)
{
	...
	m_lpwsrt = new WCHAR[XTPToUIntChecked(nDestLen) + 1];
...
void CXTPResourceManager::SetResourceFile(const CString& strResourceFile)
{
	...
	else
	{
		...
		{
			SetResourceHandle(XTPGetInstanceHandle());
		}
...
BOOL CXTPResourceManager::IsIntResource(LPCTSTR lpName, BOOL bTryParse /*= FALSE*/)
{
	return bTryParse ? 0 != MakeIntResource(lpName) : IS_INTRESOURCE(lpName);
}

WORD CXTPResourceManager::MakeIntResource(LPCTSTR lpName)
{
	ASSERT(NULL != lpName);
	...
	{
		wID = (WORD)(ULONG_PTR)lpName;
	}
	else if(_T('#') == *lpName)
	{
		wID = (WORD)_ttoi(lpName + 1);
	}
...
BOOL CXTPResourceManager::LoadBitmap(CBitmap* pBitmap, LPCTSTR lpszResourceName)
{
	HMODULE hModule = GetResourceHandle();

	if (hModule && ::FindResource(hModule, lpszResourceName, RT_BITMAP))
	{
		if (pBitmap->Attach(::LoadBitmap(hModule, lpszResourceName)))
			return TRUE;
...
{
	return LoadBitmap(pBitmap, MAKEINTRESOURCE(nIDResource));
}
...
{
	return LoadHTML(pText, MAKEINTRESOURCE(nIDResource));
}

BOOL CXTPResourceManager::LoadHTML(CString* pText, LPCTSTR pszResource)
{
	if (pText == NULL)
		return FALSE;

	HMODULE hResourceInstanse = GetResourceHandle();
	HRSRC hResource = ::FindResource(hResourceInstanse, pszResource, RT_HTML);

	if (hResource == NULL)
	{
		hResourceInstanse = AfxFindResourceHandle(pszResource, RT_HTML);
		if (hResourceInstanse == NULL)
			return FALSE;

		hResource = ::FindResource(hResourceInstanse, pszResource, RT_HTML);
	}
	...
	TCHAR *pDst = pText->GetBuffer(XTPToIntChecked(dwDstSize));
	if (pDst == NULL)
		return FALSE;
...
#ifdef _UNICODE
	int nLen = ::MultiByteToWideChar(CP_ACP, 0, (LPCSTR)pSrc, XTPToIntChecked(dwSize),
		pDst, XTPToIntChecked(dwDstSize));
#else
	::CopyMemory(pDst, pSrc, dwSize);

	int nLen = XTPToIntChecked(dwSize);
#endif
...
int CXTPResourceManager::GetMenuLocaleString(CMenu* pMenu, UINT nIDItem, CString& rString, UINT nFlags) const
{
	...
	if (nStringLen > 0)
	{
		...
		{
			LPWSTR lpwstr = new WCHAR[XTPToUIntChecked(nStringLen) + 1];
...
public:
	...
	UINT GetLength() const
	{
		UINT nLength = sizeof(MENUITEMTEMPLATEHEADER);
...
BOOL CXTPResourceManagerXML::CreateMenu(CMenu& menu, CXTPPropExchange* pPX)
{
	...
	UINT nLength = menuTemplate.GetLength() + sizeof(MENUITEMTEMPLATE);
...
{
	UINT nDlgLength = dlgTemplate.GetLength();
	...
	*lpDlgTemplate = dlgTemplate;
	...
	if (dlgTemplate.style & DS_SETFONT)
	{
		*((WORD *)pMain) = (WORD)dlgTemplate.nPointSize;
		...
		MBSTOWCS_S((LPWSTR)pMain, dlgTemplate.strFaceName,
			XTPToSizeTChecked(dlgTemplate.strFaceName.GetLength()) + 1);
	}
	...
	for (int i = 0; i < aItems.GetSize(); i++)
	{
		...
		*(DLGITEMTEMPLATEEX*)pMain = itemInfo;
		...
		// Class
		MBSTOWCS_S((LPWSTR)pControlMain, itemInfo.strClassName,
			XTPToSizeTChecked(itemInfo.strClassName.GetLength()) + 1);
...
LPDLGTEMPLATE CXTPResourceManagerXML::CreateDialogTemplate(CXTPPropExchange* pPX)
{
	...
	PX_DPI_Rect(pPX, _T("position"), rc, CRect(0, 0, 0, 0));
	PX_String(pPX, _T("fontface"), dlgTemplate.strFaceName, _T("MS Sans Serif"));
	PX_DPI_Y_Int(pPX, _T("fontsize"), dlgTemplate.nPointSize, 8);
	...
	while (pos)
	{
		...
		PX_DPI_Rect(&sec, _T("position"), rc, CRect(0, 0, 0, 0));
		PX_Bstr(&sec, _T("caption"), pItemInfo->lpszCaption, L"");
...
LPCTSTR XTPGetDefaultResourceTypeFromPath(LPCTSTR lpszPath)
{
	...
	types[] =
	{
#define RES_TYPE_ENTRY(ext, type) { ext, _countof(ext) - 1, type }
		RES_TYPE_ENTRY(_T(".png"), _T("PNG")),
		RES_TYPE_ENTRY(_T(".bmp"), RT_BITMAP),
		RES_TYPE_ENTRY(_T(".ico"), RT_ICON),
		RES_TYPE_ENTRY(_T(".htm"), RT_HTML),
		RES_TYPE_ENTRY(_T(".html"), RT_HTML),
		RES_TYPE_ENTRY(_T(".js"), _T("JS")),
		RES_TYPE_ENTRY(_T(".vbs"), _T("VBS"))
	};

	LPCTSTR lpPathEnd = lpszPath + _tcslen(lpszPath);
	for (int i = 0; i < _countof(types); ++i)
	{
		LPCTSTR lpExt = lpPathEnd - types[i].cchExt;
		if (lpszPath < lpExt)
		{
			if (0 == _tcsicmp(lpExt, types[i].lpExt))
			{
				lpType = types[i].lpType;
				break;
			}
		}
	}
...
struct FIND_RES_TYPE_DATA
{
	LPCTSTR lpName;
	CString strType;
};

static BOOL CALLBACK FindResourceTypeEnumProc(
	HMODULE hModule,
	LPTSTR lpType,
	LONG_PTR lParam)
{
	FIND_RES_TYPE_DATA* pFindData = reinterpret_cast<FIND_RES_TYPE_DATA*>(lParam);
	BOOL bFound = FALSE;

	if (NULL != FindResource(hModule, pFindData->lpName, lpType))
	{
		WORD wType = CXTPResourceManager::MakeIntResource(lpType);
		if (0 != wType)
		{
			pFindData->strType.Format(_T("#%i"), wType);
		}
		else
		{
			pFindData->strType = lpType;
		}
...
CString XTPFindResourceType(HMODULE hModule, LPCTSTR lpszName)
{
	ASSERT(NULL != lpszName);

	FIND_RES_TYPE_DATA findData;
	findData.lpName = lpszName;
	EnumResourceTypes(hModule, FindResourceTypeEnumProc, reinterpret_cast<LPARAM>(&findData));

	return findData.strType;
}
