...
//{{AFX_CODEJOCK_PRIVATE
#define XTP_IMPLEMENT_CHECKED_CASTING(fromType, toType, value, predicate, casting, defaultValue) \
	BOOL bChecked = (predicate); \
	if(XTP_CASTING_ASSERTIONS_ENABLED) ASSERT("'" #fromType "' value truncated to '" #toType "'.\n\n" \
		"To disable noisy assertions and take the risk of possible data loss " \
		"re-compile with XTP_CASTING_ASSERTIONS_ENABLED defined to FALSE or 0." && bChecked); \
...
#define XTP_IMPLEMENT_CHECKED_STATIC_CASTING(fromType, toType, value, predicate, defaultValue) \
	XTP_IMPLEMENT_CHECKED_CASTING(fromType, toType, value, predicate, (static_cast<toType>(value)), defaultValue)
...
#define XTP_IMPLEMENT_STRICT_CASTING(fromType, toType, value, format, predicate, casting) \
	if (!(predicate)) { \
		if(XTP_CASTING_ASSERTIONS_ENABLED) ASSERT("'" #fromType "' value cannot be cast to '" #toType "'.\n\n" \
		"To disable noisy assertions and take the risk of possible data loss " \
		"re-compile with XTP_CASTING_ASSERTIONS_ENABLED defined to FALSE or 0." && FALSE); \
		TRACE(XTP_CASTING_T(fromType) _T("(") _T(format) _T(") cannot be cast to '") XTP_CASTING_T(toType) _T("'"), toType(value)); \
	} \
	return (casting);

#define XTP_IMPLEMENT_STRICT_STATIC_CASTING(fromType, toType, value, format, predicate) \
	XTP_IMPLEMENT_STRICT_CASTING(fromType, toType, value, format, predicate, (static_cast<toType>(value)))

#define XTP_IMPLEMENT_SELF_CASTING(name, type) \
	AFX_INLINE type XTPTo##name(type value) { return value; } \
	AFX_INLINE type XTPTo##name##Checked(type value) { return value; } \
	AFX_INLINE type XTPTo##name##Strict(type value) { return value; }

#define XTP_IMPLEMENT_LARGER_TO_SMALLER_CASTING(name, fromType, toType, toMax, format) \
	AFX_INLINE toType XTPTo##name(fromType value) { return static_cast<toType>(value); } \
	AFX_INLINE toType XTPTo##name##Checked(fromType value) \
		{ XTP_IMPLEMENT_CHECKED_STATIC_CASTING(fromType, toType, value, (value <= toMax), toMax); }\
	AFX_INLINE toType XTPTo##name##Strict(fromType value) \
		{ XTP_IMPLEMENT_STRICT_STATIC_CASTING(fromType, toType, value, format, (value <= toMax)); }

#define XTP_IMPLEMENT_SIGNED_TO_UNSIGNED_CASTING(name, fromType, toType, format) \
	AFX_INLINE toType XTPTo##name(fromType value) { return static_cast<toType>(value); } \
	AFX_INLINE toType XTPTo##name##Checked(fromType value) \
		{ XTP_IMPLEMENT_CHECKED_STATIC_CASTING(fromType, toType, value, (fromType(0) <= value), 0); }\
	AFX_INLINE toType XTPTo##name##Strict(fromType value) \
		{ XTP_IMPLEMENT_STRICT_STATIC_CASTING(fromType, toType, value, format, (fromType(0) <= value)); }

#define XTP_IMPLEMENT_INTERPRETED_CASTING(name, fromType, toType) \
	AFX_INLINE toType XTPTo##name(fromType value) { return reinterpret_cast<toType>(value); } \
