...
//-----------------------------------------------------------------------
#ifdef _DEBUG
#define XTP_ASSERT_CHECK(expr) ((expr) || !::AfxAssertFailedLine(THIS_FILE, __LINE__) || (XTPDebugBreak(), 0))
#else
...
#define _XTP_DEPRECATED	\
	_XTP_DEPRECATE("Deprecated in " \
		XTP_STRINGIZE(_XTPLIB_VERSION_MAJOR) "." \
		XTP_STRINGIZE(_XTPLIB_VERSION_MINOR) "." \
		XTP_STRINGIZE(_XTPLIB_VERSION_REVISION) "." \
		XTP_STRINGIZE(_XTPLIB_VERSION_BUILD))
...
#define XTP_BLOCKSTATEMENT(Type, Lhs, Rhs) \
	if (Type Lhs = Rhs) { while (true) { ; } } else
