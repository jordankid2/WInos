...
public:
	...
	CXTPPenDC(HDC hDC, int nStyle, int nWidth, COLORREF crColor); // <combine CXTPPenDC::CXTPPenDC@CDC*@CPen*>
...
public:
	...
	//-----------------------------------------------------------------------
	// Summary:    Call this member function to draw a 1 or 2 pixel wide border
	//             using the colors specified by pOutsideClr and pInsideClr.  If 
	//             pInsideClr is NULL, a 1 pixel border is drawn.
	...
	//-----------------------------------------------------------------------
	static void AFX_CDECL DrawBorder(CDC* pDC, CRect rBorder, COLORREF* pOutsideClr, COLORREF* pInsideClr = NULL);
	...
	//-----------------------------------------------------------------------
	// Summary:    Checks for the system for non standard character set and
	//             retrieves DEFAULT_GUI_FONT object.  If a standard character
	//             set is used then SPI_GETICONTITLELOGFONT is used to retrieve
	//             system font information.  If bModernFont is specified the
	...
	// Parameters: pLogFont    : Destination LOGFONT structure to recive font
	//                           information.
	...
	//-----------------------------------------------------------------------
	static BOOL AFX_CDECL GetDefaultLogFont(LOGFONT* pLogFont, BOOL bModernFont = TRUE, BOOL bVertFont = FALSE);

    //-----------------------------------------------------------------------
    // Summary:    Call this member function to search for and replace a single
	...
	static void AFX_CDECL RGBtoHSL(COLORREF clr, double& h, double& s, double& l);
    ...
    //-----------------------------------------------------------------------
    // Summary:    This member function will either lighten or darken the color 
    ...
    // Parameters: clr      - RGB value representing the color to adjust.
	//             nPercent - Amount to adjust the color by, must be a value 
	//                        between -100 and +100 percent.
    // Returns:    An RGB value representing the adjusted color value. If the
	...
	//-----------------------------------------------------------------------
	// Summary:    Call this member function to convert the specified color to
	//             greyscale values.
	// Parameters: clr : RGB value of the color to convert to greyscale.
	// Returns:    An RGB value representing a greyscale version of the 
	...
	//-----------------------------------------------------------------------
	static int AFX_CDECL DrawFontText(
		CDC* pDC,
	...
	//-----------------------------------------------------------------------
	static void AFX_CDECL Triangle(CDC* pDC, CPoint pt0, CPoint pt1, CPoint pt2, COLORREF clrPen, COLORREF clrBrush = COLORREF_NULL);
	...
	//-----------------------------------------------------------------------
	// Summary: Call this member function to draw an expand triangle commonly
	//          used on tree controls or other controls that need to show the
	//          expanded state of a list or group of items.
	// Input:   pDC       - Points to the device context
	...
	//-----------------------------------------------------------------------
	static void AFX_CDECL DrawExpandTriangle(CDC* pDC, CPoint pt, BOOL bExpanded, COLORREF clrPen, COLORREF clrBrush = COLORREF_NULL);

	//-----------------------------------------------------------------------
	// Summary: Call this member function to draw an expand arrow commonly
	//          used on popup windows or other controls that need to show the
	...
	static BOOL AFX_CDECL IsVScrollOnTheLeft(HWND hWnd); // <combine CXTPDrawHelpers::IsVScrollOnTheLeft@CWnd*>
	...
	//      l         : Left edge of rectangle.
	//      t         : Top edge of rectangle.
	//      cx        : Width of rectangle.
	//      cy        : Height of rectangle.
	//		clrFill   : Background color for the rectangle.
	//		clrBorder : Border color for the rectangle.
	//-----------------------------------------------------------------------
	static void AFX_CDECL DrawColorRect(CDC* pDC, CRect rColor, COLORREF clrFill, COLORREF clrBorder);
	static void AFX_CDECL DrawColorRect(CDC* pDC, int l, int t, int cx, int cy, COLORREF clrFill, COLORREF clrBorder);
	...
	//-----------------------------------------------------------------------
	static void AFX_CDECL DrawHatchRect(CDC* pDC, CRect rColor, COLORREF clrFill, COLORREF clrBorder);
	...
	//-----------------------------------------------------------------------
	static void AFX_CDECL CreatePatternBrush(CBitmap& bmp, CBrush& brush, COLORREF clrFill, const void* pBitsBrush);
	...
	//                   values that represent an 8 x 8 pixel pattern.
	//-----------------------------------------------------------------------
	static void AFX_CDECL DrawBrushRect(CDC* pDC, CRect rColor, COLORREF clrFill, COLORREF clrBorder, const void* pBitsBrush);

	//-----------------------------------------------------------------------
	// Summary:
	//		Computes font height such that the string provided fits the specified rectangle
	//		as tight as possible. 
	// Parameters:
	//		pDC       - Pointer to a valid device context.
	//		lpStr     - Pointer to a string which is to be fitted into the specified rectangle.
	//		cch       - Number of characters to use from the string provided or -1 to 
	//		            compute the length of the string.
	//		rc        - Rectangle to contain the string specified.
	//		fntTest   - Font used to draw the text specified. Font height is ignored. Font width has to be 0.
	// Returns:
	//		If succeedes the return value is the height of the font necessary to fit the specified
	//		rectangle as tight as possible, oterwise the return value is 0.
	//-----------------------------------------------------------------------
	static int ComputeFittingFontHeight(CDC* pDC, LPCTSTR lpStr, int cch, const CRect& rc, CFont& fntTest);

	//-----------------------------------------------------------------------
	// Summary:
	//		Draws a stretched image from image list.
	// Parameters:
	//		pDC        - Pointer to a valid device context where the image is to be drawn to.
	//		rcDest     - Destination rectangle.
	//		pImageList - A valid image list pointer.
	//		nImage     - Image index to draw.
	// Returns:
	//		TRUE is an image is drawn successfully.
	//-----------------------------------------------------------------------
	static BOOL AFX_CDECL StretchImageListImage(CDC* pDC, CRect rcDest, CImageList* pImageList, int nImage);

	//-----------------------------------------------------------------------
	// Summary:
	//		Processes all pending WM_PAINT message in the message queue of the calling thread.
	//		Ensures the painting never causes deadlock.
	// Parameters:
...
//{{AFX_CODEJOCK_PRIVATE
	_XTP_DEPRECATE("This function is no longer available. Please use 'XTPSystemVersion()->GetComCtlVersion' instead")
	static DWORD AFX_CDECL GetComCtlVersion();
	_XTP_DEPRECATE("This function is no longer available. Please use 'XTPMultiMonitor()->GetWorkArea' instead")
	...
	_XTP_DEPRECATE("This function is no longer available. Please use 'XTPMultiMonitor()->GetScreenArea' instead")
...
public:
	...
	static void AFX_CDECL DoInsertHFFormatSpecifierViaMenu(CWnd* pParent, CEdit* pEdit, CButton* pButton);
	//}}AFX_CODEJOCK_PRIVATE
...
//-------------------------------------------------------------------------
// Summary:
//      Obtains native handle of the object specified.
// Returns:
...
//      documentation for it's analogue called PROCESS_DPI_AWARENESS.
//-------------------------------------------------------------------------
...
public:
	...
	//-------------------------------------------------------------------------
	// Summary:
	//      Constructs DPI helper object that works with global DPI settings.
	//-------------------------------------------------------------------------
	CXTPDpi();

	//-------------------------------------------------------------------------
	// Summary:
	//      Constructs DPI helper object that works with provided device context's settings.
	// Parameters:
	//		hDC - Device context for which DPI settings to be used.
	//-------------------------------------------------------------------------
	CXTPDpi(HDC hDC);

	//-------------------------------------------------------------------------
	// Summary:
	//      Constructs DPI helper object that works with provided window's 
	//      device context's settings.
	// Parameters:
	//		hWnd - Window handle for which DPI settings to be used.
	//-------------------------------------------------------------------------
	CXTPDpi(HWND hWnd);
	...
	//      DPI awareness level in application's manifest file. Refer to platform 
	//      SDK's documentation for SetProcessDpiAwareness and SetProcessDpiAware
	//      details and support OS versions.
	// Parameters:
	//      dpiAwareness - DPI awareness level that is to be set only if supported
	//      by OS.
	// Returns:
	//      TRUE if succeeded.
	//-------------------------------------------------------------------------
	...
	//      SDK's documentation for GetProcessDpiAwareness details and support OS versions.
	// Returns:
	//      DPI awareness level that enabled for the calling process.
	//-------------------------------------------------------------------------
	XTPDpiAwareness GetDpiAwareness() const;

	//-------------------------------------------------------------------------
	// Summary:
	//      Processes DPI related message. Must not be called for DPI helper 
	//      object specific to device context or window handle.
	// Parameters:
	//      hWnd - Window handle.
	//      message - Message code.
	//      wParam, lParam - Message arguments.
	//      pResult - Pointer to the result of message processing.
	// Returns:
	//      TRUE if a message has been processed.
	//-------------------------------------------------------------------------
	BOOL OnWndMsg(
		HWND hWnd,
		UINT message,
	...
	//      pRect   - a pointer to rectangle object to scale/unscale using current DPI settings.
	//      rc      - a reference to rectangle object to scale/unscale using current DPI settings.
	//      pPoint  - a pointer to point object to scale/unscale using current DPI settings.
	//      pt      - a reference to point object to scale/unscale using current DPI settings.
	//      pSize   - a pointer to size object to scale/unscale using current DPI settings.
	//      sz      - a reference to size object to scale/unscale using current DPI settings.
	//      iDpiScaleAbove - Scaled only if current DPI larger than specified.
	// Returns:
	//      Scaled/unscaled value.
	//-------------------------------------------------------------------------
	int ScaleX(int x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const;
	int ScaleY(int y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	float ScaleX(float x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	float ScaleY(float y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	double ScaleX(double x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	double ScaleY(double y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	unsigned int ScaleX(unsigned int x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	unsigned int ScaleY(unsigned int y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	int UnscaleX(int x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	int UnscaleY(int y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	float UnscaleX(float x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	float UnscaleY(float y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	double UnscaleX(double x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	double UnscaleY(double y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	unsigned int UnscaleX(unsigned int x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	unsigned int UnscaleY(unsigned int y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>

	int ScaleX(HDC hDC, int x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const;
	int ScaleY(HDC hDC, int y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	float ScaleX(HDC hDC, float x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	float ScaleY(HDC hDC, float y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	double ScaleX(HDC hDC, double x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	double ScaleY(HDC hDC, double y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	unsigned int ScaleX(HDC hDC, unsigned int x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	unsigned int ScaleY(HDC hDC, unsigned int y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	int UnscaleX(HDC hDC, int x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	int UnscaleY(HDC hDC, int y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	float UnscaleX(HDC hDC, float x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	float UnscaleY(HDC hDC, float y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	double UnscaleX(HDC hDC, double x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	double UnscaleY(HDC hDC, double y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	unsigned int UnscaleX(HDC hDC, unsigned int x, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	unsigned int UnscaleY(HDC hDC, unsigned int y, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>

	void ScaleRect(RECT *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	RECT ScaleRect(const RECT& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscaleRect(RECT *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	RECT UnscaleRect(const RECT& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(RECT *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	RECT Scale(const RECT& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(RECT *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	RECT Unscale(const RECT& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void ScaleRect(CRect *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CRect ScaleRect(const CRect& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscaleRect(CRect *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CRect UnscaleRect(const CRect& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(CRect *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CRect Scale(const CRect& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(CRect *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CRect Unscale(const CRect& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>

	void ScaleRect(HDC hDC, RECT *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	RECT ScaleRect(HDC hDC, const RECT& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscaleRect(HDC hDC, RECT *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	RECT UnscaleRect(HDC hDC, const RECT& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(HDC hDC, RECT *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	RECT Scale(HDC hDC, const RECT& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(HDC hDC, RECT *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	RECT Unscale(HDC hDC, const RECT& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void ScaleRect(HDC hDC, CRect *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CRect ScaleRect(HDC hDC, const CRect& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscaleRect(HDC hDC, CRect *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CRect UnscaleRect(HDC hDC, const CRect& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(HDC hDC, CRect *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CRect Scale(HDC hDC, const CRect& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(HDC hDC, CRect *pRect, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CRect Unscale(HDC hDC, const CRect& rc, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>

	void ScalePoint(POINT *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	POINT ScalePoint(const POINT& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscalePoint(POINT *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	POINT UnscalePoint(const POINT& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(POINT *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	POINT Scale(const POINT& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(POINT *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	POINT Unscale(const POINT& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void ScalePoint(CPoint *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CPoint ScalePoint(const CPoint& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscalePoint(CPoint *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CPoint UnscalePoint(const CPoint& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(CPoint *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CPoint Scale(const CPoint& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(CPoint *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CPoint Unscale(const CPoint& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>

	void ScalePoint(HDC hDC, POINT *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	POINT ScalePoint(HDC hDC, const POINT& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscalePoint(HDC hDC, POINT *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	POINT UnscalePoint(HDC hDC, const POINT& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(HDC hDC, POINT *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	POINT Scale(HDC hDC, const POINT& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(HDC hDC, POINT *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	POINT Unscale(HDC hDC, const POINT& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void ScalePoint(HDC hDC, CPoint *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CPoint ScalePoint(HDC hDC, const CPoint& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscalePoint(HDC hDC, CPoint *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CPoint UnscalePoint(HDC hDC, const CPoint& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(HDC hDC, CPoint *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CPoint Scale(HDC hDC, const CPoint& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(HDC hDC, CPoint *pPoint, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CPoint Unscale(HDC hDC, const CPoint& pt, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>

	void ScaleSize(SIZE *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	SIZE ScaleSize(const SIZE& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscaleSize(SIZE *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	SIZE UnscaleSize(const SIZE& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(SIZE *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	SIZE Scale(const SIZE& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(SIZE *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	SIZE Unscale(const SIZE& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void ScaleSize(CSize *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CSize ScaleSize(const CSize& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscaleSize(CSize *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CSize UnscaleSize(const CSize& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(CSize *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CSize Scale(const CSize& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(CSize *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CSize Unscale(const CSize& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>

	void ScaleSize(HDC hDC, SIZE *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	SIZE ScaleSize(HDC hDC, const SIZE& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscaleSize(HDC hDC, SIZE *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	SIZE UnscaleSize(HDC hDC, const SIZE& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(HDC hDC, SIZE *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	SIZE Scale(HDC hDC, const SIZE& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(HDC hDC, SIZE *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	SIZE Unscale(HDC hDC, const SIZE& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void ScaleSize(HDC hDC, CSize *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CSize ScaleSize(HDC hDC, const CSize& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void UnscaleSize(HDC hDC, CSize *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CSize UnscaleSize(HDC hDC, const CSize& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Scale(HDC hDC, CSize *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CSize Scale(HDC hDC, const CSize& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	void Unscale(HDC hDC, CSize *pSize, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>
	CSize Unscale(HDC hDC, const CSize& sz, int iDpiScaleAbove = CXTPDpi::DefaultDpi) const; // <combine CXTPDpi::ScaleX@int@int>

	//-------------------------------------------------------------------------
	// Summary:
	//      Selects one of the provided values that is specific to the current screen DPI.
	// Parameters:
	//      upToDpi120 - The value that is selected if current DPI is in range [96-120).
	//      upToDpi144 - The value that is selected if current DPI is in range [120-144).
	//      upToDpi192 - The value that is selected if current DPI is in range [144-192).
	//      theLargest - The value that is selected if current DPI is in range [192-*).
	// Returns:
	...
	LPCTSTR SelectDpiSpecific(LPCTSTR upToDpi120, LPCTSTR upToDpi144, LPCTSTR upToDpi192, LPCTSTR theLargest) const; // <combine CXTPDpi::SelectDpiSpecific@int@int@int@int>
	void* SelectDpiSpecific(void* upToDpi120, void* upToDpi144, void* upToDpi192, void* theLargest) const;
	...
	int ScaledScreenHeight() const; // <combine CXTPDpi::ScaledScreenWidth>
...
private:
	...
	typedef HRESULT (WINAPI* PFNSetProcessDpiAwareness)(
		XTPDpiAwareness dpiAwareness);
	typedef HRESULT (WINAPI* PFNGetProcessDpiAwareness)(
		HANDLE hProcess, XTPDpiAwareness *dpiAwareness);
	...
	typedef HANDLE(WINAPI* PFNMonitorFromWindow)(
...
//-------------------------------------------------------------------------
// Summary:
//      Helper macros for scalling of X and Y coordinates, rectangle, 
//      size and point objects according to active DPI settings.
//-------------------------------------------------------------------------
...
#define XTP_DC_DPI(dc, what) XTPDpiHelper()->Scale(XTPGetHandle(dc), what) // <combine XTP_DPI_X>
...
//-------------------------------------------------------------------------
// Summary:
//      Helper macros for unscalling of X and Y coordinates, rectangle, 
//      size and point objects according to active DPI settings.
//-------------------------------------------------------------------------
...
#define XTP_DC_UNDPI_Y(dc, n) XTPDpiHelper()->UnscaleY(XTPGetHandle(dc), n) // <combine XTP_UNDPI_X>
#define XTP_DC_UNDPI(dc, what) XTPDpiHelper()->Unscale(XTPGetHandle(dc), what) // <combine XTP_UNDPI_X>
...
{
	xtpPrimitiveSymbolInvalid = -1,
	xtpPrimitiveSymbolFirst = 0x27,

	// Common glyphs
	xtpPrimitiveSymbolMinimize = xtpPrimitiveSymbolFirst,
	xtpPrimitiveSymbolMaximize,
	xtpPrimitiveSymbolRestore,
	xtpPrimitiveSymbolQuestionMark,
	xtpPrimitiveSymbolClose,
	xtpPrimitiveSymbolHome,
	...
	xtpPrimitiveSymbolPlayUp,
	xtpPrimitiveSymbolPlayDown,

	// Small Play glyph
	xtpPrimitiveSymbolPlayLeft2,
	xtpPrimitiveSymbolPlayRight2,
	xtpPrimitiveSymbolPlayUp2,
	xtpPrimitiveSymbolPlayDown2,

	// Rewind glyph
	xtpPrimitiveSymbolRewindLeft,
	xtpPrimitiveSymbolRewindRight,
	xtpPrimitiveSymbolRewindUp,
	xtpPrimitiveSymbolRewindDown,

	// Shift glyph
	xtpPrimitiveSymbolShiftLeft,
	xtpPrimitiveSymbolShiftRight,
	xtpPrimitiveSymbolShiftDown,
	xtpPrimitiveSymbolShiftUp,
	
	// Eject glyph
	xtpPrimitiveSymbolEjectLeft,
	xtpPrimitiveSymbolEjectRight,
	xtpPrimitiveSymbolEjectUp,
	xtpPrimitiveSymbolEjectDown,

	// Inject glyph
	xtpPrimitiveSymbolInjectLeft,
	xtpPrimitiveSymbolInjectRight,
	xtpPrimitiveSymbolInjectUp,
	xtpPrimitiveSymbolInjectDown,

	// Pin glyph
	xtpPrimitiveSymbolPinLeft,
	xtpPrimitiveSymbolPinRight,
	xtpPrimitiveSymbolPinDown,

	xtpPrimitiveSymbolPlus,
	xtpPrimitiveSymbolMinus,
	xtpPrimitiveSymbolEllipsis,

	xtpPrimitiveSymbolTabClose,
	xtpPrimitiveSymbolTabPlayLeft,
	xtpPrimitiveSymbolTabPlayRight,
	xtpPrimitiveSymbolTabPlayLeftDisabled,
	xtpPrimitiveSymbolTabPlayRightDisabled,
...
public:
	...
	//		rc - Target rectangle.
	//		crForeColor - Foregroung color value or COLORREF_NULL if left it unchanged.
	//		crBackColor - Backgroung color value or COLORREF_NULL if left it unchanged.
	...
	// Remarks:
	//		Starting from Windows 10 when Untrusted Font Blocking feature was added to OS
	//		this method may stop working which results in no primitives drawn.
	//		The Untrusted Font Blocking feature prevents applications from loading memory
	//		fonts which is required for the correct work of this method.
	//
	//		The issue can be fixed in 2 possible ways:
	//			1. Installing Codejock.ttf file into %WINDIR%\Fonts of that host OS prior
	//				to executing an application that draws Codejock primitives.
	//			2. Less recommended. Exclude application process from Untrusted Font Blocking by adding
	//				a new empty registry key HKEY_LOCAL_MACHINE\Software\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\<process_image_name>.
	//			3. No recommended. Disabling Untrusted Font Blocking in Group Policy editor (dpedit.msc):
	//				Computer Configuration\Administrative Templates\System\Mitigation Options\Untrusted Font Blocking
	//
	//		Read more about Untrusted Font Blocking at: https://docs.microsoft.com/en-us/windows/threat-protection/block-untrusted-fonts-in-enterprise
	//-------------------------------------------------------------------------
	void DrawSymbol(
		CDC* pDC,
		XTPPrimitiveSymbol nSymbol,
		const CRect& rc,
		COLORREF crForeColor = COLORREF_NULL,
		COLORREF crBackColor = COLORREF_NULL,
		BOOL bOutline = FALSE,
...
//-------------------------------------------------------------------------
// Summary:
//      Provides access to global primitive drawer instance.
// See Also: CXTPPrimitiveDrawer
//-------------------------------------------------------------------------
