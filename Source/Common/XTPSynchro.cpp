...
long CXTPRWCriticalSection::IncrementLocalSharedLockCounter()
{
	...
	LocalSharedLockCounterMap* pMap = reinterpret_cast<LocalSharedLockCounterMap*>(
		TlsGetValue(GetLocalSharedLockCounterTlsIdx()));
	if (NULL == pMap)
	{
		pMap = new LocalSharedLockCounterMap();
		(*pMap)[this] = 1;
		TlsSetValue(GetLocalSharedLockCounterTlsIdx(), pMap);
		nCounter = 1;
	}
	else
	{
	...
	if (NULL != pMap)
	{
		...
		if (0 == nCounter)
		{
			if (pMap->RemoveKey(this))
			{
				...
				{
					delete pMap;
					TlsSetValue(GetLocalSharedLockCounterTlsIdx(), NULL);
				}
			}
		}
	}
...
DWORD CXTPRWCriticalSection::GetLocalSharedLockCounterTlsIdx()
{
	static DWORD dwTlsIdx = TlsAlloc();
	return dwTlsIdx;
}
