...
{
	::BitBlt(destHdc, 0, 0, srcRect.Width(), srcRect.Height(), srcHdc, srcRect.left, srcRect.top, SRCCOPY);
}

BEGIN_DISPATCH_MAP(CXTPMarkupButtonBase, CXTPMarkupContentControl)
	DISP_PROPERTY_EX_ID(CXTPMarkupButtonBase, "IsPressed", 700, OleGetIsPressed, OleSetIsPressed, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPMarkupButtonBase, "ClickEvent", 701, OleGetClickEvent, SetNotSupported, VT_DISPATCH)
END_DISPATCH_MAP()
