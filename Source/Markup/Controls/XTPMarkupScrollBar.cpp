...
int CXTPMarkupScrollBar::HitTestScrollBar(POINT pt) const
{
	...
	do
	{
		...
		if(m_spi.fVert)
		{
			CRect rcPart(m_spi.rc.left, m_spi.rc.top,
				m_spi.rc.right, m_spi.rc.top + m_spi.pxUpArrow);
			...
			rcPart.SetRect(m_spi.rc.left, m_spi.rc.top + m_spi.pxDownArrow,
				m_spi.rc.right, m_spi.rc.bottom);
			if(PtInRectTransform(rcPart, pt))
			{
			...
			rcPart.SetRect(m_spi.rc.left, m_spi.rc.top,
				m_spi.rc.right, m_spi.rc.top + m_spi.pxThumbTop);
			if(PtInRectTransform(rcPart, pt))
			{
		...
		{
			CRect rcPart(
				m_spi.rc.left, m_spi.rc.top,
				m_spi.rc.left + m_spi.pxUpArrow,
				m_spi.rc.bottom);
			if(PtInRectTransform(rcPart, pt))
			{
				htResult = XTP_HTSCROLLUP;
			...
			rcPart.SetRect(m_spi.rc.left + m_spi.pxDownArrow, m_spi.rc.top,
				m_spi.rc.right, m_spi.rc.bottom);
			if(PtInRectTransform(rcPart, pt))
			{
				htResult = XTP_HTSCROLLDOWN;
			...
			rcPart.SetRect(m_spi.rc.left, m_spi.rc.top,
				m_spi.rc.left + m_spi.pxThumbTop, m_spi.rc.bottom);
			if(PtInRectTransform(rcPart, pt))
			{
				htResult = XTP_HTSCROLLUPPAGE;
				break;
...
void CXTPMarkupScrollBar::PerformTrackInit(HWND hWnd, CPoint point, XTP_SCROLLBAR_POSINFO *pSBInfo, BOOL bDirect)
{
...
void CXTPMarkupScrollBar::CalcScrollBarInfo(LPRECT lprc, XTP_SCROLLBAR_POSINFO *pSBInfo, SCROLLINFO* pSI)
{
	ASSERT(NULL != pSBInfo);
	ASSERT(NULL != pSI);
	...
	if (0 < cWidth)
	{
		...
		if (!pSBInfo->fSizebox)
		{
			if (pSBInfo->fVert)
			{
				...
				{
					DrawFrameControl(hDC, &rcArrowUp, DFC_SCROLL, DFCS_SCROLLUP | (!bEnabled ? DFCS_INACTIVE : 0) | (nPressedHt == XTP_HTSCROLLUP ? DFCS_PUSHED : 0));
					DrawFrameControl(hDC, &rcArrowDown, DFC_SCROLL, DFCS_SCROLLDOWN | (!bEnabled ? DFCS_INACTIVE : 0) | (nPressedHt == XTP_HTSCROLLDOWN ? DFCS_PUSHED : 0));
					...
					{
						::SetBkColor(hDC, nPressedHt == XTP_HTSCROLLTHUMB ? m_crBackPushed : nHotHt == XTP_HTSCROLLTHUMB ? m_crBackHilite : m_crBack);
			...
			{
				CRect rcArrowLeft(rcScrollBar.left, rcScrollBar.top, pSBInfo->pxUpArrow, rcScrollBar.bottom);
				CRect rcArrowRight(pSBInfo->pxDownArrow, rcScrollBar.top, rcScrollBar.right, rcScrollBar.bottom);
				CRect rcTrack(rcArrowLeft.right, rcScrollBar.top, rcArrowRight.left, rcScrollBar.bottom);
				...
				{
					DrawFrameControl(hDC, &rcArrowLeft, DFC_SCROLL, DFCS_SCROLLLEFT | (!bEnabled ? DFCS_INACTIVE : 0) | (nPressedHt == XTP_HTSCROLLUP ? DFCS_PUSHED : 0));
					DrawFrameControl(hDC, &rcArrowRight, DFC_SCROLL, DFCS_SCROLLRIGHT | (!bEnabled ? DFCS_INACTIVE : 0) | (nPressedHt == XTP_HTSCROLLDOWN ? DFCS_PUSHED : 0));
