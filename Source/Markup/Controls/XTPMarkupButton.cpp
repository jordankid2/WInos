...
void CXTPMarkupButton::OnRender(CXTPMarkupDrawingContext* pDC)
{
	...
	else
	{
		...
		else
		{
			...
			pDC->FillRectangle(CRect(rc.left, rc.top, rc.right, m_pMarkupContext->ScaleY(1)), &brushLight);
			pDC->FillRectangle(CRect(rc.left, rc.top, rc.left + m_pMarkupContext->ScaleX(1), rc.bottom), &brushLight);

			pDC->FillRectangle(CRect(rc.left + m_pMarkupContext->ScaleX(1), rc.bottom - m_pMarkupContext->ScaleY(2), 
				rc.right - m_pMarkupContext->ScaleX(1), rc.bottom - m_pMarkupContext->ScaleY(1)), &brushDark);
			pDC->FillRectangle(CRect(rc.right - m_pMarkupContext->ScaleX(2), rc.top + m_pMarkupContext->ScaleY(1), 
				rc.right - m_pMarkupContext->ScaleX(1), rc.bottom - m_pMarkupContext->ScaleY(1)), &brushDark);

			pDC->FillRectangle(CRect(rc.left, rc.bottom - m_pMarkupContext->ScaleY(1), 
				rc.right - m_pMarkupContext->ScaleX(1), rc.bottom), &brushDarkDark);
			pDC->FillRectangle(CRect(rc.right - m_pMarkupContext->ScaleX(1), rc.top, 
				rc.right, rc.bottom), &brushDarkDark);
		}
	}
}
