...
CSize CXTPMarkupCheckBox::MeasureOverride(CXTPMarkupDrawingContext* pDC, CSize szAvailableSize)
{
	...
	if (pContent != NULL)
	{
		...
		return CSize(desiredSize.cx + size3.cx, max(m_pMarkupContext->ScaleY(13), desiredSize.cy + size3.cy));
	}
...
void CXTPMarkupCheckBox::OnRender(CXTPMarkupDrawingContext* pDC)
{
	...
	CRect rc(0, GetRenderSize().cy / 2 - m_pMarkupContext->ScaleY(6), 
		m_pMarkupContext->ScaleX(13), GetRenderSize().cy / 2 + m_pMarkupContext->ScaleY(7));
	...
	{
		DrawFrameControl(pDDC->GetDC(), rc, DFC_BUTTON, DFCS_BUTTONCHECK
			| (bPressed ? DFCS_PUSHED : 0)
			| (bChecked == BST_CHECKED ? DFCS_CHECKED : 0)
			| (bChecked == BST_INDETERMINATE ? DFCS_BUTTON3STATE | DFCS_CHECKED : 0));
	}
