// XTPMarkupSystemColorsStaticExtension.cpp: implementation of the CXTPMarkupSystemColorsStaticExtension class.
//
...
CXTPMarkupSystemColorStaticExtension::CXTPMarkupSystemColorStaticExtension(LPCWSTR lpzName, int nColorIndex)
...
CXTPMarkupSystemColorKeyStaticExtension::CXTPMarkupSystemColorKeyStaticExtension(LPCWSTR lpzName, int nColorIndex)
	: CXTPMarkupStaticExtensionProperty(lpzName)
{
...
CXTPMarkupSystemBrushStaticExtension::CXTPMarkupSystemBrushStaticExtension(LPCWSTR lpzName, int nColorIndex)
	: CXTPMarkupStaticExtensionProperty(lpzName)
{
...
CXTPMarkupSystemBrushKeyStaticExtension::CXTPMarkupSystemBrushKeyStaticExtension(LPCWSTR lpzName, int nColorIndex)
	: CXTPMarkupStaticExtensionProperty(lpzName)
{
...
#define COLOR_MENUBAR           30
	...
	systemColors[] =
	{
#define XTP_DEFINE_SYSTEM_ENTITY_NAMES(Name) Name L"Color", Name L"ColorKey", Name L"Brush", Name L"BrushKey"
		...
		{ XTP_DEFINE_SYSTEM_ENTITY_NAMES(L"GradientInactiveCaption"), COLOR_GRADIENTINACTIVECAPTION },
	...
	{
		Extend(new CXTPMarkupSystemColorStaticExtension(
			systemColors[i].lpzColorName,
			systemColors[i].nColorIndex));
		Extend(new CXTPMarkupSystemColorKeyStaticExtension(
			systemColors[i].lpzColorKeyName,
			systemColors[i].nColorIndex));
		Extend(new CXTPMarkupSystemBrushStaticExtension(
			systemColors[i].lpzBrushName,
			systemColors[i].nColorIndex));
		Extend(new CXTPMarkupSystemBrushKeyStaticExtension(
			systemColors[i].lpzBrushKeyName,
			systemColors[i].nColorIndex));
	}
}
