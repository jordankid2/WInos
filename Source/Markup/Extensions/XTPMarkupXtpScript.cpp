...
void CXTPMarkupXtpScript::RegisterMarkupClass()
{
	m_pPropertyLanguage = CXTPMarkupDependencyProperty::Register(L"Language",
		MARKUP_TYPE(CXTPMarkupString), MARKUP_TYPE(CXTPMarkupXtpScript));
...
{
	CXTPMarkupString* pLanguage = MARKUP_STATICCAST(CXTPMarkupString, GetValue(m_pPropertyLanguage));
...
void CXTPMarkupXtpScript::SetContentObject(
...
void CXTPMarkupXtpScript::OnLoaded(CXTPMarkupBuilder* pBuilder)
{
	...
	if(NULL != lpszSource)
	{
		...
		if(SUCCEEDED(hr))
		{
			...
			{
			case xtpUriFile:
				hr = LoadFileContents(uri.file, &data);
				break;
			case xtpUriRes:
				hr = LoadResourceContents(uri.res.szFilePath,
					uri.res.lpName, uri.res.lpType, &data);
				break;
			default:
		...
		if(SUCCEEDED(hr))
		{
			if(0 < data.GetSize())
			{
				m_pMarkupContext->LoadScriptCodeChunk(pBuilder,
					lpszLangauge, data.GetData());
			}
		}
		else
		{
...
HRESULT CXTPMarkupXtpScript::LoadFileContents(
	LPCTSTR lpszPath,
	CArray<WCHAR, WCHAR>* pData) const
{
	...
	do
	{
		hFile = CreateFile(lpszPath, GENERIC_READ, FILE_SHARE_READ,
		...
		hMapping = CreateFileMapping(hFile, NULL,
			PAGE_READONLY, cbFileSize.HighPart, cbFileSize.LowPart, NULL);
		if(NULL == hMapping)
		{
			hr = HRESULT_FROM_WIN32(GetLastError());
			break;
...
HRESULT CXTPMarkupXtpScript::LoadResourceContents(
	LPCTSTR lpszModule,
	LPCTSTR lpName,
	LPCTSTR lpType,
	CArray<WCHAR, WCHAR>* pData) const
...
HRESULT CXTPMarkupXtpScript::LoadCodeContent(
	...
	CArray<WCHAR, WCHAR>* pData) const
{
	...
	if(0 < cbFileData)
	{
		if(cbFileData < INT_MAX)
		{
			...
			pData->SetSize(ccBuffer);
			if(!XTPLoadUnicodeText(pFileData, cbFileData,
				pData->GetData(), &ccBuffer))
			{
