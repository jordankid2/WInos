...
void CXTPMarkupStaticExtension::Extend(CXTPMarkupStaticExtension* pExtension)
{
	if (!m_bProperty)
	{
		...
		CXTPMarkupStaticExtension*& pExt = m_mapExtensions[(LPCWSTR)pExtension->GetName()];
		ASSERT(NULL == pExt && "An extension is already added");
...
COLORREF CXTPMarkupStaticExtensionBrushProperty::GetRawValue() const
{
	...
	CXTPMarkupSolidColorBrush* pColorBrush = MARKUP_DYNAMICCAST(CXTPMarkupSolidColorBrush, m_pObject);
	if (pColorBrush)
	{
