...
CXTPMarkupObject* CXTPMarkupPoint::ConvertFrom(
	...
	CXTPMarkupObject* pObject) const
{
	...
	if (IsStringObject(pObject))
	{
		...
		if (NULL != m_pAssociatedProperty)
		{
			...
			if (0 != (m_pAssociatedProperty->GetFlags() & CXTPMarkupPropertyMetadata::flagVertDpiSensible))
			{
...
BOOL CXTPMarkupPointCollection::ConvertFromString(
	CXTPMarkupBuilder* pBuilder,
	LPCWSTR lpszValue,
	CPointArray& arr)
{
	...
	while (*lpszValue != 0)
	{
		...
		POINT pt = {
			pBuilder->GetMarkupContext()->ScaleX(x),
			pBuilder->GetMarkupContext()->ScaleY(y)
		};
		arr.Add(pt);
	}
...
#ifndef _XTP_EXCLUDE_GDIPLUS
	if ((!pMarkupContext || pMarkupContext->IsGdiPlusEnabled()) && !XTPDrawHelpers()->IsContextRTL(m_hDC))
	{
		Gdiplus::GpGraphics* pGpGraphics = CXTPMarkupGdiPlusDeviceContext::CreateGraphics(m_hDC, m_hAttribDC);
		if(NULL != pGpGraphics)
		{
...
XTPMarkupSmoothingMode CXTPMarkupDrawingContext::SetSmoothingMode(XTPMarkupSmoothingMode smoothingMode)
{
	return m_pDeviceContext->SetSmoothingMode(smoothingMode, m_bUseTextSmoothing);
}

XTPMarkupSmoothingMode CXTPMarkupDrawingContext::SetSmoothingMode(XTPMarkupSmoothingMode smoothingMode, BOOL bUseTextSmoothing)
{
...
void CXTPMarkupDrawingContext::DrawFrame(CRect rc, CXTPMarkupBrush* pBrush)
{
	...
	{
		xs = m_pMarkupContext->ScaleX(xs);
		ys = m_pMarkupContext->ScaleY(ys);
	}
...
{
	CXTPMarkupGdiPlusDeviceContext* pGdiPlusDC =  DYNAMIC_DOWNCAST(CXTPMarkupGdiPlusDeviceContext, m_pDeviceContext);
	if (pGdiPlusDC)
	{
		pGdiPlusDC->GetGdiPlus()->GdipSetPageUnit(pGdiPlusDC->GetGraphics(), UnitPixel);
	}
}
