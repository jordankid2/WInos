...
CXTPMarkupStaticExtension* CXTPMarkupContext::GetExtensionRoot()
{
	if (NULL == m_pExtensionRoot)
	{
		...
		{
		public:
			CRootStaticExtension() : CXTPMarkupStaticExtension(L"__root__", FALSE)
			{
			}
		};
...
CString CXTPMarkupContext::ValidateXML(LPCWSTR lpszBuffer)
{
	...
	do
	{
		if(!lpszBuffer)
		{
			strMessage = _T("No Text To Parse.");
			break;
		}
		...
		if(!sc.SetBuffer(lpszBuffer, lpszBuffer + wcslen(lpszBuffer)))
		{
			strMessage = sc.FormatErrorMessage();
			break;
		}
		...
		if(NULL == pUIElement)
		{
			strMessage = builder.GetLastError();
			break;
		}
...
void CXTPMarkupContext::LoadScriptCodeChunk(
	...
	LPCWSTR lpszCode)
{
	...
	if(NULL == m_pScriptEngine)
	{
		...
		hr = m_pScriptEngine->AddNamedItem(L"__XTPActiveScriptStdRuntime",
			NULL, SCRIPTITEM_GLOBALMEMBERS);
		if(FAILED(hr))
		{
			ResetScriptEngine();
			pBuilder->ThrowBuilderException(hr);
		...
		while(NULL != posNamedItem)
		{
			...
			hr = m_pScriptEngine->AddNamedItem(lpszName,
				NULL, SCRIPTITEM_ISVISIBLE);
			if(FAILED(hr))
			{
				ResetScriptEngine();
				pBuilder->ThrowBuilderException(hr);
...
void CXTPMarkupContext::RegisterScriptNamedObjects(
	CXTPMarkupBuilder* pBuilder,
...
void CXTPMarkupContext::ThrowScriptEngineException(
	CXTPMarkupBuilder* pBuilder,
	HRESULT hr)
{
	ASSERT(NULL != pBuilder);
	...
	if(NULL != pExcepInfo)
	{
		...
		message.Format(_T("Script error [0x%08X]: %ls"),
			...
			(NULL != pExcepInfo->bstrDescription ? pExcepInfo->bstrDescription : L"<unknown>"));
		pBuilder->ThrowBuilderException(message);
	}
	else
	{
...
STDMETHODIMP CXTPMarkupContext::XActiveScriptSite::GetItemInfo(
	LPCOLESTR pstrName,
	DWORD dwReturnMask,
	IUnknown **ppiunkItem,
	ITypeInfo **ppti)
		...
		if(NULL != ppiunkItem)
		{
			*ppiunkItem = NULL;
			...
			if(0 == wcscmp(pstrName, L"__XTPActiveScriptStdRuntime"))
			{
				...
				hr = pThis->m_pScriptEngineStdRuntime->GetControllingUnknown()->QueryInterface(ppiunkItem);
			}
			else
			{
...
STDMETHODIMP CXTPMarkupContext::XActiveScriptSite::OnScriptTerminate(
...
BEGIN_DISPATCH_MAP(CXTPMarkupContext, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPMarkupContext, "UnfreezeUpdates", 42, OleUnfreezeUpdates, VT_EMPTY, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPMarkupContext, "DefaultSmoothingMode", 44, OleGetDefaultSmoothingMode, OleSetDefaultSmoothingMode, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPMarkupContext, "DefaultTextSmoothingEnabled", 45, OleGetDefaultTextSmoothing, OleSetDefaultTextSmoothing, VT_BOOL)
END_DISPATCH_MAP()
...
BEGIN_DISPATCH_MAP(CXTPMarkupContextInternalDispatch, CXTPCmdTarget)
	DISP_FUNCTION_ID(CXTPMarkupContextInternalDispatch, "SetMethod", 1, OleSetMethod, VT_EMPTY, VTS_DISPATCH VTS_WBSTR)
	DISP_FUNCTION_ID(CXTPMarkupContextInternalDispatch, "SetHandler", 6, OleSetHandler, VT_EMPTY, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPMarkupContextInternalDispatch, "CreateObject", 2, OleCreateObject, VT_DISPATCH, VTS_WBSTR)
	DISP_FUNCTION_ID(CXTPMarkupContextInternalDispatch, "CreateSolidBrush", 3, OleCreateSolidBrush, VT_DISPATCH, VTS_COLOR)
	DISP_FUNCTION_ID(CXTPMarkupContextInternalDispatch, "CreateThickness", 4, OleCreateThickness, VT_DISPATCH, VTS_I4 VTS_I4 VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPMarkupContextInternalDispatch, "Parse", 5, OleParse, VT_DISPATCH, VTS_WBSTR)
	DISP_PROPERTY_EX_ID(CXTPMarkupContextInternalDispatch, "EnableGdiPlus", 7, OleGetEnableGdiPlus, OleSetEnableGdiPlus, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPMarkupContextInternalDispatch, "DpiAware", 43, OleIsDpiAware, OleSetDpiAware, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPMarkupContextInternalDispatch, "ToolTipContext", 39, OleGetToolTipContext, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPMarkupContextInternalDispatch, "Icons", 40, OleGetIcons, OleSetIcons, VT_DISPATCH)
	DISP_FUNCTION_ID(CXTPMarkupContextInternalDispatch, "ValidateXML", 8, OleValidateXML, VT_BSTR, VTS_WBSTR)
	DISP_FUNCTION_ID(CXTPMarkupContextInternalDispatch, "FreezeUpdates", 41, OleFreezeUpdates, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CXTPMarkupContextInternalDispatch, "UnfreezeUpdates", 42, OleUnfreezeUpdates, VT_EMPTY, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPMarkupContextInternalDispatch, "DefaultSmoothingMode", 44, OleGetDefaultSmoothingMode, OleSetDefaultSmoothingMode, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPMarkupContextInternalDispatch, "DefaultTextSmoothingEnabled", 45, OleGetDefaultTextSmoothing, OleSetDefaultTextSmoothing, VT_BOOL)
END_DISPATCH_MAP()
