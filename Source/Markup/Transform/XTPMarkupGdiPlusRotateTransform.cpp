// XTPMarkupGdiPlusRotateTransform.cpp: implementation of the CXTPMarkupGdiPlusRotateTransform class.
//
...
IMPLEMENT_MARKUPCLASS(L"RotateTransform", CXTPMarkupGdiPlusRotateTransform, CXTPMarkupRotateTransform);
...
BOOL CXTPMarkupGdiPlusRotateTransform::TransformMatrix(
	const CXTPGdiPlus* pGdiPlus,
	GpMatrix* pMatrix)
{
	...
	if(Gdiplus::Ok == pGdiPlus->GdipTranslateMatrix(pMatrix,
		centerX, centerY, Gdiplus::MatrixOrderPrepend))
	{
		if(Gdiplus::Ok == pGdiPlus->GdipRotateMatrix(pMatrix,
			angle, Gdiplus::MatrixOrderPrepend))
		{
			if(Gdiplus::Ok == pGdiPlus->GdipTranslateMatrix(pMatrix,
				-centerX, -centerY, Gdiplus::MatrixOrderPrepend))
			{
