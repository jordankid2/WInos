// XTPMarkupGdiPlusTransformationMatrix.cpp: CXTPMarkupGdiPlusTransformationMatrix class implementation.
//
...
CXTPMarkupGdiPlusTransformationMatrix::CXTPMarkupGdiPlusTransformationMatrix(
	...
	, m_pMatrix(pMatrix)
{
	ASSERT(NULL != m_pGdiPlus);
	ASSERT(NULL != m_pMatrix);
}

CXTPMarkupGdiPlusTransformationMatrix::~CXTPMarkupGdiPlusTransformationMatrix()
{
	m_pGdiPlus->GdipDeleteMatrix(m_pMatrix);
}
...
CXTPMarkupTransformationMatrix* CXTPMarkupGdiPlusTransformationMatrix::Clone() const
{
	...
	if(Gdiplus::Ok == m_pGdiPlus->GdipCloneMatrix(m_pMatrix, &pClonedMatrix))
	{
		pClonedInstance = new CXTPMarkupGdiPlusTransformationMatrix(m_pGdiPlus, pClonedMatrix);
	}
...
{
	m_pGdiPlus->GdipTranslateMatrix(
		m_pMatrix,
		static_cast<REAL>(offset.x),
		static_cast<REAL>(offset.y),
		Gdiplus::MatrixOrderPrepend);
}

inline BOOL IsSameTriangleSide(
	const PointF& p1,
	const PointF& p2,
	const PointF& a,
	const PointF& b)
...
inline BOOL PointInTriangle(
	...
	const PointF& c)
{
	return IsSameTriangleSide(point, a, b, c)
		&& IsSameTriangleSide(point, b, a, c)
		&& IsSameTriangleSide(point, c, a, b);
}

BOOL CXTPMarkupGdiPlusTransformationMatrix::TransformRect(
	const RECT& rect,
	...
	PointF (&transformedRectPoints)[4]) const
{
	...
	if(Ok == m_pGdiPlus->GdipTransformMatrixPoints(
		m_pMatrix, transformedRectPoints, _countof(transformedRectPoints)))
	{
		PointF offset(
			static_cast<REAL>(absoluteOffset.x),
			static_cast<REAL>(absoluteOffset.y));
...
BOOL CXTPMarkupGdiPlusTransformationMatrix::PtInRect(
	const RECT& rect,
	const POINT& absoluteOffset,
	const POINT& point) const
{
	...
	{
		PointF pointf(
			static_cast<REAL>(point.x),
			static_cast<REAL>(point.y));
		bHit = PointInTriangle(pointf,
			transformedRectPoints[0],
			transformedRectPoints[1],
			transformedRectPoints[2]);
		if(!bHit)
		{
			bHit = PointInTriangle(pointf,
				transformedRectPoints[2],
				transformedRectPoints[3],
				transformedRectPoints[0]);
		}
	}
...
CRect CXTPMarkupGdiPlusTransformationMatrix::GetBoundRect(
	const RECT& rect,
