...
BOOL CXTPMarkupParser::SetBuffer(LPCSTR lpszStart, LPCSTR lpszEnd)
{
	...
	if (LooksLikeXml(lpszStart, lpszEnd))
	{
		...
		if (CreateDataStream(lpszStart, lpszEnd - lpszStart, &pStream))
		{
...
BOOL CXTPMarkupParser::SetDataStream(IStream* pStream)
{
	...
	HRESULT hr;
	if(SUCCEEDED(hr = xmlDocument.CreateInstance(CLSID_XTPDOMDocument)))
	{
		...
		if(SUCCEEDED(hr = xmlDocument->raw_load(vtSource, &vfSuccess))
			&& VARIANT_FALSE != vfSuccess)
		{
...
#ifdef _UNICODE
			strErrorMessage.Format(_T("XML Error [0x%08X] '%ls' at %i:%i '%ls'"),
				m_errorInfo.nCode,
				NULL != lpszReason ? lpszReason : _T("{reason unknown}"),
				m_errorInfo.nLine,
				m_errorInfo.nPosition,
				NULL != lpszSourceLine ? lpszSourceLine : _T("{source unknown}"));
#else
			strErrorMessage.Format(_T("XML Error [0x%08X] '%s' at %i:%i '%s'"),
				m_errorInfo.nCode,
				NULL != lpszReason ? XTP_CW2CA(lpszReason) : "{reason unknown}",
				m_errorInfo.nLine,
				m_errorInfo.nPosition,
				NULL != lpszSourceLine ? XTP_CW2CA(lpszSourceLine) : "{source unknown}");
#endif

			if(NULL != lpszSourceLine) delete [] lpszSourceLine;
			if(NULL != lpszReason) delete [] lpszReason;
		}
		else
		{
#ifdef _UNICODE
...
CString CXTPMarkupParser::FormatMarkupErrorMessage() const
{
	...
	if(FAILED(m_errorInfo.nCode))
	{
		if(m_errorInfo.bXml)
		{
			XTPXML::IXMLDOMDocumentPtr xmlDoc;
			if(SUCCEEDED(xmlDoc.CreateInstance(CLSID_XTPDOMDocument)))
			{
				VARIANT_BOOL vtSuccess;
				if(SUCCEEDED(xmlDoc->raw_loadXML(
					L"<TextBlock>"
						L"<Run></Run> "
						L"<Hyperlink></Hyperlink> "
						L"<Run></Run>"
					L"</TextBlock>", &vtSuccess)) && VARIANT_FALSE != vtSuccess)
				{
					LPWSTR lpszReason = NULL;
					if(0 < m_errorInfo.strReason.length())
					{
					...
					xmlNode->text = (!strSourceLine.IsEmpty() 
						? (LPCTSTR)strSourceLine 
						: _T("{unknown source}"));
...
BOOL CXTPMarkupParser::CreateDataStream(
	LPCSTR lpString,
	SIZE_T cch,
...
BOOL CXTPMarkupParser::CreateDataStream(
	...
	IStream** ppStream) const
{
	const MEMORY_BLOB dataBlobs[] =
	{
		{ sizeof(UnicodeBOM), UnicodeBOM }, // Unicode lead bytes
	...
	IStream** ppStream) const
{
	...
	if (0 < cbTotal)
	{
		HGLOBAL hMem = GlobalAlloc(GMEM_MOVEABLE, cbTotal);
		if (NULL != hMem)
		{
...
void CXTPMarkupParser::ObtainXmlErrorInfo(
	XTPXML::IXMLDOMDocument* pXmlDocument,
	HRESULT hr /*= S_OK*/)
{
	...
	if(SUCCEEDED(hrLoc = pXmlDocument->get_parseError(&pParseError)))
	{
		if(pParseError)
		{
			...
			if(FAILED(m_errorInfo.nCode) || S_OK == hr)
			{
				...
				TRACE3("Markup loading failed (%i:%i): '%ls'",
					m_errorInfo.nLine, m_errorInfo.nPosition,
					(LPCWSTR)m_errorInfo.strReason);
			}
			else
			{
				ObtainComErrorInfo(hr);
...
HGLOBAL CXTPMarkupParser::ReadStreamData(IStream* pStream) const
{
	...
	STATSTG streamStat;
	if(SUCCEEDED(pStream->Stat(&streamStat, STATFLAG_NONAME))
		&& 0 == streamStat.cbSize.HighPart)
	{
...
LPWSTR CXTPMarkupParser::ReadStreamDataAsUnicode(
	IStream* pStream,
	...
	if(NULL != hData)
	{
		...
		if(NULL != lpUnicodeText)
		{
			*pchUnicodeText = static_cast<UINT>(cbData) + 1;
			...
			if(XTPLoadUnicodeText(
				lpData,
				static_cast<UINT>(cbData),
				lpUnicodeText,
				pchUnicodeText))
			{
...
BOOL CXTPMarkupParser::FixReferenceToUndeclaredNamespacePrefixX(
	IStream* pInputStream,
	IStream** ppOutputStream) const
{
	// Issue example:
	//	XML Error [0xC00CE01D] at 4:59 '<Style x:Key="{x:Type Hyperlink}" TargetType="Hyperlink">': Reference to undeclared namespace prefix: 'x'.
	//	                                       ^
	...
	*ppOutputStream = NULL;
	...
	if(0xC00CE01D == m_errorInfo.nCode
		&& NULL != wcsstr((LPCWSTR)m_errorInfo.strReason, L"'x'"))
	{
				...
				if(NULL != lpPos)
				{
					...
					if(lpPos < lpEnd)
					{
						...
						const BOOL bSrcUnicode = (sizeof(UnicodeBOM) <= dataBlobs[1].cbSize
							&& 0 == memcmp(dataBlobs[1].pBlobData, UnicodeBOM, sizeof(UnicodeBOM)));

						dataBlobs[2].pBlobData = L" ";
						dataBlobs[2].cbSize = sizeof(WCHAR);

						static const WCHAR szXNamspaceDeclaration[] = L"xmlns:x='http://schemas.microsoft.com/winfx/2006/xaml'";
						...
						dataBlobs[4].cbSize = (chXmlSource - (lpPos - lpszXmlSource)) * sizeof(WCHAR);

						CreateDataStream(
							dataBlobs + (bSrcUnicode ? 1 : 0),
