...
const GUID XTPFrameDimensionTime = { 0x6aedbd6d, 0x3fb5, 0x418a, { 0x83, 0xa6,  0x7f,  0x45,  0x22,  0x9d,  0xc8,  0x72 } };

IMPLEMENT_DYNAMIC(CXTPMarkupGdiPlusImage, CXTPMarkupDeviceDependentImage);

CXTPMarkupGdiPlusImage::CXTPMarkupGdiPlusImage(
	CXTPGdiPlus* pGdiPlus,
...
BOOL CXTPMarkupGdiPlusImage::LoadIcon(const XTP_URI& uri, int nWidth)
{
	...
	HBITMAP hBitmap = CXTPImageManagerIcon::LoadBitmapFromResource(
		AfxGetResourceHandle(), lpszResource, &bAlphaBitmap);
	if (NULL == hBitmap)
	{
...
BOOL CXTPMarkupGdiPlusImage::LoadImage(const XTP_URI& uri)
{
	...
	if (SUCCEEDED(XTPCreateReadOnlyStreamOnUri(&uri, &pImageStream)))
	{
		...
		do
		{
			if (Gdiplus::Ok != m_pGdiPlus->GdipLoadImageFromStream(pImageStream, &pImage))
			{
				break;
			}

			UINT cx = 0;
			if (Gdiplus::Ok != m_pGdiPlus->GdipGetImageWidth(pImage, &cx))
			{
				break;
			}

			UINT cy = 0;
			if (Gdiplus::Ok != m_pGdiPlus->GdipGetImageHeight(pImage, &cy))
			{
				break;
			}
		...
		if (NULL != pImage)
		{
			m_pGdiPlus->GdipDisposeImage(pImage);
		}
	}
...
void CXTPMarkupGdiPlusImage::Cleanup()
{
	...
	if(NULL != m_pImage)
	{
		m_pGdiPlus->GdipDisposeImage(m_pImage);
		m_pImage = NULL;
	}
...
void CXTPMarkupGdiPlusImage::LoadAnimationData()
{
	...
	do
	{
		// Obtain frame count.
		UINT nDimCount = 0;
		if(Gdiplus::Ok != m_pGdiPlus->GdipImageGetFrameDimensionsCount(m_pImage, &nDimCount)
			|| 0 == nDimCount)
		{
			break;
		}
		...
		if(Gdiplus::Ok != m_pGdiPlus->GdipImageGetFrameDimensionsList(
			m_pImage, lpgDimensions, nDimCount))
		{
			break;
		}

		if(Gdiplus::Ok != m_pGdiPlus->GdipImageGetFrameCount(
			m_pImage, lpgDimensions, &m_nFrameCount))
		{
			break;
		}
		...
		if(Gdiplus::Ok != m_pGdiPlus->GdipGetPropertyItemSize(m_pImage,
			PropertyTagFrameDelay, &nPropertyTagFrameDelaySize)
		...
		if(Gdiplus::Ok != m_pGdiPlus->GdipGetPropertyItem(m_pImage,
			PropertyTagFrameDelay, nPropertyTagFrameDelaySize,
			m_pDelayProperty))
		{
			free(m_pDelayProperty);
...
void CXTPMarkupGdiPlusImage::RenderAnimation()
{
	...
	m_pGdiPlus->GdipImageSelectActiveFrame(m_pImage,
		&XTPFrameDimensionTime, m_nCurrentFrame);
...
void CALLBACK CXTPMarkupGdiPlusImage::OnAnimationTimer(
	HWND hWnd,
	UINT uMsg,
	UINT_PTR nTimerId,
	DWORD dwTime)
