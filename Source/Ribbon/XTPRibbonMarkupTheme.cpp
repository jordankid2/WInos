...
void CXTPRibbonMarkupTheme::LoadParts(HZIP hZip, CXTPMarkupContext * pMarkupContext /*= NULL*/)
{
	...
	// System pop-up bar
	bMarkupLoaded = m_systemPopupBarEntry.LoadPart(hZip, _T("SystemPopupBarEntry.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_systemPopupBarButtonNormal.LoadPart(hZip, _T("SystemPopupBarButtonNormal.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_systemPopupBarButtonSelected.LoadPart(hZip, _T("SystemPopupBarButtonSelected.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pSystemMenuNormal->LoadPart(hZip, _T("RibbonFrameSystemMenuNormal.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pSystemMenuSelected->LoadPart(hZip, _T("RibbonFrameSystemMenuSelected.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pSystemMenuPopuped->LoadPart(hZip, _T("RibbonFrameSystemMenuPopuped.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGroupNormal->LoadPart(hZip, _T("RibbonGroupNormal.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGroupHighlighted->LoadPart(hZip, _T("RibbonGroupHighlighted.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGroupPopupNormal->LoadPart(hZip, _T("RibbonGroupPopupNormal.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGroupPopupPopuped->LoadPart(hZip, _T("RibbonGroupPopupPopuped.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGroupPopupSelected->LoadPart(hZip, _T("RibbonGroupPopupSelected.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);

	bMarkupLoaded = m_pGroupPopupToolBar->LoadPart(hZip, _T("RibbonGroupPopupToolBar.xaml"), pMarkupContext);
	ASSERT(bMarkupLoaded);
...
void CXTPRibbonMarkupTheme::DrawRibbonTab(CDC* pDC, CXTPRibbonTab* pItem)
{
	...
	CXTPTabPaintManagerTheme::InflateRectEx(rcEntry, CRect(0, XTP_DPI_Y(1), 0, -XTP_DPI_Y(1)), tabPosition);
...
void CXTPRibbonMarkupTheme::FillRibbonBar(CDC *pDC, CXTPRibbonBar *pRibbonBar)
{
	...
	{
		m_pPaintManager->HorizontalLine(pDC, rcRibbonClient.left, rcRibbonClient.bottom - XTP_DPI_Y(2), rcRibbonClient.right, m_clrMinimizedFrameEdgeShadow);
		m_pPaintManager->HorizontalLine(pDC, rcRibbonClient.left, rcRibbonClient.bottom - XTP_DPI_Y(1), rcRibbonClient.right, m_clrMinimizedFrameEdgeHighLight);
	}
}
...
CSize CXTPRibbonMarkupTheme::DrawControlGroupOption(CDC* pDC, CXTPControl* pControlGroupOption, BOOL bDraw)
{
	...
	if (bDraw)
	{
		...
		rc = CRect(
			CPoint((rc.left + rc.right - XTP_DPI_X(6)) / 2, (rc.bottom + rc.top - XTP_DPI_Y(8)) / 2), 
			XTP_DPI(CSize(6, 8)));
		...
		XTPDrawHelpers()->Triangle(pDC, 
			CPoint(rc.left + XTP_DPI_X(3), rc.top + XTP_DPI_Y(6)), 
...
CSize CXTPRibbonMarkupTheme::DrawControlGroupPopup(CDC* pDC, CXTPControlPopup* pControlGroupPopup, BOOL bDraw)
{
	...
	CSize szButton(max(XTP_DPI_X(42), m_pPaintManager->m_nTextHeight * 2 - XTP_DPI_X(3)), GetEditHeight() * 3);
	...
	if (bDraw)
	{
		...
		if (bDrawImage)
		{
			...
			sz = m_pPaintManager->GetStretchIconSizeDPIAwareness(pImageIcon, sz, pControlGroupPopup, rcButton.Size());
...
void CXTPRibbonMarkupTheme::DrawRibbonQuickAccessButton(CDC* pDC, CXTPControlPopup* pControl)
{
	...
	XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolEjectDown,
		rc, color);
...
void CXTPRibbonMarkupTheme::DrawRibbonQuickAccessMoreButton(CDC* pDC, CXTPControlPopup* pControl)
{
	...
	CRect rc = pControl->GetRect();
	XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolRewindRight, 
		rc, GetSysColor(COLOR_3DHIGHLIGHT));

	rc.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2));
	XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolRewindRight,
		rc, GetSysColor(COLOR_BTNTEXT));
}
...
void CXTPRibbonMarkupTheme::DrawRibbonFrameSystemButton(CDC *pDC, CXTPRibbonControlSystemButton *pControl, CRect rc)
{
	...
	if ((!pRibbonBar->IsCaptionVisible() && pRibbonBar->GetShowCaptionAlways()) || !pRibbonBar->IsTabsVisible())
	{
	...
	else
	{
			...
			CString strImageName(m_strNormalPrefix);
			if (pControl->GetSelected())
				strImageName = m_strHighlightedPrefix;
			strImageName += _T("BACKSTAGE_BACK_BUTTON_");
			strImageName += XTPDpiHelper()->SelectDpiSpecific(
				_T("SMALL"),
				_T("MEDIUM"),
				_T("LARGE"),
				_T("LARGE"));
			if (pRibbonBar->IsLayoutRTL())
				strImageName += _T("_RTL");
			CXTPResourceImage  *pImage = pImages->LoadFile(strImageName);
			CRect rcImage(rc);
			...
			if (pImage)
			{
				...
				rcImage.SetRect(rcImage.left - 3, rc.top + XTP_DPI_Y(7), rcImage.left + h - 3, rc.top + XTP_DPI_Y(7) + h);
	...
	else
	{
		...
		if (NULL != pIcon)
		{
			...
			CPoint ptIcon((rc.left + rc.right - sz.cx) / 2, nOffset + (rc.top + rc.bottom + XTP_DPI_Y(1) - sz.cy) / 2);
			pIcon->m_bDrawComposited = pControl->HasDwmCompositedRect();
			pIcon->Draw(pDC, ptIcon, pIcon->GetIcon(nState), sz);
...
void CXTPRibbonMarkupTheme::DrawRibbonScrollButton(CDC* pDC, CXTPControl* pControl, BOOL bScrollLeft)
{
	...
	if (bScrollLeft)
	{
		...
		XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayLeft, rc, clr);
	}
	else
	{
