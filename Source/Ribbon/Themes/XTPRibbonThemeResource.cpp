...
void CXTPRibbonThemeResource::FillGroupPopupToolBar(CDC* pDC, CXTPRibbonGroup* pGroup, CXTPCommandBar* pCommandBar)
{
	...
	if (tabColor != xtpRibbonTabContextColorNone)
	{
		...
		if (pImage) pImage->DrawImage(pDC, rcGroup, pImage->GetSource(0, 1), pImage->GetWidth() < 8
			? CRect(2, 8, 2, 8) : CRect(8, 8, 8, 8));
	}
	else
	{
...
void CXTPRibbonThemeResource::DrawGroup(CDC* pDC, CXTPRibbonGroup* pGroup)
{
	...
	if (!pGroup->IsReduced())
	{
		...
		if (pImage)
		{
			pImage->DrawImage(pDC, CRect(rcGroup.right - XTP_DPI_X(1), rcGroup.top + XTP_DPI_Y(2), 
				rcGroup.right + XTP_DPI_X(2), rcGroup.bottom - XTP_DPI_Y(5)), pImage->GetSource());
		}
...
void CXTPRibbonThemeResource::FillRibbonBar(CDC *pDC, CXTPRibbonBar *pRibbonBar)
{
	...
	{
		rcRibbonClient.top += (::GetSystemMetrics(SM_CYSIZEFRAME) + ::GetSystemMetrics(SM_CYCAPTION));
	}
...
void CXTPRibbonThemeResource::FillGroupRect(CDC* pDC, CXTPRibbonTab* pActiveTab, CRect rcGroups)
{
	...
	if (tabColor != xtpRibbonTabContextColorNone)
	{
		...
		if (pImage) pImage->DrawImage(pDC, rcGroups, pImage->GetSource(0, 1), pImage->GetWidth() < 5
			? CRect(1, 8, 1, 8) : CRect(8, 8, 8, 8));
	}
	else
	{
...
CSize CXTPRibbonThemeResource::DrawControlGroupOption(CDC* pDC, CXTPControl* pControlGroupOption, BOOL bDraw)
{
	if(XTPResourceImages()->IsValid() && bDraw)
	{
		...
		if (pImage) pImage->DrawImage(pDC, rc, rcImage, CXTPEmptyRect(), 0xFF00FF);
	}
		...
		if (bDrawImage)
		{
			...
			if (sz.cx < XTP_DPI_X(20))
			{
				...
				CRect rc(CPoint(rcButton.CenterPoint().x - szImageButton.cx / 2,
					rcButton.top + XTP_DPI_Y(2)), szImageButton);

				if (pImageButton) pImageButton->DrawImage(pDC, rc, pImageButton->GetSource(0, 1), CXTPEmptyRect());
...
void CXTPRibbonThemeResource::DrawRibbonTab(CDC *pDC, CXTPRibbonTab *pItem)
{
	...
	if (pItem->GetButtonLength() < pItem->GetContentLength())
	{
		...
		CRect rcSeparator(rcEntry.right, rcEntry.top, rcEntry.right + XTP_DPI_Y(pImage->GetWidth()), rcEntry.bottom - XTP_DPI_Y(1));
		if (pImage) pImage->DrawImage(pDC, rcSeparator, pImage->GetSource(), CXTPEmptyRect());
	}

	if (tabColor != xtpRibbonTabContextColorNone && pItem->GetContextHeader() &&
		(pItem->GetContextHeader()->m_pFirstTab == pItem))
	{
		CRect rcSeparator(rcEntry.left, rcEntry.top, rcEntry.left + XTP_DPI_X(1), rcEntry.bottom - XTP_DPI_Y(1));
	...
	{
		CRect rcSeparator(rcEntry.right, rcEntry.top, rcEntry.right + XTP_DPI_X(1), rcEntry.bottom - XTP_DPI_Y(1));
		CXTPResourceImage* pImage = LoadImage(FormatName(_T("CONTEXTTABSEPARATOR")));
		if (pImage) pImage->DrawImage(pDC, rcSeparator, pImage->GetSource(), CXTPEmptyRect());
	}
}
...
void CXTPRibbonThemeResource::DrawRibbonFrameCaption(CDC* pDC, CXTPRibbonBar* pRibbonBar, BOOL bActive)
{
	...
	if (!pRibbonBar->IsDwmEnabled())
	{
		...
		CRect rcTopLeft(rcCaption.left, rcCaption.top, rcCaption.left + XTP_DPI_X(rcSrc.Width()), rcCaption.bottom);
		...
		CRect rcTopRight(rcCaption.right - XTP_DPI_X(rcSrc.Width()), rcCaption.top, rcCaption.right, rcCaption.bottom);
		...
		if (hIcon)
		{
			...
			if (pRibbonBar->IsQuickAccessVisible())
			{
				...
				if (pImage)
				{
					...
					CRect rcDest(CPoint(nLeft + szIcon.cy + XTP_DPI_X(6), nTop + szIcon.cy / 2 - XTP_DPI_Y(rcSrc.Height()) / 2), XTP_DPI(rcSrc.Size()));
...
void CXTPRibbonThemeResource::DrawRibbonQuickAccessControls(CDC *pDC, CXTPRibbonBar *pRibbonBar)
{
		...
		((CXTPControls*)pRibbonBar->GetQuickAccessControls())->GetVisibleCount() > 0)
	{
		...
		{
			CRect rcQuickAccess = pRibbonBar->GetQuickAccessRect();
			CRect rcQuickAccessArea(rcQuickAccess.left - XTP_DPI_X(15), rcQuickAccess.top + XTP_DPI_Y(2), 
				rcQuickAccess.right, rcQuickAccess.top + XTP_DPI_Y(2) + pRibbonBar->GetQuickAccessHeight());
	...
	if (!pRibbonBar->IsCaptionVisible() || !pRibbonBar->IsTabsVisible() || pControl->IsSimpleButton())
	{
		...
		if (pImage)
		{
			...
			if (pControl->GetStyle() == xtpButtonIcon)
			{
				...
				if (pButtonIcon)
				{
					CSize sz(XTP_DPI(pButtonIcon->GetExtent()));

					if (sz.cy > XTP_DPI_Y(16))
						sz = XTP_DPI(CSize(16, 16));
...
void CXTPRibbonThemeResource::DrawRibbonScrollButton(CDC* pDC, CXTPControl* pControl, BOOL bScrollLeft)
{
	...
	pImageGlyph->DrawImage(pDC, rcGlyph, pImageGlyph->GetSource(nState, 3), CXTPEmptyRect(), 0xFF00FF);
}
...
void CXTPRibbonThemeResource::DrawSystemPopupBarButton(CDC* pDC, CXTPControl* pButton)
{
	...
	if (pImage)
	{
		pImage->DrawImage(pDC, pButton->GetRect(), pImage->GetSource(pButton->GetSelected() ? 1 : 0, 2), 
...
int CXTPRibbonThemeResource::GetRibbonCaptionHeight(const CXTPRibbonBar *pRibbonBar) const
{
	...
	if (XTPSystemVersion()->IsWin10OrGreater() && GetFramePaintManager()->m_bFlatFrame && pRibbonBar->IsDwmEnabled())
	{
