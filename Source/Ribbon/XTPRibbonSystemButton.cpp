...
CSize CXTPRibbonSystemPopupBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	...
	for (i = (int)arrButtons.GetSize() - 1; i >= 0; i--)
	{
		...
		pControl->SetRect(CRect(nRight - szControl.cx, sz.cy - m_rcBorders.bottom + XTP_DPI_Y(4), nRight, sz.cy - XTP_DPI_Y(3)));
...
void CXTPRibbonControlSystemPopupBarPinableListItem::Draw(CDC* pDC)
{
	...
	CXTPImageManagerIcon* pIcon = m_pIcons->GetImage(GetChecked() ? 0 : 1);
	CSize sz(XTP_DPI(pIcon->GetExtent()));
	pIcon->Draw(pDC, CPoint(rcCheck.CenterPoint().x - XTP_DPI_X(8), rcCheck.CenterPoint().y - XTP_DPI_Y(8)), sz);
}
...
void CXTPRibbonControlSystemPopupBarListCaption::Draw(CDC* pDC)
{
	...
	pPaintManager->HorizontalLine(pDC, rc.left, rc.bottom - XTP_DPI_Y(6), rc.right,
		pPaintManager->GetRibbonPaintManager()->m_clrRecentFileListEdgeShadow);
	pPaintManager->HorizontalLine(pDC, rc.left, rc.bottom - XTP_DPI_Y(5), rc.right,
...
CSize CXTPRibbonSystemPopupBarPage::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	...
	int nHeight = CXTPClientRect(pParent).Height() - (pParent->GetBorders().top + pParent->GetBorders().bottom) - XTP_DPI_Y(2);
...
void CXTPRibbonSystemPopupBarPage::AdjustExcludeRect(CRect& rc, BOOL bVertical)
{
	...
	rc.top = ((CXTPRibbonSystemPopupBar*)m_pControlPopup->GetParent())->GetBorders().top + XTP_DPI_Y(1);
