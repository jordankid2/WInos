...
protected:
	void Draw(CDC* pDC)
	{
		...
		pFramePaintManager->DrawFrameCaptionButton(pDC, GetRect(), GetID(), GetSelected(), GetPressed(), bActive);
	}
...
public:
	...
	CSize GetSize(CDC* /*pDC*/)
	{
		...
		return CSize(pPaintManager->GetEditHeight() * XTP_DPI_Y(13) / XTP_DPI_Y(22), pPaintManager->GetEditHeight());
	}
	void Draw(CDC* pDC)
	{
...
{
	CXTPPaintManager* pPaintManager = GetPaintManager();
	return m_bTabsVisible ? pPaintManager->IsCaptionbarInBackstageMode() ? XTP_DPI_Y(pPaintManager->GetBackstageSysButtonBottom()) : pPaintManager->GetRibbonPaintManager()->m_nTabsHeight : XTP_DPI_Y(2);
}
...
void CXTPRibbonBar::Reposition(int cx, int cy)
{
	...
	if (pPaintManager)
		bCaptionBarInBackstageMode = pPaintManager->IsCaptionbarInBackstageMode();
	...
	if (bShowCaption)
	{
		...
		m_rcCaption.SetRect(-nFrameBorder.cx, nCaptionOffset, cx + nFrameBorder.cx, GetCaptionHeight() + nRibbonTopBorder);
	...
	{
		m_rcQuickAccess.SetRect(
			XTP_DPI_X(3) + nCorner,
			m_rcCaption.top + nFrameBorder.cy - XTP_DPI_Y(2),
			m_rcCaption.right,
			m_rcCaption.top + nFrameBorder.cy + nQuickAccessHeight - XTP_DPI_X(2));
		...
		else if (NULL == GetFrameHook() || IsDwmEnabled())
		{
			...
			{
				CXTPCommandBarsFrameHook* pFrameHook = GetFrameHook();
				if (XTPSystemVersion()->IsWin10OrGreater() && GetPaintManager()->GetFramePaintManager()->m_bFlatFrame 
					&& pFrameHook && pFrameHook->IsDwmEnabled())
				{
	...
	if (m_pControlSystemButton && !bCaptionBarInBackstageMode)
	{
		...
		else if (!m_bTabsVisible)
		{
			...
			if (!m_bShowQuickAccessBelow) m_rcQuickAccess.left = XTP_DPI_X(3) + szControl.cx + XTP_DPI_X(12);
		}
	...
	for (i = 0; i < (int)arrFreeControls.GetSize(); i++)
	{
		...
		pControl->SetRowRect(CRect(XTP_DPI_X(2), rcTabControl.top, cy - XTP_DPI_X(2), rcTabControl.bottom));
	...
	nRight = m_bShowQuickAccess && m_bShowQuickAccessBelow ? cx - XTP_DPI_X(2) - XTP_DPI_X(13) :
	...
	for (i = 0; i < (int)arrQuickAccessControls.GetSize(); i++)
	{
		...
		CSize szControl = pControl->GetSize(&dc);
		int nTop = m_rcQuickAccess.top + (m_rcQuickAccess.Height() - szControl.cy + XTP_DPI_Y(1)) / 2;
...
void CXTPRibbonBar::AddCaptionButton(int nId, BOOL bAdd, BOOL bEnabled, CRect& rcCaption)
{
	...
	if (bAdd)
	{
		CSize szButton = pFramePaintManager->GetCaptionButtonSize(nId, GetSite()->GetSafeHwnd());
...
void CXTPRibbonBar::RepositionContextHeaders()
{
	...
	if (!IsQuickAccessBelowRibbon() && IsQuickAccessVisible() && !GetFrameHook())
		m_rcHeader.left = m_rcQuickAccess.right + (m_pControlQuickAccess ? m_pControlQuickAccess->GetRect().Width() + XTP_DPI_X(2) : XTP_DPI_X(12));
	...
	for (i = 0; i < nCount; i++)
	{
		...
		else
		{
			...
			pHeader->m_rcRect.SetRect(pTab->GetRect().left, m_rcCaption.top + XTP_DPI_Y(4), pTab->GetRect().right, m_rcCaption.bottom);
...
void CXTPRibbonBar::RepositionCaptionButtons()
{
	...
	else
	{
		...
		m_rcHeader.left += (m_pControlQuickAccess ? m_pControlQuickAccess->GetRect().Width() + XTP_DPI_X(2) : XTP_DPI_X(12));
	}
	...
	UINT nNativeSysBtnCount = (!bAddClose && bEnableClose)
...
void CXTPRibbonBar::RepositionGroups(CDC* pDC, CRect rcGroups)
{
	...
	pGroups->CalcDynamicSize(pDC, rcGroups.Width(), LM_COMMIT, 
		CRect(rcGroups.left + nMargin, rcGroups.top + XTP_DPI_Y(3), XTP_DPI_X(4) + nMargin, XTP_DPI_Y(3)));
}
...
void CXTPRibbonBar::DrawCommandBar(CDC* pDC, CRect rcClipBox)
{
	...
	if (IsGroupsVisible() && !pPaintManager->IsCaptionbarInBackstageMode())
	{
	...
	if (m_pControlTab && CRect().IntersectRect(rcClipBox, m_rcTabControl) && m_bTabsVisible && !pPaintManager->IsCaptionbarInBackstageMode())
	{
	...
	if (pSlectedTab && IsGroupsVisible() && !pPaintManager->IsCaptionbarInBackstageMode())
	{
	...
	for (i = 0; i < GetControlCount(); i++)
	{
		...
		if (CRect().IntersectRect(rcClipBox, rc))
		{
			...
			{
				if (!pPaintManager->IsCaptionbarInBackstageMode())
					pControl->Draw(pDC);
			}
			else
				pControl->Draw(pDC);
...
int CXTPRibbonBar::HitTestCaption(CPoint point) const
{
	...
	if (m_rcCaption.PtInRect(point) && m_pControls->HitTest(point) == 0)
	{
		...
		if ((point.y < m_rcCaption.top + XTP_DPI_Y(5)) && (dwStyle & WS_SIZEBOX) && ((dwStyle & WS_MAXIMIZE) == 0))
			return HTTOP;
...
void CXTPRibbonBar::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (pControl)
	{
		if ((pControl->GetFlags() & xtpFlagRibbonTabBackground) && pPaintManager->IsCaptionbarInBackstageMode())
			return;
	}
	CXTPTabManagerItem *pTab = m_pControlTab->HitTest(point);
	if (pTab && pPaintManager->IsCaptionbarInBackstageMode())
	{
		if (point.x < pPaintManager->GetBackstageModeMenuWidth())
	...
	if (pContextHeader && !pPaintManager->IsCaptionbarInBackstageMode())
	{
	...
	if (nHit != HTNOWHERE)
	{
		...
		if (nHit == HTSYSMENU)
		{
			...
			ClientToScreen(&point);
			pSite->SendMessage(WM_POPUPSYSTEMMENU, XTPToWPARAM(pSite->GetSafeHwnd()), MAKELPARAM(point.x, point.y));
...
void CXTPRibbonBar::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	if (pControl && GetSite()->SendMessage(WM_XTP_CONTROLRBUTTONUP, XTPToWPARAM(&point), XTPToLPARAM(pControl)) == TRUE)
		return;
...
void CXTPRibbonBar::MergeToolBar(CXTPCommandBar* pCommandBar, BOOL /*bSilent*/, int /*nDefaultConfirmationChoice*/)
{
...
void CXTPRibbonScrollableBar::CreateControlKeyboardTips(CXTPControl* pControl, LPCTSTR lpszPrefix, BOOL bVisible)
{
	...
	CRect rcCaption(rcGroup.left, rcGroup.bottom - pPaintManager->GetGroupCaptionHeight() + XTP_DPI_Y(2), rcGroup.right, rcGroup.bottom);
...
void CXTPRibbonBar::CreateKeyboardTips()
{
	...
	if (pCommandBars->m_keyboardTips.nLevel == 1)
	{
		...
		for (i = 0; i < m_pQuickAccessControls->GetCount(); i++)
		{
			...
			CPoint pt(pControl->GetRect().CenterPoint().x, pControl->GetRect().bottom - XTP_DPI_Y(11));
			CString strCaption;
			if (k < 10)
			{
		...
		if (m_bTabsVisible)
		{
			for (i = 0; i < GetTabCount(); i++)
			{
				...
				if (!strCaption.IsEmpty() && pTab->IsEnabled() && pTab->IsVisible())
				{
					CPoint pt(pTab->GetRect().CenterPoint().x, pTab->GetRect().bottom - XTP_DPI_Y(9));
		...
		if (GetTabCount() == 0) // Acts as simple menu
		{
			for (i = 0; i < m_pControls->GetCount(); i++)
			{
				...
				CPoint pt(pControl->GetRect().left + XTP_DPI_X(12), pControl->GetRect().bottom - XTP_DPI_Y(11));
...
void CXTPRibbonBar::CustomizeFindDropIndex(CXTPControl* pDataObject, const CPoint& point, CRect& rcMarker, int& nDropIndex, BOOL& bDropAfter)
{
	...
	if (IsCustomizable() && rcMarker.IsRectEmpty() && GetSelectedTab())
	{
		...
		if (pGroup && GetControls()->HitTest(point) == NULL)
		{
			...
			rcMarker.SetRect(rc.left + XTP_DPI_X(2), rc.top + XTP_DPI_Y(2), rc.left + XTP_DPI_X(8), rc.top + XTP_DPI_Y(22));
...
BOOL CXTPRibbonBar::PassKeyboardInputToBackstageView(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (m_pControlSystemButton)
		pBackstage = DYNAMIC_DOWNCAST(CXTPRibbonBackstageView, m_pControlSystemButton->GetCommandBar());
	if (NULL == pBackstage)
		return FALSE;
...
BEGIN_DISPATCH_MAP(CXTPRibbonBar, CXTPMenuBar)
	...
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "RibbonPaintManager", 102, OleGetRibbonPaintManager, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "TabPaintManager", 103, OleGetTabPaintManager, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "GroupsVisible", 104, IsGroupsVisible, SetGroupsVisible, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "SelectedTab", 105, OleGetSelectedTab, OleSetSelectedTab, VT_DISPATCH)
	DISP_PROPERTY_ID(CXTPRibbonBar, "MinimumVisibleWidth", 106, m_nMinVisibleWidth, VT_I4)
	DISP_FUNCTION_ID(CXTPRibbonBar, "EnableFrameTheme", 107, OleEnableFrameTheme, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CXTPRibbonBar, "AddSystemButton", 108, OleAddSystemButton, VT_DISPATCH, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "QuickAccessControls", 109, OleGetQuickAccessControls, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "ShowQuickAccessBelowRibbon", 110, IsQuickAccessBelowRibbon, ShowQuickAccessBelowRibbon, VT_BOOL)
	...
	DISP_PROPERTY_NOTIFY_ID(CXTPRibbonBar, "ShowQuickAccess", 115, m_bShowQuickAccess, OnRecalcLayout, VT_BOOL)
	DISP_PROPERTY_ID(CXTPRibbonBar, "AllowQuickAccessDuplicates", 116, m_bAllowQuickAccessDuplicates, VT_BOOL)

	DISP_PROPERTY_ID(CXTPRibbonBar, "AllowMinimize", 117, m_bAllowMinimize, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "Minimized", 118, IsRibbonMinimized, OleSetRibbonMinimized, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "FontHeight", 119, GetFontHeight, SetFontHeight, VT_I4)

	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "ControlQuickAccess", 120, OleGetControlQuickAccess, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "ControlSystemButton", 121, OleGetControlSystemButton, SetNotSupported, VT_DISPATCH)
	...
	DISP_PROPERTY_ID(CXTPRibbonBar, "AllowQuickAccessCustomization", 124, m_bAllowQuickAccessCustomization, VT_BOOL)

	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "GroupsHeight", 125, OleGetGroupsHeight, OleSetGroupsHeight, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "ClientHeight", 126, OleGetClientHeight, OleSetClientHeight, VT_I4)
	DISP_PROPERTY_ID(CXTPRibbonBar, "QuickAccessEmptyIconId", 130, m_nQuickAccessEmptyIconId, VT_I4)

	DISP_PROPERTY_EX_ID(CXTPRibbonBar, "SystemButton", 128, OleGetSystemButton, SetNotSupported, VT_DISPATCH)
...
{
	CXTPTabManagerItem* pItem = DYNAMIC_DOWNCAST(CXTPTabManagerItem, 
		CXTPTabManagerItem::FromIDispatchSafe(lpTab));
	if (NULL == pItem)
	{
