...
void CXTPRibbonBackstageButtonThemeResource::DrawButtonBackground(CDC *pDC, CXTPButton *pButton)
{
		...
		for (CWnd* pChildWnd = pWndParent->GetWindow(GW_CHILD); pChildWnd != NULL; pChildWnd = pChildWnd->GetWindow(GW_HWNDNEXT))
		{
...
#endif
			{
				...
				if (rcIntersect.IntersectRect(rcButtonScreen, rcSeparatorScreen))
				{
					...
					if (pImage)
					{
						pImage->DrawImage(pDC, CRect(rcSeparatorScreen.right - XTP_DPI_X(1), rcSeparatorScreen.top, rcSeparatorScreen.right, rcSeparatorScreen.bottom), 
	...
	if (bChecked && pBackstageButton->m_bTabStyle)
	{
		...
		if (pImage)
		{
			...
			pImage->DrawImage(pDC, CRect(rc.right - rcDst.Width(), y - rcDst.Height() / 2, rc.right, y - rcDst.Height() / 2 + rcDst.Height()), rcSrc);
		}
	}
}
...
void CXTPRibbonBackstageButtonThemeResource::DrawButton(CDC *pDC, CXTPButton *pButton)
{
...
#ifndef _XTP_ACTIVEX
	CXTPRibbonBackstageButton *pBackstageButton = DYNAMIC_DOWNCAST(CXTPRibbonBackstageButton, pButton);
	if (!pBackstageButton)
		return;
	...
	pDC->SetTextColor(GetXtremeColor(pBackstageButton->IsWindowEnabled() ? COLOR_INFOTEXT : COLOR_GRAYTEXT));
