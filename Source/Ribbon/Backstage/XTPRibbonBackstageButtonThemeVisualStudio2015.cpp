// XTPRibbonBackstageButtonThemeVisualStudio2015.cpp : implementation file
//
...
CXTPRibbonBackstageButtonThemeVisualStudio2015::CXTPRibbonBackstageButtonThemeVisualStudio2015()
{
	m_bOffsetHiliteText = FALSE;
}


void CXTPRibbonBackstageButtonThemeVisualStudio2015::DrawButtonBackground(CDC *pDC, CXTPButton *pButton)
{
	...
	if (pBackstageButton->m_bTabStyle)
	{
		...
		COLORREF clrSeparator = XTPIniColor(_T("CommandBars.Ribbon.Backstage"), _T("PageSeparator"), RGB(225, 225, 225));
		...
		for (CWnd* pChildWnd = pWndParent->GetWindow(GW_CHILD); pChildWnd != NULL; pChildWnd = pChildWnd->GetWindow(GW_HWNDNEXT))
		{
...
#endif
			{
				...
				if (rcIntersect.IntersectRect(rcButtonScreen, rcSeparatorScreen))
				{
					...
					CRect rc(rcSeparatorScreen.right - XTP_DPI_X(1), rcSeparatorScreen.top, rcSeparatorScreen.right, rcSeparatorScreen.bottom);
	...
	CXTPButtonThemeVisualStudio2015::DrawButtonBackground(pDC, pButton);
}
