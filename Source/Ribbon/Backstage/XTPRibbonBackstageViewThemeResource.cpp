...
void CXTPRibbonBackstageViewThemeResource::DrawBackground(CDC *pDC, CXTPRibbonBackstageView *pView)
{
	...
	if (pImage)
	{
		pImage->DrawImage(pDC, CRect(0, XTP_DPI_Y(2), pView->GetMenuWidth(), rc.bottom), pImage->GetSource());
	}
}
...
void CXTPRibbonBackstageViewThemeResource::DrawControl(CDC *pDC, CXTPRibbonBackstageView *pView, CXTPControl *pControl)
{
	...
	if (pView->IsCommand(pControl))
	{
		...
		CXTPImageManagerIcon* pIcon = pView->GetImageManager()->GetImage(pControl->GetIconId(), ICON_SMALL);
		if (pIcon)
		{
			...
			CXTPImageManagerIcon* pBestIcon = pView->GetImageManager()->GetImage(pControl->GetIconId(), szIconScalled.cx);
			if (NULL != pBestIcon)
			{
	...
	else
	{
		if (bHighlighted || bActiveTab || bFocused)
		{
			...
			if (bActiveTab)
			{
				...
				if (pImage)
				{
					...
					CRect rcGlyph(CPoint(rc.right - rcDst.Width(), (rc.top + rc.bottom - rcDst.Height()) / 2), rcDst.Size());
		...
		CXTPImageManagerIcon* pIcon = pView->GetImageManager()->GetImage(pControl->GetIconId(), sz.cx);
		if (pIcon)
		{
			if (!pView->IsDPIIconsScallingEnabled())
				sz = pIcon->GetExtent();

			pIcon->Draw(pDC, CPoint(rcText.left, (rcText.top + rcText.bottom - sz.cy) / 2),
				pIcon->GetIcon(bEnabled ? xtpImageNormal : xtpImageDisabled), sz);
