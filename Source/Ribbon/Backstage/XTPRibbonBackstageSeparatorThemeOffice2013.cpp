...
{
	m_clrSeparator	= XTPIniColor(_T("CommandBars.Ribbon.Backstage"), _T("PageSeparator"),	RGB(225, 225, 225));
	m_clrBack		= XTPIniColor(_T("CommandBars.Ribbon.Backstage"), _T("PageBackground"),	RGB(255, 255, 255));
}
...
void CXTPRibbonBackstageSeparatorThemeOffice2013::DrawSeparator(CDC *pDC, CXTPRibbonBackstageSeparator *pSeparator)
{
	...
	{
		pDC->FillSolidRect(CRect(rc.right - XTP_DPI_X(1), rc.top, rc.right, rc.bottom), m_clrSeparator);
	}
	else
	{
		pDC->FillSolidRect(CRect(rc.left, rc.Height() / 2, rc.right, rc.Height() / 2 + XTP_DPI_Y(1)), m_clrSeparator);
	}
}
