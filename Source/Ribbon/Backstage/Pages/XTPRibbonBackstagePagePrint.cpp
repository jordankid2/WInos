...
BOOL CXTPRibbonBackstagePagePrint::OnInitDialog()
{
	...
	m_lblPrint.SetFont(&m_xtpFontCaption);
...
void CXTPRibbonBackstagePagePrint::SetTheme(XTPControlTheme nTheme)
{
	CXTPRibbonBackstagePage::SetTheme(nTheme);
	
	m_btnPrint.SetTheme(nTheme);
	m_lblPrint.SetTheme(nTheme);
...
BOOL CXTPRibbonBackstagePagePrint::CreatePrintPreview()
{
	...
	pView->m_clrBackColor = m_clrBack;
