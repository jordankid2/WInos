...
CXTPRibbonBackstageView* AFX_CDECL CXTPRibbonBackstageView::CreateBackstageView(CXTPCommandBars *pCommandBars)
{
	...
	pPaintManager->SetBackstageSysButtonTop(gNewBacksategeSysButtonTop);
	pPaintManager->SetBackstageSysButtonBottom(gNewBacksategeHeaderHeight);
...
CXTPRibbonBackstageMenuSeparator* CXTPRibbonBackstageView::AddMenuSeparator(UINT nID)
{
	CXTPRibbonBackstageMenuSeparator* pSpace = new CXTPRibbonBackstageMenuSeparator();
	GetControls()->Add(pSpace, nID);
	return pSpace;
}
...
void CXTPRibbonBackstageView::SetTheme(XTPPaintTheme theme)
{
	...
	int nCommandHeight = m_pPaintManager->GetMenuItemCommandHeight() + m_pPaintManager->GetMenuItemCommandGapY();
	int nPageHeight = m_pPaintManager->GetMenuItemPageHeight() + m_pPaintManager->GetMenuItemPageGapY();
	m_ScrolLine = max(nCommandHeight, nPageHeight);

	m_themeCurrent = theme;

	m_clrText = XTPIniColor(_T("CommandBars.Ribbon.Backstage"),_T("PageText"), ::GetSysColor(COLOR_WINDOWTEXT));
	m_clrBack = XTPIniColor(_T("CommandBars.Ribbon.Backstage"),_T("PageBackground"), ::GetSysColor(COLOR_WINDOW));
}
...
void CXTPRibbonBackstageView::OnPaint()
{
	...
	for (int i = 0; i < GetControlCount(); i++)
	{
		...
		if ((m_nKeyboardTipsLevel > xtpNoTips) && (!IsMenuSeparator(pControl)) && (m_nKeyboardTipsLevel < xtpTipsOnTabPane))
		{
			if ((m_nKeyboardTipsLevel == xtpSecondLevel) && (pControl->GetKeyboardTip().GetAt(0) != m_chSecondLevelCurrentChar))
				continue;
...
void CXTPRibbonBackstageView::RecalcLayout()
{
	...
	bool bOldLook = (m_backStageStyle < xtpRibbonBackstageOffice2013Style) || ((m_themeCurrent != xtpThemeOffice2013) && (m_themeCurrent != xtpThemeVisualStudio2015));

	if (pRibbonBar->IsWindowVisible())
	{
		if (bOldLook)
			nCaptionHeight = pRibbonBar->GetCaptionHeight() + pRibbonBar->GetTabsHeight() + pRibbonBar->GetRibbonTopBorder() + XTP_DPI_Y(1);
		else
	...
	for (i = 0; i < GetControlCount(); i++)
	{
		...
		if (!bOldLook)
			...
			width = dc.GetTextExtent(pControl->GetCaption()).cx + m_pPaintManager->GetMenuItemCommandGapX()
				+ m_pPaintManager->GetMenuItemTextGap() + m_pPaintManager->GetMenuItemCommandGapX() + XTP_DPI_Y(6);
	...
	if (!bOldLook)
		m_pCommandBars->GetPaintManager()->SetBackstageModeMenuWidth(m_nMenuWidth);

	if (m_pSystemButton)
	{
		if (!bOldLook)
		{
			CRect rcButton(0, XTP_DPI_Y(gNewBacksategeSysButtonTop), m_nMenuWidth, XTP_DPI_Y(gNewBacksategeHeaderHeight));
	...
	for (i = 0; i < GetControlCount(); i++)
	{
		...
		if (IsCommand(pControl))
		{
			pControl->SetRect(CRect(m_pPaintManager->GetMenuItemCommandGapX(), top - m_nScrollPos,
				m_nMenuWidth - m_pPaintManager->GetMenuItemCommandGapX(), top + nCommandHeight - m_nScrollPos));
			top += nCommandHeight;
			if (i != last)
				top += nCommandGap;
		}
		else if(IsMenuSeparator(pControl))
		{
			...
			pControl->SetRect(CRect(0, top - m_nScrollPos, m_nMenuWidth, top + m_pPaintManager->GetMenuSeparatorHeight() - m_nScrollPos));
	...
	if (pWnd)
	{
		...
		if (dy > 0)
			...
			dy = -dy;
		pWnd->SetWindowPos(&wndBottom, rcContent.left, rcContent.top - m_nScrollPos, rcContent.Width(), rcContent.Height() + dy,
...
BOOL CXTPRibbonBackstageView::Popup(CXTPControlPopup* pControlPopup, BOOL /*bSelectFirst = FALSE*/)
{
	...
	if ((m_backStageStyle > xtpRibbonBackstageOffice2010Style) && ((m_themeCurrent == xtpThemeOffice2013) || (m_themeCurrent == xtpThemeVisualStudio2015)))
	{
		m_rcOldSysButtonRect = pSystemButton->GetRect();
		CXTPPaintManager *pPaintManager = m_pCommandBars->GetPaintManager();
		pPaintManager->SetBackstageSysButtonTop(gNewBacksategeSysButtonTop);
		pPaintManager->SetBackstageSysButtonBottom(gNewBacksategeHeaderHeight);
		pPaintManager->SetCaptionbarBackstageMode(TRUE);
...
{
	m_nKeyboardTipsLevel = xtpNoTips;
	m_pCommandBars->GetPaintManager()->SetCaptionbarBackstageMode(FALSE);
	m_bTracking = FALSE;
	...
	if ((m_backStageStyle > xtpRibbonBackstageOffice2010Style) && ((m_themeCurrent == xtpThemeOffice2013) || (m_themeCurrent == xtpThemeVisualStudio2015)))
	{
		...
		m_pCommandBars->GetPaintManager()->SetBackstageModeMenuWidth(0);
...
BOOL CXTPRibbonBackstageView::IsDPIIconsScallingEnabled()
{
	return m_pCommandBars->GetCommandBarsOptions()->GetToolbarsIconsScalling();
}
...
void CXTPRibbonBackstageView::SetBackstageStyle(XTPRibbonBackstageStyle style/* = xtpRibbonBackstageOffice2016Style*/)
{
	...
	for (i = 0; i < GetControlCount(); i++)
	{
		...
		if (strTips.IsEmpty())
		{
			...
			for (int j = 0; j < strCaption.GetLength(); j++)
			{
				...
				{
					if ((strCaption.GetAt(j) == _TCHAR('&')) && (strCaption.GetAt(j + 1) == _TCHAR('&')))
					{
						j++;
						continue;
					}
				...
				// Pass spaces
				if (_istspace(strCaption.GetAt(j)))
					continue;
...
XTPRibbonBackstageKeyboardTipResult CXTPRibbonBackstageView::TranslateKeyboardTip(UINT key, int &nControlIndex)
{
	if (m_nKeyboardTipsLevel == xtpTipsOnTabPane)
	{
...
BOOL CXTPRibbonBackstageView::ProcessKeyboardInput(UINT keyMessage, BOOL altPressed, UINT key, int repeat)
{
	if (altPressed && (0 == key))
	{
	...
	if (m_nKeyboardTipsLevel >= xtpUserHoldAltKey)
	{
		...
		case xtpSelectTab:
			...
			CXTPRibbonBackstageTab* pTab = DYNAMIC_DOWNCAST(CXTPRibbonBackstageTab, GetControl(index));
			if (pTab)
			{
				SetActiveTab(pTab);
...
void CXTPRibbonBackstageView::RepositionControls(int dy)
{
	...
	if (pWnd)
	{
		...
		if (m_rcOldPaneRect.top < 0)
			...
			dy = 0;
		pWnd->SetWindowPos(0, m_rcOldPaneRect.left, m_rcOldPaneRect.top, m_rcOldPaneRect.Width(), m_rcOldPaneRect.Height() + dy,
			SWP_SHOWWINDOW | SWP_NOZORDER | SWP_NOACTIVATE);
	}
}
...
BEGIN_DISPATCH_MAP(CXTPRibbonBackstageView, CXTPCommandBar)
	...
	DISP_FUNCTION_ID(CXTPRibbonBackstageView, "AddMenuSeparator", 1004, OleAddMenuSeparator, VT_DISPATCH, VTS_NONE)
	
	DISP_FUNCTION_ID(CXTPRibbonBackstageView, "GetForeColor", 1100, OleGetColorForeground, VT_COLOR, VTS_NONE)
	DISP_FUNCTION_ID(CXTPRibbonBackstageView, "GetBackColor", 1101, OleGetColorBackground, VT_COLOR, VTS_NONE)
END_DISPATCH_MAP()
