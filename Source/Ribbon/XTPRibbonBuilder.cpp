...
#include "Ribbon/XTPRibbonBuilder.h"
...
void CXTPRibbonBuilder::AddImageIndex(int nIndex, int nId, int nImageType)
{
	...
	arrImages[nIndex] = XTPToUIntChecked(nId);
}
...
void CXTPRibbonBuilder::BuildControl(CXTPPropExchange* pPX, CXTPControl* pControl)
{
	...
	CString strDefaultCommand;
	PX_String(pPX, _T("DEFAULT_COMMAND"), strDefaultCommand);
	if (strDefaultCommand == _T("TRUE") && xtpControlPopup == pControl->m_controlType)
	{
		pControl->m_controlType = xtpControlSplitButtonPopup;
	}
}
...
BOOL CXTPRibbonBuilder::BuildMainButtonPopupBar(CXTPPropExchange* pPX, CXTPRibbonBar* pRibbonBar)
{
	...
	while (pos)
	{
		...
		CString strSubElementName;
		PX_String(&pxControl, _T("ELEMENT_NAME"), strSubElementName);

		if (strSubElementName == _T("Separator"))
		{
			bBeginGroup = TRUE;
			continue;
		}
		...
		CCmdTarget* pElement = CreateElement(strSubElementName);
		if (pElement == NULL)
			continue;
		...
		CXTPPropExchangeSection pxSubElements(pxControl->GetSection(_T("ELEMENTS")));
		BuildControlPopupBar(&pxSubElements, pControl, RUNTIME_CLASS(CXTPRibbonSystemPopupBarPage));
...
BOOL CXTPRibbonBuilder::BuildTabElements(CXTPPropExchange* pPX, CXTPRibbonBar* pRibbonBar)
{
	...
	while (pos)
	{
		...
		CXTPPropExchangeSection pxSubElements(pxControl->GetSection(_T("ELEMENTS")));
		BuildControlPopupBar(&pxSubElements, pControl, RUNTIME_CLASS(CXTPPopupBar));
