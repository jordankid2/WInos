...
int CXTPRibbonGroup::_GetSizeSpecialDynamicSize()
{
	...
	for (int i = 0; i < nDataCount; i++)
	{
		...
		pControl->rcControl.SetRect(XTP_DPI_X(2) + x, y, XTP_DPI_X(2) + x + szControl.cx, y + szControl.cy);
...
void CXTPRibbonGroup::OnAdjustBorders(int nWidth, CRect rcBorder)
{
	...
	for (i = 0; i < nControlCount; i++)
	{
		...
		pControl->rcRow = CRect(rcBorder.left, rcBorder.top,
			rcBorder.left + nWidth, rcBorder.top + nGroupClientHeight - nGroupCaptionHeight);

		if (bCenterItems && !bFirstItem)
		{
			if (nColumn != pControl->rcControl.left)
			{
				CenterColumn(nFirstItem, i - 1, nGroupClientHeight - nGroupCaptionHeight - XTP_DPI_Y(3));
	...
	if (bCenterItems && nControlCount > 0) CenterColumn(nFirstItem, m_pLayoutInfo->nControlCount - 1, nGroupClientHeight - nGroupCaptionHeight - XTP_DPI_Y(3));
