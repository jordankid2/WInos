...
void CXTPRibbonScrollableBar::ShowScrollableRect(CXTPRibbonGroups* pGroups, CRect rc)
{
	...
	{
		nGroupsScrollPos = nGroupsScrollPos + rc.left - XTP_DPI_X(12);
		if (nGroupsScrollPos < 0) nGroupsScrollPos = 0;
	}
	else if (rc.right > rcGroups.right && m_pControlScrollGroupsRight->IsVisible())
	{
		nGroupsScrollPos = nGroupsScrollPos + rc.right - rcGroups.Width() + XTP_DPI_X(12);
		if (nGroupsScrollPos < 0) nGroupsScrollPos = 0;
	}
...
void CXTPRibbonScrollableBar::EnableGroupsScroll(BOOL bScrollLeft, BOOL bScrollRight)
{
	...
	if (bScrollLeft)
	{
		...
		m_pControlScrollGroupsLeft->SetRect(CRect(rcGroups.left - XTP_DPI_X(1), rcGroups.top, rcGroups.left + XTP_DPI_X(12), rcGroups.bottom));
	}
	else
	{
	...
	if (bScrollRight)
	{
		...
		m_pControlScrollGroupsRight->SetRect(CRect(rcGroups.right - XTP_DPI_X(12), rcGroups.top, rcGroups.right + XTP_DPI_X(1), rcGroups.bottom));
	}
	else
	{
...
CSize CXTPRibbonTabPopupToolBar::CalcDynamicLayout(int, DWORD /*nMode*/)
{
	...
	{
		nWidth = rcWork.right - m_ptPopup.x - XTP_DPI_X(4);
	}
	...
	m_pRibbonTab->GetGroups()->CalcDynamicSize(&dc, m_rcGroups.Width(), LM_COMMIT,
		CRect(m_rcGroups.left + XTP_DPI_X(4), m_rcGroups.top + XTP_DPI_Y(3), XTP_DPI_X(6), XTP_DPI_Y(3)));
...
CXTPRibbonGroupPopupToolBar::CXTPRibbonGroupPopupToolBar(CXTPRibbonGroup* pRibbonGroup /*= NULL*/)
{
	...
	m_rcBorders.SetRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(3), nHeight + XTP_DPI_Y(1));
...
CSize CXTPRibbonGroupPopupToolBar::CalcDynamicLayout(int, DWORD /*nMode*/)
{
	...
	return CSize(nWidth + m_rcBorders.left + m_rcBorders.right, nGroupHeight - XTP_DPI_Y(7));
}
