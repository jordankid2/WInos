...
CXTPChartDeviceCommand* CXTPChartMarker::CreateDeviceCommand(CXTPChartDeviceContext* pDC, const CXTPChartPointF& point, int nWidth, CXTPChartColor color, CXTPChartColor color2, CXTPChartColor colorBorder)
{
	...
	if (m_nType == xtpChartMarkerCircle)
	{
		...
		if (m_bBorderVisible)
		{
			pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedCircleDeviceCommand(point, nWidth / 2, colorBorder, 1));
		}
	}
	else if (m_nType == xtpChartMarkerSquare)
	{
		...
		if (m_bBorderVisible)
		{
			pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedPolygonDeviceCommand(arrPoints, colorBorder, 1));
		}
	}
