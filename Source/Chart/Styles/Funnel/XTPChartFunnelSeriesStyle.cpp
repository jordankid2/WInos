...
#include "Chart/Styles/Funnel/XTPChartFunnelSeriesStyle.h"
...
void CXTPChartFunnelSeriesPointView::CalculateLayout(int nIndex, int nCount, double dFrom, double dTo)
{
	...
	if (pStyle->IsEqualHeight())
	{
		...
		m_rc.Y = (float)CXTPMathUtils::Round(rcInnerRect.Y +(Height + pStyle->GetPointDistance())  * nIndex);

		int bottom = (int)CXTPMathUtils::Round(rcInnerRect.Y + (Height + pStyle->GetPointDistance()) * nIndex + Height);
	...
	else
	{
		...
		m_rc.Y = (float)CXTPMathUtils::Round(rcInnerRect.Y + dPosFrom);

		int bottom = (int)CXTPMathUtils::Round(rcInnerRect.Y + dPosTo);
...
IMPLEMENT_OLECREATE_EX2_CLSID(CXTPChartFunnelSeriesStyle, "Codejock.ChartFunnelSeriesStyle." _XTP_AXLIB_VERSION,
