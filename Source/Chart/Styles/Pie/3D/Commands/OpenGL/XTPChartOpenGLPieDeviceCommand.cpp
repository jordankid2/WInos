...
CXTPChartOpenGLPieDeviceCommand::CXTPChartOpenGLPieDeviceCommand(const CXTPPoint3d& center, double radius, 
	double dStartAngle, double dSweepAngle, double depth, double holePercent, 
	const CXTPChartRectF& gradientBounds, const CXTPChartColor& color, const CXTPChartColor& color2)
	: CXTPChart3dPieDeviceCommand(xtpChart3dAntialiasingPolicyEnabled, center, 
		radius, radius, dStartAngle, dSweepAngle, depth, holePercent)
...
CXTPChartElement* CXTPChartOpenGLPieDeviceCommand::HitTest(CPoint point, CXTPChartElement* pParent) const
{
...
struct CXTPChartOpenGLPieDeviceCommand::TorusRingInfo
{
	...
	void Compute(double dAngle, double dTorusRadius, double dRingRadius)
	{
		...
		for (int i = 0; i < nVerticeCount; ++i, dRingAngle += dStep)
		{
			...
			struct { double dCos, dSin; } yaw = { cos(dAngle), sin(dAngle) },
				pitch = { cos(dRingAngle), sin(dRingAngle) };
...
const int CXTPChartOpenGLPieDeviceCommand::TorusRingInfo::nVerticeCount = (360 / CXTPChart3dDiagram::m_nPrimitiveSize);
...
void CXTPChartOpenGLPieDeviceCommand::PrepareOverride(CXTPChartDeviceContext* pDC)
{
	...
	if (0 != m_listId)
	{
		glColor3f(m_color.ClampR(), m_color.ClampG(), m_color.ClampB());
		...
		{
			double dFromAngle = fabs(m_dStartAngle), dToAngle = fabs(m_dStartAngle) + fabs(m_dSweepAngle);
			if (m_pStyle->IsTorus())
			{
				DrawTorusPie(dFromAngle, dToAngle);
			}
			else
			{
				DrawBarPie(dFromAngle, dToAngle);
			}
...
void CXTPChartOpenGLPieDeviceCommand::DrawBarPie(double dFromAngle, double dToAngle)
{
	...
	while (true)
	{
		if (bFirstIteration)
		{
			...
			pRingFrom->Compute(CXTPMathUtils::Degree2Radian(dAngle), m_dRadius, m_dInnerRadius, m_dCorrectedDepth);
			DrawBarPie_OpenShape(*pRingFrom);
		...
		pRingTo->Compute(CXTPMathUtils::Degree2Radian(dAngle), m_dRadius, m_dInnerRadius, m_dCorrectedDepth);
		DrawBarPie_Shape(*pRingFrom, *pRingTo);
		...
		{
			// Second ring face
			DrawBarPie_CloseShape(*pRingTo);
			break;
		}
	}
}

void CXTPChartOpenGLPieDeviceCommand::DrawBarPie_OpenShape(const BarRingInfo& ring)
{
	CXTPPoint3d vFaceNormal = CXTPOpenGLUtils::ComputeTriangleNormal(ring.ptTopOuter, ring.ptTopInner, ring.ptBottomInner);

	glBegin(GL_TRIANGLES);
	CXTPOpenGLUtils::Vertex(ring.ptTopOuter, vFaceNormal);
	CXTPOpenGLUtils::Vertex(ring.ptTopInner, vFaceNormal);
	CXTPOpenGLUtils::Vertex(ring.ptBottomInner, vFaceNormal);
	CXTPOpenGLUtils::Vertex(ring.ptBottomInner, vFaceNormal);
	CXTPOpenGLUtils::Vertex(ring.ptBottomOuter, vFaceNormal);
	CXTPOpenGLUtils::Vertex(ring.ptTopOuter, vFaceNormal);
	...
	CXTPPoint3d ptFaceCenter = GetBarPieSliceCenter(4,
		&ring.ptTopOuter,
		&ring.ptTopInner,
		&ring.ptBottomInner,
		&ring.ptBottomOuter);
...
void CXTPChartOpenGLPieDeviceCommand::DrawBarPie_Shape(const BarRingInfo& ringFrom, const BarRingInfo& ringTo)
{
	static const CXTPPoint3d vTop(0, 1, 0), vBottom(0, -1, 0);
	glBegin(GL_TRIANGLES);

	// Outer face
	CXTPOpenGLUtils::Vertex(ringTo.ptTopOuter, ringTo.vOuterNormal);
	CXTPOpenGLUtils::Vertex(ringFrom.ptTopOuter, ringFrom.vOuterNormal);
	CXTPOpenGLUtils::Vertex(ringFrom.ptBottomOuter, ringFrom.vOuterNormal);
	CXTPOpenGLUtils::Vertex(ringFrom.ptBottomOuter, ringFrom.vOuterNormal);
	CXTPOpenGLUtils::Vertex(ringTo.ptBottomOuter, ringTo.vOuterNormal);
	CXTPOpenGLUtils::Vertex(ringTo.ptTopOuter, ringTo.vOuterNormal);
	...
	// Inner face
	CXTPOpenGLUtils::Vertex(ringFrom.ptTopInner, ringFrom.vInnerNormal);
	CXTPOpenGLUtils::Vertex(ringTo.ptTopInner, ringTo.vInnerNormal);
	CXTPOpenGLUtils::Vertex(ringTo.ptBottomInner, ringTo.vInnerNormal);
	CXTPOpenGLUtils::Vertex(ringTo.ptBottomInner, ringTo.vInnerNormal);
	CXTPOpenGLUtils::Vertex(ringFrom.ptBottomInner, ringFrom.vInnerNormal);
	CXTPOpenGLUtils::Vertex(ringFrom.ptTopInner, ringFrom.vInnerNormal);
	...
	// Top face
	CXTPOpenGLUtils::Vertex(ringTo.ptTopInner, vTop);
	CXTPOpenGLUtils::Vertex(ringFrom.ptTopInner, vTop);
	CXTPOpenGLUtils::Vertex(ringFrom.ptTopOuter, vTop);
	CXTPOpenGLUtils::Vertex(ringFrom.ptTopOuter, vTop);
	CXTPOpenGLUtils::Vertex(ringTo.ptTopOuter, vTop);
	CXTPOpenGLUtils::Vertex(ringTo.ptTopInner, vTop);
	m_3dBox.Include(ringTo.ptTopInner);
	m_3dBox.Include(ringFrom.ptTopInner);
	m_3dBox.Include(ringFrom.ptTopOuter);
	...
	// Bottom face
	CXTPOpenGLUtils::Vertex(ringTo.ptBottomInner, vBottom);
	CXTPOpenGLUtils::Vertex(ringFrom.ptBottomInner, vBottom);
	CXTPOpenGLUtils::Vertex(ringFrom.ptBottomOuter, vBottom);
	CXTPOpenGLUtils::Vertex(ringFrom.ptBottomOuter, vBottom);
	CXTPOpenGLUtils::Vertex(ringTo.ptBottomOuter, vBottom);
	CXTPOpenGLUtils::Vertex(ringTo.ptBottomInner, vBottom);
	m_3dBox.Include(ringTo.ptBottomInner);
	m_3dBox.Include(ringFrom.ptBottomInner);
	m_3dBox.Include(ringFrom.ptBottomOuter);
	...
	CXTPPoint3d ptFaceCenter = GetBarPieSliceCenter(4,
		&ringTo.ptTopInner,
		&ringTo.ptTopOuter,
		&ringTo.ptBottomOuter,
		&ringTo.ptBottomInner);
	m_3dBox.Include(ptFaceCenter, TRUE);
}

void CXTPChartOpenGLPieDeviceCommand::DrawBarPie_CloseShape(const BarRingInfo& ring)
{
	CXTPPoint3d vFaceNormal = CXTPOpenGLUtils::ComputeTriangleNormal(ring.ptTopOuter, ring.ptTopInner, ring.ptBottomInner);
...
void CXTPChartOpenGLPieDeviceCommand::DrawTorusPie(double dFromAngle, double dToAngle)
{
	double dAngle = dFromAngle;
	...
	while (true)
	{
		if (bFirstIteration)
		{
			...
			pRingFrom->Compute(CXTPMathUtils::Degree2Radian(dAngle), dTorusRadius, dTorusRingRadius);
			DrawTorusPie_OpenShape(*pRingFrom);
		...
		DrawTorusPie_Shape(*pRingFrom, *pRingTo);
		...
		{
			// Second side face
			DrawTorusPie_OpenShape(*pRingTo);
			break;
		}
	}
}

void CXTPChartOpenGLPieDeviceCommand::DrawTorusPie_OpenShape(const TorusRingInfo& ring)
{
	CXTPPoint3d vFaceNormal = CXTPOpenGLUtils::ComputeTriangleNormal(ring.ptCenter,
		ring.arVertices[0], ring.arVertices[1]);

	glBegin(GL_TRIANGLE_FAN);
	CXTPOpenGLUtils::Vertex(ring.ptCenter, vFaceNormal);
	m_3dBox.Include(ring.ptCenter, TRUE);

	for (int i = 0; i < TorusRingInfo::nVerticeCount; ++i)
	{
		CXTPOpenGLUtils::Vertex(ring.arVertices[i], vFaceNormal);
		m_3dBox.Include(ring.arVertices[i]);
	}

	CXTPOpenGLUtils::Vertex(ring.arVertices[0], vFaceNormal);
	m_3dBox.Include(ring.arVertices[0]);

	glEnd();
}

void CXTPChartOpenGLPieDeviceCommand::DrawTorusPie_Shape(
	...
	const TorusRingInfo& ringTo)
{
	...
	for (int i = 1; i <= TorusRingInfo::nVerticeCount; ++i)
	{
		...
		glBegin(GL_TRIANGLES);
		CXTPOpenGLUtils::Vertex(ringFrom.arVertices[nFrom], ringFrom.arNormals[nFrom]);
		CXTPOpenGLUtils::Vertex(ringFrom.arVertices[nTo], ringFrom.arNormals[nTo]);
		CXTPOpenGLUtils::Vertex(ringTo.arVertices[nTo], ringTo.arNormals[nTo]);
		CXTPOpenGLUtils::Vertex(ringTo.arVertices[nTo], ringTo.arNormals[nTo]);
		CXTPOpenGLUtils::Vertex(ringTo.arVertices[nFrom], ringTo.arNormals[nFrom]);
		CXTPOpenGLUtils::Vertex(ringFrom.arVertices[nFrom], ringFrom.arNormals[nFrom]);
		glEnd();
...
void CXTPChartOpenGLPieDeviceCommand::DrawTorusPie_CloseShape(const TorusRingInfo& ring)
{
	DrawTorusPie_OpenShape(ring);
}
...
void CXTPChartOpenGLPieDeviceCommand::ComputeConnectorPoints(double dFromAngle, double dToAngle)
{
	...
	double dCenterRadius = m_pStyle->IsTorus() 
		...
		: (0 != m_dInnerRadius ? (m_dRadius + m_dInnerRadius) / 2 : m_dRadius - m_dRadius * 0.33);
