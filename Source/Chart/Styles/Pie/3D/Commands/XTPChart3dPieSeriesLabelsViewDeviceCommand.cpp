...
CXTPChart3dPieSeriesLabelDeviceCommand::CXTPChart3dPieSeriesLabelDeviceCommand(CXTPChart3dPieSeriesLabelView* pSeriesLabelView)
	...
	, m_dConnectorEndpointVerticalOffset(0.)
{
	...
	m_pPointView = DYNAMIC_DOWNCAST(CXTPChart3dPieSeriesPointView, m_pSeriesLabelView->GetPointView());
...
void CXTPChart3dPieSeriesLabelDeviceCommand::ExecuteOverride(CXTPChartDeviceContext* pDC)
{
	if (!IsCommitted() && !IsFailed())
	{
		...
		if (NULL != ptStart && NULL != ptEnd)
		{
			...
			ASSERT(NULL != pLabel);
			if (pLabel->GetShowLines()
				&& m_pSeriesLabelView->IsConnectorVisible()
				&& xtpChartPieLabelInside != position
				&& xtpChartPieLabelRadial != position)
			{
				if (IsBent())
				{
					...
					if (NULL != ptBend)
					{
						pCommand = pDC->GetDrawingObjectFactory()->CreateSolidLineDeviceCommand(
							*ptStart, *ptBend, m_pSeriesLabelView->GetPointView()->GetColor(),
							XTP_CHART_PIE3D_OUTER_CONNECTOR_THICKNESS);
						AddChildCommand(pCommand);

						pCommand = pDC->GetDrawingObjectFactory()->CreateSolidLineDeviceCommand(
							*ptBend, *ptEnd, m_pSeriesLabelView->GetPointView()->GetColor(),
							XTP_CHART_PIE3D_OUTER_CONNECTOR_THICKNESS);
						AddChildCommand(pCommand);
					}
				}
			...
			AddChildCommand(pLabel->GetFillStyle()->CreateDeviceCommand(m_rcLabel, clrBackColor, clrBackColor));

			CXTPChartTextPainter painter(pDC, strText, pLabel);
			CXTPChartSizeF szLabel = painter.GetSize();
			painter.SetLocation(CXTPChartPointF(m_rcLabel.left, m_rcLabel.top));
			...
			if (0 < nThickness)
			{
				pCommand = pLabel->GetBorder()->CreateDeviceCommand(m_rcLabel, m_pSeriesLabelView->GetActualBorderColor());
				AddChildCommand(pCommand);
			}
		}
...
void CXTPChart3dPieSeriesLabelDeviceCommand::ComputeLabel(CXTPChart3dDeviceContext* pDC)
{
	...
	case xtpChartPieLabelTwoColumns:
		ComputeLabel_TwoColumn(szLabel, *ptAnchor);
		break;
	...
	case xtpChartPieLabelRadial:
		ComputeLabel_InsideAndRadial(szLabel, *ptAnchor);
		break;
	default:
	...
	CXTPChart3dDeviceContext* pDC, int y)
{
	...
	int xOffset = m_pSeriesLabelView->IsLeftDirected()
		? XTP_CHART_PIE3D_BENT_CONNECTOR_ARM_LENGTH : -XTP_CHART_PIE3D_BENT_CONNECTOR_ARM_LENGTH;
...
void CXTPChart3dPieSeriesLabelDeviceCommand::ComputeLabel_TwoColumn(
	const CSize& szLabel, const CPoint& ptAnchor)
{
	ASSERT(xtpChartPieLabelTwoColumns == m_pSeriesLabelView->GetPosition());

	if (m_pSeriesLabelView->IsLeftDirected())
	{
		m_rcLabel.SetRect(ptAnchor.x - szLabel.cx, ptAnchor.y - szLabel.cy / 2,
			ptAnchor.x, ptAnchor.y + szLabel.cy / 2);
	}
	else
	{
		m_rcLabel.SetRect(ptAnchor.x, ptAnchor.y - szLabel.cy / 2,
			ptAnchor.x + szLabel.cx, ptAnchor.y + szLabel.cy / 2);
	}
}

void CXTPChart3dPieSeriesLabelDeviceCommand::ComputeLabel_Outside(
	const CSize& szLabel, const CPoint& ptAnchorOrigin, const CPoint& ptAnchor)
{
...
void CXTPChart3dPieSeriesLabelDeviceCommand::ComputeLabel_InsideAndRadial(
	const CSize& szLabel, const CPoint& ptAnchor)
{
...
BOOL CXTPChart3dPieSeriesLabelDeviceCommand::ComputeConnectorBend(
...
CXTPPoint3d CXTPChart3dPieSeriesLabelDeviceCommand::ComputeConnectorBendPoint() const
{
	...
	CXTPPoint3d point = ComputeConnectorVectorEndPoint(m_pPointView->GetOuterCenterConnectorPoint());
	return point;
}
...
CXTPPoint3d CXTPChart3dPieSeriesLabelDeviceCommand::ComputeConnectorEndPoint() const
{
	...
	case xtpChartPieLabelInside:
	case xtpChartPieLabelRadial:
		point = m_pPointView->GetCenterShapeConnectorPoint();
		break;
	default:
		ASSERT(!"Unexpected position");
		break;
...
CXTPPoint3d CXTPChart3dPieSeriesLabelDeviceCommand::ComputeConnectorVectorEndPoint(const CXTPPoint3d& point) const
{
	CXTPPoint3d ptEnd = point;
	double dLength = sqrt(point.X * point.X
		+ point.Y * point.Y
		+ point.Z * point.Z);
	ptEnd.X /= dLength;
	ptEnd.Y /= dLength;
	...
	ptEnd.Y *= dLength;
	ptEnd.Z *= dLength;
	ptEnd.Y += m_dConnectorEndpointVerticalOffset;
...
CXTPChart3dPieSeriesLabel* CXTPChart3dPieSeriesLabelsViewDeviceCommand::GetLabel() const
{
	...
	{
		CXTPChart3dPieSeriesLabelDeviceCommand* pLabelCommand = FindLabelDeviceCommand(m_arrChildren[i]);
		if (NULL != pLabelCommand)
		{
			pLabel = DYNAMIC_DOWNCAST(CXTPChart3dPieSeriesLabel, pLabelCommand->GetSeriesLabelView()->GetLabel());
		}
	}
	...
	CXTPChart3dDeviceContext* pDC, BOOL& bNeedComputeBends)
{
	...
	for (int i = 0; i < m_arrChildren.GetSize(); i++)
	{
		...
		{
			CXTPChartDeviceCommand* pTranspositionCommand = pLabelCommand->GetSeriesLabelView()
				->CreateTranspositionDeviceCommand(pDC);
	...
	CXTPChart3dDeviceContext* pDC, CRect& rcBoundaries)
{
	...
	for (int i = 0; i < m_arrChildren.GetSize(); i++)
	{
		CXTPChart3dPieSeriesLabelDeviceCommand* pLabelCommand = FindLabelDeviceCommand(m_arrChildren[i]);
		if (NULL != pLabelCommand 
			&& pLabelCommand->IsBent() 
			&& !pLabelCommand->IsFailed())
		{
			...
			if (NULL != ptBend)
			{
				...
				CXTPChartDeviceCommand* pTranspositionCommand = pLabelView->CreateTranspositionDeviceCommand(pDC);
				ASSERT(NULL != pTranspositionCommand);
				if (NULL != pTranspositionCommand)
				{
	...
	if (rcBoundaries.left < rcBoundaries.right
		&& rcBoundaries.top < rcBoundaries.bottom)
	{
	...
	CXTPChart3dDeviceContext* pDC, const CRect& rcBoundaries)
{
	...
	if (NULL != pLabel)
	{
		...
		{
		case xtpChartPieLabelTwoColumns:
			bOverlapsResolved = (ResolveLabelOverlaps_ChangePositionInColumn(pDC, TRUE, rcBoundaries)
	...
	CXTPChart3dDeviceContext* pDC, BOOL bLeftDirected, const CRect& rcBoundaries)
{
	...
	if (0 < nLabelCount)
	{
		...
		{
			labels[i].fDistributionFactor = static_cast<float>(labels[i].ptAnchor.y - nMinY) / (nMaxY - nMinY);
		}

		// Sort according distribution factors
		qsort(labels.GetData(), nLabelCount, sizeof(LabelInfo), LabelInfo::Compare);
		...
		for (i = 0; i < nLabelCount; ++i)
		{
			...
			CXTPChartDeviceCommand* pTranspositionCommand = labelInfo.pCommand->GetSeriesLabelView()
				->CreateTranspositionDeviceCommand(pDC);
			ASSERT(NULL != pTranspositionCommand);
			if (NULL != pTranspositionCommand)
			{
...
BOOL CXTPChart3dPieSeriesLabelsViewDeviceCommand::ResolveLabelOverlaps_ChangeConnectorLength(CXTPChart3dDeviceContext* pDC)
{
			...
			while (NULL != pos)
			{
				...
				if (rcIntersection.IntersectRect(rcLabel, pOtherLabelCommand->GetLabelRect()))
				{
					...
					if (!bOverlapped)
					{
						...
						dVerticalOffset = (0 == (nLabelCounter % 2) ? rcLabel.Height() : -rcLabel.Height());
					}
...
CXTPChart3dPieSeriesLabelDeviceCommand* CXTPChart3dPieSeriesLabelsViewDeviceCommand::FindLabelDeviceCommand(
...
void CXTPChart3dPieSeriesLabelsViewDeviceCommand::OnAfterExecute(
	...
	case CommandCallbackData::ComputeVector:
		{
			...
			m_CommandCallbackData.bSuccess = m_CommandCallbackData.pLabelCommand
				->ComputeConnectorVector(m_CommandCallbackData.pDC);
			break;
		}
	...
	case CommandCallbackData::ComputeBendConnector:
		{
			...
			m_CommandCallbackData.bSuccess = m_CommandCallbackData.pLabelCommand
				->ComputeConnectorBend(m_CommandCallbackData.pDC, m_CommandCallbackData.prcBoundaries);
			break;
		}
	case CommandCallbackData::ColumnAnchorVericalPosition:
		{
			...
			m_CommandCallbackData.bSuccess = m_CommandCallbackData.pLabelCommand->SetColumnAnchorVerticalPosition(
				m_CommandCallbackData.pDC, m_CommandCallbackData.nNewYPos);
			break;
		}
	default:
		ASSERT(!"Unexpected command callback data state");
		break;
	}
}
