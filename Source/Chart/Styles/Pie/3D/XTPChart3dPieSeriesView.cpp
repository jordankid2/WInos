...
CXTPChart3dPieSeriesView::CXTPChart3dPieSeriesView(CXTPChartSeries* pSeries, CXTPChartDiagramView* pDiagramView)
...
void CXTPChart3dPieSeriesView::CalculatePointLayout(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	{
		CXTPChartPieSeriesPointViewBase* pPointView = (CXTPChartPieSeriesPointViewBase*)m_pPointsView->GetAt(i);
		pPointView->CalculateLayout(dAngle);
	}
}

CXTPChartSeriesPointView* CXTPChart3dPieSeriesView::CreateSeriesPointView(CXTPChartDeviceContext* pDC, CXTPChartSeriesPoint* pPoint, CXTPChartElementView* pParentView)
{
	UNREFERENCED_PARAMETER(pDC);
...
{
	CXTPChartDeviceCommand* pCommand = CXTPChartDrawingObjectFactory::Create3dShadeModelDeviceCommand();
	pCommand->AddChildCommand(CXTPChartSeriesView::CreateDeviceCommand(pDC));
	return pCommand;
}

CXTPChartDeviceCommand* CXTPChart3dPieSeriesView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	UNREFERENCED_PARAMETER(pDC);
