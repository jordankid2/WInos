...
IMPLEMENT_SERIAL(CXTPChart3dPieSeriesLabel, CXTPChartPieSeriesLabelBase, VERSIONABLE_SCHEMA | _XTP_SCHEMA_CURRENT)
...
CXTPChartElementView* CXTPChart3dPieSeriesLabel::CreateView(CXTPChartDeviceContext* pDC, CXTPChartSeriesPointView* pPointView, CXTPChartElementView* pParentView)
{
...
CXTPChart3dPieSeriesLabelView::CXTPChart3dPieSeriesLabelView(CXTPChartSeriesLabel* pLabel, CXTPChartSeriesPointView* pPointView, CXTPChartElementView* pParentView)
...
CXTPChartDeviceCommand* CXTPChart3dPieSeriesLabelView::CreateTranspositionDeviceCommand(CXTPChartDeviceContext* pDC)
{
	UNREFERENCED_PARAMETER(pDC);

	CXTPChart3dPieSeriesStyle* pStyle = DYNAMIC_DOWNCAST(CXTPChart3dPieSeriesStyle, m_p3dPointView->GetSeriesView()->GetStyle());
	ASSERT(NULL != pStyle);

	CXTPChartDeviceCommand* pCommand = CXTPChartDrawingObjectFactory::Create3dSaveMatrixDeviceCommand();

	// Rotate if rotated
	const CXTPChart3dRotation& rotation = pStyle->Get3dRotation();
	if (rotation.IsRotated())
	{
		pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create3dRotateDeviceCommand(rotation));
	}
	...
	if (!ptCenter.IsZero())
	{
		pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create3dTranslateDeviceCommand(
			ptCenter.X, 0, -ptCenter.Y));
	}

	return pCommand;
}

CXTPChartDeviceCommand* CXTPChart3dPieSeriesLabelView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
...
CXTPChartDeviceCommand* CXTPChart3dPieSeriesLabelView::CreateConnectorDeviceCommand(CXTPChartDeviceContext* pDC)
{
	UNREFERENCED_PARAMETER(pDC);
...
{
	return INT_MIN != m_ptLabelAnchor.x || INT_MIN != m_ptLabelAnchor.y
		? &m_ptLabelAnchor : NULL;
}
...
CXTPChartDeviceCommand* CXTPChart3dPieSeriesLabelsView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
...
CXTPChart3dPieSeriesStyle* CXTPChart3dPieSeriesLabelsView::GetStyle() const
{
	...
	{
		CXTPChartPieSeriesLabelViewBase* pChildView = static_cast<CXTPChartPieSeriesLabelViewBase*>(m_arrChildren[i]);
		pStyle = DYNAMIC_DOWNCAST(CXTPChart3dPieSeriesStyle, pChildView->GetPointView()->GetSeriesView()->GetStyle());
		ASSERT(NULL != pStyle);
	}
