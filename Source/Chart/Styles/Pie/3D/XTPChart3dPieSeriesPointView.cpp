...
CXTPChart3dPieSeriesPointView::CXTPChart3dPieSeriesPointView(CXTPChartSeriesPoint* pPoint, CXTPChartElementView* pParentView)
...
{
	m_pCommandSubscriptionList = new CXTPSubscriptionList<CXTPChartDeviceCommand, CXTPChartDeviceCommandEvents>(*this);
}
...
void CXTPChart3dPieSeriesPointView::CalculateLayout(double& dStartAngle)
{
	...
	CXTPChartEllipse ellipse(CXTPPoint3d(), CXTPChart3dDiagram::m_fChartBoxSize / 2, CXTPChart3dDiagram::m_fChartBoxSize / 2);
...
CXTPChartDeviceCommand* CXTPChart3dPieSeriesPointView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	if (NULL != m_pPie)
	{
		CXTPChartPieSeriesStyleBase* pStyle = DYNAMIC_DOWNCAST(CXTPChartPieSeriesStyleBase, GetSeriesView()->GetStyle());
		ASSERT(NULL != pStyle);
		...
		CXTPChartDeviceCommand* pPieCommand = m_pPie->CreatePieDeviceCommand(color1, color2, m_basePoint);
...
CXTPChartDeviceCommand* CXTPChart3dPieSeriesPointView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
...
void CXTPChart3dPieSeriesPointView::OnSetConnectorPoints(
