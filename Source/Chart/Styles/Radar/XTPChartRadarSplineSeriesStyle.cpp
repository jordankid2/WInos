...
CXTPChartDeviceCommand* CXTPChartRadarSplineSeriesView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (CXTPMathUtils::IsNan(pPointView->m_dInternalValue))
		{
			if (points.GetSize() > 1)
			{
				pCommand->AddChildCommand(pStyle->GetLineStyle()->CreateSplineDeviceCommand(points, m_pSeries->GetColor()));
			}
			points.RemoveAll();
			continue;
		}
...
IMPLEMENT_OLECREATE_EX2_CLSID(CXTPChartRadarSplineSeriesStyle, "Codejock.ChartRadarSplineSeriesStyle." _XTP_AXLIB_VERSION,
