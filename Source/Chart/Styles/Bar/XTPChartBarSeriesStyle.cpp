...
CXTPChartDeviceCommand* CXTPChartBarSeriesView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	pCommand->AddChildCommand(pStyle->GetFillStyle()->CreateDeviceCommand(rcBounds, m_pSeries->GetColor(), m_pSeries->GetColor2()));

	CXTPChartDeviceCommand* pInnerBorderDeviceCommand = CXTPChartDrawingObjectFactory::Create2dInnerBorderDeviceCommand(rcBounds, m_pSeries->GetColor().GetDarkColor(), 1);
...
CXTPChartDeviceCommand* CXTPChartBarSeriesPointView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	CXTPChartDeviceCommand* pContainer = CXTPChartDrawingObjectFactory::Create2dPolygonAntialiasingDeviceCommand(FALSE);
	CXTPChartDeviceCommand* pCommand = pContainer->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dHitTestElementCommand(m_pPoint, rc));
...
CXTPChartDeviceCommand* CXTPChartBarSeriesPointView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	CXTPChartDeviceCommand* pInnerBorderDeviceCommand = CXTPChartDrawingObjectFactory::Create2dInnerBorderDeviceCommand(rcBounds, GetColor().GetDarkColor(), 1);
	pCommand->AddChildCommand(pInnerBorderDeviceCommand);
...
IMPLEMENT_OLECREATE_EX2_CLSID(CXTPChartBarSeriesStyle, "Codejock.ChartBarSeriesStyle." _XTP_AXLIB_VERSION,
