...
CXTPChartDeviceCommand* CXTPChartAreaSeriesView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidLineDeviceCommand(arrPoints[0], arrPoints[1], clrBorder, 1));
	pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidLineDeviceCommand(arrPoints[1], arrPoints[2], clrBorder, 1));
...
CXTPChartDeviceCommand* CXTPChartAreaSeriesView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	if (nCount > 1)
	{
		...
		if (pStyle->GetBorder()->IsVisible())
			pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidPolylineDeviceCommand(arrPoints, clrBorder, pStyle->GetBorder()->GetThickness()));
	}
...
IMPLEMENT_OLECREATE_EX2_CLSID(CXTPChartAreaSeriesStyle, "Codejock.ChartAreaSeriesStyle." _XTP_AXLIB_VERSION,
