...
{
	CXTPChartDeviceCommand* pSeriesCommand = CXTPChartDrawingObjectFactory::Create2dHitTestElementCommand(m_pSeries);
	...
	if (nCount > 1)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (pPointPrev)
			{
				...
				pCommand->AddChildCommand(pStyle->GetLineStyle()->CreateDeviceCommand(pointPrev, pointMiddle, m_pSeries->GetColor()));

				pCommand->AddChildCommand(pStyle->GetLineStyle()->CreateDeviceCommand(pointMiddle, pointNext, m_pSeries->GetColor()));
			}
...
CXTPChartDeviceCommand* CXTPChartStepLineSeriesView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	pCommand->AddChildCommand(pStepLineSeriesStyle->GetLineStyle()->CreateDeviceCommand(CXTPPoint3d(rcBounds.left, rcBounds.bottom),
		CXTPPoint3d(rcBounds.left, ptCenter.Y), m_pSeries->GetColor()));
	pCommand->AddChildCommand(pStepLineSeriesStyle->GetLineStyle()->CreateDeviceCommand(CXTPPoint3d(rcBounds.left, ptCenter.Y),
		CXTPPoint3d(rcBounds.right, ptCenter.Y), m_pSeries->GetColor()));
	pCommand->AddChildCommand(pStepLineSeriesStyle->GetLineStyle()->CreateDeviceCommand(CXTPPoint3d(rcBounds.right, ptCenter.Y),
		CXTPPoint3d(rcBounds.right, rcBounds.top), m_pSeries->GetColor()));
...
CXTPChartDeviceCommand* CXTPChartStepLineSeriesPointView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidLineDeviceCommand(CXTPPoint3d(rcBounds.left, rcBounds.bottom),
		CXTPPoint3d(rcBounds.left, ptCenter.Y), GetSeriesView()->m_pSeries->GetColor(), 1));
	pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidLineDeviceCommand(CXTPPoint3d(rcBounds.left, ptCenter.Y),
		CXTPPoint3d(rcBounds.right, ptCenter.Y), GetSeriesView()->m_pSeries->GetColor(), 1));
	pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidLineDeviceCommand(CXTPPoint3d(rcBounds.right, ptCenter.Y),
		CXTPPoint3d(rcBounds.right, rcBounds.top), GetSeriesView()->m_pSeries->GetColor(), 1));
...
IMPLEMENT_OLECREATE_EX2_CLSID(CXTPChartStepLineSeriesStyle, "Codejock.ChartStepLineSeriesStyle." _XTP_AXLIB_VERSION,
