...
CXTPChartDeviceCommand* CXTPChartLineSeriesView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	if (nCount > 1)
	{
		if (pStyle->GetLineStyle()->GetDashStyle() != xtpChartDashStyleSolid)
		{
			...
			pCommand->AddChildCommand(pStyle->GetLineStyle()->CreateDeviceCommand(arrPoints, m_pSeries->GetColor()));
		}
		else
		{
...
CXTPChartDeviceCommand* CXTPChartLineSeriesView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	CXTPChartLineSeriesStyle* pLineSeriesStyle = DYNAMIC_DOWNCAST(CXTPChartLineSeriesStyle, m_pSeries->GetStyle());
	...
	else
	{
		...
		pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dDashedLineDeviceCommand(CXTPPoint3d(rcBounds.left,ptCenter.Y),
		CXTPPoint3d(rcBounds.right,ptCenter.Y), m_pSeries->GetColor(), m_pSeries->GetLegendItem()->GetActualLineThickness(), m_pSeries->GetLegendItem()->GetDashStyle(), dashArray));
	}
