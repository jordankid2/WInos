...
CXTPChartDeviceCommand* CXTPChartCandleStickSeriesView::CreateLegendDeviceCommand(CXTPChartDeviceContext* pDC, CRect rcBounds)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		rc.X += 2 / 2.0f;
		rc.Width -= 2;
		pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedRectangleDeviceCommand(rc, color, 2));
...
CXTPChartDeviceCommand* CXTPChartCandleStickSeriesPointView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	if (bFilled)
	{
		pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidRectangleDeviceCommand(rc, color));
	}
	else
	{
		rc.X += nLineThickness / 2.0f;
		rc.Width -= nLineThickness;
		pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedRectangleDeviceCommand(rc, color, nLineThickness));
	}
...
IMPLEMENT_OLECREATE_EX2_CLSID(CXTPChartCandleStickSeriesStyle, "Codejock.ChartCandleStickSeriesStyle." _XTP_AXLIB_VERSION,
