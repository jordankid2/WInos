...
CXTPChart3dRotation::CXTPChart3dRotation(double dYaw, double dPitch, double dRoll)
	: m_dYaw(dYaw), m_dPitch(dPitch), m_dRoll(dRoll)
{
#ifdef _XTP_ACTIVEX
	EnableAutomation();
	EnableTypeLib();
...
void CXTPChart3dRotation::SetFromString(LPCTSTR lpsz)
{
	...
	int res = SCANF_S(lpsz, _T("%lf, %lf, %lf"), &rotation.m_dYaw, &rotation.m_dPitch, &rotation.m_dRoll);
	if (res == 3)
	{
		*this = rotation;
	}
...
CXTPPoint3d CXTPChart3dBox::GetCenter(BOOL bAmongstCenterCandidates /*= FALSE*/) const
{
	...
	if (bAmongstCenterCandidates)
	{
		...
		if (NULL != pos)
		{
			...
			ptCenter.X /= m_CenterCandidates.GetCount();
			ptCenter.Y /= m_CenterCandidates.GetCount();
			ptCenter.Z /= m_CenterCandidates.GetCount();
			...
			{
				const CXTPPoint3d& pt = m_CenterCandidates.GetNext(pos);
				double dist = CXTPMathUtils::GetSegmentLength(pt.X, pt.Y, pt.Z,
...
BOOL AFX_CDECL PX_Color(CXTPPropExchange* pPX, LPCTSTR pszPropName, 
	CXTPChartColor& clrValue, CXTPChartColor clrDefault /*= CXTPChartColor::Empty*/)
{
	ASSERT_VALID(pPX);
	BOOL bSuccess = FALSE;
...
BOOL AFX_CDECL PX_Rotation(CXTPPropExchange* pPX, LPCTSTR pszPropName, CXTPChart3dRotation& rotation)
{
	ASSERT_VALID(pPX);
