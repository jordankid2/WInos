...
CXTPChartRectF AFX_CDECL CXTPChartSeriesLabelConnectorPainterBase::CalcBorderBoundsForTangentDrawing(const CXTPPoint3d& point, double angle, const CXTPChartSizeF& textSize, int borderThickness, CXTPChartRectF& innerBounds)
{
...
CXTPChartSeriesLabelLineConnectorPainter::CXTPChartSeriesLabelLineConnectorPainter(CXTPChartPointF startPoint, CXTPChartPointF finishPoint, CXTPChartRectF bounds)
	: CXTPChartSeriesLabelConnectorPainterBase(startPoint, finishPoint, bounds)
{
}

CXTPChartDeviceCommand* CXTPChartSeriesLabelLineConnectorPainter::CreateDeviceCommand(CXTPChartDeviceContext* pDC, const CXTPChartColor& color)
{
	UNREFERENCED_PARAMETER(pDC);

	CXTPChartDeviceCommand* pContaier = CXTPChartDrawingObjectFactory::Create2dPolygonAntialiasingDeviceCommand();
	pContaier->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidLineDeviceCommand(m_startPoint, m_finishPoint, color, 1));
