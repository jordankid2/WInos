...
IMPLEMENT_OLECREATE_EX2_CLSID(CXTPChartSeriesTrendline, "Codejock.ChartSeriesTrendline." _XTP_AXLIB_VERSION,
...
void CXTPChartSeriesTrendline::OleSetSource(LPDISPATCH lpSource)
{
	if (NULL != lpSource)
	{
		CXTPChartSeries* pSeries = DYNAMIC_DOWNCAST(CXTPChartSeries, 
			CXTPChartSeries::FromIDispatchSafe(lpSource));
		if (NULL == pSeries)
		{
