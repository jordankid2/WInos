...
void CXTPChartDeviceCommandEvents::OnBeforeExecute(
	CXTPChartDeviceCommand* pCommand, CXTPChartDeviceContext* pDC) {
	UNREFERENCED_PARAMETER(pCommand);
	UNREFERENCED_PARAMETER(pDC);
}

void CXTPChartDeviceCommandEvents::OnAfterExecute(
	CXTPChartDeviceCommand* pCommand, CXTPChartDeviceContext* pDC) {
	UNREFERENCED_PARAMETER(pCommand);
	UNREFERENCED_PARAMETER(pDC);
}

void CXTPChartDeviceCommandEvents::OnUpdateBounds(
	CXTPChartDeviceCommand* pCommand, CRect rcBounds) {
	UNREFERENCED_PARAMETER(pCommand);
...
BOOL CXTPChart3dDeviceCommand::IsEnabled() const
{
	...
	if (bEnabled && xtpChart3dAntialiasingPolicyDefault != m_nAntialiasingPolicy)
	{
		...
		if (NULL != pCurrentDC)
		{
			CXTPChart3dDeviceContext* p3DDC = DYNAMIC_DOWNCAST(CXTPChart3dDeviceContext, pCurrentDC);
			if (NULL != p3DDC)
			{
