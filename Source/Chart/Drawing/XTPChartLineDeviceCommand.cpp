...
CXTPChartLineDeviceCommand::CXTPChartLineDeviceCommand(
...
CXTPChartElement* CXTPChartLineDeviceCommand::HitTest(CPoint point, CXTPChartElement* pParent) const
{
	...
	if (CXTPMathUtils::HitTestLineSegment(m_p1.X, m_p1.Y, 
		m_p2.X, m_p2.Y, point.x, point.y, m_nThickness))
	{
		pHitElement = pParent;
	}
...
CXTPChart3dLineDeviceCommand::CXTPChart3dLineDeviceCommand(
...
CXTPChartElement* CXTPChart3dLineDeviceCommand::HitTest(CPoint point, CXTPChartElement* pParent) const
{
	CXTPChartElement* pHitElement = NULL;

	if (m_bProjected && CXTPMathUtils::HitTestLineSegment(m_pt2d1.x, m_pt2d1.y,
		m_pt2d2.x, m_pt2d2.y, point.x, point.y, m_nThickness))
	{
		pHitElement = pParent;
	}
