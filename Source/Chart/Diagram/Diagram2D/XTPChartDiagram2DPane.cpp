...
{
	CXTPChartDeviceCommand* pCommand = CXTPChartDrawingObjectFactory::Create2dHitTestElementCommand(m_pPane, m_rcBounds);
	...
	pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedRectangleDeviceCommand(m_rcBounds, m_pPane->GetActualBorderColor(), 1));

	CRect rcBounds(m_rcBounds);
	rcBounds.DeflateRect(1, 1, 0, 0);

	CXTPChartDeviceCommand* pClipDeviceCommand = CXTPChartDrawingObjectFactory::Create2dClipDeviceCommand(rcBounds);
