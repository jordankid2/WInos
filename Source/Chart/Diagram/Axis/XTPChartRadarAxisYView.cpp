...
CXTPChartDeviceCommand* CXTPChartRadarAxisYView::CreateInterlacedDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	for (int i = 0; i < m_arrTicks.GetSize(); i += 2)
	{
		...
		{
			CXTPChartDeviceCommand* pCommand = CXTPChartDrawingObjectFactory::Create2dGradientPieDeviceCommand(CXTPPoint3d(m_ptCenter), dRTo, dRTo,
...
CXTPChartDeviceCommand* CXTPChartRadarAxisYView::CreateConstantLinesDeviceCommand(CXTPChartDeviceContext* pDC, BOOL bBehind)
{
	...
	for (int i = 0; i < pConstantLines->GetCount(); i++)
	{
		...
		if (IsPolygonDiagramStyle())
			...
			pCommands->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedCircleDeviceCommand(m_ptCenter, dRad, pConstantLine->GetActualColor(),
...
CXTPChartDeviceCommand* CXTPChartRadarAxisYView::CreateGridLinesDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	for (int i = 0; i < m_arrTicks.GetSize(); i++)
	{
		...
		if (IsPolygonDiagramStyle())
			...
			pCommands->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedCircleDeviceCommand(m_ptCenter, dRadius, clrGridLines, pGridLines->GetLineStyle()->GetThickness()));
	}
	...
	if (bMinorVisible)
	{
		for (int i = 0; i < m_arrMinorTicks.GetSize(); i++)
		{
			...
			if (IsPolygonDiagramStyle())
				...
				pCommands->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedCircleDeviceCommand(m_ptCenter, dRadius, clrMinorGridLines, pGridLines->GetMinorLineStyle()->GetThickness()));
		}
	}
