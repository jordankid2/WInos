...
void CXTPChartAxisRange::UpdateRange(CXTPChartAxisView* pView)
{
	...
	for (int i = 0; i < pView->m_arrSeries.GetSize(); i++)
	{
		...
		for (int j = 0; j < pSeriesView->GetPointsView()->GetCount(); j++)
		{
			...
			CXTPChartErrorBarView *pErrorBarView = static_cast<CXTPChartErrorBarView*>(pSeriesView->GetErrorBarsView()->GetAt(j));
...
INTERFACE_PART(CXTPChartAxisRange, XTPDIID_ChartAxisRange, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPChartAxisRange, XTPDIID_ChartAxisRange)
