...
void CXTPChartAxis::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_Int(pPX, _T("Index"), m_nIndex, 0);
...
BEGIN_DISPATCH_MAP(CXTPChartAxis, CXTPChartElement)
	...
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "Index", 25, GetIndex, SetIndex, VT_I4)

END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPChartAxis, CXTPChartElement)
	INTERFACE_PART(CXTPChartAxis, XTPDIID_ChartAxis, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPChartAxis, XTPDIID_ChartAxis)
...
{
	CXTPChartColor CurrColor;
	if (!m_clrColor.IsEmpty())
		return m_clrColor.ToOleColor();
	else
	{
		if (GetAppearance())
			return GetAppearance()->Color.ToOleColor();
		else
			return m_clrColor.ToOleColor();
	}
