...
CXTPChartDeviceCommand* CXTPChartRadarAxisXView::CreateInterlacedDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	for (int i = 0; i + 1 < m_arrTicks.GetSize(); i += 2)
	{
		...
		{
			CXTPChartDeviceCommand* pCommand = CXTPChartDrawingObjectFactory::Create2dGradientPieDeviceCommand(CXTPPoint3d(m_ptCenter), m_nRadius, m_nRadius,
...
CXTPChartDeviceCommand* CXTPChartRadarAxisXView::CreateTickMarksDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	for (int i = 0; i < m_arrTicks.GetSize(); i++)
	{
		...
		pCommands->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidLineDeviceCommand(startPoint, finishPoint, clrAxis, nThickness));
	}
	...
	if (bMinorVisible && !IsPolygonDiagramStyle())
	{
		...
		for (int i = 0; i < m_arrMinorTicks.GetSize(); i++)
		{
			...
			CXTPChartPointF startPoint(m_ptCenter.X + (m_nRadius - nExtra)  * cs, m_ptCenter.Y - (m_nRadius - nExtra) * sn);
			CXTPChartPointF finishPoint(m_ptCenter.X + (m_nRadius + nAxisThickness + nLength) * cs, m_ptCenter.Y - (m_nRadius + nAxisThickness + nLength) * sn);

			pCommands->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dSolidLineDeviceCommand(startPoint, finishPoint, clrAxis, nMinorThickness));
		}
	}
...
double CXTPChartRadarAxisXView::ValueToAngle(double nValue) const
{
	...
	delta = CXTPMathUtils::NormalizeDegree(90 - pDiagram->GetStartAngle() - delta * (pRadarAxisX->IsDirectionClockwise() ? 1 : -1));
...
{
	CXTPChartDeviceCommand* pAxisCommand = CXTPChartDrawingObjectFactory::Create2dPolygonAntialiasingDeviceCommand(TRUE);
	...
	if (m_pAxis->IsVisible())
	{
		if (nThickness > 0)
		{
			...
			{
				pAxisCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedCircleDeviceCommand(m_ptCenter, m_nRadius, clrAxis, nThickness));
			}
		}
