...
CXTPChartScaleTypeMap* CXTPChartScaleTypeMap::Create(CXTPChartAxis* pAxis)
{
	...
	{
	case xtpChartScaleNumerical:
		pType = new CXTPChartNumericalScaleTypeMap(pAxis);
		break;

	case xtpChartScaleQualitative:
		pType = new CXTPChartQualitativeScaleTypeMap(pAxis);
		break;

	case xtpChartScaleDateTime:
		pType = new CXTPChartDateTimeScaleTypeMap(pAxis);
		break;

	case xtpChartScaleTime:
		pType = new CXTPChartTimeScaleTypeMap(pAxis);
		break;
	}
...
CXTPChartString CXTPChartTimeScaleTypeMap::InternalToValue(CXTPChartNumberFormat* pFormat, double dMark) const
{
...
{
	int nSeconds = (static_cast<int>(m_dTime) - (GetHours() * Hours) - (GetMinutes() * Minutes)) / Seconds;
...
{
	int nMilliseconds = (static_cast<int>(m_dTime) - (GetHours() * Hours) - (GetMinutes() * Minutes) - (GetSeconds() * Seconds));
