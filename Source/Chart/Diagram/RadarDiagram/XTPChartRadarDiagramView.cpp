	...
	const CXTPChartColor& color1, const CXTPChartColor& color2, CXTPChartFillStyle* pFillStyle)
{
	...
	{
		CXTPChartDeviceCommand* pCommands = CXTPChartDrawingObjectFactory::Create2dClipRegionDeviceCommand(arrClipPoints);
...
CXTPChartDeviceCommand* CXTPChartRadarDiagramView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	CXTPChartDeviceCommand* pContainer = pCommand->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dPolygonAntialiasingDeviceCommand(TRUE));
	...
	if (pDiagram->GetStyle() == xtpChartRadarDiagramCircle)
	{
		...
		pContainer->AddChildCommand(CXTPChartDrawingObjectFactory::Create2dBoundedCircleDeviceCommand(m_ptCenter, m_nRadius,
