...
CXTPChartDeviceCommand* CXTPChartTextPainter::CreateDeviceCommand(CXTPChartDeviceContext* pDC, const CXTPChartColor& color)
{
	...
	CXTPChartDeviceCommand* pStateGraphicsCommand = CXTPChartDrawingObjectFactory::Create2dSaveStateDeviceCommand();
	CXTPChartDeviceCommand* pTextAntialiasingDeviceCommand = CXTPChartDrawingObjectFactory::Create2dTextAntialiasingDeviceCommand(m_pTextProvider->GetAntialiasing());
	CXTPChartDeviceCommand* pTextContainer = pStateGraphicsCommand->AddChildCommand(pTextAntialiasingDeviceCommand);

	if (m_pMarkupUIElement)
	{
		CXTPChartDeviceCommand* pMarkupElementDeviceCommand = CXTPChartDrawingObjectFactory::Create2dMarkupElementDeviceCommand(m_pMarkupUIElement, m_pTextProvider->GetFont(), color, rect);
		pTextContainer->AddChildCommand(pMarkupElementDeviceCommand);
	}
	else
	{
		CXTPChartDeviceCommand* pBoundedTextDeviceCommand = CXTPChartDrawingObjectFactory::Create2dBoundedTextDeviceCommand(m_strText, m_pTextProvider->GetFont(), color, rect);
...
void CXTPChartRotatedTextPainterBase::CalculatePoints(XTPChartTextRotation rotation, CRect rect, float fDegree, float points[4][2])
{
	...
	points[0][0] = rect.left + height * fsin + width * (1 - fcos);
	points[0][1] = rect.top + height * (1 - fcos) - width * fsin;
	points[1][0] = rect.right + height * fsin - width * (1 - fcos);
	points[1][1] = rect.top + height * (1 - fcos) + width * fsin;
	points[2][0] = rect.right - height * fsin - width * (1 - fcos);
	points[2][1] = rect.bottom - height * (1 - fcos) + width * fsin;
	points[3][0] = rect.left - height * fsin + width * (1 - fcos);
	points[3][1] = rect.bottom - height * (1 - fcos) - width * fsin;
	...
	switch (rotation)
	{
				...
				case xtpChartTextCenterTop:
					offset[0] = -height * fsin;
					offset[1] = -height * (1 - fcos);
					break;
...
CXTPChartDeviceCommand* CXTPChartRotatedTextPainterBase::CreateDeviceCommand(CXTPChartDeviceContext* pDC, const CXTPChartColor& color)
{
	...
	CXTPChartDeviceCommand* pTranslateDeviceCommand = CXTPChartDrawingObjectFactory::Create2dTranslateDeviceCommand((float)offset.x, (float)offset.y);
	pCommand = pCommand->AddChildCommand(pTranslateDeviceCommand);

	CXTPChartDeviceCommand* pRotateDeviceCommand = CXTPChartDrawingObjectFactory::Create2dRotateDeviceCommand(m_fAngle);
	pCommand = pCommand->AddChildCommand(pRotateDeviceCommand);

	CXTPChartDeviceCommand* pTextAntialiasingDeviceCommand = CXTPChartDrawingObjectFactory::Create2dTextAntialiasingDeviceCommand(m_pTextProvider->GetAntialiasing());
	CXTPChartDeviceCommand* pTextContainer = pCommand->AddChildCommand(pTextAntialiasingDeviceCommand);

	if (m_pMarkupUIElement)
	{
		CXTPChartDeviceCommand* pMarkupElementDeviceCommand = CXTPChartDrawingObjectFactory::Create2dMarkupElementDeviceCommand(m_pMarkupUIElement, m_pTextProvider->GetFont(), color, CXTPChartRectF(rect));
		pTextContainer->AddChildCommand(pMarkupElementDeviceCommand);
	}
	else
	{
		CXTPChartDeviceCommand* pBoundedTextDeviceCommand = CXTPChartDrawingObjectFactory::Create2dBoundedTextDeviceCommand(m_strText, m_pTextProvider->GetFont(), color, CXTPChartRectF(rect));
		pTextContainer->AddChildCommand(pBoundedTextDeviceCommand);
	}
