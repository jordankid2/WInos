...
BEGIN_INTERFACE_MAP(CXTPChartSeries, CXTPChartElement)
	INTERFACE_PART(CXTPChartSeries, XTPDIID_ChartSeries, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPChartSeries, XTPDIID_ChartSeries)
...
void CXTPChartSeries::OleSetStyle(LPDISPATCH lpStyle)
{
	if (lpStyle)
	{
		...
		CXTPChartSeriesStyle* pStyle = DYNAMIC_DOWNCAST(CXTPChartSeriesStyle, 
			CXTPChartSeriesStyle::FromIDispatchSafe(lpStyle));
		if (NULL == pStyle)
		{
...
void CXTPChartSeries::OleSetDiagram(LPDISPATCH lpDiagram)
{
	if (lpDiagram)
	{
		CXTPChartDiagram* pDiagram = DYNAMIC_DOWNCAST(CXTPChartDiagram, 
			CXTPChartDiagram::FromIDispatchSafe(lpDiagram));
		if (NULL == pDiagram)
		{
			AfxThrowOleException(E_INVALIDARG);
		}
...
double CALLBACK OleGetPointValue(CXTPChartSeries* pSeries, double x)
{
