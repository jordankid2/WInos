...
CXTPTaskPanelPaintManager::CXTPTaskPanelPaintManager()
{
	...
	m_hGripperNormal = m_hGripperSpecial = NULL;

	m_rcGroupOuterMargins.SetRect(0, 0, 0, 0);
	m_rcGroupInnerMargins = XTP_DPI(CRect(11, 8, 11, 8));
	m_rcItemOuterMargins.SetRect(0, 0, 0, 0);
	m_rcItemInnerMargins = XTP_DPI(CRect(2, 2, 2, 2));
	m_rcImageLayoutIconPadding = XTP_DPI(CRect(6, 6, 6, 6));
	m_rcItemIconPadding.SetRect(0, 0, 0, 0);
	m_rcGroupIconPadding.SetRect(0, 0, 0, 0);

	m_rcControlMargins = XTP_DPI(CRect(12, 12, 12, 12));
...
{
	if (!pItem)
		return &m_xtpFontIcon;

	if (pItem->m_xtpFontItem.GetSafeHandle())
		return &pItem->m_xtpFontItem;
	...
	return (((CXTPTaskPanelGroupItem*)pItem)->IsItemHot() && (pItem->GetTaskPanel()->GetHotTrackStyle() == xtpTaskPanelHighlightText)) ?
		(bBold ? &m_xtpFontIconHotBold: &m_xtpFontIconHot) : (bBold ? &m_xtpFontIconBold : &m_xtpFontIcon);
}
...
void CXTPTaskPanelPaintManager::DrawScrollButton(CDC* pDC, CXTPTaskPanelScrollButton* pScrollButton)
{
	...
	{
		DrawScrollArrow(pDC, CPoint(pt.x - XTP_DPI_X(5), pt.y - XTP_DPI_Y(2)),
			CPoint(pt.x - XTP_DPI_X(1), pt.y + XTP_DPI_Y(2)), 
			CPoint(pt.x + XTP_DPI_X(3), pt.y - XTP_DPI_Y(2)), 
			pScrollButton->bEnabled);
	}
	else
	{
		DrawScrollArrow(pDC, CPoint(pt.x - XTP_DPI_X(5), pt.y + XTP_DPI_Y(1)),
			CPoint(pt.x - XTP_DPI_X(1), pt.y - XTP_DPI_Y(3)),
			CPoint(pt.x + XTP_DPI_X(3), pt.y + XTP_DPI_Y(1)),
			pScrollButton->bEnabled);
	}
}
...
{
	const int nHalfSide = XTP_DPI_X(14) / 2;
	pt.y -= XTP_DPI_Y(1);
	CRect rc(pt.x - nHalfSide, pt.y - nHalfSide,
		pt.x + nHalfSide, pt.y + nHalfSide);

	if (bExpanded)
	{
		XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolShiftUp, rc, clr);
	}
	else
	{
...
CRect CXTPTaskPanelPaintManager::DrawCaptionGripperBitmap(CDC* pDC, CXTPTaskPanelGroup* pGroup, BOOL bExpanded, BOOL bHot, CRect rc)
{
	...
	int nTop = max(rc.top, rc.CenterPoint().y - XTP_DPI_Y(10));
	CRect rcGripper(rc.right - XTP_DPI_X(3) - XTP_DPI_X(20), nTop, rc.right - XTP_DPI_X(3), nTop + XTP_DPI_Y(20));
	...
	if (m_bUseBitmapGrippers)
	{
		...
		if (XTPDrawHelpers()->m_pfnAlphaBlend)
		{
			...
			XTPDrawHelpers()->m_pfnAlphaBlend(pDC->GetSafeHdc(), rcGripper.left, rcGripper.top, 
				bmpInfo.bmWidth, bmpInfo.bmHeight, dcGripper, 0, 0, bmpInfo.bmWidth, bmpInfo.bmHeight, bf);
		}
	}
	else
	{
		...
		CPoint ptIcon = rcGripper.TopLeft();
		::DrawIconEx(*pDC, ptIcon.x, ptIcon.y, hIcon,
			XTP_DPI_X(19), XTP_DPI_Y(19), 0, NULL, DI_NORMAL);

		XTP_TASKPANEL_GROUPCOLORS* pGroupColors = GetGroupColors(pGroup);

		DrawCaptionGripperSigns(pDC, rcGripper.CenterPoint(), 
			bHot ? pGroupColors->clrHeadTextHot : pGroupColors->clrHeadTextNormal, bExpanded);
	}
...
int CXTPTaskPanelPaintManager::GetCaptionGripperWidth(CXTPTaskPanelGroup* pGroup, CRect rc)
{
	...
	if (rc.Width() < XTP_DPI_X(20) + XTP_DPI_X(6))
		return 0;
...
CRect CXTPTaskPanelPaintManager::DrawCaptionGripper(CDC* pDC, CXTPTaskPanelGroup* pGroup, CRect rc)
{
	...
	if (m_eGripper == xtpTaskPanelGripperPlain)
	{
		...
		CRect rcGripper(rc.right - XTP_DPI_X(6) - XTP_DPI_X(17), rc.CenterPoint().y - XTP_DPI_Y(8), 
			rc.right - XTP_DPI_X(6), rc.CenterPoint().y + XTP_DPI_Y(8));
	...
	if (m_eGripper == xtpTaskPanelGripperTriangle)
	{
		...
		CPoint pt(rc.right - XTP_DPI_X(6) - XTP_DPI_X(8) - XTP_DPI_X(5), rc.CenterPoint().y + XTP_DPI_Y(2));

		const int nHalfSide = XTP_DPI_X(22) / 2;
		CRect rcArrow(pt.x - nHalfSide, pt.y - nHalfSide,
			pt.x + nHalfSide, pt.y + nHalfSide);
		...
		{
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayRight, rcArrow, clr);
		}
...
void CXTPTaskPanelPaintManager::SetItemsFontIndirect(LOGFONT* pLogFont, BOOL bUseStandardItemsFont /*= FALSE*/)
{
	...
	m_xtpFontIcon.DeleteObject();
	m_xtpFontIconBold.DeleteObject();
	m_xtpFontIconHot.DeleteObject();
	m_xtpFontIconHotBold.DeleteObject();


	pLogFont->lfWeight = FW_NORMAL;
	m_xtpFontIcon.CreateFontIndirect(pLogFont);

	pLogFont->lfUnderline = TRUE;
	m_xtpFontIconHot.CreateFontIndirect(pLogFont);

	pLogFont->lfUnderline = FALSE;
	pLogFont->lfWeight = FW_BOLD;
	m_xtpFontIconBold.CreateFontIndirect(pLogFont);

	pLogFont->lfUnderline = TRUE;
	m_xtpFontIconHotBold.CreateFontIndirect(pLogFont);
}
...
void CXTPTaskPanelPaintManager::SetCaptionFontIndirect(LOGFONT* pLogFont, BOOL bUseStandardCaptionFont /*= FALSE*/)
{
	...
	m_xtpFontCaption.CreateFontIndirect(pLogFont);

	CWindowDC dc(CWnd::GetDesktopWindow());
	CXTPFontDC fnt(&dc, &m_xtpFontCaption);
	m_nCaptionHeight = max(XTP_DPI_Y(25), dc.GetTextExtent(_T(" "), 1).cy);
}
...
void CXTPTaskPanelPaintManager::DrawDragArrow(CDC* pDC, CXTPTaskPanelItem* pItem, CRect rc, BOOL bAfter)
{
	...
	else
	{
		if (!pItem->IsGroup() && pItem->GetItemGroup()->GetItemLayout() != xtpTaskItemLayoutImages)
		{
			...
			rc.left = rcControl.left + rcMargin.left + XTP_DPI_X(2);
			rc.right = rcControl.right - rcMargin.right - XTP_DPI_X(2);
		}

		int y = (bAfter ? rc.bottom : rc.top) - XTP_DPI_Y(1);

		COLORREF clr = RGB(0 /*xFF*/, 0, 0);

		pDC->FillSolidRect(rc.left, y, rc.Width(), XTP_DPI_Y(1), clr);

		int nArrowLength = XTP_DPI_X(3);
		int nArrowHeight = XTP_DPI_Y(1);
		for(int i = nArrowLength; i >= 0; --i, --y, nArrowHeight += 2)	//DPI scaling don't need in this cycle
		{
			pDC->FillSolidRect(rc.left  + i,     y, 1, nArrowHeight, clr);		//left  arrow
			pDC->FillSolidRect(rc.right - i - 1, y, 1, nArrowHeight, clr);		//right arrow
		}
	}
}
...
CRect CXTPTaskPanelPaintManager::DrawGroupItemImageWithText(CDC* pDC, CXTPTaskPanelGroupItem* pItem, CRect rc, BOOL bDraw)
{
	...
	if (bDraw)
	{
		...
		if (bDrawIcon)
		{
			if (hotTrackStyle == xtpTaskPanelHighlightImage)
			{
				CRect rcImage(rc.left - rcInnerMargins.left, rc.top - rcInnerMargins.top,
					rc.left + szIcon.cx + XTP_DPI_X(2) + rcIconPadding.left + rcIconPadding.right,
					rc.top + szIcon.cy + XTP_DPI_Y(2) + rcIconPadding.top + rcIconPadding.bottom);
		...
		{
			rcText.left += szIcon.cx + XTP_DPI_X(7) + rcIconPadding.left + rcIconPadding.right;
		}

		if (rcText.Width() > 0)
		{
			DrawItemCaption(pDC, pItem, rcText, static_cast<UINT>(m_nItemTextFormat) | DT_LEFT);
		}
	...
	else
	{
		...
		if (rcText.Width() > 0)
		{
			CSize szText = pItem->GetCaptionTextSize(pDC, rcText, (static_cast<UINT>(m_nItemTextFormat) | DT_LEFT) & ~(DT_VCENTER | DT_CENTER));
...
CRect CXTPTaskPanelPaintManager::DrawGroupItemImageWithTextBelow(CDC* pDC, CXTPTaskPanelGroupItem* pItem, CRect rc, BOOL bDraw)
{
	...
	if (bDraw)
	{
		...
		if (pItem->GetType() == xtpTaskItemTypeLink)
		{
			if (bDrawIcon)
			{
				...
				if (hotTrackStyle == xtpTaskPanelHighlightImage)
				{
					CRect rcImage(ptIcon.x - XTP_DPI_X(2), rc.top - XTP_DPI_Y(2), 
						ptIcon.x + szIcon.cx + XTP_DPI_X(2), rc.top + szIcon.cy + XTP_DPI_Y(2));
					DrawGroupItemFrame(pDC, pItem, rcImage);
				}
			...
			rcText.top += szIcon.cy + XTP_DPI_X(3);
		}

		if (rcText.Width() > 0)
		{
			DrawItemCaption(pDC, pItem, rcText, static_cast<UINT>(m_nItemTextFormat) | DT_CENTER);
		}
	...
	else
	{
		...
		if (rcText.Width() > 0)
		{
			CSize szText = pItem->GetCaptionTextSize(pDC, rcText, static_cast<UINT>(m_nItemTextFormat) | DT_LEFT);
			rcText.bottom = rcText.top + szText.cy;
			rcText.right = rcText.left + szText.cx;
		}
		int nHeight = rcText.Height() + (bDrawIcon ? (szIcon.cy + XTP_DPI_Y(3)) : 0);
		int nWidth = max(bDrawIcon ? (szIcon.cx + XTP_DPI_X(4)) : 0, rcText.Width()) + rcInnerMargins.right + +rcInnerMargins.left;
...
void CXTPTaskPanelPaintManager::DrawItemCaption(CDC* pDC, CXTPTaskPanelItem* pItem, CRect rcText, UINT nFormat)
{
	if (!pItem->IsGroup())
	{
		...
		if (m_bEmbossedDisabledText && !pItem->GetEnabled())
		{
			...
			pItem->DrawCaptionText(pDC, rcText + XTP_DPI(CPoint(1, 1)), nFormat);
		}
...
int CXTPTaskPanelPaintManager::DrawGroupCaption(CDC* pDC, CXTPTaskPanelGroup* pGroup, BOOL bDraw)
{
	if (bDraw)
	{
		...
		if (pGroup->IsCaptionVisible())
		{
			...
			{
				CRect rcFocus(rc);
				rcFocus.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2));
			...
			if (rcText.Width() > szIcon.cx && pImage)
			{
				...
				rcText.left += szIcon.cx - XTP_DPI_X(12) + rcIconPadding.right + rcIconPadding.left;
			}

			rcText.left += XTP_DPI_X(13);
			rcText.bottom += XTP_DPI_Y(2);
			...
			DrawItemCaption(pDC, pGroup, rcText, static_cast<UINT>(m_nCaptionTextFormat));
