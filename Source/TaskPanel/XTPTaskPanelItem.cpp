...
void CXTPTaskPanelItem::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->GetSchema() > _XTP_SCHEMA_95)
	{
		...
		PX_DPI_Rect(pPX, _T("IconPadding"), m_rcIconPadding, CXTPTaskPanelPaintManager::rectDefault);
	}
...
void CXTPTaskPanelItem::SetItemFont(LOGFONT* pLogFont)
{
	...
	if (pLogFont) m_xtpFontItem.CreateFontIndirect(pLogFont);
}
...
CXTPTaskPanelItem* AFX_CDECL CXTPTaskPanelItem::FromDispatch(LPDISPATCH pDisp)
{
	...
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPTaskPanelItem, CXTPTaskPanelItem::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
