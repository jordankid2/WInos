...
void CXTPTaskPanelGroup::OnCalcClientHeight(CDC* pDC, CRect rc)
{
	...
	//atsevtkov
	//Need to set right value for the left member and for the right member of m_rcGroupTarget and m_rcGroupCurrent,
	//like how in CXTPTaskPanelGroup::OnReposition
	m_rcGroupTarget = rc;
...
CRect CXTPTaskPanelGroup::OnReposition(CRect rc, BOOL bRecalcOnly)
{
	...
	if (!m_pPanel->IsExplorerBehaviour() && (m_pPanel->GetActiveGroup() == this || IsExpanding()))
	{
		...
		int nButtonHeight = max(XTP_DPI_Y(32), GetItemIconSize().cy + rcMarginsInner.bottom);

		int nExpandedClientHeight = max(nButtonHeight + rcMarginsInner.top + XTP_DPI_X(2), nInsideHeight);
...
{
	int nGroupSpacing = GetPaintManager()->GetGroupSpacing(m_pPanel);

	int nCaptionRight = m_rcGroupTarget.right;

	if (m_pPanel->GetScrollButton(TRUE)->pGroupCaption == this)
	{
		nCaptionRight = m_pPanel->GetScrollButton(TRUE)->rcButton.left - nGroupSpacing - 1;
	}

	if (m_pPanel->GetScrollButton(FALSE)->pGroupCaption == this)
	{
		nCaptionRight = m_pPanel->GetScrollButton(FALSE)->rcButton.left - nGroupSpacing - 1;
	}
...
CRect CXTPTaskPanelGroup::GetInvalidateRect() const
{
	if (!m_pPanel)
		return CRect(0, 0, 0, 0);

	CRect rc = m_rcGroupCurrent;
	rc.OffsetRect(0, -m_pPanel->GetScrollOffset());
...
void CXTPTaskPanelGroup::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_DPI_Rect(pPX, _T("MarginsInner"), m_rcMarginsInner, CXTPTaskPanelPaintManager::rectDefault);
	PX_DPI_Rect(pPX, _T("MarginsOuter"), m_rcMarginsOuter, CXTPTaskPanelPaintManager::rectDefault);
	PX_DPI_Size(pPX, _T("IconSize"), m_szItemIcon, CSize(0));
...
BEGIN_INTERFACE_MAP(CXTPTaskPanelGroup, CXTPTaskPanelItem)
	INTERFACE_PART(CXTPTaskPanelGroup, XTPDIID_ITaskPanelGroup, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPTaskPanelGroup, XTPDIID_ITaskPanelGroup)

BEGIN_DISPATCH_MAP(CXTPTaskPanelGroup, CXTPTaskPanelItem)
	DISP_FUNCTION_ID(CXTPTaskPanelGroup, "Items", dispidItems, OleGetItems, VT_DISPATCH, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroup, "Special", dispidSpecial, IsSpecialGroup, SetSpecialGroup, VT_BOOL)

	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroup, "CaptionVisible", dispidCaptionVisible, IsCaptionVisible, ShowCaption, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroup, "Expandable", dispidExpandable, IsExpandable, SetExpandable, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroup, "Expanded", dispidExpanded, IsExpanded, SetExpanded, VT_BOOL)
	DISP_PROPERTY_NOTIFY_ID(CXTPTaskPanelGroup, "ItemLayout", 15, m_itemLayout, RepositionPanel, VT_I4)
	DISP_FUNCTION_ID(CXTPTaskPanelGroup, "SetIconSize", 104, OleSetIconSize, VT_EMPTY, VTS_I4 VTS_I4)
	DISP_PROPERTY_ID(CXTPTaskPanelGroup, "MinimumClientHeight", 106, m_nMinClientHeight, VT_I4)

	DISP_FUNCTION_ID(CXTPTaskPanelGroup, "SetBackgroundImage", 108, OleSetBackgroundImage, VT_EMPTY, VTS_I4 VTS_I4)

END_DISPATCH_MAP()

void CXTPTaskPanelGroup::OleSetBackgroundImage(int ImageIndex, int Alignment)
{
	m_nBackgroundImage = ImageIndex;
	m_nBackgroundAlignemnt = Alignment;
}

LPDISPATCH CXTPTaskPanelGroup::OleGetItems()
{
	return m_pItems->GetIDispatch(TRUE);
}
void CXTPTaskPanelGroup::OleSetIconSize(long cx, long cy)
{
