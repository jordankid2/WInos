...
void CXTPTaskPanelVisualStudio2012Theme::RefreshMetrics()
{
	...
	m_rcItemInnerMargins.left = XTP_DPI_X(23);
	m_rcItemInnerMargins.right = XTP_DPI_X(2);
	m_rcItemInnerMargins.top = m_rcItemInnerMargins.bottom = XTP_DPI_Y(2);
...
CXTPPaintManagerColor CXTPTaskPanelVisualStudio2012Theme::GetTriangleColorAndState(CXTPTaskPanelGroup* pGroup, BOOL &bFilled)
{
	...
	if ((pGroup->GetTaskPanel()->GetSafeHwnd() == GetFocus()) && pGroup->IsItemFocused())
...
{
	static int nExpandButtonHalfWidth = XTP_DPI_X(10);
	static int nExpandButtonHalfHeight = XTP_DPI_Y(10);
	...
	if (pGroup->IsExpanded())
	{
		POINT ptLTGlyph = { XTP_DPI_X(8), XTP_DPI_Y(13) };
		SIZE szGlyph = { XTP_DPI_X(6), XTP_DPI_Y(6) };
	...
	{
		POINT ptLTGlyph = { XTP_DPI_X(9), XTP_DPI_Y(5) };
		SIZE szGlyph = { XTP_DPI_X(5), XTP_DPI_Y(10) };
...
void CXTPTaskPanelVisualStudio2012Theme::DrawGroupItemFrame(CDC* pDC, CXTPTaskPanelGroupItem* pItem, CRect rc)
{
	...
	BOOL isPanel = TRUE;
	if (pItem->IsDroppedOff())
		isPanel = FALSE;
	else if (pItem->GetTaskPanel()->GetSafeHwnd() != GetFocus())
		isPanel = FALSE;
	if ((!isPanel) && pItem->IsItemSelected())
	{
