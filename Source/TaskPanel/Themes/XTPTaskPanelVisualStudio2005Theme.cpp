// XTPTaskPanelVisualStudio2005Theme.cpp : implementation of the CXTPTaskPanelVisualStudio2005Theme class.
//
...
CRect  CXTPTaskPanelVisualStudio2005Theme::DrawExpandButton(CDC* pDC, CXTPTaskPanelGroup* pGroup, CRect rc)
{
	...
	pDC->FillSolidRect(rcButton.left + n1pxX, rcButton.top + n1pxY, 
		n9pxX - n1pxX*2, n9pxY - n1pxY*2, GetXtremeColor(COLOR_3DFACE));

//borders
	pDC->FillSolidRect(rcButton.left + n1pxX, rcButton.top, 
		n9pxX - n1pxX*2, n1pxY, clrFrame);
	pDC->FillSolidRect(rcButton.left + n1pxX, rcButton.bottom - n1pxY, 
		n9pxX - n1pxX*2, n1pxY, clrFrame);
	pDC->FillSolidRect(rcButton.left, rcButton.top + n1pxY, 
		n1pxX, n9pxY - n1pxY*2, clrFrame);
	pDC->FillSolidRect(rcButton.right - n1pxX, rcButton.top + n1pxY, 
		n1pxX, n9pxY - n1pxY*2, clrFrame);

//white blink
	pDC->FillSolidRect(rcButton.left + n1pxX, rcButton.top + n1pxY, 
		n9pxX - n1pxX*2, XTP_DPI_Y(3), 0xFFFFFF);
	pDC->FillSolidRect(rcButton.left + n1pxX, rcButton.top + XTP_DPI_Y(4), 
		n9pxX - XTP_DPI_X(4), n1pxY*2, 0xFFFFFF);

//plus/minus
	pDC->FillSolidRect(rcButton.left + n1pxX*2, rcButton.top + XTP_DPI_Y(4), 
		n9pxX - n1pxX*4, n1pxY, 0);
	if (!pGroup->IsExpanded())
	{
		pDC->FillSolidRect(rcButton.left + XTP_DPI_X(4), rcButton.top + n1pxY*2, 
			n1pxX, n9pxY - n1pxY*4, 0);
	}
...
CRect CXTPTaskPanelVisualStudio2005Theme::DrawGroupCaptionBackground(CDC* pDC, CXTPTaskPanelGroup* pGroup, CRect rc)
{
	XTP_TASKPANEL_GROUPCOLORS* pGroupColors = GetGroupColors(pGroup);
	COLORREF clrFace = pGroupColors->clrHead.clrLight;
	...
	{
		XTPDrawHelpers()->GradientFill(pDC, CRect(rc.left, rc.top, 
			rc.right, rc.bottom - XTP_DPI_Y(2)), pGroupColors->clrHead, FALSE);
...
void CXTPTaskPanelVisualStudio2005Theme::FillGroupClientFace(CDC* pDC, CXTPTaskPanelGroup* pGroup, CRect rc)
{
	XTPDrawHelpers()->GradientFill(pDC, rc, GetGroupColors(pGroup)->clrClient, TRUE);
}

void CXTPTaskPanelVisualStudio2005Theme::DrawGroupClientFace(CDC* pDC, CXTPTaskPanelGroup* pGroup, CRect rc)
{
