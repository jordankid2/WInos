...
void CXTPTaskPanelGroupItem::SetItemSelected(BOOL bSelected)
{
	if (m_pPanel && m_pPanel->IsSingleSelection())
	{
		if (bSelected) m_pPanel->SetFocusedItem(this, FALSE, FALSE);
		return;
	}
...
void CXTPTaskPanelGroupItem::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_DPI_Rect(pPX, _T("Margins"), m_rcMargins, CXTPTaskPanelPaintManager::rectDefault);
...
BEGIN_INTERFACE_MAP(CXTPTaskPanelGroupItem, CXTPTaskPanelItem)
	INTERFACE_PART(CXTPTaskPanelGroupItem, XTPDIID_ITaskPanelGroupItem, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPTaskPanelGroupItem, XTPDIID_ITaskPanelGroupItem)

BEGIN_DISPATCH_MAP(CXTPTaskPanelGroupItem, CXTPTaskPanelItem)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroupItem, "Bold", dispidBold, IsBold, SetBold, VT_BOOL)

	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "SetMargins", dispidSetMargins, OleSetMargins, VT_EMPTY, VTS_I4 VTS_I4 VTS_I4 VTS_I4)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroupItem, "Control", dispidControl, OleGetControl, OleSetControl, VT_DISPATCH)
	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "BackColor", DISPID_BACKCOLOR, GetBackColor, VT_I4, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroupItem, "Selected", 13, IsItemSelected, SetItemSelected, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroupItem, "Handle", 14, OleGetHandle, OleSetHandle, VT_HANDLE)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroupItem, "DragText", 50, OlegGetDragText, SetDragText, VT_BSTR)
	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "SetSize", 104, OleSetSize, VT_EMPTY, VTS_I4 VTS_I4 VTS_BOOL)
	DISP_PROPERTY_NOTIFY_ID(CXTPTaskPanelGroupItem, "TextColor", DISPID_FORECOLOR, m_clrText, OleRedrawPanel, VT_COLOR)
	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "SetHandle", 51, OleSetHandle, VT_EMPTY, VTS_HANDLE)
	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "GetRect", 53, OleGetRect, VT_EMPTY, VTS_PI4 VTS_PI4 VTS_PI4 VTS_PI4)
END_DISPATCH_MAP()


void CXTPTaskPanelGroupItem::OleRedrawPanel()
{
	RedrawPanel();
}

void CXTPTaskPanelGroupItem::OleGetRect(long* nLeft, long* nTop, long* nRight, long* nBottom)
{
	CRect rc = GetHitTestRect();
	if (nLeft) *nLeft = rc.left;
	if (nTop) *nTop = rc.top;
	if (nRight) *nRight = rc.right;
	if (nBottom) *nBottom = rc.bottom;
};

void CXTPTaskPanelGroupItem::OleSetMargins(long nLeft, long nTop, long nRight, long nBottom)
{
...
void CXTPTaskPanelGroupItem::OleSetSize(long cx, long cy, BOOL bAutHeight)
{
	SetSize(CSize(cx, cy), bAutHeight);
}

HWND CXTPTaskPanelGroupItem::OleGetHandle()
{
	return m_hWnd;
}


void CXTPTaskPanelGroupItem::OleSetHandle(HWND hWnd)
{
	if (GetParent(hWnd) != GetParent(m_pPanel->GetSafeHwnd()))
	{
		::SetParent(hWnd, m_pPanel->GetSafeHwnd());
	}
	CWnd::ModifyStyle(hWnd, WS_POPUP, WS_CHILD, 0);

	SetControlHandle(hWnd);

	RepositionPanel();
}

BSTR CXTPTaskPanelGroupItem::OlegGetDragText()
{
	return GetDragText().AllocSysString();
}

void CXTPTaskPanelGroupItem::OleSetControl(LPDISPATCH lpDispatch)
{
	if (m_typeItem == xtpTaskItemTypeControl)
	{
		...
		TRY
		{
			COleDispatchDriver(lpDispatch, FALSE).GetProperty(DISPID_HWND, VT_I4, &hWnd);
			if (hWnd == 0)
				return;
