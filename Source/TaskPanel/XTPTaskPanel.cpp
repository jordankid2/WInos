...
CXTPTaskPanel::CXTPTaskPanel()
{
	...
	m_szItemIcon = XTP_DPI(CSize(16, 16));
	m_szGroupIcon = XTP_DPI(CSize(32, 32));
...
void CXTPTaskPanel::SetFocusedItem(CXTPTaskPanelItem* pItem, BOOL bDrawFocusRect, BOOL bSetFocus)
{
	...
	if (bSetFocus) GrabFocus();

	if (m_pItemFocused == pItem)
		return;

	if (m_hWnd) EnsureVisible(pItem);
...
void CXTPTaskPanel::SetTheme(XTPTaskPanelPaintTheme theme)
{
	...
	case xtpTaskPanelThemeNativeWinXP:
		pPaintManager = new CXTPTaskPanelExplorerTheme();
		break;
	...
	case xtpTaskPanelThemeListView:
		pPaintManager = new CXTPTaskPanelListViewTheme();
		break;
	...
	default:
		ASSERT(!"Unknow ToolBox theme selected");
		break;
	}
...
void CXTPTaskPanel::OnNcPaint()
{
	...
	BOOL hasBorder = GetStyle() & WS_BORDER;
	BOOL hasClientEdge = GetExStyle() & WS_EX_CLIENTEDGE;

	m_borderEdgeInt.DeleteObject();
	m_borderEdgeExt.DeleteObject();
	m_border.DeleteObject();

	if(hasBorder && hasClientEdge)
	{
		m_borderEdgeInt.CreatePen(PS_SOLID, 0, dc.GetPixel(rc.Width() / 2, rc.bottom));
		m_borderEdgeExt.CreatePen(PS_SOLID, 0, dc.GetPixel(rc.Width() / 2, rc.bottom + 1));
		m_border.CreatePen(PS_SOLID, 0, dc.GetPixel(rc.Width() / 2, rc.bottom + 2));
	}
	else
	{
		if(hasBorder)
		{
			m_border.CreatePen(PS_SOLID, 0, dc.GetPixel(rc.Width() / 2, rc.bottom));
		}
...
void CXTPTaskPanel::DrawBorders(CPaintDC &paintDC)
{
	...
	GetClientRect(&rc);
	BOOL hasBorder = GetStyle() & WS_BORDER;
	BOOL hasVScroll = GetStyle() & WS_VSCROLL;
	BOOL hasClientEdge = GetExStyle() & WS_EX_CLIENTEDGE;
	...
	if(hasBorder && hasClientEdge)
	{
		CPen* oldPen = paintDC.SelectObject(&m_border);
		if(oldPen)
		{
			paintDC.MoveTo(rc.right - 1, rc.top);
			paintDC.LineTo(rc.right - 1, rc.bottom);

			paintDC.SelectObject(m_borderEdgeExt);
			paintDC.MoveTo(rc.right - 2, rc.top);
			paintDC.LineTo(rc.right - 2, rc.bottom);

			paintDC.SelectObject(m_borderEdgeInt);
			paintDC.MoveTo(rc.right - 3, rc.top);
			paintDC.LineTo(rc.right - 3, rc.bottom);
	...
	else
	{
		...
		if(hasClientEdge)
		{
			CPen* oldPen = paintDC.SelectObject(&m_borderEdgeInt);
			if(oldPen)
			{
				paintDC.MoveTo(rc.right - 2, rc.top);
				paintDC.LineTo(rc.right - 2, rc.bottom);
...
INT_PTR CXTPTaskPanel::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	if (pItem)
	{
		...
		CXTPToolTipContext::FillInToolInfo(pTI, m_hWnd, pItem->GetHitTestRect(),
...
BOOL CXTPTaskPanel::PreTranslateMessage(MSG* pMsg)
{
	if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_TAB && m_bTabItems)
	{
		...
		if (hWnd && hWnd != m_hWnd && ::IsChild(m_hWnd, hWnd))
		{
			...
			if (bShiftPressed) hWndFirst = ::GetNextDlgTabItem(hWndDlg, hWndFirst, TRUE);
			...
			if (pItemFocused)
			{
				...
				if (pItem)
				{
					...
					if (pItem->GetType() == xtpTaskItemTypeControl && bShiftPressed && pItemFocused != pItem)
					{
						hWndDlg = ((CXTPTaskPanelGroupItem*)m_pItemFocused)->GetControlHandle();
						if (hWndDlg) hWndFirst = ::GetNextDlgTabItem(hWndDlg, NULL, FALSE);
						if (hWndFirst) hWndFirst = ::GetNextDlgTabItem(hWndDlg, hWndFirst, TRUE);
						if (hWndFirst) ::SetFocus(hWndFirst);
					}
				}
