...
void AFX_CDECL XTPDeflateRectDirection(CRect& rc, int left, int top, int right, int bottom, BOOL bVertical)
{
	if (bVertical)
	{
...
void AFX_CDECL XTPFillFramePartRect(CDC* pDC, CRect rc, CWnd* pWnd, CWnd* pParentFrame, const CXTPPaintManagerColorGradient& clrHeaderFace, BOOL bHoriz)
{
	ASSERT(NULL != pWnd);
...
CXTPTabPaintManagerColorSet* CXTPDockingPaneTabPaintManager::SetColor(XTPTabColorStyle tabColor)
{
	...
	case xtpTabColorOffice2013:
		return SetColorSet(new CXTPDockingPaneTabColorSet2013());
	...
	case xtpTabColorVisualStudio2015:
		return SetColorSet(new CXTPDockingPaneTabColorSet2015());
	}

	return CXTPTabPaintManager::SetColor(tabColor);
}

CXTPTabPaintManagerTheme* CXTPDockingPaneTabPaintManager::SetAppearance(XTPTabAppearanceStyle tabAppearance)
{
	...
	case xtpTabAppearanceOffice2013:
		return SetAppearanceSet(new CXTPDockingPaneTabTheme2013());
...
CXTPTabPaintManagerTheme* CXTPDockingPanePanelPaintManager::SetAppearance(XTPTabAppearanceStyle tabAppearance)
{
	m_tabAppearance = tabAppearance;
...
void CXTPDockingPanePaintManager::DrawSplitter(CDC* pDC, CXTPDockingPaneSplitterWnd* pSplitter)
{
	...
	if (m_dwSplitterStyle & xtpPaneSplitter3D)
	{
		...
		if (pSplitter->IsHorizontal())
		{
			...
			pDC->FillSolidRect(0, rc.bottom - XTP_DPI_Y(2), rc.Width(), XTP_DPI_Y(1), GetXtremeColor(XPCOLOR_3DSHADOW));
			pDC->FillSolidRect(0, rc.bottom - XTP_DPI_Y(1), rc.Width(), XTP_DPI_Y(1), GetXtremeColor(COLOR_BTNTEXT));
		}
		else
		{
			pDC->FillSolidRect(0, 0, 1, rc.Height(), GetXtremeColor(COLOR_3DHIGHLIGHT));
			pDC->FillSolidRect(rc.right - XTP_DPI_X(2), 0, XTP_DPI_X(1), rc.Height(), GetXtremeColor(XPCOLOR_3DSHADOW));
			pDC->FillSolidRect(rc.right - XTP_DPI_X(1), 0, XTP_DPI_X(1), rc.Height(), GetXtremeColor(COLOR_BTNTEXT));
		}
	}
	else if (m_dwSplitterStyle & xtpPaneSplitterSoft3D)
	{
		...
		if (pSplitter->IsHorizontal())
		{
			...
			pDC->FillSolidRect(0, rc.bottom - XTP_DPI_Y(1), rc.Width(), XTP_DPI_Y(1), GetXtremeColor(COLOR_3DDKSHADOW));

			pDC->FillSolidRect(0, XTP_DPI_Y(1), rc.Width(), nCenter - XTP_DPI_Y(1), GetXtremeColor(COLOR_WINDOW));
			pDC->FillSolidRect(0, nCenter, rc.Width(), rc.Height() - nCenter - XTP_DPI_Y(1), m_clrSplitter);
		}
		else
		{
			pDC->FillSolidRect(0, 0, XTP_DPI_X(1), rc.Height(), GetXtremeColor(COLOR_3DDKSHADOW));
			pDC->FillSolidRect(rc.right - XTP_DPI_X(1), 0, XTP_DPI_X(1), rc.Height(), GetXtremeColor(COLOR_3DDKSHADOW));

			pDC->FillSolidRect(XTP_DPI_X(1), 0, nCenter - XTP_DPI_X(1), rc.Height(), GetXtremeColor(COLOR_WINDOW));
			pDC->FillSolidRect(nCenter, 0, rc.Width() - nCenter - XTP_DPI_X(1), rc.Height(), m_clrSplitter);
		}
	}
	...
	if (m_dwSplitterStyle & xtpPaneSplitterGripperOfficeXP)
	{
		if (pSplitter->IsHorizontal())
		{
			pDC->FillSolidRect(rc.CenterPoint().x, 0 + XTP_DPI_Y(2), XTP_DPI_X(11), XTP_DPI_Y(1), m_clrSplitterGripper);
			pDC->FillSolidRect(rc.CenterPoint().x, 0 + XTP_DPI_Y(4), XTP_DPI_X(11), XTP_DPI_Y(1), m_clrSplitterGripper);
		}
		else
		{
			pDC->FillSolidRect(XTP_DPI_X(2), rc.CenterPoint().y, XTP_DPI_X(1), XTP_DPI_Y(11), m_clrSplitterGripper);
			pDC->FillSolidRect(XTP_DPI_X(4), rc.CenterPoint().y, XTP_DPI_X(1), XTP_DPI_Y(11), m_clrSplitterGripper);
		}
	}
	if (m_dwSplitterStyle & xtpPaneSplitterGripperOffice2003)
	{
		...
		else
		{
			...
			{
				pDC->FillSolidRect(CRect(x + XTP_DPI_X(1), y + XTP_DPI_Y(1), x + XTP_DPI_X(3), y + XTP_DPI_Y(3)), GetXtremeColor(COLOR_BTNHIGHLIGHT));
				pDC->FillSolidRect(CRect(x, y, x + XTP_DPI_X(2), y + XTP_DPI_Y(2)), m_clrSplitterGripper);
			}
		}
	}
}
...
void CXTPDockingPanePaintManager::DrawCaptionButtonEntry(CDC* pDC, CXTPDockingPaneCaptionButton* pButton, COLORREF clrButton, CSize szOffset)
{
	CRect rc = pButton->GetRect();
	...
	case XTP_IDS_DOCKINGPANE_AUTOHIDE:
		bPinned ? rc.DeflateRect(0, XTP_DPI_Y(1)) : rc.DeflateRect(XTP_DPI_X(-1), XTP_DPI_Y(1), 0, 0);
		CXTPDockingPaneCaptionButton::DrawPinnButton(pDC, rc, bPinned, clrButton);
		break;
	...
	case XTP_IDS_DOCKINGPANE_MENU:
		...
		break;
	default:
		ASSERT(!"Unexpected button identifier");
		break;
	}
}
...
void CXTPDockingPanePaintManager::ClipAndAddEllipsisIfNeeded(CDC* pDC, CString& strTitle, int nAvailableWidth) const
{
	CSize szText = pDC->GetTextExtent(strTitle);
	...
	int nRestSymbols = int(nAvailableWidth / fAverageSymbolWidth);
...
{
	return AxCreateOleFont(&m_xtpFontTitle, this, (LPFNFONTCHANGED)&CXTPDockingPanePaintManager::OleSetFont);
}
