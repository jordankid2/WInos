...
int CXTPDockingPane::GetIconID() const
{
	return m_nIconID == -1 ? m_nID : m_nIconID;
}

void CXTPDockingPane::SetIconID(UINT nID)
{
	m_nIconID = nID;
}

CWnd* CXTPDockingPane::GetChild() const
{
	return m_hwndChild ? CWnd ::FromHandle(m_hwndChild): NULL;
}

CSize CXTPDockingPane::SetMinTrackSize(CSize sz)
{
...
CSize CXTPDockingPane::SetMaxTrackSize(CSize sz)
{
...
{
	if (XTP_SAFE_GET1(m_pParentContainer, GetType(), 0) != xtpPaneTypeTabbedContainer)
		return FALSE;

	if (IsHidden())
		return FALSE;

	return (XTP_SAFE_GET2(m_pParentContainer, GetDockingSite(),
		IsKindOf(RUNTIME_CLASS(CXTPDockingPaneMiniWnd)), FALSE));
}

BOOL CXTPDockingPane::IsSidePanel() const
{
	return (XTP_SAFE_GET2(m_pParentContainer, GetDockingSite(),
		IsKindOf(RUNTIME_CLASS(CXTPDockingPaneSidePanel)), FALSE));
}

BOOL CXTPDockingPane::IsTabsVisible() const
{
	if (XTP_SAFE_GET1(m_pParentContainer, GetType(), 0) != xtpPaneTypeTabbedContainer)
		return FALSE;

	return (XTP_SAFE_GET1(((CXTPDockingPaneTabbedContainer*)m_pParentContainer),
		IsTabsVisible(), FALSE));
}
...
IMPLEMENT_DYNAMIC(CXTPDockingPane, CXTPCmdTarget)

BEGIN_DISPATCH_MAP(CXTPDockingPane, CXTPCmdTarget)
...
IMPLEMENT_OLETYPELIB_EX(CXTPDockingPane, XTPDIID_IPane)

CXTPDockingPane* AFX_CDECL CXTPDockingPane::FromDispatch(LPDISPATCH pDisp)
{
	CXTPDockingPane* pInst = NULL;
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPDockingPane, CXTPDockingPane::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneTrackSize, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPDockingPaneTrackSize, CXTPCmdTarget)
	INTERFACE_PART(CXTPDockingPaneTrackSize, XTPDIID_IDockingPaneTrackSize, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPDockingPaneTrackSize, XTPDIID_IDockingPaneTrackSize)
...
BEGIN_INTERFACE_MAP(CXTPDockingPane, CXTPCmdTarget)
	INTERFACE_PART(CXTPDockingPane, IID_IAccessible, ExternalAccessible)
#ifdef _XTP_ACTIVEX
	INTERFACE_PART(CXTPDockingPane, XTPDIID_IPane, Dispatch)
	INTERFACE_PART(CXTPDockingPane, XTPDIID_IPaneContainer, Dispatch)
#endif
END_INTERFACE_MAP()
