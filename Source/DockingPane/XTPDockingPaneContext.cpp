...
{
	int nXOffset = (pt.x < rect.left) ? (pt.x - rect.left - XTP_DPI_X(6)) :
		(pt.x > rect.right) ? (pt.x - rect.right + XTP_DPI_X(6)) : 0;
	int nYOffset = (pt.y < rect.top) ? (pt.y - rect.top - XTP_DPI_Y(6)) :
...
void CXTPDockingPaneContext::_CreateRgn(CRgn& rgnResult, CRect rc, BOOL bTabbedRgn, BOOL bRemove)
{
	...
	if (bTabbedRgn)
	{
		CSize szTab(min(XTP_DPI_X(50), rc.Width() - XTP_DPI_X(5)), min(XTP_DPI_Y(20), rc.Height() / 2));
...
void CXTPDockingPaneContext::DrawFocusRect(BOOL bRemoveRect)
{
	...
	if (m_bUseAlphaContext)
	{
		...
		if (m_bAttach)
		{
			CSize szTab(min(XTP_DPI_X(50), rect.Width() - XTP_DPI_X(5)), min(XTP_DPI_Y(20), rect.Height() / 2));
			CRect rcTab;
				...
				&& m_pContainer && m_pContainer->GetType() == xtpPaneTypeTabbedContainer)
			{
				...
				rect.top += szTab.cy;
				rcTab = CRect(rect.left + XTP_DPI_X(5), rect.top - szTab.cy, rect.left + XTP_DPI_X(5) + szTab.cx, rect.top);
			}
			else
			{
				rect.bottom -= szTab.cy;
				rcTab = CRect(rect.left + XTP_DPI_X(5), rect.bottom, rect.left + XTP_DPI_X(5) + szTab.cx, rect.bottom + szTab.cy);
			}
...
BOOL CXTPDockingPaneContext::CanDock(CRect rc, CPoint pt, CXTPDockingPaneBase* pPane, BOOL bInside)
{
	...
	if (!bInside && !CRect(rc.left - XTP_DPI_X(20), rc.top - XTP_DPI_Y(20), 
		rc.right + XTP_DPI_X(20), rc.bottom + XTP_DPI_Y(20)).PtInRect(pt))
		return FALSE;
...
void CXTPDockingPaneContext::UpdateDockingStickers()
{
	...
	if (m_pLastStickerPane != m_pStickerPane || m_lstStickers.IsEmpty())
	{
		...
		{
			CreateNewSticker(CRect(CPoint(rcWindow.CenterPoint().x - GetStickerSize(xtpPaneStickerTop).cx / 2,
				rcWindow.top + XTP_DPI_Y(16)), GetStickerSize(xtpPaneStickerTop)), xtpPaneStickerTop);
		}
		if (IsAllowDockingTo(pTopPane, xtpPaneDockLeft))
		{
			CreateNewSticker(CRect(CPoint(rcWindow.left + XTP_DPI_X(16), rcWindow.CenterPoint().y - GetStickerSize(xtpPaneStickerLeft).cy / 2),
				GetStickerSize(xtpPaneStickerLeft)), xtpPaneStickerLeft);
		}
		if (IsAllowDockingTo(pTopPane, xtpPaneDockBottom))
		{
			CreateNewSticker(CRect(CPoint(rcWindow.CenterPoint().x - GetStickerSize(xtpPaneStickerBottom).cx / 2,
				rcWindow.bottom - XTP_DPI_Y(16) - GetStickerSize(xtpPaneStickerBottom).cy),
				GetStickerSize(xtpPaneStickerBottom)), xtpPaneStickerBottom);
		}
		if (IsAllowDockingTo(pTopPane, xtpPaneDockRight))
		{
			CreateNewSticker(CRect(CPoint(rcWindow.right - GetStickerSize(xtpPaneStickerRight).cx - XTP_DPI_X(16), 
				rcWindow.CenterPoint().y - GetStickerSize(xtpPaneStickerRight).cy / 2), 
				GetStickerSize(xtpPaneStickerRight)), xtpPaneStickerRight);
		}
...
void CXTPDockingPaneContext::AcquireDesktopDC()
{
	...
	UINT nFlags = (pWnd->LockWindowUpdate()
