...
void CXTPDockingPanePanelTheme2012::RefreshMetrics()
{
	...
	m_rcHeaderMargin = XTP_DPI(XTPIniRect(_T("DockingPane.Panel"), _T("HeaderMargin"), CRect(0,0,0,0)));
}
...
void CXTPDockingPanePanelTheme2012::DrawSingleButton(CDC *pDC, CXTPTabManagerItem *pItem)
{
	...
	switch (pItem->GetTabManager()->GetPosition())
	{
		case xtpTabPositionTop:
		{
			...
			CRect rcGripper(rcItem.left, rcItem.bottom-XTP_DPI_Y(m_nGripperWidth), rcItem.right, rcItem.bottom);
		...
		case xtpTabPositionBottom:
		{
			...
			CRect rcGripper(rcItem.left, rcItem.top, rcItem.right, rcItem.top + XTP_DPI_Y(m_nGripperWidth));
			pDC->FillSolidRect(rcGripper, pColorSet->GetItemColor(pItem));
		...
		case xtpTabPositionLeft:
		{
			...
			CRect rcGripper(rcItem.right - XTP_DPI_X(m_nGripperWidth), rcItem.top, 
				rcItem.right, rcItem.bottom - XTP_DPI_Y(10));
			pDC->FillSolidRect(rcGripper, pColorSet->GetItemColor(pItem));
		...
		case xtpTabPositionRight:
		{
			...
			CRect rcGripper(rcItem.left, rcItem.top, 
				rcItem.left + XTP_DPI_X(m_nGripperWidth), rcItem.bottom - XTP_DPI_Y(10));
			pDC->FillSolidRect(rcGripper, pColorSet->GetItemColor(pItem));
