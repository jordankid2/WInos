...
void CXTPDockingPaneDefaultTheme::DrawCaptionPart(CDC* pDC, CXTPDockingPaneBase* pPane, CRect rcCaption, const CString& strTitle, BOOL bActive, BOOL bVertical)
{
	...
	DrawCaptionButtons(pDC, pPane->GetCaptionButtons(), rcCaption, GetXtremeColor(COLOR_BTNTEXT), nButtonSize/*XTP_DPI_X(14)*/, 0, bVertical);
...
void CXTPDockingPaneDefaultTheme::DrawCaption(CDC* pDC, CXTPDockingPaneTabbedContainer* pPane, CRect rc)
{
	...
	CRect rcCaption(rc);
	XTPDeflateRectDirection(rcCaption, 0, XTP_DPI_Y(2), 0, XTPGetRectDirectionHeight(rc, bVertical) - m_nTitleHeight - XTP_DPI_Y(1), bVertical);

	BOOL bActive = m_bHighlightActiveCaption && pPane->IsActive();

	if (bVertical)
	{
		pDC->FillSolidRect(rc.left, rc.top, rcCaption.right + XTP_DPI_X(2), rcCaption.Height(), GetXtremeColor(COLOR_3DFACE));
	}
	else
	{
