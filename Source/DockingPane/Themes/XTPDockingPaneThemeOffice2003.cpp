...
void CXTPDockingPaneOffice2003Theme::DrawCaptionPart(CDC* pDC, CXTPDockingPaneBase* pPane, CRect rcCaption, const CString& strTitle, BOOL bActive, BOOL bVertical)
{
	...
	if (!bVertical && m_bDrawGripper && (rcCaption.right > rcCaption.left + XTP_DPI_X(7)))
	{
		...
		for (int y = rcCaption.top + XTP_DPI_Y(5); y < rcCaption.bottom - XTP_DPI_Y(5); y += szGripper2.cy)
		{
			pDC->FillSolidRect(x2, y + szGripper05.cy,
				szGripper.cx, szGripper.cy, GetXtremeColor(COLOR_WINDOW));
			pDC->FillSolidRect(x1, y, 
				szGripper.cx, szGripper.cy, GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
	...
	if (bVertical && m_bDrawGripper && (rcCaption.bottom > rcCaption.top + XTP_DPI_Y(7)))
	{
		...
		for (int x = rcCaption.left + szGripper2.cy; x < rcCaption.right - XTP_DPI_X(5); x += szGripper2.cx)
		{
			pDC->FillSolidRect(x + szGripper05.cx, y2, 
				szGripper.cx, szGripper.cy, GetXtremeColor(COLOR_WINDOW));
			pDC->FillSolidRect(x, y1, 
				szGripper.cx, szGripper.cy, GetXtremeColor(XPCOLOR_TOOLBAR_GRIPPER));
		}
