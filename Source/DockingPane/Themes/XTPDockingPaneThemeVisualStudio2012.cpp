// XTPDockingPaneThemeVisualStudio2012.cpp : implementation of the CXTPDockingPaneVisualStudio2012Theme class.
//
...
void CXTPDockingPaneVisualStudio2012Theme::SetPaneColorSets(CXTPDockingPaneColorSet* pcsActive, CXTPDockingPaneColorSet* pcsInactive)
{
	DeletePaneColorSets();
...
void CXTPDockingPaneVisualStudio2012Theme::RefreshMetrics()
{
	...
	m_clrSplitterGradient.clrDark = XTPIniColor(strSection,
		_T("SplitterDark"), RGB(204, 206, 219));
}
...
void CXTPDockingPaneVisualStudio2012Theme::DrawGripper(CDC *pDC, CRect rcGripper, COLORREF clr, BOOL bVertical)
{
	if (bVertical)
	{
		...
		DrawDottedLine(pDC, rcGripper.left + XTP_DPI_X(2), rcGripper.top + XTP_DPI_Y(2), rcGripper.bottom, clr, TRUE);
		DrawDottedLine(pDC, rcGripper.left + XTP_DPI_X(4), rcGripper.top, rcGripper.bottom, clr, TRUE);
	}
	else
	{
		DrawDottedLine(pDC, rcGripper.left, rcGripper.top, rcGripper.right, clr, FALSE);
		DrawDottedLine(pDC, rcGripper.left + XTP_DPI_X(2), rcGripper.top + XTP_DPI_Y(2), rcGripper.right, clr, FALSE);
		DrawDottedLine(pDC, rcGripper.left, rcGripper.top + XTP_DPI_Y(4), rcGripper.right, clr, FALSE);
	}
}

void CXTPDockingPaneVisualStudio2012Theme::DrawDottedLine(CDC *pDC, int x, int y, int nSize, COLORREF clr, BOOL bVertical)
{
	if (bVertical)
	{
		while (y < nSize)
...
void CXTPDockingPaneVisualStudio2012Theme::DrawCaptionButton(CDC *pDC, CXTPDockingPaneCaptionButton *pButton, COLORREF clrButton)
{
	...
	CXTPDockingPaneColorSet* pColorSet = GetColorSet(XTP_SAFE_GET2(pButton, GetPane(), IsActive() && m_bHighlightActiveCaption, FALSE));
	ASSERT(pColorSet);
	...
	BOOL bActive = XTP_SAFE_GET2(pButton, GetPane(), IsActive() && m_bHighlightActiveCaption, FALSE);
	...
	{
		pDC->Draw3dRect(pButton->GetRect(), colorSetButton.m_clrOutline, colorSetButton.m_clrOutline);
	}
}
