// XTPDockingPaneThemeVisualStudio6.cpp : implementation of the CXTPDockingPaneVisualStudio6Theme class.
//
...
void CXTPDockingPaneVisualStudio6Theme::DrawCaptionPart(CDC* pDC, CXTPDockingPaneBase* pPane, CRect rcCaption, const CString& strTitle, BOOL bActive, BOOL bVertical)
{
	...
	if (bVertical)
	{
		if (rcCaption.bottom > nOffsetX)
		{
			...
			pDC->Draw3dRect(nCenter - XTP_DPI_X(3), nOffsetY, XTP_DPI_X(3), rcCaption.bottom - nOffsetY, GetXtremeColor(COLOR_BTNHILIGHT), GetXtremeColor(COLOR_3DSHADOW));
			pDC->Draw3dRect(nCenter + XTP_DPI_X(1), nOffsetY, XTP_DPI_X(3), rcCaption.bottom - nOffsetY, GetXtremeColor(COLOR_BTNHILIGHT), GetXtremeColor(COLOR_3DSHADOW));
		}
	}
	else
	{
		if (rcCaption.right > nOffsetX)
		{
			...
			pDC->Draw3dRect(nOffsetX, nCenter - XTP_DPI_Y(3), rcCaption.right - nOffsetX, XTP_DPI_Y(3), GetXtremeColor(COLOR_BTNHILIGHT), GetXtremeColor(COLOR_3DSHADOW));
			pDC->Draw3dRect(nOffsetX, nCenter + XTP_DPI_Y(1), rcCaption.right - nOffsetX, XTP_DPI_Y(3), GetXtremeColor(COLOR_BTNHILIGHT), GetXtremeColor(COLOR_3DSHADOW));
		}
	}
}
