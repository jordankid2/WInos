...
void CXTPDockingPaneOffice2013Theme::DrawSplitter(CDC *pDC, CXTPDockingPaneSplitterWnd *pSplitter)
{
	...
	CXTPPenDC pen(*pDC, PS_SOLID, 1, m_clrSplitter);

	switch (pSplitter->GetPaneDirection())
	{
	case xtpPaneDockLeft:
		{
			pDC->MoveTo(rc.right-XTP_DPI_X(1), rc.top);
			pDC->LineTo(rc.right-XTP_DPI_X(1), rc.bottom);
		}
		break;
	case xtpPaneDockTop:
		{
...
void CXTPDockingPaneOffice2013Theme::DrawTitleBar(CDC *pDC, CXTPDockingPaneBase *pPane, CRect rcCaption, BOOL bVertical)
{
...
void CXTPDockingPaneOffice2013Theme::DrawAutoHidePanel(CDC* pDC, CRect rc, XTPDockingPaneDirection direction)
{
	pDC->FillSolidRect(rc, m_clrBackground);
	...
	{
	case xtpPaneDockLeft:
		pDC->FillSolidRect(rc.right-XTP_DPI_X(1), rc.top, XTP_DPI_X(1), rc.Height(), m_clrSplitter);
		break;

	case xtpPaneDockTop:
		pDC->FillSolidRect(rc.left, rc.bottom - XTP_DPI_Y(1), rc.Width(), XTP_DPI_Y(1), m_clrSplitter);
		break;
