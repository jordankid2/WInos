...
void CXTPDockingPaneGripperedTheme::DrawCaptionPart(CDC* pDC, CXTPDockingPaneBase* pPane, CRect rcCaption, const CString& strTitle, BOOL bActive, BOOL bVertical)
{
	...
	DrawCaptionButtons(pDC, pPane->GetCaptionButtons(), rcCaption, clrText, XTP_DPI_X(14), XTP_DPI_X(2), bVertical);
	...
	if(NULL == hCustomBrush)
	{
		if (bVertical)
		{
			if (rcCaption.bottom > nOffset)
			{
				...
				pDC->Draw3dRect(nCenter - XTP_DPI_X(3), nOffset, XTP_DPI_X(3), rcCaption.bottom - nOffset, GetXtremeColor(COLOR_BTNHILIGHT), GetXtremeColor(COLOR_3DSHADOW));
				pDC->Draw3dRect(nCenter + XTP_DPI_X(1), nOffset, XTP_DPI_X(3), rcCaption.bottom - nOffset, GetXtremeColor(COLOR_BTNHILIGHT), GetXtremeColor(COLOR_3DSHADOW));
			}
		}
		else
		{
			if (rcCaption.right > nOffset)
			{
				...
				pDC->Draw3dRect(nOffset, nCenter - XTP_DPI_Y(3), rcCaption.right - nOffset, XTP_DPI_Y(3), GetXtremeColor(COLOR_BTNHILIGHT), GetXtremeColor(COLOR_3DSHADOW));
				pDC->Draw3dRect(nOffset, nCenter + XTP_DPI_Y(1), rcCaption.right - nOffset, XTP_DPI_Y(3), GetXtremeColor(COLOR_BTNHILIGHT), GetXtremeColor(COLOR_3DSHADOW));
			}
		}
	}
}
...
void CXTPDockingPaneGripperedTheme::DrawCaption(CDC* pDC, CXTPDockingPaneTabbedContainer* pPane, CRect rc)
{
	...
	CRect rcCaption(rc);
	XTPDeflateRectDirection(rcCaption, 0, m_rcCaptionMargin.top, 0, XTPGetRectDirectionHeight(rc, bVertical) - nTitleHeight - XTP_DPI_Y(1), bVertical);

	BOOL bActive = m_bHighlightActiveCaption && pPane->IsActive();

	if (bVertical)
	{
		pDC->FillSolidRect(rc.left, rc.top, rcCaption.right + XTP_DPI_X(2), rc.Height(), GetXtremeColor(COLOR_3DFACE));
	}
	else
	{
		pDC->FillSolidRect(rc.left, rc.top, rc.Width(), rcCaption.bottom + XTP_DPI_Y(2), GetXtremeColor(COLOR_3DFACE));
	}
