// XTPDockingPaneThemeVisualStudio2015.cpp : implementation of the CXTPDockingPaneVisualStudio2015Theme class.
//
...
#include "DockingPane/TabThemes/XTPDockingPanePanelTheme2015.h"
...
CXTPDockingPaneVisualStudio2015Theme::CXTPDockingPaneVisualStudio2015Theme()
{
	// Tab paint manager
	m_pTabPaintManager->SetAppearanceSet(new CXTPDockingPaneTabTheme2015());
	m_pTabPaintManager->SetColorSet(new CXTPDockingPaneTabColorSet2015());

	// Panel paint manager
	m_pPanelPaintManager->SetAppearanceSet(new CXTPDockingPanePanelTheme2015());
	m_pPanelPaintManager->SetColorSet(new CXTPDockingPanePanelColorSet2015());

	// Pane color sets
	SetPaneColorSets(new CXTPDockingPaneColorSet2015, new CXTPDockingPaneColorSet2015);
}
...
void CXTPDockingPaneVisualStudio2015Theme::RefreshMetrics()
{
	...
	m_strFontFace = XTPIniString(strSection,
        _T("FontFace"), _T("Segoe UI"));
	m_nFontSize = XTP_DPI_Y(XTPIniInt(strSection,
		_T("FontSize"), 10));
	m_rcCaptionMargin = XTP_DPI(XTPIniRect(strSection,
		_T("CaptionMargin"), CRect(5, 0, 5, 0)));
	m_clrSplitter = XTPIniColor(strSection,
		_T("Splitter"), RGB(239, 239, 242));
	m_clrBackground = XTPIniColor(strSection,
		_T("Background"), RGB(239, 239, 242));
	m_clrSplitterGradient.clrLight = XTPIniColor(strSection,
		_T("Splitter"), RGB(239, 239, 242));
	m_clrSplitterGradient.clrDark  = XTPIniColor(strSection,
		_T("SplitterDark"), RGB(239, 239, 242));
	m_clrCaptionBorder = XTPIniColor(strSection + _T(".Tabs"),
		_T("ButtonBorder"), RGB(204, 206, 219));
}

void CXTPDockingPaneVisualStudio2015Theme::DrawSplitter(CDC *pDC, CXTPDockingPaneSplitterWnd *pSplitter)
{
...
void CXTPDockingPaneVisualStudio2015Theme::DrawTitleBar(CDC *pDC, CXTPDockingPaneBase *pPane, CRect rcCaption, BOOL bVertical)
{
	...
	if (NULL == hCustomBrush)
	{
		...
		else
		{
			...
			COLORREF clrBorder = bActive ? pColorSet->m_clrCaption : m_clrCaptionBorder;
			COLORREF pClrBorder[4] = { clrBorder, clrBorder, clrBorder, pColorSet->m_clrCaption };
...
void CXTPDockingPaneVisualStudio2015Theme::DrawAutoHidePanel(CDC* pDC, CRect rc, XTPDockingPaneDirection direction)
{
	...
	case xtpPaneDockLeft:
		pDC->FillSolidRect(rc.right - XTP_DPI_X(4), rc.top, XTP_DPI_X(4), rc.Height(), m_clrSplitter);
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top, XTP_DPI_X(1), rc.Height(), m_clrCaptionBorder);
		break;

	case xtpPaneDockTop:
		pDC->FillSolidRect(rc.left, rc.bottom - XTP_DPI_Y(4), rc.Width(), XTP_DPI_Y(4), m_clrSplitter);
		pDC->FillSolidRect(rc.left, rc.bottom - XTP_DPI_Y(1), rc.Width(), XTP_DPI_Y(1), m_clrCaptionBorder);
		break;
