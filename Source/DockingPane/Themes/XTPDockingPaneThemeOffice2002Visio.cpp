// XTPDockingPaneThemeOffice2002Visio.cpp : implementation of the CXTPDockingPaneOffice2002VisioTheme class.
//
...
void CXTPDockingPaneOffice2002VisioTheme::DrawPane(CDC* pDC, CXTPDockingPaneTabbedContainer* pPane, CRect rc)
{
	...
	{
		pDC->Draw3dRect(rc.left - XTP_DPI_X(1), rc.top - XTP_DPI_Y(1), 
			rc.Width() + XTP_DPI_X(2), rc.Height() + XTP_DPI_Y(2), 
			m_pTabPaintManager->GetColorSet()->m_clrFrameBorder, m_pTabPaintManager->GetColorSet()->m_clrFrameBorder);
	}
...
void CXTPDockingPaneOffice2002VisioTheme::DrawFloatingFrame(CDC* pDC, CXTPDockingPaneMiniWnd* pPane, CRect rc)
{
	...
	DrawCaptionButtons(pDC, pPane->GetCaptionButtons(), rcCaption, GetXtremeColor(COLOR_BTNTEXT), XTP_DPI_X(16), 0, FALSE);
	...
	if(NULL == hCustomBrush)
	{
		if (nRight - nLeft > 0)
		{
			...
			Line(pDC, nLeft, nCenter - XTP_DPI_Y(3), nRight, nCenter - XTP_DPI_Y(3), !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
			Line(pDC, nLeft, nCenter - XTP_DPI_Y(1), nRight, nCenter - XTP_DPI_Y(1), !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
			Line(pDC, nLeft, nCenter + XTP_DPI_Y(1), nRight, nCenter + XTP_DPI_Y(1), !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
			Line(pDC, nLeft, nCenter + XTP_DPI_Y(3), nRight, nCenter + XTP_DPI_Y(3), !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
		}
	}
...
void CXTPDockingPaneOffice2002VisioTheme::DrawCaption(CDC* pDC, CXTPDockingPaneTabbedContainer* pPane, CRect rc)
{
	...
	XTPDeflateRectDirection(rcCaption, 0, XTP_DPI_Y(2), XTP_DPI_X(1), 
		XTPGetRectDirectionHeight(rc, bVertical) - nTitleHeight - XTP_DPI_Y(3), bVertical);
	...
	if(NULL == hCustomBrush)
	{
		if (bVertical)
		{
			if (rcCaption.bottom - nOffset > 0)
			{
				...
				Line(pDC, nCenter - XTP_DPI_X(3), nOffset, nCenter - XTP_DPI_X(3), rcCaption.bottom, !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
				Line(pDC, nCenter - XTP_DPI_X(1), nOffset, nCenter - XTP_DPI_X(1), rcCaption.bottom, !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
				Line(pDC, nCenter + XTP_DPI_X(1), nOffset, nCenter + XTP_DPI_X(1), rcCaption.bottom, !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
				Line(pDC, nCenter + XTP_DPI_X(3), nOffset, nCenter + XTP_DPI_X(3), rcCaption.bottom, !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
			}
		}
		else
		{
			...
			{
				int nCenter = rcCaption.CenterPoint().y;
				Line(pDC, nOffset, nCenter - XTP_DPI_Y(3), rcCaption.right, nCenter - XTP_DPI_Y(3), !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
				Line(pDC, nOffset, nCenter - XTP_DPI_Y(1), rcCaption.right, nCenter - XTP_DPI_Y(1), !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
				Line(pDC, nOffset, nCenter + XTP_DPI_Y(1), rcCaption.right, nCenter + XTP_DPI_Y(1), !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
				Line(pDC, nOffset, nCenter + XTP_DPI_Y(3), rcCaption.right, nCenter + XTP_DPI_Y(3), !bActive ? XPCOLOR_TOOLBAR_GRIPPER : COLOR_BTNTEXT);
			}
		}
