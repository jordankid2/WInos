...
BOOL CXTPDockingPaneTabbedContainer::IsCaptionButtonVisible(CXTPDockingPaneCaptionButton* pButton)
{
	...
	{
	case XTP_IDS_DOCKINGPANE_CLOSE:
		bVisible = m_pSelectedPane && ((m_pSelectedPane->GetOptions() & xtpPaneNoCloseable) == 0);
		break;

	case XTP_IDS_DOCKINGPANE_AUTOHIDE:
		bVisible = m_pSelectedPane && ((m_pSelectedPane->GetOptions() & xtpPaneNoHideable) == 0) && (pButton->GetState() & xtpPanePinVisible);
		break;

	case XTP_IDS_DOCKINGPANE_MAXIMIZE:
		bVisible = IsPaneRestored();
		break;

	case XTP_IDS_DOCKINGPANE_RESTORE:
		bVisible = IsPaneMaximized();
		break;

	case XTP_IDS_DOCKINGPANE_MENU:
		bVisible = m_pSelectedPane && (m_pSelectedPane->GetOptions() & xtpPaneHasMenuButton);
		break;
	}
...
void CXTPDockingPaneTabbedContainer::GetMinMaxInfo(LPMINMAXINFO pMinMaxInfo) const
{
	...
	if (IsPaneMinimized())
	{
		if (m_pParentContainer && ((CXTPDockingPaneSplitterContainer*)m_pParentContainer)->IsHoriz())
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneTabbedContainer, CWnd)
	DISP_FUNCTION_ID(CXTPDockingPaneTabbedContainer, "Count", 1, OleGetItemCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPDockingPaneTabbedContainer, "Pane", DISPID_VALUE, OleGetPane, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPDockingPaneTabbedContainer, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "ParentContainer", 20, OleGetContainer, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "Type", 21, OleGetType, SetNotSupported, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "hWnd", DISPID_HWND, OleGetHwnd, SetNotSupported, VT_HANDLE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "IsEmpty", 22, OleIsEmpty, SetNotSupported, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "Position", 23, OleGetPosition, SetNotSupported, VT_I4)
END_DISPATCH_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPDockingPaneTabbedContainer, XTPDIID_IPaneContainer)

IMPLEMENT_ENUM_VARIANT(CXTPDockingPaneTabbedContainer);

LPDISPATCH CXTPDockingPaneTabbedContainer::OleGetContainer()
{
	return m_pParentContainer ? m_pParentContainer->OleGetDispatch(TRUE) : NULL;
}

int CXTPDockingPaneTabbedContainer::OleGetPosition()
{
	return GetDockingPaneManager()->GetPaneDirection(this);
}

int CXTPDockingPaneTabbedContainer::OleGetType()
{
	return m_type;
}

BOOL CXTPDockingPaneTabbedContainer::OleIsEmpty()
{
	return IsEmpty();
}

HWND CXTPDockingPaneTabbedContainer::OleGetHwnd()
{
	return m_hWnd;
}

int CXTPDockingPaneTabbedContainer::OleGetItemCount()
{
	return static_cast<int>(m_lstPanes.GetCount());
}

LPDISPATCH CXTPDockingPaneTabbedContainer::OleGetPane(int nIndex)
{
	return OleGetItem(nIndex - 1);
}

LPDISPATCH CXTPDockingPaneTabbedContainer::OleGetItem(int nIndex)
{
	POSITION pos = m_lstPanes.FindIndex(nIndex);
	if (!pos)
		return NULL;

	CXTPDockingPane* pPane = (CXTPDockingPane*)m_lstPanes.GetAt(pos);
	return XTPGetDispatch(pPane);
}

LPDISPATCH CXTPDockingPaneTabbedContainer::OleGetDispatch(BOOL bAddRef)
{
...
BEGIN_INTERFACE_MAP(CXTPDockingPaneTabbedContainer, CWnd)
	INTERFACE_PART(CXTPDockingPaneTabbedContainer, IID_IAccessible, ExternalAccessible)
	XTP_DECLARE_CMDTARGETPROVIDER_INTERFACE_PART(CXTPDockingPaneTabbedContainer)
#ifdef _XTP_ACTIVEX
	INTERFACE_PART(CXTPDockingPaneTabbedContainer, XTPDIID_IPaneContainer, Dispatch)
#endif
END_INTERFACE_MAP()

XTP_IMPLEMENT_CMDTARGETPROVIDER_INTERFACE(CXTPDockingPaneTabbedContainer)
