...
IMPLEMENT_DYNAMIC(CXTPDockingPaneLayout, CXTPCmdTarget);
...
BOOL CXTPDockingPaneSidePanel::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_DPI_Rect(pPX, _T("WindowRect"), m_rcWindow, CRect(0, 0, 0, 0));
...
BOOL CXTPDockingPaneMiniWnd::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_Bool(pPX, _T("Collapsed"), m_bCollapsed, FALSE);
	PX_DPI_Y_Int(pPX, _T("ExpandedHeight"), m_nExpandedHeight, 0);
...
void CXTPDockingPaneLayout::HidePane(CXTPDockingPaneBase* pPane)
{
	...
	if (direction < xtpPaneDockLeft || direction > xtpPaneDockBottom)
...
void CXTPDockingPaneLayout::OnSizeParent(CWnd* pParent, CRect rect, LPVOID lParam)
{
	...
	CRect rcClientMargins(m_pManager->GetClientMargins());
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneLayout, CXTPCmdTarget)
	//{{AFX_DISPATCH_MAP(CXTPDockingPaneLayout)
	DISP_FUNCTION_ID(CXTPDockingPaneLayout, "DoPropExchange", 1, OleDoPropExchange, VT_EMPTY, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPDockingPaneLayout, "PanesCount", 2, OleGetPaneCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPDockingPaneLayout, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
	DISP_FUNCTION_ID(CXTPDockingPaneLayout, "Pane", DISPID_VALUE, OleGetPane, VT_DISPATCH, VTS_I4)

	DISP_FUNCTION_ID(CXTPDockingPaneLayout, "ContainersCount", 3, OleGetItemCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPDockingPaneLayout, "Container", 4, OleGetContainer, VT_DISPATCH, VTS_I4)

	DISP_PROPERTY_EX_ID(CXTPDockingPaneLayout, "TopContainer", 5, OleGetTopContainer, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneLayout, "ClientContainer", 6, OleGetClientContainer, SetNotSupported, VT_DISPATCH)
	//}}AFX_DISPATCH_MAP
END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPDockingPaneLayout, CXTPCmdTarget)
	INTERFACE_PART(CXTPDockingPaneLayout, XTPDIID_IDockingPaneLayout, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPDockingPaneLayout, XTPDIID_IDockingPaneLayout)
IMPLEMENT_ENUM_VARIANT(CXTPDockingPaneLayout);

CXTPDockingPaneLayout* AFX_CDECL CXTPDockingPaneLayout::FromDispatch(LPDISPATCH pDisp)
{
	CXTPDockingPaneLayout* pInst = NULL;
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPDockingPaneLayout, CXTPDockingPaneLayout::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
...
void CXTPDockingPaneLayout::OleDoPropExchange(LPDISPATCH lpPropExchage)
{
...
long CXTPDockingPaneLayout::OleGetPaneCount()
{
	return (long)GetPaneList().GetCount();
}

LPDISPATCH CXTPDockingPaneLayout::OleGetPane(int nIndex)
{
	POSITION pos = m_lstPanes.FindIndex(nIndex - 1);
	if (!pos)
		return NULL;

	CXTPDockingPaneBase* pPane = (CXTPDockingPaneBase*)m_lstPanes.GetAt(pos);
	return pPane ? pPane->OleGetDispatch(TRUE) : 0;
}

LPDISPATCH CXTPDockingPaneLayout::OleGetItem(int nIndex)
{
	POSITION pos = m_lstStack.FindIndex(nIndex);
	if (!pos)
		return NULL;

	CXTPDockingPaneBase* pPane = (CXTPDockingPaneBase*)m_lstStack.GetAt(pos);
	return pPane ? pPane->OleGetDispatch(TRUE) : 0;
}

long CXTPDockingPaneLayout::OleGetItemCount()
{
	return (long)m_lstStack.GetCount();
}

LPDISPATCH CXTPDockingPaneLayout::OleGetContainer(int nIndex)
{
	POSITION pos = m_lstStack.FindIndex(nIndex - 1);
	if (!pos)
		return NULL;
...
LPDISPATCH CXTPDockingPaneLayout::OleGetTopContainer()
{
	return XTPGetDispatch(m_pTopContainer);
}

LPDISPATCH CXTPDockingPaneLayout::OleGetClientContainer()
{
	return m_pClient ? m_pClient->OleGetDispatch(TRUE) : NULL;
}
