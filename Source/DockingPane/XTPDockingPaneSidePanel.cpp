...
IMPLEMENT_DYNAMIC(CXTPDockingPaneSidePanel, CMiniFrameWnd)
...
int AFX_CDECL CXTPDockingPaneSidePanel::CompareLength(const void *arg1, const void *arg2)
{
...
void AFX_CDECL CXTPDockingPaneSidePanel::SortLength(LENGTH* pLength, int nFirstIndex, int nLastIndex)
{
...
BOOL CXTPDockingPaneSidePanel::IsCaptionVertical() const
{
	XTPDockingPaneDirection direction = GetDirection();
	return (xtpPaneDockLeft == direction || xtpPaneDockRight == direction);
}
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneSidePanel, CMiniFrameWnd)
	DISP_FUNCTION_ID(CXTPDockingPaneSidePanel, "Count", 1, OleGetItemCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPDockingPaneSidePanel, "Pane", DISPID_VALUE, OleGetPane, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPDockingPaneSidePanel, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "ParentContainer", 20, OleGetContainer, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "Type", 21, OleGetType, SetNotSupported, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "hWnd", DISPID_HWND, OleGetHwnd, SetNotSupported, VT_HANDLE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "IsEmpty", 22, OleIsEmpty, SetNotSupported, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "Position", 23, OleGetPosition, SetNotSupported, VT_I4)
END_DISPATCH_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPDockingPaneSidePanel, XTPDIID_IPaneContainer)

BEGIN_INTERFACE_MAP(CXTPDockingPaneSidePanel, CMiniFrameWnd)
	INTERFACE_PART(CXTPDockingPaneSidePanel, XTPDIID_IPaneContainer, Dispatch)
	XTP_DECLARE_CMDTARGETPROVIDER_INTERFACE_PART(CXTPDockingPaneSidePanel)
END_INTERFACE_MAP()

IMPLEMENT_ENUM_VARIANT(CXTPDockingPaneSidePanel);
XTP_IMPLEMENT_CMDTARGETPROVIDER_INTERFACE(CXTPDockingPaneSidePanel);

LPDISPATCH CXTPDockingPaneSidePanel::OleGetContainer()
{
	return m_pParentContainer ? m_pParentContainer->OleGetDispatch(TRUE) : NULL;
}

int CXTPDockingPaneSidePanel::OleGetPosition()
{
	return m_direction;
}

int CXTPDockingPaneSidePanel::OleGetType()
{
	return m_type;
}

BOOL CXTPDockingPaneSidePanel::OleIsEmpty()
{
	return IsEmpty();
}

HWND CXTPDockingPaneSidePanel::OleGetHwnd()
{
	return m_hWnd;
}

int CXTPDockingPaneSidePanel::OleGetItemCount()
{
	return static_cast<int>(m_lstPanes.GetCount());
}

LPDISPATCH CXTPDockingPaneSidePanel::OleGetPane(int nIndex)
{
	return OleGetItem(nIndex - 1);
}

LPDISPATCH CXTPDockingPaneSidePanel::OleGetItem(int nIndex)
{
...
LPDISPATCH CXTPDockingPaneSidePanel::OleGetDispatch(BOOL bAddRef)
{
