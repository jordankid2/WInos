...
int CXTPDockingPaneWindowSelect::CalcItemHeight(CDC* pDC)
{
	...
	return max(szIcon.cy + XTP_DPI_X(4), szFont.cy + XTP_DPI_Y(4));
}
...
{
#ifdef _XTP_ACTIVEX
	HWND hWndClient = OleFindMDIClient(m_pManager->GetSite());
#else
...
BOOL CXTPDockingPaneWindowSelect::Reposition()
{
	...
	int nItemWidth = XTP_DPI_X(163);

	int y = nItemHeight + XTP_DPI_Y(13);
	int x = XTP_DPI_X(9);
	int nRow = 0;
	int nColumn = 0;
	...
	while (pos)
	{
		...
		if (nColumn == 0 && m_pManager->IsClientHidden() && nRow > paneList.GetCount() / 2)
		{
			...
			nRow = 0;
			nColumn++;
			y = nItemHeight + XTP_DPI_Y(13);
			x += nItemWidth + XTP_DPI_X(12);
		}
	...
	CSize sz(XTP_DPI_X(4) + (nItemWidth + XTP_DPI_X(12)) * nTotalColumns,
		nItemHeight + XTP_DPI_Y(13) + nTotalRow * (nItemHeight + XTP_DPI_Y(1)) + XTP_DPI_Y(110));
...
void CXTPDockingPaneWindowSelect::OnPaint()
{
	...
	if (m_nPaneCount > 0)
	{
		CRect rcActiveTools(XTP_DPI_X(9), 0, XTP_DPI_X(9) + XTP_DPI_X(163), nItemHeight + XTP_DPI_Y(12));
		dc.DrawText(m_strActiveTools, rcActiveTools, DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);

		if (!m_pManager->IsClientHidden())
		{
			CRect rcActiveFiles(XTP_DPI_X(9) + XTP_DPI_X(13) + XTP_DPI_X(163), 0, 
				XTP_DPI_X(9) + XTP_DPI_X(13) + 2 * XTP_DPI_X(163), nItemHeight + XTP_DPI_Y(12));
			dc.DrawText(m_strActiveFiles, rcActiveFiles, DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
		}
	}
	else
	{
		CRect rcActiveFiles(XTP_DPI_X(9), 0, XTP_DPI_X(9) + 2 * XTP_DPI_X(163), nItemHeight + XTP_DPI_Y(12));
		dc.DrawText(m_strActiveFiles, rcActiveFiles, DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
	}
	...
	for (int i = 0; i < (int)m_arrItems.GetSize(); i++)
	{
		...
		{
			CPoint ptIcon(pItem->rc.left + XTP_DPI_X(2), pItem->rc.CenterPoint().y - szIcon.cy / 2);
		...
		rcText.left += XTP_DPI_X(5) + szIcon.cx;
		dc.DrawText(GetItemCaption(pItem), rcText, DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
	}

	CRect rcInfoPane(XTP_DPI_X(11), rc.bottom - XTP_DPI_Y(100), 
		rc.right - XTP_DPI_X(11), rc.bottom - XTP_DPI_Y(10));
	dc.Draw3dRect(rcInfoPane, GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER), GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER));

	if (!m_pSelected)
		return;

	rcInfoPane.DeflateRect(XTP_DPI_X(15), XTP_DPI_Y(13), XTP_DPI_X(15), XTP_DPI_Y(13));
