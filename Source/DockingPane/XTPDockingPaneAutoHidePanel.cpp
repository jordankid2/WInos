...
const int SPLITTER_GAP_X = 4;
const int SPLITTER_GAP_Y = 4;
...
void CXTPDockingPaneAutoHideWnd::GetMinMaxInfo (LPMINMAXINFO pMinMaxInfo, BOOL bIncludeSplitter /*= TRUE*/) const
{
	...
	{
		pMinMaxInfo->ptMinTrackSize.x += XTP_DPI_X(SPLITTER_GAP_X);
		pMinMaxInfo->ptMaxTrackSize.x += XTP_DPI_X(SPLITTER_GAP_X);
	}
	else
	{
		pMinMaxInfo->ptMinTrackSize.y += XTP_DPI_Y(SPLITTER_GAP_Y);
		pMinMaxInfo->ptMaxTrackSize.y += XTP_DPI_Y(SPLITTER_GAP_Y);
	}
}
	...
	: m_pPanel(pPanel), m_pPane(pPane)
{
	...
	CSize szDocking(pPane->m_pSelectedPane->m_szDocking.cx + XTP_DPI_X(SPLITTER_GAP_X), 
		pPane->m_pSelectedPane->m_szDocking.cy + XTP_DPI_Y(SPLITTER_GAP_Y));

	switch (pManager->GetRTLDirection(m_direction))
	{
	case xtpPaneDockLeft:
		szDocking.cx = min(szDocking.cx, rcAvail.right + XTP_DPI_X(SPLITTER_GAP_X) - rcClient.left);
		break;
	...
	case xtpPaneDockTop:
		szDocking.cy = min(szDocking.cy, rcAvail.bottom + XTP_DPI_Y(SPLITTER_GAP_Y) - rcClient.top);
		break;
...
void CXTPDockingPaneAutoHideWnd::RecalcLayout(BOOL /*bNotify*/)
{
	...
	switch (m_direction)
	{
		case xtpPaneDockLeft:
			...
			rcPane.right -= XTP_DPI_X(SPLITTER_GAP_X);
			break;

		case xtpPaneDockTop:
			rcPane.OffsetRect(0, - m_rcWindow.Height() + rcClient.Height());
			rcPane.bottom -= XTP_DPI_Y(SPLITTER_GAP_Y);
			break;

		case xtpPaneDockRight:
			rcPane.left += XTP_DPI_X(SPLITTER_GAP_X);
			break;

		case xtpPaneDockBottom:
			rcPane.top += XTP_DPI_Y(SPLITTER_GAP_Y);
			break;
	}
...
BOOL CXTPDockingPaneAutoHideWnd::GetAvailableRect(CRect& rcAvail, CRect& rc) const
{
	...
	int nGap = pManager->m_nSplitterGap + XTP_DPI_X(SPLITTER_GAP_X);
	...
	switch (m_direction)
	{
		case xtpPaneDockLeft:
			...
			rc.left = rc.right - XTP_DPI_X(SPLITTER_GAP_X);
			break;
		...
		case xtpPaneDockRight:
			...
			rc.right = rc.left + XTP_DPI_X(SPLITTER_GAP_X);
			break;
		...
		case xtpPaneDockTop:
			...
			rc.top = rc.bottom - XTP_DPI_Y(SPLITTER_GAP_Y);
			break;
		...
		case xtpPaneDockBottom:
			...
			rc.bottom = rc.top + XTP_DPI_X(SPLITTER_GAP_X);
			break;
	}
	return TRUE;
}
...
void CXTPDockingPaneAutoHidePanel::RefreshManagersArray(CDC* pDC, CRect rcClient)
{
	...
	if (bVertical)
	{
		rcClient.top += pPaneManager->GetClientMargins().top;
	}
	else
	{
		rcClient.left += pPaneManager->GetClientMargins().left;
	}
	...
	int nMarginIndent = rcMargins.left + rcMargins.right + XTP_DPI_X(8);
	...
	if (pPaneManager->m_bShowPanelScrollButtons)
	{
		...
		if (bAddButton)
		{
			if (m_nHeaderOffset < 0)
			{
				...
				if (m_nItemsLength + m_nHeaderOffset < nWindowEnd - XTP_DPI_Y(24) - nMarginIndent - nPanelIndent)
				{
					m_nHeaderOffset = min(0, nWindowEnd - XTP_DPI_Y(24) - nMarginIndent - m_nItemsLength - nPanelIndent);
				}
			}
...
LRESULT CXTPDockingPaneAutoHidePanel::OnMouseHover(WPARAM, LPARAM lParam)
{
	...
	CXTPDockingPane* pPane = HitTest(XTP_POINT_FROM_LPARAM(lParam));
	if (pPane)
	{
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneAutoHidePanel, CWnd)
...
IMPLEMENT_OLETYPELIB_EX(CXTPDockingPaneAutoHidePanel, XTPDIID_IPaneContainer)

BEGIN_INTERFACE_MAP(CXTPDockingPaneAutoHidePanel, CWnd)
	INTERFACE_PART(CXTPDockingPaneAutoHidePanel, XTPDIID_IPaneContainer, Dispatch)
	XTP_DECLARE_CMDTARGETPROVIDER_INTERFACE_PART(CXTPDockingPaneAutoHidePanel)
END_INTERFACE_MAP()

IMPLEMENT_ENUM_VARIANT(CXTPDockingPaneAutoHidePanel);
XTP_IMPLEMENT_CMDTARGETPROVIDER_INTERFACE(CXTPDockingPaneAutoHidePanel);
