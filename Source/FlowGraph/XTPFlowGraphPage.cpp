...
IMPLEMENT_SERIAL(CXTPFlowGraphPage, CXTPCmdTarget, VERSIONABLE_SCHEMA | _XTP_SCHEMA_CURRENT)
	...
	if (bAtLeastOneWindowNode) // for optimization purposes
	{
		...
		for (i=0; i<nodesWindowVisibleRegions.GetSize(); i++)
		{
			for (k=0; k<i; k++)
			{
				...
				{
					result = CombineRgn(nodesWindowVisibleRegions[k], nodesWindowVisibleRegions[k], hRegionIntersection, RGN_DIFF);
		...
		for (i=0; i<nodesWindowVisibleRegions.GetSize(); i++)
		{
			if (allNodes[i]->GetWindowHandle())
			{
				...
				// visible part of node, intersection with window
...
BEGIN_DISPATCH_MAP(CXTPFlowGraphPage, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPFlowGraphPage, CXTPCmdTarget)
	INTERFACE_PART(CXTPFlowGraphPage, XTPDIID_FlowGraphPage, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPFlowGraphPage, XTPDIID_FlowGraphPage)
