...
COLORREF AFX_CDECL CXTPFlowGraphPaintManager::GetDarkColor(COLORREF clrBackground)
{
...
void CXTPFlowGraphPaintManager::DrawConnection(CXTPFlowGraphDrawContext* pDC, CXTPFlowGraphConnection* pConnection)
{
	...
	pDC->SetPen(clrConnection, pConnection->GetWidth(), pConnection->GetDashed());
	...
	if (HasEndingArrow(type))
	{
		if ( pConnection->m_pPath)
		{
			...
			CXTPArrowDrawer arrowDrawer((float) (pConnection->GetWidth() + ARROW_BASE_WIDTH));
...
BEGIN_DISPATCH_MAP(CXTPFlowGraphPaintManager, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPFlowGraphPaintManager, CXTPCmdTarget)
	INTERFACE_PART(CXTPFlowGraphPaintManager, XTPDIID_FlowGraphPaintManager, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPFlowGraphPaintManager, XTPDIID_FlowGraphPaintManager)
