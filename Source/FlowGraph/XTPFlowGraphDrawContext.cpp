...
CXTPFlowGraphDrawContext::CXTPFlowGraphDrawContext(HDC hDC, HDC /*hAttribDC*/, BOOL bPrinting, BOOL bSaveToFile)
{
	m_hDC = hDC;
...
void CXTPFlowGraphDrawContext::Line(CPoint p1, CPoint p2)
{
...
void CXTPFlowGraphDrawContext::SetPen(COLORREF clrPen, int nWidth, BOOL bDashed /*= FALSE*/)
{
	if (m_hActivePen)
	{
	...
	if (nWidth)
	{
		m_hActivePen = ::CreatePen(bDashed ? PS_DASH : PS_SOLID, nWidth, clrPen);
	}
}
...
void CXTPFlowGraphDrawContext::DrawGrid(CRect rc, CPoint ptScrollOffset, double dZoomLevel, COLORREF clrGridColor)
{
	...
	{
		FillSolidRect((int)x, rc.top, 1, rc.Height(), clrGridColor);
	}
