...
CString AFX_CDECL CXTPFlowGraphControl::LoadXamlTemplate(LPCTSTR szTemplate)
{
	CString strTemplate;
...
void CXTPFlowGraphControl::StartDragConnectionPoint(CXTPFlowGraphConnectionPoint* pPoint)
{
	...
	//adding connection was canceled through notification in AddConnection()
	...
	while (::GetCapture() == m_hWnd)
	{
		...
		else if (msg.message == WM_MOUSEMOVE && pt != msg.pt)
		{
			...
			CXTPFlowGraphConnectionPoint* pHitPoint = HitTestConnectionArea(point);

			if (pHitPoint && (pHitPoint->GetType() == xtpFlowGraphPointInput || pHitPoint->GetType() == xtpFlowGraphPointBoth) && !pHitPoint->IsLocked() &&
				pHitPoint->GetNode() != pDragConnection->GetOutputPoint()->GetNode())
			{
				if ((pHitPoint->GetMaxConnections() == -1 
					|| pHitPoint->GetConnectionsCount() < pHitPoint->GetMaxConnections())
					&& pInputConnectionPoint != pHitPoint)
				{
					XTP_NM_FLOWGRAPH_CONNECTIONCHANGING cc;
					cc.pConnection = pDragConnection;
					cc.pInputConnectionPoint = pHitPoint;
