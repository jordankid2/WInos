...
void CXTPFlowGraphNode::UpdateMarkupImage()
{
	...
	if (m_pMarkup && m_nImageIndex != -1)
	{
		...
		{
			TCHAR szBuff[11];
			ITOT_S(m_nImageIndex, szBuff, _countof(szBuff));
...
void CXTPFlowGraphNode::UpdateMarkupPointImage(CXTPFlowGraphConnectionPoint *pPoint)
{
	...
	if (m_pMarkup && iImageIndex != -1)
	{
		...
		if (pNodeItem)
		{
			...
			{
				TCHAR szBuff[11];
				ITOT_S(iImageIndex, szBuff, _countof(szBuff));
...
void CXTPFlowGraphNode::UpdateMarkupFont()
{
	...
	{
		CXTPFont font;
		font.CreateFontIndirect(&GetControl()->GetPaintManager()->m_lfText);

		XTPMarkupSetDefaultFont(XTPMarkupElementContext(m_pMarkup),
			font, GetControl()->GetPaintManager()->m_clrTextColor);
	}
}
...
void CXTPFlowGraphNode::CalcMinSize()
{
	...
	if (m_pMarkup)
	{
		...
		if (pNodeCaption)
		{
			int nCaptionBorderPadding_X = XTP_DPI_X(20);	//from FlowGraph\res\node.xaml
			iMinWidth = pNodeCaption->GetActualSize().cx + nCaptionBorderPadding_X * 2; // left and right padding
		}
...
void CXTPFlowGraphNode::DoCommonPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->IsStoring())
	{
		...
		szStoredSize.cy -= 2;
		PX_Size(pPX, _T("UserSize"), szStoredSize, CSize(0, 0));
	}
	else
	{
...
{
	//If user size never been set (user still not make any resize), set it to actual size.
	if (m_szUserSize == CSize(0,0))
	{
		m_szUserSize = m_szActualSize;
	}
...
CRect CXTPFlowGraphNode::GetCaptionRect() const
{
	...
	if (pNodeCaption)
	{
		rect = GetMarkupVisualElementRect(pNodeCaption->GetVisualParent());
		rect.OffsetRect(GetLocation());

		// Resize caption width to the maximum visual parent element width
		int nMaxParentWidth = 0;
		CXTPMarkupVisual *pParent = pNodeCaption->GetVisualParent();
		while (pParent)
		{
			int nWidth = pParent->GetBoundRect().Width();
			if (nMaxParentWidth < nWidth)
			{
				nMaxParentWidth = nWidth;
			}

			pParent = pParent->GetVisualParent();
		}

		if (0 < nMaxParentWidth && rect.Width() < nMaxParentWidth)
		{
			int nExtra = (nMaxParentWidth - rect.Width()) / 2;
			rect.left -= nExtra;
			rect.right += nExtra;
		}
	}
...
BEGIN_INTERFACE_MAP(CXTPFlowGraphNode, CXTPFlowGraphElement)
	INTERFACE_PART(CXTPFlowGraphNode, XTPDIID_FlowGraphNode, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPFlowGraphNode, XTPDIID_FlowGraphNode)
