...
IMPLEMENT_DYNAMIC(CXTPPropertyGridItem, CXTPCmdTarget)
...
void CXTPPropertyGridItem::OnDeselect()
{
	if (m_pGrid)
	{
		if (m_pGrid->GetSafeHwnd())
			m_pGrid->Invalidate(FALSE);
		m_pGrid->FocusInplaceButton(NULL);
	}
	OnValidateEdit();
...
void CXTPPropertyGridItem::SetEditText(const CString& str)
{
	...
	if (wndEdit.GetSafeHwnd() && wndEdit.m_pItem == this)
	{
		...
		wndEdit.SetMargins(XTP_DPI_X(3), XTP_DPI_Y(3));
	}
}
...
CRect CXTPPropertyGridItem::GetInplaceWindowsRect()
{
	...
	CRect rcValue(m_pGrid->GetDividerPos() + 1, rc.top + 1, rc.right, rc.bottom - m_pGrid->GetPaintManager()->GetItemsBorderHeight());
...
BOOL CXTPPropertyGridItem::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	int nOffset = (m_nIndent - (IsCategory() ? 0 : 1)) * XTP_DPI_X(XTP_PGI_EXPAND_BORDER);

	if ((point.x >= nOffset && point.x <= XTP_DPI_X(XTP_PGI_EXPAND_BORDER) + nOffset) && IsExpandable())
	{
		if (m_bExpanded)
			Collapse();
		else
...
CXTPPropertyGridInplaceEdit& CXTPPropertyGridItem::GetInplaceEdit()
{
	...
	return CXTPSingleton<CXTPPropertyGridInplaceEdit>::Instance();
}

CXTPPropertyGridInplaceList& CXTPPropertyGridItem::GetInplaceList()
{
	if (m_pGrid) return *m_pGrid->m_pGrid->m_pInplaceListBox;
	return CXTPSingleton<CXTPPropertyGridInplaceList>::Instance();
}
...
{
	if (m_pGrid && 
		(GetMarkupValue() || GetMarkupCaption()))
	{
		CXTPPropertyGridPaintManager *pPaintManager = m_pGrid->GetPaintManager();

		CRect rc = GetItemRect();
		if (rc.right <= 0 && rc.right <= rc.left)
		{
			CXTPClientRect rcClient(m_pGrid);
			rc.right = rc.left + rcClient.Width();
		}

		CRect rcCaption(rc.left, rc.top, rc.left + m_pGrid->GetDividerPos(), rc.bottom);
		CRect rcValue(rcCaption.right, rc.top, rc.right, rc.bottom - pPaintManager->GetItemsBorderHeight());
		
		rcValue.left += XTP_DPI_X(4); // as in CXTPPropertyGridPaintManager::DrawItemValue
		...
		if (GetMarkupCaption())
		{
			CSize sz = XTPMarkupMeasureElement(GetMarkupCaption(), rcCaption.Width(), INT_MAX);
			lpMeasureItemStruct->itemHeight = max(lpMeasureItemStruct->itemHeight, (UINT)sz.cy);
		}
	}
	else
	{
		...
		{
			lpMeasureItemStruct->itemHeight = (lpMeasureItemStruct->itemHeight - XTP_DPI_Y(4)) * m_nLinesCount + 4;
...
CSize CXTPPropertyGridItem::OnMergeItemConstraint(CDC* pDC, CXTPPropertyGridItemConstraint* pConstraint)
{
	...
	return CSize(sz.cx + nThumbLength * 2, sz.cy + XTP_DPI_Y(3));
}
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItemConstraint, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPPropertyGridItemConstraint, CXTPCmdTarget)
	INTERFACE_PART(CXTPPropertyGridItemConstraint, XTPDIID_IPropertyGridItemConstraint, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridItemConstraint, XTPDIID_IPropertyGridItemConstraint)


///////////////////////////////////////////////////////////////////////////////
/// CXTPPropertyGridItemConstraints

BEGIN_DISPATCH_MAP(CXTPPropertyGridItemConstraints, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPPropertyGridItemConstraints, CXTPCmdTarget)
	INTERFACE_PART(CXTPPropertyGridItemConstraints, XTPDIID_IPropertyGridItemConstraints, Dispatch)
	//INTERFACE_PART(CXTPPropertyGridItemConstraints, IID_IEnumVARIANT, EnumVARIANT)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridItemConstraints, XTPDIID_IPropertyGridItemConstraints)
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItems, CXTPCmdTarget)
...
BEGIN_INTERFACE_MAP(CXTPPropertyGridItems, CXTPCmdTarget)
	INTERFACE_PART(CXTPPropertyGridItems, XTPDIID_IPropertyGridItems, Dispatch)
	//INTERFACE_PART(CXTPPropertyGridItems, IID_IEnumVARIANT, EnumVARIANT)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridItems, XTPDIID_IPropertyGridItems)
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItem, CXTPCmdTarget)
...
IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridItem, XTPDIID_IPropertyGridItem)
...
BEGIN_INTERFACE_MAP(CXTPPropertyGridItem, CXTPCmdTarget)
	INTERFACE_PART(CXTPPropertyGridItem, IID_IAccessible, ExternalAccessible)
#ifdef _XTP_ACTIVEX
	INTERFACE_PART(CXTPPropertyGridItem, XTPDIID_IPropertyGridItem, Dispatch)
#endif
END_INTERFACE_MAP()
