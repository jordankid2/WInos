...
void CXTPPropertyGridView::OnNcPaint()
{
	...
	ASSERT(m_pGrid);
	GetPaintManager()->DrawPropertyGridBorder(&dc, rc, xtpGridGridWindow);
}
...
void CXTPPropertyGridView::ResetContent()
{
	...
	if (m_hWnd)
	{
		...
		m_nItemHeight = dc.GetTextExtent(_T(" "), 1).cy + XTP_DPI_Y(4);
...
int  CXTPPropertyGridView::InsertItem(CXTPPropertyGridItem* pItem, int nIndex)
{
	...
	if (GetStyle() & LBS_OWNERDRAWVARIABLE)
	{
		...
		//The LBS_OWNERDRAWVARIABLE style specifies that the items in the list box are variable in height 
		//and that the list box parent window is responsible for drawing, sorting, and storing the list box contents. 
		//The height of an item is limited to 255 pixels.
		SetItemHeight(nIndex, measureItemStruct.itemHeight < XTP_DPI_Y(MAX_ALLOWED_LIST_BOX_ITEM_HEIGHT) ? measureItemStruct.itemHeight : XTP_DPI_Y(MAX_ALLOWED_LIST_BOX_ITEM_HEIGHT));
	}
...
void CXTPPropertyGridView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	AxGetParentOleControl(this)->FireMouseMove(AxButtonState(), AxShiftState(), pointGrid.x, pointGrid.y);
#endif
}
...
int CXTPPropertyGridView::HitTest(CPoint point) const
{
	...
	if ((point.x > nDivider - XTP_DPI_X(4) && point.x <= nDivider + XTP_DPI_X(2)) && (GetCount() > 0))
...
void CXTPPropertyGridView::ShowToolTip(CPoint pt)
{
	...
	if (rcCaption.PtInRect(pt))
	{
		...
		rcCaptionText.left += pItem->m_nIndent * XTP_DPI_X(XTP_PGI_EXPAND_BORDER) + XTP_DPI_X(3 - 3 - 1);
		rcCaptionText.top--;
		...
		if (nTextExtent + XTP_DPI_X(3) > rcCaptionText.Width())
		{
			...
			{
				rcCaptionText.left = rcCaption.right - nTextExtent 
					- XTP_DPI_X(7) - (pItem->m_nIndent * XTP_DPI_X(XTP_PGI_EXPAND_BORDER));
