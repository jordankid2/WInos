...
void CXTPPropertyGridPaintManager::RefreshMetrics()
{
	...
	m_pMetrics->m_clrVerbFace.SetStandardValue(XTPSystemVersion()->IsWinVistaOrGreater() 
		&& m_themeButton->IsAppThemed()  ? RGB(0, 102, 204) : GetXtremeColor(COLOR_ACTIVECAPTION));

	m_nItemsBorderHeight = 1;	//grid line between items no need DPI scaling
...
void CXTPPropertyGridPaintManager::DrawSplitter(CDC* pDC, BOOL bIsHelp)
{
	...
	if (m_pGrid->GetBorderStyle() == xtpGridBorderNone ||
		m_pGrid->GetBorderStyle() == xtpGridBorderStaticEdge)
	{
		CRect rcTop(rc);
		rcTop.bottom = rcTop.top + 1;
		pDC->FillSolidRect(rcTop, m_clrSplitter.IsDefaultValue() ? GetXtremeColor(COLOR_3DFACE) : m_clrSplitter);
		
		if (m_pGrid->GetBorderStyle() == xtpGridBorderStaticEdge)
		{
			CRect rcLeft(rc);
	...
	{
		pDC->FillSolidRect(rc, m_clrSplitter.IsDefaultValue() ? GetXtremeColor(COLOR_3DFACE) : m_clrSplitter);
	}
}
...
void CXTPPropertyGridPaintManager::FillPropertyGrid(CDC* pDC)
{
	...
	if (m_pGrid->IsHelpVisible())
	{
		...
		DrawPropertyGridBorder(pDC, rcHelp, xtpGridHelpWindow);
		...
		if (pItem)
		{
			...
			if (!strDesc.IsEmpty())
			{
				...
				CRect rcDesc(rcHelp);
				rcDesc.DeflateRect(XTP_DPI_X(3) + XTP_DPI_X(3), XTP_DPI_Y(3) + XTP_DPI_Y(3) + rcCaptionHeight.Height(), 
				...
				if (pItem->GetMarkupDescription())
				{
					XTPMarkupSetDefaultFont(XTPMarkupElementContext(pItem->GetMarkupDescription()),
						(HFONT)m_pMetrics->m_xtpFontNormal, pDC->GetTextColor());
	...
	if (m_pGrid->IsVerbsVisible())
	{
		...
		DrawPropertyGridBorder(pDC, rcVerbs, xtpGridVerbWindow);
...
int CXTPPropertyGridPaintManager::HitTestVerbs(CDC* pDC, CRect rcVerbs, CPoint pt)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (bDraw)
		{
			...
			pDC->SetTextColor(m_pMetrics->m_clrHelpFore);
			pDC->DrawText(_T(","), 1, CRect(x, y, x + XTP_DPI_X(8), 
				min(y + nTextHeight, rcVerbs.bottom)), DT_SINGLELINE | DT_NOPREFIX);
...
void CXTPPropertyGridPaintManager::DrawPropertyGridBorder(CDC* pDC, RECT& rc, XTPPropertyGridWindow nWindow)
{
	if (!pDC)
	{
		return;
	...
	case xtpGridBorderStaticEdge:
		{
			...
			case xtpGridVerbWindow:
			default:
				clrBorder[0] = m_clrShadow; // left
				clrBorder[1] = m_bThinSplitter ? m_pMetrics->m_clrHelpBack : m_clrShadow; // top
				clrBorder[2] = m_clrShadow; // right
				clrBorder[3] = m_bThinSplitter ? m_pMetrics->m_clrHelpBack : m_clrShadow; // bottom
				break;
			}
	...
	case xtpGridBorderFlat:
		{
			...
			{
				pDC->Draw3dRect(rc.left + i, rc.top + i, rc.right - rc.left - i*2, rc.bottom - rc.top - i*2,
	...
	case xtpGridBorderClientEdge:
		{
			...
			{
				clrTL = GetXtremeColor( (i < nHalfThickness) ? COLOR_3DSHADOW    : COLOR_3DDKSHADOW );
				clrBR = GetXtremeColor( (i < nHalfThickness) ? COLOR_3DHIGHLIGHT : COLOR_3DFACE );
...
void CXTPPropertyGridPaintManager::DrawInplaceListItem(CDC* pDC, CXTPPropertyGridItemConstraint* pConstraint, CRect rc, BOOL bSelected)
{
	...
	if (pImage)
	{
		...
		rcText.left += nWidth + XTP_DPI_X(4);
	}
...
void CXTPPropertyGridPaintManager::DrawItemCaption(CDC* pDC, CXTPPropertyGridItem* pItem, CRect rcCaption)
{
	...
	rcText.left = (pItem->GetIndent() + (pItem->IsCategory() ? 1 : 0)) * XTP_DPI_X(XTP_PGI_EXPAND_BORDER) + XTP_DPI_X(3);

	rcText.right = rcCaption.right - XTP_DPI_X(1);
	rcText.bottom -= GetItemsBorderHeight();
...
void CXTPPropertyGridPaintManager::DrawItem(PDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if (pItem->IsCategory())
	{
		...
		CRect rcText(((pItem->GetIndent() + 1) * XTP_DPI_X(XTP_PGI_EXPAND_BORDER)) + XTP_DPI_X(3), 
			rc.top, rc.right, rc.bottom - GetItemsBorderHeight());
		...
		if (lpDrawItemStruct->itemState & ODS_FOCUS)
		{
			...
			rcFocus.SetRect(rcText.left - XTP_DPI_X(3), rcText.top + XTP_DPI_Y(1), 
				rcText.left + rcFocus.Width() + XTP_DPI_X(3), rcText.bottom - XTP_DPI_Y(1));
	...
	else
	{
		...
		CRect rcValue(rcCaption.right, rc.top, rc.right, rc.bottom - GetItemsBorderHeight());
		...
		BOOL bFocused = bSelected && CWnd::GetFocus() && ((CWnd::GetFocus() == pView) 
			|| (CWnd::GetFocus()->GetParent() == pView) 
		...
		while (pCategory != NULL)
		{
			...
			{
				dc.FillSolidRect(rc.left + pCategory->GetIndent() * XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rc.top,
					XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rc.Height(), m_pMetrics->m_clrLine);
			}
		...
		dc.MoveTo(0, rc.bottom - GetItemsBorderHeight()); dc.LineTo(rc.right, rc.bottom - GetItemsBorderHeight());
		dc.MoveTo(rcCaption.right, rc.top); dc.LineTo(rcCaption.right, rc.bottom - GetItemsBorderHeight());
	...
	{
		dc.FillSolidRect(pNext->GetIndent() * XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rc.bottom - GetItemsBorderHeight(), 
			rc.Width(), GetItemsBorderHeight(), m_clrCategorySeparator.IsDefaultValue() ? m_clrFace : m_clrCategorySeparator);
	}
...
void CXTPPropertyGridPaintManager::DrawExpandButton(CDC& dc, CXTPPropertyGridItem* pItem, CRect rcCaption)
{
	...
	if (m_buttonsStyle == xtpGridButtonsThemed && m_themeTree->IsAppThemeActive())
	{
	...
	else
	{
		...
		XTPPrimitiveSymbol nSymbol = (pItem->IsExpanded()
...
void CXTPPropertyGridPaintManager::FillInplaceButton(CDC* pDC, CXTPPropertyGridInplaceButton* pButton)
{
	...
	else if (m_buttonsStyle == xtpGridButtonsThemed && m_themeButton->IsAppThemeActive() && m_themeCombo->IsAppThemeActive())
	{
#ifdef _XTP_INCLUDE_CONTROLS
	...
	if (pButton->GetID() != XTP_ID_PROPERTYGRID_COMBOBUTTON)
	{
		...
		else if (!pButton->GetCaption().IsEmpty())
		{
			CXTPFontDC font(pDC, pButton->IsHyperlink() ? &m_pMetrics->m_xtpFontUnderline :
				(m_pGrid->IsHighlightChangedButtonItems() ? GetItemFont(pButton->GetItem(), TRUE) : &m_pMetrics->m_xtpFontNormal));
...
{
	if (m_buttonsStyle == xtpGridButtonsThemed && m_themeButton->IsAppThemeActive())
	{
#ifdef _XTP_INCLUDE_CONTROLS
		if (m_bUseThemedControls)
		{
...
void CXTPPropertyGridPaintManager::DrawCheckBox(CDC* pDC, CXTPPropertyGridItemBool* pItem)
{
	...
	CRect rcSample(rcValue.left + XTP_DPI_X(2), rcValue.CenterPoint().y - XTP_DPI_Y(6), 
		rcValue.left + XTP_DPI_X(2) + XTP_DPI_X(6) + XTP_DPI_X(7), rcValue.CenterPoint().y + XTP_DPI_Y(7));
...
{
	return AxCreateOleFont(m_xtpFontNormal.GetSafeHandle() ? &m_xtpFontNormal : &m_pPaintManager->GetItemMetrics()->m_xtpFontNormal,
