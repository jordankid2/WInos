...
void CXTPPropertyGridItemSize::OnAddChildItem()
{
	...
	if (!XTPResourceManager()->LoadString(&strWidth, XTP_IDS_PROPERTYGRID_WIDTH))
		strWidth = _T("Width");
	
	if (!XTPResourceManager()->LoadString(&strHeight, XTP_IDS_PROPERTYGRID_HEIGHT))
		strHeight = _T("Height");

	m_pItemWidth = (CXTPPropertyGridItemSizeWidth*)AddChildItem(new CXTPPropertyGridItemSizeWidth(strWidth));
	m_pItemHeight = (CXTPPropertyGridItemSizeHeight*)AddChildItem(new CXTPPropertyGridItemSizeHeight(strHeight));
