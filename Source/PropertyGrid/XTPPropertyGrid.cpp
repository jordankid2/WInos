...
const int TOOLBAR_HEIGHT = 25; // DPI height is currently not supported due to CToolBar limitiations.
...
CXTPPropertyGridItem* CXTPPropertyGrid::AddCategory(LPCTSTR strCaption, CXTPPropertyGridItem* pCategory)
{
	return GetGridView().AddCategory(strCaption, pCategory);
}

CXTPPropertyGridItem* CXTPPropertyGrid::InsertCategory(int nIndex, LPCTSTR strCaption, CXTPPropertyGridItem* pCategory)
{
...
void CXTPPropertyGrid::RecreateView()
{
	...
	DWORD dwListStyle = m_bVariableItemsHeight ? LBS_OWNERDRAWVARIABLE | LBS_NOINTEGRALHEIGHT : LBS_OWNERDRAWFIXED | LBS_NOINTEGRALHEIGHT;
	if (m_bMultiSelect)
	{
		dwListStyle |= LBS_MULTIPLESEL | LBS_EXTENDEDSEL;
	}
	...
	while (nUI != nUIElement)
	{
		...
		if (nUI == xtpGridUIInplaceButton)
		{
			...
			if (pItem && pButtons && pButtons->GetCount() > 0)
			{
				...
				for (;;)
				{
					if (bForward) nIndex++; else nIndex--;
...
int CXTPPropertyGrid::HitTest(CPoint pt)
{
	...
	if (m_bHelpVisible)
	{
		CRect rcSplitter(CPoint(rc.left, rc.bottom - m_pPaintManager->m_nSplitterHeight - m_pPaintManager->m_nHelpHeight), CSize(rc.Width(), m_pPaintManager->m_nSplitterHeight));
	...
	if (IsVerbsVisible())
	{
		CRect rcSplitter(CPoint(rc.left, rc.bottom - m_pPaintManager->m_nSplitterHeight - m_pPaintManager->m_nVerbsHeight), CSize(rc.Width(), m_pPaintManager->m_nSplitterHeight));
...
void CXTPPropertyGrid::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if ((nHitTest == xtpGridHitHelpSplitter) || (nHitTest == xtpGridHitVerbsSplitter))
	{
		...
		CRect rcAvail = bHelpHitTest ? CRect(0, XTP_DPI_Y(20) + XTP_DPI_Y(TOOLBAR_HEIGHT), rc.right, rc.bottom) :
			CRect(0, XTP_DPI_Y(20) + XTP_DPI_Y(TOOLBAR_HEIGHT), rc.right, rc.bottom - (IsHelpVisible() ? m_pPaintManager->m_nHelpHeight + m_pPaintManager->m_nSplitterHeight : 0));
...
void CXTPPropertyGrid::Reposition(int cx, int cy)
{
	...
	if (m_bHelpVisible)
	{
		if (cy > nMinViewHeight && cy - m_pPaintManager->m_nHelpHeight - m_pPaintManager->m_nSplitterHeight < nMinViewHeight && m_bVariableHelpHeight)
		{
			m_pPaintManager->m_nHelpHeight = max(0, cy - m_pPaintManager->m_nSplitterHeight - nMinViewHeight);
		}
	...
	if (!m_bViewVisible)
	{
		if (m_bVerbsVisible) m_pPaintManager->m_nVerbsHeight += rcView.Height() + m_pPaintManager->m_nSplitterHeight;
...
{
#if _MSC_VER < 1200
	m_wndToolbar.Create(this, TBSTYLE_TRANSPARENT | TBSTYLE_FLAT | WS_CHILD | WS_VISIBLE | WS_BORDER, 0);
#else
	m_wndToolbar.CreateEx(this, TBSTYLE_TRANSPARENT | TBSTYLE_FLAT, WS_BORDER | WS_CHILD | WS_VISIBLE | CBRS_ALIGN_TOP);
#endif

	UINT ResourceID = XTPDpiHelper()->SelectDpiSpecific(
		XTP_IDR_PROPERTYGRID_TOOLBAR_100, 
		XTP_IDR_PROPERTYGRID_TOOLBAR_125, 
		XTP_IDR_PROPERTYGRID_TOOLBAR_150, 
		XTP_IDR_PROPERTYGRID_TOOLBAR_200);
...
void CXTPPropertyGrid::RefreshToolBarButtons()
{
	if (m_bToolBarVisible)
	{
		m_wndToolbar.GetToolBarCtrl().SetState(XTP_IDC_PROPERTYGRID_CATEGORIZED, 
			TBSTATE_ENABLED | (GetGridView().m_properetySort == xtpGridSortCategorized ? TBSTATE_CHECKED : 0));
		m_wndToolbar.GetToolBarCtrl().SetState(XTP_IDC_PROPERTYGRID_ALPHABETIC, 
			TBSTATE_ENABLED | (GetGridView().m_properetySort == xtpGridSortAlphabetical ? TBSTATE_CHECKED : 0));
	}
}
...
CXTPPropertyGridItem* CXTPPropertyGrid::InsertCategory(int nIndex, int nID, CXTPPropertyGridItem* pCategory)
{
	CString strCaption;
	VERIFY(strCaption.LoadString(nID));
...
void CXTPPropertyGrid::SetTheme(XTPPropertyGridPaintTheme paintTheme)
{
    ...
    else if (paintTheme == xtpGridThemeOfficeXP)              SetCustomTheme(new CXTPPropertyGridOfficeXPTheme(this));
    else if (paintTheme == xtpGridThemeOffice2003)            SetCustomTheme(new CXTPPropertyGridOffice2003Theme(this));
    else if (paintTheme == xtpGridThemeOffice2007)            SetCustomTheme(new CXTPPropertyGridOffice2007Theme(this));
    else if (paintTheme == xtpGridThemeOffice2013)            SetCustomTheme(new CXTPPropertyGridOffice2013Theme(this));
    else if (paintTheme == xtpGridThemeVisualStudio2005)      SetCustomTheme(new CXTPPropertyGridVisualStudio2005Theme(this));
    else if (paintTheme == xtpGridThemeVisualStudio2010)      SetCustomTheme(new CXTPPropertyGridVisualStudio2010Theme(this));
    else if (paintTheme == xtpGridThemeVisualStudio2012Light) SetCustomTheme(new CXTPPropertyGridVisualStudio2012Theme(this, TRUE));
    else if (paintTheme == xtpGridThemeVisualStudio2012Dark)  SetCustomTheme(new CXTPPropertyGridVisualStudio2012Theme(this, FALSE));
    else if (paintTheme == xtpGridThemeVisualStudio2015)      SetCustomTheme(new CXTPPropertyGridVisualStudio2015Theme(this));
	else
	{
		ASSERT(FALSE);
	}
...
void CXTPPropertyGrid::SetCustomTheme(CXTPPropertyGridPaintManager* pPaintManager)
{
	...
	// recalc positions and size.
	Reposition();
	
	if (NULL != GetSafeHwnd())
	{
		// force non-client borders to be redrawn.
...
BOOL CXTPPropertyGrid::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	if (m_pMarkupContext)
	{
		...
		{
			if (XTPMarkupRelayMessage(pItem->GetMarkupDescription(), message, wParam, lParam, pResult))
				return TRUE;
		}
	}
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridVerbs, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPPropertyGridVerbs, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)
END_DISPATCH_MAP()
