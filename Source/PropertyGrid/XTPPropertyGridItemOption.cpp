...
void CXTPPropertyGridItemOption::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	lpMeasureItemStruct->itemHeight = (lpMeasureItemStruct->itemHeight - XTP_DPI_Y(3)) * nLinesCount + 4;
}
...
BOOL CXTPPropertyGridItemOption::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	rcValue.DeflateRect(0, XTP_DPI_Y(2), 0, XTP_DPI_Y(1));
...
BOOL CXTPPropertyGridItemOption::OnDrawItemValue(CDC& dc, CRect rcValue)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		CRect rcSample(rcItem.left - XTP_DPI_X(1), rcItem.CenterPoint().y - XTP_DPI_Y(7), 
			rcItem.left + XTP_DPI_X(12), rcItem.CenterPoint().y + XTP_DPI_Y(6));
		...
		CRect rcText(rcItem.left + XTP_DPI_X(14), rcItem.top, rcItem.right, rcItem.bottom);
		...
		{
			rcText.left--;
			rcText.right = rcText.left + dc.GetTextExtent(pConstraint->m_strConstraint).cx + XTP_DPI_X(2);
...
void CXTPPropertyGridItemOption::OleSetValue(const VARIANT* varValue)
{
	SetOption(VariantToLong(varValue, 0));
}

const VARIANT CXTPPropertyGridItemOption::OleGetValue()
{
	return CComVariant(m_nValue);
}

void CXTPPropertyGridItemOption::BindDispatch()
{
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItemOption, CXTPPropertyGridItem)
	DISP_PROPERTY_ID(CXTPPropertyGridItemOption, "CheckBoxStyle", 50, m_bCheckBoxStyle,VT_BOOL)
END_DISPATCH_MAP()

BEGIN_INTERFACE_MAP(CXTPPropertyGridItemOption, CXTPPropertyGridItem)
	INTERFACE_PART(CXTPPropertyGridItemOption, XTPDIID_PropertyGridItemOption, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridItemOption, XTPDIID_PropertyGridItemOption)
