// XTPPropertyGridInplaceEdit.cpp : implementation of the CXTPPropertyGridInplaceEdit class.
//
...
void CXTPPropertyGridInplaceEdit::Create(CXTPPropertyGridItem* pItem, CRect rect)
{
	...
	SetMargins(XTP_DPI_X(3), XTP_DPI_Y(3));
...
HBRUSH CXTPPropertyGridInplaceEdit::CtlColor(CDC* pDC, UINT /*nCtlColor*/)
{
	...
	if (clr != m_clrBack || !m_xtpBrushBack.GetSafeHandle())
	{
		m_xtpBrushBack.DeleteObject();
		m_xtpBrushBack.CreateSolidBrush(clr);
		m_clrBack = clr;
...
int CXTPPropertyGridInplaceSlider::CalcualteEditWidth()
{
	...
	int nWidth = max(dc.GetTextExtent(strMax).cx, dc.GetTextExtent(strMin).cx) + XTP_DPI_X(6);
...
BEGIN_INTERFACE_MAP(CXTPPropertyGridInplaceSlider, CCmdTarget)
	INTERFACE_PART(CXTPPropertyGridInplaceSlider, XTPDIID_IPropertyGridInplaceSliderControl, Dispatch)
	XTP_DECLARE_CMDTARGETPROVIDER_INTERFACE_PART(CXTPPropertyGridInplaceSlider)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridInplaceSlider, XTPDIID_IPropertyGridInplaceSliderControl)
XTP_IMPLEMENT_CMDTARGETPROVIDER_INTERFACE(CXTPPropertyGridInplaceSlider)
...
BEGIN_INTERFACE_MAP(CXTPPropertyGridInplaceSpinButton, CCmdTarget)
	INTERFACE_PART(CXTPPropertyGridInplaceSpinButton, XTPDIID_IPropertyGridInplaceSpinButton, Dispatch)
	XTP_DECLARE_CMDTARGETPROVIDER_INTERFACE_PART(CXTPPropertyGridInplaceSpinButton)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPPropertyGridInplaceSpinButton, XTPDIID_IPropertyGridInplaceSpinButton)
XTP_IMPLEMENT_CMDTARGETPROVIDER_INTERFACE(CXTPPropertyGridInplaceSpinButton)
