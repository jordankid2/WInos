...
void CXTPPropertyGridItemColor::OnInplaceButtonDown(CXTPPropertyGridInplaceButton* pButton)
{
	...
	if (m_colorEditor == xtpGridItemColorPopup)
	{
		...
		pColorPopup->Create(rcItem, m_pGrid,
			CPS_XTP_RIGHTALIGN | CPS_XTP_EXTENDED | CPS_XTP_MORECOLORS | CPS_XTP_RECENTCOLORS |
			CPS_XTP_SHOWEYEDROPPER | CPS_XTP_SHOWHEXVALUE, GetColor(), clrDefault);
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItemColor, CXTPPropertyGridItem)
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItemColor, "EditorStyle", 50, GetEditorStyle, SetEditorStyle, VT_I4)
END_DISPATCH_MAP()
