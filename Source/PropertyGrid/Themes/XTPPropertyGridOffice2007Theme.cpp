	...
	if (m_buttonsStyle == xtpGridButtonsThemed)
	{
		...
		else
		{
			CRect rcSign(CPoint(XTP_DPI_X(XTP_PGI_EXPAND_BORDER) / 2 - XTP_DPI_X(8), 
				rcCaption.CenterPoint().y - XTP_DPI_Y(8)), XTP_DPI(CSize(16, 16)));
...
void CXTPPropertyGridOffice2007Theme::FillInplaceButton(CDC* pDC, CXTPPropertyGridInplaceButton* pButton)
{
	...
	else
	{
		...
		else
		{
			pDC->FillSolidRect(rect, pButton->IsPressed() && pButton->IsHot() 
				? RGB(255, 171, 63) : pButton->IsHot() ? RGB(255, 231, 162) : m_clrFace);

			COLORREF clrBorder = GetXtremeColor(pButton->IsPressed() && pButton->IsHot() 
				? RGB(251, 140, 60) : pButton->IsHot() ? RGB(255, 189, 105) : m_clrShadow);
	...
	if (pButton->GetID() != XTP_ID_PROPERTYGRID_COMBOBUTTON)
	{
		...
		{
			CXTPFontDC font(pDC, pButton->IsHyperlink() ? &m_pMetrics->m_xtpFontUnderline : GetItemFont(pButton->GetItem(), TRUE));
