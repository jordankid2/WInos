...
void CXTPPropertyGridCoolTheme::DrawItem(PDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if (pItem->IsCategory())
	{
		...
		CRect rcText(nIndent + XTP_DPI_X(3), rc.top , rc.right, rc.bottom - GetItemsBorderHeight());
		...
		dc.FillSolidRect((pItem->GetIndent() + (pItem->IsExpanded() ? 1 : 0)) * XTP_DPI_X(XTP_PGI_EXPAND_BORDER) - 1, 
			rc.bottom - GetItemsBorderHeight(), rc.Width(), GetItemsBorderHeight(), clrShadow);
		...
		while (pCategory != NULL)
		{
			...
			{
				dc.FillSolidRect(rc.left + pCategory->GetIndent() * XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rc.top, XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rc.Height(), clrLine);
				dc.FillSolidRect(rc.left + (pCategory->GetIndent() + 1) * XTP_DPI_X(XTP_PGI_EXPAND_BORDER) - 1, rc.top, 1, rc.Height() + 2, clrShadow);
			}
	...
	else
	{
		...
		BOOL bFocused = bSelected && CWnd::GetFocus() && ((CWnd::GetFocus() == pView) 
			|| (CWnd::GetFocus()->GetParent() == pView) || (CWnd::GetFocus()->GetOwner() == pView));
		if (bFocused)
		{
			dc.SetTextColor(m_clrHighlightText);
		...
		dc.MoveTo(0, rc.bottom - GetItemsBorderHeight());
		dc.LineTo(rc.right, rc.bottom - GetItemsBorderHeight());
		dc.MoveTo(rcCaption.right, rc.top);
		dc.LineTo(rcCaption.right, rc.bottom - GetItemsBorderHeight());
	...
	{
		dc.FillSolidRect(0, rc.bottom - GetItemsBorderHeight(), rc.Width(), GetItemsBorderHeight(), clrShadow);
	}
	else if (pNext && pNext->IsCategory() && pNext->IsVisible())
	{
		dc.FillSolidRect(pNext->GetIndent() * XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rc.bottom - GetItemsBorderHeight(), rc.Width(), GetItemsBorderHeight(), clrShadow);
	}
	...
	while (pCategory != NULL)
	{
		...
		{
			dc.FillSolidRect(pCategory->GetIndent() * XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rc.bottom - GetItemsBorderHeight(), rc.Width(), GetItemsBorderHeight(), clrShadow);
		}
		else
			break;
		pPrev = pCategory;
