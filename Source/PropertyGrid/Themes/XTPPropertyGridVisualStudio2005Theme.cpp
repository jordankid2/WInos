...
void CXTPPropertyGridVisualStudio2005Theme::DrawCategoryCaptionBackground(CDC* pDC, CXTPPropertyGridItem* /*pItem*/, CRect rc)
{
	...
	XTPDrawHelpers()->GradientFill(pDC, CRect(rc.left, rc.bottom - XTP_DPI_Y(2), 
		rc.right, rc.bottom - XTP_DPI_Y(1)), 
...
void CXTPPropertyGridVisualStudio2005Theme::DrawExpandButton(CDC& dc, CXTPPropertyGridItem* pItem, CRect rcCaption)
{
	...
	CRect rcButton(CPoint(XTP_DPI_X(XTP_PGI_EXPAND_BORDER) / 2 - XTP_DPI_X(5), 
		rcCaption.CenterPoint().y - XTP_DPI_Y(4)), XTP_DPI(CSize(9, 9)));

	if (pItem->GetIndent() > 0)
		rcButton.OffsetRect((pItem->GetIndent() - (pItem->IsCategory() ? 0 : 1)) * XTP_DPI_X(XTP_PGI_EXPAND_BORDER), 0);
	...
	pDC->FillSolidRect(rcButton.left + n1pxX, rcButton.top + n1pxY, 
		n9pxX - n1pxX*2, n9pxY - n1pxY*2, clrButton);
