...
void CXTPPropertyGridSimpleTheme::DrawItem(PDRAWITEMSTRUCT lpDrawItemStruct)
{
...

	{
		...
		if (bFocused)
		{
			...
			dc.FillSolidRect(CRect(XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rcCaption.top, rcCaption.right, rcCaption.bottom), m_clrHighlight);
		}
		else if (bSelected && !m_pGrid->m_bHideSelection)
		{
			dc.FillSolidRect(CRect(XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rcCaption.top, rcCaption.right, rcCaption.bottom), clrLine);
		}
		dc.FillSolidRect(XTP_DPI_X(XTP_PGI_EXPAND_BORDER) - 1, rc.top, 1, rc.Height(), clrLine);

		CXTPPenDC pen(dc, clrLine);
		dc.MoveTo(XTP_DPI_X(XTP_PGI_EXPAND_BORDER), rc.bottom - GetItemsBorderHeight());
	...
	if (pView->GetCount() == (int)lpDrawItemStruct->itemID + 1)
		dc.FillSolidRect(0, rc.bottom - GetItemsBorderHeight(), rc.Width(), GetItemsBorderHeight(), clrLine);
