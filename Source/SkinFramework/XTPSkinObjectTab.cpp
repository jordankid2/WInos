...
void CXTPSkinObjectTab::DrawTabIcon(CDC* pDC, CRect& rcItem, int iItem)
{
	...
	if (IsHorz(dwStyle))
	{
		point.Offset(GetMetrics()->m_cxEdge * 2, IsTop(dwStyle) ? GetMetrics()->m_cyEdge : XTP_DPI_Y(1));
	}
	else
	{
		point.Offset(IsLeft(dwStyle) ? GetMetrics()->m_cxEdge : XTP_DPI_X(1), GetMetrics()->m_cyEdge * 2);
	}
...
void CXTPSkinObjectTab::DrawTabText(CDC* pDC, CRect& rcItem, int iItem, 
	int iCount, BOOL bSelected, BOOL bFocused)
{
	CTabCtrl* pTabCtrl = (CTabCtrl*)this;
