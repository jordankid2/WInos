...
HWND AFX_CDECL CXTPSkinObjectComboBox::GetComboListBox(HWND hWnd)
{
	...
	if (modUser32Dll.GetModuleHandle(_T("USER32.DLL")))
	{
		...
		PFNGETCOMBOBOXINFO pfnGetComboBoxInfo = (PFNGETCOMBOBOXINFO)::GetProcAddress(modUser32Dll, "GetComboBoxInfo");
...
void CXTPSkinObjectComboBox::OnDraw(CDC* pDC)
{
	...
	if (bRightAlign)
	{
		::FillRect(memDC.GetSafeHdc(), CRect(rc.left, rc.top, rc.left
			+ max(GetMetrics()->m_cxVScroll + XTP_DPI_X(1), GetMetrics()->m_cxOsVScroll + XTP_DPI_X(3)), rc.bottom), hBrush);
	}
	else
	{
		::FillRect(memDC.GetSafeHdc(), CRect(rc.right
			- max(GetMetrics()->m_cxVScroll + XTP_DPI_X(1), GetMetrics()->m_cxOsVScroll + XTP_DPI_X(3)), rc.top, rc.right, rc.bottom), hBrush);
	}
...
void CXTPSkinObjectDateTime::OnDraw(CDC* pDC)
{
	...
	if (!bVistaStyle)
	{
		memDC.FillSolidRect(CRect(rc.right - nThumbWidth - XTP_DPI_X(3), rc.top, rc.right, rc.bottom), clr);
	}
