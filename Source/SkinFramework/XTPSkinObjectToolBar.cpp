...
BOOL CXTPSkinObjectToolBar::IsSpecialSystemToolbar()
{
	if (m_nSpecialToolbar == -1)
	{
		...
		{
			if (_tcsicmp(lpszClassName, _T("Breadcrumb Parent")) == 0 || _tcsicmp(lpszClassName, _T("TravelBand")) == 0 ||
				_tcsicmp(lpszClassName, _T("msctls_progress32")) == 0 || _tcsicmp(lpszClassName, _T("Search Control")) == 0)
			{
				m_nSpecialToolbar = 1;
			}
		}
...
void CXTPSkinObjectToolBar::DrawButtonImage(CDC* pDC, int x, int y, int nIndex)
{
	...
	if (himl && (iImage != -1))
	{
		...
		else if (bMonoBitmap)
		{
			...
			BitBlt(pDC->GetSafeHdc(), x + XTP_DPI_X(1), y + XTP_DPI_Y(1), iDxBitmap, iDyBitmap, dcMono, 0, 0, PSDPxax);
...
void CXTPSkinObjectToolBar::DrawButton(CDC* pDC, int nIndex)
{
	...
	if (!(dwCustom & CDRF_SKIPDEFAULT))
	{
		...
		if (!strText.IsEmpty())
		{
			...
			if (dxText > 0)
			{
				CRect rcText(x + XTP_DPI_X(1), y + XTP_DPI_Y(1), 
					x + XTP_DPI_X(1) + dxText, y + XTP_DPI_Y(1) + dyText);
		...
		if (!bSplit && HasDropDownArrow(&tbb))
		{
			...
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayDown,
				rcGlyph, GetColor(COLOR_BTNTEXT));
		}
	}
...
{
	return static_cast<BOOL>(::SendMessage(m_hWnd, TB_GETBUTTON, nIndex, reinterpret_cast<LPARAM>(lpButton)));
}

BOOL CXTPSkinObjectToolBar::GetToolbarItemRect(int nIndex, LPRECT lpRect) const
{
	return static_cast<BOOL>(::SendMessage(m_hWnd, TB_GETITEMRECT, nIndex, reinterpret_cast<LPARAM>(lpRect)));
}
