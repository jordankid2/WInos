...
#include "SkinFramework/XTPSkinObjectMDI.h"
...
IMPLEMENT_DYNAMIC(CXTPSkinObject, CWnd);
...
CString AFX_CDECL CXTPSkinObject::GetWndClassTreePath(HWND hWnd, BOOL bPathOnly /*= FALSE*/)
{
	ASSERT(NULL != hWnd);
...
BOOL CXTPSkinObject::OnHookDefWindowProc(UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult)
{
	...
	{
		OnSkinChanged(static_cast<BOOL>(wParam), static_cast<BOOL>(lParam));
		lResult = 0;
		return TRUE;
	}
