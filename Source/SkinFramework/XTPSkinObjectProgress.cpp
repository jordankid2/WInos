...
void CXTPSkinObjectProgress::OnDraw(CDC* pDC)
{
	...
	CRect rcMargins = XTP_DPI(pClassProgress->GetThemeRect(nBarPart, 0,  TMT_SIZINGMARGINS));
	...
	if (GetStyle() & PBS_MARQUEE)
	{
		...
		dxBlock = XTP_DPI_X(pClassProgress->GetThemeInt(0, 0, TMT_PROGRESSCHUNKSIZE, XTP_UNDPI_X(rc.Height() * 2 / 3)));
		int dxSpace = XTP_DPI_X(pClassProgress->GetThemeInt(0, 0, TMT_PROGRESSSPACESIZE, 2));

		rc.left = m_nMarqueePos + rcClient.left;

		if (dxBlock == XTP_UNDPI_X(1) && dxSpace == 0)
		{
			dxBlock = XTP_DPI_X(8);
	...
	dxBlock = XTP_DPI_X(pClassProgress->GetThemeInt(0, 0, TMT_PROGRESSCHUNKSIZE, XTP_UNDPI_X(dxBlock)));
	int dxSpace = XTP_DPI_X(pClassProgress->GetThemeInt(0, 0, TMT_PROGRESSSPACESIZE, 2));
...
void CXTPSkinObjectProgress::DoStep(int nStep)
{
	...
	DWORD dwStyle = GetStyle();
	BOOL bVert = dwStyle & PBS_VERTICAL;
	CRect rcMargins = XTP_DPI(pClassProgress->GetThemeRect(bVert ? PP_BARVERT : PP_BAR, 0,  TMT_SIZINGMARGINS));

	rcClient.DeflateRect(rcMargins);

	int dxBlock = XTP_DPI_X(pClassProgress->GetThemeInt(0, 0, TMT_PROGRESSCHUNKSIZE, 
		XTP_UNDPI_X(rcClient.Height() * 2 / 3)));
	int dxSpace = XTP_DPI_X(pClassProgress->GetThemeInt(0, 0, TMT_PROGRESSSPACESIZE, 2));
