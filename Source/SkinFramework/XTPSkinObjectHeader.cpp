...
void CXTPSkinObjectHeader::DrawItemEntry(CDC* pDC, int nIndex, CRect rcItem, int nState)
{
	...
	if (pImageList && (hdi.fmt & HDF_IMAGE) && hdi.iImage >= 0 && hdi.iImage < pImageList->GetImageCount())
	{
		...
		cxBitmap = XTP_DPI_X(cxBitmap);
		cyBitmap = XTP_DPI_Y(cxBitmap);
		...
		if (hdi.fmt & HDF_BITMAP_ON_RIGHT)
		{
			...
			CRect rcImage(pt, CSize(cxBitmap, cyBitmap));
			CXTPDrawHelpers::StretchImageListImage(pDC, rcImage, pImageList, hdi.iImage);
			rcText.right = pt.x + XTP_DPI_X(6);
		}
		else
		{
			if (nState == HIS_PRESSED)
	...
	if (((hdi.fmt & HDF_IMAGE) == 0) && ((hdi.fmt & HDF_SORTUP) || (hdi.fmt & HDF_SORTDOWN)))
	{
		...
		CPoint pt(rcItem.left + iBitmapMargin, (rcItem.bottom + rcItem.top - XTP_DPI_Y(2)) / 2);
		...
		CRect rcGlyph(pt.x - szGlyph.cx / 2, pt.y - szGlyph.cy / 2,
			pt.x + szGlyph.cx / 2, pt.y + szGlyph.cy / 2);

		if (hdi.fmt & HDF_SORTUP)
		{
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayUp,
				rcGlyph, GetColor(COLOR_3DSHADOW));
		}
		else
		{
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayDown,
				rcGlyph, GetColor(COLOR_3DSHADOW));
		}
	...
	case HDF_LEFT:
		nFormat |= DT_LEFT;
		rcText.DeflateRect(XTP_DPI_X(9), 0, XTP_DPI_X(6), 0);
		break;

	case HDF_CENTER:
		nFormat |= DT_CENTER;
		rcText.DeflateRect(XTP_DPI_X(6), 0, XTP_DPI_X(6), 0);
		break;

	case HDF_RIGHT:
		nFormat |= DT_RIGHT;
		rcText.DeflateRect(XTP_DPI_X(6), 0, XTP_DPI_X(9), 0);
		break;
	}
