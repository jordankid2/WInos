...
CXTPSkinManagerModuleList::CModuleEnumerator::CModuleEnumerator(
	DWORD dwProcessId, BOOL bUseCache)
	...
	, m_nReferenceLockCounter(0)
{
	...
	CXTPSkinManagerApiFunction* pFunction = pApiHook->GetHookedFunction(
#ifdef _UNICODE
	...
	pFunction = pApiHook->GetHookedFunction(xtpSkinApiFreeLibrary_KERNEL32);
...
BOOL CXTPSkinManagerModuleList::CModuleEnumerator::GetModuleInformation(HMODULE hModule, XTP_MODULEINFO* lpModInfo)
{
...
class CXTPSkinManagerModuleList::CPsapiModuleEnumerator : public CModuleEnumerator
{
	...
	typedef BOOL (WINAPI* PFNGETMODULEINFORMATION) (HANDLE hProcess, HMODULE hModule, XTP_MODULEINFO* lpModInfo, DWORD cb);
...
CXTPSkinManagerModuleList::CPsapiModuleEnumerator::CPsapiModuleEnumerator(DWORD dwProcessId, BOOL bUseCache, HMODULE hModPSAPI)
...
void CXTPSkinManagerModuleList::CPsapiModuleEnumerator::CollectModules(CArray<XTP_MODULEINFO, const XTP_MODULEINFO&>& modules)
{
	modules.RemoveAll();
	...
	if (NULL != m_pfnEnumProcessModules)
	{
		...
		if (NULL != hProcess)
		{
			...
			{
				int nCount = cbNeeded / sizeof(HMODULE);
				ASSERT(1 <= nCount);
				...
				if (m_pfnEnumProcessModules(hProcess, &handles[0], cbNeeded, &cbNeeded))
				{
					for (INT_PTR i = 0; i < handles.GetSize(); ++i)
					{
						...
						if (m_pfnGetModuleInformation(hProcess, handles[i], &info, XTP_WIN32SDK_MODULEINFO_SIZE))
						{
...
BOOL CXTPSkinManagerModuleList::CPsapiModuleEnumerator::GetModuleInformation(HMODULE hModule, XTP_MODULEINFO* lpModInfo)
{
	ASSERT(NULL != lpModInfo);
	...
	if (NULL != m_pfnGetModuleInformation
		...
		&& NULL != lpModInfo)
	{
		HANDLE hProcess = ::OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, this->GetProcessId());
		if (NULL != hProcess)
		{
			bResult = m_pfnGetModuleInformation(hProcess, hModule, lpModInfo, XTP_WIN32SDK_MODULEINFO_SIZE);
			if (bResult)
			{
...
CXTPSkinManagerModuleList::CToolHelpModuleEnumerator::CToolHelpModuleEnumerator(DWORD dwProcessId, BOOL bUseCache)
	: CModuleEnumerator(dwProcessId, bUseCache)
{
}

void CXTPSkinManagerModuleList::CToolHelpModuleEnumerator::CollectModules(CArray<XTP_MODULEINFO, const XTP_MODULEINFO&>& modules)
{
	modules.RemoveAll();
