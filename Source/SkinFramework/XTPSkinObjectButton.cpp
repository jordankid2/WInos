...
void CXTPSkinObjectButton::CalcRect(CDC* pDC, LPRECT lprc, int code)
{
	...
	case CBR_GROUPTEXT:
		...
		case BS_CENTER:
			lprc->left += GetMetrics()->m_cxBorder + (lprc->left + lprc->right - extent.cx) / 2;
...
void CXTPSkinObjectButton::DrawButtonText(CDC* pDC, int nPart, int nState)
{
	...
	else if (bStyle !=  LOBYTE(BS_USERBUTTON))
	{
		...
		if (GetStyle() & BS_MULTILINE)
		{
			pClass->DrawThemeText(pDC, nPart, nState, strText, dsFlags | DT_NOCLIP, CRect(x, y, x + cx, y + cy));
		}
		else
		{
...
void CXTPSkinObjectButton::OnDraw(CDC* pDC, CRect rc)
{
	...
	if (IsPushButton())
	{
		nState = !bEnabled? PBS_DISABLED: bPressed? PBS_PRESSED: bHot? PBS_HOT: bChecked ? PBS_PRESSED : bDefault? PBS_DEFAULTED: PBS_NORMAL;
		nPart = BP_PUSHBUTTON;
		...
		if (bStyle == BS_SPLITBUTTON || bStyle == BS_DEFSPLITBUTTON)
		{
			...
			pDC->FillSolidRect(rcItem.left, rcItem.top, XTP_DPI_X(1), rcItem.Height(), GetColor(!bEnabled ? COLOR_GRAYTEXT : COLOR_BTNSHADOW));
			pDC->FillSolidRect(rcItem.left + XTP_DPI_X(1), rcItem.top, XTP_DPI_X(1), rcItem.Height(), GetColor(COLOR_BTNHIGHLIGHT));
		}
	}
	else if (bStyle == BS_RADIOBUTTON || bStyle == BS_AUTORADIOBUTTON)
	{
		nState = !bEnabled? RBS_UNCHECKEDDISABLED: bPressed? RBS_UNCHECKEDPRESSED: bHot? RBS_UNCHECKEDHOT: RBS_UNCHECKEDNORMAL;
