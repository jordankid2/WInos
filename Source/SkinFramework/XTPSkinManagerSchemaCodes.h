...
static const struct TMS_SCHEMA_ENTRY gSchemaClassAndPartsCodes[] =
{
	// CJ
	_T("Globals"),          1,
	_T("Documentation"),    2,
...
static const struct TMS_SCHEMA_ENTRY gSchemaEnumCodes[] =
{
...
static const struct TMS_SCHEMA_ENTRY gSchemaStateCodes[] =
{
	_T("FRAME_ACTIVE"),     1,
	_T("FRAME_INACTIVE"),   2,
...
#ifdef UNICODE
#define TM_PROP(val, prefix, name, primval) L"P_" L#name, PROPERTY_N(val, XTP_SKINPROPERTY_##primval),
#else
#define TM_PROP(val, prefix, name, primval)   "P_" #name, PROPERTY_N(val, XTP_SKINPROPERTY_##primval),
#endif

static const struct TMS_SCHEMA_ENTRY gSchemaPropertyCodes[] =
{
...
class CXTPSkinManagerSchemaCodeManager : public CXTPSingleton<CXTPSkinManagerSchemaCodeManager>
{
	friend CXTPSingleton<CXTPSkinManagerSchemaCodeManager>;

	CMap<CString, LPCTSTR, int, int> m_cache;
	CMap<CString, LPCTSTR, int, int> m_cacheIgnored;

	CXTPSkinManagerSchemaCodeManager()
	{
		CacheSchemaCodes(gSchemaClassAndPartsCodes);
		CacheSchemaCodes(gSchemaEnumCodes);
		CacheSchemaCodes(gSchemaStateCodes);
		CacheSchemaCodes(gSchemaPropertyCodes);
	}

	void CacheSchemaCodes(const struct TMS_SCHEMA_ENTRY* arSchemaArray)
	{
		int k = 0;
		int nValue = -1;
		LPCTSTR name = arSchemaArray[k].str;

		while(name)
		{
			ASSERT(!m_cache.Lookup(name, nValue));	// keys should be unique
			UNUSED(nValue);

			m_cache[arSchemaArray[k].str] = arSchemaArray[k].n;
			name = arSchemaArray[++k].str;
		}
	}
	
	static void AFX_CDECL CacheIgnored(TCHAR* szHashName)
	{
		ASSERT(szHashName);
		int nValue = -1;
		if (Instance().m_cacheIgnored.GetCount() < 500 &&
			!Instance().m_cacheIgnored.Lookup(szHashName, nValue))
		{
			Instance().m_cacheIgnored[szHashName] = -1;
		}
	}

public:
	static int AFX_CDECL MapKey(const TMS_SCHEMA_ENTRY *pArray, LPCTSTR key, LPCTSTR prefix = NULL)
	{
		ASSERT(NULL != pArray);
		ASSERT(NULL != key);
		...
		{
			_tcscpy(szHashName, key);
		}

		::EnterCriticalSection(&GetInstanceAccess());

		if (!Instance().m_cacheIgnored.Lookup(szHashName, nValue) &&
			!Instance().m_cache.Lookup(szHashName, nValue))
		{
			int i = 0;
			LPCTSTR name = pArray[i].str;
			while (name)
			{
				if (_tcsicmp(name, szHashName) == 0)
				{
					Instance().m_cache[szHashName] = pArray[i].n;
					nValue = pArray[i].n;
					break;
				}
			...
			if (nValue == -1)
				CacheIgnored(szHashName);
		}

		::LeaveCriticalSection(&GetInstanceAccess());

		return nValue;
	}

	static int AFX_CDECL MapKey2(const TMS_SCHEMA_ENTRY *pArray, LPCTSTR key, LPCTSTR key2)
	{
		ASSERT(NULL != key);
		ASSERT(NULL != key2);
		...
		return MapKey(pArray, szContencanated);
	}
};

#pragma warning(default: 4996)

#define GET_CLASS_CODE(cls)             CXTPSkinManagerSchemaCodeManager::MapKey(gSchemaClassAndPartsCodes, cls)
#define GET_ENUM_CODE(prop, val)        CXTPSkinManagerSchemaCodeManager::MapKey2(gSchemaEnumCodes, prop, val)
#define GET_CLASSPART_CODE(cls, part)   CXTPSkinManagerSchemaCodeManager::MapKey2(gSchemaClassAndPartsCodes, cls, part)
#define GET_STATE_CODE(part, state)     CXTPSkinManagerSchemaCodeManager::MapKey2(gSchemaStateCodes, part, state)
#define GET_PROPERTY_CODE(prop)         CXTPSkinManagerSchemaCodeManager::MapKey(gSchemaPropertyCodes, prop, _T("P_"))
