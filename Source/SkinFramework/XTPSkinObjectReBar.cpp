	...
	if(0 != (dwStyle & RBS_BANDBORDERS) && 0 < nBands)
	{
		...
		for(nBand = 1; nBand < nBands; ++nBand)
		{
				...
				&& 0 != SendMessage(RB_GETRECT, nBand - 1, reinterpret_cast<LPARAM>(&rcPrevBand)))
			{
				...
				pDC->Draw3dRect(
					nBorderX - XTP_DPI_X(1), rcBand.top - nVertSpacing / 2,
					XTP_DPI_X(2), rcBand.Height() + nVertSpacing,
					clrBorderColorDark,
					clrBorderColorLight);
				...
				if(rcPrevBand.bottom < rcBand.top)
				{
					...
					pDC->Draw3dRect(
						0, nBorderY - XTP_DPI_Y(1),
						rcClient.right, XTP_DPI_Y(2),
						clrBorderColorDark,
						clrBorderColorLight);
				}
			}
			...
			if(NULL != barInfo.himl)
			{
				...
				if(ImageList_GetIconSize(barInfo.himl, &cx, &cy))
				{
					...
					CXTPDrawHelpers::StretchImageListImage(pDC, rcImage,
						CImageList::FromHandle(barInfo.himl), rbbi.iImage);
				}
			}
		}
	}
