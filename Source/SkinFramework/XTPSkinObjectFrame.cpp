...
BOOL CXTPSkinObjectFrame::OnCtlColor(UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult,
	BOOL bConsiderDlgTexture /*= FALSE*/)
{
		...
		|| WM_CTLCOLORLISTBOX == nMessage)
	{
		do
		{
			...
			if (bConsiderDlgTexture && IsDefWindowProcAvail(nMessage)
			...
			if (nMessage == WM_CTLCOLOREDIT || nMessage == WM_CTLCOLORLISTBOX)
			{
				XTP_GUARD_SHARED_(CXTPSkinManager, XTPSkinManager(), pSkinManager)
				{
					...
					if (NULL != pSkinObject)
					{
						...
						lResult = reinterpret_cast<LRESULT>(pSkinObject->GetMetrics()->m_xtpBrushWindow.GetSafeHandle());
						bHandled = TRUE;
						break;
					}
				}
			...
			if (CWnd::GrayCtlColor(hCtlDC, hCtlWnd, nCtlColorMsgId,
				GetMetrics()->m_xtpBrushDialog, 0))
			{
...
BOOL CXTPSkinObjectFrame::OnHookMessage(UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult)
{
	...
	return !bHandled 
		? CXTPSkinObject::OnHookMessage(nMessage, wParam, lParam, lResult)
		: TRUE;
}
...
void CXTPSkinObjectFrame::OnPaint()
{
	...
	if (m_spi[SB_VERT].fVisible)
	{
		...
		if (XTPDrawHelpers()->IsVScrollOnTheLeft(this))
		{
			rcScroll.SetRect(rcWnd.left, rcWnd.top,
				rcWnd.left + nWidth, rcWnd.bottom);
		}
		else
		{
			rcScroll.SetRect(rcWnd.right - nWidth,
				rcWnd.top, rcWnd.right, rcWnd.bottom);
		}
	...
	if (m_spi[SB_HORZ].fVisible)
	{
		CXTPWindowRect rcWnd(this);
		ScreenToClient(&rcWnd);
		CRect rcScroll(rcWnd.right, rcWnd.bottom - GetMetrics()->m_cyHScroll,
...
HBRUSH CXTPSkinObjectFrame::GetFillBackgroundBrush(CDC* pDC, int nMessage)
{
	...
	return nMessage == WM_CTLCOLOREDIT || nMessage == WM_CTLCOLORLISTBOX ?
		GetMetrics()->m_xtpBrushWindow : GetMetrics()->m_brTheme[COLOR_3DFACE];
}
...
void CXTPSkinObjectFrame::CalcScrollBarInfo(LPRECT lprc, XTP_SKINSCROLLBAR_POSINFO *pSBInfo, SCROLLINFO* pSI)
{
	int cpx;
	DWORD dwRange;
	int denom;
