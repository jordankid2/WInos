...
BOOL CXTPSkinManagerSchema::DrawThemeBackgroundBorder(CDC* pDC, CXTPSkinManagerClass* pClass, int iPartId, int iStateId, const RECT *pRect)
{
	...
	if ((nBorderSize > 0) && (clrBorderColor != COLORREF_NULL))
	{
		...
		pDC->FillSolidRect(rc.left, rc.bottom - XTP_DPI_Y(nBorderSize), 
			rc.Width(), XTP_DPI_Y(nBorderSize), clrBorderColor);

		pDC->FillSolidRect(rc.left, rc.top + XTP_DPI_Y(nBorderSize), 
			XTP_DPI_X(nBorderSize), rc.Height() - XTP_DPI_Y(nBorderSize) * 2, clrBorderColor);
		pDC->FillSolidRect(rc.right - XTP_DPI_X(nBorderSize), rc.top + XTP_DPI_Y(nBorderSize),
			XTP_DPI_Y(nBorderSize), rc.Height() - XTP_DPI_Y(nBorderSize) * 2, clrBorderColor);
...
int CXTPSkinManagerSchema::DrawThemeFrameButtons(CDC* pDC, CXTPSkinObjectFrame* pFrame)
{
	...
	CRect rcButton = CRect(CPoint(rc.right - szButton.cx - cBorders - XTP_DPI_X(2), 
		rc.top + nTop + (nCaptionHeight - szButton.cy)/2), szButton);
...
void CXTPSkinManagerSchema::DrawNonClientRect(CDC* pDC, CRect rcFrame, CXTPSkinObjectFrame* pFrame)
{
	...
	{
		int cBorders = GetWindowBorders(dwStyle, dwExStyle, TRUE);
...
void CXTPSkinManagerSchema::DrawThemeFrame(CDC* pDC, CXTPSkinObjectFrame* pFrame)
{
	XTP_GUARD_SHARED(CXTPSkinObjectFrame, pFrame)
	{
		...
		if (pFrame->HasWindowBorder()
			|| (dwExStyle & (WS_EX_CLIENTEDGE | WS_EX_WINDOWEDGE | WS_EX_STATICEDGE | WS_EX_DLGMODALFRAME)))
		{
			CXTPWindowRect rc(pFrame);
			rc.OffsetRect(-rc.TopLeft());
			...
			if (pFrame->HasCaption())
			{
				...
				if (dwExStyle & WS_EX_CLIENTEDGE)
				{
					DrawClientEdge(pDC, CRect(rcBorders.left - XTP_DPI_X(2),
						rcBorders.top - XTP_DPI_Y(2),
						rc.Width() - rcBorders.right + XTP_DPI_X(2),
						rc.Height() - rcBorders.bottom + XTP_DPI_Y(2)), pFrame);
				...
				XTP_GUARD_SHARED(CXTPSkinManagerClass, pClassWindow)
				{
					...
					int cBorders = GetWindowBorders(dwStyle, dwExStyle, TRUE) - GetMetrics()->m_cyBorder;
					...
					CFont* pOldFont = dc.SelectObject(bToolWindow ? &GetMetrics()->m_xtpFontSmCaption : &GetMetrics()->m_xtpFontCaption);
...
CRect CXTPSkinManagerSchema::CalcFrameBorders(DWORD dwStyle, DWORD dwExStyle, CXTPSkinObjectFrame* pFrame)
{
...
BOOL CXTPSkinManagerSchema::DrawWindowPart(CXTPSkinObjectFrame* pFrame, CBitmap& bmp, int iPartId, BOOL bVertical, int nBorderHeight, CRect& rcDest, CRect& rcSizingMargins, COLORREF& clrTransparent)
{
	...
	rcDest = bVertical ? CRect(0, 0, nBorderHeight, XTP_DPI_Y(rcSrc.Height())) : CRect(0, 0, XTP_DPI_X(sz.cx), nBorderHeight);
...
CXTPSkinManagerClass* CXTPSkinManagerSchema::GetClassFromThemeHandle(HTHEME hTheme)
{
	CXTPSkinManagerClass* pClass = NULL;
	m_mapClassPtrs.Lookup(reinterpret_cast<void*>(hTheme), reinterpret_cast<void*&>(pClass));
	return pClass;
}
...
CXTPSkinManagerClass* CXTPSkinManagerSchema::GetClass(const CString& strClassList)
{
	...
	for(i = 0; !pClass && i < arClassList.GetSize(); i++)
	{
		...
		{
			int nClassCode = GetClassCode(strClassName);
			if (0 != nClassCode)
			{
				...
				XTP_GUARD_EXCLUSIVE(CXTPSkinManagerSchema, this)
				{
					...
					m_mapClassPtrs.SetAt(pClass, pClass);
				}
			}
			else
			{
