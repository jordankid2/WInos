...
_XTP_EXT_CLASS void AFX_CDECL XTPSkinFrameworkDrawFrame(HDC hdc, LPRECT lprc, int xSize, int ySize, COLORREF clr);
_XTP_EXT_CLASS void AFX_CDECL XTPSkinFrameworkDrawFrame(HDC hdc, LPRECT lprc, int nSize, HBRUSH hBrush);
_XTP_EXT_CLASS int AFX_CDECL XTPSkinFrameworkDrawText(HDC hdc, LPCTSTR lpText, int cch, 
	LPRECT lprc, UINT nFormat, LPLOGFONT pAssociatedFont = NULL);

AFX_INLINE int AFX_CDECL XTPSkinFrameworkDrawText(HDC hdc, const CString& strText,
	LPRECT lprc, UINT nFormat, LPLOGFONT pAssociatedFont = NULL)
{
	return XTPSkinFrameworkDrawText(hdc, strText, strText.GetLength(), 
		lprc, nFormat, pAssociatedFont);
}
