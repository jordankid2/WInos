	...
	for (int nIndex = 0; nIndex < nCount; nIndex++ )
	{
		...
		if (hbm == HBMMENU_SYSTEM)
		{
			...
			if (hwndItem && IsWindow(hwndItem))
			{
				...
				if (hIcon)
				{
					CPoint ptTopLeft(rcItem.CenterPoint().x - XTP_DPI_X(8), rcItem.CenterPoint().y - XTP_DPI_Y(8));
					rcItem = CRect(ptTopLeft, XTP_DPI(CSize(16, 16)));
	...
	for (nItem = 0; nItem<nCount; nItem++ )
	{
		...
		if (!pItem->IsOwnerDraw() && !pItem->IsSeparator())
		{
			...
			if (hbm && HIWORD(hbm) != 0)
			{
				...
				m_nGripperWidth = max(m_nGripperWidth, bmpInfo.bmWidth + XTP_DPI_X(4) + (IsMenuThemed() ? XTP_DPI_X(5) + XTP_DPI_X(3) : 0));
			}
		}
...
void CXTPSkinPopupMenu::OnPaint()
{
	...
	if (m_scrollInfo.bScroll)
	{
		...
		{
			XTPPrimitiveDrawer()->DrawSymbol(&dc, xtpPrimitiveSymbolPlayUp,
				m_scrollInfo.btnUp.m_rc, pMetrics->GetColor(COLOR_MENUTEXT));
		...
		if (!m_scrollInfo.btnDown.m_rc.IsRectEmpty())
		{
			XTPPrimitiveDrawer()->DrawSymbol(&dc, xtpPrimitiveSymbolPlayDown,
				m_scrollInfo.btnDown.m_rc, pMetrics->GetColor(COLOR_MENUTEXT));
	...
	for (int nIndex=m_scrollInfo.nScrollFirst; nIndex < nCount; nIndex++)
	{
		...
		{
			dc.FillSolidRect(rcItem.left - XTP_DPI_X(3), rcClient.top + XTP_DPI_Y(3),
				XTP_DPI_X(1), rcClient.Height() - XTP_DPI_Y(6), pMetrics->GetColor(COLOR_3DSHADOW));
			dc.FillSolidRect(rcItem.left - XTP_DPI_X(2), rcClient.top + XTP_DPI_Y(3),
				XTP_DPI_X(1), rcClient.Height() - XTP_DPI_Y(6), pMetrics->GetColor(COLOR_3DHIGHLIGHT));
		}
		...
		if (pItem->IsOwnerDraw())
		{
			...
			{
				COLORREF clr = pMetrics->GetColor(!bEnabled ? COLOR_GRAYTEXT : bSelected ? COLOR_HIGHLIGHTTEXT : COLOR_MENUTEXT);
		...
		if (bSeparator)
		{
			if (!bTheme)
			{
				dc.FillSolidRect(rcItem.left + XTP_DPI_X(1), rcItem.CenterPoint().y, 
					rcItem.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), pMetrics->GetColor(COLOR_3DSHADOW));
				dc.FillSolidRect(rcItem.left + XTP_DPI_X(1), rcItem.CenterPoint().y + XTP_DPI_Y(1),
					rcItem.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), pMetrics->GetColor(COLOR_3DHIGHLIGHT));
			}
			else
			{
		...
		if (nFind != -1)
		{
			...
			XTPSkinFrameworkDrawText(dc, strShortcutText, rcShortcut, DT_VCENTER | DT_RIGHT | DT_SINGLELINE);
		}
		...
		if (pItem->GetPopupMenu())
		{
			if (bTheme)
			{
				CRect rcPopup(rcText.right - XTP_DPI_X(16), rcText.top, 
					rcText.right - XTP_DPI_X(7), rcText.bottom);
			...
			else
			{
				...
				rcGlyph.left = rcGlyph.right - rcGlyph.Height();
				XTPPrimitiveDrawer()->DrawSymbol(&dc, xtpPrimitiveSymbolPlayRight, rcGlyph, dc.GetTextColor());
			}
		}
		...
		else if (bChecked)
		{
			...
			else
			{
				...
				XTPPrimitiveDrawer()->DrawSymbol(&dc, xtpPrimitiveSymbolCheckMark, rcGlyph, dc.GetTextColor());
			}
		}
		else
		{
			...
			if (nSysBitmap != 0)
			{
				if (bTheme)
				{
					...
					CRect rcBitmap(CPoint(XTP_DPI_X(bTheme ? 16 : 11) -  sz.cx / 2 , (rcItem.top + rcItem.bottom - sz.cy) / 2), sz);
				...
				else
				{
					CRect rcGlyph = rcItem;
					rcGlyph.right = rcGlyph.left + rcGlyph.Height();
					XTPPrimitiveDrawer()->DrawSymbol(&dc, nSysBitmap == XTP_MP_SYSTEMMINIMIZE ? xtpPrimitiveSymbolMinimize :
						nSysBitmap == XTP_MP_SYSTEMRESTORE ? xtpPrimitiveSymbolRestore :
						nSysBitmap == XTP_MP_SYSTEMMAXIMIZE ? xtpPrimitiveSymbolMaximize : xtpPrimitiveSymbolClose,
			...
			else if (hbm)
			{
				...
				int nCenter = XTP_DPI_X(3) + (m_nGripperWidth - (bTheme ? XTP_DPI_X(3) + XTP_DPI_X(5) : 0)) / 2;

				if (bmpInfo.bmBitsPixel == 32 && bmpInfo.bmBits && XTPSystemVersion()->IsWinVistaOrGreater())
				{
					CXTPImageManagerIcon::DrawAlphaBitmap(&dc, hbm, 
						CPoint(nCenter -  bmpInfo.bmWidth / 2 , (rcItem.top + rcItem.bottom - bmpInfo.bmHeight) / 2),
...
void CXTPSkinPopupMenu::ShowScrollDown(BOOL bShow)
{
	if (bShow && m_scrollInfo.btnDown.m_rc.IsRectEmpty())
	{
		...
		m_scrollInfo.btnDown.m_rc.top    = m_nHeight - m_rcBorder.bottom - XTP_DPI_Y(XTP_SKINPOPUPMENU_SCROLL);
