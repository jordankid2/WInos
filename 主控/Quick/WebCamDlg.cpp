...
void CWebCamDlg::OnReceive()
{
	if (m_pContext == NULL)
		return;
	CString str;
	str.Format(_T("\\\\%s [%d * %d] -远程视频  [收包:%d 收:%d KB] [发包:%d 发:%d KB]"), m_IPAddress, m_lpbmi->bmiHeader.biWidth, m_lpbmi->bmiHeader.biHeight, m_nCount, m_pContext->m_allpack_rev, int(m_pContext->m_alldata_rev / 1024), m_pContext->m_allpack_send, int(m_pContext->m_alldata_send / 1024));
	SetWindowText(str);
}
...
BOOL CWebCamDlg::OnInitDialog()
{
	...
	if (pSysMenu != NULL)
	{
		pSysMenu->AppendMenu(MF_STRING, IDM_SAVEDIB, _T("保存截图(&P)"));
		pSysMenu->AppendMenu(MF_STRING, IDM_SAVEAVI, _T("保存视频—必须安装Xvid录制视频解码器(&A)"));
		pSysMenu->AppendMenu(MF_STRING, IDM_SAVEAVI_MAKE, _T("保存视频(&B)"));
...
void CWebCamDlg::ResetScreen()
{
	...
	if (IsWindowVisible())
		InitMMI();
	CString str;
	str.Format(_T("\\\\%s 【%d * %d】 -远程视频"), m_IPAddress, m_lpbmi->bmiHeader.biWidth, m_lpbmi->bmiHeader.biHeight);
	SetWindowText(str);
	m_bReset = TRUE;
}
