...
void osIPC::Server::create(const char *addr)
{
	...
	if (!m_sEvtAvail) { TRACE("server_create: failed: %01d\n", __LINE__); return; }
	...
	if (!m_sEvtFilled) { free(m_sEvtAvail); TRACE("server_create: failed: %01d\n", __LINE__); return; }
	...
	{ 
		free(m_sEvtAvail); 
		free(m_sEvtFilled); 
		TRACE("server_create: failed: %01d\n", __LINE__); 
		return; 
	}
	...
	if (!sEvtFilled)
	{
		...
		free(m_sEvtFilled); 
		free(m_sMemName);
		TRACE("client_create: failed: %01d\n", __LINE__); 
		return; 
	}
	...
	m_hMapFile = CreateFileMapping(	INVALID_HANDLE_VALUE,
									...
									sizeof(MemBuff),
									memName);
	if (m_hMapFile == NULL || m_hMapFile == INVALID_HANDLE_VALUE)  { free(m_sEvtAvail); free(m_sEvtFilled); free(m_sMemName); TRACE("server_create: failed: %01d\n", __LINE__); return; }
...
{
	TRACE("m_ReadStart  is : %lu\n", m_pBuf->m_ReadStart);
	TRACE("m_ReadEnd    is : %lu\n", m_pBuf->m_ReadEnd);
	TRACE("m_WriteStart is : %lu\n", m_pBuf->m_WriteStart);
	TRACE("m_WriteEnd   is : %lu\n", m_pBuf->m_WriteEnd);
	TRACE("nTryGetBlock      is : %lu\n", m_stat.nTryGetBlock);
	TRACE("nGetBlock         is : %lu\n", m_stat.nGetBlock);
	TRACE("nRetBlock         is : %lu\n", m_stat.nRetBlock);
	TRACE("nWait             is : %lu\n", m_stat.nWait);
	TRACE("nWokeUpSuccess    is : %lu\n", m_stat.nWokeUpSuccess);
	TRACE("nReleaseSemaphore is : %lu\n", m_stat.nReleaseSemaphore);
}
...
osIPC::Client::Client(const char *connectAddr)
{
	...
	{ 
		...
		free(m_sEvtFilled); 
		free(m_sMemName); 
		TRACE("client open file mapping: failed: %01d\n", __LINE__); 
		return; 
	}
	...
	{ 
		...
		free(m_sEvtFilled); 
		free(m_sMemName); 
		TRACE("client map view: failed: %01d\n", __LINE__); 
		return; 
	}
...
void osIPC::Client::write(char* str, int mode)
{
	
	logInfo mlogInfo;
	mlogInfo.mode = mode+4;
	memcpy(mlogInfo.text, str, strlen(str)+1);
