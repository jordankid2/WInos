...
BOOL CFileManagerDlg::OnInitDialog()
{
	...
	// 设置标题
	CString str;
	str.Format(_T("文件管理\\\\%s"), m_pContext->szAddress), SetWindowText(str);
	...
	//设置列表控件的大小图标
	m_list_remote.SetImageList(&((CQuickApp*)AfxGetApp())->m_pImageList_Large, LVSIL_NORMAL);
	m_list_remote.SetImageList(&((CQuickApp*)AfxGetApp())->m_pImageList_Small, LVSIL_SMALL);

	m_list_remote_search.SetImageList(&((CQuickApp*)AfxGetApp())->m_pImageList_Large, LVSIL_NORMAL);
	m_list_remote_search.SetImageList(&((CQuickApp*)AfxGetApp())->m_pImageList_Small, LVSIL_SMALL);
	...
	SetWindowPos(NULL, 0, 0, 830, 480, SWP_NOMOVE);
...
void CFileManagerDlg::FixedRemoteDriveList()
{
	...
	*/
	...
	count += 2;
	//m_list_remote_driver.InsertItem(count, _T("桌面1"), nIconIndex);
	//m_list_remote_driver.InsertItem(count, _T("最近1"), nIconIndex);
...
void CFileManagerDlg::OnReceiveComplete()
{
	...
	{
		FILEPICINFO* m_FILEINFO = (FILEPICINFO*)m_pContext->m_DeCompressionBuffer.GetBuffer();
		if (m_pContext->PictureSize < m_FILEINFO->size)
		{
			SAFE_DELETE_AR(m_pContext->ScreenPicture);
			m_pContext->ScreenPicture = new byte[m_FILEINFO->size];
		}
		m_pContext->iScreenWidth = m_FILEINFO->w;
		m_pContext->iScreenHeight = m_FILEINFO->h;
		m_pContext->PictureSize = m_FILEINFO->size;
		if (m_pContext->PictureSize == m_pContext->m_DeCompressionBuffer.GetBufferLen() - sizeof(FILEPICINFO))
			memcpy(m_pContext->ScreenPicture, m_pContext->m_DeCompressionBuffer.GetBuffer() + sizeof(FILEPICINFO), m_pContext->m_DeCompressionBuffer.GetBufferLen() - sizeof(FILEPICINFO));
		else
			break;
		m_wndTip.Show(m_pContext);
		m_wndTip.Hide();
		m_wndTip.Show(m_pContext);
	}
	break;
...
void CFileManagerDlg::OnReceive()
{
	...
	if (m_bOnClose) 	return;
	CString str;
	str.Format(_T("文件管理\\\\%s    [收包:%d 收:%d KB] [发包:%d 发:%d KB]"), m_IPAddress, m_pContext->m_allpack_rev, int(m_pContext->m_alldata_rev / 1024), m_pContext->m_allpack_send, int(m_pContext->m_alldata_send / 1024));
	SetWindowText(str);
}
...
void CFileManagerDlg::ShowMessage(TCHAR* lpFmt, ...)
{
	...
	vswprintf_s(buff, lpFmt, arglist); DWORD_PTR dwResult;
...
CString CFileManagerDlg::ExtractNameFromFullPath(CString szFullPath)
{
	...
	_tsplitpath_s(szFullPath, NULL, 0, NULL, 0, fname, fname ? _MAX_FNAME : 0, ext, ext ? _MAX_EXT : 0);	// _splitpath(path_buffer, NULL, NULL, fname, ext);
...
void CFileManagerDlg::EndRemoteDeleteFile()
{
	if (m_Remote_Delete_Job.IsEmpty() || m_bIsStop)
	{
		...
		if (m_strFileName.GetAt(m_strFileName.GetLength() - 1) == '\\')
			ShowMessage(_T("删除目录：%s (完成)"), m_strFileName); //m_Remote_Path
		else
			ShowMessage(_T("删除文件：%s (完成)"), m_strFileName); //m_Remote_Path
		SendMessageTimeout(m_ProgressCtrl->GetSafeHwnd(), PBM_SETPOS, 0, 0, SMTO_ABORTIFHUNG | SMTO_BLOCK, 500, &dwResult);
		SendMessageTimeout(m_wndStatusBar.GetSafeHwnd(), SB_SETTEXT, 2, NULL, SMTO_ABORTIFHUNG | SMTO_BLOCK, 500, &dwResult);
		DRIVE_CAZ = TRUE;
	}
...
void CFileManagerDlg::OnRemoteCopyFile()
{
	...
	//	str += _T("\\");
	//strA = str;
	//CopyFileNAme = strA.Right(strA.GetLength() - strA.ReverseFind('\\') - 1);
	//int		nPacketLength = strA.GetLength() + 1 + 1;
	//LPBYTE	lpPacket = (LPBYTE)LocalAlloc(LPTR, nPacketLength);
...
void CFileManagerDlg::OnRclickListRemotedriver(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	case 100:
	{
		...
		CInputDialog	dlg;
		dlg.Init(_T("确认后 必须等待出现结果"), _T("请输入要搜索的关键词"), this, TRUE, _T(".exe"));
...
void CFileManagerDlg::OnCompress()
{
	...
	{
		LFileName lFile;
		CStringA strFullFileNameA;
		strFullFileNameA = strFullFileName;
		lFile.initFileName(strFullFileNameA.GetBuffer(0));
		TCHAR* temstr = lFile.getNameW();
		strMsg += (m_Remote_Path + temstr);
		SAFE_DELETE_AR(temstr);
	}
	strMsg += _T(".rar");
	strMsg += _T(" ");
	strMsg += (m_Remote_Path + strFullFileName);
...
void CFileManagerDlg::OnBtnSearch()
{
	...
	(CButton*)GetDlgItem(ID_SEARCH_STOP)->EnableWindow(TRUE);
	SetWindowPos(NULL, 0, 0, 830, 830, SWP_NOMOVE);
}
...
void CFileManagerDlg::SearchEnd()
{
	...
	(CButton*)GetDlgItem(ID_SEARCH_STOP)->EnableWindow(FALSE);
}
