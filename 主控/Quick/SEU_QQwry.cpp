...
void SEU_QQwry::SaveToFile(CString Name)
{
	...
	hFile = CreateFile(Name+ _T(".txt"), GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
	...
	for(m_i=0;m_i<GetRecordCount();m_i++)
	{
		...
		str1.Format(_T("%d.%d.%d.%d"),m_ipoff.b3,m_ipoff.b2,m_ipoff.b1,m_ipoff.b0);//开始IP
		str2.Format(_T("%d.%d.%d.%d"),m_ei.b3,m_ei.b2,m_ei.b1,m_ei.b0);//结束IP
		m_buf.Format(_T("%-15s %-15s %s\r\n"),
