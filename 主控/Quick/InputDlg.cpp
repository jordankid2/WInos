...
WORD* CDlgTemplateBuilder::AddText(WORD* buf, LPCTSTR text)
{
	if (text) {
		USES_CONVERSION;
		_tcscpy_s((WCHAR*)buf,(lstrlen(text)+1)*sizeof(TCHAR), T2W((LPTSTR)text));
...
//
BOOL CInputDialog::Init(LPCTSTR caption, LPCTSTR prompt, CWnd* pParent, WORD nIDIcon, CString str)
{
