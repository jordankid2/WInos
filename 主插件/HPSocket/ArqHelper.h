...
#define DEFAULT_ARQ_NO_DELAY			TRUE
#define DEFAULT_ARQ_TURNOFF_NC			TRUE
#define DEFAULT_ARQ_FLUSH_INTERVAL		10
#define DEFAULT_ARQ_RESEND_BY_ACKS		2
...
#define DEFAULT_ARQ_MAX_MSG_SIZE		4096
...
public:
	CArqSessionT* Renew(T* pContext, S* pSocket, const TArqAttr& attr, DWORD dwPeerConvID = 0)
	{
		m_pContext		= pContext;
		m_pSocket		= pSocket;
		 DWORD s_dwConvID = ::timeGetTime();
		 m_dwSelfConvID = ::InterlockedIncrement(&s_dwConvID);
		if (m_dwSelfConvID == 0)
			m_dwSelfConvID = ::InterlockedIncrement(&s_dwConvID);
	...
	BOOL DoHandShake()
	{
...

		{
			...
			if(::GetTimeGap32(m_dwCreateTime, dwCurrent) > m_pContext->m_arqAttr.dwHandShakeTimeout)
			{
...

		{
			...
			if(IsReady())
			{
				...
				else if(cmd.peerID != m_dwSelfConvID)
				{
					...
					{
						if(::GetTimeGap32(m_dwCreateTime) > 2 * m_pContext->m_arqAttr.dwHandShakeTimeout)
							bReset = TRUE;
					}
				}
			...
			else
			{
				...
				if(cmd.peerID == m_dwSelfConvID)
				{
					...
					return m_pContext->OnHandShake(m_pSocket, m_pSocket->m_dwConnID);
				}
	...
	EnHandleResult ReceiveArq(const BYTE* pData, int iLength, BYTE* pBuffer, int iCapacity)
	{
...

		{
			...
			while(TRUE)
			{
				...
				if(iRead >= 0)
				{
					EnHandleResult result = m_pContext->OnReceive(m_pSocket, pBuffer, iRead);
...
template<class T, class S> const DWORD CArqSessionPoolT<T, S>::DEFAULT_SESSION_LOCK_TIME	= (30 * 1000);
template<class T, class S> const DWORD CArqSessionPoolT<T, S>::DEFAULT_SESSION_POOL_SIZE	= 600;
template<class T, class S> const DWORD CArqSessionPoolT<T, S>::DEFAULT_SESSION_POOL_HOLD	= 600;
