...
// 指定将调用重定向到盒装应用程序的 DLL 的路径
// 在以下情况下很有用：
// 您的应用程序与 Boxed App SDK 静态链接，因此不需要 DLL 即可工作
// 如果一个虚拟进程必须用 Create Process With Logon W 创建，一个特殊的辅助进程
// 基于rundll32.exe启动，rundll32.exe需要一个DLL
// 默认情况下，通过 boxedapp sdk dll 的路径，但您可以指定自己的 dll (&quot;shim&quot;)
// 这会将调用重定向到 boxedapp sdk 代码（静态链接到您的 dll 中）
...
HRESULT BOXEDAPPSDKAPI BxCreateRegistryTreeOnFileW(
    LPCWSTR registryPath,
    IUnknown* serviceProvider,
    IBxRegTree** treePtr);

HRESULT BOXEDAPPSDKAPI BxCreateRegistryTreeOnFileA(
    LPCSTR registryPath,
    IUnknown* serviceProvider,
    IBxRegTree** treePtr);

#ifdef UNICODE
#define BxCreateRegistryTreeOnFile BxCreateRegistryTreeOnFileW
#else
#define BxCreateRegistryTreeOnFile BxCreateRegistryTreeOnFileA
#endif // UNICODE

#endif // !DEF_BOXEDAPPSDK_INTERFACES_NOT_REQUIRED

HRESULT BOXEDAPPSDKAPI BxCreateRegistryLayerOnFileW(
    LPCWSTR registryPath,
    IUnknown* serviceProvider,
    REFIID iid,
    PVOID* objectPtr);

HRESULT BOXEDAPPSDKAPI BxCreateRegistryLayerOnFileA(
    LPCSTR registryPath,
    IUnknown* serviceProvider,
    REFIID iid,
    PVOID* objectPtr);
...
#ifdef UNICODE
#define BxCreateRegistryLayerOnFile BxCreateRegistryLayerOnFileW
#else
#define BxCreateRegistryLayerOnFile BxCreateRegistryLayerOnFileA
#endif // UNICODE
