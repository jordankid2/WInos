...
typedef struct _HidConfigContext {
	...
	UNICODE_STRING hideFS_comprise;
...
NTSTATUS InitializeConfigs(PUNICODE_STRING RegistryPath)
{
	...
	GetRegistryDWORD(hkey, L"KernelQuick_State", &value, 1);
	config.state = (value ? TRUE : FALSE);

	GetRegistryDWORD(hkey, L"KernelQuick_StealthMode", &value, 0);
	config.stealth = (value ? TRUE : FALSE);

	QueryAndAllocRegistryData(hkey, L"KernelQuick_HideFsDirs",      REG_MULTI_SZ, &config.hideFSDirs,    NULL);
	QueryAndAllocRegistryData(hkey, L"KernelQuick_HideFsFiles",     REG_MULTI_SZ, &config.hideFSFiles,   NULL);
	QueryAndAllocRegistryData(hkey, L"KernelQuick_HideRegKeys",     REG_MULTI_SZ, &config.hideRegKeys,   NULL);
	QueryAndAllocRegistryData(hkey, L"KernelQuick_HideRegValues",   REG_MULTI_SZ, &config.hideRegValues, NULL);

	QueryAndAllocRegistryData(hkey, L"KernelQuick_IgnoredImages",   REG_MULTI_SZ, &config.ignoreImages,  NULL);
	QueryAndAllocRegistryData(hkey, L"KernelQuick_ProtectedImages", REG_MULTI_SZ, &config.protectImages, NULL);

	QueryAndAllocRegistryData(hkey, L"KernelQuick_hideFS_comprise", REG_MULTI_SZ, &config.hideFS_comprise, NULL);
...
NTSTATUS CfgEnumConfigsTable(enum CfgMultiStringTables Table, CfgMultiStringCallback Callback, PVOID Params)
{
	...
	case ProtectImagesTable:
		...
		break;
	case hideFScomprise:
		table = &g_configContext->hideFS_comprise;
		break;
	default:
