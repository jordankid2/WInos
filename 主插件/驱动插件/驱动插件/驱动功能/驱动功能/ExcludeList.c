...
NTSTATUS AddExcludeListEntrycomprise(ExcludeContext Context, PUNICODE_STRING FilePath, UINT32 Type, PExcludeEntryId EntryId, ExcludeEntryId ParentId);
...
NTSTATUS AddExcludeListFilecomprise(ExcludeContext Context, PUNICODE_STRING FilePath, PExcludeEntryId EntryId, ExcludeEntryId ParentId)
{
	return AddExcludeListEntrycomprise(Context, FilePath, ExcludeFilecomprise, EntryId, ParentId);
}
...
NTSTATUS AddExcludeListEntrycomprise(ExcludeContext Context, PUNICODE_STRING FilePath, UINT32 Type, PExcludeEntryId EntryId, ExcludeEntryId ParentId)
{
	...
	entry->path.fullPath.Buffer=(PWCH)((PCHAR)entry + sizeof(EXCLUDE_FILE_LIST_ENTRY));;
	entry->path.fullPath.Length = 0;;
	entry->path.fullPath.MaximumLength = FilePath->Length;
	RtlCopyUnicodeString(&entry->path.fullPath, FilePath);
	/*if (!FillDirectoryFromPath(&entry->path, &temp))
	{
		ExFreePoolWithTag(entry, EXCLUDE_ALLOC_TAG);
		LogWarning("Warning, exclude file list is not NULL : %p", cntx);
...
BOOLEAN CheckExcludeListDirFilecomprise(ExcludeContext Context, PCUNICODE_STRING Dir, PCUNICODE_STRING File)
{
	PEXCLUDE_FILE_CONTEXT cntx = (PEXCLUDE_FILE_CONTEXT)Context;
	PEXCLUDE_FILE_LIST_ENTRY entry;
	UNICODE_STRING Directory;
	...
	while (entry != (PEXCLUDE_FILE_LIST_ENTRY)&cntx->listHead)
	{
		if (SpyFindSubString( &Directory, &entry->path.fullPath)  
			|| SpyFindSubString(File ,&entry->path.fullPath) )
		{
			result = TRUE;
			break;
		}
