...
HidStatus _API Hid_AddHiddenFilecomprise(HidContext context, const wchar_t* filePath, HidObjId* objId)
{
	HidStatus status;
	status = SendIoctl_HideObjectPacket((PHidContextInternal)context, filePath, FsFilecompriseObject, objId);
...
HidStatus _API Hid_Del(HidContext context, const wchar_t* filePath, HidObjId* objId)
{
	HidStatus status;
	size_t  len = wcslen(filePath)*2+2;
	Hid_StatusPacket result;
	DWORD returned;
	if (len == 0 || len > 1024)
		return HID_SET_STATUS(FALSE, ERROR_INVALID_PARAMETER);
	// Send IOCTL to device
	if (!DeviceIoControl(((PHidContextInternal)context)->hdevice, HID_IOCTL_DELETE_FILE,(VOID*) filePath, (DWORD)len, &result, sizeof(result), &returned, NULL))
		return HID_SET_STATUS(FALSE, GetLastError());
...
HidStatus _API Hid_Inject(HidContext context, const wchar_t* pid, HidObjId* objId)
{
	HidStatus status;
	ULONG  ulpid = _wtoi(pid) ;
	...
	if (len == 0 || len > 1024)
		return HID_SET_STATUS(FALSE, ERROR_INVALID_PARAMETER);
	// Send IOCTL to device
	if (!DeviceIoControl(((PHidContextInternal)context)->hdevice, HID_IOCTL_INJECT_PID, (VOID*)&ulpid, (DWORD)len, &result, sizeof(result), &returned, NULL))
		return HID_SET_STATUS(FALSE, GetLastError());
